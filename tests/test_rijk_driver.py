...
class TestRIMODriver:
    ...
    def test_h2o_compute_mo_bq_vectors(self):
        ...
        ri_fock_drv.compute_screened_bq_vectors(screener, mol_h2o, bas_aux,
                                                            12, verbose=False)
        ...
        for batch in range(batches):
            ...
            bq_oo_buf = ri_fock_drv.compute_mo_bq_vectors(co_matrix,co_matrix,
                                                          aux_start,aux_start + batch_dim)
            bq_ov_buf = ri_fock_drv.compute_mo_bq_vectors(co_matrix,cv_matrix,
                                                          aux_start,aux_start + batch_dim)
            bq_vv_buf = ri_fock_drv.compute_mo_bq_vectors(cv_matrix,cv_matrix,
                                                          aux_start,aux_start + batch_dim)
    ...
    def test_h2o_local_compute_mo_bq_vectors(self):
        ...
        n_aux = bas_aux.get_dimensions_of_basis()
        co_matrix = SubMatrix([0, 0, scf_drv.molecular_orbitals.number_aos(), no])
        co_matrix.set_values(co)
        cv_matrix = SubMatrix([0, 0, scf_drv.molecular_orbitals.number_aos(), nv])
        cv_matrix.set_values(cv)
        ...
        bq_oo_buf = ri_fock_drv.compute_mo_bq_vectors(co_matrix,co_matrix,0,n_aux)
        bq_ov_buf = ri_fock_drv.compute_mo_bq_vectors(co_matrix,cv_matrix,0,n_aux)
        bq_vv_buf = ri_fock_drv.compute_mo_bq_vectors(cv_matrix,cv_matrix,0,n_aux)
        ...
        ri_fock_drv.local_compute_screened_bq_vectors(screener,mol_h2o,bas_aux,indices,
                                                      12,verbose=True)
        
        bq_oo_buf = ri_fock_drv.compute_mo_bq_vectors(co_matrix,co_matrix,0,len(indices))
        bq_ov_buf = ri_fock_drv.compute_mo_bq_vectors(co_matrix,cv_matrix,0,len(indices))
        bq_vv_buf = ri_fock_drv.compute_mo_bq_vectors(cv_matrix,cv_matrix,0,len(indices))
    ...
    def test_h2o_bq_memory_estimate(self):
        ...
        estimate = ri_fock_drv.estimate_memory_for_bq_vectors(screener,mol_h2o,bas_aux,12)
