...
class TestBasisFunction:
    ...
    def test_normalize(self):
        ...
        # normalize() sorts primitives by decreasing exponent, so the expected
        # functions use exponents [2.7, 1.5, 0.8] with norms reordered to match.

        # testing normalization for s functions
        bf_a = self.basis_funtion(0)
        bf_a.normalize()
        bf_b = BasisFunction(
            [2.700000000000000, 1.50000000000000, 0.800000000000000],
            [-0.281296410762365, 0.253418226705534, 0.542250667462678], 0)
        assert bf_a == bf_b
        ...
        bf_b = BasisFunction(
            [2.700000000000000, 1.50000000000000, 0.800000000000000],
            [-0.913275834565931, 0.613252561079874, 0.958298883615608], 1)
        assert bf_a == bf_b
        ...
        bf_b = BasisFunction(
            [2.700000000000000, 1.50000000000000, 0.800000000000000],
            [-1.7171725369825064, 0.8594390563363751, 0.980790072864359], 2)
        assert bf_a == bf_b
        ...
        bf_a.normalize()
        bf_b = BasisFunction(
            [2.70000000000000, 1.50000000000000, 0.800000000000000],
            [-2.5070090082858605, 0.9352350910818505, 0.779437206859724], 3)
        assert bf_a == bf_b
        ...
        bf_a.normalize()
        bf_b = BasisFunction(
            [2.700000000000000, 1.50000000000000, 0.8000000000000000],
            [-3.099366655142966, 0.8617903028794913, 0.5245190989624026], 4)
        assert bf_a == bf_b
        ...
        bf_a.normalize()
        bf_b = BasisFunction(
            [2.700000000000000, 1.5000000000000000, 0.8000000000000000],
            [-3.3844565233104653, 0.7014251498674728, 0.3117743633857545], 5)
        assert bf_a == bf_b
        ...
        bf_b = BasisFunction(
            [2.700000000000000, 1.5000000000000000, 0.8000000000000000],
            [-3.3470959216753484, 0.5170401872124587, 0.1678350767963739], 6)
        assert bf_a == bf_b
