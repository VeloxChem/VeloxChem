...
from test_evb_helper import (evb_assert_energy_finite, evb_chdir_tmp,
                             evb_compare_serialized_systems, evb_data_dir,
                             evb_ff_pair)
...
class TestSystemBuilderReference:
    ...
    def test_vacuum_water_implicit(self):
        ...
        lambdas = [0, 0.4, 1]
        vac_systems, _, _ = _build(evb_ff_pair(), vac_conf, lambdas)
        wat_systems, _, _ = _build(evb_ff_pair(), wat_conf, lambdas)
        impl_systems, _, _ = _build(evb_ff_pair(), impl_conf, lambdas)

        for lam, tag in [(0, "0.000"), (0.4, "0.400"), (1, "1.000")]:
            evb_compare_serialized_systems(
...
class TestSystemBuilderBranches:
    ...
    @pytest.mark.parametrize("name", list(_branch_configs().keys()))
    def test_branch_energy_finite(self, name):
        config, constraints = _branch_configs()[name]
        lambdas = [0, 0.5, 1]
        systems, topology, positions = _build(evb_ff_pair(), config, lambdas,
                                              constraints)
        for lam in lambdas:
            evb_assert_energy_finite(systems[lam],
                                     positions,
...
class TestPdbInput:
    ...
    def test_petase_pdb_vacuum(self):
        ...
        with evb_chdir_tmp():
            systems, topology, positions = _build(ff_pair, config, lambdas)
