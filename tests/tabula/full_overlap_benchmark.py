"""Full two-center overlap benchmark — Tabula vs VeloxChem.

For each (molecule, basis) it reports, best-of-5:
  - Tabula dense `compute` wall time,
  - VeloxChem `OverlapDriver` wall time,
  - Tabula `compute_sparse` wall time and its stored footprint (vs dense),
  - the maximum element deviation of Tabula's dense overlap from VeloxChem's
...
for molecule_name, bases in PLAN:
    ...
    for basis_label in bases:
        try:
            ...
            tabula_time = best_time(lambda: TabulaOverlapDriver().compute(molecule, basis))
            veloxchem_time = best_time(lambda: OverlapDriver().compute(molecule, basis))
            sparse_time = best_time(lambda: TabulaOverlapDriver().compute_sparse(molecule, basis, SCREEN_THRESHOLD))

            sparse = TabulaOverlapDriver().compute_sparse(molecule, basis, SCREEN_THRESHOLD)
            ...
            if 2.0 * dimension * dimension * 8.0 < VALIDATE_BYTE_BUDGET:
                tabula_matrix = TabulaOverlapDriver().compute(molecule, basis)
                tabula_values = tabula_matrix.to_numpy()
                veloxchem_full = OverlapDriver().compute(molecule, basis).full_matrix().to_numpy()
