"""Full two-center coulomb benchmark — Tabula vs VeloxChem.

For each (molecule, basis) it reports, best-of-5:
  - Tabula dense `compute` wall time,
  - VeloxChem `TwoCenterElectronRepulsionDriver` wall time,
  - Tabula `compute_sparse` wall time and its stored footprint (vs dense),
  - the maximum element deviation of Tabula's dense coulomb from VeloxChem's
...
for molecule_name, bases in PLAN:
    ...
    for basis_label in bases:
        try:
            ...
            tabula_time = best_time(lambda: TabulaCoulombDriver().compute(molecule, basis))
            veloxchem_time = best_time(lambda: TwoCenterElectronRepulsionDriver().compute(molecule, basis))
            sparse_time = best_time(lambda: TabulaCoulombDriver().compute_sparse(molecule, basis, SCREEN_THRESHOLD))

            sparse = TabulaCoulombDriver().compute_sparse(molecule, basis, SCREEN_THRESHOLD)
            ...
            if 2.0 * dimension * dimension * 8.0 < VALIDATE_BYTE_BUDGET:
                tabula_matrix = TabulaCoulombDriver().compute(molecule, basis)
                tabula_values = tabula_matrix.to_numpy()
                veloxchem_full = TwoCenterElectronRepulsionDriver().compute(molecule, basis).full_matrix().to_numpy()
