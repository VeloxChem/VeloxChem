...
class TestTabulaCoulombSparse:
    """Tests for CoulombDriver.compute_sparse — the block-sparse Coulomb
    ...
    def _setup(self, xyz, basis_label):
        ...
        dense = TabulaCoulombDriver().compute(mol, bas).to_numpy()
    ...
    def test_unscreened_matches_dense(self):
        ...
        # dense Coulomb matrix, validating the block-sparse scatter and
        # reconstruction
        ...
        sparse = TabulaCoulombDriver().compute_sparse(mol, bas, 0.0)
    ...
    def test_screened_within_threshold(self):
        ...
        for threshold in (1.0e-12, 1.0e-9):
            sparse = TabulaCoulombDriver().compute_sparse(mol, bas, threshold)
            reconstructed = sparse.to_dense().to_numpy()
