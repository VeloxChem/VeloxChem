...
class TestVisualization:
    ...
    def test_gen_cubes(self):
        ...
        here = Path(__file__).parent
        inpfile = str(here / 'data' / 'water.inp')
        ...
        dens_cube_fpath = here / 'data' / f'vlx_dens_{random_string}.cube'
        homo_cube_fpath = here / 'data' / f'vlx_homo_{random_string}.cube'
        ...
        vis_drv.compute(cubic_grid, task.molecule, task.ao_basis, mo_coefs,
                        task.molecule.number_of_alpha_occupied_orbitals(
                            task.ao_basis) - 1)
