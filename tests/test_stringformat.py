from veloxchem.veloxchemlib import fmt_t
...
class TestStringFormat:
    ...
    def test_format(self):
        ...
        assert format("Slate", 9, fmt_t.center) == "  Slate  "

        assert format("Slate", 9, fmt_t.left) == "Slate    "

        assert format("Slate", 9, fmt_t.right) == "    Slate"

        assert format("Slate", 2, fmt_t.center) == "Sl"

        assert format("Slate", 3, fmt_t.left) == "Sla"

        assert format("Slate", 4, fmt_t.right) == "Slat"

    def test_to_string_float(self):

        assert to_string(0.52917721067, 5, 12, fmt_t.center) == "   0.52918  "

        assert to_string(-0.52917721067, 5, 12, fmt_t.center) == "  -0.52918  "

        assert to_string(0.52917721067, 5, 12, fmt_t.left) == " 0.52918    "

        assert to_string(-0.52917721067, 5, 12, fmt_t.left) == "-0.52918    "

        assert to_string(0.52917721067, 5, 12, fmt_t.right) == "     0.52918"
    ...
    def test_to_string_int(self):
        ...
        assert to_string(124, 6, fmt_t.center) == "  124 "

        assert to_string(-124, 6, fmt_t.center) == " -124 "

        assert to_string(124, 6, fmt_t.left) == " 124  "

        assert to_string(-124, 6, fmt_t.left) == "-124  "

        assert to_string(124, 6, fmt_t.right) == "   124"
