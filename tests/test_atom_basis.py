...
class TestAtomBasis:
    ...
    def test_pickle(self):
        ...
        abas_a = self.get_hydrogen_svp()
        bobj = pickle.dumps(abas_a)
        abas_b = pickle.loads(bobj)
        assert abas_a == abas_b
    ...
    def test_get_basis_functions(self):
        ...
        assert bfs_all[0] == bf1s
        assert bfs_all[1] == bf2s
        assert bfs_all[2] == bf1p
    ...
    def test_mpi_bcast(self):
        ...
        abas_a = None
        if comm.Get_rank() == 0:
            abas_a = self.get_lithium_svp()
        abas_a = comm.bcast(abas_a)
        abas_b = self.get_lithium_svp()
        assert abas_a == abas_b
