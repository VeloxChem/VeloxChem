...
def evb_compare_dict(dict1, dict2, float_tol=1e-2):
    ...
    for key in dict1:
        ...
        if isinstance(val1, dict):
            evb_compare_dict(val1, val2, float_tol=float_tol)
...
def evb_compare_serialized_systems(system, path):
    ...
    ref_string = _evb_read_reference_xml(path)
    ...
    while (i + sys_i_offset + 1 < len(sys_lines)
        ...
        if (sys_line.startswith('<A x=') and ref_line.startswith('<A x=')
            ...
            cond = (_evb_round_numbers_in_line(sys_line,
                                               0) == _evb_round_numbers_in_line(
                                                   ref_line, 0))
        else:
            cond = (_evb_round_numbers_in_line(sys_line) ==
                    _evb_round_numbers_in_line(ref_line))
