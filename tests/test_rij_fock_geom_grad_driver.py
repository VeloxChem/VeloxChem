...
class TestRIJFockGeomGradDriver:
    ...
    def test_h2o_fock_2j_grad_h2_sto3g(self):
        ...
        for i in range(3):
            assert mt.isclose(ref_grad[i], grad[i], rel_tol=5.0e-5, abs_tol=5.0e-5)
    ...
    def test_h2o_fock_2j_grad_h2o_sto3g(self):
        ...
        for i in range(3):
            ...
            for j in range(3):
                assert mt.isclose(ref_grad[3 * i + j], grad[j], rel_tol=5.0e-5, abs_tol=5.0e-5)
    ...
    def test_h2o_fock_2j_grad_h2o_sto3g_screened_direct(self):
        ...
        # compute RI gradients for all atoms
        ri_grad_drv = RIFockGradDriver()
        g = ri_grad_drv.direct_compute(t4c_drv, bas_sto3g, bas_aux, mol_h2o, gv, den_mat, [0, 1, 2], 12)
