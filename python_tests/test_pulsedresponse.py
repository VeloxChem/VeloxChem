...
# ------------------------------------------------------------------------------------------------------------------------
amplitudes = np.array([ 1.59140758e-09-5.77836923e-11j,-3.67213109e-09+6.83655709e-09j,
...
frequencies = np.array([0.231, 0.238, 0.245, 0.252, 0.259, 0.266, 0.273, 0.28 , 0.287, 0.294, 0.301, 0.308,
                        0.315, 0.322, 0.329, 0.336, 0.343, 0.35 , 0.357, 0.364, 0.371, 0.378, 0.385, 0.392])

zero_padded_frequencies = np.array([0.   , 0.007, 0.014, 0.021, 0.028, 0.035, 0.042, 0.049, 0.056, 0.063, 0.07 , 0.077,
                                    0.084, 0.091, 0.098, 0.105, 0.112, 0.119, 0.126, 0.133, 0.14 , 0.147, 0.154, 0.161,
                                    0.168, 0.175, 0.182, 0.189, 0.196, 0.203, 0.21 , 0.217, 0.224, 0.231, 0.238, 0.245,
                                    0.252, 0.259, 0.266, 0.273, 0.28 , 0.287, 0.294, 0.301, 0.308, 0.315, 0.322, 0.329,
                                    0.336, 0.343, 0.35 , 0.357, 0.364, 0.371, 0.378, 0.385, 0.392])

xx = np.array([  0.+0.j, 0.+0.j, 0.+0.j, 0.+0.j, 0.+0.j, 0.+0.j, 0.+0.j, 0.+0.j, 0.+0.j, 0.+0.j,
                 0.+0.j, 0.+0.j, 0.+0.j, 0.+0.j, 0.+0.j, 0.+0.j, 0.+0.j, 0.+0.j, 0.+0.j, 0.+0.j,
                 0.+0.j, 0.+0.j, 0.+0.j, 0.+0.j, 0.+0.j, 0.+0.j, 0.+0.j, 0.+0.j, 0.+0.j, 0.+0.j,
                 0.+0.j, 0.+0.j, 0.+0.j  , -9.38450147 -0.15495907j,
               ...
               -23.39852404 -5.55077652j ,-32.37389248-17.93780658j,
                ...
                -2.19004003 -0.66663608j , -3.07350937 -0.51583667j,
                -3.7778817  -0.41984064j , -4.36546063 -0.35690017j,
                -4.87587013 -0.3157651j])
# ------------------------------------------------------------------------------------------------------------------------
...
class TestComplexResponse(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.results = pulsed_response.compute(task.molecule, task.ao_basis, scf_tensors)
    ...
    def test_filesave(self):
        ...
        with h5py.File('{}.h5'.format(self.h5fname), 'r') as hf:
            ...
            # Verify that the non-zero padded list of amplitudes and frequencies match
            self.assertTrue(len(hf.get('amplitudes')[()]) == len(hf.get('frequencies')[()]))
            ...
            for key in directions:
              try:
                # Match the length of the frequency list (zero padded or non zero padded )
                # and the polarization direction lists of amplitudes
                self.assertTrue(len(hf.get(primary_key)[()]) == len(hf.get(key)[()]))
              except:
            ...
            self.assertTrue(np.allclose(hf.get('zero_padded_frequencies')[()], zero_padded_frequencies))
            self.assertTrue(np.allclose(hf.get('xx')[()], xx))


    def test_pulsed_response(self):

        expected_keys = ['properties','solutions','kappas','pulse_settings','properties_zeropad']
        ...
        # Verify that the results stored are the same as expected
        self.assertTrue(np.allclose(self.results['pulse_settings']['freq_amplitude'],  amplitudes))
        self.assertTrue(np.allclose(self.results['pulse_settings']['frequencies'], frequencies))
