...
class TestTPA(unittest.TestCase):
    ...
    def run_tpa(self,inpfile):
        ...
        if task.mpi_rank == mpi_master():
            ...
            self.assertTrue(np.max(np.abs(1/15*T3[(w, -w, w)].imag - T3_ref.imag)) < 1.0e-4)

            self.assertTrue(np.max(np.abs(1/15*T4[(w, -w, w)].real - T4_ref.real)) < 1.0e-4)
            self.assertTrue(np.max(np.abs(1/15*T4[(w, -w, w)].imag - T4_ref.imag)) < 1.0e-4)

            self.assertTrue(np.max(np.abs(1/15*X2[(w, -w, w)].real - X2_ref.real)) < 1.0e-4)
            self.assertTrue(np.max(np.abs(1/15*X2[(w, -w, w)].imag - X2_ref.imag)) < 1.0e-4)

            self.assertTrue(np.max(np.abs(1/15*X3[(w, -w, w)].real - X3_ref.real)) < 1.0e-4)
            self.assertTrue(np.max(np.abs(1/15*X3[(w, -w, w)].imag - X3_ref.imag)) < 1.0e-4)

            self.assertTrue(np.max(np.abs(1/15*A2[(w, -w, w)].real - A2_ref.real)) < 1.0e-4)
            self.assertTrue(np.max(np.abs(1/15*A2[(w, -w, w)].imag - A2_ref.imag)) < 1.0e-4)

            self.assertTrue(np.max(np.abs(1/15*A3[(w, -w, w)].real - A3_ref.real)) < 1.0e-4)
            self.assertTrue(np.max(np.abs(1/15*A3[(w, -w, w)].imag - A3_ref.imag)) < 1.0e-4)
