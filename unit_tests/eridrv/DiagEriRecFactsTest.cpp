...
TEST_F(CDiagEriRecFactsTest, CompHostDistancesPQ)
{
    ...
    const BufferHostMY<double, 3> rpq_01(
        {
...
TEST_F(CDiagEriRecFactsTest, CompHostFactorRho)
{
    ...
    const auto frho_00 = BufferHostX<double>({2.00000000000000000000, 1.80000000000000000000, 2.10000000000000000000, 1.90000000000000000000}, 4);

    EXPECT_EQ(frho_00, frho);

    derirec::compHostFactorsRho(frho.data(), &s21pairs, 0, 4, 0, 1);

    const auto frho_01 = BufferHostX<double>({1.61194029850746268656, 1.40338983050847457627, 1.81621621621621621621, 1.61212121212121212121}, 4);

    EXPECT_EQ(frho_01, frho);

    derirec::compHostFactorsRho(frho.data(), &s21pairs, 0, 4, 1, 1);

    const auto frho_11 = BufferHostX<double>({1.35000000000000000000, 1.15000000000000000000, 1.60000000000000000000, 1.40000000000000000000}, 4);
...
TEST_F(CDiagEriRecFactsTest, CompHostFactorNorm)
{
    ...
    const auto fnorm_00 = BufferHostX<double>({0.19938331312932041708, 0.01729165758259379585, 0.00964583723283758917, 0.13759209114032758783}, 4);

    EXPECT_EQ(fnorm_00, fnorm);

    derirec::compHostFactorsNorm(fnorm.data(), &s21pairs, 0, 4, 0, 1);

    const auto fnorm_01 = BufferHostX<double>({0.45025968744901134950, 0.05045735105393916422, 0.02576024760043138316, 0.32099138041350482142}, 4);

    EXPECT_EQ(fnorm_01, fnorm);

    derirec::compHostFactorsNorm(fnorm.data(), &s21pairs, 0, 4, 1, 1);

    const auto fnorm_11 = BufferHostX<double>({0.25420104491165196955, 0.03680885223437551286, 0.01719887917495777140, 0.18721182563226586746}, 4);
...
TEST_F(CDiagEriRecFactsTest, CompHostBoysArguments)
{
    ...
    const auto fargs_01 = BufferHostX<double>({0.00000000000000000000, 0.03830279210677147299, 0.02504826254826254822, 0.00000000000000000000}, 4);
...
TEST_F(CDiagEriRecFactsTest, CompHostFactorZeta)
{
    ...
    const auto fzeta_00 = BufferHostX<double>({0.12500000000000000000, 0.13888888888888888888, 0.11904761904761904761, 0.13157894736842105263}, 4);

    EXPECT_EQ(fzeta_00, fzeta);

    derirec::compHostFactorsZeta(fzeta.data(), &s21pairs, 0, 4, 0, 1);

    const auto fzeta_01 = BufferHostX<double>({0.14925373134328358208, 0.16949152542372881355, 0.13513513513513513513, 0.15151515151515151515}, 4);

    EXPECT_EQ(fzeta_01, fzeta);

    derirec::compHostFactorsZeta(fzeta.data(), &s21pairs, 0, 4, 1, 1);

    const auto fzeta_11 = BufferHostX<double>({0.18518518518518518518, 0.21739130434782608695, 0.15625000000000000000, 0.17857142857142857142}, 4);
...
TEST_F(CDiagEriRecFactsTest, CompHostFactorPartialZeta)
{
    ...
    const auto fzeta_0 = BufferHostX<double>({0.25000000000000000000, 0.27777777777777777777, 0.23809523809523809523, 0.26315789473684210526}, 4);

    EXPECT_EQ(fzeta_0, fzeta);

    derirec::compHostFactorsPartialZeta(fzeta.data(), &s21pairs, 0, 4, 1);

    const auto fzeta_1 = BufferHostX<double>({0.37037037037037037037, 0.43478260869565217391, 0.31250000000000000000, 0.35714285714285714285}, 4);
...
TEST_F(CDiagEriRecFactsTest, CompHostCoordinatesW)
{
    ...
    const BufferHostMY<double, 3> rw_00(
        {
            0.00000000000000000000,
    ...
    const BufferHostMY<double, 3> rw_01(
        {
            0.00000000000000000000,
    ...
    const BufferHostMY<double, 3> rw_11({0.00000000000000000000,
...
TEST_F(CDiagEriRecFactsTest, CompHostDistancesWT)
{
    ...
    const BufferHostMY<double, 3> wt_010(
        {
            0.00000000000000000000,
    ...
    const BufferHostMY<double, 3> wt_011(
        {
            0.00000000000000000000,
...
TEST_F(CDiagEriRecFactsTest, CompHostDistancesPT)
{
    ...
    const BufferHostMY<double, 3> pt_0(
        {
            0.00000000000000000000,
    ...
    const BufferHostMY<double, 3> pt_1(
        {
            0.00000000000000000000,
...
TEST_F(CDiagEriRecFactsTest, SelectHostMaxValues)
{
    ...
    const BufferHostMY<double, 3> tints(
        {
            1.0,
            2.0,
            2.0,
    ...
    const BufferHostX<double> rints(
        {
            0.0,
            7.0,
            2.0,
