                     ...
                     const double c_exp) -> void
{
    ...
    #pragma omp simd aligned(ga_x, ts_x_xxx, ts_x_xxy, ts_x_xxz, ts_x_xyy, ts_x_xyz, ts_x_xzz, ts_x_yyy, ts_x_yyz, ts_x_yzz, ts_x_zzz, ts_xx_xx, ts_xx_xxx, ts_xx_xxy, ts_xx_xxz, ts_xx_xy, ts_xx_xyy, ts_xx_xyz, ts_xx_xz, ts_xx_xzz, ts_xx_yy, ts_xx_yyy, ts_xx_yyz, ts_xx_yz, ts_xx_yzz, ts_xx_zz, ts_xx_zzz, ts_xxx_xxx, ts_xxx_xxy, ts_xxx_xxz, ts_xxx_xyy, ts_xxx_xyz, ts_xxx_xzz, ts_xxx_yyy, ts_xxx_yyz, ts_xxx_yzz, ts_xxx_zzz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xxx_xxx[i] = 2.0 * ts_x_xxx[i] * gfe_0 + 3.0 * ts_xx_xx[i] * gfe_0 + ts_xx_xxx[i] * ga_x[i];

        ts_xxx_xxy[i] = 2.0 * ts_x_xxy[i] * gfe_0 + 2.0 * ts_xx_xy[i] * gfe_0 + ts_xx_xxy[i] * ga_x[i];

        ts_xxx_xxz[i] = 2.0 * ts_x_xxz[i] * gfe_0 + 2.0 * ts_xx_xz[i] * gfe_0 + ts_xx_xxz[i] * ga_x[i];

        ts_xxx_xyy[i] = 2.0 * ts_x_xyy[i] * gfe_0 + ts_xx_yy[i] * gfe_0 + ts_xx_xyy[i] * ga_x[i];

        ts_xxx_xyz[i] = 2.0 * ts_x_xyz[i] * gfe_0 + ts_xx_yz[i] * gfe_0 + ts_xx_xyz[i] * ga_x[i];

        ts_xxx_xzz[i] = 2.0 * ts_x_xzz[i] * gfe_0 + ts_xx_zz[i] * gfe_0 + ts_xx_xzz[i] * ga_x[i];

        ts_xxx_yyy[i] = 2.0 * ts_x_yyy[i] * gfe_0 + ts_xx_yyy[i] * ga_x[i];

        ts_xxx_yyz[i] = 2.0 * ts_x_yyz[i] * gfe_0 + ts_xx_yyz[i] * ga_x[i];

        ts_xxx_yzz[i] = 2.0 * ts_x_yzz[i] * gfe_0 + ts_xx_yzz[i] * ga_x[i];

        ts_xxx_zzz[i] = 2.0 * ts_x_zzz[i] * gfe_0 + ts_xx_zzz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ga_y, ts_xx_xx, ts_xx_xxx, ts_xx_xxy, ts_xx_xxz, ts_xx_xy, ts_xx_xyy, ts_xx_xyz, ts_xx_xz, ts_xx_xzz, ts_xx_zzz, ts_xxy_xxx, ts_xxy_xxy, ts_xxy_xxz, ts_xxy_xyy, ts_xxy_xyz, ts_xxy_xzz, ts_xxy_yyy, ts_xxy_yyz, ts_xxy_yzz, ts_xxy_zzz, ts_xy_yyy, ts_xy_yyz, ts_xy_yzz, ts_y_yyy, ts_y_yyz, ts_y_yzz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xxy_xxx[i] = ts_xx_xxx[i] * ga_y[i];

        ts_xxy_xxy[i] = ts_xx_xx[i] * gfe_0 + ts_xx_xxy[i] * ga_y[i];

        ts_xxy_xxz[i] = ts_xx_xxz[i] * ga_y[i];

        ts_xxy_xyy[i] = 2.0 * ts_xx_xy[i] * gfe_0 + ts_xx_xyy[i] * ga_y[i];

        ts_xxy_xyz[i] = ts_xx_xz[i] * gfe_0 + ts_xx_xyz[i] * ga_y[i];

        ts_xxy_xzz[i] = ts_xx_xzz[i] * ga_y[i];

        ts_xxy_yyy[i] = ts_y_yyy[i] * gfe_0 + ts_xy_yyy[i] * ga_x[i];

        ts_xxy_yyz[i] = ts_y_yyz[i] * gfe_0 + ts_xy_yyz[i] * ga_x[i];

        ts_xxy_yzz[i] = ts_y_yzz[i] * gfe_0 + ts_xy_yzz[i] * ga_x[i];

        ts_xxy_zzz[i] = ts_xx_zzz[i] * ga_y[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ga_z, ts_xx_xx, ts_xx_xxx, ts_xx_xxy, ts_xx_xxz, ts_xx_xy, ts_xx_xyy, ts_xx_xyz, ts_xx_xz, ts_xx_xzz, ts_xx_yyy, ts_xxz_xxx, ts_xxz_xxy, ts_xxz_xxz, ts_xxz_xyy, ts_xxz_xyz, ts_xxz_xzz, ts_xxz_yyy, ts_xxz_yyz, ts_xxz_yzz, ts_xxz_zzz, ts_xz_yyz, ts_xz_yzz, ts_xz_zzz, ts_z_yyz, ts_z_yzz, ts_z_zzz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xxz_xxx[i] = ts_xx_xxx[i] * ga_z[i];

        ts_xxz_xxy[i] = ts_xx_xxy[i] * ga_z[i];

        ts_xxz_xxz[i] = ts_xx_xx[i] * gfe_0 + ts_xx_xxz[i] * ga_z[i];

        ts_xxz_xyy[i] = ts_xx_xyy[i] * ga_z[i];

        ts_xxz_xyz[i] = ts_xx_xy[i] * gfe_0 + ts_xx_xyz[i] * ga_z[i];

        ts_xxz_xzz[i] = 2.0 * ts_xx_xz[i] * gfe_0 + ts_xx_xzz[i] * ga_z[i];

        ts_xxz_yyy[i] = ts_xx_yyy[i] * ga_z[i];

        ts_xxz_yyz[i] = ts_z_yyz[i] * gfe_0 + ts_xz_yyz[i] * ga_x[i];

        ts_xxz_yzz[i] = ts_z_yzz[i] * gfe_0 + ts_xz_yzz[i] * ga_x[i];

        ts_xxz_zzz[i] = ts_z_zzz[i] * gfe_0 + ts_xz_zzz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ts_xyy_xxx, ts_xyy_xxy, ts_xyy_xxz, ts_xyy_xyy, ts_xyy_xyz, ts_xyy_xzz, ts_xyy_yyy, ts_xyy_yyz, ts_xyy_yzz, ts_xyy_zzz, ts_yy_xx, ts_yy_xxx, ts_yy_xxy, ts_yy_xxz, ts_yy_xy, ts_yy_xyy, ts_yy_xyz, ts_yy_xz, ts_yy_xzz, ts_yy_yy, ts_yy_yyy, ts_yy_yyz, ts_yy_yz, ts_yy_yzz, ts_yy_zz, ts_yy_zzz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xyy_xxx[i] = 3.0 * ts_yy_xx[i] * gfe_0 + ts_yy_xxx[i] * ga_x[i];

        ts_xyy_xxy[i] = 2.0 * ts_yy_xy[i] * gfe_0 + ts_yy_xxy[i] * ga_x[i];

        ts_xyy_xxz[i] = 2.0 * ts_yy_xz[i] * gfe_0 + ts_yy_xxz[i] * ga_x[i];

        ts_xyy_xyy[i] = ts_yy_yy[i] * gfe_0 + ts_yy_xyy[i] * ga_x[i];

        ts_xyy_xyz[i] = ts_yy_yz[i] * gfe_0 + ts_yy_xyz[i] * ga_x[i];

        ts_xyy_xzz[i] = ts_yy_zz[i] * gfe_0 + ts_yy_xzz[i] * ga_x[i];

        ts_xyy_yyy[i] = ts_yy_yyy[i] * ga_x[i];

        ts_xyy_yyz[i] = ts_yy_yyz[i] * ga_x[i];

        ts_xyy_yzz[i] = ts_yy_yzz[i] * ga_x[i];

        ts_xyy_zzz[i] = ts_yy_zzz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ga_y, ga_z, ts_xy_xxy, ts_xy_xyy, ts_xyz_xxx, ts_xyz_xxy, ts_xyz_xxz, ts_xyz_xyy, ts_xyz_xyz, ts_xyz_xzz, ts_xyz_yyy, ts_xyz_yyz, ts_xyz_yzz, ts_xyz_zzz, ts_xz_xxx, ts_xz_xxz, ts_xz_xzz, ts_yz_xyz, ts_yz_yyy, ts_yz_yyz, ts_yz_yz, ts_yz_yzz, ts_yz_zzz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xyz_xxx[i] = ts_xz_xxx[i] * ga_y[i];

        ts_xyz_xxy[i] = ts_xy_xxy[i] * ga_z[i];

        ts_xyz_xxz[i] = ts_xz_xxz[i] * ga_y[i];

        ts_xyz_xyy[i] = ts_xy_xyy[i] * ga_z[i];

        ts_xyz_xyz[i] = ts_yz_yz[i] * gfe_0 + ts_yz_xyz[i] * ga_x[i];

        ts_xyz_xzz[i] = ts_xz_xzz[i] * ga_y[i];

        ts_xyz_yyy[i] = ts_yz_yyy[i] * ga_x[i];

        ts_xyz_yyz[i] = ts_yz_yyz[i] * ga_x[i];

        ts_xyz_yzz[i] = ts_yz_yzz[i] * ga_x[i];

        ts_xyz_zzz[i] = ts_yz_zzz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ts_xzz_xxx, ts_xzz_xxy, ts_xzz_xxz, ts_xzz_xyy, ts_xzz_xyz, ts_xzz_xzz, ts_xzz_yyy, ts_xzz_yyz, ts_xzz_yzz, ts_xzz_zzz, ts_zz_xx, ts_zz_xxx, ts_zz_xxy, ts_zz_xxz, ts_zz_xy, ts_zz_xyy, ts_zz_xyz, ts_zz_xz, ts_zz_xzz, ts_zz_yy, ts_zz_yyy, ts_zz_yyz, ts_zz_yz, ts_zz_yzz, ts_zz_zz, ts_zz_zzz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xzz_xxx[i] = 3.0 * ts_zz_xx[i] * gfe_0 + ts_zz_xxx[i] * ga_x[i];

        ts_xzz_xxy[i] = 2.0 * ts_zz_xy[i] * gfe_0 + ts_zz_xxy[i] * ga_x[i];

        ts_xzz_xxz[i] = 2.0 * ts_zz_xz[i] * gfe_0 + ts_zz_xxz[i] * ga_x[i];

        ts_xzz_xyy[i] = ts_zz_yy[i] * gfe_0 + ts_zz_xyy[i] * ga_x[i];

        ts_xzz_xyz[i] = ts_zz_yz[i] * gfe_0 + ts_zz_xyz[i] * ga_x[i];

        ts_xzz_xzz[i] = ts_zz_zz[i] * gfe_0 + ts_zz_xzz[i] * ga_x[i];

        ts_xzz_yyy[i] = ts_zz_yyy[i] * ga_x[i];

        ts_xzz_yyz[i] = ts_zz_yyz[i] * ga_x[i];

        ts_xzz_yzz[i] = ts_zz_yzz[i] * ga_x[i];

        ts_xzz_zzz[i] = ts_zz_zzz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_y, ts_y_xxx, ts_y_xxy, ts_y_xxz, ts_y_xyy, ts_y_xyz, ts_y_xzz, ts_y_yyy, ts_y_yyz, ts_y_yzz, ts_y_zzz, ts_yy_xx, ts_yy_xxx, ts_yy_xxy, ts_yy_xxz, ts_yy_xy, ts_yy_xyy, ts_yy_xyz, ts_yy_xz, ts_yy_xzz, ts_yy_yy, ts_yy_yyy, ts_yy_yyz, ts_yy_yz, ts_yy_yzz, ts_yy_zz, ts_yy_zzz, ts_yyy_xxx, ts_yyy_xxy, ts_yyy_xxz, ts_yyy_xyy, ts_yyy_xyz, ts_yyy_xzz, ts_yyy_yyy, ts_yyy_yyz, ts_yyy_yzz, ts_yyy_zzz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_yyy_xxx[i] = 2.0 * ts_y_xxx[i] * gfe_0 + ts_yy_xxx[i] * ga_y[i];

        ts_yyy_xxy[i] = 2.0 * ts_y_xxy[i] * gfe_0 + ts_yy_xx[i] * gfe_0 + ts_yy_xxy[i] * ga_y[i];

        ts_yyy_xxz[i] = 2.0 * ts_y_xxz[i] * gfe_0 + ts_yy_xxz[i] * ga_y[i];

        ts_yyy_xyy[i] = 2.0 * ts_y_xyy[i] * gfe_0 + 2.0 * ts_yy_xy[i] * gfe_0 + ts_yy_xyy[i] * ga_y[i];

        ts_yyy_xyz[i] = 2.0 * ts_y_xyz[i] * gfe_0 + ts_yy_xz[i] * gfe_0 + ts_yy_xyz[i] * ga_y[i];

        ts_yyy_xzz[i] = 2.0 * ts_y_xzz[i] * gfe_0 + ts_yy_xzz[i] * ga_y[i];

        ts_yyy_yyy[i] = 2.0 * ts_y_yyy[i] * gfe_0 + 3.0 * ts_yy_yy[i] * gfe_0 + ts_yy_yyy[i] * ga_y[i];

        ts_yyy_yyz[i] = 2.0 * ts_y_yyz[i] * gfe_0 + 2.0 * ts_yy_yz[i] * gfe_0 + ts_yy_yyz[i] * ga_y[i];

        ts_yyy_yzz[i] = 2.0 * ts_y_yzz[i] * gfe_0 + ts_yy_zz[i] * gfe_0 + ts_yy_yzz[i] * ga_y[i];

        ts_yyy_zzz[i] = 2.0 * ts_y_zzz[i] * gfe_0 + ts_yy_zzz[i] * ga_y[i];
    }
    ...
    #pragma omp simd aligned(ga_y, ga_z, ts_yy_xxx, ts_yy_xxy, ts_yy_xy, ts_yy_xyy, ts_yy_xyz, ts_yy_yy, ts_yy_yyy, ts_yy_yyz, ts_yy_yz, ts_yy_yzz, ts_yyz_xxx, ts_yyz_xxy, ts_yyz_xxz, ts_yyz_xyy, ts_yyz_xyz, ts_yyz_xzz, ts_yyz_yyy, ts_yyz_yyz, ts_yyz_yzz, ts_yyz_zzz, ts_yz_xxz, ts_yz_xzz, ts_yz_zzz, ts_z_xxz, ts_z_xzz, ts_z_zzz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_yyz_xxx[i] = ts_yy_xxx[i] * ga_z[i];

        ts_yyz_xxy[i] = ts_yy_xxy[i] * ga_z[i];

        ts_yyz_xxz[i] = ts_z_xxz[i] * gfe_0 + ts_yz_xxz[i] * ga_y[i];

        ts_yyz_xyy[i] = ts_yy_xyy[i] * ga_z[i];

        ts_yyz_xyz[i] = ts_yy_xy[i] * gfe_0 + ts_yy_xyz[i] * ga_z[i];

        ts_yyz_xzz[i] = ts_z_xzz[i] * gfe_0 + ts_yz_xzz[i] * ga_y[i];

        ts_yyz_yyy[i] = ts_yy_yyy[i] * ga_z[i];

        ts_yyz_yyz[i] = ts_yy_yy[i] * gfe_0 + ts_yy_yyz[i] * ga_z[i];

        ts_yyz_yzz[i] = 2.0 * ts_yy_yz[i] * gfe_0 + ts_yy_yzz[i] * ga_z[i];

        ts_yyz_zzz[i] = ts_z_zzz[i] * gfe_0 + ts_yz_zzz[i] * ga_y[i];
    }
    ...
    #pragma omp simd aligned(ga_y, ts_yzz_xxx, ts_yzz_xxy, ts_yzz_xxz, ts_yzz_xyy, ts_yzz_xyz, ts_yzz_xzz, ts_yzz_yyy, ts_yzz_yyz, ts_yzz_yzz, ts_yzz_zzz, ts_zz_xx, ts_zz_xxx, ts_zz_xxy, ts_zz_xxz, ts_zz_xy, ts_zz_xyy, ts_zz_xyz, ts_zz_xz, ts_zz_xzz, ts_zz_yy, ts_zz_yyy, ts_zz_yyz, ts_zz_yz, ts_zz_yzz, ts_zz_zz, ts_zz_zzz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_yzz_xxx[i] = ts_zz_xxx[i] * ga_y[i];

        ts_yzz_xxy[i] = ts_zz_xx[i] * gfe_0 + ts_zz_xxy[i] * ga_y[i];

        ts_yzz_xxz[i] = ts_zz_xxz[i] * ga_y[i];

        ts_yzz_xyy[i] = 2.0 * ts_zz_xy[i] * gfe_0 + ts_zz_xyy[i] * ga_y[i];

        ts_yzz_xyz[i] = ts_zz_xz[i] * gfe_0 + ts_zz_xyz[i] * ga_y[i];

        ts_yzz_xzz[i] = ts_zz_xzz[i] * ga_y[i];

        ts_yzz_yyy[i] = 3.0 * ts_zz_yy[i] * gfe_0 + ts_zz_yyy[i] * ga_y[i];

        ts_yzz_yyz[i] = 2.0 * ts_zz_yz[i] * gfe_0 + ts_zz_yyz[i] * ga_y[i];

        ts_yzz_yzz[i] = ts_zz_zz[i] * gfe_0 + ts_zz_yzz[i] * ga_y[i];

        ts_yzz_zzz[i] = ts_zz_zzz[i] * ga_y[i];
    }
    ...
    #pragma omp simd aligned(ga_z, ts_z_xxx, ts_z_xxy, ts_z_xxz, ts_z_xyy, ts_z_xyz, ts_z_xzz, ts_z_yyy, ts_z_yyz, ts_z_yzz, ts_z_zzz, ts_zz_xx, ts_zz_xxx, ts_zz_xxy, ts_zz_xxz, ts_zz_xy, ts_zz_xyy, ts_zz_xyz, ts_zz_xz, ts_zz_xzz, ts_zz_yy, ts_zz_yyy, ts_zz_yyz, ts_zz_yz, ts_zz_yzz, ts_zz_zz, ts_zz_zzz, ts_zzz_xxx, ts_zzz_xxy, ts_zzz_xxz, ts_zzz_xyy, ts_zzz_xyz, ts_zzz_xzz, ts_zzz_yyy, ts_zzz_yyz, ts_zzz_yzz, ts_zzz_zzz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_zzz_xxx[i] = 2.0 * ts_z_xxx[i] * gfe_0 + ts_zz_xxx[i] * ga_z[i];

        ts_zzz_xxy[i] = 2.0 * ts_z_xxy[i] * gfe_0 + ts_zz_xxy[i] * ga_z[i];

        ts_zzz_xxz[i] = 2.0 * ts_z_xxz[i] * gfe_0 + ts_zz_xx[i] * gfe_0 + ts_zz_xxz[i] * ga_z[i];

        ts_zzz_xyy[i] = 2.0 * ts_z_xyy[i] * gfe_0 + ts_zz_xyy[i] * ga_z[i];

        ts_zzz_xyz[i] = 2.0 * ts_z_xyz[i] * gfe_0 + ts_zz_xy[i] * gfe_0 + ts_zz_xyz[i] * ga_z[i];

        ts_zzz_xzz[i] = 2.0 * ts_z_xzz[i] * gfe_0 + 2.0 * ts_zz_xz[i] * gfe_0 + ts_zz_xzz[i] * ga_z[i];

        ts_zzz_yyy[i] = 2.0 * ts_z_yyy[i] * gfe_0 + ts_zz_yyy[i] * ga_z[i];

        ts_zzz_yyz[i] = 2.0 * ts_z_yyz[i] * gfe_0 + ts_zz_yy[i] * gfe_0 + ts_zz_yyz[i] * ga_z[i];

        ts_zzz_yzz[i] = 2.0 * ts_z_yzz[i] * gfe_0 + 2.0 * ts_zz_yz[i] * gfe_0 + ts_zz_yzz[i] * ga_z[i];

        ts_zzz_zzz[i] = 2.0 * ts_z_zzz[i] * gfe_0 + 3.0 * ts_zz_zz[i] * gfe_0 + ts_zz_zzz[i] * ga_z[i];
    }
