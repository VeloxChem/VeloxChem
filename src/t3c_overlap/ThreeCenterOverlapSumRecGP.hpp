#ifndef ThreeCenterOverlapSumRecGP_hpp
#define ThreeCenterOverlapSumRecGP_hpp
...
/// @brief Computes (G|G(r)|P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gp(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3ovlrec::comp_prim_overlap_ps(pbuffer, 5, 1, factors, 14);

                    t3ovlrec::comp_prim_overlap_pp(pbuffer, 8, 1, 2, factors, 14, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ds(pbuffer, 17, 1, 5, factors, 14, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dp(pbuffer, 23, 2, 5, 8, factors, 14, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fs(pbuffer, 41, 5, 17, factors, 14, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fp(pbuffer, 51, 8, 17, 23, factors, 14, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_gp(pbuffer, 81, 23, 41, 51, factors, 14, a_exp, exgtos[l]);

                    t2cfunc::reduce(cbuffer, pbuffer, 81, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapSumRecGP_hpp */
