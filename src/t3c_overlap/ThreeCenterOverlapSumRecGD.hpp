#ifndef ThreeCenterOverlapSumRecGD_hpp
#define ThreeCenterOverlapSumRecGD_hpp
...
/// @brief Computes (G|G(r)|D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gd(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3ovlrec::comp_prim_overlap_ds(pbuffer, 41, 1, 11, factors, 14, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dp(pbuffer, 47, 2, 11, 14, factors, 14, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dd(pbuffer, 65, 5, 14, 23, factors, 14, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fp(pbuffer, 101, 14, 41, 47, factors, 14, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fd(pbuffer, 131, 23, 47, 65, factors, 14, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_gd(pbuffer, 191, 65, 101, 131, factors, 14, a_exp, exgtos[l]);
...
#endif /* ThreeCenterOverlapSumRecGD_hpp */
