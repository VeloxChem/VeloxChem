        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3ovlrec::comp_prim_overlap_ps(pbuffer, 21, 1, factors, 14);

                    t3ovlrec::comp_prim_overlap_pp(pbuffer, 24, 1, 2, factors, 14, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pd(pbuffer, 33, 2, 5, factors, 14, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pf(pbuffer, 51, 5, 11, factors, 14, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dp(pbuffer, 81, 2, 21, 24, factors, 14, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dd(pbuffer, 99, 5, 24, 33, factors, 14, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_df(pbuffer, 135, 11, 33, 51, factors, 14, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fd(pbuffer, 195, 33, 81, 99, factors, 14, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ff(pbuffer, 255, 51, 99, 135, factors, 14, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_gf(pbuffer, 355, 135, 195, 255, factors, 14, a_exp, exgtos[l]);
