#ifndef ThreeCenterOverlapSumRecDF_hpp
#define ThreeCenterOverlapSumRecDF_hpp
...
/// @brief Computes (D|G(r)|F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_df(T& distributor,
                    ...
                    const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(129, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3ovlrec::comp_prim_overlap_pd(pbuffer, 21, 2, 5, factors, 14, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pf(pbuffer, 39, 5, 11, factors, 14, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_df(pbuffer, 69, 11, 21, 39, factors, 14, a_exp, exgtos[l]);

                    t2cfunc::reduce(cbuffer, pbuffer, 69, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapSumRecDF_hpp */
