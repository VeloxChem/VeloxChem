                     ...
                     const double c_exp) -> void
{
    ...
    #pragma omp simd aligned(ga_x, ts_xxx_xx, ts_xxx_xy, ts_xxx_xz, ts_xxx_yy, ts_xxx_yz, ts_xxx_zz, ts_xxxx_x, ts_xxxx_xx, ts_xxxx_xy, ts_xxxx_xz, ts_xxxx_y, ts_xxxx_yy, ts_xxxx_yz, ts_xxxx_z, ts_xxxx_zz, ts_xxxxx_xx, ts_xxxxx_xy, ts_xxxxx_xz, ts_xxxxx_yy, ts_xxxxx_yz, ts_xxxxx_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xxxxx_xx[i] = 4.0 * ts_xxx_xx[i] * gfe_0 + 2.0 * ts_xxxx_x[i] * gfe_0 + ts_xxxx_xx[i] * ga_x[i];

        ts_xxxxx_xy[i] = 4.0 * ts_xxx_xy[i] * gfe_0 + ts_xxxx_y[i] * gfe_0 + ts_xxxx_xy[i] * ga_x[i];

        ts_xxxxx_xz[i] = 4.0 * ts_xxx_xz[i] * gfe_0 + ts_xxxx_z[i] * gfe_0 + ts_xxxx_xz[i] * ga_x[i];

        ts_xxxxx_yy[i] = 4.0 * ts_xxx_yy[i] * gfe_0 + ts_xxxx_yy[i] * ga_x[i];

        ts_xxxxx_yz[i] = 4.0 * ts_xxx_yz[i] * gfe_0 + ts_xxxx_yz[i] * ga_x[i];

        ts_xxxxx_zz[i] = 4.0 * ts_xxx_zz[i] * gfe_0 + ts_xxxx_zz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ga_y, ts_xxxx_x, ts_xxxx_xx, ts_xxxx_xy, ts_xxxx_xz, ts_xxxx_zz, ts_xxxxy_xx, ts_xxxxy_xy, ts_xxxxy_xz, ts_xxxxy_yy, ts_xxxxy_yz, ts_xxxxy_zz, ts_xxxy_yy, ts_xxxy_yz, ts_xxy_yy, ts_xxy_yz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xxxxy_xx[i] = ts_xxxx_xx[i] * ga_y[i];

        ts_xxxxy_xy[i] = ts_xxxx_x[i] * gfe_0 + ts_xxxx_xy[i] * ga_y[i];

        ts_xxxxy_xz[i] = ts_xxxx_xz[i] * ga_y[i];

        ts_xxxxy_yy[i] = 3.0 * ts_xxy_yy[i] * gfe_0 + ts_xxxy_yy[i] * ga_x[i];

        ts_xxxxy_yz[i] = 3.0 * ts_xxy_yz[i] * gfe_0 + ts_xxxy_yz[i] * ga_x[i];

        ts_xxxxy_zz[i] = ts_xxxx_zz[i] * ga_y[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ga_z, ts_xxxx_x, ts_xxxx_xx, ts_xxxx_xy, ts_xxxx_xz, ts_xxxx_yy, ts_xxxxz_xx, ts_xxxxz_xy, ts_xxxxz_xz, ts_xxxxz_yy, ts_xxxxz_yz, ts_xxxxz_zz, ts_xxxz_yz, ts_xxxz_zz, ts_xxz_yz, ts_xxz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xxxxz_xx[i] = ts_xxxx_xx[i] * ga_z[i];

        ts_xxxxz_xy[i] = ts_xxxx_xy[i] * ga_z[i];

        ts_xxxxz_xz[i] = ts_xxxx_x[i] * gfe_0 + ts_xxxx_xz[i] * ga_z[i];

        ts_xxxxz_yy[i] = ts_xxxx_yy[i] * ga_z[i];

        ts_xxxxz_yz[i] = 3.0 * ts_xxz_yz[i] * gfe_0 + ts_xxxz_yz[i] * ga_x[i];

        ts_xxxxz_zz[i] = 3.0 * ts_xxz_zz[i] * gfe_0 + ts_xxxz_zz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ga_y, ts_xxx_xx, ts_xxx_xz, ts_xxxy_xx, ts_xxxy_xz, ts_xxxyy_xx, ts_xxxyy_xy, ts_xxxyy_xz, ts_xxxyy_yy, ts_xxxyy_yz, ts_xxxyy_zz, ts_xxyy_xy, ts_xxyy_y, ts_xxyy_yy, ts_xxyy_yz, ts_xxyy_zz, ts_xyy_xy, ts_xyy_yy, ts_xyy_yz, ts_xyy_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xxxyy_xx[i] = ts_xxx_xx[i] * gfe_0 + ts_xxxy_xx[i] * ga_y[i];

        ts_xxxyy_xy[i] = 2.0 * ts_xyy_xy[i] * gfe_0 + ts_xxyy_y[i] * gfe_0 + ts_xxyy_xy[i] * ga_x[i];

        ts_xxxyy_xz[i] = ts_xxx_xz[i] * gfe_0 + ts_xxxy_xz[i] * ga_y[i];

        ts_xxxyy_yy[i] = 2.0 * ts_xyy_yy[i] * gfe_0 + ts_xxyy_yy[i] * ga_x[i];

        ts_xxxyy_yz[i] = 2.0 * ts_xyy_yz[i] * gfe_0 + ts_xxyy_yz[i] * ga_x[i];

        ts_xxxyy_zz[i] = 2.0 * ts_xyy_zz[i] * gfe_0 + ts_xxyy_zz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ga_y, ga_z, ts_xxxy_xy, ts_xxxy_yy, ts_xxxyz_xx, ts_xxxyz_xy, ts_xxxyz_xz, ts_xxxyz_yy, ts_xxxyz_yz, ts_xxxyz_zz, ts_xxxz_xx, ts_xxxz_xz, ts_xxxz_zz, ts_xxyz_yz, ts_xyz_yz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xxxyz_xx[i] = ts_xxxz_xx[i] * ga_y[i];

        ts_xxxyz_xy[i] = ts_xxxy_xy[i] * ga_z[i];

        ts_xxxyz_xz[i] = ts_xxxz_xz[i] * ga_y[i];

        ts_xxxyz_yy[i] = ts_xxxy_yy[i] * ga_z[i];

        ts_xxxyz_yz[i] = 2.0 * ts_xyz_yz[i] * gfe_0 + ts_xxyz_yz[i] * ga_x[i];

        ts_xxxyz_zz[i] = ts_xxxz_zz[i] * ga_y[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ga_z, ts_xxx_xx, ts_xxx_xy, ts_xxxz_xx, ts_xxxz_xy, ts_xxxzz_xx, ts_xxxzz_xy, ts_xxxzz_xz, ts_xxxzz_yy, ts_xxxzz_yz, ts_xxxzz_zz, ts_xxzz_xz, ts_xxzz_yy, ts_xxzz_yz, ts_xxzz_z, ts_xxzz_zz, ts_xzz_xz, ts_xzz_yy, ts_xzz_yz, ts_xzz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xxxzz_xx[i] = ts_xxx_xx[i] * gfe_0 + ts_xxxz_xx[i] * ga_z[i];

        ts_xxxzz_xy[i] = ts_xxx_xy[i] * gfe_0 + ts_xxxz_xy[i] * ga_z[i];

        ts_xxxzz_xz[i] = 2.0 * ts_xzz_xz[i] * gfe_0 + ts_xxzz_z[i] * gfe_0 + ts_xxzz_xz[i] * ga_x[i];

        ts_xxxzz_yy[i] = 2.0 * ts_xzz_yy[i] * gfe_0 + ts_xxzz_yy[i] * ga_x[i];

        ts_xxxzz_yz[i] = 2.0 * ts_xzz_yz[i] * gfe_0 + ts_xxzz_yz[i] * ga_x[i];

        ts_xxxzz_zz[i] = 2.0 * ts_xzz_zz[i] * gfe_0 + ts_xxzz_zz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ga_y, ts_xxy_xx, ts_xxy_xz, ts_xxyy_xx, ts_xxyy_xz, ts_xxyyy_xx, ts_xxyyy_xy, ts_xxyyy_xz, ts_xxyyy_yy, ts_xxyyy_yz, ts_xxyyy_zz, ts_xyyy_xy, ts_xyyy_y, ts_xyyy_yy, ts_xyyy_yz, ts_xyyy_zz, ts_yyy_xy, ts_yyy_yy, ts_yyy_yz, ts_yyy_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xxyyy_xx[i] = 2.0 * ts_xxy_xx[i] * gfe_0 + ts_xxyy_xx[i] * ga_y[i];

        ts_xxyyy_xy[i] = ts_yyy_xy[i] * gfe_0 + ts_xyyy_y[i] * gfe_0 + ts_xyyy_xy[i] * ga_x[i];

        ts_xxyyy_xz[i] = 2.0 * ts_xxy_xz[i] * gfe_0 + ts_xxyy_xz[i] * ga_y[i];

        ts_xxyyy_yy[i] = ts_yyy_yy[i] * gfe_0 + ts_xyyy_yy[i] * ga_x[i];

        ts_xxyyy_yz[i] = ts_yyy_yz[i] * gfe_0 + ts_xyyy_yz[i] * ga_x[i];

        ts_xxyyy_zz[i] = ts_yyy_zz[i] * gfe_0 + ts_xyyy_zz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ga_y, ga_z, ts_xxyy_xx, ts_xxyy_xy, ts_xxyy_yy, ts_xxyyz_xx, ts_xxyyz_xy, ts_xxyyz_xz, ts_xxyyz_yy, ts_xxyyz_yz, ts_xxyyz_zz, ts_xxyz_xz, ts_xxz_xz, ts_xyyz_yz, ts_xyyz_zz, ts_yyz_yz, ts_yyz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xxyyz_xx[i] = ts_xxyy_xx[i] * ga_z[i];

        ts_xxyyz_xy[i] = ts_xxyy_xy[i] * ga_z[i];

        ts_xxyyz_xz[i] = ts_xxz_xz[i] * gfe_0 + ts_xxyz_xz[i] * ga_y[i];

        ts_xxyyz_yy[i] = ts_xxyy_yy[i] * ga_z[i];

        ts_xxyyz_yz[i] = ts_yyz_yz[i] * gfe_0 + ts_xyyz_yz[i] * ga_x[i];

        ts_xxyyz_zz[i] = ts_yyz_zz[i] * gfe_0 + ts_xyyz_zz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ga_y, ts_xxyzz_xx, ts_xxyzz_xy, ts_xxyzz_xz, ts_xxyzz_yy, ts_xxyzz_yz, ts_xxyzz_zz, ts_xxzz_x, ts_xxzz_xx, ts_xxzz_xy, ts_xxzz_xz, ts_xxzz_zz, ts_xyzz_yy, ts_xyzz_yz, ts_yzz_yy, ts_yzz_yz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xxyzz_xx[i] = ts_xxzz_xx[i] * ga_y[i];

        ts_xxyzz_xy[i] = ts_xxzz_x[i] * gfe_0 + ts_xxzz_xy[i] * ga_y[i];

        ts_xxyzz_xz[i] = ts_xxzz_xz[i] * ga_y[i];

        ts_xxyzz_yy[i] = ts_yzz_yy[i] * gfe_0 + ts_xyzz_yy[i] * ga_x[i];

        ts_xxyzz_yz[i] = ts_yzz_yz[i] * gfe_0 + ts_xyzz_yz[i] * ga_x[i];

        ts_xxyzz_zz[i] = ts_xxzz_zz[i] * ga_y[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ga_z, ts_xxz_xx, ts_xxz_xy, ts_xxzz_xx, ts_xxzz_xy, ts_xxzzz_xx, ts_xxzzz_xy, ts_xxzzz_xz, ts_xxzzz_yy, ts_xxzzz_yz, ts_xxzzz_zz, ts_xzzz_xz, ts_xzzz_yy, ts_xzzz_yz, ts_xzzz_z, ts_xzzz_zz, ts_zzz_xz, ts_zzz_yy, ts_zzz_yz, ts_zzz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xxzzz_xx[i] = 2.0 * ts_xxz_xx[i] * gfe_0 + ts_xxzz_xx[i] * ga_z[i];

        ts_xxzzz_xy[i] = 2.0 * ts_xxz_xy[i] * gfe_0 + ts_xxzz_xy[i] * ga_z[i];

        ts_xxzzz_xz[i] = ts_zzz_xz[i] * gfe_0 + ts_xzzz_z[i] * gfe_0 + ts_xzzz_xz[i] * ga_x[i];

        ts_xxzzz_yy[i] = ts_zzz_yy[i] * gfe_0 + ts_xzzz_yy[i] * ga_x[i];

        ts_xxzzz_yz[i] = ts_zzz_yz[i] * gfe_0 + ts_xzzz_yz[i] * ga_x[i];

        ts_xxzzz_zz[i] = ts_zzz_zz[i] * gfe_0 + ts_xzzz_zz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ts_xyyyy_xx, ts_xyyyy_xy, ts_xyyyy_xz, ts_xyyyy_yy, ts_xyyyy_yz, ts_xyyyy_zz, ts_yyyy_x, ts_yyyy_xx, ts_yyyy_xy, ts_yyyy_xz, ts_yyyy_y, ts_yyyy_yy, ts_yyyy_yz, ts_yyyy_z, ts_yyyy_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xyyyy_xx[i] = 2.0 * ts_yyyy_x[i] * gfe_0 + ts_yyyy_xx[i] * ga_x[i];

        ts_xyyyy_xy[i] = ts_yyyy_y[i] * gfe_0 + ts_yyyy_xy[i] * ga_x[i];

        ts_xyyyy_xz[i] = ts_yyyy_z[i] * gfe_0 + ts_yyyy_xz[i] * ga_x[i];

        ts_xyyyy_yy[i] = ts_yyyy_yy[i] * ga_x[i];

        ts_xyyyy_yz[i] = ts_yyyy_yz[i] * ga_x[i];

        ts_xyyyy_zz[i] = ts_yyyy_zz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ga_z, ts_xyyy_xx, ts_xyyy_xy, ts_xyyyz_xx, ts_xyyyz_xy, ts_xyyyz_xz, ts_xyyyz_yy, ts_xyyyz_yz, ts_xyyyz_zz, ts_yyyz_xz, ts_yyyz_yy, ts_yyyz_yz, ts_yyyz_z, ts_yyyz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xyyyz_xx[i] = ts_xyyy_xx[i] * ga_z[i];

        ts_xyyyz_xy[i] = ts_xyyy_xy[i] * ga_z[i];

        ts_xyyyz_xz[i] = ts_yyyz_z[i] * gfe_0 + ts_yyyz_xz[i] * ga_x[i];

        ts_xyyyz_yy[i] = ts_yyyz_yy[i] * ga_x[i];

        ts_xyyyz_yz[i] = ts_yyyz_yz[i] * ga_x[i];

        ts_xyyyz_zz[i] = ts_yyyz_zz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ts_xyyzz_xx, ts_xyyzz_xy, ts_xyyzz_xz, ts_xyyzz_yy, ts_xyyzz_yz, ts_xyyzz_zz, ts_yyzz_x, ts_yyzz_xx, ts_yyzz_xy, ts_yyzz_xz, ts_yyzz_y, ts_yyzz_yy, ts_yyzz_yz, ts_yyzz_z, ts_yyzz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xyyzz_xx[i] = 2.0 * ts_yyzz_x[i] * gfe_0 + ts_yyzz_xx[i] * ga_x[i];

        ts_xyyzz_xy[i] = ts_yyzz_y[i] * gfe_0 + ts_yyzz_xy[i] * ga_x[i];

        ts_xyyzz_xz[i] = ts_yyzz_z[i] * gfe_0 + ts_yyzz_xz[i] * ga_x[i];

        ts_xyyzz_yy[i] = ts_yyzz_yy[i] * ga_x[i];

        ts_xyyzz_yz[i] = ts_yyzz_yz[i] * ga_x[i];

        ts_xyyzz_zz[i] = ts_yyzz_zz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ga_y, ts_xyzzz_xx, ts_xyzzz_xy, ts_xyzzz_xz, ts_xyzzz_yy, ts_xyzzz_yz, ts_xyzzz_zz, ts_xzzz_xx, ts_xzzz_xz, ts_yzzz_xy, ts_yzzz_y, ts_yzzz_yy, ts_yzzz_yz, ts_yzzz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xyzzz_xx[i] = ts_xzzz_xx[i] * ga_y[i];

        ts_xyzzz_xy[i] = ts_yzzz_y[i] * gfe_0 + ts_yzzz_xy[i] * ga_x[i];

        ts_xyzzz_xz[i] = ts_xzzz_xz[i] * ga_y[i];

        ts_xyzzz_yy[i] = ts_yzzz_yy[i] * ga_x[i];

        ts_xyzzz_yz[i] = ts_yzzz_yz[i] * ga_x[i];

        ts_xyzzz_zz[i] = ts_yzzz_zz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ts_xzzzz_xx, ts_xzzzz_xy, ts_xzzzz_xz, ts_xzzzz_yy, ts_xzzzz_yz, ts_xzzzz_zz, ts_zzzz_x, ts_zzzz_xx, ts_zzzz_xy, ts_zzzz_xz, ts_zzzz_y, ts_zzzz_yy, ts_zzzz_yz, ts_zzzz_z, ts_zzzz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xzzzz_xx[i] = 2.0 * ts_zzzz_x[i] * gfe_0 + ts_zzzz_xx[i] * ga_x[i];

        ts_xzzzz_xy[i] = ts_zzzz_y[i] * gfe_0 + ts_zzzz_xy[i] * ga_x[i];

        ts_xzzzz_xz[i] = ts_zzzz_z[i] * gfe_0 + ts_zzzz_xz[i] * ga_x[i];

        ts_xzzzz_yy[i] = ts_zzzz_yy[i] * ga_x[i];

        ts_xzzzz_yz[i] = ts_zzzz_yz[i] * ga_x[i];

        ts_xzzzz_zz[i] = ts_zzzz_zz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_y, ts_yyy_xx, ts_yyy_xy, ts_yyy_xz, ts_yyy_yy, ts_yyy_yz, ts_yyy_zz, ts_yyyy_x, ts_yyyy_xx, ts_yyyy_xy, ts_yyyy_xz, ts_yyyy_y, ts_yyyy_yy, ts_yyyy_yz, ts_yyyy_z, ts_yyyy_zz, ts_yyyyy_xx, ts_yyyyy_xy, ts_yyyyy_xz, ts_yyyyy_yy, ts_yyyyy_yz, ts_yyyyy_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_yyyyy_xx[i] = 4.0 * ts_yyy_xx[i] * gfe_0 + ts_yyyy_xx[i] * ga_y[i];

        ts_yyyyy_xy[i] = 4.0 * ts_yyy_xy[i] * gfe_0 + ts_yyyy_x[i] * gfe_0 + ts_yyyy_xy[i] * ga_y[i];

        ts_yyyyy_xz[i] = 4.0 * ts_yyy_xz[i] * gfe_0 + ts_yyyy_xz[i] * ga_y[i];

        ts_yyyyy_yy[i] = 4.0 * ts_yyy_yy[i] * gfe_0 + 2.0 * ts_yyyy_y[i] * gfe_0 + ts_yyyy_yy[i] * ga_y[i];

        ts_yyyyy_yz[i] = 4.0 * ts_yyy_yz[i] * gfe_0 + ts_yyyy_z[i] * gfe_0 + ts_yyyy_yz[i] * ga_y[i];

        ts_yyyyy_zz[i] = 4.0 * ts_yyy_zz[i] * gfe_0 + ts_yyyy_zz[i] * ga_y[i];
    }
    ...
    #pragma omp simd aligned(ga_y, ga_z, ts_yyyy_xx, ts_yyyy_xy, ts_yyyy_y, ts_yyyy_yy, ts_yyyy_yz, ts_yyyyz_xx, ts_yyyyz_xy, ts_yyyyz_xz, ts_yyyyz_yy, ts_yyyyz_yz, ts_yyyyz_zz, ts_yyyz_xz, ts_yyyz_zz, ts_yyz_xz, ts_yyz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_yyyyz_xx[i] = ts_yyyy_xx[i] * ga_z[i];

        ts_yyyyz_xy[i] = ts_yyyy_xy[i] * ga_z[i];

        ts_yyyyz_xz[i] = 3.0 * ts_yyz_xz[i] * gfe_0 + ts_yyyz_xz[i] * ga_y[i];

        ts_yyyyz_yy[i] = ts_yyyy_yy[i] * ga_z[i];

        ts_yyyyz_yz[i] = ts_yyyy_y[i] * gfe_0 + ts_yyyy_yz[i] * ga_z[i];

        ts_yyyyz_zz[i] = 3.0 * ts_yyz_zz[i] * gfe_0 + ts_yyyz_zz[i] * ga_y[i];
    }
    ...
    #pragma omp simd aligned(ga_y, ga_z, ts_yyy_xy, ts_yyy_yy, ts_yyyz_xy, ts_yyyz_yy, ts_yyyzz_xx, ts_yyyzz_xy, ts_yyyzz_xz, ts_yyyzz_yy, ts_yyyzz_yz, ts_yyyzz_zz, ts_yyzz_xx, ts_yyzz_xz, ts_yyzz_yz, ts_yyzz_z, ts_yyzz_zz, ts_yzz_xx, ts_yzz_xz, ts_yzz_yz, ts_yzz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_yyyzz_xx[i] = 2.0 * ts_yzz_xx[i] * gfe_0 + ts_yyzz_xx[i] * ga_y[i];

        ts_yyyzz_xy[i] = ts_yyy_xy[i] * gfe_0 + ts_yyyz_xy[i] * ga_z[i];

        ts_yyyzz_xz[i] = 2.0 * ts_yzz_xz[i] * gfe_0 + ts_yyzz_xz[i] * ga_y[i];

        ts_yyyzz_yy[i] = ts_yyy_yy[i] * gfe_0 + ts_yyyz_yy[i] * ga_z[i];

        ts_yyyzz_yz[i] = 2.0 * ts_yzz_yz[i] * gfe_0 + ts_yyzz_z[i] * gfe_0 + ts_yyzz_yz[i] * ga_y[i];

        ts_yyyzz_zz[i] = 2.0 * ts_yzz_zz[i] * gfe_0 + ts_yyzz_zz[i] * ga_y[i];
    }
    ...
    #pragma omp simd aligned(ga_y, ga_z, ts_yyz_xy, ts_yyz_yy, ts_yyzz_xy, ts_yyzz_yy, ts_yyzzz_xx, ts_yyzzz_xy, ts_yyzzz_xz, ts_yyzzz_yy, ts_yyzzz_yz, ts_yyzzz_zz, ts_yzzz_xx, ts_yzzz_xz, ts_yzzz_yz, ts_yzzz_z, ts_yzzz_zz, ts_zzz_xx, ts_zzz_xz, ts_zzz_yz, ts_zzz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_yyzzz_xx[i] = ts_zzz_xx[i] * gfe_0 + ts_yzzz_xx[i] * ga_y[i];

        ts_yyzzz_xy[i] = 2.0 * ts_yyz_xy[i] * gfe_0 + ts_yyzz_xy[i] * ga_z[i];

        ts_yyzzz_xz[i] = ts_zzz_xz[i] * gfe_0 + ts_yzzz_xz[i] * ga_y[i];

        ts_yyzzz_yy[i] = 2.0 * ts_yyz_yy[i] * gfe_0 + ts_yyzz_yy[i] * ga_z[i];

        ts_yyzzz_yz[i] = ts_zzz_yz[i] * gfe_0 + ts_yzzz_z[i] * gfe_0 + ts_yzzz_yz[i] * ga_y[i];

        ts_yyzzz_zz[i] = ts_zzz_zz[i] * gfe_0 + ts_yzzz_zz[i] * ga_y[i];
    }
    ...
    #pragma omp simd aligned(ga_y, ts_yzzzz_xx, ts_yzzzz_xy, ts_yzzzz_xz, ts_yzzzz_yy, ts_yzzzz_yz, ts_yzzzz_zz, ts_zzzz_x, ts_zzzz_xx, ts_zzzz_xy, ts_zzzz_xz, ts_zzzz_y, ts_zzzz_yy, ts_zzzz_yz, ts_zzzz_z, ts_zzzz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_yzzzz_xx[i] = ts_zzzz_xx[i] * ga_y[i];

        ts_yzzzz_xy[i] = ts_zzzz_x[i] * gfe_0 + ts_zzzz_xy[i] * ga_y[i];

        ts_yzzzz_xz[i] = ts_zzzz_xz[i] * ga_y[i];

        ts_yzzzz_yy[i] = 2.0 * ts_zzzz_y[i] * gfe_0 + ts_zzzz_yy[i] * ga_y[i];

        ts_yzzzz_yz[i] = ts_zzzz_z[i] * gfe_0 + ts_zzzz_yz[i] * ga_y[i];

        ts_yzzzz_zz[i] = ts_zzzz_zz[i] * ga_y[i];
    }
    ...
    #pragma omp simd aligned(ga_z, ts_zzz_xx, ts_zzz_xy, ts_zzz_xz, ts_zzz_yy, ts_zzz_yz, ts_zzz_zz, ts_zzzz_x, ts_zzzz_xx, ts_zzzz_xy, ts_zzzz_xz, ts_zzzz_y, ts_zzzz_yy, ts_zzzz_yz, ts_zzzz_z, ts_zzzz_zz, ts_zzzzz_xx, ts_zzzzz_xy, ts_zzzzz_xz, ts_zzzzz_yy, ts_zzzzz_yz, ts_zzzzz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_zzzzz_xx[i] = 4.0 * ts_zzz_xx[i] * gfe_0 + ts_zzzz_xx[i] * ga_z[i];

        ts_zzzzz_xy[i] = 4.0 * ts_zzz_xy[i] * gfe_0 + ts_zzzz_xy[i] * ga_z[i];

        ts_zzzzz_xz[i] = 4.0 * ts_zzz_xz[i] * gfe_0 + ts_zzzz_x[i] * gfe_0 + ts_zzzz_xz[i] * ga_z[i];

        ts_zzzzz_yy[i] = 4.0 * ts_zzz_yy[i] * gfe_0 + ts_zzzz_yy[i] * ga_z[i];

        ts_zzzzz_yz[i] = 4.0 * ts_zzz_yz[i] * gfe_0 + ts_zzzz_y[i] * gfe_0 + ts_zzzz_yz[i] * ga_z[i];

        ts_zzzzz_zz[i] = 4.0 * ts_zzz_zz[i] * gfe_0 + 2.0 * ts_zzzz_z[i] * gfe_0 + ts_zzzz_zz[i] * ga_z[i];
    }
