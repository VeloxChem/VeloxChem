#include "ThreeCenterOverlapPrimRecGD.hpp"
                     ...
                     const double c_exp) -> void
{
    ...
    #pragma omp simd aligned(ga_x, ts_xx_xx, ts_xx_xy, ts_xx_xz, ts_xx_yy, ts_xx_yz, ts_xx_zz, ts_xxx_x, ts_xxx_xx, ts_xxx_xy, ts_xxx_xz, ts_xxx_y, ts_xxx_yy, ts_xxx_yz, ts_xxx_z, ts_xxx_zz, ts_xxxx_xx, ts_xxxx_xy, ts_xxxx_xz, ts_xxxx_yy, ts_xxxx_yz, ts_xxxx_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xxxx_xx[i] = 3.0 * ts_xx_xx[i] * gfe_0 + 2.0 * ts_xxx_x[i] * gfe_0 + ts_xxx_xx[i] * ga_x[i];

        ts_xxxx_xy[i] = 3.0 * ts_xx_xy[i] * gfe_0 + ts_xxx_y[i] * gfe_0 + ts_xxx_xy[i] * ga_x[i];

        ts_xxxx_xz[i] = 3.0 * ts_xx_xz[i] * gfe_0 + ts_xxx_z[i] * gfe_0 + ts_xxx_xz[i] * ga_x[i];

        ts_xxxx_yy[i] = 3.0 * ts_xx_yy[i] * gfe_0 + ts_xxx_yy[i] * ga_x[i];

        ts_xxxx_yz[i] = 3.0 * ts_xx_yz[i] * gfe_0 + ts_xxx_yz[i] * ga_x[i];

        ts_xxxx_zz[i] = 3.0 * ts_xx_zz[i] * gfe_0 + ts_xxx_zz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ga_y, ts_xxx_x, ts_xxx_xx, ts_xxx_xy, ts_xxx_xz, ts_xxx_zz, ts_xxxy_xx, ts_xxxy_xy, ts_xxxy_xz, ts_xxxy_yy, ts_xxxy_yz, ts_xxxy_zz, ts_xxy_yy, ts_xxy_yz, ts_xy_yy, ts_xy_yz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xxxy_xx[i] = ts_xxx_xx[i] * ga_y[i];

        ts_xxxy_xy[i] = ts_xxx_x[i] * gfe_0 + ts_xxx_xy[i] * ga_y[i];

        ts_xxxy_xz[i] = ts_xxx_xz[i] * ga_y[i];

        ts_xxxy_yy[i] = 2.0 * ts_xy_yy[i] * gfe_0 + ts_xxy_yy[i] * ga_x[i];

        ts_xxxy_yz[i] = 2.0 * ts_xy_yz[i] * gfe_0 + ts_xxy_yz[i] * ga_x[i];

        ts_xxxy_zz[i] = ts_xxx_zz[i] * ga_y[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ga_z, ts_xxx_x, ts_xxx_xx, ts_xxx_xy, ts_xxx_xz, ts_xxx_yy, ts_xxxz_xx, ts_xxxz_xy, ts_xxxz_xz, ts_xxxz_yy, ts_xxxz_yz, ts_xxxz_zz, ts_xxz_yz, ts_xxz_zz, ts_xz_yz, ts_xz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xxxz_xx[i] = ts_xxx_xx[i] * ga_z[i];

        ts_xxxz_xy[i] = ts_xxx_xy[i] * ga_z[i];

        ts_xxxz_xz[i] = ts_xxx_x[i] * gfe_0 + ts_xxx_xz[i] * ga_z[i];

        ts_xxxz_yy[i] = ts_xxx_yy[i] * ga_z[i];

        ts_xxxz_yz[i] = 2.0 * ts_xz_yz[i] * gfe_0 + ts_xxz_yz[i] * ga_x[i];

        ts_xxxz_zz[i] = 2.0 * ts_xz_zz[i] * gfe_0 + ts_xxz_zz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ga_y, ts_xx_xx, ts_xx_xz, ts_xxy_xx, ts_xxy_xz, ts_xxyy_xx, ts_xxyy_xy, ts_xxyy_xz, ts_xxyy_yy, ts_xxyy_yz, ts_xxyy_zz, ts_xyy_xy, ts_xyy_y, ts_xyy_yy, ts_xyy_yz, ts_xyy_zz, ts_yy_xy, ts_yy_yy, ts_yy_yz, ts_yy_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xxyy_xx[i] = ts_xx_xx[i] * gfe_0 + ts_xxy_xx[i] * ga_y[i];

        ts_xxyy_xy[i] = ts_yy_xy[i] * gfe_0 + ts_xyy_y[i] * gfe_0 + ts_xyy_xy[i] * ga_x[i];

        ts_xxyy_xz[i] = ts_xx_xz[i] * gfe_0 + ts_xxy_xz[i] * ga_y[i];

        ts_xxyy_yy[i] = ts_yy_yy[i] * gfe_0 + ts_xyy_yy[i] * ga_x[i];

        ts_xxyy_yz[i] = ts_yy_yz[i] * gfe_0 + ts_xyy_yz[i] * ga_x[i];

        ts_xxyy_zz[i] = ts_yy_zz[i] * gfe_0 + ts_xyy_zz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ga_y, ga_z, ts_xxy_xy, ts_xxy_yy, ts_xxyz_xx, ts_xxyz_xy, ts_xxyz_xz, ts_xxyz_yy, ts_xxyz_yz, ts_xxyz_zz, ts_xxz_xx, ts_xxz_xz, ts_xxz_zz, ts_xyz_yz, ts_yz_yz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xxyz_xx[i] = ts_xxz_xx[i] * ga_y[i];

        ts_xxyz_xy[i] = ts_xxy_xy[i] * ga_z[i];

        ts_xxyz_xz[i] = ts_xxz_xz[i] * ga_y[i];

        ts_xxyz_yy[i] = ts_xxy_yy[i] * ga_z[i];

        ts_xxyz_yz[i] = ts_yz_yz[i] * gfe_0 + ts_xyz_yz[i] * ga_x[i];

        ts_xxyz_zz[i] = ts_xxz_zz[i] * ga_y[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ga_z, ts_xx_xx, ts_xx_xy, ts_xxz_xx, ts_xxz_xy, ts_xxzz_xx, ts_xxzz_xy, ts_xxzz_xz, ts_xxzz_yy, ts_xxzz_yz, ts_xxzz_zz, ts_xzz_xz, ts_xzz_yy, ts_xzz_yz, ts_xzz_z, ts_xzz_zz, ts_zz_xz, ts_zz_yy, ts_zz_yz, ts_zz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xxzz_xx[i] = ts_xx_xx[i] * gfe_0 + ts_xxz_xx[i] * ga_z[i];

        ts_xxzz_xy[i] = ts_xx_xy[i] * gfe_0 + ts_xxz_xy[i] * ga_z[i];

        ts_xxzz_xz[i] = ts_zz_xz[i] * gfe_0 + ts_xzz_z[i] * gfe_0 + ts_xzz_xz[i] * ga_x[i];

        ts_xxzz_yy[i] = ts_zz_yy[i] * gfe_0 + ts_xzz_yy[i] * ga_x[i];

        ts_xxzz_yz[i] = ts_zz_yz[i] * gfe_0 + ts_xzz_yz[i] * ga_x[i];

        ts_xxzz_zz[i] = ts_zz_zz[i] * gfe_0 + ts_xzz_zz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ts_xyyy_xx, ts_xyyy_xy, ts_xyyy_xz, ts_xyyy_yy, ts_xyyy_yz, ts_xyyy_zz, ts_yyy_x, ts_yyy_xx, ts_yyy_xy, ts_yyy_xz, ts_yyy_y, ts_yyy_yy, ts_yyy_yz, ts_yyy_z, ts_yyy_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xyyy_xx[i] = 2.0 * ts_yyy_x[i] * gfe_0 + ts_yyy_xx[i] * ga_x[i];

        ts_xyyy_xy[i] = ts_yyy_y[i] * gfe_0 + ts_yyy_xy[i] * ga_x[i];

        ts_xyyy_xz[i] = ts_yyy_z[i] * gfe_0 + ts_yyy_xz[i] * ga_x[i];

        ts_xyyy_yy[i] = ts_yyy_yy[i] * ga_x[i];

        ts_xyyy_yz[i] = ts_yyy_yz[i] * ga_x[i];

        ts_xyyy_zz[i] = ts_yyy_zz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ga_z, ts_xyy_xx, ts_xyy_xy, ts_xyyz_xx, ts_xyyz_xy, ts_xyyz_xz, ts_xyyz_yy, ts_xyyz_yz, ts_xyyz_zz, ts_yyz_xz, ts_yyz_yy, ts_yyz_yz, ts_yyz_z, ts_yyz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xyyz_xx[i] = ts_xyy_xx[i] * ga_z[i];

        ts_xyyz_xy[i] = ts_xyy_xy[i] * ga_z[i];

        ts_xyyz_xz[i] = ts_yyz_z[i] * gfe_0 + ts_yyz_xz[i] * ga_x[i];

        ts_xyyz_yy[i] = ts_yyz_yy[i] * ga_x[i];

        ts_xyyz_yz[i] = ts_yyz_yz[i] * ga_x[i];

        ts_xyyz_zz[i] = ts_yyz_zz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ga_y, ts_xyzz_xx, ts_xyzz_xy, ts_xyzz_xz, ts_xyzz_yy, ts_xyzz_yz, ts_xyzz_zz, ts_xzz_xx, ts_xzz_xz, ts_yzz_xy, ts_yzz_y, ts_yzz_yy, ts_yzz_yz, ts_yzz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xyzz_xx[i] = ts_xzz_xx[i] * ga_y[i];

        ts_xyzz_xy[i] = ts_yzz_y[i] * gfe_0 + ts_yzz_xy[i] * ga_x[i];

        ts_xyzz_xz[i] = ts_xzz_xz[i] * ga_y[i];

        ts_xyzz_yy[i] = ts_yzz_yy[i] * ga_x[i];

        ts_xyzz_yz[i] = ts_yzz_yz[i] * ga_x[i];

        ts_xyzz_zz[i] = ts_yzz_zz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_x, ts_xzzz_xx, ts_xzzz_xy, ts_xzzz_xz, ts_xzzz_yy, ts_xzzz_yz, ts_xzzz_zz, ts_zzz_x, ts_zzz_xx, ts_zzz_xy, ts_zzz_xz, ts_zzz_y, ts_zzz_yy, ts_zzz_yz, ts_zzz_z, ts_zzz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_xzzz_xx[i] = 2.0 * ts_zzz_x[i] * gfe_0 + ts_zzz_xx[i] * ga_x[i];

        ts_xzzz_xy[i] = ts_zzz_y[i] * gfe_0 + ts_zzz_xy[i] * ga_x[i];

        ts_xzzz_xz[i] = ts_zzz_z[i] * gfe_0 + ts_zzz_xz[i] * ga_x[i];

        ts_xzzz_yy[i] = ts_zzz_yy[i] * ga_x[i];

        ts_xzzz_yz[i] = ts_zzz_yz[i] * ga_x[i];

        ts_xzzz_zz[i] = ts_zzz_zz[i] * ga_x[i];
    }
    ...
    #pragma omp simd aligned(ga_y, ts_yy_xx, ts_yy_xy, ts_yy_xz, ts_yy_yy, ts_yy_yz, ts_yy_zz, ts_yyy_x, ts_yyy_xx, ts_yyy_xy, ts_yyy_xz, ts_yyy_y, ts_yyy_yy, ts_yyy_yz, ts_yyy_z, ts_yyy_zz, ts_yyyy_xx, ts_yyyy_xy, ts_yyyy_xz, ts_yyyy_yy, ts_yyyy_yz, ts_yyyy_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_yyyy_xx[i] = 3.0 * ts_yy_xx[i] * gfe_0 + ts_yyy_xx[i] * ga_y[i];

        ts_yyyy_xy[i] = 3.0 * ts_yy_xy[i] * gfe_0 + ts_yyy_x[i] * gfe_0 + ts_yyy_xy[i] * ga_y[i];

        ts_yyyy_xz[i] = 3.0 * ts_yy_xz[i] * gfe_0 + ts_yyy_xz[i] * ga_y[i];

        ts_yyyy_yy[i] = 3.0 * ts_yy_yy[i] * gfe_0 + 2.0 * ts_yyy_y[i] * gfe_0 + ts_yyy_yy[i] * ga_y[i];

        ts_yyyy_yz[i] = 3.0 * ts_yy_yz[i] * gfe_0 + ts_yyy_z[i] * gfe_0 + ts_yyy_yz[i] * ga_y[i];

        ts_yyyy_zz[i] = 3.0 * ts_yy_zz[i] * gfe_0 + ts_yyy_zz[i] * ga_y[i];
    }
    ...
    #pragma omp simd aligned(ga_y, ga_z, ts_yyy_xx, ts_yyy_xy, ts_yyy_y, ts_yyy_yy, ts_yyy_yz, ts_yyyz_xx, ts_yyyz_xy, ts_yyyz_xz, ts_yyyz_yy, ts_yyyz_yz, ts_yyyz_zz, ts_yyz_xz, ts_yyz_zz, ts_yz_xz, ts_yz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_yyyz_xx[i] = ts_yyy_xx[i] * ga_z[i];

        ts_yyyz_xy[i] = ts_yyy_xy[i] * ga_z[i];

        ts_yyyz_xz[i] = 2.0 * ts_yz_xz[i] * gfe_0 + ts_yyz_xz[i] * ga_y[i];

        ts_yyyz_yy[i] = ts_yyy_yy[i] * ga_z[i];

        ts_yyyz_yz[i] = ts_yyy_y[i] * gfe_0 + ts_yyy_yz[i] * ga_z[i];

        ts_yyyz_zz[i] = 2.0 * ts_yz_zz[i] * gfe_0 + ts_yyz_zz[i] * ga_y[i];
    }
    ...
    #pragma omp simd aligned(ga_y, ga_z, ts_yy_xy, ts_yy_yy, ts_yyz_xy, ts_yyz_yy, ts_yyzz_xx, ts_yyzz_xy, ts_yyzz_xz, ts_yyzz_yy, ts_yyzz_yz, ts_yyzz_zz, ts_yzz_xx, ts_yzz_xz, ts_yzz_yz, ts_yzz_z, ts_yzz_zz, ts_zz_xx, ts_zz_xz, ts_zz_yz, ts_zz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_yyzz_xx[i] = ts_zz_xx[i] * gfe_0 + ts_yzz_xx[i] * ga_y[i];

        ts_yyzz_xy[i] = ts_yy_xy[i] * gfe_0 + ts_yyz_xy[i] * ga_z[i];

        ts_yyzz_xz[i] = ts_zz_xz[i] * gfe_0 + ts_yzz_xz[i] * ga_y[i];

        ts_yyzz_yy[i] = ts_yy_yy[i] * gfe_0 + ts_yyz_yy[i] * ga_z[i];

        ts_yyzz_yz[i] = ts_zz_yz[i] * gfe_0 + ts_yzz_z[i] * gfe_0 + ts_yzz_yz[i] * ga_y[i];

        ts_yyzz_zz[i] = ts_zz_zz[i] * gfe_0 + ts_yzz_zz[i] * ga_y[i];
    }
    ...
    #pragma omp simd aligned(ga_y, ts_yzzz_xx, ts_yzzz_xy, ts_yzzz_xz, ts_yzzz_yy, ts_yzzz_yz, ts_yzzz_zz, ts_zzz_x, ts_zzz_xx, ts_zzz_xy, ts_zzz_xz, ts_zzz_y, ts_zzz_yy, ts_zzz_yz, ts_zzz_z, ts_zzz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_yzzz_xx[i] = ts_zzz_xx[i] * ga_y[i];

        ts_yzzz_xy[i] = ts_zzz_x[i] * gfe_0 + ts_zzz_xy[i] * ga_y[i];

        ts_yzzz_xz[i] = ts_zzz_xz[i] * ga_y[i];

        ts_yzzz_yy[i] = 2.0 * ts_zzz_y[i] * gfe_0 + ts_zzz_yy[i] * ga_y[i];

        ts_yzzz_yz[i] = ts_zzz_z[i] * gfe_0 + ts_zzz_yz[i] * ga_y[i];

        ts_yzzz_zz[i] = ts_zzz_zz[i] * ga_y[i];
    }
    ...
    #pragma omp simd aligned(ga_z, ts_zz_xx, ts_zz_xy, ts_zz_xz, ts_zz_yy, ts_zz_yz, ts_zz_zz, ts_zzz_x, ts_zzz_xx, ts_zzz_xy, ts_zzz_xz, ts_zzz_y, ts_zzz_yy, ts_zzz_yz, ts_zzz_z, ts_zzz_zz, ts_zzzz_xx, ts_zzzz_xy, ts_zzzz_xz, ts_zzzz_yy, ts_zzzz_yz, ts_zzzz_zz, b_exps : 64)
    for (size_t i = 0; i < nelems; i++)
    {
        const double gfe_0 = 0.5 / (a_exp + b_exps[i] + c_exp);

        ts_zzzz_xx[i] = 3.0 * ts_zz_xx[i] * gfe_0 + ts_zzz_xx[i] * ga_z[i];

        ts_zzzz_xy[i] = 3.0 * ts_zz_xy[i] * gfe_0 + ts_zzz_xy[i] * ga_z[i];

        ts_zzzz_xz[i] = 3.0 * ts_zz_xz[i] * gfe_0 + ts_zzz_x[i] * gfe_0 + ts_zzz_xz[i] * ga_z[i];

        ts_zzzz_yy[i] = 3.0 * ts_zz_yy[i] * gfe_0 + ts_zzz_yy[i] * ga_z[i];

        ts_zzzz_yz[i] = 3.0 * ts_zz_yz[i] * gfe_0 + ts_zzz_y[i] * gfe_0 + ts_zzz_yz[i] * ga_z[i];

        ts_zzzz_zz[i] = 3.0 * ts_zz_zz[i] * gfe_0 + 2.0 * ts_zzz_z[i] * gfe_0 + ts_zzz_zz[i] * ga_z[i];
    }
