#ifndef ProjectedCorePotentialGeom110SGForS_hpp
#define ProjectedCorePotentialGeom110SGForS_hpp
...
template <class T>
auto
comp_projected_core_potential_geom_110_sg_for_s(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < ecp_nppt; l++)
                {
                    ...
                    t2pecp::comp_prim_projected_core_potential_sh_s(pbuffer, 170, 10, 20, 474, 484, 0, -1, -1, pfactors, 2, a_exp, c_exp);

                    t2pecp::comp_prim_projected_core_potential_sh_s(pbuffer, 609, 584, 594, 705, 715, 1, 429, 439, pfactors, 2, a_exp, c_exp);

                    t2pecp::comp_prim_projected_core_potential_pf_s(pbuffer, 191, 10, 584, 0, -1, pfactors, r_a, a_exp, c_exp);

                    t2pecp::comp_prim_projected_core_potential_pg_s(pbuffer, 221, 20, 594, 0, -1, pfactors, r_a, a_exp, c_exp);

                    t2pecp::comp_prim_projected_core_potential_pg_s(pbuffer, 630, 594, 800, 0, -1, pfactors, r_a, a_exp, c_exp);

                    t2pecp::comp_prim_projected_core_potential_ph_s(pbuffer, 266, 170, 609, 0, -1, pfactors, r_a, a_exp, c_exp);

                    t2pecp::comp_prim_projected_core_potential_dg_s(pbuffer, 329, 20, 221, 594, 630, 0, -1, -1, pfactors, r_a, a_exp, c_exp);
...
#endif /* ProjectedCorePotentialGeom110SGForS_hpp */
