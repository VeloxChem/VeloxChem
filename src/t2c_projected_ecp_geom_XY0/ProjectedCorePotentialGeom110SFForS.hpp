#ifndef ProjectedCorePotentialGeom110SFForS_hpp
#define ProjectedCorePotentialGeom110SFForS_hpp
...
template <class T>
auto
comp_projected_core_potential_geom_110_sf_for_s(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < ecp_nppt; l++)
                {
                    ...
                    t2pecp::comp_prim_projected_core_potential_sg_s(pbuffer, 110, 4, 10, 306, 312, 0, -1, -1, pfactors, 2, a_exp, c_exp);

                    t2pecp::comp_prim_projected_core_potential_sg_s(pbuffer, 373, 357, 363, 432, 438, 1, 282, 288, pfactors, 2, a_exp, c_exp);

                    t2pecp::comp_prim_projected_core_potential_pd_s(pbuffer, 125, 4, 357, 0, -1, pfactors, r_a, a_exp, c_exp);

                    t2pecp::comp_prim_projected_core_potential_pf_s(pbuffer, 143, 10, 363, 0, -1, pfactors, r_a, a_exp, c_exp);

                    t2pecp::comp_prim_projected_core_potential_pf_s(pbuffer, 388, 363, 478, 0, -1, pfactors, r_a, a_exp, c_exp);

                    t2pecp::comp_prim_projected_core_potential_pg_s(pbuffer, 173, 110, 373, 0, -1, pfactors, r_a, a_exp, c_exp);

                    t2pecp::comp_prim_projected_core_potential_df_s(pbuffer, 218, 10, 143, 363, 388, 0, -1, -1, pfactors, r_a, a_exp, c_exp);
...
#endif /* ProjectedCorePotentialGeom110SFForS_hpp */
