...
class COneElectronExpectationDriver
{
   public:
    COneElectronExpectationDriver() = default;
            ...
            const CMolecule&       molecule) const -> void
    {
        ...
        if (op_case == 1)
        {
            ...
            {
                CKineticEnergyGeomX00Driver<1> kin_exp_drv;

                kin_exp_drv.compute(task.get_results(), densities, basis, molecule);

                CNuclearPotentialGeomX00Driver<1> nuc_exp_drv;

                nuc_exp_drv.compute(task.get_results(), densities, basis, molecule);
            }

            if (geom_order == T({2, 0, 0, 0}))
            {
                CKineticEnergyGeomX00Driver<2> kin_exp_drv;

                kin_exp_drv.compute(task.get_results(), densities, basis, molecule);

                CNuclearPotentialGeomX00Driver<2> nuc_exp_drv;
            ...
            {
                CKineticEnergyGeomX0YDriver<1, 1> kin_exp_drv;

                kin_exp_drv.compute(task.get_results(), densities, basis, molecule);

                CNuclearPotentialGeomX0YDriver<1, 1> nuc_exp_drv;
            ...
            {
                CNuclearPotentialGeom0X0Driver<1> nuc_exp_drv;
            ...
            {
                CNuclearPotentialGeom0X0Driver<2> nuc_exp_drv;
        ...
        if (op_case == 2)
        {
            for (const auto& tarch : task.get_tasks())
            {
                ...
                if (geom_order == T({1, 0, 0, 0}))
                {
                    COverlapGeomX00Driver<1> exp_drv;
                ...
                if (geom_order == T({2, 0, 0, 0}))
                {
                    COverlapGeomX00Driver<2> exp_drv;
                ...
                if (geom_order == T({1, 0, 1, 0}))
                {
                    COverlapGeomX0YDriver<1, 1> exp_drv;
