#ifndef TwoElectronFockDriver_hpp
#define TwoElectronFockDriver_hpp

template <class T, class U, class W>
class CTwoElectronFockDriver
{
   public:
    CTwoElectronFockDriver() = default;
            ...
            const CMolecule&       molecule) const -> void
    {
        ...
        for (const auto& tarch : task.getTasks())
        {
            ...
            if (geom_order == T({1, 0, 0, 0}))
            {
                ...
                fock_drv.compute(tarch, task.get_results(), densities, basis, molecule, screener);
            }
            ...
            if (geom_order == T({1, 0, 1, 0}))
            {
                ...
                fock_drv.compute(tarch, task.get_results(), densities, basis, molecule);
            }
        }
    }
};

#endif /* TwoElectronFockDriver_hpp */
