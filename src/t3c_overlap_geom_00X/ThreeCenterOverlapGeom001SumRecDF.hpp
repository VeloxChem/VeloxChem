#ifndef ThreeCenterOverlapGeom001SumRecDF_hpp
#define ThreeCenterOverlapGeom001SumRecDF_hpp
...
/// @brief Computes (D|G(r)|d^(1)/dB^(1)F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_geom_01_df(T& distributor,
                            ...
                            const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(444, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3ovlrec::comp_prim_overlap_dg(pbuffer, 174, 21, 63, 93, factors, 11, a_exp, exgtos[l]);

                    t2cgeom::comp_geom_deriv_0x1_df(pbuffer, 264, 138, 174, factors, 1);

                    t2cfunc::reduce(cbuffer, pbuffer, 264, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGeom001SumRecDF_hpp */
