#ifndef ThreeCenterOverlapGeom001SumRecDG_hpp
#define ThreeCenterOverlapGeom001SumRecDG_hpp
...
/// @brief Computes (D|G(r)|d^(1)/dB^(1)G)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_geom_01_dg(T& distributor,
                            ...
                            const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(669, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3ovlrec::comp_prim_overlap_pg(pbuffer, 105, 11, 21, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ph(pbuffer, 150, 21, 36, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_df(pbuffer, 213, 11, 57, 75, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dh(pbuffer, 273, 36, 105, 150, factors, 11, a_exp, exgtos[l]);

                    t2cgeom::comp_geom_deriv_0x1_dg(pbuffer, 399, 213, 273, factors, 1);

                    t2cfunc::reduce(cbuffer, pbuffer, 399, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGeom001SumRecDG_hpp */
