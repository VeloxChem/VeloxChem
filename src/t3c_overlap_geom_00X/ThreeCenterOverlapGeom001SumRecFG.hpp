#ifndef ThreeCenterOverlapGeom001SumRecFG_hpp
#define ThreeCenterOverlapGeom001SumRecFG_hpp
...
/// @brief Computes (F|G(r)|d^(1)/dB^(1)G)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_geom_01_fg(T& distributor,
                            ...
                            const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(1294, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3ovlrec::comp_prim_overlap_dg(pbuffer, 318, 21, 84, 114, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dh(pbuffer, 408, 36, 114, 159, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ff(pbuffer, 534, 84, 222, 258, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fh(pbuffer, 634, 159, 318, 408, factors, 11, a_exp, exgtos[l]);

                    t2cgeom::comp_geom_deriv_0x1_fg(pbuffer, 844, 534, 634, factors, 1);

                    t2cfunc::reduce(cbuffer, pbuffer, 844, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGeom001SumRecFG_hpp */
