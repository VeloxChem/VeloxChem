#ifndef TwoCenterElectronRepulsionGeom100RecDD_hpp
#define TwoCenterElectronRepulsionGeom100RecDD_hpp
...
/// @brief Computes (d^(1)/dA^(1)D|1/|r-r'||D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_electron_repulsion_geom_10_dd(T& distributor,
                                   ...
                                   const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(323, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                t2ceri::comp_prim_electron_repulsion_ps(pbuffer, 35, 2, factors, 8);

                t2ceri::comp_prim_electron_repulsion_pp(pbuffer, 38, 2, 11, factors, 8, a_exp);

                t2ceri::comp_prim_electron_repulsion_pd(pbuffer, 47, 5, 17, factors, 8, a_exp);

                t2ceri::comp_prim_electron_repulsion_pd(pbuffer, 65, 8, 23, factors, 8, a_exp);

                t2ceri::comp_prim_electron_repulsion_pd(pbuffer, 83, 11, 29, factors, 8, a_exp);

                t2ceri::comp_prim_electron_repulsion_dp(pbuffer, 101, 5, 8, 35, 38, factors, 8, a_exp);

                t2ceri::comp_prim_electron_repulsion_dd(pbuffer, 119, 17, 23, 38, 83, factors, 8, a_exp);

                t2ceri::comp_prim_electron_repulsion_fd(pbuffer, 155, 47, 65, 101, 119, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_10_dx(pbuffer, 215, 47, 155, 1, 6, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 215, ket_width, ket_npgtos);
            }
...
#endif /* TwoCenterElectronRepulsionGeom100RecDD_hpp */
