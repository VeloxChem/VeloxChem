#ifndef TwoCenterElectronRepulsionGeom100RecFP_hpp
#define TwoCenterElectronRepulsionGeom100RecFP_hpp
...
/// @brief Computes (d^(1)/dA^(1)F|1/|r-r'||P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_electron_repulsion_geom_10_fp(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                t2ceri::comp_prim_electron_repulsion_ps(pbuffer, 20, 1, factors, 8);

                t2ceri::comp_prim_electron_repulsion_ps(pbuffer, 23, 2, factors, 8);

                t2ceri::comp_prim_electron_repulsion_ps(pbuffer, 26, 3, factors, 8);

                t2ceri::comp_prim_electron_repulsion_pp(pbuffer, 29, 1, 11, factors, 8, a_exp);

                t2ceri::comp_prim_electron_repulsion_pp(pbuffer, 38, 2, 14, factors, 8, a_exp);

                t2ceri::comp_prim_electron_repulsion_pp(pbuffer, 47, 3, 17, factors, 8, a_exp);

                t2ceri::comp_prim_electron_repulsion_ds(pbuffer, 56, 1, 2, 26, factors, 8, a_exp);

                t2ceri::comp_prim_electron_repulsion_dp(pbuffer, 62, 5, 8, 20, 29, factors, 8, a_exp);

                t2ceri::comp_prim_electron_repulsion_dp(pbuffer, 80, 8, 11, 23, 38, factors, 8, a_exp);

                t2ceri::comp_prim_electron_repulsion_dp(pbuffer, 98, 11, 14, 26, 47, factors, 8, a_exp);

                t2ceri::comp_prim_electron_repulsion_fs(pbuffer, 116, 20, 23, 56, factors, 8, a_exp);

                t2ceri::comp_prim_electron_repulsion_fp(pbuffer, 126, 29, 38, 56, 98, factors, 8, a_exp);

                t2ceri::comp_prim_electron_repulsion_gp(pbuffer, 156, 62, 80, 116, 126, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_10_fx(pbuffer, 201, 62, 156, 1, 3, a_exp);
...
#endif /* TwoCenterElectronRepulsionGeom100RecFP_hpp */
