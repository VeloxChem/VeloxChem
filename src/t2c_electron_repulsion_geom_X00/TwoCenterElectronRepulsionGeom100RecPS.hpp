#ifndef TwoCenterElectronRepulsionGeom100RecPS_hpp
#define TwoCenterElectronRepulsionGeom100RecPS_hpp
...
/// @brief Computes (d^(1)/dA^(1)P|1/|r-r'||S)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_electron_repulsion_geom_10_ps(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                t2ceri::comp_prim_electron_repulsion_ps(pbuffer, 3, 2, factors, 8);

                t2ceri::comp_prim_electron_repulsion_ds(pbuffer, 6, 0, 1, 3, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_10_px(pbuffer, 12, 0, 6, 1, 1, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 12, ket_width, ket_npgtos);
            }
...
#endif /* TwoCenterElectronRepulsionGeom100RecPS_hpp */
