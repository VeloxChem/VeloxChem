...
CElectronicPotentialIntegralsDriver::compute(const CMolecule& molecule, const CMolecularBasis& basis) const
{
...
void
CElectronicPotentialIntegralsDriver::compute(double* intsBatch, const CGtoBlock& braGtoBlock, const CGtoBlock& ketGtoBlock) const
{
                                                                       ...
                                                                       const CGtoContainer* ketGtoContainer) const
{
    ...
    COneIntsDistribution* distpat = new COneIntsDistribution(epotmat.values(), nrow, ncol, dstyp);
                                                                          ...
                                                                          const CGtoBlock&      ketGtoBlock) const
{
    ...
    for (int32_t i = 0; i < bragtos.getNumberOfContrGtos(); i++)
    {
        ...
        _compPrimElectronicPotentialInts(pbuffer, recvec, recidx, bftab, bargs, bvals, bord, rfacts, rab, rpa, rpb, bragtos, ketgtos, i);
                                                                      ...
                                                                      const int32_t               iContrGto) const
{
    ...
    epotrecfunc::compElectronicPotentialForSS(
        primBuffer, recPattern, recIndexes, bfTable, bfArguments, bfValues, bfOrder, osFactors, abDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (s|g(r,r')|p) integrals

    epotrecfunc::compElectronicPotentialForSP(primBuffer, recPattern, recIndexes, pbDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (p|g(r,r')|s) integrals

    epotrecfunc::compElectronicPotentialForPS(primBuffer, recPattern, recIndexes, paDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (p|g(r,r')|p) integrals

    epotrecfunc::compElectronicPotentialForPP(primBuffer, recPattern, recIndexes, osFactors, paDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (s|g(r,r')|d) integrals

    epotrecfunc::compElectronicPotentialForSD(primBuffer, recPattern, recIndexes, osFactors, pbDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (d|g(r,r')|s) integrals

    epotrecfunc::compElectronicPotentialForDS(primBuffer, recPattern, recIndexes, osFactors, paDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (p|g(r,r')|d) integrals

    epotrecfunc::compElectronicPotentialForPD(primBuffer, recPattern, recIndexes, osFactors, paDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (d|g(r,r')|p) integrals

    epotrecfunc::compElectronicPotentialForDP(primBuffer, recPattern, recIndexes, osFactors, paDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (d|g(r,r')|d) integrals

    epotrecfunc::compElectronicPotentialForDD(primBuffer, recPattern, recIndexes, osFactors, paDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (s|g(r,r')|f) integrals

    epotrecfunc::compElectronicPotentialForSF(primBuffer, recPattern, recIndexes, osFactors, pbDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (f|g(r,r')|s) integrals

    epotrecfunc::compElectronicPotentialForFS(primBuffer, recPattern, recIndexes, osFactors, paDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (p|g(r,r')|f) integrals

    epotrecfunc::compElectronicPotentialForPF(primBuffer, recPattern, recIndexes, osFactors, paDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (f|g(r,r')|p) integrals

    epotrecfunc::compElectronicPotentialForFP(primBuffer, recPattern, recIndexes, osFactors, paDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (d|g(r,r')|f) integrals

    epotrecfunc::compElectronicPotentialForDF(primBuffer, recPattern, recIndexes, osFactors, paDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (f|g(r,r')|d) integrals

    epotrecfunc::compElectronicPotentialForFD(primBuffer, recPattern, recIndexes, osFactors, paDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (f|g(r,r')|f) integrals

    epotrecfunc::compElectronicPotentialForFF(primBuffer, recPattern, recIndexes, osFactors, paDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (s|g(r,r')|g) integrals

    epotrecfunc::compElectronicPotentialForSG(primBuffer, recPattern, recIndexes, osFactors, pbDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (g|g(r,r')|s) integrals

    epotrecfunc::compElectronicPotentialForGS(primBuffer, recPattern, recIndexes, osFactors, paDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (p|g(r,r')|g) integrals

    epotrecfunc::compElectronicPotentialForPG(primBuffer, recPattern, recIndexes, osFactors, paDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (g|g(r,r')|p) integrals

    epotrecfunc::compElectronicPotentialForGP(primBuffer, recPattern, recIndexes, osFactors, paDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (d|g(r,r')|g) integrals

    epotrecfunc::compElectronicPotentialForDG(primBuffer, recPattern, recIndexes, osFactors, paDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (g|g(r,r')|d) integrals

    epotrecfunc::compElectronicPotentialForGD(primBuffer, recPattern, recIndexes, osFactors, paDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (f|g(r,r')|g) integrals

    epotrecfunc::compElectronicPotentialForFG(primBuffer, recPattern, recIndexes, osFactors, paDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (g|g(r,r')|f) integrals

    epotrecfunc::compElectronicPotentialForGF(primBuffer, recPattern, recIndexes, osFactors, paDistances, braGtoBlock, ketGtoBlock, iContrGto);

    // compute (g|g(r,r')|g) integrals

    epotrecfunc::compElectronicPotentialForGG(primBuffer, recPattern, recIndexes, osFactors, paDistances, braGtoBlock, ketGtoBlock, iContrGto);
}

CVecThreeIndexes
CElectronicPotentialIntegralsDriver::_getRecursionPattern(const CGtoBlock& braGtoBlock, const CGtoBlock& ketGtoBlock) const
{
    ...
    while (true)
    {
        ...
        for (int32_t i = spos; i < epos; i++)
        {
            ...
            if (cidx.first() != 0)
            {
                ...
                CThreeIndexes t1idx(cidx.first() - 1,
                                    cidx.second(),
                ...
                CThreeIndexes t20idx(cidx.first() - 2,
                                     cidx.second(),
                ...
                CThreeIndexes t21idx(cidx.first() - 2,
                                     cidx.second(),
                ...
                CThreeIndexes tkidx(cidx.first() - 1,
                                    cidx.second() - 1,
            ...
            else
            {
                ...
                CThreeIndexes t1idx(cidx.first(),
                                    cidx.second() - 1,
                ...
                CThreeIndexes t20idx(cidx.first(),
                                     cidx.second() - 2,
                ...
                CThreeIndexes t21idx(cidx.first(),
                                     cidx.second() - 2,
                                                                    ...
                                                                    const int32_t           maxPrimGtos) const
{
    ...
    for (size_t i = 0; i < recPattern.size(); i++)
    {
        ...
        nblk += maxPrimGtos * angmom::to_CartesianComponents(recPattern[i].first(), recPattern[i].second());
    }
