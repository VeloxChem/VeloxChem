#ifndef ElectronRepulsionGeom2000RecSDFF_hpp
#define ElectronRepulsionGeom2000RecSDFF_hpp
...
/// @brief Computes d^(2)/dA^(2)(SD|1/|r-r'||FF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_sdff(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(14356, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(7693, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 150, pbuffer, 2049, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 276, pbuffer, 2427, 168, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 444, pbuffer, 1599, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 504, pbuffer, 1779, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 594, pbuffer, 2049, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 720, pbuffer, 2427, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 888, pbuffer, 2991, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 988, pbuffer, 3191, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1138, pbuffer, 3491, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1348, pbuffer, 3911, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1628, pbuffer, 4471, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1778, pbuffer, 4621, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2003, pbuffer, 4846, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2318, pbuffer, 5161, 420, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<3, 3>(skbuffer, 1764, ckbuffer, 1728, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 2058, ckbuffer, 4056, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 2352, ckbuffer, 7536, 0, 3);

            t4cfunc::ket_transform<3, 3>(skbuffer, 2842, ckbuffer, 12856, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 3577, 2058, 2352, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 4459, 2352, 2842, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 5929, 3577, 4459, r_ab, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 1764, 5929, 2, 3, 3);
...
#endif /* ElectronRepulsionGeom2000RecSDFF_hpp */
