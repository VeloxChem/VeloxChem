#ifndef ElectronRepulsionGeom2000RecSSDF_hpp
#define ElectronRepulsionGeom2000RecSSDF_hpp
...
/// @brief Computes d^(2)/dA^(2)(SS|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_ssdf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(1225, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(4.0 * a_exp * a_exp, {256, 286});

                pbuffer.scale(4.0 * a_exp * a_exp, {316, 361});

                pbuffer.scale(4.0 * a_exp * a_exp, {406, 469});

                pbuffer.scale(4.0 * a_exp * a_exp, {532, 592});

                pbuffer.scale(4.0 * a_exp * a_exp, {592, 682});

                pbuffer.scale(4.0 * a_exp * a_exp, {682, 808});
            ...
            t4cfunc::ket_transform<2, 3>(skbuffer, 210, ckbuffer, 75, 0, 0);

            t4cfunc::ket_transform<2, 3>(skbuffer, 245, ckbuffer, 210, 0, 0);

            t4cfunc::ket_transform<2, 3>(skbuffer, 280, ckbuffer, 495, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 385, ckbuffer, 1125, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 595, 245, 280, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 700, 280, 385, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_dsxx(skbuffer, 1015, 595, 700, r_ab, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 210, 1015, 0, 2, 3);
...
#endif /* ElectronRepulsionGeom2000RecSSDF_hpp */
