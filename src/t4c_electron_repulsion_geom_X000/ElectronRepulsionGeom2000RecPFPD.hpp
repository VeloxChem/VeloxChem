#ifndef ElectronRepulsionGeom2000RecPFPD_hpp
#define ElectronRepulsionGeom2000RecPFPD_hpp
...
/// @brief Computes d^(2)/dA^(2)(PF|1/|r-r'||PD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_pfpd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(11205, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<1, 2>(skbuffer, 4950, ckbuffer, 0, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 5100, ckbuffer, 180, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 5775, ckbuffer, 450, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 5925, ckbuffer, 630, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 6150, ckbuffer, 900, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 6465, ckbuffer, 1278, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 5325, 4950, 5100, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 6885, 5775, 5925, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 7335, 5925, 6150, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 8010, 6150, 6465, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 8955, 6885, 7335, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 9855, 7335, 8010, r_ab, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 4950, 8955, 3, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 900, 5100, 9855, 4, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 2250, 5325, 0, 900, r_ab, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 2250, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 315, skbuffer, 2700, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 630, skbuffer, 3150, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 945, skbuffer, 3600, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1260, skbuffer, 4050, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1575, skbuffer, 4500, 1, 2);
...
#endif /* ElectronRepulsionGeom2000RecPFPD_hpp */
