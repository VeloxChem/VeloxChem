#ifndef ElectronRepulsionGeom2000RecFSPP_hpp
#define ElectronRepulsionGeom2000RecFSPP_hpp
...
/// @brief Computes d^(2)/dA^(2)(FS|1/|r-r'||PP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_fspp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(8064, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 45, pbuffer, 77, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 54, pbuffer, 122, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 72, pbuffer, 230, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 90, pbuffer, 302, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 126, pbuffer, 466, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 156, pbuffer, 556, 60, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 216, pbuffer, 8, 3, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 219, pbuffer, 29, 6, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 225, pbuffer, 77, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 234, pbuffer, 122, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 252, pbuffer, 230, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 270, pbuffer, 302, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 306, pbuffer, 466, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 336, pbuffer, 556, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 396, pbuffer, 751, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 441, pbuffer, 841, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 531, pbuffer, 1021, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 594, pbuffer, 1084, 126, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 36, cbuffer, 36, 39, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 45, cbuffer, 45, 54, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 72, cbuffer, 72, 90, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 126, cbuffer, 126, 156, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 216, cbuffer, 216, 219, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 225, cbuffer, 225, 234, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 252, cbuffer, 252, 270, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 306, cbuffer, 306, 336, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 396, cbuffer, 396, 441, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 531, cbuffer, 531, 594, cfactors, 6, 0, 5);

            t4cfunc::ket_transform<1, 1>(skbuffer, 0, ckbuffer, 0, 0, 0);

            t4cfunc::ket_transform<1, 1>(skbuffer, 63, ckbuffer, 9, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 5085, ckbuffer, 36, 0, 0);

            t4cfunc::ket_transform<1, 1>(skbuffer, 5094, ckbuffer, 45, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 5121, ckbuffer, 72, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 5175, ckbuffer, 126, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 5535, ckbuffer, 216, 0, 0);

            t4cfunc::ket_transform<1, 1>(skbuffer, 5544, ckbuffer, 225, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 5571, ckbuffer, 252, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 5625, ckbuffer, 306, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 5715, ckbuffer, 396, 0, 4);

            t4cfunc::ket_transform<1, 1>(skbuffer, 5850, ckbuffer, 531, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 1116, 0, 63, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 5265, 5085, 5094, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 5292, 5094, 5121, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 5373, 5121, 5175, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 6039, 5535, 5544, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 6066, 5544, 5571, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 6147, 5571, 5625, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 6309, 5625, 5715, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 6579, 5715, 5850, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_dsxx(skbuffer, 6984, 6039, 6066, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 7038, 6066, 6147, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 7200, 6147, 6309, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 7524, 6309, 6579, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_psxx(skbuffer, 1143, 0, 5265, 5292, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 1386, 63, 5292, 5373, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dsxx(skbuffer, 3087, 1116, 1143, 1386, r_ab, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 9, 5085, 6984, 0, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 90, 5094, 7038, 1, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 252, 5121, 7200, 2, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 576, 5175, 7524, 3, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_psxx(skbuffer, 1224, 5265, 9, 90, r_ab, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ppxx(skbuffer, 1629, 5292, 90, 252, r_ab, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 2115, 5373, 252, 576, r_ab, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dsxx(skbuffer, 3249, 1143, 1224, 1629, r_ab, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dpxx(skbuffer, 3573, 1386, 1629, 2115, r_ab, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_fsxx(skbuffer, 4545, 3087, 3249, 3573, r_ab, 1, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 0, skbuffer, 4545, 1, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 63, skbuffer, 4635, 1, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 126, skbuffer, 4725, 1, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 189, skbuffer, 4815, 1, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 252, skbuffer, 4905, 1, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 315, skbuffer, 4995, 1, 1);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 3, 0, 1, 1, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom2000RecFSPP_hpp */
