#ifndef ElectronRepulsionGeom1000RecPFDF_hpp
#define ElectronRepulsionGeom1000RecPFDF_hpp
...
/// @brief Computes d^(1)/dA^(1)(PF|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1000_pfdf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(7735, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * a_exp, {4375, 4600});

                pbuffer.scale(2.0 * a_exp, {4825, 5140});

                pbuffer.scale(2.0 * a_exp, {5455, 5665});

                pbuffer.scale(2.0 * a_exp, {5665, 5980});

                pbuffer.scale(2.0 * a_exp, {5980, 6421});
            ...
            t4cfunc::ket_transform<2, 3>(skbuffer, 350, ckbuffer, 2100, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 700, ckbuffer, 3825, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 1225, ckbuffer, 6300, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 1960, 350, 700, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 3010, 700, 1225, r_ab, 2, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 4585, 0, 1960, 3010, r_ab, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 4585, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 735, skbuffer, 5635, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1470, skbuffer, 6685, 2, 3);
...
#endif /* ElectronRepulsionGeom1000RecPFDF_hpp */
