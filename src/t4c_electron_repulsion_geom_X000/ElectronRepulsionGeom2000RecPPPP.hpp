#ifndef ElectronRepulsionGeom2000RecPPPP_hpp
#define ElectronRepulsionGeom2000RecPPPP_hpp
...
/// @brief Computes d^(2)/dA^(2)(PP|1/|r-r'||PP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_pppp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(2439, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<1, 1>(skbuffer, 972, ckbuffer, 0, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 999, ckbuffer, 27, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 1134, ckbuffer, 81, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 1161, ckbuffer, 108, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 1215, ckbuffer, 162, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 1305, ckbuffer, 252, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 1053, 972, 999, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 1440, 1134, 1161, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 1521, 1161, 1215, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 1683, 1215, 1305, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 1953, 1440, 1521, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 2115, 1521, 1683, r_ab, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 972, 1953, 1, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 162, 999, 2115, 2, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ppxx(skbuffer, 486, 1053, 0, 162, r_ab, 1, 1);

            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 486, 1, 1);

            t4cfunc::bra_transform<1, 1>(sbuffer, 81, skbuffer, 567, 1, 1);

            t4cfunc::bra_transform<1, 1>(sbuffer, 162, skbuffer, 648, 1, 1);

            t4cfunc::bra_transform<1, 1>(sbuffer, 243, skbuffer, 729, 1, 1);

            t4cfunc::bra_transform<1, 1>(sbuffer, 324, skbuffer, 810, 1, 1);

            t4cfunc::bra_transform<1, 1>(sbuffer, 405, skbuffer, 891, 1, 1);
...
#endif /* ElectronRepulsionGeom2000RecPPPP_hpp */
