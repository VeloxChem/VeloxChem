...
/// @brief Computes d^(2)/dA^(2)(SD|1/|r-r'||SP)  integral derivatives.
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(471, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<0, 1>(skbuffer, 108, cbuffer, 0, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 126, cbuffer, 18, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 144, cbuffer, 36, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 174, cbuffer, 66, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 219, 126, 144, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 273, 144, 174, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 363, 219, 273, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 108, 363, 2, 0, 1);
