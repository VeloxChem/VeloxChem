#ifndef ElectronRepulsionGeom1000RecPFSS_hpp
#define ElectronRepulsionGeom1000RecPFSS_hpp
...
/// @brief Computes (PF|1/|r-r'||SS)  integral derivatives for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_geom_1000_pfss(T&                               distributor,
                                       ...
                                       const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(56, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(221, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 20, pbuffer, 75, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 35, pbuffer, 105, 21, ket_width, ket_npgtos);
            }

            t4cfunc::ket_transform<0, 0>(skbuffer, 0, cbuffer, 0, 0, 3);
            
            t4cfunc::ket_transform<0, 0>(skbuffer, 10, cbuffer, 10, 0, 3);
            
            t4cfunc::ket_transform<0, 0>(skbuffer, 20, cbuffer, 20, 0, 4);
            
            t4cfunc::ket_transform<0, 0>(skbuffer, 35, cbuffer, 35, 0, 5);
            
            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 56, 10, 20, r_ab, 0, 0);
            
            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 86, 20, 35, r_ab, 0, 0);
            
            erirec::comp_bra_geom_hrr_electron_repulsion_pfxx(skbuffer, 131, 56, 86, 0, r_ab, 0, 0);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 131, 0, 0);
            
            t4cfunc::bra_transform<1, 3>(sbuffer, 21, skbuffer, 161, 0, 0);
            
            t4cfunc::bra_transform<1, 3>(sbuffer, 42, skbuffer, 191, 0, 0);
...
#endif /* ElectronRepulsionGeom1000RecPFSS_hpp */
