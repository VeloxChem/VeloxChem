#ifndef ElectronRepulsionGeom2000RecFDSD_hpp
#define ElectronRepulsionGeom2000RecFDSD_hpp
...
/// @brief Computes d^(2)/dA^(2)(FD|1/|r-r'||SD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_fdsd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(14525, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 394, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 36, pbuffer, 760, 60, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 96, pbuffer, 394, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 132, pbuffer, 760, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 192, pbuffer, 1225, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 282, pbuffer, 1732, 126, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 720, pbuffer, 2194, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 888, pbuffer, 2530, 216, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 2>(skbuffer, 0, cbuffer, 0, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 210, cbuffer, 36, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 10460, cbuffer, 96, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 10490, cbuffer, 132, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 10540, cbuffer, 192, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 10615, cbuffer, 282, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 11185, cbuffer, 408, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 11215, cbuffer, 444, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 11265, cbuffer, 504, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 11340, cbuffer, 594, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 11445, cbuffer, 720, 0, 6);

            t4cfunc::ket_transform<0, 2>(skbuffer, 11585, cbuffer, 888, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 1640, 0, 210, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 10720, 10460, 10490, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 10810, 10490, 10540, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 10960, 10540, 10615, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 11765, 11185, 11215, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 11855, 11215, 11265, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 12005, 11265, 11340, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 12230, 11340, 11445, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pixx(skbuffer, 12545, 11445, 11585, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 12965, 11765, 11855, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 13145, 11855, 12005, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 13445, 12005, 12230, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_dhxx(skbuffer, 13895, 12230, 12545, r_ab, 0, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 1730, 0, 10720, 10810, r_ab, 0, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 2540, 210, 10810, 10960, r_ab, 0, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ddxx(skbuffer, 5240, 1640, 1730, 2540, r_ab, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 30, 10460, 12965, 2, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 260, 10490, 13145, 3, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 560, 10540, 13445, 4, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 1010, 10615, 13895, 5, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 2000, 10720, 30, 260, r_ab, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 2990, 10810, 260, 560, r_ab, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pgxx(skbuffer, 3890, 10960, 560, 1010, r_ab, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ddxx(skbuffer, 5780, 1730, 2000, 2990, r_ab, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dfxx(skbuffer, 6860, 2540, 2990, 3890, r_ab, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_fdxx(skbuffer, 8660, 5240, 5780, 6860, r_ab, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 0, skbuffer, 8660, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 175, skbuffer, 8960, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 350, skbuffer, 9260, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 525, skbuffer, 9560, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 700, skbuffer, 9860, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 875, skbuffer, 10160, 0, 2);
...
#endif /* ElectronRepulsionGeom2000RecFDSD_hpp */
