#ifndef ElectronRepulsionGeom2000RecPDSF_hpp
#define ElectronRepulsionGeom2000RecPDSF_hpp
...
/// @brief Computes d^(2)/dA^(2)(PD|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_pdsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(3353, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 540, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 60, pbuffer, 930, 100, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 160, pbuffer, 540, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 220, pbuffer, 930, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 320, pbuffer, 1320, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 470, pbuffer, 1620, 210, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 3>(skbuffer, 1428, cbuffer, 0, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1470, cbuffer, 60, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1666, cbuffer, 160, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1708, cbuffer, 220, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1778, cbuffer, 320, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1883, cbuffer, 470, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 1540, 1428, 1470, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 2030, 1666, 1708, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 2156, 1708, 1778, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 2366, 1778, 1883, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 2681, 2030, 2156, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 2933, 2156, 2366, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 1428, 2681, 2, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 252, 1470, 2933, 3, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 672, 1540, 0, 252, r_ab, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 0, skbuffer, 672, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 105, skbuffer, 798, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 210, skbuffer, 924, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 315, skbuffer, 1050, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 420, skbuffer, 1176, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 525, skbuffer, 1302, 0, 3);
...
#endif /* ElectronRepulsionGeom2000RecPDSF_hpp */
