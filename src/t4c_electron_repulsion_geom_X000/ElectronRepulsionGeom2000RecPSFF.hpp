#ifndef ElectronRepulsionGeom2000RecPSFF_hpp
#define ElectronRepulsionGeom2000RecPSFF_hpp
...
/// @brief Computes d^(2)/dA^(2)(PS|1/|r-r'||FF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_psff(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(9312, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(6027, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(4.0 * a_exp * a_exp, {1209, 1269});

                pbuffer.scale(4.0 * a_exp * a_exp, {1329, 1419});

                pbuffer.scale(4.0 * a_exp * a_exp, {1509, 1635});

                pbuffer.scale(4.0 * a_exp * a_exp, {1761, 1929});

                pbuffer.scale(4.0 * a_exp * a_exp, {2097, 2197});

                pbuffer.scale(4.0 * a_exp * a_exp, {2197, 2347});

                pbuffer.scale(4.0 * a_exp * a_exp, {2347, 2557});

                pbuffer.scale(4.0 * a_exp * a_exp, {2557, 2837});

                t2cfunc::reduce(cbuffer, 296, pbuffer, 85, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 306, pbuffer, 155, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 321, pbuffer, 245, 21, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 342, pbuffer, 350, 28, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 370, pbuffer, 507, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 400, pbuffer, 597, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 445, pbuffer, 732, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 508, pbuffer, 921, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 592, pbuffer, 1209, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 652, pbuffer, 1329, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 742, pbuffer, 1509, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 868, pbuffer, 1761, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1036, pbuffer, 2097, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1136, pbuffer, 2197, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1286, pbuffer, 2347, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1496, pbuffer, 2557, 280, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<3, 3>(skbuffer, 2058, ckbuffer, 288, 0, 0);

            t4cfunc::ket_transform<3, 3>(skbuffer, 2107, ckbuffer, 1252, 0, 1);

            t4cfunc::ket_transform<3, 3>(skbuffer, 2401, ckbuffer, 1840, 0, 0);

            t4cfunc::ket_transform<3, 3>(skbuffer, 2450, ckbuffer, 2804, 0, 1);

            t4cfunc::ket_transform<3, 3>(skbuffer, 2597, ckbuffer, 4832, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 2891, ckbuffer, 8312, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 2254, 2058, 2107, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 3381, 2401, 2450, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 3528, 2450, 2597, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 3969, 2597, 2891, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_dsxx(skbuffer, 4851, 3381, 3528, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 5145, 3528, 3969, r_ab, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 2058, 4851, 0, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 294, 2107, 5145, 1, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_psxx(skbuffer, 1176, 2254, 0, 294, r_ab, 3, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 0, skbuffer, 1176, 3, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 147, skbuffer, 1323, 3, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 294, skbuffer, 1470, 3, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 441, skbuffer, 1617, 3, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 588, skbuffer, 1764, 3, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 735, skbuffer, 1911, 3, 3);
...
#endif /* ElectronRepulsionGeom2000RecPSFF_hpp */
