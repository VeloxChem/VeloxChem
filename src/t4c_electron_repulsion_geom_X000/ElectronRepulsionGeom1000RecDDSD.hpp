...
/// @brief Computes d^(1)/dA^(1)(DD|1/|r-r'||SD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1000_ddsd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<0, 2>(skbuffer, 170, cbuffer, 96, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 200, cbuffer, 132, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 250, cbuffer, 192, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 325, cbuffer, 282, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 80, 0, 30, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 430, 170, 200, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 520, 200, 250, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 670, 250, 325, r_ab, 0, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 895, 0, 430, 520, r_ab, 0, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 1165, 30, 520, 670, r_ab, 0, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ddxx(skbuffer, 1615, 80, 895, 1165, r_ab, 0, 2);

            t4cfunc::bra_transform<2, 2>(sbuffer, 0, skbuffer, 1615, 0, 2);

            t4cfunc::bra_transform<2, 2>(sbuffer, 125, skbuffer, 1795, 0, 2);

            t4cfunc::bra_transform<2, 2>(sbuffer, 250, skbuffer, 1975, 0, 2);
