#ifndef ElectronRepulsionGeom1000RecDDSD_hpp
#define ElectronRepulsionGeom1000RecDDSD_hpp
...
/// @brief Computes (DD|1/|r-r'||SD)  integral derivatives for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_geom_1000_ddsd(T&                               distributor,
                                       ...
                                       const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(2185, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<0, 2>(skbuffer, 80, cbuffer, 96, 0, 2);
            
            t4cfunc::ket_transform<0, 2>(skbuffer, 110, cbuffer, 132, 0, 3);
            
            t4cfunc::ket_transform<0, 2>(skbuffer, 160, cbuffer, 192, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 235, cbuffer, 282, 0, 5);
            
            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 340, 0, 30, r_ab, 0, 2);
            
            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 430, 80, 110, r_ab, 0, 2);
          
            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 520, 110, 160, r_ab, 0, 2);
            
            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 670, 160, 235, r_ab, 0, 2);
            
            erirec::comp_bra_geom_hrr_electron_repulsion_pdxx(skbuffer, 925, 430, 520, 0, r_ab, 0, 2);
            
            erirec::comp_bra_geom_hrr_electron_repulsion_pfxx(skbuffer, 1195, 520, 670, 30, r_ab, 0, 2);
            
            erirec::comp_bra_geom_hrr_electron_repulsion_ddxx(skbuffer, 1645, 925, 1195, 340, r_ab, 0, 2);

            t4cfunc::bra_transform<2, 2>(sbuffer, 0, skbuffer, 1645, 0, 2);
            
            t4cfunc::bra_transform<2, 2>(sbuffer, 125, skbuffer, 1825, 0, 2);
            
            t4cfunc::bra_transform<2, 2>(sbuffer, 250, skbuffer, 2005, 0, 2);
...
#endif /* ElectronRepulsionGeom1000RecDDSD_hpp */
