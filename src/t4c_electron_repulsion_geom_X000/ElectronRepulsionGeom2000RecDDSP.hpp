#ifndef ElectronRepulsionGeom2000RecDDSP_hpp
#define ElectronRepulsionGeom2000RecDDSP_hpp
...
/// @brief Computes d^(2)/dA^(2)(DD|1/|r-r'||SP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_ddsp(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 36, pbuffer, 242, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 66, pbuffer, 392, 45, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 204, pbuffer, 548, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 267, pbuffer, 674, 84, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 1>(skbuffer, 0, cbuffer, 0, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2250, cbuffer, 18, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2268, cbuffer, 36, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2298, cbuffer, 66, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2487, cbuffer, 111, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2505, cbuffer, 129, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2535, cbuffer, 159, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2580, cbuffer, 204, 0, 5);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2643, cbuffer, 267, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 2343, 2250, 2268, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 2397, 2268, 2298, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 2727, 2487, 2505, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 2781, 2505, 2535, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 2871, 2535, 2580, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 3006, 2580, 2643, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 3195, 2727, 2781, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 3303, 2781, 2871, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 3483, 2871, 3006, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 576, 0, 2343, 2397, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 18, 2250, 3195, 2, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 126, 2268, 3303, 3, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 306, 2298, 3483, 4, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 738, 2343, 18, 126, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 1062, 2397, 126, 306, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ddxx(skbuffer, 1602, 576, 738, 1062, r_ab, 0, 1);

            t4cfunc::bra_transform<2, 2>(sbuffer, 0, skbuffer, 1602, 0, 1);

            t4cfunc::bra_transform<2, 2>(sbuffer, 75, skbuffer, 1710, 0, 1);

            t4cfunc::bra_transform<2, 2>(sbuffer, 150, skbuffer, 1818, 0, 1);

            t4cfunc::bra_transform<2, 2>(sbuffer, 225, skbuffer, 1926, 0, 1);

            t4cfunc::bra_transform<2, 2>(sbuffer, 300, skbuffer, 2034, 0, 1);

            t4cfunc::bra_transform<2, 2>(sbuffer, 375, skbuffer, 2142, 0, 1);
...
#endif /* ElectronRepulsionGeom2000RecDDSP_hpp */
