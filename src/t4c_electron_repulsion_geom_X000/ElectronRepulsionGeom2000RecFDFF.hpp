#ifndef ElectronRepulsionGeom2000RecFDFF_hpp
#define ElectronRepulsionGeom2000RecFDFF_hpp
...
/// @brief Computes d^(2)/dA^(2)(FD|1/|r-r'||FF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_fdff(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(13616, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(71392, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(142345, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 1184, pbuffer, 2787, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1244, pbuffer, 3147, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1334, pbuffer, 3687, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1460, pbuffer, 4443, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1628, pbuffer, 5801, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1728, pbuffer, 6301, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1878, pbuffer, 7051, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2088, pbuffer, 8101, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2368, pbuffer, 9876, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2518, pbuffer, 10476, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2743, pbuffer, 11376, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3058, pbuffer, 12636, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3478, pbuffer, 14631, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3688, pbuffer, 15261, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4003, pbuffer, 16206, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4444, pbuffer, 17529, 588, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 8880, pbuffer, 19461, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 9160, pbuffer, 20021, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 9580, pbuffer, 20861, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 10168, pbuffer, 22037, 784, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 10952, pbuffer, 23605, 360, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 11312, pbuffer, 23965, 540, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 11852, pbuffer, 24505, 756, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 12608, pbuffer, 25261, 1008, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 26384, cbuffer, 5032, 5092, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 26564, cbuffer, 5092, 5182, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 26834, cbuffer, 5182, 5308, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 27212, 26384, 26564, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 27572, 26564, 26834, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 28112, 27212, 27572, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 28712, cbuffer, 5476, 5576, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 29012, cbuffer, 5576, 5726, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 29462, cbuffer, 5726, 5936, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 30092, 28712, 29012, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 30692, 29012, 29462, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 31592, 30092, 30692, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 32592, cbuffer, 6216, 6366, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 33042, cbuffer, 6366, 6591, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 33717, cbuffer, 6591, 6906, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 34662, 32592, 33042, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 35562, 33042, 33717, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 36912, 34662, 35562, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 38412, cbuffer, 7326, 7536, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 39042, cbuffer, 7536, 7851, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 39987, cbuffer, 7851, 8292, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 41310, 38412, 39042, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 42570, 39042, 39987, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 44460, 41310, 42570, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 46560, cbuffer, 8880, 9160, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 47400, cbuffer, 9160, 9580, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 48660, cbuffer, 9580, 10168, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 50424, 46560, 47400, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 52104, 47400, 48660, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 54624, 50424, 52104, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 57424, cbuffer, 10952, 11312, cfactors, 6, 0, 7);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 58504, cbuffer, 11312, 11852, cfactors, 6, 0, 7);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 60124, cbuffer, 11852, 12608, cfactors, 6, 0, 7);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 62392, 57424, 58504, cfactors, 6, 0, 7);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 64552, 58504, 60124, cfactors, 6, 0, 7);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 67792, 62392, 64552, cfactors, 6, 0, 7);

            t4cfunc::ket_transform<3, 3>(skbuffer, 0, ckbuffer, 1728, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 2058, ckbuffer, 5208, 0, 3);

            t4cfunc::ket_transform<3, 3>(skbuffer, 102508, ckbuffer, 7936, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 102802, ckbuffer, 11416, 0, 3);

            t4cfunc::ket_transform<3, 3>(skbuffer, 103292, ckbuffer, 16736, 0, 4);

            t4cfunc::ket_transform<3, 3>(skbuffer, 104027, ckbuffer, 24284, 0, 5);

            t4cfunc::ket_transform<3, 3>(skbuffer, 109613, ckbuffer, 28112, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 109907, ckbuffer, 31592, 0, 3);

            t4cfunc::ket_transform<3, 3>(skbuffer, 110397, ckbuffer, 36912, 0, 4);

            t4cfunc::ket_transform<3, 3>(skbuffer, 111132, ckbuffer, 44460, 0, 5);

            t4cfunc::ket_transform<3, 3>(skbuffer, 112161, ckbuffer, 54624, 0, 6);

            t4cfunc::ket_transform<3, 3>(skbuffer, 113533, ckbuffer, 67792, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 16072, 0, 2058, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 105056, 102508, 102802, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 105938, 102802, 103292, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 107408, 103292, 104027, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 115297, 109613, 109907, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 116179, 109907, 110397, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 117649, 110397, 111132, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 119854, 111132, 112161, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pixx(skbuffer, 122941, 112161, 113533, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 127057, 115297, 116179, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 128821, 116179, 117649, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 131761, 117649, 119854, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_dhxx(skbuffer, 136171, 119854, 122941, r_ab, 3, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 16954, 0, 105056, 105938, r_ab, 3, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 24892, 2058, 105938, 107408, r_ab, 3, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ddxx(skbuffer, 51352, 16072, 16954, 24892, r_ab, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 294, 102508, 127057, 2, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 2548, 102802, 128821, 3, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 5488, 103292, 131761, 4, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 9898, 104027, 136171, 5, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 19600, 105056, 294, 2548, r_ab, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 29302, 105938, 2548, 5488, r_ab, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pgxx(skbuffer, 38122, 107408, 5488, 9898, r_ab, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ddxx(skbuffer, 56644, 16954, 19600, 29302, r_ab, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dfxx(skbuffer, 67228, 24892, 29302, 38122, r_ab, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_fdxx(skbuffer, 84868, 51352, 56644, 67228, r_ab, 3, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 0, skbuffer, 84868, 3, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1715, skbuffer, 87808, 3, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 3430, skbuffer, 90748, 3, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 5145, skbuffer, 93688, 3, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 6860, skbuffer, 96628, 3, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 8575, skbuffer, 99568, 3, 3);
...
#endif /* ElectronRepulsionGeom2000RecFDFF_hpp */
