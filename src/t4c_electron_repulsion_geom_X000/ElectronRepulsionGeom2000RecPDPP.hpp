#ifndef ElectronRepulsionGeom2000RecPDPP_hpp
#define ElectronRepulsionGeom2000RecPDPP_hpp
...
/// @brief Computes d^(2)/dA^(2)(PD|1/|r-r'||PP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_pdpp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(4311, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<1, 1>(skbuffer, 1836, ckbuffer, 0, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 1890, ckbuffer, 54, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 2142, ckbuffer, 144, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 2196, ckbuffer, 198, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 2286, ckbuffer, 288, 0, 4);

            t4cfunc::ket_transform<1, 1>(skbuffer, 2421, ckbuffer, 423, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 1980, 1836, 1890, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 2610, 2142, 2196, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 2772, 2196, 2286, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 3042, 2286, 2421, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 3447, 2610, 2772, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 3771, 2772, 3042, r_ab, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 1836, 3447, 2, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 324, 1890, 3771, 3, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 864, 1980, 0, 324, r_ab, 1, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 0, skbuffer, 864, 1, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 135, skbuffer, 1026, 1, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 270, skbuffer, 1188, 1, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 405, skbuffer, 1350, 1, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 540, skbuffer, 1512, 1, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 675, skbuffer, 1674, 1, 1);
...
#endif /* ElectronRepulsionGeom2000RecPDPP_hpp */
