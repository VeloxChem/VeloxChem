...
/// @brief Computes (DD|1/|r-r'||SS)  integral derivatives for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_geom_1000_ddss(T&                               distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * a_exp, {75, 90});
                
                pbuffer.scale(2.0 * a_exp, {105, 126});
            ...
            t4cfunc::ket_transform<0, 0>(skbuffer, 16, cbuffer, 16, 0, 2);
            
            t4cfunc::ket_transform<0, 0>(skbuffer, 22, cbuffer, 22, 0, 3);
            
            t4cfunc::ket_transform<0, 0>(skbuffer, 32, cbuffer, 32, 0, 4);
            
            t4cfunc::ket_transform<0, 0>(skbuffer, 47, cbuffer, 47, 0, 5);
            
            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 68, 0, 6, r_ab, 0, 0);
            
            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 86, 16, 22, r_ab, 0, 0);
          
            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 104, 22, 32, r_ab, 0, 0);
            
            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 134, 32, 47, r_ab, 0, 0);
            
            erirec::comp_bra_geom_hrr_electron_repulsion_pdxx(skbuffer, 179, 86, 104, 0, r_ab, 0, 0);
            
            erirec::comp_bra_geom_hrr_electron_repulsion_pfxx(skbuffer, 233, 104, 134, 6, r_ab, 0, 0);
            
            erirec::comp_bra_geom_hrr_electron_repulsion_ddxx(skbuffer, 323, 179, 233, 68, r_ab, 0, 0);

            t4cfunc::bra_transform<2, 2>(sbuffer, 0, skbuffer, 323, 0, 0);
            
            t4cfunc::bra_transform<2, 2>(sbuffer, 25, skbuffer, 359, 0, 0);
            
            t4cfunc::bra_transform<2, 2>(sbuffer, 50, skbuffer, 395, 0, 0);
