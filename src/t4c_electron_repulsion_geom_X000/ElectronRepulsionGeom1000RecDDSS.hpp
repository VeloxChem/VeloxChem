...
/// @brief Computes d^(1)/dA^(1)(DD|1/|r-r'||SS)  integral derivatives.
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<0, 0>(skbuffer, 34, cbuffer, 16, 0, 2);

            t4cfunc::ket_transform<0, 0>(skbuffer, 40, cbuffer, 22, 0, 3);

            t4cfunc::ket_transform<0, 0>(skbuffer, 50, cbuffer, 32, 0, 4);

            t4cfunc::ket_transform<0, 0>(skbuffer, 65, cbuffer, 47, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 16, 0, 6, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 86, 34, 40, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 104, 40, 50, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 134, 50, 65, r_ab, 0, 0);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 179, 0, 86, 104, r_ab, 0, 0);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 233, 6, 104, 134, r_ab, 0, 0);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ddxx(skbuffer, 323, 16, 179, 233, r_ab, 0, 0);
