#ifndef ElectronRepulsionGeom2000RecFFSF_hpp
#define ElectronRepulsionGeom2000RecFFSF_hpp
...
/// @brief Computes d^(2)/dA^(2)(FF|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_ffsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(2540, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(30611, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 1830, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 100, pbuffer, 2955, 150, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {1830, 1930});

                pbuffer.scale(2.0 * a_exp, {2955, 3105});

                pbuffer.scale(2.0 * a_exp, {4230, 4440});

                pbuffer.scale(2.0 * a_exp, {5490, 5770});

                t2cfunc::reduce(cbuffer, 250, pbuffer, 1830, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 350, pbuffer, 2955, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 500, pbuffer, 4230, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 710, pbuffer, 5490, 280, ket_width, ket_npgtos);
                ...
                pbuffer.scale(4.0 * a_exp * a_exp, {6546, 6906});

                pbuffer.scale(4.0 * a_exp * a_exp, {7266, 7716});

                t2cfunc::reduce(cbuffer, 990, pbuffer, 1830, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1090, pbuffer, 2955, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1240, pbuffer, 4230, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1450, pbuffer, 5490, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1730, pbuffer, 6546, 360, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2090, pbuffer, 7266, 450, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 3>(skbuffer, 0, cbuffer, 0, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 490, cbuffer, 100, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 22624, cbuffer, 250, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 22694, cbuffer, 350, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 22799, cbuffer, 500, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 22946, cbuffer, 710, 0, 6);

            t4cfunc::ket_transform<0, 3>(skbuffer, 24108, cbuffer, 990, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 24178, cbuffer, 1090, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 24283, cbuffer, 1240, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 24430, cbuffer, 1450, 0, 6);

            t4cfunc::ket_transform<0, 3>(skbuffer, 24626, cbuffer, 1730, 0, 7);

            t4cfunc::ket_transform<0, 3>(skbuffer, 24878, cbuffer, 2090, 0, 8);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 3283, 0, 490, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 23142, 22624, 22694, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 23352, 22694, 22799, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 23667, 22799, 22946, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 25193, 24108, 24178, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 25403, 24178, 24283, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 25718, 24283, 24430, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pixx(skbuffer, 26159, 24430, 24626, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pkxx(skbuffer, 26747, 24626, 24878, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 27503, 25193, 25403, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 27923, 25403, 25718, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dhxx(skbuffer, 28553, 25718, 26159, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dixx(skbuffer, 29435, 26159, 26747, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 3493, 0, 23142, 23352, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pgxx(skbuffer, 5383, 490, 23352, 23667, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dfxx(skbuffer, 10864, 3283, 3493, 5383, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 70, 22624, 27503, 3, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 595, 22694, 27923, 4, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 1225, 22799, 28553, 5, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 2107, 22946, 29435, 6, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 4123, 23142, 70, 595, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pgxx(skbuffer, 6328, 23352, 595, 1225, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_phxx(skbuffer, 8218, 23667, 1225, 2107, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dfxx(skbuffer, 12124, 3493, 4123, 6328, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dgxx(skbuffer, 14644, 5383, 6328, 8218, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ffxx(skbuffer, 18424, 10864, 12124, 14644, r_ab, 0, 3);

            t4cfunc::bra_transform<3, 3>(sbuffer, 0, skbuffer, 18424, 0, 3);

            t4cfunc::bra_transform<3, 3>(sbuffer, 343, skbuffer, 19124, 0, 3);

            t4cfunc::bra_transform<3, 3>(sbuffer, 686, skbuffer, 19824, 0, 3);

            t4cfunc::bra_transform<3, 3>(sbuffer, 1029, skbuffer, 20524, 0, 3);

            t4cfunc::bra_transform<3, 3>(sbuffer, 1372, skbuffer, 21224, 0, 3);

            t4cfunc::bra_transform<3, 3>(sbuffer, 1715, skbuffer, 21924, 0, 3);
...
#endif /* ElectronRepulsionGeom2000RecFFSF_hpp */
