...
/// @brief Computes d^(2)/dA^(2)(SD|1/|r-r'||SD)  integral derivatives.
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(785, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 36, pbuffer, 202, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 72, pbuffer, 340, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 132, pbuffer, 460, 90, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 2>(skbuffer, 180, cbuffer, 0, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 210, cbuffer, 36, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 240, cbuffer, 72, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 290, cbuffer, 132, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 365, 210, 240, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 455, 240, 290, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 605, 365, 455, r_ab, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 180, 605, 2, 0, 2);
