#ifndef ElectronRepulsionGeom2000RecDDPF_hpp
#define ElectronRepulsionGeom2000RecDDPF_hpp
...
/// @brief Computes d^(2)/dA^(2)(DD|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_ddpf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(2925, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(3510, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(26271, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 150, pbuffer, 1075, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 210, pbuffer, 1375, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 300, pbuffer, 2075, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 400, pbuffer, 2475, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 550, pbuffer, 3300, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 700, pbuffer, 3750, 225, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 1700, pbuffer, 4551, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1910, pbuffer, 4971, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2225, pbuffer, 5601, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2505, pbuffer, 5881, 420, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1110, cbuffer, 925, 985, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1290, cbuffer, 1075, 1175, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1590, cbuffer, 1325, 1475, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2040, cbuffer, 1700, 1910, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2670, cbuffer, 2225, 2505, cfactors, 6, 0, 6);

            t4cfunc::ket_transform<1, 3>(skbuffer, 0, ckbuffer, 0, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 15750, ckbuffer, 180, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 15876, ckbuffer, 360, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 16086, ckbuffer, 660, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 17409, ckbuffer, 1110, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 17535, ckbuffer, 1290, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 17745, ckbuffer, 1590, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 18060, ckbuffer, 2040, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 18501, ckbuffer, 2670, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 16401, 15750, 15876, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 16779, 15876, 16086, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 19089, 17409, 17535, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 19467, 17535, 17745, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 20097, 17745, 18060, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 21042, 18060, 18501, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 22365, 19089, 19467, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 23121, 19467, 20097, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 24381, 20097, 21042, r_ab, 1, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 4032, 0, 16401, 16779, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 126, 15750, 22365, 2, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 882, 15876, 23121, 3, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 2142, 16086, 24381, 4, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 5166, 16401, 126, 882, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 7434, 16779, 882, 2142, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ddxx(skbuffer, 11214, 4032, 5166, 7434, r_ab, 1, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 0, skbuffer, 11214, 1, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 525, skbuffer, 11970, 1, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 1050, skbuffer, 12726, 1, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 1575, skbuffer, 13482, 1, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 2100, skbuffer, 14238, 1, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 2625, skbuffer, 14994, 1, 3);
...
#endif /* ElectronRepulsionGeom2000RecDDPF_hpp */
