#ifndef ElectronRepulsionGeom2000RecSFSS_hpp
#define ElectronRepulsionGeom2000RecSFSS_hpp
...
/// @brief Computes d^(2)/dA^(2)(SF|1/|r-r'||SS)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_sfss(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(251, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<0, 0>(skbuffer, 60, cbuffer, 0, 0, 3);

            t4cfunc::ket_transform<0, 0>(skbuffer, 70, cbuffer, 10, 0, 3);

            t4cfunc::ket_transform<0, 0>(skbuffer, 80, cbuffer, 20, 0, 4);

            t4cfunc::ket_transform<0, 0>(skbuffer, 95, cbuffer, 35, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 116, 70, 80, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 146, 80, 95, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 191, 116, 146, r_ab, 0, 0);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 60, 191, 3, 0, 0);
...
#endif /* ElectronRepulsionGeom2000RecSFSS_hpp */
