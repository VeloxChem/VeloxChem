#ifndef ElectronRepulsionGeom2000RecFDDF_hpp
#define ElectronRepulsionGeom2000RecFDDF_hpp
...
/// @brief Computes d^(2)/dA^(2)(FD|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_fddf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(8464, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(24840, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(101675, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 736, pbuffer, 1919, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 796, pbuffer, 2279, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 886, pbuffer, 2819, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1012, pbuffer, 3925, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1112, pbuffer, 4425, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1262, pbuffer, 5175, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1472, pbuffer, 6600, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1622, pbuffer, 7200, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1847, pbuffer, 8100, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2162, pbuffer, 9675, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2372, pbuffer, 10305, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2687, pbuffer, 11250, 441, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 5520, pbuffer, 12741, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5800, pbuffer, 13301, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 6220, pbuffer, 14141, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 6808, pbuffer, 15317, 360, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 7168, pbuffer, 15677, 540, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 7708, pbuffer, 16217, 756, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 9180, cbuffer, 3128, 3188, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 9360, cbuffer, 3188, 3278, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 9630, 9180, 9360, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 9990, cbuffer, 3404, 3504, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 10290, cbuffer, 3504, 3654, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 10740, 9990, 10290, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 11340, cbuffer, 3864, 4014, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 11790, cbuffer, 4014, 4239, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 12465, 11340, 11790, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 13365, cbuffer, 4554, 4764, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 13995, cbuffer, 4764, 5079, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 14940, 13365, 13995, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 16200, cbuffer, 5520, 5800, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 17040, cbuffer, 5800, 6220, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 18300, 16200, 17040, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 19980, cbuffer, 6808, 7168, cfactors, 6, 0, 7);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 21060, cbuffer, 7168, 7708, cfactors, 6, 0, 7);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 22680, 19980, 21060, cfactors, 6, 0, 7);

            t4cfunc::ket_transform<2, 3>(skbuffer, 0, ckbuffer, 450, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 1470, ckbuffer, 1560, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 73220, ckbuffer, 2610, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 73430, ckbuffer, 3720, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 73780, ckbuffer, 5445, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 74305, ckbuffer, 7920, 0, 5);

            t4cfunc::ket_transform<2, 3>(skbuffer, 78295, ckbuffer, 9630, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 78505, ckbuffer, 10740, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 78855, ckbuffer, 12465, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 79380, ckbuffer, 14940, 0, 5);

            t4cfunc::ket_transform<2, 3>(skbuffer, 80115, ckbuffer, 18300, 0, 6);

            t4cfunc::ket_transform<2, 3>(skbuffer, 81095, ckbuffer, 22680, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 11480, 0, 1470, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 75040, 73220, 73430, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 75670, 73430, 73780, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 76720, 73780, 74305, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 82355, 78295, 78505, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 82985, 78505, 78855, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 84035, 78855, 79380, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 85610, 79380, 80115, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pixx(skbuffer, 87815, 80115, 81095, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 90755, 82355, 82985, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 92015, 82985, 84035, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 94115, 84035, 85610, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_dhxx(skbuffer, 97265, 85610, 87815, r_ab, 2, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 12110, 0, 75040, 75670, r_ab, 2, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 17780, 1470, 75670, 76720, r_ab, 2, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ddxx(skbuffer, 36680, 11480, 12110, 17780, r_ab, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 210, 73220, 90755, 2, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 1820, 73430, 92015, 3, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 3920, 73780, 94115, 4, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 7070, 74305, 97265, 5, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 14000, 75040, 210, 1820, r_ab, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 20930, 75670, 1820, 3920, r_ab, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pgxx(skbuffer, 27230, 76720, 3920, 7070, r_ab, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ddxx(skbuffer, 40460, 12110, 14000, 20930, r_ab, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dfxx(skbuffer, 48020, 17780, 20930, 27230, r_ab, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_fdxx(skbuffer, 60620, 36680, 40460, 48020, r_ab, 2, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 0, skbuffer, 60620, 2, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1225, skbuffer, 62720, 2, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 2450, skbuffer, 64820, 2, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 3675, skbuffer, 66920, 2, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 4900, skbuffer, 69020, 2, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 6125, skbuffer, 71120, 2, 3);
...
#endif /* ElectronRepulsionGeom2000RecFDDF_hpp */
