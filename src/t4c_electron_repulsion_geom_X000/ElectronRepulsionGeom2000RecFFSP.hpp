#ifndef ElectronRepulsionGeom2000RecFFSP_hpp
#define ElectronRepulsionGeom2000RecFFSP_hpp
...
/// @brief Computes d^(2)/dA^(2)(FF|1/|r-r'||SP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_ffsp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(762, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(13119, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 342, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 30, pbuffer, 582, 45, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {342, 372});

                pbuffer.scale(2.0 * a_exp, {582, 627});

                pbuffer.scale(2.0 * a_exp, {870, 933});

                pbuffer.scale(2.0 * a_exp, {1178, 1262});

                t2cfunc::reduce(cbuffer, 75, pbuffer, 342, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 105, pbuffer, 582, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 870, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 213, pbuffer, 1178, 84, ket_width, ket_npgtos);
                ...
                pbuffer.scale(4.0 * a_exp * a_exp, {1466, 1574});

                pbuffer.scale(4.0 * a_exp * a_exp, {1682, 1817});

                t2cfunc::reduce(cbuffer, 297, pbuffer, 342, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 327, pbuffer, 582, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 372, pbuffer, 870, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 435, pbuffer, 1178, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 519, pbuffer, 1466, 108, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 627, pbuffer, 1682, 135, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 1>(skbuffer, 0, cbuffer, 0, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 210, cbuffer, 30, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 9696, cbuffer, 75, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 9726, cbuffer, 105, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 9771, cbuffer, 150, 0, 5);

            t4cfunc::ket_transform<0, 1>(skbuffer, 9834, cbuffer, 213, 0, 6);

            t4cfunc::ket_transform<0, 1>(skbuffer, 10332, cbuffer, 297, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 10362, cbuffer, 327, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 10407, cbuffer, 372, 0, 5);

            t4cfunc::ket_transform<0, 1>(skbuffer, 10470, cbuffer, 435, 0, 6);

            t4cfunc::ket_transform<0, 1>(skbuffer, 10554, cbuffer, 519, 0, 7);

            t4cfunc::ket_transform<0, 1>(skbuffer, 10662, cbuffer, 627, 0, 8);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 1407, 0, 210, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 9918, 9696, 9726, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 10008, 9726, 9771, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 10143, 9771, 9834, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 10797, 10332, 10362, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 10887, 10362, 10407, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 11022, 10407, 10470, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pixx(skbuffer, 11211, 10470, 10554, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pkxx(skbuffer, 11463, 10554, 10662, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 11787, 10797, 10887, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 11967, 10887, 11022, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dhxx(skbuffer, 12237, 11022, 11211, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dixx(skbuffer, 12615, 11211, 11463, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 1497, 0, 9918, 10008, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pgxx(skbuffer, 2307, 210, 10008, 10143, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dfxx(skbuffer, 4656, 1407, 1497, 2307, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 30, 9696, 11787, 3, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 255, 9726, 11967, 4, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 525, 9771, 12237, 5, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 903, 9834, 12615, 6, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 1767, 9918, 30, 255, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pgxx(skbuffer, 2712, 10008, 255, 525, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_phxx(skbuffer, 3522, 10143, 525, 903, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dfxx(skbuffer, 5196, 1497, 1767, 2712, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dgxx(skbuffer, 6276, 2307, 2712, 3522, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ffxx(skbuffer, 7896, 4656, 5196, 6276, r_ab, 0, 1);

            t4cfunc::bra_transform<3, 3>(sbuffer, 0, skbuffer, 7896, 0, 1);

            t4cfunc::bra_transform<3, 3>(sbuffer, 147, skbuffer, 8196, 0, 1);

            t4cfunc::bra_transform<3, 3>(sbuffer, 294, skbuffer, 8496, 0, 1);

            t4cfunc::bra_transform<3, 3>(sbuffer, 441, skbuffer, 8796, 0, 1);

            t4cfunc::bra_transform<3, 3>(sbuffer, 588, skbuffer, 9096, 0, 1);

            t4cfunc::bra_transform<3, 3>(sbuffer, 735, skbuffer, 9396, 0, 1);
...
#endif /* ElectronRepulsionGeom2000RecFFSP_hpp */
