...
/// @brief Computes (SD|1/|r-r'||PD)  integral derivatives for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_geom_1000_sdpd(T&                               distributor,
                                       ...
                                       const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::bra_transform<0, 2>(sbuffer, 0, skbuffer, 240, 1, 2);
            
            t4cfunc::bra_transform<0, 2>(sbuffer, 75, skbuffer, 330, 1, 2);
            
            t4cfunc::bra_transform<0, 2>(sbuffer, 150, skbuffer, 420, 1, 2);
