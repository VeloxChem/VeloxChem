...
/// @brief Computes d^(2)/dA^(2)(PP|1/|r-r'||SS)  integral derivatives.
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(271, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<0, 0>(skbuffer, 108, cbuffer, 0, 0, 1);

            t4cfunc::ket_transform<0, 0>(skbuffer, 111, cbuffer, 3, 0, 2);

            t4cfunc::ket_transform<0, 0>(skbuffer, 126, cbuffer, 9, 0, 1);

            t4cfunc::ket_transform<0, 0>(skbuffer, 129, cbuffer, 12, 0, 2);

            t4cfunc::ket_transform<0, 0>(skbuffer, 135, cbuffer, 18, 0, 3);

            t4cfunc::ket_transform<0, 0>(skbuffer, 145, cbuffer, 28, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 117, 108, 111, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 160, 126, 129, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 169, 129, 135, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 187, 135, 145, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 217, 160, 169, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 235, 169, 187, r_ab, 0, 0);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 108, 217, 1, 0, 0);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 18, 111, 235, 2, 0, 0);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ppxx(skbuffer, 54, 117, 0, 18, r_ab, 0, 0);

            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 54, 0, 0);

            t4cfunc::bra_transform<1, 1>(sbuffer, 9, skbuffer, 63, 0, 0);

            t4cfunc::bra_transform<1, 1>(sbuffer, 18, skbuffer, 72, 0, 0);

            t4cfunc::bra_transform<1, 1>(sbuffer, 27, skbuffer, 81, 0, 0);

            t4cfunc::bra_transform<1, 1>(sbuffer, 36, skbuffer, 90, 0, 0);

            t4cfunc::bra_transform<1, 1>(sbuffer, 45, skbuffer, 99, 0, 0);
