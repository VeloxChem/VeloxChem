#ifndef ElectronRepulsionGeom2000RecDDFF_hpp
#define ElectronRepulsionGeom2000RecDDFF_hpp
...
/// @brief Computes d^(2)/dA^(2)(DD|1/|r-r'||FF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_ddff(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(8658, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(45396, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(61299, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 444, pbuffer, 2391, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 504, pbuffer, 2691, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 594, pbuffer, 3141, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 720, pbuffer, 3771, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 888, pbuffer, 4861, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 988, pbuffer, 5261, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1138, pbuffer, 5861, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1348, pbuffer, 6701, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1628, pbuffer, 8046, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1778, pbuffer, 8496, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2003, pbuffer, 9171, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2318, pbuffer, 10116, 420, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 5032, pbuffer, 11502, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5242, pbuffer, 11922, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5557, pbuffer, 12552, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5998, pbuffer, 13434, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 6586, pbuffer, 14610, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 6866, pbuffer, 14890, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 7286, pbuffer, 15310, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 7874, pbuffer, 15898, 784, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 8536, cbuffer, 1628, 1778, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 8986, cbuffer, 1778, 2003, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 9661, cbuffer, 2003, 2318, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 10606, 8536, 8986, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 11506, 8986, 9661, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 12856, 10606, 11506, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 14356, cbuffer, 2738, 2798, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 14536, cbuffer, 2798, 2888, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 14806, cbuffer, 2888, 3014, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 15184, 14356, 14536, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 15544, 14536, 14806, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 16084, 15184, 15544, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 16684, cbuffer, 3182, 3282, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 16984, cbuffer, 3282, 3432, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 17434, cbuffer, 3432, 3642, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 18064, 16684, 16984, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 18664, 16984, 17434, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 19564, 18064, 18664, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 20564, cbuffer, 3922, 4072, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 21014, cbuffer, 4072, 4297, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 21689, cbuffer, 4297, 4612, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 22634, 20564, 21014, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 23534, 21014, 21689, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 24884, 22634, 23534, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 26384, cbuffer, 5032, 5242, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 27014, cbuffer, 5242, 5557, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 27959, cbuffer, 5557, 5998, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 29282, 26384, 27014, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 30542, 27014, 27959, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 32432, 29282, 30542, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 34532, cbuffer, 6586, 6866, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 35372, cbuffer, 6866, 7286, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 36632, cbuffer, 7286, 7874, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 38396, 34532, 35372, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 40076, 35372, 36632, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 42596, 38396, 40076, cfactors, 6, 0, 6);

            t4cfunc::ket_transform<3, 3>(skbuffer, 0, ckbuffer, 1728, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 36750, ckbuffer, 4056, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 37044, ckbuffer, 7536, 0, 3);

            t4cfunc::ket_transform<3, 3>(skbuffer, 37534, ckbuffer, 12856, 0, 4);

            t4cfunc::ket_transform<3, 3>(skbuffer, 40621, ckbuffer, 16084, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 40915, ckbuffer, 19564, 0, 3);

            t4cfunc::ket_transform<3, 3>(skbuffer, 41405, ckbuffer, 24884, 0, 4);

            t4cfunc::ket_transform<3, 3>(skbuffer, 42140, ckbuffer, 32432, 0, 5);

            t4cfunc::ket_transform<3, 3>(skbuffer, 43169, ckbuffer, 42596, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 38269, 36750, 37044, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 39151, 37044, 37534, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 44541, 40621, 40915, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 45423, 40915, 41405, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 46893, 41405, 42140, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 49098, 42140, 43169, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 52185, 44541, 45423, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 53949, 45423, 46893, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 56889, 46893, 49098, r_ab, 3, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 9408, 0, 38269, 39151, r_ab, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 294, 36750, 52185, 2, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 2058, 37044, 53949, 3, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 4998, 37534, 56889, 4, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 12054, 38269, 294, 2058, r_ab, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 17346, 39151, 2058, 4998, r_ab, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ddxx(skbuffer, 26166, 9408, 12054, 17346, r_ab, 3, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 0, skbuffer, 26166, 3, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 1225, skbuffer, 27930, 3, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 2450, skbuffer, 29694, 3, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 3675, skbuffer, 31458, 3, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 4900, skbuffer, 33222, 3, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 6125, skbuffer, 34986, 3, 3);
...
#endif /* ElectronRepulsionGeom2000RecDDFF_hpp */
