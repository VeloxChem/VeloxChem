...
/// @brief Computes (PD|1/|r-r'||PD)  integral derivatives for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_geom_1000_pdpd(T&                               distributor,
                                       ...
                                       const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<1, 2>(skbuffer, 330, ckbuffer, 396, 0, 4);
            
            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 555, 90, 180, r_ab, 1, 2);
            
            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 825, 180, 330, r_ab, 1, 2);
            
            erirec::comp_bra_geom_hrr_electron_repulsion_pdxx(skbuffer, 1275, 555, 825, 0, r_ab, 1, 2);
            
            t4cfunc::bra_transform<1, 2>(sbuffer, 0, skbuffer, 1275, 1, 2);
            
            t4cfunc::bra_transform<1, 2>(sbuffer, 225, skbuffer, 1545, 1, 2);
            
            t4cfunc::bra_transform<1, 2>(sbuffer, 450, skbuffer, 1815, 1, 2);
