#ifndef ElectronRepulsionGeom2000RecSFDF_hpp
#define ElectronRepulsionGeom2000RecSFDF_hpp
...
/// @brief Computes d^(2)/dA^(2)(SF|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_sfdf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(2576, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(7560, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(8785, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 2605, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 100, pbuffer, 2905, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 250, pbuffer, 3355, 210, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 920, pbuffer, 4075, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1070, pbuffer, 4375, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1295, pbuffer, 4825, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1610, pbuffer, 5455, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1820, pbuffer, 5665, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2135, pbuffer, 5980, 441, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<2, 3>(skbuffer, 2100, ckbuffer, 750, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 2450, ckbuffer, 2100, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 2800, ckbuffer, 3825, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 3325, ckbuffer, 6300, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 4060, 2450, 2800, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 5110, 2800, 3325, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 6685, 4060, 5110, r_ab, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 2100, 6685, 3, 2, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 0, skbuffer, 0, 2, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 245, skbuffer, 350, 2, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 490, skbuffer, 700, 2, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 735, skbuffer, 1050, 2, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 980, skbuffer, 1400, 2, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 1225, skbuffer, 1750, 2, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 3, 2, 3, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom2000RecSFDF_hpp */
