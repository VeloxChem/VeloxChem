#ifndef ElectronRepulsionGeom2000RecDPPF_hpp
#define ElectronRepulsionGeom2000RecDPPF_hpp
...
/// @brief Computes d^(2)/dA^(2)(DP|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_dppf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(1925, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(2310, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(15351, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 75, pbuffer, 350, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 105, pbuffer, 500, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 875, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 210, pbuffer, 1115, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 300, pbuffer, 1625, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 400, pbuffer, 1925, 150, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 550, pbuffer, 350, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 580, pbuffer, 500, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 625, pbuffer, 875, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 685, pbuffer, 1115, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 775, pbuffer, 1625, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 875, pbuffer, 1925, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1025, pbuffer, 2465, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1175, pbuffer, 2765, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1400, pbuffer, 3215, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1610, pbuffer, 3425, 315, ket_width, ket_npgtos);

            }

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 0, cbuffer, 0, 30, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 90, cbuffer, 75, 105, cfactors, 6, 0, 1);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 660, cbuffer, 550, 580, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 750, cbuffer, 625, 685, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 930, cbuffer, 775, 875, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1230, cbuffer, 1025, 1175, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1680, cbuffer, 1400, 1610, cfactors, 6, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 0, ckbuffer, 0, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 8694, ckbuffer, 90, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 8757, ckbuffer, 180, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 8883, ckbuffer, 360, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 9660, ckbuffer, 660, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 9723, ckbuffer, 750, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 9849, ckbuffer, 930, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 10059, ckbuffer, 1230, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 10374, ckbuffer, 1680, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 9093, 8694, 8757, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 9282, 8757, 8883, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 10815, 9660, 9723, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 11004, 9723, 9849, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 11382, 9849, 10059, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 12012, 10059, 10374, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 12957, 10815, 11004, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 13335, 11004, 11382, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 14091, 11382, 12012, r_ab, 1, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 2457, 0, 9093, 9282, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 63, 8694, 12957, 1, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 441, 8757, 13335, 2, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 1197, 8883, 14091, 3, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ppxx(skbuffer, 3024, 9093, 63, 441, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 4158, 9282, 441, 1197, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dpxx(skbuffer, 6426, 2457, 3024, 4158, r_ab, 1, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 0, skbuffer, 6426, 1, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 315, skbuffer, 6804, 1, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 630, skbuffer, 7182, 1, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 945, skbuffer, 7560, 1, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 1260, skbuffer, 7938, 1, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 1575, skbuffer, 8316, 1, 3);
...
#endif /* ElectronRepulsionGeom2000RecDPPF_hpp */
