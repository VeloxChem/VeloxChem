#ifndef ElectronRepulsionGeom1000RecDFSS_hpp
#define ElectronRepulsionGeom1000RecDFSS_hpp
...
/// @brief Computes d^(1)/dA^(1)(DF|1/|r-r'||SS)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1000_dfss(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<0, 0>(skbuffer, 55, cbuffer, 25, 0, 3);

            t4cfunc::ket_transform<0, 0>(skbuffer, 65, cbuffer, 35, 0, 4);

            t4cfunc::ket_transform<0, 0>(skbuffer, 80, cbuffer, 50, 0, 5);

            t4cfunc::ket_transform<0, 0>(skbuffer, 101, cbuffer, 71, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 25, 0, 10, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 129, 55, 65, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 159, 65, 80, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 204, 80, 101, r_ab, 0, 0);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 267, 0, 129, 159, r_ab, 0, 0);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pgxx(skbuffer, 357, 10, 159, 204, r_ab, 0, 0);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dfxx(skbuffer, 492, 25, 267, 357, r_ab, 0, 0);

            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 492, 0, 0);

            t4cfunc::bra_transform<2, 3>(sbuffer, 35, skbuffer, 552, 0, 0);

            t4cfunc::bra_transform<2, 3>(sbuffer, 70, skbuffer, 612, 0, 0);
...
#endif /* ElectronRepulsionGeom1000RecDFSS_hpp */
