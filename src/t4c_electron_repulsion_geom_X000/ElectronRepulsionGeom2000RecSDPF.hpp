#ifndef ElectronRepulsionGeom2000RecSDPF_hpp
#define ElectronRepulsionGeom2000RecSDPF_hpp
...
/// @brief Computes d^(2)/dA^(2)(SD|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_sdpf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(3297, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<1, 3>(skbuffer, 756, ckbuffer, 0, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 882, ckbuffer, 180, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1008, ckbuffer, 360, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1218, ckbuffer, 660, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 1533, 882, 1008, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 1911, 1008, 1218, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 2541, 1533, 1911, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 756, 2541, 2, 1, 3);
...
#endif /* ElectronRepulsionGeom2000RecSDPF_hpp */
