#ifndef ElectronRepulsionGeom1000RecDSDD_hpp
#define ElectronRepulsionGeom1000RecDSDD_hpp
...
/// @brief Computes d^(1)/dA^(1)(DS|1/|r-r'||DD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1000_dsdd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(744, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(2016, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(2775, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * a_exp, {565, 625});

                pbuffer.scale(2.0 * a_exp, {685, 775});

                pbuffer.scale(2.0 * a_exp, {865, 925});

                pbuffer.scale(2.0 * a_exp, {925, 1025});

                pbuffer.scale(2.0 * a_exp, {1025, 1175});

                t2cfunc::reduce(cbuffer, 124, pbuffer, 29, 6, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 130, pbuffer, 65, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 140, pbuffer, 115, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 155, pbuffer, 196, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 173, pbuffer, 250, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 203, pbuffer, 340, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 248, pbuffer, 493, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 284, pbuffer, 565, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 344, pbuffer, 685, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 434, pbuffer, 865, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 494, pbuffer, 925, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 594, pbuffer, 1025, 150, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<2, 2>(skbuffer, 175, ckbuffer, 384, 0, 0);

            t4cfunc::ket_transform<2, 2>(skbuffer, 200, ckbuffer, 564, 0, 1);

            t4cfunc::ket_transform<2, 2>(skbuffer, 275, ckbuffer, 960, 0, 2);

            t4cfunc::ket_transform<2, 2>(skbuffer, 425, ckbuffer, 1656, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 100, 0, 25, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 675, 175, 200, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 750, 200, 275, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 975, 275, 425, r_ab, 2, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_psxx(skbuffer, 1425, 0, 675, 750, r_ab, 2, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 1650, 25, 750, 975, r_ab, 2, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dsxx(skbuffer, 2325, 100, 1425, 1650, r_ab, 2, 2);

            t4cfunc::bra_transform<2, 0>(sbuffer, 0, skbuffer, 2325, 2, 2);

            t4cfunc::bra_transform<2, 0>(sbuffer, 125, skbuffer, 2475, 2, 2);

            t4cfunc::bra_transform<2, 0>(sbuffer, 250, skbuffer, 2625, 2, 2);
...
#endif /* ElectronRepulsionGeom1000RecDSDD_hpp */
