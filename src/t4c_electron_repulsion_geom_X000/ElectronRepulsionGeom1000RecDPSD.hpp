#ifndef ElectronRepulsionGeom1000RecDPSD_hpp
#define ElectronRepulsionGeom1000RecDPSD_hpp
...
/// @brief Computes d^(1)/dA^(1)(DP|1/|r-r'||SD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1000_dpsd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<0, 2>(skbuffer, 90, cbuffer, 54, 0, 1);

            t4cfunc::ket_transform<0, 2>(skbuffer, 105, cbuffer, 72, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 135, cbuffer, 108, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 185, cbuffer, 168, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 45, 0, 15, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 260, 90, 105, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 305, 105, 135, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 395, 135, 185, r_ab, 0, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 545, 0, 260, 305, r_ab, 0, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 680, 15, 305, 395, r_ab, 0, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dpxx(skbuffer, 950, 45, 545, 680, r_ab, 0, 2);

            t4cfunc::bra_transform<2, 1>(sbuffer, 0, skbuffer, 950, 0, 2);

            t4cfunc::bra_transform<2, 1>(sbuffer, 75, skbuffer, 1040, 0, 2);

            t4cfunc::bra_transform<2, 1>(sbuffer, 150, skbuffer, 1130, 0, 2);
...
#endif /* ElectronRepulsionGeom1000RecDPSD_hpp */
