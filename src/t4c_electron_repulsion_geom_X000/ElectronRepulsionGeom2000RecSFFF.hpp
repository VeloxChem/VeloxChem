#ifndef ElectronRepulsionGeom2000RecSFFF_hpp
#define ElectronRepulsionGeom2000RecSFFF_hpp
...
/// @brief Computes d^(2)/dA^(2)(SF|1/|r-r'||FF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_sfff(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(12299, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<3, 3>(skbuffer, 2940, ckbuffer, 2880, 0, 3);

            t4cfunc::ket_transform<3, 3>(skbuffer, 3430, ckbuffer, 6760, 0, 3);

            t4cfunc::ket_transform<3, 3>(skbuffer, 3920, ckbuffer, 12080, 0, 4);

            t4cfunc::ket_transform<3, 3>(skbuffer, 4655, ckbuffer, 19628, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 5684, 3430, 3920, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 7154, 3920, 4655, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 9359, 5684, 7154, r_ab, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 2940, 9359, 3, 3, 3);
...
#endif /* ElectronRepulsionGeom2000RecSFFF_hpp */
