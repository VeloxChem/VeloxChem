#ifndef ElectronRepulsionGeom2000RecDFPD_hpp
#define ElectronRepulsionGeom2000RecDFPD_hpp
...
/// @brief Computes d^(2)/dA^(2)(DF|1/|r-r'||PD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_dfpd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(2656, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(2988, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(28725, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 160, pbuffer, 1420, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 220, pbuffer, 1720, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 320, pbuffer, 2385, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 410, pbuffer, 2745, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 560, pbuffer, 3492, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 686, pbuffer, 3870, 210, ket_width, ket_npgtos);
                ...
                pbuffer.scale(4.0 * a_exp * a_exp, {4584, 4752});

                pbuffer.scale(4.0 * a_exp * a_exp, {4920, 5200});

                pbuffer.scale(4.0 * a_exp * a_exp, {5480, 5696});

                pbuffer.scale(4.0 * a_exp * a_exp, {5696, 6056});

                t2cfunc::reduce(cbuffer, 896, pbuffer, 1420, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 956, pbuffer, 1720, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1056, pbuffer, 2385, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1146, pbuffer, 2745, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1296, pbuffer, 3492, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1422, pbuffer, 3870, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1632, pbuffer, 4584, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1800, pbuffer, 4920, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2080, pbuffer, 5480, 216, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2296, pbuffer, 5696, 360, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1008, cbuffer, 896, 956, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1188, cbuffer, 1056, 1146, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1458, cbuffer, 1296, 1422, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1836, cbuffer, 1632, 1800, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 2340, cbuffer, 2080, 2296, cfactors, 6, 0, 7);

            t4cfunc::ket_transform<1, 2>(skbuffer, 0, ckbuffer, 0, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 17790, ckbuffer, 180, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 17940, ckbuffer, 360, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 18165, ckbuffer, 630, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 19605, ckbuffer, 1008, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 19755, ckbuffer, 1188, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 19980, ckbuffer, 1458, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 20295, ckbuffer, 1836, 0, 6);

            t4cfunc::ket_transform<1, 2>(skbuffer, 20715, ckbuffer, 2340, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 18480, 17790, 17940, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 18930, 17940, 18165, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 21255, 19605, 19755, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 21705, 19755, 19980, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 22380, 19980, 20295, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pixx(skbuffer, 23325, 20295, 20715, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 24585, 21255, 21705, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 25485, 21705, 22380, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_dhxx(skbuffer, 26835, 22380, 23325, r_ab, 1, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 4290, 0, 18480, 18930, r_ab, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 150, 17790, 24585, 3, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 1050, 17940, 25485, 4, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 2400, 18165, 26835, 5, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 5640, 18480, 150, 1050, r_ab, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pgxx(skbuffer, 8340, 18930, 1050, 2400, r_ab, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dfxx(skbuffer, 12390, 4290, 5640, 8340, r_ab, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 12390, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 525, skbuffer, 13290, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1050, skbuffer, 14190, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1575, skbuffer, 15090, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 2100, skbuffer, 15990, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 2625, skbuffer, 16890, 1, 2);
...
#endif /* ElectronRepulsionGeom2000RecDFPD_hpp */
