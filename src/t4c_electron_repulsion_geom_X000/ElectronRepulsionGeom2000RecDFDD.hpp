#ifndef ElectronRepulsionGeom2000RecDFDD_hpp
#define ElectronRepulsionGeom2000RecDFDD_hpp
...
/// @brief Computes d^(2)/dA^(2)(DF|1/|r-r'||DD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_dfdd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(5146, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(13944, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(47875, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 310, pbuffer, 2415, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 370, pbuffer, 2715, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 470, pbuffer, 3215, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 620, pbuffer, 4130, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 710, pbuffer, 4490, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 860, pbuffer, 5090, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1085, pbuffer, 6137, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1211, pbuffer, 6515, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1421, pbuffer, 7145, 315, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 1736, pbuffer, 2415, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1796, pbuffer, 2715, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1896, pbuffer, 3215, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2046, pbuffer, 4130, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2136, pbuffer, 4490, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2286, pbuffer, 5090, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2511, pbuffer, 6137, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2637, pbuffer, 6515, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2847, pbuffer, 7145, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3162, pbuffer, 8174, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3330, pbuffer, 8510, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3610, pbuffer, 9070, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4030, pbuffer, 9910, 216, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4246, pbuffer, 10126, 360, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4606, pbuffer, 10486, 540, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 4704, cbuffer, 1736, 1796, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 4884, cbuffer, 1796, 1896, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 5184, 4704, 4884, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 5544, cbuffer, 2046, 2136, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 5814, cbuffer, 2136, 2286, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 6264, 5544, 5814, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 6804, cbuffer, 2511, 2637, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 7182, cbuffer, 2637, 2847, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 7812, 6804, 7182, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 8568, cbuffer, 3162, 3330, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 9072, cbuffer, 3330, 3610, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 9912, 8568, 9072, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 10920, cbuffer, 4030, 4246, cfactors, 6, 0, 7);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 11568, cbuffer, 4246, 4606, cfactors, 6, 0, 7);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 12648, 10920, 11568, cfactors, 6, 0, 7);

            t4cfunc::ket_transform<2, 2>(skbuffer, 0, ckbuffer, 480, 0, 3);

            t4cfunc::ket_transform<2, 2>(skbuffer, 29650, ckbuffer, 1320, 0, 3);

            t4cfunc::ket_transform<2, 2>(skbuffer, 29900, ckbuffer, 2400, 0, 4);

            t4cfunc::ket_transform<2, 2>(skbuffer, 30275, ckbuffer, 3948, 0, 5);

            t4cfunc::ket_transform<2, 2>(skbuffer, 32675, ckbuffer, 5184, 0, 3);

            t4cfunc::ket_transform<2, 2>(skbuffer, 32925, ckbuffer, 6264, 0, 4);

            t4cfunc::ket_transform<2, 2>(skbuffer, 33300, ckbuffer, 7812, 0, 5);

            t4cfunc::ket_transform<2, 2>(skbuffer, 33825, ckbuffer, 9912, 0, 6);

            t4cfunc::ket_transform<2, 2>(skbuffer, 34525, ckbuffer, 12648, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 30800, 29650, 29900, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 31550, 29900, 30275, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 35425, 32675, 32925, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 36175, 32925, 33300, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 37300, 33300, 33825, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_pixx(skbuffer, 38875, 33825, 34525, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 40975, 35425, 36175, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 42475, 36175, 37300, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_dhxx(skbuffer, 44725, 37300, 38875, r_ab, 2, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 7150, 0, 30800, 31550, r_ab, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 250, 29650, 40975, 3, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 1750, 29900, 42475, 4, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 4000, 30275, 44725, 5, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 9400, 30800, 250, 1750, r_ab, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pgxx(skbuffer, 13900, 31550, 1750, 4000, r_ab, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dfxx(skbuffer, 20650, 7150, 9400, 13900, r_ab, 2, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 20650, 2, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 875, skbuffer, 22150, 2, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1750, skbuffer, 23650, 2, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 2625, skbuffer, 25150, 2, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 3500, skbuffer, 26650, 2, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 4375, skbuffer, 28150, 2, 2);
...
#endif /* ElectronRepulsionGeom2000RecDFDD_hpp */
