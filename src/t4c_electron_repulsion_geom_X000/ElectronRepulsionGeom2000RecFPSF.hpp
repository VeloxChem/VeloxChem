#ifndef ElectronRepulsionGeom2000RecFPSF_hpp
#define ElectronRepulsionGeom2000RecFPSF_hpp
...
/// @brief Computes d^(2)/dA^(2)(FP|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_fpsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(12222, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * a_exp, {1230, 1330});

                pbuffer.scale(2.0 * a_exp, {1855, 2005});

                t2cfunc::reduce(cbuffer, 90, pbuffer, 290, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 120, pbuffer, 680, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 180, pbuffer, 1230, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 280, pbuffer, 1855, 150, ket_width, ket_npgtos);
                ...
                pbuffer.scale(4.0 * a_exp * a_exp, {2431, 2641});

                pbuffer.scale(4.0 * a_exp * a_exp, {2851, 3131});

                t2cfunc::reduce(cbuffer, 430, pbuffer, 290, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 460, pbuffer, 680, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 520, pbuffer, 1230, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 620, pbuffer, 1855, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 770, pbuffer, 2431, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 980, pbuffer, 2851, 280, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 3>(skbuffer, 0, cbuffer, 0, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 147, cbuffer, 30, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 8421, cbuffer, 90, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 8442, cbuffer, 120, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 8484, cbuffer, 180, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 8554, cbuffer, 280, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 9058, cbuffer, 430, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 9079, cbuffer, 460, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 9121, cbuffer, 520, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 9191, cbuffer, 620, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 9296, cbuffer, 770, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 9443, cbuffer, 980, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 1491, 0, 147, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 8659, 8421, 8442, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 8722, 8442, 8484, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 8848, 8484, 8554, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 9639, 9058, 9079, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 9702, 9079, 9121, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 9828, 9121, 9191, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 10038, 9191, 9296, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 10353, 9296, 9443, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 10794, 9639, 9702, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 10920, 9702, 9828, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 11172, 9828, 10038, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 11592, 10038, 10353, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 1554, 0, 8659, 8722, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 2121, 147, 8722, 8848, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dpxx(skbuffer, 4515, 1491, 1554, 2121, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 21, 8421, 10794, 1, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 189, 8442, 10920, 2, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 441, 8484, 11172, 3, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 861, 8554, 11592, 4, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ppxx(skbuffer, 1743, 8659, 21, 189, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 2499, 8722, 189, 441, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 3255, 8848, 441, 861, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dpxx(skbuffer, 4893, 1554, 1743, 2499, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ddxx(skbuffer, 5649, 2121, 2499, 3255, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_fpxx(skbuffer, 7161, 4515, 4893, 5649, r_ab, 0, 3);

            t4cfunc::bra_transform<3, 1>(sbuffer, 0, skbuffer, 7161, 0, 3);

            t4cfunc::bra_transform<3, 1>(sbuffer, 147, skbuffer, 7371, 0, 3);

            t4cfunc::bra_transform<3, 1>(sbuffer, 294, skbuffer, 7581, 0, 3);

            t4cfunc::bra_transform<3, 1>(sbuffer, 441, skbuffer, 7791, 0, 3);

            t4cfunc::bra_transform<3, 1>(sbuffer, 588, skbuffer, 8001, 0, 3);

            t4cfunc::bra_transform<3, 1>(sbuffer, 735, skbuffer, 8211, 0, 3);
...
#endif /* ElectronRepulsionGeom2000RecFPSF_hpp */
