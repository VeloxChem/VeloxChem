#ifndef ElectronRepulsionGeom1000RecFPSP_hpp
#define ElectronRepulsionGeom1000RecFPSP_hpp
...
/// @brief Computes d^(1)/dA^(1)(FP|1/|r-r'||SP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1000_fpsp(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 57, pbuffer, 37, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 66, pbuffer, 100, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 84, pbuffer, 192, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 114, pbuffer, 297, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 159, pbuffer, 387, 63, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<0, 1>(skbuffer, 27, cbuffer, 27, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 192, cbuffer, 57, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 201, cbuffer, 66, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 219, cbuffer, 84, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 249, cbuffer, 114, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 294, cbuffer, 159, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 57, 0, 9, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 84, 9, 27, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 138, 57, 84, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 357, 192, 201, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 384, 201, 219, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 438, 219, 249, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 528, 249, 294, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 663, 0, 357, 384, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 744, 9, 384, 438, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 906, 27, 438, 528, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dpxx(skbuffer, 1176, 57, 663, 744, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ddxx(skbuffer, 1338, 84, 744, 906, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_fpxx(skbuffer, 1662, 138, 1176, 1338, r_ab, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 0, skbuffer, 1662, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 63, skbuffer, 1752, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 126, skbuffer, 1842, 0, 1);
...
#endif /* ElectronRepulsionGeom1000RecFPSP_hpp */
