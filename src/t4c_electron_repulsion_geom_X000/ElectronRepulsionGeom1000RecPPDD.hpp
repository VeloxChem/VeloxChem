...
/// @brief Computes (PP|1/|r-r'||DD)  integral derivatives for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_geom_1000_ppdd(T&                               distributor,
                                       ...
                                       const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(1900, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<2, 2>(skbuffer, 75, ckbuffer, 396, 0, 1);
        
            t4cfunc::ket_transform<2, 2>(skbuffer, 150, ckbuffer, 792, 0, 2);
            
            t4cfunc::ket_transform<2, 2>(skbuffer, 300, ckbuffer, 1488, 0, 3);
            
            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 550, 75, 150, r_ab, 2, 2);
            
            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 775, 150, 300, r_ab, 2, 2);
            
            erirec::comp_bra_geom_hrr_electron_repulsion_ppxx(skbuffer, 1225, 550, 775, 0, r_ab, 2, 2);
            
            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 1225, 2, 2);
            
            t4cfunc::bra_transform<1, 1>(sbuffer, 225, skbuffer, 1450, 2, 2);
            
            t4cfunc::bra_transform<1, 1>(sbuffer, 450, skbuffer, 1675, 2, 2);
