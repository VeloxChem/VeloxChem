#ifndef ElectronRepulsionGeom2000RecFPSP_hpp
#define ElectronRepulsionGeom2000RecFPSP_hpp
...
/// @brief Computes d^(2)/dA^(2)(FP|1/|r-r'||SP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_fpsp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(5238, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * a_exp, {242, 272});

                pbuffer.scale(2.0 * a_exp, {392, 437});

                t2cfunc::reduce(cbuffer, 27, pbuffer, 44, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 36, pbuffer, 122, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 54, pbuffer, 242, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 84, pbuffer, 392, 45, ket_width, ket_npgtos);
                ...
                pbuffer.scale(4.0 * a_exp * a_exp, {548, 611});

                pbuffer.scale(4.0 * a_exp * a_exp, {674, 758});

                t2cfunc::reduce(cbuffer, 129, pbuffer, 44, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 138, pbuffer, 122, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 156, pbuffer, 242, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 186, pbuffer, 392, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 231, pbuffer, 548, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 294, pbuffer, 674, 84, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 1>(skbuffer, 0, cbuffer, 0, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 63, cbuffer, 9, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 3609, cbuffer, 27, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 3618, cbuffer, 36, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 3636, cbuffer, 54, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 3666, cbuffer, 84, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 3882, cbuffer, 129, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 3891, cbuffer, 138, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 3909, cbuffer, 156, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 3939, cbuffer, 186, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 3984, cbuffer, 231, 0, 5);

            t4cfunc::ket_transform<0, 1>(skbuffer, 4047, cbuffer, 294, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 639, 0, 63, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 3711, 3609, 3618, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 3738, 3618, 3636, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 3792, 3636, 3666, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 4131, 3882, 3891, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 4158, 3891, 3909, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 4212, 3909, 3939, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 4302, 3939, 3984, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 4437, 3984, 4047, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 4626, 4131, 4158, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 4680, 4158, 4212, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 4788, 4212, 4302, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 4968, 4302, 4437, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 666, 0, 3711, 3738, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 909, 63, 3738, 3792, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dpxx(skbuffer, 1935, 639, 666, 909, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 9, 3609, 4626, 1, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 81, 3618, 4680, 2, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 189, 3636, 4788, 3, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 369, 3666, 4968, 4, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ppxx(skbuffer, 747, 3711, 9, 81, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 1071, 3738, 81, 189, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 1395, 3792, 189, 369, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dpxx(skbuffer, 2097, 666, 747, 1071, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ddxx(skbuffer, 2421, 909, 1071, 1395, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_fpxx(skbuffer, 3069, 1935, 2097, 2421, r_ab, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 0, skbuffer, 3069, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 63, skbuffer, 3159, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 126, skbuffer, 3249, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 189, skbuffer, 3339, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 252, skbuffer, 3429, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 315, skbuffer, 3519, 0, 1);
...
#endif /* ElectronRepulsionGeom2000RecFPSP_hpp */
