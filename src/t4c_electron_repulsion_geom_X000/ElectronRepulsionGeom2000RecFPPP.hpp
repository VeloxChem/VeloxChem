#ifndef ElectronRepulsionGeom2000RecFPPP_hpp
#define ElectronRepulsionGeom2000RecFPPP_hpp
...
/// @brief Computes d^(2)/dA^(2)(FP|1/|r-r'||PP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_fppp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(1134, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(1134, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(15714, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 81, pbuffer, 90, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 90, pbuffer, 144, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 108, pbuffer, 276, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 126, pbuffer, 366, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 162, pbuffer, 576, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 192, pbuffer, 696, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 252, pbuffer, 966, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 297, pbuffer, 1101, 90, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 387, pbuffer, 90, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 396, pbuffer, 144, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 414, pbuffer, 276, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 432, pbuffer, 366, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 468, pbuffer, 576, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 498, pbuffer, 696, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 558, pbuffer, 966, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 603, pbuffer, 1101, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 693, pbuffer, 1392, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 756, pbuffer, 1518, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 882, pbuffer, 1770, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 966, pbuffer, 1854, 168, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 387, cbuffer, 387, 396, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 414, cbuffer, 414, 432, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 468, cbuffer, 468, 498, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 558, cbuffer, 558, 603, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 693, cbuffer, 693, 756, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 882, cbuffer, 882, 966, cfactors, 6, 0, 6);

            t4cfunc::ket_transform<1, 1>(skbuffer, 0, ckbuffer, 0, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 189, ckbuffer, 27, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 10827, ckbuffer, 81, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 10854, ckbuffer, 108, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 10908, ckbuffer, 162, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 10998, ckbuffer, 252, 0, 4);

            t4cfunc::ket_transform<1, 1>(skbuffer, 11646, ckbuffer, 387, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 11673, ckbuffer, 414, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 11727, ckbuffer, 468, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 11817, ckbuffer, 558, 0, 4);

            t4cfunc::ket_transform<1, 1>(skbuffer, 11952, ckbuffer, 693, 0, 5);

            t4cfunc::ket_transform<1, 1>(skbuffer, 12141, ckbuffer, 882, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 1917, 0, 189, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 11133, 10827, 10854, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 11214, 10854, 10908, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 11376, 10908, 10998, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 12393, 11646, 11673, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 12474, 11673, 11727, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 12636, 11727, 11817, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 12906, 11817, 11952, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 13311, 11952, 12141, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 13878, 12393, 12474, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 14040, 12474, 12636, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 14364, 12636, 12906, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 14904, 12906, 13311, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 1998, 0, 11133, 11214, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 2727, 189, 11214, 11376, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dpxx(skbuffer, 5805, 1917, 1998, 2727, r_ab, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 27, 10827, 13878, 1, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 243, 10854, 14040, 2, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 567, 10908, 14364, 3, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 1107, 10998, 14904, 4, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ppxx(skbuffer, 2241, 11133, 27, 243, r_ab, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 3213, 11214, 243, 567, r_ab, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 4185, 11376, 567, 1107, r_ab, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dpxx(skbuffer, 6291, 1998, 2241, 3213, r_ab, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ddxx(skbuffer, 7263, 2727, 3213, 4185, r_ab, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_fpxx(skbuffer, 9207, 5805, 6291, 7263, r_ab, 1, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 0, skbuffer, 9207, 1, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 189, skbuffer, 9477, 1, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 378, skbuffer, 9747, 1, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 567, skbuffer, 10017, 1, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 756, skbuffer, 10287, 1, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 945, skbuffer, 10557, 1, 1);
...
#endif /* ElectronRepulsionGeom2000RecFPPP_hpp */
