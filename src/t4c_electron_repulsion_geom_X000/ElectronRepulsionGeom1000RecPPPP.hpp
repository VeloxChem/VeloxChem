#ifndef ElectronRepulsionGeom1000RecPPPP_hpp
#define ElectronRepulsionGeom1000RecPPPP_hpp
...
/// @brief Computes (PP|1/|r-r'||PP)  integral derivatives for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_geom_1000_pppp(T&                               distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * a_exp, {138, 156});
                
                pbuffer.scale(2.0 * a_exp, {174, 210});
                
                pbuffer.scale(2.0 * a_exp, {246, 276});
                
                pbuffer.scale(2.0 * a_exp, {276, 336});
            ...
            t4cfunc::ket_transform<1, 1>(skbuffer, 54, ckbuffer, 54, 0, 2);
            
            t4cfunc::ket_transform<1, 1>(skbuffer, 108, ckbuffer, 108, 0, 3);
            
            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 198, 27, 54, r_ab, 1, 1);
            
            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 279, 54, 108, r_ab, 1, 1);
            
            erirec::comp_bra_geom_hrr_electron_repulsion_ppxx(skbuffer, 441, 198, 279, 0, r_ab, 1, 1);
            
            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 441, 1, 1);
            
            t4cfunc::bra_transform<1, 1>(sbuffer, 81, skbuffer, 522, 1, 1);
            
            t4cfunc::bra_transform<1, 1>(sbuffer, 162, skbuffer, 603, 1, 1);
...
#endif /* ElectronRepulsionGeom1000RecPPPP_hpp */
