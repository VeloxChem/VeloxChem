#ifndef ElectronRepulsionGeom2000RecDFSF_hpp
#define ElectronRepulsionGeom2000RecDFSF_hpp
...
/// @brief Computes d^(2)/dA^(2)(DF|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_dfsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(1660, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(13405, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 100, pbuffer, 1530, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 200, pbuffer, 2405, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 350, pbuffer, 3320, 210, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 560, pbuffer, 1530, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 660, pbuffer, 2405, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 810, pbuffer, 3320, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1020, pbuffer, 4118, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1300, pbuffer, 4678, 360, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 3>(skbuffer, 0, cbuffer, 0, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 8302, cbuffer, 100, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 8372, cbuffer, 200, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 8477, cbuffer, 350, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 9149, cbuffer, 560, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 9219, cbuffer, 660, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 9324, cbuffer, 810, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 9471, cbuffer, 1020, 0, 6);

            t4cfunc::ket_transform<0, 3>(skbuffer, 9667, cbuffer, 1300, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 8624, 8302, 8372, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 8834, 8372, 8477, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 9919, 9149, 9219, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 10129, 9219, 9324, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 10444, 9324, 9471, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pixx(skbuffer, 10885, 9471, 9667, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 11473, 9919, 10129, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 11893, 10129, 10444, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dhxx(skbuffer, 12523, 10444, 10885, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 2002, 0, 8624, 8834, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 70, 8302, 11473, 3, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 490, 8372, 11893, 4, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 1120, 8477, 12523, 5, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 2632, 8624, 70, 490, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pgxx(skbuffer, 3892, 8834, 490, 1120, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dfxx(skbuffer, 5782, 2002, 2632, 3892, r_ab, 0, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 5782, 0, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 245, skbuffer, 6202, 0, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 490, skbuffer, 6622, 0, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 735, skbuffer, 7042, 0, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 980, skbuffer, 7462, 0, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1225, skbuffer, 7882, 0, 3);
...
#endif /* ElectronRepulsionGeom2000RecDFSF_hpp */
