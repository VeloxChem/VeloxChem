#ifndef ElectronRepulsionGeom2000RecDPSF_hpp
#define ElectronRepulsionGeom2000RecDPSF_hpp
...
/// @brief Computes d^(2)/dA^(2)(DP|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_dpsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(770, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(5117, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 30, pbuffer, 240, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 60, pbuffer, 540, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 120, pbuffer, 930, 100, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 220, pbuffer, 240, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 250, pbuffer, 540, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 310, pbuffer, 930, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 410, pbuffer, 1320, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 560, pbuffer, 1620, 210, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 3>(skbuffer, 0, cbuffer, 0, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2898, cbuffer, 30, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2919, cbuffer, 60, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2961, cbuffer, 120, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 3220, cbuffer, 220, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 3241, cbuffer, 250, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 3283, cbuffer, 310, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 3353, cbuffer, 410, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 3458, cbuffer, 560, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 3031, 2898, 2919, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 3094, 2919, 2961, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 3605, 3220, 3241, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 3668, 3241, 3283, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 3794, 3283, 3353, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 4004, 3353, 3458, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 4319, 3605, 3668, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 4445, 3668, 3794, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 4697, 3794, 4004, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 819, 0, 3031, 3094, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 21, 2898, 4319, 1, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 147, 2919, 4445, 2, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 399, 2961, 4697, 3, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ppxx(skbuffer, 1008, 3031, 21, 147, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 1386, 3094, 147, 399, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dpxx(skbuffer, 2142, 819, 1008, 1386, r_ab, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 0, skbuffer, 2142, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 105, skbuffer, 2268, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 210, skbuffer, 2394, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 315, skbuffer, 2520, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 420, skbuffer, 2646, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 525, skbuffer, 2772, 0, 3);
...
#endif /* ElectronRepulsionGeom2000RecDPSF_hpp */
