#ifndef ElectronRepulsionGeom2000RecPFPF_hpp
#define ElectronRepulsionGeom2000RecPFPF_hpp
...
/// @brief Computes d^(2)/dA^(2)(PF|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_pfpf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(15687, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<1, 3>(skbuffer, 6930, ckbuffer, 0, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 7140, ckbuffer, 300, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 8085, ckbuffer, 750, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 8295, ckbuffer, 1050, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 8610, ckbuffer, 1500, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 9051, ckbuffer, 2130, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 7455, 6930, 7140, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 9639, 8085, 8295, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 10269, 8295, 8610, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 11214, 8610, 9051, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 12537, 9639, 10269, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 13797, 10269, 11214, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 6930, 12537, 3, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 1260, 7140, 13797, 4, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 3150, 7455, 0, 1260, r_ab, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 3150, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 441, skbuffer, 3780, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 882, skbuffer, 4410, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1323, skbuffer, 5040, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1764, skbuffer, 5670, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 2205, skbuffer, 6300, 1, 3);
...
#endif /* ElectronRepulsionGeom2000RecPFPF_hpp */
