...
/// @brief Computes d^(2)/dA^(2)(SP|1/|r-r'||PD)  integral derivatives.
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(1275, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<1, 2>(skbuffer, 270, ckbuffer, 0, 0, 1);

            t4cfunc::ket_transform<1, 2>(skbuffer, 315, ckbuffer, 54, 0, 1);

            t4cfunc::ket_transform<1, 2>(skbuffer, 360, ckbuffer, 108, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 450, ckbuffer, 216, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 600, 315, 360, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 735, 360, 450, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 1005, 600, 735, r_ab, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 270, 1005, 1, 1, 2);
