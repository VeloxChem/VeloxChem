#ifndef ElectronRepulsionGeom2000RecPPDF_hpp
#define ElectronRepulsionGeom2000RecPPDF_hpp
...
/// @brief Computes d^(2)/dA^(2)(PP|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_ppdf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(9485, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<2, 3>(skbuffer, 3780, ckbuffer, 225, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 3885, ckbuffer, 855, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 4410, ckbuffer, 1440, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 4515, ckbuffer, 2070, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 4725, ckbuffer, 3180, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 5075, ckbuffer, 4905, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 4095, 3780, 3885, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 5600, 4410, 4515, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 5915, 4515, 4725, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 6545, 4725, 5075, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 7595, 5600, 5915, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 8225, 5915, 6545, r_ab, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 3780, 7595, 1, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 630, 3885, 8225, 2, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ppxx(skbuffer, 1890, 4095, 0, 630, r_ab, 2, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 1890, 2, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 315, skbuffer, 2205, 2, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 630, skbuffer, 2520, 2, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 945, skbuffer, 2835, 2, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 1260, skbuffer, 3150, 2, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 1575, skbuffer, 3465, 2, 3);
...
#endif /* ElectronRepulsionGeom2000RecPPDF_hpp */
