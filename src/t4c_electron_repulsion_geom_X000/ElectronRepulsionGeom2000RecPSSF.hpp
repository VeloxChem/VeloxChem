#ifndef ElectronRepulsionGeom2000RecPSSF_hpp
#define ElectronRepulsionGeom2000RecPSSF_hpp
...
/// @brief Computes d^(2)/dA^(2)(PS|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_pssf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(4.0 * a_exp * a_exp, {266, 326});

                pbuffer.scale(4.0 * a_exp * a_exp, {386, 486});
            ...
            t4cfunc::ket_transform<0, 3>(skbuffer, 294, cbuffer, 0, 0, 0);

            t4cfunc::ket_transform<0, 3>(skbuffer, 301, cbuffer, 10, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 343, cbuffer, 40, 0, 0);

            t4cfunc::ket_transform<0, 3>(skbuffer, 350, cbuffer, 50, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 371, cbuffer, 80, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 413, cbuffer, 140, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 322, 294, 301, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 483, 343, 350, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 504, 350, 371, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 567, 371, 413, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dsxx(skbuffer, 693, 483, 504, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 735, 504, 567, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 294, 693, 0, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 42, 301, 735, 1, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_psxx(skbuffer, 168, 322, 0, 42, r_ab, 0, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 0, skbuffer, 168, 0, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 21, skbuffer, 189, 0, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 42, skbuffer, 210, 0, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 63, skbuffer, 231, 0, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 84, skbuffer, 252, 0, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 105, skbuffer, 273, 0, 3);
...
#endif /* ElectronRepulsionGeom2000RecPSSF_hpp */
