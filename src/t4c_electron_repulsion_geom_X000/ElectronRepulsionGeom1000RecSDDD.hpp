#ifndef ElectronRepulsionGeom1000RecSDDD_hpp
#define ElectronRepulsionGeom1000RecSDDD_hpp
...
/// @brief Computes (SD|1/|r-r'||DD)  integral derivatives for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_geom_1000_sddd(T&                               distributor,
                                       ...
                                       const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(1344, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(850, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 36, pbuffer, 565, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 96, pbuffer, 685, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 186, pbuffer, 865, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 246, pbuffer, 925, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 346, pbuffer, 1025, 150, ket_width, ket_npgtos);
            }
            ...
            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 400, 0, 150, r_ab, 2, 2);

            t4cfunc::bra_transform<0, 2>(sbuffer, 0, skbuffer, 400, 2, 2);
            
            t4cfunc::bra_transform<0, 2>(sbuffer, 125, skbuffer, 550, 2, 2);
            
            t4cfunc::bra_transform<0, 2>(sbuffer, 250, skbuffer, 700, 2, 2);
...
#endif /* ElectronRepulsionGeom1000RecSDDD_hpp */
