#ifndef ElectronRepulsionGeom2000RecSPSD_hpp
#define ElectronRepulsionGeom2000RecSPSD_hpp
...
/// @brief Computes d^(2)/dA^(2)(SP|1/|r-r'||SD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_spsd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(425, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 66, 18, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 18, pbuffer, 66, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 36, pbuffer, 138, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 72, pbuffer, 210, 60, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 2>(skbuffer, 90, cbuffer, 0, 0, 1);

            t4cfunc::ket_transform<0, 2>(skbuffer, 105, cbuffer, 18, 0, 1);

            t4cfunc::ket_transform<0, 2>(skbuffer, 120, cbuffer, 36, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 150, cbuffer, 72, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 200, 105, 120, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 245, 120, 150, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 335, 200, 245, r_ab, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 90, 335, 1, 0, 2);
...
#endif /* ElectronRepulsionGeom2000RecSPSD_hpp */
