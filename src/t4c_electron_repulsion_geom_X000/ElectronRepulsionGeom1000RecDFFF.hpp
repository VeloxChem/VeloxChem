#ifndef ElectronRepulsionGeom1000RecDFFF_hpp
#define ElectronRepulsionGeom1000RecDFFF_hpp
...
/// @brief Computes d^(1)/dA^(1)(DF|1/|r-r'||FF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1000_dfff(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(32928, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<3, 3>(skbuffer, 2695, ckbuffer, 12580, 0, 3);

            t4cfunc::ket_transform<3, 3>(skbuffer, 3185, ckbuffer, 17900, 0, 4);

            t4cfunc::ket_transform<3, 3>(skbuffer, 3920, ckbuffer, 25448, 0, 5);

            t4cfunc::ket_transform<3, 3>(skbuffer, 4949, ckbuffer, 35612, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 1225, 0, 490, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 6321, 2695, 3185, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 7791, 3185, 3920, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 9996, 3920, 4949, r_ab, 3, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 13083, 0, 6321, 7791, r_ab, 3, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pgxx(skbuffer, 17493, 490, 7791, 9996, r_ab, 3, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dfxx(skbuffer, 24108, 1225, 13083, 17493, r_ab, 3, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 24108, 3, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1715, skbuffer, 27048, 3, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 3430, skbuffer, 29988, 3, 3);
...
#endif /* ElectronRepulsionGeom1000RecDFFF_hpp */
