...
/// @brief Computes d^(2)/dA^(2)(SP|1/|r-r'||SP)  integral derivatives.
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(255, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<0, 1>(skbuffer, 54, cbuffer, 0, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 63, cbuffer, 9, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 72, cbuffer, 18, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 90, cbuffer, 36, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 120, 63, 72, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 147, 72, 90, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 201, 120, 147, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 54, 201, 1, 0, 1);
