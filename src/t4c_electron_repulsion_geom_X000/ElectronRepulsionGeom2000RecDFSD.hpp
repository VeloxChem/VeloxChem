#ifndef ElectronRepulsionGeom2000RecDFSD_hpp
#define ElectronRepulsionGeom2000RecDFSD_hpp
...
/// @brief Computes d^(2)/dA^(2)(DF|1/|r-r'||SD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_dfsd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(996, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(9575, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 760, 60, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {760, 820});

                pbuffer.scale(2.0 * a_exp, {1225, 1315});

                pbuffer.scale(2.0 * a_exp, {1732, 1858});

                t2cfunc::reduce(cbuffer, 60, pbuffer, 760, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 120, pbuffer, 1225, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 210, pbuffer, 1732, 126, ket_width, ket_npgtos);
                ...
                pbuffer.scale(4.0 * a_exp * a_exp, {2194, 2362});

                pbuffer.scale(4.0 * a_exp * a_exp, {2530, 2746});

                t2cfunc::reduce(cbuffer, 336, pbuffer, 760, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 396, pbuffer, 1225, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 486, pbuffer, 1732, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 612, pbuffer, 2194, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 780, pbuffer, 2530, 216, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 2>(skbuffer, 0, cbuffer, 0, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 5930, cbuffer, 60, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 5980, cbuffer, 120, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 6055, cbuffer, 210, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 6535, cbuffer, 336, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 6585, cbuffer, 396, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 6660, cbuffer, 486, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 6765, cbuffer, 612, 0, 6);

            t4cfunc::ket_transform<0, 2>(skbuffer, 6905, cbuffer, 780, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 6160, 5930, 5980, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 6310, 5980, 6055, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 7085, 6535, 6585, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 7235, 6585, 6660, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 7460, 6660, 6765, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pixx(skbuffer, 7775, 6765, 6905, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 8195, 7085, 7235, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 8495, 7235, 7460, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_dhxx(skbuffer, 8945, 7460, 7775, r_ab, 0, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 1430, 0, 6160, 6310, r_ab, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 50, 5930, 8195, 3, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 350, 5980, 8495, 4, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 800, 6055, 8945, 5, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 1880, 6160, 50, 350, r_ab, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pgxx(skbuffer, 2780, 6310, 350, 800, r_ab, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dfxx(skbuffer, 4130, 1430, 1880, 2780, r_ab, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 4130, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 175, skbuffer, 4430, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 350, skbuffer, 4730, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 525, skbuffer, 5030, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 700, skbuffer, 5330, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 875, skbuffer, 5630, 0, 2);
...
#endif /* ElectronRepulsionGeom2000RecDFSD_hpp */
