#ifndef ElectronRepulsionGeom2000RecDSPF_hpp
#define ElectronRepulsionGeom2000RecDSPF_hpp
...
/// @brief Computes d^(2)/dA^(2)(DS|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_dspf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(1380, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(7455, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 50, pbuffer, 285, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 80, pbuffer, 405, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 125, pbuffer, 675, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 185, pbuffer, 855, 90, ket_width, ket_npgtos);
                ...
                pbuffer.scale(4.0 * a_exp * a_exp, {1185, 1285});

                pbuffer.scale(4.0 * a_exp * a_exp, {1385, 1535});

                pbuffer.scale(4.0 * a_exp * a_exp, {1685, 1835});

                pbuffer.scale(4.0 * a_exp * a_exp, {1835, 2060});

                t2cfunc::reduce(cbuffer, 275, pbuffer, 75, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 285, pbuffer, 135, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 300, pbuffer, 285, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 330, pbuffer, 405, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 375, pbuffer, 675, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 435, pbuffer, 855, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 525, pbuffer, 1185, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 625, pbuffer, 1385, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 775, pbuffer, 1685, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 925, pbuffer, 1835, 225, ket_width, ket_npgtos);

            }

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 0, cbuffer, 0, 10, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 30, cbuffer, 25, 35, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 60, cbuffer, 50, 80, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 150, cbuffer, 125, 185, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 330, cbuffer, 275, 285, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 360, cbuffer, 300, 330, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 450, cbuffer, 375, 435, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 630, cbuffer, 525, 625, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 930, cbuffer, 775, 925, cfactors, 6, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 0, ckbuffer, 0, 0, 0);

            t4cfunc::ket_transform<1, 3>(skbuffer, 3738, ckbuffer, 30, 0, 0);

            t4cfunc::ket_transform<1, 3>(skbuffer, 3759, ckbuffer, 60, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 3822, ckbuffer, 150, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 4200, ckbuffer, 330, 0, 0);

            t4cfunc::ket_transform<1, 3>(skbuffer, 4221, ckbuffer, 360, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 4284, ckbuffer, 450, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 4410, ckbuffer, 630, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 4620, ckbuffer, 930, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 3948, 3738, 3759, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 4011, 3759, 3822, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 4935, 4200, 4221, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 4998, 4221, 4284, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 5187, 4284, 4410, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 5565, 4410, 4620, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dsxx(skbuffer, 6195, 4935, 4998, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 6321, 4998, 5187, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 6699, 5187, 5565, r_ab, 1, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_psxx(skbuffer, 1281, 0, 3948, 4011, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 21, 3738, 6195, 0, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 147, 3759, 6321, 1, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 525, 3822, 6699, 2, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_psxx(skbuffer, 1470, 3948, 21, 147, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ppxx(skbuffer, 1848, 4011, 147, 525, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dsxx(skbuffer, 2982, 1281, 1470, 1848, r_ab, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 0, skbuffer, 2982, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 105, skbuffer, 3108, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 210, skbuffer, 3234, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 315, skbuffer, 3360, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 420, skbuffer, 3486, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 525, skbuffer, 3612, 1, 3);
...
#endif /* ElectronRepulsionGeom2000RecDSPF_hpp */
