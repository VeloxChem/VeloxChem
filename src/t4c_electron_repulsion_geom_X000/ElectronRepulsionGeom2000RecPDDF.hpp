#ifndef ElectronRepulsionGeom2000RecPDDF_hpp
#define ElectronRepulsionGeom2000RecPDDF_hpp
...
/// @brief Computes d^(2)/dA^(2)(PD|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_pddf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(3128, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(9180, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(16765, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 60, pbuffer, 1591, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 1951, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 276, pbuffer, 2605, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 376, pbuffer, 2905, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 526, pbuffer, 3355, 210, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 736, pbuffer, 1351, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 796, pbuffer, 1591, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 886, pbuffer, 1951, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1012, pbuffer, 2605, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1112, pbuffer, 2905, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1262, pbuffer, 3355, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1472, pbuffer, 4075, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1622, pbuffer, 4375, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1847, pbuffer, 4825, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2162, pbuffer, 5455, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2372, pbuffer, 5665, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2687, pbuffer, 5980, 441, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2160, cbuffer, 736, 796, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 2340, cbuffer, 796, 886, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 2610, 2160, 2340, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2970, cbuffer, 1012, 1112, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 3270, cbuffer, 1112, 1262, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 3720, 2970, 3270, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 4320, cbuffer, 1472, 1622, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 4770, cbuffer, 1622, 1847, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 5445, 4320, 4770, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 6345, cbuffer, 2162, 2372, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 6975, cbuffer, 2372, 2687, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 7920, 6345, 6975, cfactors, 6, 0, 5);

            t4cfunc::ket_transform<2, 3>(skbuffer, 7140, ckbuffer, 450, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 7350, ckbuffer, 1560, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 8330, ckbuffer, 2610, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 8540, ckbuffer, 3720, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 8890, ckbuffer, 5445, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 9415, ckbuffer, 7920, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 7700, 7140, 7350, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 10150, 8330, 8540, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 10780, 8540, 8890, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 11830, 8890, 9415, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 13405, 10150, 10780, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 14665, 10780, 11830, r_ab, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 7140, 13405, 2, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 1260, 7350, 14665, 3, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 3360, 7700, 0, 1260, r_ab, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 0, skbuffer, 3360, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 525, skbuffer, 3990, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 1050, skbuffer, 4620, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 1575, skbuffer, 5250, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 2100, skbuffer, 5880, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 2625, skbuffer, 6510, 2, 3);
...
#endif /* ElectronRepulsionGeom2000RecPDDF_hpp */
