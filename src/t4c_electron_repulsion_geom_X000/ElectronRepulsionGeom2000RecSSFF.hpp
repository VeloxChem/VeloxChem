#ifndef ElectronRepulsionGeom2000RecSSFF_hpp
#define ElectronRepulsionGeom2000RecSSFF_hpp
...
/// @brief Computes d^(2)/dA^(2)(SS|1/|r-r'||FF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_ssff(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(1715, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(4.0 * a_exp * a_exp, {396, 426});

                pbuffer.scale(4.0 * a_exp * a_exp, {456, 501});

                pbuffer.scale(4.0 * a_exp * a_exp, {546, 609});

                pbuffer.scale(4.0 * a_exp * a_exp, {672, 756});

                pbuffer.scale(4.0 * a_exp * a_exp, {840, 900});

                pbuffer.scale(4.0 * a_exp * a_exp, {900, 990});

                pbuffer.scale(4.0 * a_exp * a_exp, {990, 1116});

                pbuffer.scale(4.0 * a_exp * a_exp, {1116, 1284});
            ...
            t4cfunc::ket_transform<3, 3>(skbuffer, 294, ckbuffer, 288, 0, 0);

            t4cfunc::ket_transform<3, 3>(skbuffer, 343, ckbuffer, 676, 0, 0);

            t4cfunc::ket_transform<3, 3>(skbuffer, 392, ckbuffer, 1640, 0, 1);

            t4cfunc::ket_transform<3, 3>(skbuffer, 539, ckbuffer, 3668, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 833, 343, 392, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 980, 392, 539, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_dsxx(skbuffer, 1421, 833, 980, r_ab, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 294, 1421, 0, 3, 3);
...
#endif /* ElectronRepulsionGeom2000RecSSFF_hpp */
