#ifndef ElectronRepulsionGeom2000RecDPSD_hpp
#define ElectronRepulsionGeom2000RecDPSD_hpp
...
/// @brief Computes d^(2)/dA^(2)(DP|1/|r-r'||SD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_dpsd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(462, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(3655, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 18, pbuffer, 110, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 36, pbuffer, 266, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 72, pbuffer, 480, 60, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 132, pbuffer, 110, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 266, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 186, pbuffer, 480, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 246, pbuffer, 705, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 336, pbuffer, 885, 126, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 2>(skbuffer, 0, cbuffer, 0, 0, 1);

            t4cfunc::ket_transform<0, 2>(skbuffer, 2070, cbuffer, 18, 0, 1);

            t4cfunc::ket_transform<0, 2>(skbuffer, 2085, cbuffer, 36, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 2115, cbuffer, 72, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 2300, cbuffer, 132, 0, 1);

            t4cfunc::ket_transform<0, 2>(skbuffer, 2315, cbuffer, 150, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 2345, cbuffer, 186, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 2395, cbuffer, 246, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 2470, cbuffer, 336, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 2165, 2070, 2085, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 2210, 2085, 2115, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 2575, 2300, 2315, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 2620, 2315, 2345, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 2710, 2345, 2395, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 2860, 2395, 2470, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 3085, 2575, 2620, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 3175, 2620, 2710, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 3355, 2710, 2860, r_ab, 0, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 585, 0, 2165, 2210, r_ab, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 15, 2070, 3085, 1, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 105, 2085, 3175, 2, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 285, 2115, 3355, 3, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ppxx(skbuffer, 720, 2165, 15, 105, r_ab, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 990, 2210, 105, 285, r_ab, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dpxx(skbuffer, 1530, 585, 720, 990, r_ab, 0, 2);

            t4cfunc::bra_transform<2, 1>(sbuffer, 0, skbuffer, 1530, 0, 2);

            t4cfunc::bra_transform<2, 1>(sbuffer, 75, skbuffer, 1620, 0, 2);

            t4cfunc::bra_transform<2, 1>(sbuffer, 150, skbuffer, 1710, 0, 2);

            t4cfunc::bra_transform<2, 1>(sbuffer, 225, skbuffer, 1800, 0, 2);

            t4cfunc::bra_transform<2, 1>(sbuffer, 300, skbuffer, 1890, 0, 2);

            t4cfunc::bra_transform<2, 1>(sbuffer, 375, skbuffer, 1980, 0, 2);
...
#endif /* ElectronRepulsionGeom2000RecDPSD_hpp */
