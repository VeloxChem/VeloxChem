...
/// @brief Computes (SP|1/|r-r'||DD)  integral derivatives for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_geom_1000_spdd(T&                               distributor,
                                       ...
                                       const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::bra_transform<0, 1>(sbuffer, 0, skbuffer, 225, 2, 2);
            
            t4cfunc::bra_transform<0, 1>(sbuffer, 75, skbuffer, 300, 2, 2);
            
            t4cfunc::bra_transform<0, 1>(sbuffer, 150, skbuffer, 375, 2, 2);
