#ifndef ElectronRepulsionGeom2000RecPSPD_hpp
#define ElectronRepulsionGeom2000RecPSPD_hpp
...
/// @brief Computes d^(2)/dA^(2)(PS|1/|r-r'||PD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_pspd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(1845, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(4.0 * a_exp * a_exp, {278, 314});

                pbuffer.scale(4.0 * a_exp * a_exp, {350, 410});

                pbuffer.scale(4.0 * a_exp * a_exp, {470, 530});

                pbuffer.scale(4.0 * a_exp * a_exp, {530, 630});
            ...
            t4cfunc::ket_transform<1, 2>(skbuffer, 630, ckbuffer, 0, 0, 0);

            t4cfunc::ket_transform<1, 2>(skbuffer, 645, ckbuffer, 18, 0, 1);

            t4cfunc::ket_transform<1, 2>(skbuffer, 735, ckbuffer, 72, 0, 0);

            t4cfunc::ket_transform<1, 2>(skbuffer, 750, ckbuffer, 90, 0, 1);

            t4cfunc::ket_transform<1, 2>(skbuffer, 795, ckbuffer, 144, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 885, ckbuffer, 252, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 690, 630, 645, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 1035, 735, 750, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 1080, 750, 795, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 1215, 795, 885, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_dsxx(skbuffer, 1485, 1035, 1080, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 1575, 1080, 1215, r_ab, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 630, 1485, 0, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 90, 645, 1575, 1, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_psxx(skbuffer, 360, 690, 0, 90, r_ab, 1, 2);

            t4cfunc::bra_transform<1, 0>(sbuffer, 0, skbuffer, 360, 1, 2);

            t4cfunc::bra_transform<1, 0>(sbuffer, 45, skbuffer, 405, 1, 2);

            t4cfunc::bra_transform<1, 0>(sbuffer, 90, skbuffer, 450, 1, 2);

            t4cfunc::bra_transform<1, 0>(sbuffer, 135, skbuffer, 495, 1, 2);

            t4cfunc::bra_transform<1, 0>(sbuffer, 180, skbuffer, 540, 1, 2);

            t4cfunc::bra_transform<1, 0>(sbuffer, 225, skbuffer, 585, 1, 2);
...
#endif /* ElectronRepulsionGeom2000RecPSPD_hpp */
