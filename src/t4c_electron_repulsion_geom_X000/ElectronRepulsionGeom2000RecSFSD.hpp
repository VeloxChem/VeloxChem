#ifndef ElectronRepulsionGeom2000RecSFSD_hpp
#define ElectronRepulsionGeom2000RecSFSD_hpp
...
/// @brief Computes d^(2)/dA^(2)(SF|1/|r-r'||SD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_sfsd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(1255, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<0, 2>(skbuffer, 300, cbuffer, 0, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 350, cbuffer, 60, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 400, cbuffer, 120, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 475, cbuffer, 210, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 580, 350, 400, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 730, 400, 475, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 955, 580, 730, r_ab, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 300, 955, 3, 0, 2);
...
#endif /* ElectronRepulsionGeom2000RecSFSD_hpp */
