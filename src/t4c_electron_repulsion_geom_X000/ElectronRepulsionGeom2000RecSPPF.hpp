#ifndef ElectronRepulsionGeom2000RecSPPF_hpp
#define ElectronRepulsionGeom2000RecSPPF_hpp
...
/// @brief Computes d^(2)/dA^(2)(SP|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_sppf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(4.0 * a_exp * a_exp, {481, 541});

                pbuffer.scale(4.0 * a_exp * a_exp, {601, 691});

                pbuffer.scale(4.0 * a_exp * a_exp, {781, 881});

                pbuffer.scale(4.0 * a_exp * a_exp, {881, 1031});
            ...
            t4cfunc::ket_transform<1, 3>(skbuffer, 378, ckbuffer, 0, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 441, ckbuffer, 90, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 504, ckbuffer, 180, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 630, ckbuffer, 360, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 840, 441, 504, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 1029, 504, 630, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 1407, 840, 1029, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 378, 1407, 1, 1, 3);
...
#endif /* ElectronRepulsionGeom2000RecSPPF_hpp */
