...
/// @brief Computes (SD|1/|r-r'||PP)  integral derivatives for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_geom_1000_sdpp(T&                               distributor,
                                       ...
                                       const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::bra_transform<0, 2>(sbuffer, 0, skbuffer, 144, 1, 1);
            
            t4cfunc::bra_transform<0, 2>(sbuffer, 45, skbuffer, 198, 1, 1);
            
            t4cfunc::bra_transform<0, 2>(sbuffer, 90, skbuffer, 252, 1, 1);
