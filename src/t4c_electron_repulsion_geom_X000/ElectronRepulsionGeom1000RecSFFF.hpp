#ifndef ElectronRepulsionGeom1000RecSFFF_hpp
#define ElectronRepulsionGeom1000RecSFFF_hpp
...
/// @brief Computes (SF|1/|r-r'||FF)  integral derivatives for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_geom_1000_sfff(T&                               distributor,
                                       ...
                                       const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(9700, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(2695, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 250, pbuffer, 3491, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 460, pbuffer, 3911, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 740, pbuffer, 4471, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 890, pbuffer, 4621, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1115, pbuffer, 4846, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1430, pbuffer, 5161, 420, ket_width, ket_npgtos);
            }
            ...
            t4cfunc::bra_transform<0, 3>(sbuffer, 0, skbuffer, 1225, 3, 3);
            
            t4cfunc::bra_transform<0, 3>(sbuffer, 343, skbuffer, 1715, 3, 3);
            
            t4cfunc::bra_transform<0, 3>(sbuffer, 686, skbuffer, 2205, 3, 3);
...
#endif /* ElectronRepulsionGeom1000RecSFFF_hpp */
