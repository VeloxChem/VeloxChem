#ifndef ElectronRepulsionGeom1000RecFDSF_hpp
#define ElectronRepulsionGeom1000RecFDSF_hpp
...
/// @brief Computes d^(1)/dA^(1)(FD|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1000_fdsf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 680, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 60, pbuffer, 1230, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 160, pbuffer, 1855, 150, ket_width, ket_npgtos);
                ...
                pbuffer.scale(2.0 * a_exp, {2431, 2641});

                pbuffer.scale(2.0 * a_exp, {2851, 3131});

                t2cfunc::reduce(cbuffer, 310, pbuffer, 680, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 370, pbuffer, 1230, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 470, pbuffer, 1855, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 620, pbuffer, 2431, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 830, pbuffer, 2851, 280, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<0, 3>(skbuffer, 805, cbuffer, 310, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 847, cbuffer, 370, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 917, cbuffer, 470, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1022, cbuffer, 620, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1169, cbuffer, 830, 0, 6);
            ...
            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 1365, 805, 847, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 1491, 847, 917, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 1701, 917, 1022, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 2016, 1022, 1169, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 2457, 0, 1365, 1491, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 2835, 42, 1491, 1701, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pgxx(skbuffer, 3465, 112, 1701, 2016, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ddxx(skbuffer, 4410, 217, 2457, 2835, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dfxx(skbuffer, 5166, 343, 2835, 3465, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_fdxx(skbuffer, 6426, 553, 4410, 5166, r_ab, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 0, skbuffer, 6426, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 245, skbuffer, 6846, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 490, skbuffer, 7266, 0, 3);
...
#endif /* ElectronRepulsionGeom1000RecFDSF_hpp */
