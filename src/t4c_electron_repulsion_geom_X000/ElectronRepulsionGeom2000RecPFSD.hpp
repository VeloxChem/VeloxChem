#ifndef ElectronRepulsionGeom2000RecPFSD_hpp
#define ElectronRepulsionGeom2000RecPFSD_hpp
...
/// @brief Computes d^(2)/dA^(2)(PF|1/|r-r'||SD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_pfsd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(3735, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<0, 2>(skbuffer, 1650, cbuffer, 0, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1700, cbuffer, 60, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1925, cbuffer, 150, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1975, cbuffer, 210, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 2050, cbuffer, 300, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 2155, cbuffer, 426, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 1775, 1650, 1700, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 2295, 1925, 1975, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 2445, 1975, 2050, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 2670, 2050, 2155, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 2985, 2295, 2445, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 3285, 2445, 2670, r_ab, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 1650, 2985, 3, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 300, 1700, 3285, 4, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 750, 1775, 0, 300, r_ab, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 750, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 105, skbuffer, 900, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 210, skbuffer, 1050, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 315, skbuffer, 1200, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 420, skbuffer, 1350, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 525, skbuffer, 1500, 0, 2);
...
#endif /* ElectronRepulsionGeom2000RecPFSD_hpp */
