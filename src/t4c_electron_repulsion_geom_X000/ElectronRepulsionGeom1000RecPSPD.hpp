#ifndef ElectronRepulsionGeom1000RecPSPD_hpp
#define ElectronRepulsionGeom1000RecPSPD_hpp
...
/// @brief Computes (PS|1/|r-r'||PD)  integral derivatives for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_geom_1000_pspd(T&                               distributor,
                                       ...
                                       const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<1, 2>(skbuffer, 15, ckbuffer, 18, 0, 0);
            
            t4cfunc::ket_transform<1, 2>(skbuffer, 30, ckbuffer, 36, 0, 1);
            
            t4cfunc::ket_transform<1, 2>(skbuffer, 75, ckbuffer, 90, 0, 2);
            
            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 165, skbuffer, 30, 15, r_ab, 1, 2);
            
            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 210, 30, 75, r_ab, 1, 2);
            
            erirec::comp_bra_geom_hrr_electron_repulsion_psxx(skbuffer, 345, 165, 210, 0, r_ab, 1, 2);
            
            t4cfunc::bra_transform<1, 0>(sbuffer, 0, skbuffer, 345, 1, 2);
            
            t4cfunc::bra_transform<1, 0>(sbuffer, 45, skbuffer, 390, 1, 2);
            
            t4cfunc::bra_transform<1, 0>(sbuffer, 90, skbuffer, 435, 1, 2);
...
#endif /* ElectronRepulsionGeom1000RecPSPD_hpp */
