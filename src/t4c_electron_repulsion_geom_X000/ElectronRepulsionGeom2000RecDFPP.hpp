#ifndef ElectronRepulsionGeom2000RecDFPP_hpp
#define ElectronRepulsionGeom2000RecDFPP_hpp
...
/// @brief Computes d^(2)/dA^(2)(DF|1/|r-r'||PP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_dfpp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(1494, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(1494, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(17235, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 90, pbuffer, 686, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 120, pbuffer, 836, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 180, pbuffer, 1181, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 225, pbuffer, 1361, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 315, pbuffer, 1763, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 378, pbuffer, 1952, 126, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 504, pbuffer, 686, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 534, pbuffer, 836, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 594, pbuffer, 1181, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 639, pbuffer, 1361, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 729, pbuffer, 1763, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 792, pbuffer, 1952, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 918, pbuffer, 2358, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1002, pbuffer, 2526, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1170, pbuffer, 2862, 108, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1278, pbuffer, 2970, 216, ket_width, ket_npgtos);

            }

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 0, cbuffer, 0, 30, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 90, cbuffer, 90, 120, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 180, cbuffer, 180, 225, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 315, cbuffer, 315, 378, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 504, cbuffer, 504, 534, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 594, cbuffer, 594, 639, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 729, cbuffer, 729, 792, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 918, cbuffer, 918, 1002, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 1170, cbuffer, 1170, 1278, cfactors, 6, 0, 7);

            t4cfunc::ket_transform<1, 1>(skbuffer, 0, ckbuffer, 0, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 10674, ckbuffer, 90, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 10764, ckbuffer, 180, 0, 4);

            t4cfunc::ket_transform<1, 1>(skbuffer, 10899, ckbuffer, 315, 0, 5);

            t4cfunc::ket_transform<1, 1>(skbuffer, 11763, ckbuffer, 504, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 11853, ckbuffer, 594, 0, 4);

            t4cfunc::ket_transform<1, 1>(skbuffer, 11988, ckbuffer, 729, 0, 5);

            t4cfunc::ket_transform<1, 1>(skbuffer, 12177, ckbuffer, 918, 0, 6);

            t4cfunc::ket_transform<1, 1>(skbuffer, 12429, ckbuffer, 1170, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 11088, 10674, 10764, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 11358, 10764, 10899, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 12753, 11763, 11853, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 13023, 11853, 11988, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 13428, 11988, 12177, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pixx(skbuffer, 13995, 12177, 12429, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 14751, 12753, 13023, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 15291, 13023, 13428, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_dhxx(skbuffer, 16101, 13428, 13995, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 2574, 0, 11088, 11358, r_ab, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 90, 10674, 14751, 3, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 630, 10764, 15291, 4, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 1440, 10899, 16101, 5, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 3384, 11088, 90, 630, r_ab, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pgxx(skbuffer, 5004, 11358, 630, 1440, r_ab, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dfxx(skbuffer, 7434, 2574, 3384, 5004, r_ab, 1, 1);

            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 7434, 1, 1);

            t4cfunc::bra_transform<2, 3>(sbuffer, 315, skbuffer, 7974, 1, 1);

            t4cfunc::bra_transform<2, 3>(sbuffer, 630, skbuffer, 8514, 1, 1);

            t4cfunc::bra_transform<2, 3>(sbuffer, 945, skbuffer, 9054, 1, 1);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1260, skbuffer, 9594, 1, 1);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1575, skbuffer, 10134, 1, 1);
...
#endif /* ElectronRepulsionGeom2000RecDFPP_hpp */
