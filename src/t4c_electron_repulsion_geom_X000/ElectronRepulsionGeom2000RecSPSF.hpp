#ifndef ElectronRepulsionGeom2000RecSPSF_hpp
#define ElectronRepulsionGeom2000RecSPSF_hpp
...
/// @brief Computes d^(2)/dA^(2)(SP|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_spsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(220, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(595, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 30, pbuffer, 140, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 60, pbuffer, 266, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 120, pbuffer, 386, 100, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 3>(skbuffer, 126, cbuffer, 0, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 147, cbuffer, 30, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 168, cbuffer, 60, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 210, cbuffer, 120, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 280, 147, 168, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 343, 168, 210, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 469, 280, 343, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 126, 469, 1, 0, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 0, skbuffer, 0, 0, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 21, skbuffer, 21, 0, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 42, skbuffer, 42, 0, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 63, skbuffer, 63, 0, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 84, skbuffer, 84, 0, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 105, skbuffer, 105, 0, 3);
...
#endif /* ElectronRepulsionGeom2000RecSPSF_hpp */
