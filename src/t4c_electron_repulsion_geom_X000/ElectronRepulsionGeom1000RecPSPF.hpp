#ifndef ElectronRepulsionGeom1000RecPSPF_hpp
#define ElectronRepulsionGeom1000RecPSPF_hpp
...
/// @brief Computes (PS|1/|r-r'||PF)  integral derivatives for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_geom_1000_pspf(T&                               distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * a_exp, {158, 188});
                
                pbuffer.scale(2.0 * a_exp, {218, 263});
                
                pbuffer.scale(2.0 * a_exp, {308, 368});
                
                pbuffer.scale(2.0 * a_exp, {368, 458});
            ...
            t4cfunc::ket_transform<1, 3>(skbuffer, 21, ckbuffer, 30, 0, 0);
            
            t4cfunc::ket_transform<1, 3>(skbuffer, 42, ckbuffer, 60, 0, 1);
            
            t4cfunc::ket_transform<1, 3>(skbuffer, 105, ckbuffer, 150, 0, 2);
            
            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 231, skbuffer, 42, 21, r_ab, 1, 3);
            
            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 294, 42, 105, r_ab, 1, 3);
            
            erirec::comp_bra_geom_hrr_electron_repulsion_psxx(skbuffer, 483, 231, 294, 0, r_ab, 1, 3);
            
            t4cfunc::bra_transform<1, 0>(sbuffer, 0, skbuffer, 483, 1, 3);
            
            t4cfunc::bra_transform<1, 0>(sbuffer, 63, skbuffer, 546, 1, 3);
            
            t4cfunc::bra_transform<1, 0>(sbuffer, 126, skbuffer, 609, 1, 3);
...
#endif /* ElectronRepulsionGeom1000RecPSPF_hpp */
