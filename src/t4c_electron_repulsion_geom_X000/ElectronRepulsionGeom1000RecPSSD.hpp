#ifndef ElectronRepulsionGeom1000RecPSSD_hpp
#define ElectronRepulsionGeom1000RecPSSD_hpp
...
/// @brief Computes (PS|1/|r-r'||SD)  integral derivatives for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_geom_1000_pssd(T&                               distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 12, pbuffer, 44, 18, ket_width, ket_npgtos);
                
                t2cfunc::reduce(cbuffer, 30, pbuffer, 80, 36, ket_width, ket_npgtos);
            }

            t4cfunc::ket_transform<0, 2>(skbuffer, 0, cbuffer, 0, 0, 0);
            
            t4cfunc::ket_transform<0, 2>(skbuffer, 5, cbuffer, 6, 0, 0);
            
            t4cfunc::ket_transform<0, 2>(skbuffer, 10, cbuffer, 12, 0, 1);
            
            t4cfunc::ket_transform<0, 2>(skbuffer, 25, cbuffer, 30, 0, 2);
            
            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 55, skbuffer, 10, 5, r_ab, 0, 2);
            
            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 70, 10, 25, r_ab, 0, 2);
            
            erirec::comp_bra_geom_hrr_electron_repulsion_psxx(skbuffer, 115, 55, 70, 0, r_ab, 0, 2);
            
            t4cfunc::bra_transform<1, 0>(sbuffer, 0, skbuffer, 115, 0, 2);
            
            t4cfunc::bra_transform<1, 0>(sbuffer, 15, skbuffer, 130, 0, 2);
            
            t4cfunc::bra_transform<1, 0>(sbuffer, 30, skbuffer, 145, 0, 2);
...
#endif /* ElectronRepulsionGeom1000RecPSSD_hpp */
