#ifndef ElectronRepulsionGeom2000RecSPFF_hpp
#define ElectronRepulsionGeom2000RecSPFF_hpp
...
/// @brief Computes d^(2)/dA^(2)(SP|1/|r-r'||FF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_spff(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(8536, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(4165, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 507, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 30, pbuffer, 597, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 75, pbuffer, 732, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 138, pbuffer, 921, 84, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 222, pbuffer, 507, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 252, pbuffer, 597, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 297, pbuffer, 732, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 360, pbuffer, 921, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 444, pbuffer, 1209, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 504, pbuffer, 1329, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 594, pbuffer, 1509, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 720, pbuffer, 1761, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 888, pbuffer, 2097, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 988, pbuffer, 2197, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1138, pbuffer, 2347, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1348, pbuffer, 2557, 280, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<3, 3>(skbuffer, 882, ckbuffer, 864, 0, 1);

            t4cfunc::ket_transform<3, 3>(skbuffer, 1029, ckbuffer, 2028, 0, 1);

            t4cfunc::ket_transform<3, 3>(skbuffer, 1176, ckbuffer, 4056, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 1470, ckbuffer, 7536, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 1960, 1029, 1176, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 2401, 1176, 1470, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 3283, 1960, 2401, r_ab, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 882, 3283, 1, 3, 3);
...
#endif /* ElectronRepulsionGeom2000RecSPFF_hpp */
