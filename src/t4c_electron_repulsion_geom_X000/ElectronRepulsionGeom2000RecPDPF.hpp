#ifndef ElectronRepulsionGeom2000RecPDPF_hpp
#define ElectronRepulsionGeom2000RecPDPF_hpp
...
/// @brief Computes d^(2)/dA^(2)(PD|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_pdpf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(10059, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<1, 3>(skbuffer, 4284, ckbuffer, 0, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 4410, ckbuffer, 180, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 4998, ckbuffer, 480, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 5124, ckbuffer, 660, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 5334, ckbuffer, 960, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 5649, ckbuffer, 1410, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 4620, 4284, 4410, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 6090, 4998, 5124, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 6468, 5124, 5334, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 7098, 5334, 5649, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 8043, 6090, 6468, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 8799, 6468, 7098, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 4284, 8043, 2, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 756, 4410, 8799, 3, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 2016, 4620, 0, 756, r_ab, 1, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 0, skbuffer, 2016, 1, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 315, skbuffer, 2394, 1, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 630, skbuffer, 2772, 1, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 945, skbuffer, 3150, 1, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 1260, skbuffer, 3528, 1, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 1575, skbuffer, 3906, 1, 3);
...
#endif /* ElectronRepulsionGeom2000RecPDPF_hpp */
