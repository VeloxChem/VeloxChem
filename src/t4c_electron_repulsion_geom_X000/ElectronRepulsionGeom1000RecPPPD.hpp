...
/// @brief Computes (PP|1/|r-r'||PD)  integral derivatives for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_geom_1000_pppd(T&                               distributor,
                                       ...
                                       const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<1, 2>(skbuffer, 45, ckbuffer, 54, 0, 1);

            t4cfunc::ket_transform<1, 2>(skbuffer, 90, ckbuffer, 108, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 180, ckbuffer, 216, 0, 3);
            
            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 330, 45, 90, r_ab, 1, 2);
            
            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 465, 90, 180, r_ab, 1, 2);
            
            erirec::comp_bra_geom_hrr_electron_repulsion_ppxx(skbuffer, 735, 330, 465, 0, r_ab, 1, 2);
            
            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 735, 1, 2);
            
            t4cfunc::bra_transform<1, 1>(sbuffer, 135, skbuffer, 870, 1, 2);
            
            t4cfunc::bra_transform<1, 1>(sbuffer, 270, skbuffer, 1005, 1, 2);
