#ifndef ElectronRepulsionGeom2000RecDFDF_hpp
#define ElectronRepulsionGeom2000RecDFDF_hpp
...
/// @brief Computes d^(2)/dA^(2)(DF|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_dfdf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(7636, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(22410, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(67025, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(4.0 * a_exp * a_exp, {12741, 13021});

                pbuffer.scale(4.0 * a_exp * a_exp, {13301, 13721});

                pbuffer.scale(4.0 * a_exp * a_exp, {14141, 14729});

                pbuffer.scale(4.0 * a_exp * a_exp, {15317, 15677});

                pbuffer.scale(4.0 * a_exp * a_exp, {15677, 16217});

                pbuffer.scale(4.0 * a_exp * a_exp, {16217, 16973});

                t2cfunc::reduce(cbuffer, 2576, pbuffer, 3925, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2676, pbuffer, 4425, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2826, pbuffer, 5175, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3036, pbuffer, 6600, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3186, pbuffer, 7200, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3411, pbuffer, 8100, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3726, pbuffer, 9675, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3936, pbuffer, 10305, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4251, pbuffer, 11250, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4692, pbuffer, 12741, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4972, pbuffer, 13301, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5392, pbuffer, 14141, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5980, pbuffer, 15317, 360, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 6340, pbuffer, 15677, 540, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 6880, pbuffer, 16217, 756, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1350, cbuffer, 460, 560, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 1650, cbuffer, 560, 710, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 2100, 1350, 1650, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2700, cbuffer, 920, 1070, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 3150, cbuffer, 1070, 1295, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 3825, 2700, 3150, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 4725, cbuffer, 1610, 1820, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 5355, cbuffer, 1820, 2135, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 6300, 4725, 5355, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 7560, cbuffer, 2576, 2676, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 7860, cbuffer, 2676, 2826, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 8310, 7560, 7860, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 8910, cbuffer, 3036, 3186, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 9360, cbuffer, 3186, 3411, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 10035, 8910, 9360, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 10935, cbuffer, 3726, 3936, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 11565, cbuffer, 3936, 4251, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 12510, 10935, 11565, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 13770, cbuffer, 4692, 4972, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 14610, cbuffer, 4972, 5392, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 15870, 13770, 14610, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 17550, cbuffer, 5980, 6340, cfactors, 6, 0, 7);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 18630, cbuffer, 6340, 6880, cfactors, 6, 0, 7);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 20250, 17550, 18630, cfactors, 6, 0, 7);

            t4cfunc::ket_transform<2, 3>(skbuffer, 0, ckbuffer, 750, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 41510, ckbuffer, 2100, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 41860, ckbuffer, 3825, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 42385, ckbuffer, 6300, 0, 5);

            t4cfunc::ket_transform<2, 3>(skbuffer, 45745, ckbuffer, 8310, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 46095, ckbuffer, 10035, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 46620, ckbuffer, 12510, 0, 5);

            t4cfunc::ket_transform<2, 3>(skbuffer, 47355, ckbuffer, 15870, 0, 6);

            t4cfunc::ket_transform<2, 3>(skbuffer, 48335, ckbuffer, 20250, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 43120, 41510, 41860, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 44170, 41860, 42385, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 49595, 45745, 46095, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 50645, 46095, 46620, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 52220, 46620, 47355, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pixx(skbuffer, 54425, 47355, 48335, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 57365, 49595, 50645, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 59465, 50645, 52220, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_dhxx(skbuffer, 62615, 52220, 54425, r_ab, 2, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 10010, 0, 43120, 44170, r_ab, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 350, 41510, 57365, 3, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 2450, 41860, 59465, 4, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 5600, 42385, 62615, 5, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 13160, 43120, 350, 2450, r_ab, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pgxx(skbuffer, 19460, 44170, 2450, 5600, r_ab, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dfxx(skbuffer, 28910, 10010, 13160, 19460, r_ab, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 28910, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1225, skbuffer, 31010, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 2450, skbuffer, 33110, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 3675, skbuffer, 35210, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 4900, skbuffer, 37310, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 6125, skbuffer, 39410, 2, 3);
...
#endif /* ElectronRepulsionGeom2000RecDFDF_hpp */
