#ifndef ElectronRepulsionGeom2000RecPFDD_hpp
#define ElectronRepulsionGeom2000RecPFDD_hpp
...
/// @brief Computes d^(2)/dA^(2)(PF|1/|r-r'||DD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_pfdd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(18675, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<2, 2>(skbuffer, 8250, ckbuffer, 480, 0, 3);

            t4cfunc::ket_transform<2, 2>(skbuffer, 8500, ckbuffer, 1560, 0, 4);

            t4cfunc::ket_transform<2, 2>(skbuffer, 9625, ckbuffer, 2580, 0, 3);

            t4cfunc::ket_transform<2, 2>(skbuffer, 9875, ckbuffer, 3660, 0, 4);

            t4cfunc::ket_transform<2, 2>(skbuffer, 10250, ckbuffer, 5208, 0, 5);

            t4cfunc::ket_transform<2, 2>(skbuffer, 10775, ckbuffer, 7308, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 8875, 8250, 8500, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 11475, 9625, 9875, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 12225, 9875, 10250, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 13350, 10250, 10775, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 14925, 11475, 12225, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 16425, 12225, 13350, r_ab, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 8250, 14925, 3, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 1500, 8500, 16425, 4, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 3750, 8875, 0, 1500, r_ab, 2, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 3750, 2, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 525, skbuffer, 4500, 2, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1050, skbuffer, 5250, 2, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1575, skbuffer, 6000, 2, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 2100, skbuffer, 6750, 2, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 2625, skbuffer, 7500, 2, 2);
...
#endif /* ElectronRepulsionGeom2000RecPFDD_hpp */
