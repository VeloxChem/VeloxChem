#ifndef ElectronRepulsionGeom2000RecPFSP_hpp
#define ElectronRepulsionGeom2000RecPFSP_hpp
...
/// @brief Computes d^(2)/dA^(2)(PF|1/|r-r'||SP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_pfsp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(2241, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<0, 1>(skbuffer, 990, cbuffer, 0, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1020, cbuffer, 30, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1155, cbuffer, 75, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1185, cbuffer, 105, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1230, cbuffer, 150, 0, 5);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1293, cbuffer, 213, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 1065, 990, 1020, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 1377, 1155, 1185, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 1467, 1185, 1230, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 1602, 1230, 1293, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 1791, 1377, 1467, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 1971, 1467, 1602, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 990, 1791, 3, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 180, 1020, 1971, 4, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 450, 1065, 0, 180, r_ab, 0, 1);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 450, 0, 1);

            t4cfunc::bra_transform<1, 3>(sbuffer, 63, skbuffer, 540, 0, 1);

            t4cfunc::bra_transform<1, 3>(sbuffer, 126, skbuffer, 630, 0, 1);

            t4cfunc::bra_transform<1, 3>(sbuffer, 189, skbuffer, 720, 0, 1);

            t4cfunc::bra_transform<1, 3>(sbuffer, 252, skbuffer, 810, 0, 1);

            t4cfunc::bra_transform<1, 3>(sbuffer, 315, skbuffer, 900, 0, 1);
...
#endif /* ElectronRepulsionGeom2000RecPFSP_hpp */
