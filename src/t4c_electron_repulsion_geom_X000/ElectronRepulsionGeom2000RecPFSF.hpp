#ifndef ElectronRepulsionGeom2000RecPFSF_hpp
#define ElectronRepulsionGeom2000RecPFSF_hpp
...
/// @brief Computes d^(2)/dA^(2)(PF|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_pfsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(5229, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 250, pbuffer, 1230, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 350, pbuffer, 1855, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 500, pbuffer, 2431, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 710, pbuffer, 2851, 280, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 3>(skbuffer, 2310, cbuffer, 0, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2380, cbuffer, 100, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2695, cbuffer, 250, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2765, cbuffer, 350, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2870, cbuffer, 500, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 3017, cbuffer, 710, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 2485, 2310, 2380, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 3213, 2695, 2765, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 3423, 2765, 2870, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 3738, 2870, 3017, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 4179, 3213, 3423, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 4599, 3423, 3738, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 2310, 4179, 3, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 420, 2380, 4599, 4, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 1050, 2485, 0, 420, r_ab, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 1050, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 147, skbuffer, 1260, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 294, skbuffer, 1470, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 441, skbuffer, 1680, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 588, skbuffer, 1890, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 735, skbuffer, 2100, 0, 3);
...
#endif /* ElectronRepulsionGeom2000RecPFSF_hpp */
