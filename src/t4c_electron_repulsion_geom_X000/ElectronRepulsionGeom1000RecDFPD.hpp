#ifndef ElectronRepulsionGeom1000RecDFPD_hpp
#define ElectronRepulsionGeom1000RecDFPD_hpp
...
/// @brief Computes d^(1)/dA^(1)(DF|1/|r-r'||PD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1000_dfpd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<1, 2>(skbuffer, 825, ckbuffer, 450, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 975, ckbuffer, 630, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 1200, ckbuffer, 900, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 1515, ckbuffer, 1278, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 375, 0, 150, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 1935, 825, 975, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 2385, 975, 1200, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 3060, 1200, 1515, r_ab, 1, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 4005, 0, 1935, 2385, r_ab, 1, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pgxx(skbuffer, 5355, 150, 2385, 3060, r_ab, 1, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dfxx(skbuffer, 7380, 375, 4005, 5355, r_ab, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 7380, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 525, skbuffer, 8280, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1050, skbuffer, 9180, 1, 2);
...
#endif /* ElectronRepulsionGeom1000RecDFPD_hpp */
