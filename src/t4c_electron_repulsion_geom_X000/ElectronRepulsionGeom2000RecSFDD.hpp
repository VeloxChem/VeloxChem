#ifndef ElectronRepulsionGeom2000RecSFDD_hpp
#define ElectronRepulsionGeom2000RecSFDD_hpp
...
/// @brief Computes d^(2)/dA^(2)(SF|1/|r-r'||DD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_sfdd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(6275, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<2, 2>(skbuffer, 1500, ckbuffer, 480, 0, 3);

            t4cfunc::ket_transform<2, 2>(skbuffer, 1750, ckbuffer, 1320, 0, 3);

            t4cfunc::ket_transform<2, 2>(skbuffer, 2000, ckbuffer, 2400, 0, 4);

            t4cfunc::ket_transform<2, 2>(skbuffer, 2375, ckbuffer, 3948, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 2900, 1750, 2000, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 3650, 2000, 2375, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 4775, 2900, 3650, r_ab, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 1500, 4775, 3, 2, 2);
...
#endif /* ElectronRepulsionGeom2000RecSFDD_hpp */
