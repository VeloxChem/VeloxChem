#ifndef ElectronRepulsionGeom2000RecFSSP_hpp
#define ElectronRepulsionGeom2000RecFSSP_hpp
...
/// @brief Computes d^(2)/dA^(2)(FS|1/|r-r'||SP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_fssp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(2688, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 15, pbuffer, 37, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 24, pbuffer, 100, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 42, pbuffer, 192, 30, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 72, pbuffer, 7, 3, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 75, pbuffer, 37, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 84, pbuffer, 100, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 102, pbuffer, 192, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 132, pbuffer, 297, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 177, pbuffer, 387, 63, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 1>(skbuffer, 0, cbuffer, 0, 0, 0);

            t4cfunc::ket_transform<0, 1>(skbuffer, 21, cbuffer, 3, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1695, cbuffer, 12, 0, 0);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1698, cbuffer, 15, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1707, cbuffer, 24, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1725, cbuffer, 42, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1845, cbuffer, 72, 0, 0);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1848, cbuffer, 75, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1857, cbuffer, 84, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1875, cbuffer, 102, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1905, cbuffer, 132, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1950, cbuffer, 177, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 372, 0, 21, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 1755, 1695, 1698, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 1764, 1698, 1707, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 1791, 1707, 1725, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 2013, 1845, 1848, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 2022, 1848, 1857, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 2049, 1857, 1875, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 2103, 1875, 1905, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 2193, 1905, 1950, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dsxx(skbuffer, 2328, 2013, 2022, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 2346, 2022, 2049, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 2400, 2049, 2103, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 2508, 2103, 2193, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_psxx(skbuffer, 381, 0, 1755, 1764, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 462, 21, 1764, 1791, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dsxx(skbuffer, 1029, 372, 381, 462, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 3, 1695, 2328, 0, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 30, 1698, 2346, 1, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 84, 1707, 2400, 2, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 192, 1725, 2508, 3, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_psxx(skbuffer, 408, 1755, 3, 30, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ppxx(skbuffer, 543, 1764, 30, 84, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 705, 1791, 84, 192, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dsxx(skbuffer, 1083, 381, 408, 543, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dpxx(skbuffer, 1191, 462, 543, 705, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_fsxx(skbuffer, 1515, 1029, 1083, 1191, r_ab, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 0, skbuffer, 1515, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 21, skbuffer, 1545, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 42, skbuffer, 1575, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 63, skbuffer, 1605, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 84, skbuffer, 1635, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 105, skbuffer, 1665, 0, 1);
...
#endif /* ElectronRepulsionGeom2000RecFSSP_hpp */
