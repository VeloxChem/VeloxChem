#ifndef ElectronRepulsionGeom2000RecSFSF_hpp
#define ElectronRepulsionGeom2000RecSFSF_hpp
...
/// @brief Computes d^(2)/dA^(2)(SF|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_sfsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(560, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(1757, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 930, 100, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 200, pbuffer, 1320, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 350, pbuffer, 1620, 210, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 3>(skbuffer, 420, cbuffer, 0, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 490, cbuffer, 100, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 560, cbuffer, 200, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 665, cbuffer, 350, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 812, 490, 560, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 1022, 560, 665, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 1337, 812, 1022, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 420, 1337, 3, 0, 3);
...
#endif /* ElectronRepulsionGeom2000RecSFSF_hpp */
