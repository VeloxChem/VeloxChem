#ifndef ElectronRepulsionGeom1000RecFDPP_hpp
#define ElectronRepulsionGeom1000RecFDPP_hpp
...
/// @brief Computes d^(1)/dA^(1)(FD|1/|r-r'||PP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1000_fdpp(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * a_exp, {1392, 1455});

                pbuffer.scale(2.0 * a_exp, {1518, 1644});

                pbuffer.scale(2.0 * a_exp, {1770, 1854});

                pbuffer.scale(2.0 * a_exp, {1854, 2022});
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 144, cbuffer, 144, 189, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 279, cbuffer, 279, 297, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 333, cbuffer, 333, 363, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 423, cbuffer, 423, 468, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 558, cbuffer, 558, 621, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 747, cbuffer, 747, 831, cfactors, 6, 0, 6);

            t4cfunc::ket_transform<1, 1>(skbuffer, 0, ckbuffer, 0, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 54, ckbuffer, 54, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 144, ckbuffer, 144, 0, 4);

            t4cfunc::ket_transform<1, 1>(skbuffer, 1035, ckbuffer, 279, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 1089, ckbuffer, 333, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 1179, ckbuffer, 423, 0, 4);

            t4cfunc::ket_transform<1, 1>(skbuffer, 1314, ckbuffer, 558, 0, 5);

            t4cfunc::ket_transform<1, 1>(skbuffer, 1503, ckbuffer, 747, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 279, 0, 54, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 441, 54, 144, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 711, 279, 441, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 1755, 1035, 1089, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 1917, 1089, 1179, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 2187, 1179, 1314, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 2592, 1314, 1503, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 3159, 0, 1755, 1917, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 3645, 54, 1917, 2187, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pgxx(skbuffer, 4455, 144, 2187, 2592, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ddxx(skbuffer, 5670, 279, 3159, 3645, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dfxx(skbuffer, 6642, 441, 3645, 4455, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_fdxx(skbuffer, 8262, 711, 5670, 6642, r_ab, 1, 1);

            t4cfunc::bra_transform<3, 2>(sbuffer, 0, skbuffer, 8262, 1, 1);

            t4cfunc::bra_transform<3, 2>(sbuffer, 315, skbuffer, 8802, 1, 1);

            t4cfunc::bra_transform<3, 2>(sbuffer, 630, skbuffer, 9342, 1, 1);
...
#endif /* ElectronRepulsionGeom1000RecFDPP_hpp */
