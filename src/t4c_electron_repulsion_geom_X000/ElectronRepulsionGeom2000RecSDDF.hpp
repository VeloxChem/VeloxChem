#ifndef ElectronRepulsionGeom2000RecSDDF_hpp
#define ElectronRepulsionGeom2000RecSDDF_hpp
...
/// @brief Computes d^(2)/dA^(2)(SD|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_sddf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(1702, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(4995, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(5495, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 276, pbuffer, 1067, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 336, pbuffer, 1247, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 426, pbuffer, 1517, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 552, pbuffer, 1955, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 652, pbuffer, 2155, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 802, pbuffer, 2455, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1012, pbuffer, 2875, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1162, pbuffer, 3025, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1387, pbuffer, 3250, 315, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<2, 3>(skbuffer, 1260, ckbuffer, 450, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 1470, ckbuffer, 1260, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 1680, ckbuffer, 2370, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 2030, ckbuffer, 4095, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 2555, 1470, 1680, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 3185, 1680, 2030, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 4235, 2555, 3185, r_ab, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 1260, 4235, 2, 2, 3);
...
#endif /* ElectronRepulsionGeom2000RecSDDF_hpp */
