#ifndef ElectronRepulsionGeom1000RecPPSD_hpp
#define ElectronRepulsionGeom1000RecPPSD_hpp
...
/// @brief Computes (PP|1/|r-r'||SD)  integral derivatives for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_geom_1000_ppsd(T&                               distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * a_exp, {138, 174});
                
                pbuffer.scale(2.0 * a_exp, {210, 270});
            ...
            t4cfunc::ket_transform<0, 2>(skbuffer, 15, cbuffer, 18, 0, 1);
            
            t4cfunc::ket_transform<0, 2>(skbuffer, 30, cbuffer, 36, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 60, cbuffer, 72, 0, 3);
            
            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 110, 15, 30, r_ab, 0, 2);
            
            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 155, 30, 60, r_ab, 0, 2);
            
            erirec::comp_bra_geom_hrr_electron_repulsion_ppxx(skbuffer, 245, 110, 155, 0, r_ab, 0, 2);

            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 245, 0, 2);
            
            t4cfunc::bra_transform<1, 1>(sbuffer, 45, skbuffer, 290, 0, 2);
            
            t4cfunc::bra_transform<1, 1>(sbuffer, 90, skbuffer, 335, 0, 2);
...
#endif /* ElectronRepulsionGeom1000RecPPSD_hpp */
