#ifndef ElectronRepulsionGeom2000RecPPPF_hpp
#define ElectronRepulsionGeom2000RecPPPF_hpp
...
/// @brief Computes d^(2)/dA^(2)(PP|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_pppf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(5691, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<1, 3>(skbuffer, 2268, ckbuffer, 0, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 2331, ckbuffer, 90, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 2646, ckbuffer, 270, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 2709, ckbuffer, 360, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 2835, ckbuffer, 540, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 3045, ckbuffer, 840, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 2457, 2268, 2331, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 3360, 2646, 2709, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 3549, 2709, 2835, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 3927, 2835, 3045, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 4557, 3360, 3549, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 4935, 3549, 3927, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 2268, 4557, 1, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 378, 2331, 4935, 2, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ppxx(skbuffer, 1134, 2457, 0, 378, r_ab, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 1134, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 189, skbuffer, 1323, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 378, skbuffer, 1512, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 567, skbuffer, 1701, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 756, skbuffer, 1890, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 945, skbuffer, 2079, 1, 3);
...
#endif /* ElectronRepulsionGeom2000RecPPPF_hpp */
