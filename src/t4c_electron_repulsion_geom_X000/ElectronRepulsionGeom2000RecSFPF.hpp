#ifndef ElectronRepulsionGeom2000RecSFPF_hpp
#define ElectronRepulsionGeom2000RecSFPF_hpp
...
/// @brief Computes d^(2)/dA^(2)(SF|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_sfpf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(5271, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 300, cbuffer, 250, 350, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 600, cbuffer, 500, 650, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1050, cbuffer, 875, 1085, cfactors, 6, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1260, ckbuffer, 0, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1470, ckbuffer, 300, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1680, ckbuffer, 600, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1995, ckbuffer, 1050, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 2436, 1470, 1680, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 3066, 1680, 1995, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 4011, 2436, 3066, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 1260, 4011, 3, 1, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 0, skbuffer, 0, 1, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 147, skbuffer, 210, 1, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 294, skbuffer, 420, 1, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 441, skbuffer, 630, 1, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 588, skbuffer, 840, 1, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 735, skbuffer, 1050, 1, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 3, 1, 3, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom2000RecSFPF_hpp */
