#ifndef ElectronRepulsionGeom1000RecPPSF_hpp
#define ElectronRepulsionGeom1000RecPPSF_hpp
...
/// @brief Computes (PP|1/|r-r'||SF)  integral derivatives for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_geom_1000_ppsf(T&                               distributor,
                                       ...
                                       const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(532, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<0, 3>(skbuffer, 21, cbuffer, 30, 0, 1);
            
            t4cfunc::ket_transform<0, 3>(skbuffer, 42, cbuffer, 60, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 84, cbuffer, 120, 0, 3);
            
            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 154, 21, 42, r_ab, 0, 3);
            
            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 217, 42, 84, r_ab, 0, 3);
            
            erirec::comp_bra_geom_hrr_electron_repulsion_ppxx(skbuffer, 343, 154, 217, 0, r_ab, 0, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 343, 0, 3);
            
            t4cfunc::bra_transform<1, 1>(sbuffer, 63, skbuffer, 406, 0, 3);
            
            t4cfunc::bra_transform<1, 1>(sbuffer, 126, skbuffer, 469, 0, 3);
...
#endif /* ElectronRepulsionGeom1000RecPPSF_hpp */
