#ifndef ElectronRepulsionGeom2000RecDFPF_hpp
#define ElectronRepulsionGeom2000RecDFPF_hpp
...
/// @brief Computes d^(2)/dA^(2)(DF|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_dfpf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(4150, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(4980, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(40215, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 250, pbuffer, 2525, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 350, pbuffer, 3025, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 500, pbuffer, 4150, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 650, pbuffer, 4750, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 875, pbuffer, 5965, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1085, pbuffer, 6595, 315, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 1400, pbuffer, 2525, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1500, pbuffer, 3025, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1650, pbuffer, 4150, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1800, pbuffer, 4750, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2025, pbuffer, 5965, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2235, pbuffer, 6595, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2550, pbuffer, 7708, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2830, pbuffer, 8268, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3250, pbuffer, 9108, 360, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3610, pbuffer, 9468, 540, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1680, cbuffer, 1400, 1500, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1980, cbuffer, 1650, 1800, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2430, cbuffer, 2025, 2235, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 3060, cbuffer, 2550, 2830, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 3900, cbuffer, 3250, 3610, cfactors, 6, 0, 7);

            t4cfunc::ket_transform<1, 3>(skbuffer, 0, ckbuffer, 0, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 24906, ckbuffer, 300, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 25116, ckbuffer, 600, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 25431, ckbuffer, 1050, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 27447, ckbuffer, 1680, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 27657, ckbuffer, 1980, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 27972, ckbuffer, 2430, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 28413, ckbuffer, 3060, 0, 6);

            t4cfunc::ket_transform<1, 3>(skbuffer, 29001, ckbuffer, 3900, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 25872, 24906, 25116, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 26502, 25116, 25431, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 29757, 27447, 27657, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 30387, 27657, 27972, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 31332, 27972, 28413, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pixx(skbuffer, 32655, 28413, 29001, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 34419, 29757, 30387, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 35679, 30387, 31332, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dhxx(skbuffer, 37569, 31332, 32655, r_ab, 1, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 6006, 0, 25872, 26502, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 210, 24906, 34419, 3, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 1470, 25116, 35679, 4, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 3360, 25431, 37569, 5, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 7896, 25872, 210, 1470, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pgxx(skbuffer, 11676, 26502, 1470, 3360, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dfxx(skbuffer, 17346, 6006, 7896, 11676, r_ab, 1, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 17346, 1, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 735, skbuffer, 18606, 1, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1470, skbuffer, 19866, 1, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 2205, skbuffer, 21126, 1, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 2940, skbuffer, 22386, 1, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 3675, skbuffer, 23646, 1, 3);
...
#endif /* ElectronRepulsionGeom2000RecDFPF_hpp */
