#ifndef ElectronRepulsionGeom2000RecFDSF_hpp
#define ElectronRepulsionGeom2000RecFDSF_hpp
...
/// @brief Computes d^(2)/dA^(2)(FD|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_fdsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(1840, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(20335, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 160, pbuffer, 820, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 220, pbuffer, 1530, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 320, pbuffer, 2405, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 470, pbuffer, 3320, 210, ket_width, ket_npgtos);
                ...
                pbuffer.scale(4.0 * a_exp * a_exp, {4118, 4398});

                pbuffer.scale(4.0 * a_exp * a_exp, {4678, 5038});
                ...
                t2cfunc::reduce(cbuffer, 1200, pbuffer, 4118, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1480, pbuffer, 4678, 360, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 3>(skbuffer, 0, cbuffer, 0, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 294, cbuffer, 60, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 14644, cbuffer, 160, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 14686, cbuffer, 220, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 14756, cbuffer, 320, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 14861, cbuffer, 470, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 15659, cbuffer, 680, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 15701, cbuffer, 740, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 15771, cbuffer, 840, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 15876, cbuffer, 990, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 16023, cbuffer, 1200, 0, 6);

            t4cfunc::ket_transform<0, 3>(skbuffer, 16219, cbuffer, 1480, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 2296, 0, 294, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 15008, 14644, 14686, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 15134, 14686, 14756, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 15344, 14756, 14861, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 16471, 15659, 15701, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 16597, 15701, 15771, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 16807, 15771, 15876, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 17122, 15876, 16023, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pixx(skbuffer, 17563, 16023, 16219, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 18151, 16471, 16597, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 18403, 16597, 16807, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 18823, 16807, 17122, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dhxx(skbuffer, 19453, 17122, 17563, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 2422, 0, 15008, 15134, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 3556, 294, 15134, 15344, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ddxx(skbuffer, 7336, 2296, 2422, 3556, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 42, 14644, 18151, 2, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 364, 14686, 18403, 3, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 784, 14756, 18823, 4, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 1414, 14861, 19453, 5, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 2800, 15008, 42, 364, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 4186, 15134, 364, 784, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pgxx(skbuffer, 5446, 15344, 784, 1414, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ddxx(skbuffer, 8092, 2422, 2800, 4186, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dfxx(skbuffer, 9604, 3556, 4186, 5446, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_fdxx(skbuffer, 12124, 7336, 8092, 9604, r_ab, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 0, skbuffer, 12124, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 245, skbuffer, 12544, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 490, skbuffer, 12964, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 735, skbuffer, 13384, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 980, skbuffer, 13804, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1225, skbuffer, 14224, 0, 3);
...
#endif /* ElectronRepulsionGeom2000RecFDSF_hpp */
