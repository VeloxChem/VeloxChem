#ifndef ElectronRepulsionGeom1000RecPDSP_hpp
#define ElectronRepulsionGeom1000RecPDSP_hpp
...
/// @brief Computes (PD|1/|r-r'||SP)  integral derivatives for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_geom_1000_pdsp(T&                               distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 36, pbuffer, 142, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 66, pbuffer, 202, 45, ket_width, ket_npgtos);
            }
            ...
            t4cfunc::ket_transform<0, 1>(skbuffer, 66, cbuffer, 66, 0, 4);
            
            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 111, 18, 36, r_ab, 0, 1);
            
            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 165, 36, 66, r_ab, 0, 1);
            
            erirec::comp_bra_geom_hrr_electron_repulsion_pdxx(skbuffer, 255, 111, 165, 0, r_ab, 0, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 0, skbuffer, 255, 0, 1);
            
            t4cfunc::bra_transform<1, 2>(sbuffer, 45, skbuffer, 309, 0, 1);
            
            t4cfunc::bra_transform<1, 2>(sbuffer, 90, skbuffer, 363, 0, 1);
...
#endif /* ElectronRepulsionGeom1000RecPDSP_hpp */
