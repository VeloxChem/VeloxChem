#ifndef ElectronRepulsionGeom1000RecFSPP_hpp
#define ElectronRepulsionGeom1000RecFSPP_hpp
...
/// @brief Computes d^(1)/dA^(1)(FS|1/|r-r'||PP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1000_fspp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(405, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 9, pbuffer, 64, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 18, pbuffer, 100, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 36, pbuffer, 184, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 54, pbuffer, 238, 36, ket_width, ket_npgtos);
                ...
                pbuffer.scale(2.0 * a_exp, {356, 386});

                pbuffer.scale(2.0 * a_exp, {416, 476});

                pbuffer.scale(2.0 * a_exp, {536, 581});

                pbuffer.scale(2.0 * a_exp, {581, 671});

                t2cfunc::reduce(cbuffer, 90, pbuffer, 7, 3, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 93, pbuffer, 25, 6, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 99, pbuffer, 64, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 108, pbuffer, 100, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 126, pbuffer, 184, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 144, pbuffer, 238, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 180, pbuffer, 356, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 210, pbuffer, 416, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 270, pbuffer, 536, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 315, pbuffer, 581, 90, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 36, cbuffer, 36, 54, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 90, cbuffer, 90, 93, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 99, cbuffer, 99, 108, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 126, cbuffer, 126, 144, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 180, cbuffer, 180, 210, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 270, cbuffer, 270, 315, cfactors, 6, 0, 4);

            t4cfunc::ket_transform<1, 1>(skbuffer, 0, ckbuffer, 0, 0, 0);

            t4cfunc::ket_transform<1, 1>(skbuffer, 9, ckbuffer, 9, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 36, ckbuffer, 36, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 252, ckbuffer, 90, 0, 0);

            t4cfunc::ket_transform<1, 1>(skbuffer, 261, ckbuffer, 99, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 288, ckbuffer, 126, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 342, ckbuffer, 180, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 432, ckbuffer, 270, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 90, 0, 9, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 117, 9, 36, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_dsxx(skbuffer, 198, 90, 117, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 567, 252, 261, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 594, 261, 288, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 675, 288, 342, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 837, 342, 432, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_psxx(skbuffer, 1107, 0, 567, 594, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 1188, 9, 594, 675, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 1431, 36, 675, 837, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dsxx(skbuffer, 1917, 90, 1107, 1188, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dpxx(skbuffer, 2079, 117, 1188, 1431, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_fsxx(skbuffer, 2565, 198, 1917, 2079, r_ab, 1, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 0, skbuffer, 2565, 1, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 63, skbuffer, 2655, 1, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 126, skbuffer, 2745, 1, 1);
...
#endif /* ElectronRepulsionGeom1000RecFSPP_hpp */
