#ifndef ElectronRepulsionGeom2000RecSPPP_hpp
#define ElectronRepulsionGeom2000RecSPPP_hpp
...
/// @brief Computes d^(2)/dA^(2)(SP|1/|r-r'||PP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_sppp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(765, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<1, 1>(skbuffer, 162, ckbuffer, 0, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 189, ckbuffer, 27, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 216, ckbuffer, 54, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 270, ckbuffer, 108, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 360, 189, 216, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 441, 216, 270, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 603, 360, 441, r_ab, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 162, 603, 1, 1, 1);
...
#endif /* ElectronRepulsionGeom2000RecSPPP_hpp */
