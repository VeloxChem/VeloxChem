#ifndef ElectronRepulsionGeom1000RecDDPP_hpp
#define ElectronRepulsionGeom1000RecDDPP_hpp
...
/// @brief Computes (DD|1/|r-r'||PP)  integral derivatives for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_geom_1000_ddpp(T&                               distributor,
                                       ...
                                       const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<1, 1>(skbuffer, 144, ckbuffer, 144, 0, 2);
            
            t4cfunc::ket_transform<1, 1>(skbuffer, 198, ckbuffer, 198, 0, 3);
            
            t4cfunc::ket_transform<1, 1>(skbuffer, 288, ckbuffer, 288, 0, 4);

            t4cfunc::ket_transform<1, 1>(skbuffer, 423, ckbuffer, 423, 0, 5);
            
            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 612, 0, 54, r_ab, 1, 1);
            
            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 774, 144, 198, r_ab, 1, 1);
          
            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 936, 198, 288, r_ab, 1, 1);
            
            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 1206, 288, 423, r_ab, 1, 1);
            
            erirec::comp_bra_geom_hrr_electron_repulsion_pdxx(skbuffer, 1611, 774, 936, 0, r_ab, 1, 1);
            
            erirec::comp_bra_geom_hrr_electron_repulsion_pfxx(skbuffer, 2097, 936, 1206, 54, r_ab, 1, 1);
            
            erirec::comp_bra_geom_hrr_electron_repulsion_ddxx(skbuffer, 2907, 1611, 2097, 612, r_ab, 1, 1);

            t4cfunc::bra_transform<2, 2>(sbuffer, 0, skbuffer, 2907, 1, 1);
            
            t4cfunc::bra_transform<2, 2>(sbuffer, 225, skbuffer, 3231, 1, 1);
            
            t4cfunc::bra_transform<2, 2>(sbuffer, 450, skbuffer, 3555, 1, 1);
...
#endif /* ElectronRepulsionGeom1000RecDDPP_hpp */
