...
/// @brief Computes d^(1)/dA^(1)(DD|1/|r-r'||PP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1000_ddpp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<1, 1>(skbuffer, 306, ckbuffer, 144, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 360, ckbuffer, 198, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 450, ckbuffer, 288, 0, 4);

            t4cfunc::ket_transform<1, 1>(skbuffer, 585, ckbuffer, 423, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 144, 0, 54, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 774, 306, 360, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 936, 360, 450, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 1206, 450, 585, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 1611, 0, 774, 936, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 2097, 54, 936, 1206, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ddxx(skbuffer, 2907, 144, 1611, 2097, r_ab, 1, 1);
