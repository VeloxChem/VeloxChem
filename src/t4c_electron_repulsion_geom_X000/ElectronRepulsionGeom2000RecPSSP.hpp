#ifndef ElectronRepulsionGeom2000RecPSSP_hpp
#define ElectronRepulsionGeom2000RecPSSP_hpp
...
/// @brief Computes d^(2)/dA^(2)(PS|1/|r-r'||SP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_pssp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(369, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(4.0 * a_exp * a_exp, {56, 74});

                pbuffer.scale(4.0 * a_exp * a_exp, {92, 122});

                t2cfunc::reduce(cbuffer, 12, pbuffer, 5, 3, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 15, pbuffer, 23, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 24, pbuffer, 56, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 42, pbuffer, 92, 30, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 1>(skbuffer, 126, cbuffer, 0, 0, 0);

            t4cfunc::ket_transform<0, 1>(skbuffer, 129, cbuffer, 3, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 147, cbuffer, 12, 0, 0);

            t4cfunc::ket_transform<0, 1>(skbuffer, 150, cbuffer, 15, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 159, cbuffer, 24, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 177, cbuffer, 42, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 138, 126, 129, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 207, 147, 150, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 216, 150, 159, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 243, 159, 177, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dsxx(skbuffer, 297, 207, 216, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 315, 216, 243, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 126, 297, 0, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 18, 129, 315, 1, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_psxx(skbuffer, 72, 138, 0, 18, r_ab, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 0, skbuffer, 72, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 9, skbuffer, 81, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 18, skbuffer, 90, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 27, skbuffer, 99, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 36, skbuffer, 108, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 45, skbuffer, 117, 0, 1);
...
#endif /* ElectronRepulsionGeom2000RecPSSP_hpp */
