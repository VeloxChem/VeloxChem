#ifndef ElectronRepulsionGeom2000RecPDSP_hpp
#define ElectronRepulsionGeom2000RecPDSP_hpp
...
/// @brief Computes d^(2)/dA^(2)(PD|1/|r-r'||SP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_pdsp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(1437, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 100, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 18, pbuffer, 192, 30, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 96, pbuffer, 297, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 141, pbuffer, 387, 63, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 1>(skbuffer, 612, cbuffer, 0, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 630, cbuffer, 18, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 714, cbuffer, 48, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 732, cbuffer, 66, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 762, cbuffer, 96, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 807, cbuffer, 141, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 660, 612, 630, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 870, 714, 732, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 924, 732, 762, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 1014, 762, 807, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 1149, 870, 924, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 1257, 924, 1014, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 612, 1149, 2, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 108, 630, 1257, 3, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 288, 660, 0, 108, r_ab, 0, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 0, skbuffer, 288, 0, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 45, skbuffer, 342, 0, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 90, skbuffer, 396, 0, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 135, skbuffer, 450, 0, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 180, skbuffer, 504, 0, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 225, skbuffer, 558, 0, 1);
...
#endif /* ElectronRepulsionGeom2000RecPDSP_hpp */
