#ifndef ElectronRepulsionGeom2000RecSFPD_hpp
#define ElectronRepulsionGeom2000RecSFPD_hpp
...
/// @brief Computes d^(2)/dA^(2)(SF|1/|r-r'||PD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_sfpd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(3765, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<1, 2>(skbuffer, 900, ckbuffer, 0, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 1050, ckbuffer, 180, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 1200, ckbuffer, 360, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 1425, ckbuffer, 630, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 1740, 1050, 1200, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 2190, 1200, 1425, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 2865, 1740, 2190, r_ab, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 900, 2865, 3, 1, 2);
...
#endif /* ElectronRepulsionGeom2000RecSFPD_hpp */
