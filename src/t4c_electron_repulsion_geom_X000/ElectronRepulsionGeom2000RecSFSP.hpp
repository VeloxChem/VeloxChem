#ifndef ElectronRepulsionGeom2000RecSFSP_hpp
#define ElectronRepulsionGeom2000RecSFSP_hpp
...
/// @brief Computes d^(2)/dA^(2)(SF|1/|r-r'||SP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_sfsp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(753, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<0, 1>(skbuffer, 180, cbuffer, 0, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 210, cbuffer, 30, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 240, cbuffer, 60, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 285, cbuffer, 105, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 348, 210, 240, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 438, 240, 285, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 573, 348, 438, r_ab, 0, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 180, 573, 3, 0, 1);
...
#endif /* ElectronRepulsionGeom2000RecSFSP_hpp */
