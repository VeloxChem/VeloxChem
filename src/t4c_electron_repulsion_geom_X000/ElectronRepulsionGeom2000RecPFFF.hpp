#ifndef ElectronRepulsionGeom2000RecPFFF_hpp
#define ElectronRepulsionGeom2000RecPFFF_hpp
...
/// @brief Computes d^(2)/dA^(2)(PF|1/|r-r'||FF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_pfff(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(38412, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(36603, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 4861, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 100, pbuffer, 5261, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 250, pbuffer, 5861, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 460, pbuffer, 6701, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 740, pbuffer, 8046, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 890, pbuffer, 8496, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1115, pbuffer, 9171, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1430, pbuffer, 10116, 420, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 1850, pbuffer, 4861, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1950, pbuffer, 5261, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2100, pbuffer, 5861, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2310, pbuffer, 6701, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2590, pbuffer, 8046, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2740, pbuffer, 8496, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2965, pbuffer, 9171, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3280, pbuffer, 10116, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3700, pbuffer, 11502, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3910, pbuffer, 11922, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4225, pbuffer, 12552, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4666, pbuffer, 13434, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5254, pbuffer, 14610, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5534, pbuffer, 14890, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5954, pbuffer, 15310, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 6542, pbuffer, 15898, 784, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<3, 3>(skbuffer, 16170, ckbuffer, 2880, 0, 3);

            t4cfunc::ket_transform<3, 3>(skbuffer, 16660, ckbuffer, 8200, 0, 4);

            t4cfunc::ket_transform<3, 3>(skbuffer, 18865, ckbuffer, 12580, 0, 3);

            t4cfunc::ket_transform<3, 3>(skbuffer, 19355, ckbuffer, 17900, 0, 4);

            t4cfunc::ket_transform<3, 3>(skbuffer, 20090, ckbuffer, 25448, 0, 5);

            t4cfunc::ket_transform<3, 3>(skbuffer, 21119, ckbuffer, 35612, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 17395, 16170, 16660, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 22491, 18865, 19355, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 23961, 19355, 20090, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 26166, 20090, 21119, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 29253, 22491, 23961, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 32193, 23961, 26166, r_ab, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 16170, 29253, 3, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 2940, 16660, 32193, 4, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 7350, 17395, 0, 2940, r_ab, 3, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 7350, 3, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1029, skbuffer, 8820, 3, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 2058, skbuffer, 10290, 3, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 3087, skbuffer, 11760, 3, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 4116, skbuffer, 13230, 3, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 5145, skbuffer, 14700, 3, 3);
...
#endif /* ElectronRepulsionGeom2000RecPFFF_hpp */
