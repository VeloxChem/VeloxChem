#ifndef ElectronRepulsionGeom2000RecPSPF_hpp
#define ElectronRepulsionGeom2000RecPSPF_hpp
...
/// @brief Computes d^(2)/dA^(2)(PS|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_pspf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(2583, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<1, 3>(skbuffer, 882, ckbuffer, 0, 0, 0);

            t4cfunc::ket_transform<1, 3>(skbuffer, 903, ckbuffer, 30, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1029, ckbuffer, 120, 0, 0);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1050, ckbuffer, 150, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1113, ckbuffer, 240, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1239, ckbuffer, 420, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 966, 882, 903, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 1449, 1029, 1050, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 1512, 1050, 1113, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 1701, 1113, 1239, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dsxx(skbuffer, 2079, 1449, 1512, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 2205, 1512, 1701, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 882, 2079, 0, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 126, 903, 2205, 1, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_psxx(skbuffer, 504, 966, 0, 126, r_ab, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 0, skbuffer, 504, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 63, skbuffer, 567, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 126, skbuffer, 630, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 189, skbuffer, 693, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 252, skbuffer, 756, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 315, skbuffer, 819, 1, 3);
...
#endif /* ElectronRepulsionGeom2000RecPSPF_hpp */
