#ifndef ElectronRepulsionGeom2000RecDDSF_hpp
#define ElectronRepulsionGeom2000RecDDSF_hpp
...
/// @brief Computes d^(2)/dA^(2)(DD|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_ddsf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 60, pbuffer, 680, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 120, pbuffer, 1230, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 220, pbuffer, 1855, 150, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 680, pbuffer, 2431, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 890, pbuffer, 2851, 280, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 3>(skbuffer, 0, cbuffer, 0, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 5250, cbuffer, 60, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 5292, cbuffer, 120, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 5362, cbuffer, 220, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 5803, cbuffer, 370, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 5845, cbuffer, 430, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 5915, cbuffer, 530, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 6020, cbuffer, 680, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 6167, cbuffer, 890, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 5467, 5250, 5292, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 5593, 5292, 5362, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 6363, 5803, 5845, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 6489, 5845, 5915, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 6699, 5915, 6020, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 7014, 6020, 6167, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 7455, 6363, 6489, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 7707, 6489, 6699, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 8127, 6699, 7014, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 1344, 0, 5467, 5593, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 42, 5250, 7455, 2, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 294, 5292, 7707, 3, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 714, 5362, 8127, 4, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 1722, 5467, 42, 294, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 2478, 5593, 294, 714, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ddxx(skbuffer, 3738, 1344, 1722, 2478, r_ab, 0, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 0, skbuffer, 3738, 0, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 175, skbuffer, 3990, 0, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 350, skbuffer, 4242, 0, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 525, skbuffer, 4494, 0, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 700, skbuffer, 4746, 0, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 875, skbuffer, 4998, 0, 3);
...
#endif /* ElectronRepulsionGeom2000RecDDSF_hpp */
