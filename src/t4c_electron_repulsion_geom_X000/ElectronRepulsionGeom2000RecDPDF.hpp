#ifndef ElectronRepulsionGeom2000RecDPDF_hpp
#define ElectronRepulsionGeom2000RecDPDF_hpp
...
/// @brief Computes d^(2)/dA^(2)(DP|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_dpdf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(3542, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(10395, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(25585, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 30, pbuffer, 661, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 75, pbuffer, 886, 63, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 138, pbuffer, 511, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 168, pbuffer, 661, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 213, pbuffer, 886, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 276, pbuffer, 1351, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 336, pbuffer, 1591, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 426, pbuffer, 1951, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 552, pbuffer, 2605, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 652, pbuffer, 2905, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 802, pbuffer, 3355, 210, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 1012, pbuffer, 511, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1042, pbuffer, 661, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1087, pbuffer, 886, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1150, pbuffer, 1351, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1210, pbuffer, 1591, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1300, pbuffer, 1951, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1426, pbuffer, 2605, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1526, pbuffer, 2905, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1676, pbuffer, 3355, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1886, pbuffer, 4075, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2036, pbuffer, 4375, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2261, pbuffer, 4825, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2576, pbuffer, 5455, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2786, pbuffer, 5665, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3101, pbuffer, 5980, 441, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2970, cbuffer, 1012, 1042, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 3060, cbuffer, 1042, 1087, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 3195, 2970, 3060, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 3375, cbuffer, 1150, 1210, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 3555, cbuffer, 1210, 1300, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 3825, 3375, 3555, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 4185, cbuffer, 1426, 1526, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 4485, cbuffer, 1526, 1676, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 4935, 4185, 4485, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 5535, cbuffer, 1886, 2036, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 5985, cbuffer, 2036, 2261, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 6660, 5535, 5985, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 7560, cbuffer, 2576, 2786, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 8190, cbuffer, 2786, 3101, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 9135, 7560, 8190, cfactors, 6, 0, 5);

            t4cfunc::ket_transform<2, 3>(skbuffer, 0, ckbuffer, 225, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 14490, ckbuffer, 630, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 14595, ckbuffer, 1260, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 14805, ckbuffer, 2370, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 16100, ckbuffer, 3195, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 16205, ckbuffer, 3825, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 16415, ckbuffer, 4935, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 16765, ckbuffer, 6660, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 17290, ckbuffer, 9135, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 15155, 14490, 14595, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 15470, 14595, 14805, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 18025, 16100, 16205, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 18340, 16205, 16415, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 18970, 16415, 16765, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 20020, 16765, 17290, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 21595, 18025, 18340, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 22225, 18340, 18970, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 23485, 18970, 20020, r_ab, 2, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 4095, 0, 15155, 15470, r_ab, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 105, 14490, 21595, 1, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 735, 14595, 22225, 2, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 1995, 14805, 23485, 3, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ppxx(skbuffer, 5040, 15155, 105, 735, r_ab, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 6930, 15470, 735, 1995, r_ab, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dpxx(skbuffer, 10710, 4095, 5040, 6930, r_ab, 2, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 0, skbuffer, 10710, 2, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 525, skbuffer, 11340, 2, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 1050, skbuffer, 11970, 2, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 1575, skbuffer, 12600, 2, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 2100, skbuffer, 13230, 2, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 2625, skbuffer, 13860, 2, 3);
...
#endif /* ElectronRepulsionGeom2000RecDPDF_hpp */
