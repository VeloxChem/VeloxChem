#ifndef ElectronRepulsionGeom2000RecPFSS_hpp
#define ElectronRepulsionGeom2000RecPFSS_hpp
...
/// @brief Computes d^(2)/dA^(2)(PF|1/|r-r'||SS)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_pfss(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(747, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<0, 0>(skbuffer, 330, cbuffer, 0, 0, 3);

            t4cfunc::ket_transform<0, 0>(skbuffer, 340, cbuffer, 10, 0, 4);

            t4cfunc::ket_transform<0, 0>(skbuffer, 385, cbuffer, 25, 0, 3);

            t4cfunc::ket_transform<0, 0>(skbuffer, 395, cbuffer, 35, 0, 4);

            t4cfunc::ket_transform<0, 0>(skbuffer, 410, cbuffer, 50, 0, 5);

            t4cfunc::ket_transform<0, 0>(skbuffer, 431, cbuffer, 71, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 355, 330, 340, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 459, 385, 395, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 489, 395, 410, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 534, 410, 431, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 597, 459, 489, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 657, 489, 534, r_ab, 0, 0);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 330, 597, 3, 0, 0);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 60, 340, 657, 4, 0, 0);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 150, 355, 0, 60, r_ab, 0, 0);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 150, 0, 0);

            t4cfunc::bra_transform<1, 3>(sbuffer, 21, skbuffer, 180, 0, 0);

            t4cfunc::bra_transform<1, 3>(sbuffer, 42, skbuffer, 210, 0, 0);

            t4cfunc::bra_transform<1, 3>(sbuffer, 63, skbuffer, 240, 0, 0);

            t4cfunc::bra_transform<1, 3>(sbuffer, 84, skbuffer, 270, 0, 0);

            t4cfunc::bra_transform<1, 3>(sbuffer, 105, skbuffer, 300, 0, 0);
...
#endif /* ElectronRepulsionGeom2000RecPFSS_hpp */
