#ifndef ElectronRepulsionGeom1000RecPPPF_hpp
#define ElectronRepulsionGeom1000RecPPPF_hpp
...
/// @brief Computes (PP|1/|r-r'||PF)  integral derivatives for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_geom_1000_pppf(T&                               distributor,
                                       ...
                                       const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<1, 3>(skbuffer, 63, ckbuffer, 90, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 126, ckbuffer, 180, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 252, ckbuffer, 360, 0, 3);
            
            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 462, 63, 126, r_ab, 1, 3);
            
            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 651, 126, 252, r_ab, 1, 3);
            
            erirec::comp_bra_geom_hrr_electron_repulsion_ppxx(skbuffer, 1029, 462, 651, 0, r_ab, 1, 3);
            
            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 1029, 1, 3);
            
            t4cfunc::bra_transform<1, 1>(sbuffer, 189, skbuffer, 1218, 1, 3);
            
            t4cfunc::bra_transform<1, 1>(sbuffer, 378, skbuffer, 1407, 1, 3);
...
#endif /* ElectronRepulsionGeom1000RecPPPF_hpp */
