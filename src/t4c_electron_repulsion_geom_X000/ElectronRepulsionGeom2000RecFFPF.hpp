#ifndef ElectronRepulsionGeom2000RecFFPF_hpp
#define ElectronRepulsionGeom2000RecFFPF_hpp
...
/// @brief Computes d^(2)/dA^(2)(FF|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_ffpf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(6350, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(7620, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(91833, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 2975, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 100, pbuffer, 3575, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 250, pbuffer, 5000, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 400, pbuffer, 5750, 225, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {2975, 3075});

                pbuffer.scale(2.0 * a_exp, {3575, 3725});

                pbuffer.scale(2.0 * a_exp, {5000, 5150});

                pbuffer.scale(2.0 * a_exp, {5750, 5975});

                pbuffer.scale(2.0 * a_exp, {7400, 7610});

                pbuffer.scale(2.0 * a_exp, {8240, 8555});

                pbuffer.scale(2.0 * a_exp, {9920, 10200});

                pbuffer.scale(2.0 * a_exp, {10760, 11180});

                t2cfunc::reduce(cbuffer, 625, pbuffer, 2975, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 725, pbuffer, 3575, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 875, pbuffer, 5000, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1025, pbuffer, 5750, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1250, pbuffer, 7400, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1460, pbuffer, 8240, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1775, pbuffer, 9920, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2055, pbuffer, 10760, 420, ket_width, ket_npgtos);
                ...
                pbuffer.scale(4.0 * a_exp * a_exp, {12236, 12596});

                pbuffer.scale(4.0 * a_exp * a_exp, {12956, 13496});

                pbuffer.scale(4.0 * a_exp * a_exp, {14036, 14486});

                pbuffer.scale(4.0 * a_exp * a_exp, {14486, 15161});

                t2cfunc::reduce(cbuffer, 2475, pbuffer, 2975, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2575, pbuffer, 3575, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2725, pbuffer, 5000, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2875, pbuffer, 5750, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3100, pbuffer, 7400, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3310, pbuffer, 8240, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3625, pbuffer, 9920, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3905, pbuffer, 10760, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4325, pbuffer, 12236, 360, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4685, pbuffer, 12956, 540, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5225, pbuffer, 14036, 450, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5675, pbuffer, 14486, 675, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 750, cbuffer, 625, 725, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1050, cbuffer, 875, 1025, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1500, cbuffer, 1250, 1460, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2130, cbuffer, 1775, 2055, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2970, cbuffer, 2475, 2575, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 3270, cbuffer, 2725, 2875, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 3720, cbuffer, 3100, 3310, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 4350, cbuffer, 3625, 3905, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 5190, cbuffer, 4325, 4685, cfactors, 6, 0, 7);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 6270, cbuffer, 5225, 5675, cfactors, 6, 0, 8);

            t4cfunc::ket_transform<1, 3>(skbuffer, 0, ckbuffer, 0, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1470, ckbuffer, 300, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 67872, ckbuffer, 750, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 68082, ckbuffer, 1050, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 68397, ckbuffer, 1500, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 68838, ckbuffer, 2130, 0, 6);

            t4cfunc::ket_transform<1, 3>(skbuffer, 72324, ckbuffer, 2970, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 72534, ckbuffer, 3270, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 72849, ckbuffer, 3720, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 73290, ckbuffer, 4350, 0, 6);

            t4cfunc::ket_transform<1, 3>(skbuffer, 73878, ckbuffer, 5190, 0, 7);

            t4cfunc::ket_transform<1, 3>(skbuffer, 74634, ckbuffer, 6270, 0, 8);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 9849, 0, 1470, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 69426, 67872, 68082, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 70056, 68082, 68397, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 71001, 68397, 68838, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 75579, 72324, 72534, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 76209, 72534, 72849, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 77154, 72849, 73290, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pixx(skbuffer, 78477, 73290, 73878, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pkxx(skbuffer, 80241, 73878, 74634, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 82509, 75579, 76209, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 83769, 76209, 77154, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dhxx(skbuffer, 85659, 77154, 78477, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dixx(skbuffer, 88305, 78477, 80241, r_ab, 1, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 10479, 0, 69426, 70056, r_ab, 1, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pgxx(skbuffer, 16149, 1470, 70056, 71001, r_ab, 1, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dfxx(skbuffer, 32592, 9849, 10479, 16149, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 210, 67872, 82509, 3, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 1785, 68082, 83769, 4, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 3675, 68397, 85659, 5, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 6321, 68838, 88305, 6, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 12369, 69426, 210, 1785, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pgxx(skbuffer, 18984, 70056, 1785, 3675, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_phxx(skbuffer, 24654, 71001, 3675, 6321, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dfxx(skbuffer, 36372, 10479, 12369, 18984, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dgxx(skbuffer, 43932, 16149, 18984, 24654, r_ab, 1, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ffxx(skbuffer, 55272, 32592, 36372, 43932, r_ab, 1, 3);

            t4cfunc::bra_transform<3, 3>(sbuffer, 0, skbuffer, 55272, 1, 3);

            t4cfunc::bra_transform<3, 3>(sbuffer, 1029, skbuffer, 57372, 1, 3);

            t4cfunc::bra_transform<3, 3>(sbuffer, 2058, skbuffer, 59472, 1, 3);

            t4cfunc::bra_transform<3, 3>(sbuffer, 3087, skbuffer, 61572, 1, 3);

            t4cfunc::bra_transform<3, 3>(sbuffer, 4116, skbuffer, 63672, 1, 3);

            t4cfunc::bra_transform<3, 3>(sbuffer, 5145, skbuffer, 65772, 1, 3);
...
#endif /* ElectronRepulsionGeom2000RecFFPF_hpp */
