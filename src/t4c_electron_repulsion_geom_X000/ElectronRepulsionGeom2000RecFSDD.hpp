#ifndef ElectronRepulsionGeom2000RecFSDD_hpp
#define ElectronRepulsionGeom2000RecFSDD_hpp
...
/// @brief Computes d^(2)/dA^(2)(FS|1/|r-r'||DD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_fsdd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(2480, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(6720, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(22400, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 155, pbuffer, 290, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 173, pbuffer, 380, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 203, pbuffer, 530, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 248, pbuffer, 821, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 284, pbuffer, 965, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 344, pbuffer, 1205, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 434, pbuffer, 1635, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 494, pbuffer, 1815, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 594, pbuffer, 2115, 150, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 744, pbuffer, 37, 6, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 750, pbuffer, 85, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 760, pbuffer, 155, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 775, pbuffer, 290, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 793, pbuffer, 380, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 823, pbuffer, 530, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 868, pbuffer, 821, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 904, pbuffer, 965, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 964, pbuffer, 1205, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1054, pbuffer, 1635, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1114, pbuffer, 1815, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1214, pbuffer, 2115, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1364, pbuffer, 2610, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1454, pbuffer, 2790, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1604, pbuffer, 3090, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1829, pbuffer, 3540, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1955, pbuffer, 3666, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2165, pbuffer, 3876, 315, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 336, cbuffer, 124, 130, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 354, cbuffer, 130, 140, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 384, 336, 354, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 420, cbuffer, 155, 173, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 474, cbuffer, 173, 203, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 564, 420, 474, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 672, cbuffer, 248, 284, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 780, cbuffer, 284, 344, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 960, 672, 780, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1176, cbuffer, 434, 494, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1356, cbuffer, 494, 594, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 1656, 1176, 1356, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 2016, cbuffer, 744, 750, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2034, cbuffer, 750, 760, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 2064, 2016, 2034, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 2100, cbuffer, 775, 793, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2154, cbuffer, 793, 823, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 2244, 2100, 2154, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 2352, cbuffer, 868, 904, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2460, cbuffer, 904, 964, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 2640, 2352, 2460, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 2856, cbuffer, 1054, 1114, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 3036, cbuffer, 1114, 1214, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 3336, 2856, 3036, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 3696, cbuffer, 1364, 1454, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 3966, cbuffer, 1454, 1604, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 4416, 3696, 3966, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 4956, cbuffer, 1829, 1955, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 5334, cbuffer, 1955, 2165, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 5964, 4956, 5334, cfactors, 6, 0, 5);

            t4cfunc::ket_transform<2, 2>(skbuffer, 0, ckbuffer, 48, 0, 0);

            t4cfunc::ket_transform<2, 2>(skbuffer, 175, ckbuffer, 228, 0, 1);

            t4cfunc::ket_transform<2, 2>(skbuffer, 14125, ckbuffer, 384, 0, 0);

            t4cfunc::ket_transform<2, 2>(skbuffer, 14150, ckbuffer, 564, 0, 1);

            t4cfunc::ket_transform<2, 2>(skbuffer, 14225, ckbuffer, 960, 0, 2);

            t4cfunc::ket_transform<2, 2>(skbuffer, 14375, ckbuffer, 1656, 0, 3);

            t4cfunc::ket_transform<2, 2>(skbuffer, 15375, ckbuffer, 2064, 0, 0);

            t4cfunc::ket_transform<2, 2>(skbuffer, 15400, ckbuffer, 2244, 0, 1);

            t4cfunc::ket_transform<2, 2>(skbuffer, 15475, ckbuffer, 2640, 0, 2);

            t4cfunc::ket_transform<2, 2>(skbuffer, 15625, ckbuffer, 3336, 0, 3);

            t4cfunc::ket_transform<2, 2>(skbuffer, 15875, ckbuffer, 4416, 0, 4);

            t4cfunc::ket_transform<2, 2>(skbuffer, 16250, ckbuffer, 5964, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 3100, 0, 175, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 14625, 14125, 14150, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 14700, 14150, 14225, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 14925, 14225, 14375, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 16775, 15375, 15400, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 16850, 15400, 15475, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 17075, 15475, 15625, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 17525, 15625, 15875, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 18275, 15875, 16250, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_dsxx(skbuffer, 19400, 16775, 16850, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 19550, 16850, 17075, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 20000, 17075, 17525, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 20900, 17525, 18275, r_ab, 2, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_psxx(skbuffer, 3175, 0, 14625, 14700, r_ab, 2, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 3850, 175, 14700, 14925, r_ab, 2, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dsxx(skbuffer, 8575, 3100, 3175, 3850, r_ab, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 25, 14125, 19400, 0, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 250, 14150, 19550, 1, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 700, 14225, 20000, 2, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 1600, 14375, 20900, 3, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_psxx(skbuffer, 3400, 14625, 25, 250, r_ab, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ppxx(skbuffer, 4525, 14700, 250, 700, r_ab, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 5875, 14925, 700, 1600, r_ab, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dsxx(skbuffer, 9025, 3175, 3400, 4525, r_ab, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dpxx(skbuffer, 9925, 3850, 4525, 5875, r_ab, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_fsxx(skbuffer, 12625, 8575, 9025, 9925, r_ab, 2, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 0, skbuffer, 12625, 2, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 175, skbuffer, 12875, 2, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 350, skbuffer, 13125, 2, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 525, skbuffer, 13375, 2, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 700, skbuffer, 13625, 2, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 875, skbuffer, 13875, 2, 2);
...
#endif /* ElectronRepulsionGeom2000RecFSDD_hpp */
