#ifndef ElectronRepulsionGeom2000RecDSSD_hpp
#define ElectronRepulsionGeom2000RecDSSD_hpp
...
/// @brief Computes d^(2)/dA^(2)(DS|1/|r-r'||SD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_dssd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(1775, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 12, pbuffer, 88, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 30, pbuffer, 202, 36, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 72, pbuffer, 88, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 90, pbuffer, 202, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 126, pbuffer, 340, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 186, pbuffer, 460, 90, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 2>(skbuffer, 0, cbuffer, 0, 0, 0);

            t4cfunc::ket_transform<0, 2>(skbuffer, 890, cbuffer, 6, 0, 0);

            t4cfunc::ket_transform<0, 2>(skbuffer, 895, cbuffer, 12, 0, 1);

            t4cfunc::ket_transform<0, 2>(skbuffer, 910, cbuffer, 30, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1000, cbuffer, 66, 0, 0);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1005, cbuffer, 72, 0, 1);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1020, cbuffer, 90, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1050, cbuffer, 126, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1100, cbuffer, 186, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 940, 890, 895, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 955, 895, 910, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 1175, 1000, 1005, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 1190, 1005, 1020, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 1235, 1020, 1050, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 1325, 1050, 1100, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_dsxx(skbuffer, 1475, 1175, 1190, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 1505, 1190, 1235, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 1595, 1235, 1325, r_ab, 0, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_psxx(skbuffer, 305, 0, 940, 955, r_ab, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 5, 890, 1475, 0, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 35, 895, 1505, 1, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 125, 910, 1595, 2, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_psxx(skbuffer, 350, 940, 5, 35, r_ab, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ppxx(skbuffer, 440, 955, 35, 125, r_ab, 0, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dsxx(skbuffer, 710, 305, 350, 440, r_ab, 0, 2);

            t4cfunc::bra_transform<2, 0>(sbuffer, 0, skbuffer, 710, 0, 2);

            t4cfunc::bra_transform<2, 0>(sbuffer, 25, skbuffer, 740, 0, 2);

            t4cfunc::bra_transform<2, 0>(sbuffer, 50, skbuffer, 770, 0, 2);

            t4cfunc::bra_transform<2, 0>(sbuffer, 75, skbuffer, 800, 0, 2);

            t4cfunc::bra_transform<2, 0>(sbuffer, 100, skbuffer, 830, 0, 2);

            t4cfunc::bra_transform<2, 0>(sbuffer, 125, skbuffer, 860, 0, 2);
...
#endif /* ElectronRepulsionGeom2000RecDSSD_hpp */
