#ifndef ElectronRepulsionGeom2000RecPPFF_hpp
#define ElectronRepulsionGeom2000RecPPFF_hpp
...
/// @brief Computes d^(2)/dA^(2)(PP|1/|r-r'||FF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_ppff(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(13279, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<3, 3>(skbuffer, 5292, ckbuffer, 864, 0, 1);

            t4cfunc::ket_transform<3, 3>(skbuffer, 5439, ckbuffer, 2892, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 6174, ckbuffer, 4356, 0, 1);

            t4cfunc::ket_transform<3, 3>(skbuffer, 6321, ckbuffer, 6384, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 6615, ckbuffer, 9864, 0, 3);

            t4cfunc::ket_transform<3, 3>(skbuffer, 7105, ckbuffer, 15184, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 5733, 5292, 5439, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 7840, 6174, 6321, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 8281, 6321, 6615, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 9163, 6615, 7105, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 10633, 7840, 8281, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 11515, 8281, 9163, r_ab, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 5292, 10633, 1, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 882, 5439, 11515, 2, 3, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ppxx(skbuffer, 2646, 5733, 0, 882, r_ab, 3, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 2646, 3, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 441, skbuffer, 3087, 3, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 882, skbuffer, 3528, 3, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 1323, skbuffer, 3969, 3, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 1764, skbuffer, 4410, 3, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 2205, skbuffer, 4851, 3, 3);
...
#endif /* ElectronRepulsionGeom2000RecPPFF_hpp */
