#ifndef ElectronRepulsionGeom2000RecPFDF_hpp
#define ElectronRepulsionGeom2000RecPFDF_hpp
...
/// @brief Computes d^(2)/dA^(2)(PF|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_pfdf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(4554, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(13365, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 1150, pbuffer, 3265, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1250, pbuffer, 3665, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1400, pbuffer, 4265, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1610, pbuffer, 5330, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1760, pbuffer, 5780, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1985, pbuffer, 6455, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2300, pbuffer, 7526, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2510, pbuffer, 7946, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2825, pbuffer, 8576, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3266, pbuffer, 9458, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3546, pbuffer, 9738, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3966, pbuffer, 10158, 588, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<2, 3>(skbuffer, 11550, ckbuffer, 750, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 11900, ckbuffer, 2475, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 13475, ckbuffer, 4125, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 13825, ckbuffer, 5850, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 14350, ckbuffer, 8325, 0, 5);

            t4cfunc::ket_transform<2, 3>(skbuffer, 15085, ckbuffer, 11685, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 12425, 11550, 11900, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 16065, 13475, 13825, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 17115, 13825, 14350, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 18690, 14350, 15085, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 20895, 16065, 17115, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 22995, 17115, 18690, r_ab, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 11550, 20895, 3, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 2100, 11900, 22995, 4, 2, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 5250, 12425, 0, 2100, r_ab, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 5250, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 735, skbuffer, 6300, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1470, skbuffer, 7350, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 2205, skbuffer, 8400, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 2940, skbuffer, 9450, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 3675, skbuffer, 10500, 2, 3);
...
#endif /* ElectronRepulsionGeom2000RecPFDF_hpp */
