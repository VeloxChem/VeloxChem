#ifndef ElectronRepulsionGeom2000RecPDPD_hpp
#define ElectronRepulsionGeom2000RecPDPD_hpp
...
/// @brief Computes d^(2)/dA^(2)(PD|1/|r-r'||PD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_pdpd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(7185, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<1, 2>(skbuffer, 3060, ckbuffer, 0, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 3150, ckbuffer, 108, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 3570, ckbuffer, 288, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 3660, ckbuffer, 396, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 3810, ckbuffer, 576, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 4035, ckbuffer, 846, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 3300, 3060, 3150, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 4350, 3570, 3660, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 4620, 3660, 3810, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 5070, 3810, 4035, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 5745, 4350, 4620, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 6285, 4620, 5070, r_ab, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 3060, 5745,  2, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 540, 3150, 6285, 3, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 1440, 3300, 0, 540, r_ab, 1, 2);

            t4cfunc::bra_transform<1, 2>(sbuffer, 0, skbuffer, 1440, 1, 2);

            t4cfunc::bra_transform<1, 2>(sbuffer, 225, skbuffer, 1710, 1, 2);

            t4cfunc::bra_transform<1, 2>(sbuffer, 450, skbuffer, 1980, 1, 2);

            t4cfunc::bra_transform<1, 2>(sbuffer, 675, skbuffer, 2250, 1, 2);

            t4cfunc::bra_transform<1, 2>(sbuffer, 900, skbuffer, 2520, 1, 2);

            t4cfunc::bra_transform<1, 2>(sbuffer, 1125, skbuffer, 2790, 1, 2);
...
#endif /* ElectronRepulsionGeom2000RecPDPD_hpp */
