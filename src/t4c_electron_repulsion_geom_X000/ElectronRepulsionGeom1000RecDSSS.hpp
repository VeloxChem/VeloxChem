#ifndef ElectronRepulsionGeom1000RecDSSS_hpp
#define ElectronRepulsionGeom1000RecDSSS_hpp
...
/// @brief Computes d^(1)/dA^(1)(DS|1/|r-r'||SS)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1000_dsss(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<0, 0>(skbuffer, 7, cbuffer, 4, 0, 0);

            t4cfunc::ket_transform<0, 0>(skbuffer, 8, cbuffer, 5, 0, 1);

            t4cfunc::ket_transform<0, 0>(skbuffer, 11, cbuffer, 8, 0, 2);

            t4cfunc::ket_transform<0, 0>(skbuffer, 17, cbuffer, 14, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 4, 0, 1, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 27, 7, 8, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 30, 8, 11, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 39, 11, 17, r_ab, 0, 0);

            erirec::comp_bra_geom10_hrr_electron_repulsion_psxx(skbuffer, 57, 0, 27, 30, r_ab, 0, 0);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 66, 1, 30, 39, r_ab, 0, 0);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dsxx(skbuffer, 93, 4, 57, 66, r_ab, 0, 0);

            t4cfunc::bra_transform<2, 0>(sbuffer, 0, skbuffer, 93, 0, 0);

            t4cfunc::bra_transform<2, 0>(sbuffer, 5, skbuffer, 99, 0, 0);

            t4cfunc::bra_transform<2, 0>(sbuffer, 10, skbuffer, 105, 0, 0);
...
#endif /* ElectronRepulsionGeom1000RecDSSS_hpp */
