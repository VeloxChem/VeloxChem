#ifndef ElectronRepulsionGeom2000RecFDPD_hpp
#define ElectronRepulsionGeom2000RecFDPD_hpp
...
/// @brief Computes d^(2)/dA^(2)(FD|1/|r-r'||PD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_fdpd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(3312, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(43575, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 256, pbuffer, 694, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 292, pbuffer, 910, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 352, pbuffer, 1420, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 412, pbuffer, 1720, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 512, pbuffer, 2385, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 602, pbuffer, 2745, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 752, pbuffer, 3492, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 878, pbuffer, 3870, 210, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 1920, pbuffer, 4584, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2088, pbuffer, 4920, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2368, pbuffer, 5480, 216, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2584, pbuffer, 5696, 360, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1224, cbuffer, 1088, 1124, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1332, cbuffer, 1184, 1244, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1512, cbuffer, 1344, 1434, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1782, cbuffer, 1584, 1710, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 2160, cbuffer, 1920, 2088, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 2664, cbuffer, 2368, 2584, cfactors, 6, 0, 7);

            t4cfunc::ket_transform<1, 2>(skbuffer, 0, ckbuffer, 0, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 630, ckbuffer, 108, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 31380, ckbuffer, 288, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 31470, ckbuffer, 396, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 31620, ckbuffer, 576, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 31845, ckbuffer, 846, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 33555, ckbuffer, 1224, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 33645, ckbuffer, 1332, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 33795, ckbuffer, 1512, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 34020, ckbuffer, 1782, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 34335, ckbuffer, 2160, 0, 6);

            t4cfunc::ket_transform<1, 2>(skbuffer, 34755, ckbuffer, 2664, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 4920, 0, 630, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 32160, 31380, 31470, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 32430, 31470, 31620, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 32880, 31620, 31845, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 35295, 33555, 33645, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 35565, 33645, 33795, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 36015, 33795, 34020, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 36690, 34020, 34335, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pixx(skbuffer, 37635, 34335, 34755, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 38895, 35295, 35565, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 39435, 35565, 36015, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 40335, 36015, 36690, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_dhxx(skbuffer, 41685, 36690, 37635, r_ab, 1, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 5190, 0, 32160, 32430, r_ab, 1, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 7620, 630, 32430, 32880, r_ab, 1, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ddxx(skbuffer, 15720, 4920, 5190, 7620, r_ab, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 90, 31380, 38895, 2, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 780, 31470, 39435, 3, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 1680, 31620, 40335, 4, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 3030, 31845, 41685, 5, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pdxx(skbuffer, 6000, 32160, 90, 780, r_ab, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pfxx(skbuffer, 8970, 32430, 780, 1680, r_ab, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_pgxx(skbuffer, 11670, 32880, 1680, 3030, r_ab, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ddxx(skbuffer, 17340, 5190, 6000, 8970, r_ab, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_dfxx(skbuffer, 20580, 7620, 8970, 11670, r_ab, 1, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_fdxx(skbuffer, 25980, 15720, 17340, 20580, r_ab, 1, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 0, skbuffer, 25980, 1, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 525, skbuffer, 26880, 1, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1050, skbuffer, 27780, 1, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1575, skbuffer, 28680, 1, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 2100, skbuffer, 29580, 1, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 2625, skbuffer, 30480, 1, 2);
...
#endif /* ElectronRepulsionGeom2000RecFDPD_hpp */
