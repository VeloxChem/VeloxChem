#ifndef ElectronRepulsionGeom2000RecSSSF_hpp
#define ElectronRepulsionGeom2000RecSSSF_hpp
...
/// @brief Computes d^(2)/dA^(2)(SS|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_sssf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(4.0 * a_exp * a_exp, {93, 123});

                pbuffer.scale(4.0 * a_exp * a_exp, {153, 213});
            ...
            t4cfunc::ket_transform<0, 3>(skbuffer, 42, cbuffer, 0, 0, 0);

            t4cfunc::ket_transform<0, 3>(skbuffer, 49, cbuffer, 10, 0, 0);

            t4cfunc::ket_transform<0, 3>(skbuffer, 56, cbuffer, 20, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 77, cbuffer, 50, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 119, 49, 56, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 140, 56, 77, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dsxx(skbuffer, 203, 119, 140, r_ab, 0, 3);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 42, 203, 0, 0, 3);
...
#endif /* ElectronRepulsionGeom2000RecSSSF_hpp */
