#ifndef ElectronRepulsionGeom2000RecSFPP_hpp
#define ElectronRepulsionGeom2000RecSFPP_hpp
...
/// @brief Computes d^(2)/dA^(2)(SF|1/|r-r'||PP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom2000_sfpp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(2259, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<1, 1>(skbuffer, 540, ckbuffer, 0, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 630, ckbuffer, 90, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 720, ckbuffer, 180, 0, 4);

            t4cfunc::ket_transform<1, 1>(skbuffer, 855, ckbuffer, 315, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 1044, 630, 720, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 1314, 720, 855, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 1719, 1044, 1314, r_ab, 1, 1);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 540, 1719, 3, 1, 1);
...
#endif /* ElectronRepulsionGeom2000RecSFPP_hpp */
