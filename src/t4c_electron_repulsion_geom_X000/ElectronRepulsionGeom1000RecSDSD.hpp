...
/// @brief Computes (SD|1/|r-r'||SD)  integral derivatives for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_geom_1000_sdsd(T&                               distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 138, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 36, pbuffer, 210, 60, ket_width, ket_npgtos);
            }
            ...
            t4cfunc::bra_transform<0, 2>(sbuffer, 0, skbuffer, 80, 0, 2);
            
            t4cfunc::bra_transform<0, 2>(sbuffer, 25, skbuffer, 110, 0, 2);
            
            t4cfunc::bra_transform<0, 2>(sbuffer, 50, skbuffer, 140, 0, 2);
