...
/// @brief Computes d^(2)/dA^(2)(PP|1/|r-r'||DD)  integral derivatives.
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(6775, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<2, 2>(skbuffer, 2700, ckbuffer, 144, 0, 1);

            t4cfunc::ket_transform<2, 2>(skbuffer, 2775, ckbuffer, 540, 0, 2);

            t4cfunc::ket_transform<2, 2>(skbuffer, 3150, ckbuffer, 900, 0, 1);

            t4cfunc::ket_transform<2, 2>(skbuffer, 3225, ckbuffer, 1296, 0, 2);

            t4cfunc::ket_transform<2, 2>(skbuffer, 3375, ckbuffer, 1992, 0, 3);

            t4cfunc::ket_transform<2, 2>(skbuffer, 3625, ckbuffer, 3072, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 2925, 2700, 2775, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 4000, 3150, 3225, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 4225, 3225, 3375, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 4675, 3375, 3625, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 5425, 4000, 4225, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 5875, 4225, 4675, r_ab, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 0, 2700, 5425, 1, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_sxxx(skbuffer, 450, 2775, 5875, 2, 2, 2);

            erirec::comp_bra_geom20_hrr_electron_repulsion_ppxx(skbuffer, 1350, 2925, 0, 450, r_ab, 2, 2);

            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 1350, 2, 2);

            t4cfunc::bra_transform<1, 1>(sbuffer, 225, skbuffer, 1575, 2, 2);

            t4cfunc::bra_transform<1, 1>(sbuffer, 450, skbuffer, 1800, 2, 2);

            t4cfunc::bra_transform<1, 1>(sbuffer, 675, skbuffer, 2025, 2, 2);

            t4cfunc::bra_transform<1, 1>(sbuffer, 900, skbuffer, 2250, 2, 2);

            t4cfunc::bra_transform<1, 1>(sbuffer, 1125, skbuffer, 2475, 2, 2);
