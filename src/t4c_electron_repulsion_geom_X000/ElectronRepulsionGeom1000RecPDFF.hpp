#ifndef ElectronRepulsionGeom1000RecPDFF_hpp
#define ElectronRepulsionGeom1000RecPDFF_hpp
...
/// @brief Computes d^(1)/dA^(1)(PD|1/|r-r'||FF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1000_pdff(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(6811, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<3, 3>(skbuffer, 1078, ckbuffer, 12856, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 1813, 294, 588, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 2695, 588, 1078, r_ab, 3, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 4165, 0, 1813, 2695, r_ab, 3, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 0, skbuffer, 4165, 3, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 735, skbuffer, 5047, 3, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 1470, skbuffer, 5929, 3, 3);
...
#endif /* ElectronRepulsionGeom1000RecPDFF_hpp */
