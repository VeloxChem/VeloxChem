#ifndef ThreeCenterOverlapGradientGeom100SumRecDD_hpp
#define ThreeCenterOverlapGradientGeom100SumRecDD_hpp
...
/// @brief Computes (d^(1)/dA^(1)D|GX(r)|D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_10_dd(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    g3ovlrec::comp_prim_overlap_gradient_pd(pbuffer, 41, 5, 14, 23, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ds(pbuffer, 95, 1, 11, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dp(pbuffer, 101, 2, 11, 14, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dd(pbuffer, 119, 5, 14, 23, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fp(pbuffer, 155, 14, 95, 101, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fd(pbuffer, 185, 23, 101, 119, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_fd(pbuffer, 245, 119, 155, 185, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_prim_op_geom_10_dx(pbuffer, 425, 41, 245, 3, 6, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 425, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGradientGeom100SumRecDD_hpp */
