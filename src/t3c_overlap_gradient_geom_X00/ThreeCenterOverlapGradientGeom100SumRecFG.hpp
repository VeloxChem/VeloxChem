#ifndef ThreeCenterOverlapGradientGeom100SumRecFG_hpp
#define ThreeCenterOverlapGradientGeom100SumRecFG_hpp
...
/// @brief Computes (d^(1)/dA^(1)F|GX(r)|G)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_10_fg(T& distributor,
                                     ...
                                     const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(3325, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    g3ovlrec::comp_prim_overlap_gradient_dg(pbuffer, 345, 96, 195, 255, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fd(pbuffer, 615, 48, 141, 159, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ff(pbuffer, 675, 66, 159, 195, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fg(pbuffer, 775, 96, 195, 255, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_gf(pbuffer, 925, 195, 615, 675, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_gg(pbuffer, 1075, 255, 675, 775, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_gg(pbuffer, 1300, 775, 925, 1075, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_prim_op_geom_10_fx(pbuffer, 1975, 345, 1300, 3, 15, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 1975, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGradientGeom100SumRecFG_hpp */
