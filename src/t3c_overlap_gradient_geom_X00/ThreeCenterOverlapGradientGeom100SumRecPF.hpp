#ifndef ThreeCenterOverlapGradientGeom100SumRecPF_hpp
#define ThreeCenterOverlapGradientGeom100SumRecPF_hpp
...
/// @brief Computes (d^(1)/dA^(1)P|GX(r)|F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_10_pf(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    g3ovlrec::comp_prim_overlap_gradient_sf(pbuffer, 21, 5, 11, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pp(pbuffer, 51, 1, 2, factors, 11, a_exp, exgtos[l]);
                    ...
                    t3ovlrec::comp_prim_overlap_dd(pbuffer, 108, 5, 51, 60, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_df(pbuffer, 144, 11, 60, 78, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_df(pbuffer, 204, 78, 108, 144, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_prim_op_geom_10_px(pbuffer, 384, 21, 204, 3, 10, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 384, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGradientGeom100SumRecPF_hpp */
