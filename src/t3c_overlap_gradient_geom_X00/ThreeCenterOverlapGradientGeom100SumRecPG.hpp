#ifndef ThreeCenterOverlapGradientGeom100SumRecPG_hpp
#define ThreeCenterOverlapGradientGeom100SumRecPG_hpp
...
/// @brief Computes (d^(1)/dA^(1)P|GX(r)|G)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_10_pg(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    g3ovlrec::comp_prim_overlap_gradient_sg(pbuffer, 36, 11, 21, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pd(pbuffer, 81, 2, 5, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pf(pbuffer, 99, 5, 11, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pg(pbuffer, 129, 11, 21, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_df(pbuffer, 174, 11, 81, 99, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dg(pbuffer, 234, 21, 99, 129, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_dg(pbuffer, 324, 129, 174, 234, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_prim_op_geom_10_px(pbuffer, 594, 36, 324, 3, 15, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 594, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGradientGeom100SumRecPG_hpp */
