#ifndef ThreeCenterOverlapGradientGeom100SumRecPD_hpp
#define ThreeCenterOverlapGradientGeom100SumRecPD_hpp
...
/// @brief Computes (d^(1)/dA^(1)P|GX(r)|D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_10_pd(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    g3ovlrec::comp_prim_overlap_gradient_sd(pbuffer, 11, 2, 5, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ps(pbuffer, 29, 1, factors, 11);

                    t3ovlrec::comp_prim_overlap_pp(pbuffer, 32, 1, 2, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pd(pbuffer, 41, 2, 5, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dp(pbuffer, 59, 2, 29, 32, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dd(pbuffer, 77, 5, 32, 41, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_dd(pbuffer, 113, 41, 59, 77, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_prim_op_geom_10_px(pbuffer, 221, 11, 113, 3, 6, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 221, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGradientGeom100SumRecPD_hpp */
