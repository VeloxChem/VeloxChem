#ifndef ThreeCenterOverlapGradientGeom100SumRecDF_hpp
#define ThreeCenterOverlapGradientGeom100SumRecDF_hpp
...
/// @brief Computes (d^(1)/dA^(1)D|GX(r)|F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_10_df(T& distributor,
                                     ...
                                     const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(1285, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    g3ovlrec::comp_prim_overlap_gradient_pf(pbuffer, 81, 11, 33, 51, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dp(pbuffer, 171, 2, 21, 24, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dd(pbuffer, 189, 5, 24, 33, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_df(pbuffer, 225, 11, 33, 51, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fd(pbuffer, 285, 33, 171, 189, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ff(pbuffer, 345, 51, 189, 225, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_ff(pbuffer, 445, 225, 285, 345, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_prim_op_geom_10_dx(pbuffer, 745, 81, 445, 3, 10, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 745, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGradientGeom100SumRecDF_hpp */
