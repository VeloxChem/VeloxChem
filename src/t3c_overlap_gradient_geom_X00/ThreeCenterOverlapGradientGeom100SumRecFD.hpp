#ifndef ThreeCenterOverlapGradientGeom100SumRecFD_hpp
#define ThreeCenterOverlapGradientGeom100SumRecFD_hpp
...
/// @brief Computes (d^(1)/dA^(1)F|GX(r)|D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_10_fd(T& distributor,
                                     ...
                                     const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(1254, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    g3ovlrec::comp_prim_overlap_gradient_dd(pbuffer, 101, 23, 47, 65, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fs(pbuffer, 209, 11, 41, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fp(pbuffer, 219, 14, 41, 47, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fd(pbuffer, 249, 23, 47, 65, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_gp(pbuffer, 309, 47, 209, 219, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_gd(pbuffer, 354, 65, 219, 249, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_gd(pbuffer, 444, 249, 309, 354, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_prim_op_geom_10_fx(pbuffer, 714, 101, 444, 3, 6, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 714, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGradientGeom100SumRecFD_hpp */
