#ifndef ThreeCenterOverlapGradientGeom100SumRecDG_hpp
#define ThreeCenterOverlapGradientGeom100SumRecDG_hpp
...
/// @brief Computes (d^(1)/dA^(1)D|GX(r)|G)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_10_dg(T& distributor,
                                     ...
                                     const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(1969, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    g3ovlrec::comp_prim_overlap_gradient_pg(pbuffer, 138, 21, 63, 93, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dd(pbuffer, 273, 5, 36, 45, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_df(pbuffer, 309, 11, 45, 63, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dg(pbuffer, 369, 21, 63, 93, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ff(pbuffer, 459, 63, 273, 309, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fg(pbuffer, 559, 93, 309, 369, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_fg(pbuffer, 709, 369, 459, 559, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_prim_op_geom_10_dx(pbuffer, 1159, 138, 709, 3, 15, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 1159, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGradientGeom100SumRecDG_hpp */
