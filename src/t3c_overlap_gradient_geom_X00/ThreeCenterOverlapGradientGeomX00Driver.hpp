#ifndef ThreeCenterOverlapGradientGeomX00Driver_hpp
#define ThreeCenterOverlapGradientGeomX00Driver_hpp
...
#include "ThreeCenterOverlapGradientGeom100Func.hpp"
...
template <int N>
class CThreeCenterOverlapGradientGeomX00Driver
{
   public:
    /// @brief Creates an overlap derivative integrals driver.
    CThreeCenterOverlapGradientGeomX00Driver() = default;

    /// @brief The default copy constructor.
    /// @param other The overlap derivative integrals driver to be copied.
    CThreeCenterOverlapGradientGeomX00Driver(const CThreeCenterOverlapGradientGeomX00Driver &other) = delete;

    /// @brief The default move constructor.
    /// @param other The overlap derivative integrals driver  to be moved.
    CThreeCenterOverlapGradientGeomX00Driver(CThreeCenterOverlapGradientGeomX00Driver &&other) noexcept = delete;
    ...
    auto operator=(const CThreeCenterOverlapGradientGeomX00Driver &other) -> CThreeCenterOverlapGradientGeomX00Driver & = delete;

    /// @brief The default move assignment operator.
    /// @param other The overlap derivative integrals driver to be move assigned.
    /// @return The assigned overlap derivative integrals driver .
    auto operator=(CThreeCenterOverlapGradientGeomX00Driver &&other) noexcept -> CThreeCenterOverlapGradientGeomX00Driver & = delete;
    ...
    auto operator==(const CThreeCenterOverlapGradientGeomX00Driver &other) const -> bool = delete;
...
template <int N>
auto
CThreeCenterOverlapGradientGeomX00Driver<N>::compute(const std::vector<double>         &exponents,
                                                     const std::vector<double>         &factors,
                                                     const std::vector<TPoint<double>> &coordinates,
                                                     const CMolecularBasis             &basis,
                                                     const CMolecule                   &molecule,
                                                     const int                         iatom) const -> CMatrices
{
    ...
    auto ovl_mats = matfunc::make_matrices(std::array<int, 2>{N, 1}, basis, mat_t::general);
...
#pragma omp task firstprivate(bra_gtos, ket_gtos, bra_indices, ket_indices)
                {
                    ...
                    if constexpr (N == 1)
                    {
                        g3ovlfunc::compute_geom_100(distributor, bra_gtos, ket_gtos, bra_indices, ket_indices, false);
                    }
                }
            });
        }
...
#endif /* ThreeCenterOverlapGradientGeomX00Driver_hpp */
