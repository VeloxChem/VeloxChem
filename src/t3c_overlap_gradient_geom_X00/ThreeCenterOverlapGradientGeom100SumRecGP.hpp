#ifndef ThreeCenterOverlapGradientGeom100SumRecGP_hpp
#define ThreeCenterOverlapGradientGeom100SumRecGP_hpp
...
/// @brief Computes (d^(1)/dA^(1)G|GX(r)|P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_10_gp(T& distributor,
                                     ...
                                     const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(909, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    g3ovlrec::comp_prim_overlap_gradient_fp(pbuffer, 81, 23, 41, 51, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_gs(pbuffer, 171, 17, 41, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_gp(pbuffer, 186, 23, 41, 51, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_hs(pbuffer, 231, 41, 171, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_hp(pbuffer, 252, 51, 171, 186, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_hp(pbuffer, 315, 186, 231, 252, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_prim_op_geom_10_gx(pbuffer, 504, 81, 315, 3, 3, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 504, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGradientGeom100SumRecGP_hpp */
