#ifndef ThreeCenterOverlapGradientGeom100SumRecDP_hpp
#define ThreeCenterOverlapGradientGeom100SumRecDP_hpp
...
/// @brief Computes (d^(1)/dA^(1)D|GX(r)|P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_10_dp(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3ovlrec::comp_prim_overlap_ds(pbuffer, 44, 1, 5, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dp(pbuffer, 50, 2, 5, 8, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fs(pbuffer, 68, 5, 44, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fp(pbuffer, 78, 8, 44, 50, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_fp(pbuffer, 108, 50, 68, 78, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_prim_op_geom_10_dx(pbuffer, 198, 17, 108, 3, 3, a_exp);
...
#endif /* ThreeCenterOverlapGradientGeom100SumRecDP_hpp */
