#ifndef ThreeCenterOverlapGradientGeom100SumRecPP_hpp
#define ThreeCenterOverlapGradientGeom100SumRecPP_hpp
...
/// @brief Computes (d^(1)/dA^(1)P|GX(r)|P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_10_pp(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    g3ovlrec::comp_prim_overlap_gradient_sp(pbuffer, 5, 1, 2, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ps(pbuffer, 14, 1, factors, 11);

                    t3ovlrec::comp_prim_overlap_pp(pbuffer, 17, 1, 2, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ds(pbuffer, 26, 1, 14, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dp(pbuffer, 32, 2, 14, 17, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_dp(pbuffer, 50, 17, 26, 32, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_prim_op_geom_10_px(pbuffer, 104, 5, 50, 3, 3, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 104, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGradientGeom100SumRecPP_hpp */
