#ifndef ThreeCenterOverlapGradientGeom100SumRecFP_hpp
#define ThreeCenterOverlapGradientGeom100SumRecFP_hpp
...
/// @brief Computes (d^(1)/dA^(1)F|GX(r)|P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_10_fp(T& distributor,
                                     ...
                                     const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(600, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    g3ovlrec::comp_prim_overlap_gradient_dp(pbuffer, 41, 8, 17, 23, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fs(pbuffer, 95, 5, 17, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fp(pbuffer, 105, 8, 17, 23, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_gs(pbuffer, 135, 17, 95, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_gp(pbuffer, 150, 23, 95, 105, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_gp(pbuffer, 195, 105, 135, 150, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_prim_op_geom_10_fx(pbuffer, 330, 41, 195, 3, 3, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 330, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGradientGeom100SumRecFP_hpp */
