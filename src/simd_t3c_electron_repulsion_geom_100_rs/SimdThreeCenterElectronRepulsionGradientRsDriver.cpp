...
// NOTE: both tables in one translation unit, which the shared name they used to
// carry made ill formed. They are named apart now and this is the unit that
// needs both of them.
#include "SimdThreeCenterElectronRepulsionGeom100RsBufferRows.hpp"
#include "SimdThreeCenterElectronRepulsionGeom010RsBufferRows.hpp"
#include "SimdThreeCenterElectronRepulsionGeom100RsFunc.hpp"
#include "SimdThreeCenterElectronRepulsionGeom010RsFunc.hpp"
...
auto
CSimdThreeCenterElectronRepulsionGradientRsDriver::compute(const CTripleSparsityPattern &pattern,
    ...
    -> std::pair<CSparseTensor, CSparseTensor>
{
    errors::assertMsgCritical(
        omega > 0.0,
        std::string("SimdThreeCenterElectronRepulsionGradientRsDriver: The range separation parameter must be "
                    "positive"));
    ...
    // NOTE: one tensor per operator, both on the one pattern, so an element of
    // either is at the same place in the other. Six values an element in each, the
    // three directions of each of the two atoms on bra side.
    ...
    auto tensor_attenuated = tensor;

    const auto nblocks = static_cast<size_t>(pattern.number_of_blocks());

    if (nblocks == 0) return {std::move(tensor), std::move(tensor_attenuated)};
    ...
    for (size_t iblk = 0; iblk < nblocks; iblk++)
    {
        ...
        arena_rows = std::max(arena_rows,
                              std::max(simdt3ceri::number_of_geom_100_rs_buffer_rows(la, lb, lc),
                                       simdt3ceri::number_of_geom_010_rs_buffer_rows(la, lb, lc)));
    ...
    if (ntasks == 0) return {std::move(tensor), std::move(tensor_attenuated)};
...
#pragma omp for schedule(dynamic)
        for (int itask = 0; itask < ntasks; itask++)
        {
            ...
            auto *values_attenuated =
                tensor_attenuated.values(task.iblock, la, ia, lb, jb, lc, kc);

            const auto chunk = ncomps * natoms * task.npairs;

            const auto stride = 3 * chunk;

            // NOTE: a kernel writes the two operators one after the other, three
            // components each, while the tensors want one operator each. So it
            // writes into scratch and the halves are copied out. That copy is what
            // holding the two operators apart costs, and it is the same trade the
            // distributor of the attenuated integrals makes.

            scratch.assign(6 * chunk, 0.0);

            simdt3cerigrad::compute_rs_electron_repulsion_geom_100(scratch.data(),
                                                                   task.npairs,
                                                                   natoms,
            ...
            std::copy_n(scratch.data() + _coulomb_block * chunk, stride, values);

            std::copy_n(scratch.data() + _attenuated_block * chunk, stride, values_attenuated);

            scratch.assign(6 * chunk, 0.0);

            simdt3cerigrad::compute_rs_electron_repulsion_geom_010(scratch.data(),
                                                                   task.npairs,
                                                                   natoms,
                                                                   a_basis.functions()[task.i],
                                                                   b_basis.functions()[task.j],
            ...
            std::copy_n(scratch.data() + _coulomb_block * chunk, stride, values + stride);

            std::copy_n(scratch.data() + _attenuated_block * chunk, stride,
                        values_attenuated + stride);
        }
    }

    return {std::move(tensor), std::move(tensor_attenuated)};
}
