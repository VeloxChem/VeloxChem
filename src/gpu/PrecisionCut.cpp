...
int32_t cut_descending(const std::vector<double>& desc, double thr) {
    int32_t left = 0, right = (int32_t)desc.size();
    while(left < right) {
...
std::vector<int32_t> build_cut_ij_tile(
    ...
    double tau) 
{
    ...
    const int32_t nkl_tiles = (kl_count + tile_dim - 1) / tile_dim;
    std::vector<double> QD_tile_max(nkl_tiles);

    for(int t = 0; t < nkl_tiles; ++t)
    {
        QD_tile_max[t] = QD_kl[t * tile_dim];
    }

    // build cut_ij_tile (粒度为 tile，scheme A / per ij-tile cut)
    std::vector<int32_t> cut(nij_tiles, 0);
    for(int32_t t = 0; t < nij_tiles; ++t) {
