...
//  Shared two-center driver core — operator-agnostic scaffolding: block
//  fetch, atom-span screening, the task list, the parallel loop, and the
//  dense / block-sparse scatter. The operator supplies only the fused
//  kernel and the screening estimate (`TwoCenterOperator`).
//
...
ket_kept(const int                l_a,
         ...
         const TwoCenterOperator &op) -> bool
{
                  ...
                  KernelProfile           *profile) -> int
{
    ...
    // the fused kernel — bra span A against the kept ket block. The kernel
    // writes every component's [0, cdim) so the spherical buffer needs no
    // zero-fill
...
auto
two_center_compute(const CMolecule&         molecule,
                   ...
                   ThreadBalance&           balance) -> DenseMatrix
{
    ...
    // Unscreened (threshold <= 0) the scatter writes every element, so the
    // zero-fill is redundant — skip it. Screened, the screened-out elements
    // must stay zero, so initialize.
...
auto
two_center_compute_sparse(const CMolecule&         molecule,
                          const CMolecularBasis&   basis,
                          const double             threshold,
                          const TwoCenterOperator& op,
                          KernelProfile*           profile,
