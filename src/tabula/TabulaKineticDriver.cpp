...
/// @brief A conservative upper bound on the kinetic magnitude of the
    ...
    -> bool
{
    ...
    return kinetic_estimate(l_a, l_c, A, B, dx * dx + dy * dy + dz * dz) >= threshold;
}
                  ...
                  KineticProfile      *profile) -> int
{
    ...
    // the fused kinetic kernel — bra span A against the kept ket block. The
    ...
    kinetic_kernel(l_a, l_c, bra.view(), A.cgto_begin, A.cgto_end, ket_view, spherical.data());
...
/// @brief Evaluates one task into the dense kinetic matrix.
auto
...
/// @brief Evaluates one task into the block-sparse kinetic matrix.
auto
...
KineticDriver::compute(const CMolecule       &molecule,
                       ...
                       const double           threshold,
                       KineticProfile        *profile) const -> DenseMatrix
{
    ...
    std::vector<KineticProfile> thread_profiles;
    if (profile != nullptr)
    {
...
#pragma omp parallel for schedule(dynamic)
    for (int p = 0; p < static_cast<int>(tasks.size()); p++)
    {
        ...
        KineticProfile *slot = (profile != nullptr) ? &thread_profiles[tid] : nullptr;
...
KineticDriver::computeSparse(const CMolecule       &molecule,
                             const CMolecularBasis &basis,
                             const double           threshold,
                             KineticProfile        *profile) const -> BlockSparseMatrix
{
    const auto t_blocks = std::chrono::steady_clock::now();
