#ifndef KineticEnergyGeom100RecSD_hpp
#define KineticEnergyGeom100RecSD_hpp
...
/// @brief Computes (d^(1)/dA^(1)S|T|D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_kinetic_energy_geom_10_sd(T&                               distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_pd(pbuffer, 20, 2, 8, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_pd(pbuffer, 38, 5, 14, 20, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_10_sx(pbuffer, 56, 38, 1, 6, a_exp);
...
#endif /* KineticEnergyGeom100RecSD_hpp */
