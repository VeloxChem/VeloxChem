#ifndef KineticEnergyGeom200RecFS_hpp
#define KineticEnergyGeom200RecFS_hpp
...
/// @brief Computes (d^(2)/dA^(2)F|T|S)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_kinetic_energy_geom_20_fs(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cgeom::comp_prim_op_geom_20_fx(pbuffer, 112, 5, 30, 91, 1, 1, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 112, ket_width, ket_npgtos);
            }
...
#endif /* KineticEnergyGeom200RecFS_hpp */
