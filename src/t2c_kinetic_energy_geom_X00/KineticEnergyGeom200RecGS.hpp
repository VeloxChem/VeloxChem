#ifndef KineticEnergyGeom200RecGS_hpp
#define KineticEnergyGeom200RecGS_hpp
...
/// @brief Computes (d^(2)/dA^(2)G|T|S)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_kinetic_energy_geom_20_gs(T&                               distributor,
                               ...
                               const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(258, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_is(pbuffer, 112, 40, 70, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_is(pbuffer, 140, 40, 55, 91, 112, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_20_gx(pbuffer, 168, 14, 55, 140, 1, 1, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 168, ket_width, ket_npgtos);
            }
...
#endif /* KineticEnergyGeom200RecGS_hpp */
