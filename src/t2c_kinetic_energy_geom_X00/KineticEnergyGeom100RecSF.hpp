#ifndef KineticEnergyGeom100RecSF_hpp
#define KineticEnergyGeom100RecSF_hpp
...
/// @brief Computes (d^(1)/dA^(1)S|T|F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_kinetic_energy_geom_10_sf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_pf(pbuffer, 40, 8, 20, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_pf(pbuffer, 70, 14, 30, 40, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_10_sx(pbuffer, 100, 70, 1, 10, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 100, ket_width, ket_npgtos);
            }
...
#endif /* KineticEnergyGeom100RecSF_hpp */
