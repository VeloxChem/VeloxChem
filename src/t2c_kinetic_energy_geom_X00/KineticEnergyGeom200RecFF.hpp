#ifndef KineticEnergyGeom200RecFF_hpp
#define KineticEnergyGeom200RecFF_hpp
...
/// @brief Computes (d^(2)/dA^(2)F|T|F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_kinetic_energy_geom_20_ff(T&                               distributor,
                               ...
                               const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(2280, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_fp(pbuffer, 400, 46, 160, 172, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_fp(pbuffer, 430, 46, 55, 166, 190, 400, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_fd(pbuffer, 460, 64, 172, 208, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_fd(pbuffer, 520, 64, 82, 190, 244, 460, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_ff(pbuffer, 580, 100, 208, 280, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_ff(pbuffer, 680, 100, 130, 244, 340, 580, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_gd(pbuffer, 780, 208, 400, 460, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_gd(pbuffer, 870, 208, 244, 430, 520, 780, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_gf(pbuffer, 960, 280, 460, 580, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_gf(pbuffer, 1110, 280, 340, 520, 680, 960, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_hf(pbuffer, 1260, 580, 780, 960, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_hf(pbuffer, 1470, 580, 680, 870, 1110, 1260, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_20_fx(pbuffer, 1680, 130, 680, 1470, 1, 10, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 1680, ket_width, ket_npgtos);
            }
...
#endif /* KineticEnergyGeom200RecFF_hpp */
