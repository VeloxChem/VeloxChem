#ifndef KineticEnergyGeom100RecFD_hpp
#define KineticEnergyGeom100RecFD_hpp
...
/// @brief Computes (d^(1)/dA^(1)F|T|D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_kinetic_energy_geom_10_fd(T&                               distributor,
                               ...
                               const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(740, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_fp(pbuffer, 200, 26, 80, 92, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_fp(pbuffer, 230, 26, 35, 86, 110, 200, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_fd(pbuffer, 260, 44, 92, 128, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_fd(pbuffer, 320, 44, 62, 110, 164, 260, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_gd(pbuffer, 380, 128, 200, 260, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_gd(pbuffer, 470, 128, 164, 230, 320, 380, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_10_fx(pbuffer, 560, 164, 470, 1, 6, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 560, ket_width, ket_npgtos);
            }
...
#endif /* KineticEnergyGeom100RecFD_hpp */
