#ifndef KineticEnergyGeom200RecGF_hpp
#define KineticEnergyGeom200RecGF_hpp
...
/// @brief Computes (d^(2)/dA^(2)G|T|F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_kinetic_energy_geom_20_gf(T&                               distributor,
                               ...
                               const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(3502, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_gd(pbuffer, 890, 208, 420, 480, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_gd(pbuffer, 980, 208, 244, 450, 540, 890, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_gf(pbuffer, 1070, 280, 480, 600, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_gf(pbuffer, 1220, 280, 340, 540, 700, 1070, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_hd(pbuffer, 1370, 480, 800, 890, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_hd(pbuffer, 1496, 480, 540, 845, 980, 1370, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_hf(pbuffer, 1622, 600, 890, 1070, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_hf(pbuffer, 1832, 600, 700, 980, 1220, 1622, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_if(pbuffer, 2042, 1070, 1370, 1622, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_if(pbuffer, 2322, 1070, 1220, 1496, 1832, 2042, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_20_gx(pbuffer, 2602, 340, 1220, 2322, 1, 10, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 2602, ket_width, ket_npgtos);
            }
...
#endif /* KineticEnergyGeom200RecGF_hpp */
