                                      ...
                                      const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> cart_buffer_0_geom1100_ppdd(2916, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_geom1100_ppdd(2025, ket_dim);
    ...
    {
        cart_buffer_0_geom1100_ppdd.zero();

        spher_buffer_0_geom1100_ppdd.zero();
        ...
        for (int j = 0; j < bra_npgtos; j++)
        {
            ...
            t4c_geom::comp_geom1100_ppdd_0(prim_buffer_0_geom1100_ppdd, prim_buffer_0_ssdd, prim_buffer_0_sddd, prim_buffer_0_dsdd, prim_buffer_0_dddd, a_exp, b_exp );

            t2cfunc::reduce(cart_buffer_0_geom1100_ppdd, prim_buffer_0_geom1100_ppdd, ket_dim, ket_npgtos);
        }

        t4cfunc::full_transform<1, 1, 2, 2>(spher_buffer_0_geom1100_ppdd, cart_spher_buffer_0_geom1100_ppdd);

        distributor->distribute(spher_buffer_0_geom1100_ppdd, a_indices, b_indices, c_indices, d_indices, 1, 1, 2, 2, 1, 1, 0, 0, i, ket_indices);
    }
}
