                                      ...
                                      const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> cart_buffer_0_geom1100_pdpd(2916, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_geom1100_pdpd(2025, ket_dim);
    ...
    {
        cart_buffer_0_geom1100_pdpd.zero();

        spher_buffer_0_geom1100_pdpd.zero();
        ...
        for (int j = 0; j < bra_npgtos; j++)
        {
            ...
            t4c_geom::comp_geom1100_pdpd_0(prim_buffer_0_geom1100_pdpd, prim_buffer_0_sppd, prim_buffer_0_sfpd, prim_buffer_0_dppd, prim_buffer_0_dfpd, a_exp, b_exp );

            t2cfunc::reduce(cart_buffer_0_geom1100_pdpd, prim_buffer_0_geom1100_pdpd, ket_dim, ket_npgtos);
        }

        t4cfunc::full_transform<1, 2, 1, 2>(spher_buffer_0_geom1100_pdpd, cart_spher_buffer_0_geom1100_pdpd);

        distributor->distribute(spher_buffer_0_geom1100_pdpd, a_indices, b_indices, c_indices, d_indices, 1, 2, 1, 2, 1, 1, 0, 0, i, ket_indices);
    }
}
