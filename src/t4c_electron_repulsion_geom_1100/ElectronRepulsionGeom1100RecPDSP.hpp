...
/// Computes (PD|1/|r-r'||SP)  integrals for two GTOs pair blocks.
                                      ...
                                      const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> prim_buffer_1_pfss(30, ket_pdim);

    CSimdArray<double> prim_buffer_0_pfsp(90, ket_pdim);

    CSimdArray<double> prim_buffer_1_pfsp(90, ket_pdim);

    CSimdArray<double> prim_buffer_0_dpsp(54, ket_pdim);

    CSimdArray<double> prim_buffer_0_dfsp(180, ket_pdim);

    // allocate aligned Cartesian integrals

    CSimdArray<double> cart_buffer_0_geom1100_pdsp(486, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_geom1100_pdsp(405, ket_dim);
    ...
    {
        cart_buffer_0_geom1100_pdsp.zero();

        spher_buffer_0_geom1100_pdsp.zero();
        ...
        for (int j = 0; j < bra_npgtos; j++)
        {
            ...
            erirec::comp_prim_electron_repulsion_pfss(prim_buffer_1_pfss, prim_buffer_1_sdss, prim_buffer_2_sdss, prim_buffer_1_sfss, prim_buffer_2_sfss, pa_x, pa_y, pa_z, wp_x[0], wp_y[0], wp_z[0], a_exp, b_exp, c_exps[0], d_exps[0]);

            erirec::comp_prim_electron_repulsion_pfsp(prim_buffer_0_pfsp, prim_buffer_0_sdsp, prim_buffer_1_sdsp, prim_buffer_1_sfss, prim_buffer_0_sfsp, prim_buffer_1_sfsp, pa_x, pa_y, pa_z, wp_x[0], wp_y[0], wp_z[0], a_exp, b_exp, c_exps[0], d_exps[0]);
            ...
            erirec::comp_prim_electron_repulsion_dfsp(prim_buffer_0_dfsp, prim_buffer_0_sfsp, prim_buffer_1_sfsp, prim_buffer_0_pdsp, prim_buffer_1_pdsp, prim_buffer_1_pfss, prim_buffer_0_pfsp, prim_buffer_1_pfsp, pa_x, pa_y, pa_z, wp_x[0], wp_y[0], wp_z[0], a_exp, b_exp, c_exps[0], d_exps[0]);

            t4c_geom::comp_geom1100_pdsp_0(prim_buffer_0_geom1100_pdsp, prim_buffer_0_spsp, prim_buffer_0_sfsp, prim_buffer_0_dpsp, prim_buffer_0_dfsp, a_exp, b_exp );

            t2cfunc::reduce(cart_buffer_0_geom1100_pdsp, prim_buffer_0_geom1100_pdsp, ket_dim, ket_npgtos);
        }

        t4cfunc::full_transform<1, 2, 0, 1>(spher_buffer_0_geom1100_pdsp, cart_spher_buffer_0_geom1100_pdsp);

        distributor->distribute(spher_buffer_0_geom1100_pdsp, a_indices, b_indices, c_indices, d_indices, 1, 2, 0, 1, 1, 1, 0, 0, i, ket_indices);
    }
}
