                                      ...
                                      const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> cart_buffer_0_geom1000_ddpd(1944, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_geom1000_ddpd(1125, ket_dim);
    ...
    {
        cart_buffer_0_geom1000_ddpd.zero();

        spher_buffer_0_geom1000_ddpd.zero();
        ...
        for (int j = 0; j < bra_npgtos; j++)
        {
            ...
            t4c_geom::comp_geom1000_ddpd_0(prim_buffer_0_geom1000_ddpd, prim_buffer_0_pdpd, prim_buffer_0_fdpd, a_exp );

            t2cfunc::reduce(cart_buffer_0_geom1000_ddpd, prim_buffer_0_geom1000_ddpd, ket_dim, ket_npgtos);
        }

        t4cfunc::full_transform<2, 2, 1, 2>(spher_buffer_0_geom1000_ddpd, cart_spher_buffer_0_geom1000_ddpd);

        distributor->distribute(spher_buffer_0_geom1000_ddpd, a_indices, b_indices, c_indices, d_indices, 2, 2, 1, 2, 1, 0, 0, 0, i, ket_indices);
    }
}
