                                      ...
                                      const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> cart_buffer_0_geom1000_dddd(3888, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_geom1000_dddd(1875, ket_dim);
    ...
    {
        cart_buffer_0_geom1000_dddd.zero();

        spher_buffer_0_geom1000_dddd.zero();
        ...
        for (int j = 0; j < bra_npgtos; j++)
        {
            ...
            t4c_geom::comp_geom1000_dddd_0(prim_buffer_0_geom1000_dddd, prim_buffer_0_pddd, prim_buffer_0_fddd, a_exp );

            t2cfunc::reduce(cart_buffer_0_geom1000_dddd, prim_buffer_0_geom1000_dddd, ket_dim, ket_npgtos);
        }

        t4cfunc::full_transform<2, 2, 2, 2>(spher_buffer_0_geom1000_dddd, cart_spher_buffer_0_geom1000_dddd);

        distributor->distribute(spher_buffer_0_geom1000_dddd, a_indices, b_indices, c_indices, d_indices, 2, 2, 2, 2, 1, 0, 0, 0, i, ket_indices);
    }
}
