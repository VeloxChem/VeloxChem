                                      ...
                                      const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> cart_buffer_0_geom1000_pddd(1944, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_geom1000_pddd(1125, ket_dim);
    ...
    {
        cart_buffer_0_geom1000_pddd.zero();

        spher_buffer_0_geom1000_pddd.zero();
        ...
        for (int j = 0; j < bra_npgtos; j++)
        {
            ...
            t4c_geom::comp_geom1000_pddd_0(prim_buffer_0_geom1000_pddd, prim_buffer_0_sddd, prim_buffer_0_dddd, a_exp );

            t2cfunc::reduce(cart_buffer_0_geom1000_pddd, prim_buffer_0_geom1000_pddd, ket_dim, ket_npgtos);
        }

        t4cfunc::full_transform<1, 2, 2, 2>(spher_buffer_0_geom1000_pddd, cart_spher_buffer_0_geom1000_pddd);

        distributor->distribute(spher_buffer_0_geom1000_pddd, a_indices, b_indices, c_indices, d_indices, 1, 2, 2, 2, 1, 0, 0, 0, i, ket_indices);
    }
}
