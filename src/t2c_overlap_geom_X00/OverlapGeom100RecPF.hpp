#ifndef OverlapGeom100RecPF_hpp
#define OverlapGeom100RecPF_hpp
...
/// @brief Computes (d^(1)/dA^(1)P|F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_overlap_geom_10_pf(T& distributor,
                        ...
                        const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(218, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_pd(pbuffer, 20, 1, 4, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_pf(pbuffer, 38, 4, 10, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_df(pbuffer, 68, 10, 20, 38, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_10_px(pbuffer, 128, 10, 68, 1, 10, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 128, ket_width, ket_npgtos);
            }
