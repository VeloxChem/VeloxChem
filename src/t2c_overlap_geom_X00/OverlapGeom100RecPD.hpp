#ifndef OverlapGeom100RecPD_hpp
#define OverlapGeom100RecPD_hpp
...
/// @brief Computes (d^(1)/dA^(1)P|D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_overlap_geom_10_pd(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_pp(pbuffer, 10, 0, 1, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_pd(pbuffer, 19, 1, 4, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_dd(pbuffer, 37, 4, 10, 19, factors, 8, a_exp);
