#ifndef OverlapGeom100RecSF_hpp
#define OverlapGeom100RecSF_hpp
...
/// @brief Computes (d^(1)/dA^(1)S|F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_overlap_geom_10_sf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_pf(pbuffer, 20, 4, 10, factors, 8, a_exp);
