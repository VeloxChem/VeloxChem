#ifndef OverlapGeom200RecGP_hpp
#define OverlapGeom200RecGP_hpp
...
/// @brief Computes (d^(2)/dA^(2)G|P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_overlap_geom_20_gp(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_gs(pbuffer, 80, 16, 40, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_gp(pbuffer, 95, 22, 40, 50, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_hs(pbuffer, 140, 40, 80, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_hp(pbuffer, 161, 50, 80, 95, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_ip(pbuffer, 224, 95, 140, 161, factors, 8, a_exp);
