#ifndef OverlapGeom100RecSD_hpp
#define OverlapGeom100RecSD_hpp
...
/// @brief Computes (d^(1)/dA^(1)S|D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_overlap_geom_10_sd(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cgeom::comp_prim_op_geom_10_sx(pbuffer, 28, 10, 1, 6, a_exp);
