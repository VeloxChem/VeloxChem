#ifndef OverlapGeom200RecPD_hpp
#define OverlapGeom200RecPD_hpp
...
/// @brief Computes (d^(2)/dA^(2)P|D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_overlap_geom_20_pd(T& distributor,
                        ...
                        const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(262, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cgeom::comp_prim_op_geom_20_px(pbuffer, 154, 22, 94, 1, 6, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 154, ket_width, ket_npgtos);
            }
