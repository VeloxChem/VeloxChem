#ifndef ElectronRepulsionGeom1100RecPDDF_hpp
#define ElectronRepulsionGeom1100RecPDDF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(PD|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_pddf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(3588, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(17190, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(17955, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 138, pbuffer, 2605, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 238, pbuffer, 2905, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 388, pbuffer, 3355, 210, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 598, pbuffer, 511, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 628, pbuffer, 661, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 673, pbuffer, 886, 63, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 1472, pbuffer, 2605, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1572, pbuffer, 2905, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1722, pbuffer, 3355, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1932, pbuffer, 4075, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2082, pbuffer, 4375, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2307, pbuffer, 4825, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2622, pbuffer, 5455, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2832, pbuffer, 5665, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3147, pbuffer, 5980, 441, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1485, cbuffer, 138, 238, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 1785, cbuffer, 238, 388, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 2235, 1485, 1785, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2835, cbuffer, 598, 628, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 2925, cbuffer, 628, 673, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 3060, 2835, 2925, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 3240, cbuffer, 736, 796, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 3420, cbuffer, 796, 886, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 3690, 3240, 3420, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 5130, cbuffer, 1012, 1112, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 5430, cbuffer, 1112, 1262, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 5880, 5130, 5430, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 10980, cbuffer, 1472, 1572, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 11280, cbuffer, 1572, 1722, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 11730, 10980, 11280, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 12330, cbuffer, 1932, 2082, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 12780, cbuffer, 2082, 2307, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 13455, 12330, 12780, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 14355, cbuffer, 2622, 2832, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 14985, cbuffer, 2832, 3147, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 15930, 14355, 14985, cfactors, 6, 0, 5);

            t4cfunc::ket_transform<2, 3>(skbuffer, 0, ckbuffer, 225, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 11445, ckbuffer, 2235, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 11795, ckbuffer, 3060, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 11900, ckbuffer, 3690, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 12740, ckbuffer, 5880, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 16345, ckbuffer, 11730, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 16695, ckbuffer, 13455, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 17220, ckbuffer, 15930, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 15715, 11900, 12740, r_ab, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 105, 0, 11445, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 12110, 11795, 16345, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 13090, 11900, 16695, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 14140, 12740, 17220, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 735, 11900, 12110, 13090, r_ab, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 2625, 12740, 13090, 14140, r_ab, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pdxx(skbuffer, 5775, 105, 15715, 735, 2625, r_ab, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 0, skbuffer, 5775, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 525, skbuffer, 6405, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 1050, skbuffer, 7035, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 1575, skbuffer, 7665, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 2100, skbuffer, 8295, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 2625, skbuffer, 8925, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 3150, skbuffer, 9555, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 3675, skbuffer, 10185, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 4200, skbuffer, 10815, 2, 3);
...
#endif /* ElectronRepulsionGeom1100RecPDDF_hpp */
