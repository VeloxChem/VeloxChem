#ifndef ElectronRepulsionGeom1100RecFDSD_hpp
#define ElectronRepulsionGeom1100RecFDSD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(FD|1/|r-r'||SD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_fdsd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(1380, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(19435, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 154, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 18, pbuffer, 394, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 54, pbuffer, 760, 60, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * b_exp, {760, 820});

                pbuffer.scale(2.0 * b_exp, {1225, 1315});

                pbuffer.scale(2.0 * b_exp, {1732, 1858});

                t2cfunc::reduce(cbuffer, 114, pbuffer, 760, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 174, pbuffer, 1225, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 264, pbuffer, 1732, 126, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {154, 172});

                pbuffer.scale(2.0 * a_exp, {394, 430});

                pbuffer.scale(a_exp / b_exp, {760, 820});

                pbuffer.scale(a_exp / b_exp, {1225, 1315});

                pbuffer.scale(a_exp / b_exp, {1732, 1858});

                t2cfunc::reduce(cbuffer, 390, pbuffer, 154, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 408, pbuffer, 394, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 444, pbuffer, 760, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 504, pbuffer, 1225, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 594, pbuffer, 1732, 126, ket_width, ket_npgtos);
                ...
                pbuffer.scale(4.0 * a_exp * b_exp, {2194, 2362});

                pbuffer.scale(4.0 * a_exp * b_exp, {2530, 2746});

                t2cfunc::reduce(cbuffer, 720, pbuffer, 760, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 780, pbuffer, 1225, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 870, pbuffer, 1732, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 996, pbuffer, 2194, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1164, pbuffer, 2530, 216, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<0, 2>(skbuffer, 405, cbuffer, 54, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 16715, cbuffer, 114, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 16765, cbuffer, 174, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 16840, cbuffer, 264, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 16945, cbuffer, 390, 0, 1);

            t4cfunc::ket_transform<0, 2>(skbuffer, 16960, cbuffer, 408, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 17080, cbuffer, 444, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 17280, cbuffer, 504, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 17580, cbuffer, 594, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 18885, cbuffer, 720, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 18935, cbuffer, 780, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 19010, cbuffer, 870, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 19115, cbuffer, 996, 0, 6);

            t4cfunc::ket_transform<0, 2>(skbuffer, 19255, cbuffer, 1164, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 2900, 15, 405, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 18420, 16960, 17080, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 18510, 17080, 17280, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 18660, 17280, 17580, r_ab, 0, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 3260, 15, 18420, 18510, r_ab, 0, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 4790, 405, 18510, 18660, r_ab, 0, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ddxx(skbuffer, 9155, 2900, 3260, 4790, r_ab, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 45, 0, 16715, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 455, 15, 16765, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 1055, 405, 16840, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_pdxx(skbuffer, 2990, 15, 45, 455, r_ab, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_pfxx(skbuffer, 4340, 405, 455, 1055, r_ab, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_ddxx(skbuffer, 8615, 2900, 2990, 4340, r_ab, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 16990, 16945, 18885, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 17130, 16960, 18935, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 17355, 17080, 19010, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_shxx(skbuffer, 17685, 17280, 19115, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sixx(skbuffer, 18000, 17580, 19255, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 135, 16960, 16990, 17130, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 605, 17080, 17130, 17355, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sgxx(skbuffer, 1280, 17280, 17355, 17685, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_shxx(skbuffer, 1955, 17580, 17685, 18000, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pdxx(skbuffer, 3530, 45, 18420, 135, 605, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pfxx(skbuffer, 5240, 455, 18510, 605, 1280, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pgxx(skbuffer, 6590, 1055, 18660, 1280, 1955, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ddxx(skbuffer, 9695, 2990, 3260, 3530, 5240, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dfxx(skbuffer, 11315, 4340, 4790, 5240, 6590, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_fdxx(skbuffer, 14015, 8615, 9155, 9695, 11315, r_ab, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 0, skbuffer, 14015, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 175, skbuffer, 14315, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 350, skbuffer, 14615, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 525, skbuffer, 14915, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 700, skbuffer, 15215, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 875, skbuffer, 15515, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1050, skbuffer, 15815, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1225, skbuffer, 16115, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1400, skbuffer, 16415, 0, 2);
...
#endif /* ElectronRepulsionGeom1100RecFDSD_hpp */
