#ifndef ElectronRepulsionGeom1100RecSFPF_hpp
#define ElectronRepulsionGeom1100RecSFPF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SF|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_sfpf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(1300, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(3810, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(4557, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 400, pbuffer, 2465, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 550, pbuffer, 2765, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 775, pbuffer, 3215, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 985, pbuffer, 3425, 315, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<1, 3>(skbuffer, 1890, ckbuffer, 0, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 2016, ckbuffer, 180, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 3801, ckbuffer, 2730, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 4116, ckbuffer, 3180, 0, 5);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 2226, 1890, 3801, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 2856, 2016, 4116, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 0, 2016, 2226, 2856, r_ab, 1, 3);
            ...
            t4cfunc::bra_transform<0, 3>(sbuffer, 882, skbuffer, 1260, 1, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 1029, skbuffer, 1470, 1, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 1176, skbuffer, 1680, 1, 3);
...
#endif /* ElectronRepulsionGeom1100RecSFPF_hpp */
