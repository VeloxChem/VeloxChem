#ifndef ElectronRepulsionGeom1100RecFDPF_hpp
#define ElectronRepulsionGeom1100RecFDPF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(FD|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_fdpf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(5750, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(16890, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(81627, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 480, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 30, pbuffer, 690, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 75, pbuffer, 1275, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 135, pbuffer, 1635, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 225, pbuffer, 2525, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 325, pbuffer, 3025, 150, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * b_exp, {2525, 2625});

                pbuffer.scale(2.0 * b_exp, {3025, 3175});
                ...
                t2cfunc::reduce(cbuffer, 475, pbuffer, 2525, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 575, pbuffer, 3025, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 725, pbuffer, 4150, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 875, pbuffer, 4750, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1100, pbuffer, 5965, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1310, pbuffer, 6595, 315, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {480, 510});

                pbuffer.scale(2.0 * a_exp, {690, 735});

                pbuffer.scale(2.0 * a_exp, {1275, 1335});

                pbuffer.scale(2.0 * a_exp, {1635, 1725});

                pbuffer.scale(a_exp / b_exp, {2525, 2625});

                pbuffer.scale(a_exp / b_exp, {3025, 3175});
                ...
                t2cfunc::reduce(cbuffer, 1625, pbuffer, 480, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1655, pbuffer, 690, 45, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 3000, pbuffer, 2525, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3100, pbuffer, 3025, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3250, pbuffer, 4150, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3400, pbuffer, 4750, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3625, pbuffer, 5965, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3835, pbuffer, 6595, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4150, pbuffer, 7708, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4430, pbuffer, 8268, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4850, pbuffer, 9108, 360, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5210, pbuffer, 9468, 540, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 810, cbuffer, 225, 325, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 3360, cbuffer, 475, 575, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 3660, cbuffer, 725, 875, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 4110, cbuffer, 1100, 1310, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 4740, cbuffer, 1625, 1655, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 4830, cbuffer, 1700, 1760, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 5550, cbuffer, 1850, 1950, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 6750, cbuffer, 2100, 2250, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 8550, cbuffer, 2475, 2685, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 13590, cbuffer, 3000, 3100, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 13890, cbuffer, 3250, 3400, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 14340, cbuffer, 3625, 3835, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 14970, cbuffer, 4150, 4430, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 15810, cbuffer, 4850, 5210, cfactors, 6, 0, 7);

            t4cfunc::ket_transform<1, 3>(skbuffer, 0, ckbuffer, 0, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 63, ckbuffer, 90, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1701, ckbuffer, 810, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 70203, ckbuffer, 3360, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 70413, ckbuffer, 3660, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 70728, ckbuffer, 4110, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 71169, ckbuffer, 4740, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 71232, ckbuffer, 4830, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 71736, ckbuffer, 5550, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 72576, ckbuffer, 6750, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 73836, ckbuffer, 8550, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 79317, ckbuffer, 13590, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 79527, ckbuffer, 13890, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 79842, ckbuffer, 14340, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 80283, ckbuffer, 14970, 0, 6);

            t4cfunc::ket_transform<1, 3>(skbuffer, 80871, ckbuffer, 15810, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 12180, 63, 1701, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 77364, 71232, 71736, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 77742, 71736, 72576, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 78372, 72576, 73836, r_ab, 1, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 13692, 63, 77364, 77742, r_ab, 1, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 20118, 1701, 77742, 78372, r_ab, 1, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ddxx(skbuffer, 38451, 12180, 13692, 20118, r_ab, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 189, 0, 70203, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 1911, 63, 70413, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 4431, 1701, 70728, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_pdxx(skbuffer, 12558, 63, 189, 1911, r_ab, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_pfxx(skbuffer, 18228, 1701, 1911, 4431, r_ab, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_ddxx(skbuffer, 36183, 12180, 12558, 18228, r_ab, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 71358, 71169, 79317, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 71946, 71232, 79527, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 72891, 71736, 79842, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_shxx(skbuffer, 74277, 72576, 80283, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sixx(skbuffer, 75600, 73836, 80871, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 567, 71232, 71358, 71946, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 2541, 71736, 71946, 72891, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sgxx(skbuffer, 5376, 72576, 72891, 74277, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_shxx(skbuffer, 8211, 73836, 74277, 75600, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pdxx(skbuffer, 14826, 189, 77364, 567, 2541, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pfxx(skbuffer, 22008, 1911, 77742, 2541, 5376, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pgxx(skbuffer, 27678, 4431, 78372, 5376, 8211, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ddxx(skbuffer, 40719, 12558, 13692, 14826, 22008, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dfxx(skbuffer, 47523, 18228, 20118, 22008, 27678, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_fdxx(skbuffer, 58863, 36183, 38451, 40719, 47523, r_ab, 1, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 0, skbuffer, 58863, 1, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 735, skbuffer, 60123, 1, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1470, skbuffer, 61383, 1, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 2205, skbuffer, 62643, 1, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 2940, skbuffer, 63903, 1, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 3675, skbuffer, 65163, 1, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 4410, skbuffer, 66423, 1, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 5145, skbuffer, 67683, 1, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 5880, skbuffer, 68943, 1, 3);
...
#endif /* ElectronRepulsionGeom1100RecFDPF_hpp */
