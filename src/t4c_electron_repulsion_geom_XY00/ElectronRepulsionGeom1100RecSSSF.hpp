#ifndef ElectronRepulsionGeom1100RecSSSF_hpp
#define ElectronRepulsionGeom1100RecSSSF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SS|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_sssf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(4.0 * a_exp * b_exp, {93, 123});

                pbuffer.scale(4.0 * a_exp * b_exp, {153, 213});
            ...
            t4cfunc::bra_transform<0, 0>(sbuffer, 0, skbuffer, 63, 0, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 7, skbuffer, 70, 0, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 14, skbuffer, 77, 0, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 21, skbuffer, 84, 0, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 28, skbuffer, 91, 0, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 35, skbuffer, 98, 0, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 42, skbuffer, 105, 0, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 49, skbuffer, 112, 0, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 56, skbuffer, 119, 0, 3);
...
#endif /* ElectronRepulsionGeom1100RecSSSF_hpp */
