#ifndef ElectronRepulsionGeom1100RecPPFF_hpp
#define ElectronRepulsionGeom1100RecPPFF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(PP|1/|r-r'||FF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_ppff(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(3552, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(25224, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(13965, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * b_exp, {1599, 1659});

                pbuffer.scale(2.0 * b_exp, {1779, 1869});

                pbuffer.scale(2.0 * b_exp, {2049, 2175});

                pbuffer.scale(2.0 * b_exp, {2427, 2595});

                t2cfunc::reduce(cbuffer, 74, pbuffer, 1599, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 134, pbuffer, 1779, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 224, pbuffer, 2049, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 350, pbuffer, 2427, 168, ket_width, ket_npgtos);
                ...
                pbuffer.scale(a_exp / b_exp, {1599, 1659});

                pbuffer.scale(a_exp / b_exp, {1779, 1869});

                pbuffer.scale(a_exp / b_exp, {2049, 2175});

                pbuffer.scale(a_exp / b_exp, {2427, 2595});

                t2cfunc::reduce(cbuffer, 518, pbuffer, 95, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 528, pbuffer, 175, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 543, pbuffer, 280, 21, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 564, pbuffer, 406, 28, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 592, pbuffer, 621, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 622, pbuffer, 741, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 667, pbuffer, 921, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 730, pbuffer, 1173, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 814, pbuffer, 1599, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 874, pbuffer, 1779, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 964, pbuffer, 2049, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1090, pbuffer, 2427, 168, ket_width, ket_npgtos);
                ...
                pbuffer.scale(4.0 * a_exp * b_exp, {2991, 3091});

                pbuffer.scale(4.0 * a_exp * b_exp, {3191, 3341});

                pbuffer.scale(4.0 * a_exp * b_exp, {3491, 3701});

                pbuffer.scale(4.0 * a_exp * b_exp, {3911, 4191});

                pbuffer.scale(4.0 * a_exp * b_exp, {4471, 4621});

                pbuffer.scale(4.0 * a_exp * b_exp, {4621, 4846});

                pbuffer.scale(4.0 * a_exp * b_exp, {4846, 5161});

                pbuffer.scale(4.0 * a_exp * b_exp, {5161, 5581});

                t2cfunc::reduce(cbuffer, 1258, pbuffer, 1599, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1318, pbuffer, 1779, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1408, pbuffer, 2049, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1534, pbuffer, 2427, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1702, pbuffer, 2991, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1802, pbuffer, 3191, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1952, pbuffer, 3491, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2162, pbuffer, 3911, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2442, pbuffer, 4471, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2592, pbuffer, 4621, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2817, pbuffer, 4846, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3132, pbuffer, 5161, 420, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1288, cbuffer, 74, 134, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 1468, cbuffer, 134, 224, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 1738, cbuffer, 224, 350, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 2116, 1288, 1468, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 2476, 1468, 1738, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 3016, 2116, 2476, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 3616, cbuffer, 518, 528, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 3646, cbuffer, 528, 543, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 3691, cbuffer, 543, 564, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 3754, 3616, 3646, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 3814, 3646, 3691, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 3904, 3754, 3814, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 4004, cbuffer, 592, 622, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 4094, cbuffer, 622, 667, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 4229, cbuffer, 667, 730, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 4418, 4004, 4094, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 4598, 4094, 4229, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 4868, 4418, 4598, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 6068, cbuffer, 814, 874, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 6248, cbuffer, 874, 964, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 6518, cbuffer, 964, 1090, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 6896, 6068, 6248, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 7256, 6248, 6518, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 7796, 6896, 7256, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 13196, cbuffer, 1258, 1318, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 13376, cbuffer, 1318, 1408, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 13646, cbuffer, 1408, 1534, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 14024, 13196, 13376, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 14384, 13376, 13646, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 14924, 14024, 14384, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 15524, cbuffer, 1702, 1802, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 15824, cbuffer, 1802, 1952, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 16274, cbuffer, 1952, 2162, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 16904, 15524, 15824, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 17504, 15824, 16274, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 18404, 16904, 17504, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 19404, cbuffer, 2442, 2592, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 19854, cbuffer, 2592, 2817, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 20529, cbuffer, 2817, 3132, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 21474, 19404, 19854, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 22374, 19854, 20529, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 23724, 21474, 22374, cfactors, 6, 0, 4);

            t4cfunc::ket_transform<3, 3>(skbuffer, 0, ckbuffer, 288, 0, 0);

            t4cfunc::ket_transform<3, 3>(skbuffer, 8428, ckbuffer, 3016, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 8722, ckbuffer, 3904, 0, 0);

            t4cfunc::ket_transform<3, 3>(skbuffer, 8771, ckbuffer, 4868, 0, 1);

            t4cfunc::ket_transform<3, 3>(skbuffer, 9359, ckbuffer, 7796, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 12446, ckbuffer, 14924, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 12740, ckbuffer, 18404, 0, 3);

            t4cfunc::ket_transform<3, 3>(skbuffer, 13230, ckbuffer, 23724, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 12005, 8771, 9359, r_ab, 3, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 49, 0, 8428, 3, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 8918, 8722, 12446, 3, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 9653, 8771, 12740, 3, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 10535, 9359, 13230, 3, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 490, 8771, 8918, 9653, r_ab, 3, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 1813, 9359, 9653, 10535, r_ab, 3, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ppxx(skbuffer, 4459, 49, 12005, 490, 1813, r_ab, 3, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 4459, 3, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 441, skbuffer, 4900, 3, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 882, skbuffer, 5341, 3, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 1323, skbuffer, 5782, 3, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 1764, skbuffer, 6223, 3, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 2205, skbuffer, 6664, 3, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 2646, skbuffer, 7105, 3, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 3087, skbuffer, 7546, 3, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 3528, skbuffer, 7987, 3, 3);
...
#endif /* ElectronRepulsionGeom1100RecPPFF_hpp */
