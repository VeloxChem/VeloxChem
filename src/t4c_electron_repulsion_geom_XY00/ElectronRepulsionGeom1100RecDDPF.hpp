#ifndef ElectronRepulsionGeom1100RecDDPF_hpp
#define ElectronRepulsionGeom1100RecDDPF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(DD|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_ddpf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(3550, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(10380, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(32277, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * b_exp, {2075, 2175});

                pbuffer.scale(2.0 * b_exp, {2475, 2625});

                pbuffer.scale(2.0 * b_exp, {3300, 3450});

                pbuffer.scale(2.0 * b_exp, {3750, 3975});

                t2cfunc::reduce(cbuffer, 225, pbuffer, 2075, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 325, pbuffer, 2475, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 475, pbuffer, 3300, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 625, pbuffer, 3750, 225, ket_width, ket_npgtos);
                ...
                pbuffer.scale(a_exp / b_exp, {2075, 2175});

                pbuffer.scale(a_exp / b_exp, {2475, 2625});

                pbuffer.scale(a_exp / b_exp, {3300, 3450});

                pbuffer.scale(a_exp / b_exp, {3750, 3975});

                t2cfunc::reduce(cbuffer, 850, pbuffer, 415, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 880, pbuffer, 595, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 925, pbuffer, 1075, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 985, pbuffer, 1375, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1075, pbuffer, 2075, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1175, pbuffer, 2475, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1325, pbuffer, 3300, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1475, pbuffer, 3750, 225, ket_width, ket_npgtos);
                ...
                pbuffer.scale(4.0 * a_exp * b_exp, {4551, 4761});

                pbuffer.scale(4.0 * a_exp * b_exp, {4971, 5286});

                pbuffer.scale(4.0 * a_exp * b_exp, {5601, 5881});

                pbuffer.scale(4.0 * a_exp * b_exp, {5881, 6301});

                t2cfunc::reduce(cbuffer, 1700, pbuffer, 2075, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1800, pbuffer, 2475, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1950, pbuffer, 3300, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2100, pbuffer, 3750, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2325, pbuffer, 4551, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2535, pbuffer, 4971, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2850, pbuffer, 5601, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3130, pbuffer, 5881, 420, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2460, cbuffer, 850, 880, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2550, cbuffer, 925, 985, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 3270, cbuffer, 1075, 1175, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 4470, cbuffer, 1325, 1475, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 8160, cbuffer, 1700, 1800, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 8460, cbuffer, 1950, 2100, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 8910, cbuffer, 2325, 2535, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 9540, cbuffer, 2850, 3130, cfactors, 6, 0, 6);

            t4cfunc::ket_transform<1, 3>(skbuffer, 0, ckbuffer, 0, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 63, ckbuffer, 90, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 25200, ckbuffer, 1710, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 25410, ckbuffer, 2010, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 25725, ckbuffer, 2460, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 25788, ckbuffer, 2550, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 26292, ckbuffer, 3270, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 27132, ckbuffer, 4470, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 30723, ckbuffer, 8160, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 30933, ckbuffer, 8460, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 31248, ckbuffer, 8910, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 31689, ckbuffer, 9540, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 29715, 25788, 26292, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 30093, 26292, 27132, r_ab, 1, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 8190, 63, 29715, 30093, r_ab, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 189, 0, 25200, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 1701, 63, 25410, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_pdxx(skbuffer, 7056, 63, 189, 1701, r_ab, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 25914, 25725, 30723, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 26502, 25788, 30933, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 27447, 26292, 31248, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_shxx(skbuffer, 28392, 27132, 31689, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 567, 25788, 25914, 26502, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 2331, 26292, 26502, 27447, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sgxx(skbuffer, 4221, 27132, 27447, 28392, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pdxx(skbuffer, 9324, 189, 29715, 567, 2331, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pfxx(skbuffer, 12726, 1701, 30093, 2331, 4221, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ddxx(skbuffer, 18396, 7056, 8190, 9324, 12726, r_ab, 1, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 0, skbuffer, 18396, 1, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 525, skbuffer, 19152, 1, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 1050, skbuffer, 19908, 1, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 1575, skbuffer, 20664, 1, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 2100, skbuffer, 21420, 1, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 2625, skbuffer, 22176, 1, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 3150, skbuffer, 22932, 1, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 3675, skbuffer, 23688, 1, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 4200, skbuffer, 24444, 1, 3);
...
#endif /* ElectronRepulsionGeom1100RecDDPF_hpp */
