#ifndef ElectronRepulsionGeom1100RecSFDF_hpp
#define ElectronRepulsionGeom1100RecSFDF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SF|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_sfdf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(2392, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(11520, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(7595, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 736, pbuffer, 4075, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 886, pbuffer, 4375, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1111, pbuffer, 4825, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1426, pbuffer, 5455, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1636, pbuffer, 5665, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1951, pbuffer, 5980, 441, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<2, 3>(skbuffer, 3150, ckbuffer, 450, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 3360, ckbuffer, 1560, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 6335, ckbuffer, 7785, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 6860, ckbuffer, 10260, 0, 5);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 3710, 3150, 6335, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 4760, 3360, 6860, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 0, 3360, 3710, 4760, r_ab, 2, 3);
...
#endif /* ElectronRepulsionGeom1100RecSFDF_hpp */
