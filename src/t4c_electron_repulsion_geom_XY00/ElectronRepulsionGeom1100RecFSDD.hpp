#ifndef ElectronRepulsionGeom1100RecFSDD_hpp
#define ElectronRepulsionGeom1100RecFSDD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(FS|1/|r-r'||DD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_fsdd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(3038, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(12876, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(28100, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * b_exp, {290, 308});

                pbuffer.scale(2.0 * b_exp, {380, 410});

                pbuffer.scale(2.0 * b_exp, {530, 575});
                ...
                pbuffer.scale(a_exp / b_exp, {290, 308});

                pbuffer.scale(a_exp / b_exp, {380, 410});

                pbuffer.scale(a_exp / b_exp, {530, 575});
                ...
                t2cfunc::reduce(cbuffer, 713, pbuffer, 37, 6, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 719, pbuffer, 85, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 729, pbuffer, 155, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 744, pbuffer, 290, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 762, pbuffer, 380, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 792, pbuffer, 530, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 837, pbuffer, 821, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 873, pbuffer, 965, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 933, pbuffer, 1205, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1023, pbuffer, 1635, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1083, pbuffer, 1815, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1183, pbuffer, 2115, 150, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 1333, pbuffer, 290, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1351, pbuffer, 380, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1381, pbuffer, 530, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1426, pbuffer, 821, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1462, pbuffer, 965, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1522, pbuffer, 1205, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1612, pbuffer, 1635, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1672, pbuffer, 1815, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1772, pbuffer, 2115, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1922, pbuffer, 2610, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2012, pbuffer, 2790, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2162, pbuffer, 3090, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2387, pbuffer, 3540, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2513, pbuffer, 3666, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2723, pbuffer, 3876, 315, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1560, cbuffer, 217, 253, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1668, cbuffer, 253, 313, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 1848, 1560, 1668, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 2064, cbuffer, 403, 463, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2244, cbuffer, 463, 563, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 2544, 2064, 2244, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 2904, cbuffer, 713, 719, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2922, cbuffer, 719, 729, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 2952, 2904, 2922, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 2988, cbuffer, 744, 762, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 3042, cbuffer, 762, 792, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 3132, 2988, 3042, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 3564, cbuffer, 837, 873, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 3672, cbuffer, 873, 933, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 3852, 3564, 3672, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 4716, cbuffer, 1023, 1083, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 4896, cbuffer, 1083, 1183, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 5196, 4716, 4896, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 8256, cbuffer, 1333, 1351, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 8310, cbuffer, 1351, 1381, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 8400, 8256, 8310, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 8508, cbuffer, 1426, 1462, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 8616, cbuffer, 1462, 1522, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 8796, 8508, 8616, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 9012, cbuffer, 1612, 1672, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 9192, cbuffer, 1672, 1772, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 9492, 9012, 9192, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 9852, cbuffer, 1922, 2012, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 10122, cbuffer, 2012, 2162, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 10572, 9852, 10122, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 11112, cbuffer, 2387, 2513, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 11490, cbuffer, 2513, 2723, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 12120, 11112, 11490, cfactors, 6, 0, 5);

            t4cfunc::ket_transform<2, 2>(skbuffer, 0, ckbuffer, 48, 0, 0);

            t4cfunc::ket_transform<2, 2>(skbuffer, 25, ckbuffer, 228, 0, 1);

            t4cfunc::ket_transform<2, 2>(skbuffer, 22225, ckbuffer, 1452, 0, 1);

            t4cfunc::ket_transform<2, 2>(skbuffer, 22300, ckbuffer, 1848, 0, 2);

            t4cfunc::ket_transform<2, 2>(skbuffer, 22450, ckbuffer, 2544, 0, 3);

            t4cfunc::ket_transform<2, 2>(skbuffer, 22700, ckbuffer, 2952, 0, 0);

            t4cfunc::ket_transform<2, 2>(skbuffer, 22725, ckbuffer, 3132, 0, 1);

            t4cfunc::ket_transform<2, 2>(skbuffer, 23025, ckbuffer, 3852, 0, 2);

            t4cfunc::ket_transform<2, 2>(skbuffer, 23625, ckbuffer, 5196, 0, 3);

            t4cfunc::ket_transform<2, 2>(skbuffer, 26500, ckbuffer, 8400, 0, 1);

            t4cfunc::ket_transform<2, 2>(skbuffer, 26575, ckbuffer, 8796, 0, 2);

            t4cfunc::ket_transform<2, 2>(skbuffer, 26725, ckbuffer, 9492, 0, 3);

            t4cfunc::ket_transform<2, 2>(skbuffer, 26975, ckbuffer, 10572, 0, 4);

            t4cfunc::ket_transform<2, 2>(skbuffer, 27350, ckbuffer, 12120, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 5050, 0, 25, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 25750, 22700, 22725, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 25825, 22725, 23025, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 26050, 23025, 23625, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 27875, 26500, 26575, r_ab, 2, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_psxx(skbuffer, 5350, 0, 25750, 25825, r_ab, 2, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 6925, 25, 25825, 26050, r_ab, 2, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dsxx(skbuffer, 14125, 5050, 5350, 6925, r_ab, 2, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 100, 0, 22300, 2, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 1000, 25, 22450, 2, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_psxx(skbuffer, 5125, 0, 22225, 100, r_ab, 2, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_ppxx(skbuffer, 6250, 25, 100, 1000, r_ab, 2, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_dsxx(skbuffer, 13675, 5050, 5125, 6250, r_ab, 2, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 22800, 22700, 26575, 2, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 23175, 22725, 26725, 2, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 23875, 23025, 26975, 2, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 24625, 23625, 27350, 2, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 325, 22725, 22800, 23175, r_ab, 2, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 1450, 23025, 23175, 23875, r_ab, 2, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 2800, 23625, 23875, 24625, r_ab, 2, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_psxx(skbuffer, 5575, 22225, 25750, 27875, 325, r_ab, 2, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ppxx(skbuffer, 7600, 100, 25825, 325, 1450, r_ab, 2, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pdxx(skbuffer, 9625, 1000, 26050, 1450, 2800, r_ab, 2, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dsxx(skbuffer, 14575, 5125, 5350, 5575, 7600, r_ab, 2, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dpxx(skbuffer, 15925, 6250, 6925, 7600, 9625, r_ab, 2, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_fsxx(skbuffer, 19975, 13675, 14125, 14575, 15925, r_ab, 2, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 0, skbuffer, 19975, 2, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 175, skbuffer, 20225, 2, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 350, skbuffer, 20475, 2, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 525, skbuffer, 20725, 2, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 700, skbuffer, 20975, 2, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 875, skbuffer, 21225, 2, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 1050, skbuffer, 21475, 2, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 1225, skbuffer, 21725, 2, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 1400, skbuffer, 21975, 2, 2);
...
#endif /* ElectronRepulsionGeom1100RecFSDD_hpp */
