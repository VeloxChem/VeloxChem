#ifndef ElectronRepulsionGeom1100RecDFPD_hpp
#define ElectronRepulsionGeom1100RecDFPD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(DF|1/|r-r'||PD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_dfpd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(9018, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(35775, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 694, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 36, pbuffer, 910, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 96, pbuffer, 1420, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 156, pbuffer, 1720, 100, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 256, pbuffer, 2385, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 346, pbuffer, 2745, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 496, pbuffer, 3492, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 622, pbuffer, 3870, 210, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 832, pbuffer, 694, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 868, pbuffer, 910, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 928, pbuffer, 1420, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 988, pbuffer, 1720, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1088, pbuffer, 2385, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1178, pbuffer, 2745, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1328, pbuffer, 3492, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1454, pbuffer, 3870, 210, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 1664, pbuffer, 2385, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1754, pbuffer, 2745, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1904, pbuffer, 3492, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2030, pbuffer, 3870, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2240, pbuffer, 4584, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2408, pbuffer, 4920, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2688, pbuffer, 5480, 216, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2904, pbuffer, 5696, 360, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1638, cbuffer, 256, 346, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1908, cbuffer, 496, 622, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 2286, cbuffer, 832, 868, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 2394, cbuffer, 928, 988, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 3114, cbuffer, 1088, 1178, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 4194, cbuffer, 1328, 1454, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 7218, cbuffer, 1664, 1754, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 7488, cbuffer, 1904, 2030, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 7866, cbuffer, 2240, 2408, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 8370, cbuffer, 2688, 2904, cfactors, 6, 0, 7);

            t4cfunc::ket_transform<1, 2>(skbuffer, 0, ckbuffer, 0, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 90, ckbuffer, 108, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 28500, ckbuffer, 1638, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 28725, ckbuffer, 1908, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 29040, ckbuffer, 2286, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 29130, ckbuffer, 2394, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 29730, ckbuffer, 3114, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 30630, ckbuffer, 4194, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 34275, ckbuffer, 7218, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 34500, ckbuffer, 7488, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 34815, ckbuffer, 7866, 0, 6);

            t4cfunc::ket_transform<1, 2>(skbuffer, 35235, ckbuffer, 8370, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 33150, 29130, 29730, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 33600, 29730, 30630, r_ab, 1, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 8925, 90, 33150, 33600, r_ab, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 240, 0, 28500, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 2040, 90, 28725, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_pfxx(skbuffer, 7575, 90, 240, 2040, r_ab, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 29280, 29040, 34275, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 29955, 29130, 34500, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_shxx(skbuffer, 30945, 29730, 34815, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sixx(skbuffer, 31890, 30630, 35235, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 690, 29130, 29280, 29955, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sgxx(skbuffer, 2715, 29730, 29955, 30945, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_shxx(skbuffer, 4740, 30630, 30945, 31890, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pfxx(skbuffer, 10275, 240, 33150, 690, 2715, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pgxx(skbuffer, 14325, 2040, 33600, 2715, 4740, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dfxx(skbuffer, 20400, 7575, 8925, 10275, 14325, r_ab, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 20400, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 525, skbuffer, 21300, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1050, skbuffer, 22200, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1575, skbuffer, 23100, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 2100, skbuffer, 24000, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 2625, skbuffer, 24900, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 3150, skbuffer, 25800, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 3675, skbuffer, 26700, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 4200, skbuffer, 27600, 1, 2);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 2, 3, 1, 2, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1100RecDFPD_hpp */
