#ifndef ElectronRepulsionGeom1100RecSFDD_hpp
#define ElectronRepulsionGeom1100RecSFDD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SF|1/|r-r'||DD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_sfdd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(1612, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(7068, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(5425, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 496, pbuffer, 2610, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 586, pbuffer, 2790, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 736, pbuffer, 3090, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 961, pbuffer, 3540, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1087, pbuffer, 3666, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1297, pbuffer, 3876, 315, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<2, 2>(skbuffer, 2250, ckbuffer, 288, 0, 2);

            t4cfunc::ket_transform<2, 2>(skbuffer, 2400, ckbuffer, 984, 0, 3);

            t4cfunc::ket_transform<2, 2>(skbuffer, 4525, ckbuffer, 4764, 0, 4);

            t4cfunc::ket_transform<2, 2>(skbuffer, 4900, ckbuffer, 6312, 0, 5);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 2650, 2250, 4525, 2, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 3400, 2400, 4900, 2, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 0, 2400, 2650, 3400, r_ab, 2, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 0, skbuffer, 0, 2, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 175, skbuffer, 250, 2, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 350, skbuffer, 500, 2, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 525, skbuffer, 750, 2, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 700, skbuffer, 1000, 2, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 875, skbuffer, 1250, 2, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 1050, skbuffer, 1500, 2, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 1225, skbuffer, 1750, 2, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 1400, skbuffer, 2000, 2, 2);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 3, 2, 2, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1100RecSFDD_hpp */
