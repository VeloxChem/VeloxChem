#ifndef ElectronRepulsionGeom1100RecSFSD_hpp
#define ElectronRepulsionGeom1100RecSFSD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SF|1/|r-r'||SD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_sfsd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(312, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(1085, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 266, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 36, pbuffer, 480, 60, ket_width, ket_npgtos);

                pbuffer.scale(4.0 * a_exp * b_exp, {705, 795});

                pbuffer.scale(4.0 * a_exp * b_exp, {885, 1011});

                t2cfunc::reduce(cbuffer, 96, pbuffer, 705, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 186, pbuffer, 885, 126, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 2>(skbuffer, 450, cbuffer, 0, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 480, cbuffer, 36, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 905, cbuffer, 96, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 980, cbuffer, 186, 0, 5);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 530, 450, 905, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 680, 480, 980, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 0, 480, 530, 680, r_ab, 0, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 0, skbuffer, 0, 0, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 35, skbuffer, 50, 0, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 70, skbuffer, 100, 0, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 105, skbuffer, 150, 0, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 140, skbuffer, 200, 0, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 175, skbuffer, 250, 0, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 210, skbuffer, 300, 0, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 245, skbuffer, 350, 0, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 280, skbuffer, 400, 0, 2);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 3, 0, 2, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1100RecSFSD_hpp */
