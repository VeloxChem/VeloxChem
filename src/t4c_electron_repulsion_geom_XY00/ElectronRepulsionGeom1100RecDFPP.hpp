#ifndef ElectronRepulsionGeom1100RecDFPP_hpp
#define ElectronRepulsionGeom1100RecDFPP_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(DF|1/|r-r'||PP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_dfpp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(4509, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(21465, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 144, pbuffer, 1181, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 189, pbuffer, 1361, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 279, pbuffer, 1763, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 342, pbuffer, 1952, 126, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 468, pbuffer, 322, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 486, pbuffer, 430, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 522, pbuffer, 686, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 552, pbuffer, 836, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 612, pbuffer, 1181, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 657, pbuffer, 1361, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 747, pbuffer, 1763, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 810, pbuffer, 1952, 126, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 936, pbuffer, 1181, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 981, pbuffer, 1361, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1071, pbuffer, 1763, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1134, pbuffer, 1952, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1260, pbuffer, 2358, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1344, pbuffer, 2526, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1512, pbuffer, 2862, 108, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1620, pbuffer, 2970, 216, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 1143, cbuffer, 468, 486, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 1197, cbuffer, 522, 552, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 1557, cbuffer, 612, 657, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 2097, cbuffer, 747, 810, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 3609, cbuffer, 936, 981, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 3744, cbuffer, 1071, 1134, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 3933, cbuffer, 1260, 1344, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 4185, cbuffer, 1512, 1620, cfactors, 6, 0, 7);

            t4cfunc::ket_transform<1, 1>(skbuffer, 0, ckbuffer, 0, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 54, ckbuffer, 54, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 17100, ckbuffer, 819, 0, 4);

            t4cfunc::ket_transform<1, 1>(skbuffer, 17235, ckbuffer, 954, 0, 5);

            t4cfunc::ket_transform<1, 1>(skbuffer, 17424, ckbuffer, 1143, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 17478, ckbuffer, 1197, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 17838, ckbuffer, 1557, 0, 4);

            t4cfunc::ket_transform<1, 1>(skbuffer, 18378, ckbuffer, 2097, 0, 5);

            t4cfunc::ket_transform<1, 1>(skbuffer, 20565, ckbuffer, 3609, 0, 4);

            t4cfunc::ket_transform<1, 1>(skbuffer, 20700, ckbuffer, 3744, 0, 5);

            t4cfunc::ket_transform<1, 1>(skbuffer, 20889, ckbuffer, 3933, 0, 6);

            t4cfunc::ket_transform<1, 1>(skbuffer, 21141, ckbuffer, 4185, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 19890, 17478, 17838, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 20160, 17838, 18378, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 5355, 54, 19890, 20160, r_ab, 1, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 144, 0, 17100, 1, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 1224, 54, 17235, 1, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_pfxx(skbuffer, 4545, 54, 144, 1224, r_ab, 1, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 17568, 17424, 20565, 1, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 17973, 17478, 20700, 1, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_shxx(skbuffer, 18567, 17838, 20889, 1, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sixx(skbuffer, 19134, 18378, 21141, 1, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 414, 17478, 17568, 17973, r_ab, 1, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sgxx(skbuffer, 1629, 17838, 17973, 18567, r_ab, 1, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_shxx(skbuffer, 2844, 18378, 18567, 19134, r_ab, 1, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pfxx(skbuffer, 6165, 144, 19890, 414, 1629, r_ab, 1, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pgxx(skbuffer, 8595, 1224, 20160, 1629, 2844, r_ab, 1, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dfxx(skbuffer, 12240, 4545, 5355, 6165, 8595, r_ab, 1, 1);

            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 12240, 1, 1);

            t4cfunc::bra_transform<2, 3>(sbuffer, 315, skbuffer, 12780, 1, 1);

            t4cfunc::bra_transform<2, 3>(sbuffer, 630, skbuffer, 13320, 1, 1);

            t4cfunc::bra_transform<2, 3>(sbuffer, 945, skbuffer, 13860, 1, 1);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1260, skbuffer, 14400, 1, 1);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1575, skbuffer, 14940, 1, 1);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1890, skbuffer, 15480, 1, 1);

            t4cfunc::bra_transform<2, 3>(sbuffer, 2205, skbuffer, 16020, 1, 1);

            t4cfunc::bra_transform<2, 3>(sbuffer, 2520, skbuffer, 16560, 1, 1);
...
#endif /* ElectronRepulsionGeom1100RecDFPP_hpp */
