#ifndef ElectronRepulsionGeom1100RecSSPF_hpp
#define ElectronRepulsionGeom1100RecSSPF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SS|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_sspf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(4.0 * a_exp * b_exp, {158, 188});

                pbuffer.scale(4.0 * a_exp * b_exp, {218, 263});

                pbuffer.scale(4.0 * a_exp * b_exp, {308, 368});

                pbuffer.scale(4.0 * a_exp * b_exp, {368, 458});

                t2cfunc::reduce(cbuffer, 0, pbuffer, 158, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 30, pbuffer, 218, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 75, pbuffer, 308, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 135, pbuffer, 368, 90, ket_width, ket_npgtos);
            ...
            t4cfunc::bra_transform<0, 0>(sbuffer, 0, skbuffer, 189, 1, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 21, skbuffer, 210, 1, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 42, skbuffer, 231, 1, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 63, skbuffer, 252, 1, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 84, skbuffer, 273, 1, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 105, skbuffer, 294, 1, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 126, skbuffer, 315, 1, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 147, skbuffer, 336, 1, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 168, skbuffer, 357, 1, 3);
...
#endif /* ElectronRepulsionGeom1100RecSSPF_hpp */
