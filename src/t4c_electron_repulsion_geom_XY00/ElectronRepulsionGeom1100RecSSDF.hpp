#ifndef ElectronRepulsionGeom1100RecSSDF_hpp
#define ElectronRepulsionGeom1100RecSSDF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SS|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_ssdf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(414, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(1215, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(630, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(4.0 * a_exp * b_exp, {256, 286});

                pbuffer.scale(4.0 * a_exp * b_exp, {316, 361});

                pbuffer.scale(4.0 * a_exp * b_exp, {406, 469});

                pbuffer.scale(4.0 * a_exp * b_exp, {532, 592});

                pbuffer.scale(4.0 * a_exp * b_exp, {592, 682});

                pbuffer.scale(4.0 * a_exp * b_exp, {682, 808});

                t2cfunc::reduce(cbuffer, 0, pbuffer, 256, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 30, pbuffer, 316, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 75, pbuffer, 406, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 138, pbuffer, 532, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 198, pbuffer, 592, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 288, pbuffer, 682, 126, ket_width, ket_npgtos);
            ...
            t4cfunc::bra_transform<0, 0>(sbuffer, 0, skbuffer, 315, 2, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 35, skbuffer, 350, 2, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 70, skbuffer, 385, 2, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 105, skbuffer, 420, 2, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 140, skbuffer, 455, 2, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 175, skbuffer, 490, 2, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 210, skbuffer, 525, 2, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 245, skbuffer, 560, 2, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 280, skbuffer, 595, 2, 3);
...
#endif /* ElectronRepulsionGeom1100RecSSDF_hpp */
