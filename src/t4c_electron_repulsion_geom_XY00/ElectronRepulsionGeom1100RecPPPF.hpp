#ifndef ElectronRepulsionGeom1100RecPPPF_hpp
#define ElectronRepulsionGeom1100RecPPPF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(PP|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_pppf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(3420, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(5985, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 25, pbuffer, 675, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 85, pbuffer, 855, 90, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 175, pbuffer, 75, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 185, pbuffer, 135, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 200, pbuffer, 285, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 230, pbuffer, 405, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 275, pbuffer, 675, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 335, pbuffer, 855, 90, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 425, pbuffer, 675, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 485, pbuffer, 855, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 575, pbuffer, 1185, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 675, pbuffer, 1385, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 825, pbuffer, 1685, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 975, pbuffer, 1835, 225, ket_width, ket_npgtos);

            }

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 0, cbuffer, 0, 10, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 300, cbuffer, 25, 85, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 480, cbuffer, 175, 185, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 510, cbuffer, 200, 230, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 870, cbuffer, 275, 335, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2490, cbuffer, 425, 485, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2670, cbuffer, 575, 675, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2970, cbuffer, 825, 975, cfactors, 6, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 0, ckbuffer, 0, 0, 0);

            t4cfunc::ket_transform<1, 3>(skbuffer, 3612, ckbuffer, 300, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 3738, ckbuffer, 480, 0, 0);

            t4cfunc::ket_transform<1, 3>(skbuffer, 3759, ckbuffer, 510, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 4011, ckbuffer, 870, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 5334, ckbuffer, 2490, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 5460, ckbuffer, 2670, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 5670, ckbuffer, 2970, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 5145, 3759, 4011, r_ab, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 21, 0, 3612, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 3822, 3738, 5334, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 4137, 3759, 5460, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 4515, 4011, 5670, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 210, 3759, 3822, 4137, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 777, 4011, 4137, 4515, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ppxx(skbuffer, 1911, 21, 5145, 210, 777, r_ab, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 1911, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 189, skbuffer, 2100, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 378, skbuffer, 2289, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 567, skbuffer, 2478, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 756, skbuffer, 2667, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 945, skbuffer, 2856, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 1134, skbuffer, 3045, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 1323, skbuffer, 3234, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 1512, skbuffer, 3423, 1, 3);
...
#endif /* ElectronRepulsionGeom1100RecPPPF_hpp */
