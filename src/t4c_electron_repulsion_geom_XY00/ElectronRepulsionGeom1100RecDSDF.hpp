#ifndef ElectronRepulsionGeom1100RecDSDF_hpp
#define ElectronRepulsionGeom1100RecDSDF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(DS|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_dsdf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(2484, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(11250, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(14070, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * b_exp, {425, 455});

                pbuffer.scale(2.0 * b_exp, {545, 590});

                pbuffer.scale(2.0 * b_exp, {725, 788});
                ...
                pbuffer.scale(a_exp / b_exp, {425, 455});

                pbuffer.scale(a_exp / b_exp, {545, 590});

                pbuffer.scale(a_exp / b_exp, {725, 788});
                ...
                t2cfunc::reduce(cbuffer, 920, pbuffer, 425, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 950, pbuffer, 545, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 995, pbuffer, 725, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1058, pbuffer, 1067, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1118, pbuffer, 1247, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1208, pbuffer, 1517, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1334, pbuffer, 1955, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1434, pbuffer, 2155, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1584, pbuffer, 2455, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1794, pbuffer, 2875, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1944, pbuffer, 3025, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2169, pbuffer, 3250, 315, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 675, cbuffer, 46, 76, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 765, cbuffer, 76, 121, cfactors, 6, 0, 1);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1890, cbuffer, 460, 470, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 1920, cbuffer, 470, 485, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 1965, 1890, 1920, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2025, cbuffer, 506, 536, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 2115, cbuffer, 536, 581, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 2250, 2025, 2115, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2970, cbuffer, 644, 704, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 3150, cbuffer, 704, 794, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 3420, 2970, 3150, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 6660, cbuffer, 920, 950, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 6750, cbuffer, 950, 995, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 6885, 6660, 6750, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 7065, cbuffer, 1058, 1118, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 7245, cbuffer, 1118, 1208, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 7515, 7065, 7245, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 7875, cbuffer, 1334, 1434, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 8175, cbuffer, 1434, 1584, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 8625, 7875, 8175, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 9225, cbuffer, 1794, 1944, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 9675, cbuffer, 1944, 2169, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 10350, 9225, 9675, cfactors, 6, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 0, ckbuffer, 75, 0, 0);

            t4cfunc::ket_transform<2, 3>(skbuffer, 9485, ckbuffer, 900, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 9590, ckbuffer, 1530, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 9800, ckbuffer, 1965, 0, 0);

            t4cfunc::ket_transform<2, 3>(skbuffer, 9835, ckbuffer, 2250, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 10255, ckbuffer, 3420, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 12565, ckbuffer, 6885, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 12670, ckbuffer, 7515, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 12880, ckbuffer, 8625, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 13230, ckbuffer, 10350, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 12145, 9800, 9835, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 12250, 9835, 10255, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 13755, 12565, 12670, r_ab, 2, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_psxx(skbuffer, 3500, 0, 12145, 12250, r_ab, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 35, 0, 9590, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_psxx(skbuffer, 3185, 0, 9485, 35, r_ab, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 9940, 9800, 12670, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 10465, 9835, 12880, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 11095, 10255, 13230, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 350, 9835, 9940, 10465, r_ab, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 1295, 10255, 10465, 11095, r_ab, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_psxx(skbuffer, 3815, 9485, 12145, 13755, 350, r_ab, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ppxx(skbuffer, 4760, 35, 12250, 350, 1295, r_ab, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dsxx(skbuffer, 7595, 3185, 3500, 3815, 4760, r_ab, 2, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 0, skbuffer, 7595, 2, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 175, skbuffer, 7805, 2, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 350, skbuffer, 8015, 2, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 525, skbuffer, 8225, 2, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 700, skbuffer, 8435, 2, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 875, skbuffer, 8645, 2, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 1050, skbuffer, 8855, 2, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 1225, skbuffer, 9065, 2, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 1400, skbuffer, 9275, 2, 3);
...
#endif /* ElectronRepulsionGeom1100RecDSDF_hpp */
