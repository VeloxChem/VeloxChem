#ifndef ElectronRepulsionGeom1100RecPFSP_hpp
#define ElectronRepulsionGeom1100RecPFSP_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(PF|1/|r-r'||SP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_pfsp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(348, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(2427, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 18, pbuffer, 392, 45, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 63, pbuffer, 122, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 81, pbuffer, 242, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 111, pbuffer, 392, 45, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 156, pbuffer, 392, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 201, pbuffer, 548, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 264, pbuffer, 674, 84, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 1>(skbuffer, 0, cbuffer, 0, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1593, cbuffer, 18, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1638, cbuffer, 63, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1656, cbuffer, 81, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1776, cbuffer, 111, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2235, cbuffer, 156, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2280, cbuffer, 201, 0, 5);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2343, cbuffer, 264, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 2145, 1656, 1776, r_ab, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 18, 0, 1593, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 1686, 1638, 2235, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 1821, 1656, 2280, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_shxx(skbuffer, 1956, 1776, 2343, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 108, 1656, 1686, 1821, r_ab, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sgxx(skbuffer, 378, 1776, 1821, 1956, r_ab, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pfxx(skbuffer, 783, 18, 2145, 108, 378, r_ab, 0, 1);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 783, 0, 1);

            t4cfunc::bra_transform<1, 3>(sbuffer, 63, skbuffer, 873, 0, 1);

            t4cfunc::bra_transform<1, 3>(sbuffer, 126, skbuffer, 963, 0, 1);

            t4cfunc::bra_transform<1, 3>(sbuffer, 189, skbuffer, 1053, 0, 1);

            t4cfunc::bra_transform<1, 3>(sbuffer, 252, skbuffer, 1143, 0, 1);

            t4cfunc::bra_transform<1, 3>(sbuffer, 315, skbuffer, 1233, 0, 1);

            t4cfunc::bra_transform<1, 3>(sbuffer, 378, skbuffer, 1323, 0, 1);

            t4cfunc::bra_transform<1, 3>(sbuffer, 441, skbuffer, 1413, 0, 1);

            t4cfunc::bra_transform<1, 3>(sbuffer, 504, skbuffer, 1503, 0, 1);
...
#endif /* ElectronRepulsionGeom1100RecPFSP_hpp */
