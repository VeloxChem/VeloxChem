#ifndef ElectronRepulsionGeom1100RecSDFF_hpp
#define ElectronRepulsionGeom1100RecSDFF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SD|1/|r-r'||FF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_sdff(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(17992, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(6664, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 666, pbuffer, 2991, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 766, pbuffer, 3191, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 916, pbuffer, 3491, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1126, pbuffer, 3911, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1406, pbuffer, 4471, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1556, pbuffer, 4621, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1781, pbuffer, 4846, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2096, pbuffer, 5161, 420, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<3, 3>(skbuffer, 2646, ckbuffer, 864, 0, 1);

            t4cfunc::ket_transform<3, 3>(skbuffer, 2793, ckbuffer, 2892, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 5439, ckbuffer, 11172, 0, 3);

            t4cfunc::ket_transform<3, 3>(skbuffer, 5929, ckbuffer, 16492, 0, 4);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 3087, 2646, 5439, 3, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 3969, 2793, 5929, 3, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 0, 2793, 3087, 3969, r_ab, 3, 3);

            t4cfunc::bra_transform<0, 2>(sbuffer, 0, skbuffer, 0, 3, 3);

            t4cfunc::bra_transform<0, 2>(sbuffer, 245, skbuffer, 294, 3, 3);

            t4cfunc::bra_transform<0, 2>(sbuffer, 490, skbuffer, 588, 3, 3);

            t4cfunc::bra_transform<0, 2>(sbuffer, 735, skbuffer, 882, 3, 3);

            t4cfunc::bra_transform<0, 2>(sbuffer, 980, skbuffer, 1176, 3, 3);

            t4cfunc::bra_transform<0, 2>(sbuffer, 1225, skbuffer, 1470, 3, 3);

            t4cfunc::bra_transform<0, 2>(sbuffer, 1470, skbuffer, 1764, 3, 3);

            t4cfunc::bra_transform<0, 2>(sbuffer, 1715, skbuffer, 2058, 3, 3);

            t4cfunc::bra_transform<0, 2>(sbuffer, 1960, skbuffer, 2352, 3, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 2, 3, 3, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1100RecSDFF_hpp */
