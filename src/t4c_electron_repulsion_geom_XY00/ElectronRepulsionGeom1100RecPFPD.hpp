#ifndef ElectronRepulsionGeom1100RecPFPD_hpp
#define ElectronRepulsionGeom1100RecPFPD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(PF|1/|r-r'||PD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_pfpd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(1856, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(5112, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(12135, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 96, pbuffer, 1925, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 186, pbuffer, 2195, 150, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 336, pbuffer, 590, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 372, pbuffer, 770, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 432, pbuffer, 1180, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 492, pbuffer, 1420, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 592, pbuffer, 1925, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 682, pbuffer, 2195, 150, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 832, pbuffer, 1925, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 922, pbuffer, 2195, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1072, pbuffer, 2708, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1198, pbuffer, 2960, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1408, pbuffer, 3380, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1576, pbuffer, 3548, 280, ket_width, ket_npgtos);

            }

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 0, cbuffer, 0, 36, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 648, cbuffer, 96, 186, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 918, cbuffer, 336, 372, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1026, cbuffer, 432, 492, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1746, cbuffer, 592, 682, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 3960, cbuffer, 832, 922, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 4230, cbuffer, 1072, 1198, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 4608, cbuffer, 1408, 1576, cfactors, 6, 0, 6);

            t4cfunc::ket_transform<1, 2>(skbuffer, 0, ckbuffer, 0, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 7965, ckbuffer, 648, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 8190, ckbuffer, 918, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 8280, ckbuffer, 1026, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 8880, ckbuffer, 1746, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 11175, ckbuffer, 3960, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 11400, ckbuffer, 4230, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 11715, ckbuffer, 4608, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 10725, 8280, 8880, r_ab, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 90, 0, 7965, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 8430, 8190, 11175, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 9105, 8280, 11400, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_shxx(skbuffer, 9780, 8880, 11715, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 540, 8280, 8430, 9105, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sgxx(skbuffer, 1890, 8880, 9105, 9780, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pfxx(skbuffer, 3915, 90, 10725, 540, 1890, r_ab, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 3915, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 315, skbuffer, 4365, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 630, skbuffer, 4815, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 945, skbuffer, 5265, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1260, skbuffer, 5715, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1575, skbuffer, 6165, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1890, skbuffer, 6615, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 2205, skbuffer, 7065, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 2520, skbuffer, 7515, 1, 2);
...
#endif /* ElectronRepulsionGeom1100RecPFPD_hpp */
