#ifndef ElectronRepulsionGeom1100RecPDFF_hpp
#define ElectronRepulsionGeom1100RecPDFF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(PD|1/|r-r'||FF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_pdff(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(41364, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(25137, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 222, pbuffer, 3921, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 322, pbuffer, 4221, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 472, pbuffer, 4671, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 682, pbuffer, 5301, 280, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 962, pbuffer, 735, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 992, pbuffer, 885, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1037, pbuffer, 1110, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1100, pbuffer, 1425, 84, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 2368, pbuffer, 3921, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2468, pbuffer, 4221, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2618, pbuffer, 4671, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2828, pbuffer, 5301, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3108, pbuffer, 6231, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3258, pbuffer, 6531, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3483, pbuffer, 6981, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3798, pbuffer, 7611, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4218, pbuffer, 8451, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4428, pbuffer, 8661, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4743, pbuffer, 8976, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5184, pbuffer, 9417, 588, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2964, cbuffer, 222, 322, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 3264, cbuffer, 322, 472, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 3714, cbuffer, 472, 682, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 4344, 2964, 3264, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 4944, 3264, 3714, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 5844, 4344, 4944, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 6844, cbuffer, 962, 992, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 6934, cbuffer, 992, 1037, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 7069, cbuffer, 1037, 1100, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 7258, 6844, 6934, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 7438, 6934, 7069, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 7708, 7258, 7438, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 8008, cbuffer, 1184, 1244, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 8188, cbuffer, 1244, 1334, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 8458, cbuffer, 1334, 1460, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 8836, 8008, 8188, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 9196, 8188, 8458, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 9736, 8836, 9196, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 12136, cbuffer, 1628, 1728, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 12436, cbuffer, 1728, 1878, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 12886, cbuffer, 1878, 2088, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 13516, 12136, 12436, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 14116, 12436, 12886, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 15016, 13516, 14116, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 23516, cbuffer, 2368, 2468, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 23816, cbuffer, 2468, 2618, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 24266, cbuffer, 2618, 2828, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 24896, 23516, 23816, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 25496, 23816, 24266, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 26396, 24896, 25496, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 27396, cbuffer, 3108, 3258, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 27846, cbuffer, 3258, 3483, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 28521, cbuffer, 3483, 3798, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 29466, 27396, 27846, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 30366, 27846, 28521, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 31716, 29466, 30366, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 33216, cbuffer, 4218, 4428, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 33846, cbuffer, 4428, 4743, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 34791, cbuffer, 4743, 5184, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 36114, 33216, 33846, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 37374, 33846, 34791, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 39264, 36114, 37374, cfactors, 6, 0, 5);

            t4cfunc::ket_transform<3, 3>(skbuffer, 0, ckbuffer, 864, 0, 1);

            t4cfunc::ket_transform<3, 3>(skbuffer, 16023, ckbuffer, 5844, 0, 3);

            t4cfunc::ket_transform<3, 3>(skbuffer, 16513, ckbuffer, 7708, 0, 1);

            t4cfunc::ket_transform<3, 3>(skbuffer, 16660, ckbuffer, 9736, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 17836, ckbuffer, 15016, 0, 3);

            t4cfunc::ket_transform<3, 3>(skbuffer, 22883, ckbuffer, 26396, 0, 3);

            t4cfunc::ket_transform<3, 3>(skbuffer, 23373, ckbuffer, 31716, 0, 4);

            t4cfunc::ket_transform<3, 3>(skbuffer, 24108, ckbuffer, 39264, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 22001, 16660, 17836, r_ab, 3, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 147, 0, 16023, 3, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 16954, 16513, 22883, 3, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 18326, 16660, 23373, 3, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 19796, 17836, 24108, 3, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 1029, 16660, 16954, 18326, r_ab, 3, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 3675, 17836, 18326, 19796, r_ab, 3, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pdxx(skbuffer, 8085, 147, 22001, 1029, 3675, r_ab, 3, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 0, skbuffer, 8085, 3, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 735, skbuffer, 8967, 3, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 1470, skbuffer, 9849, 3, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 2205, skbuffer, 10731, 3, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 2940, skbuffer, 11613, 3, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 3675, skbuffer, 12495, 3, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 4410, skbuffer, 13377, 3, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 5145, skbuffer, 14259, 3, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 5880, skbuffer, 15141, 3, 3);
...
#endif /* ElectronRepulsionGeom1100RecPDFF_hpp */
