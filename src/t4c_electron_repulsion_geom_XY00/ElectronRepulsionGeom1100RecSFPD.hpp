#ifndef ElectronRepulsionGeom1100RecSFPD_hpp
#define ElectronRepulsionGeom1100RecSFPD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SF|1/|r-r'||PD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_sfpd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(832, 1);
    ...
    CSimdArray<double> skbuffer(3255, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 256, pbuffer, 1465, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 346, pbuffer, 1645, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 496, pbuffer, 1945, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 622, pbuffer, 2071, 210, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<1, 2>(skbuffer, 1350, ckbuffer, 0, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 1440, ckbuffer, 108, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 2715, ckbuffer, 1638, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 2940, ckbuffer, 1908, 0, 5);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 1590, 1350, 2715, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 2040, 1440, 2940, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 0, 1440, 1590, 2040, r_ab, 1, 2);
...
#endif /* ElectronRepulsionGeom1100RecSFPD_hpp */
