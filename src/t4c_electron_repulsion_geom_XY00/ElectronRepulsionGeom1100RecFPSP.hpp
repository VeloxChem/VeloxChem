#ifndef ElectronRepulsionGeom1100RecFPSP_hpp
#define ElectronRepulsionGeom1100RecFPSP_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(FP|1/|r-r'||SP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_fpsp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(468, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(6867, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 3, pbuffer, 44, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 12, pbuffer, 122, 18, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * b_exp, {122, 140});

                pbuffer.scale(2.0 * b_exp, {242, 272});

                pbuffer.scale(2.0 * b_exp, {392, 437});

                t2cfunc::reduce(cbuffer, 30, pbuffer, 122, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 48, pbuffer, 242, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 78, pbuffer, 392, 45, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {8, 11});

                pbuffer.scale(2.0 * a_exp, {44, 53});

                pbuffer.scale(a_exp / b_exp, {122, 140});

                pbuffer.scale(a_exp / b_exp, {242, 272});

                pbuffer.scale(a_exp / b_exp, {392, 437});

                t2cfunc::reduce(cbuffer, 123, pbuffer, 8, 3, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 126, pbuffer, 44, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 135, pbuffer, 122, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 153, pbuffer, 242, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 183, pbuffer, 392, 45, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 228, pbuffer, 122, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 246, pbuffer, 242, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 276, pbuffer, 392, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 321, pbuffer, 548, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 384, pbuffer, 674, 84, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<0, 1>(skbuffer, 120, cbuffer, 12, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5763, cbuffer, 30, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5781, cbuffer, 48, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5811, cbuffer, 78, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5856, cbuffer, 123, 0, 0);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5859, cbuffer, 126, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5895, cbuffer, 135, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5967, cbuffer, 153, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 6087, cbuffer, 183, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 6627, cbuffer, 228, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 6645, cbuffer, 246, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 6675, cbuffer, 276, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 6720, cbuffer, 321, 0, 5);

            t4cfunc::ket_transform<0, 1>(skbuffer, 6783, cbuffer, 384, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 1119, 3, 120, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 6456, 5859, 5895, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 6483, 5895, 5967, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 6537, 5967, 6087, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 1227, 3, 6456, 6483, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 1713, 120, 6483, 6537, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dpxx(skbuffer, 3333, 1119, 1227, 1713, r_ab, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 12, 0, 5763, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 138, 3, 5781, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 354, 120, 5811, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_ppxx(skbuffer, 1146, 3, 12, 138, r_ab, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_pdxx(skbuffer, 1551, 120, 138, 354, r_ab, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_dpxx(skbuffer, 3171, 1119, 1146, 1551, r_ab, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 5868, 5856, 6627, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 5913, 5859, 6645, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 5997, 5895, 6675, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 6132, 5967, 6720, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_shxx(skbuffer, 6267, 6087, 6783, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 39, 5859, 5868, 5913, r_ab, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 192, 5895, 5913, 5997, r_ab, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 444, 5967, 5997, 6132, r_ab, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sgxx(skbuffer, 714, 6087, 6132, 6267, r_ab, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ppxx(skbuffer, 1308, 12, 6456, 39, 192, r_ab, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pdxx(skbuffer, 1875, 138, 6483, 192, 444, r_ab, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pfxx(skbuffer, 2361, 354, 6537, 444, 714, r_ab, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dpxx(skbuffer, 3495, 1146, 1227, 1308, 1875, r_ab, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ddxx(skbuffer, 3981, 1551, 1713, 1875, 2361, r_ab, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_fpxx(skbuffer, 4953, 3171, 3333, 3495, 3981, r_ab, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 0, skbuffer, 4953, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 63, skbuffer, 5043, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 126, skbuffer, 5133, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 189, skbuffer, 5223, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 252, skbuffer, 5313, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 315, skbuffer, 5403, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 378, skbuffer, 5493, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 441, skbuffer, 5583, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 504, skbuffer, 5673, 0, 1);
...
#endif /* ElectronRepulsionGeom1100RecFPSP_hpp */
