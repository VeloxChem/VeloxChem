#ifndef ElectronRepulsionGeom1100RecPSFF_hpp
#define ElectronRepulsionGeom1100RecPSFF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(PS|1/|r-r'||FF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_psff(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(1924, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(12788, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(5831, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * b_exp, {507, 537});

                pbuffer.scale(2.0 * b_exp, {597, 642});

                pbuffer.scale(2.0 * b_exp, {732, 795});

                pbuffer.scale(2.0 * b_exp, {921, 1005});
                ...
                pbuffer.scale(a_exp / b_exp, {507, 537});

                pbuffer.scale(a_exp / b_exp, {597, 642});

                pbuffer.scale(a_exp / b_exp, {732, 795});

                pbuffer.scale(a_exp / b_exp, {921, 1005});

                t2cfunc::reduce(cbuffer, 222, pbuffer, 85, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 232, pbuffer, 155, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 247, pbuffer, 245, 21, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 268, pbuffer, 350, 28, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 296, pbuffer, 507, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 326, pbuffer, 597, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 371, pbuffer, 732, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 434, pbuffer, 921, 84, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 518, pbuffer, 507, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 548, pbuffer, 597, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 593, pbuffer, 732, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 656, pbuffer, 921, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 740, pbuffer, 1209, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 800, pbuffer, 1329, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 890, pbuffer, 1509, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1016, pbuffer, 1761, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1184, pbuffer, 2097, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1284, pbuffer, 2197, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1434, pbuffer, 2347, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1644, pbuffer, 2557, 280, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1164, cbuffer, 222, 232, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 1194, cbuffer, 232, 247, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 1239, cbuffer, 247, 268, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 1302, 1164, 1194, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 1362, 1194, 1239, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 1452, 1302, 1362, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1552, cbuffer, 296, 326, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 1642, cbuffer, 326, 371, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 1777, cbuffer, 371, 434, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 1966, 1552, 1642, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 2146, 1642, 1777, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 2416, 1966, 2146, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 5416, cbuffer, 518, 548, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 5506, cbuffer, 548, 593, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 5641, cbuffer, 593, 656, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 5830, 5416, 5506, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 6010, 5506, 5641, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 6280, 5830, 6010, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 6580, cbuffer, 740, 800, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 6760, cbuffer, 800, 890, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 7030, cbuffer, 890, 1016, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 7408, 6580, 6760, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 7768, 6760, 7030, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 8308, 7408, 7768, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 8908, cbuffer, 1184, 1284, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 9208, cbuffer, 1284, 1434, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 9658, cbuffer, 1434, 1644, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 10288, 8908, 9208, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 10888, 9208, 9658, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 11788, 10288, 10888, cfactors, 6, 0, 3);

            t4cfunc::ket_transform<3, 3>(skbuffer, 2646, ckbuffer, 864, 0, 1);

            t4cfunc::ket_transform<3, 3>(skbuffer, 2793, ckbuffer, 1452, 0, 0);

            t4cfunc::ket_transform<3, 3>(skbuffer, 2842, ckbuffer, 2416, 0, 1);

            t4cfunc::ket_transform<3, 3>(skbuffer, 4459, ckbuffer, 6280, 0, 1);

            t4cfunc::ket_transform<3, 3>(skbuffer, 4606, ckbuffer, 8308, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 4900, ckbuffer, 11788, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 4312, 2793, 2842, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 5390, 4459, 4606, r_ab, 3, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 2989, 2793, 4606, 3, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 3430, 2842, 4900, 3, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 0, 2842, 2989, 3430, r_ab, 3, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_psxx(skbuffer, 1323, 2646, 4312, 5390, 0, r_ab, 3, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 0, skbuffer, 1323, 3, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 147, skbuffer, 1470, 3, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 294, skbuffer, 1617, 3, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 441, skbuffer, 1764, 3, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 588, skbuffer, 1911, 3, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 735, skbuffer, 2058, 3, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 882, skbuffer, 2205, 3, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 1029, skbuffer, 2352, 3, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 1176, skbuffer, 2499, 3, 3);
...
#endif /* ElectronRepulsionGeom1100RecPSFF_hpp */
