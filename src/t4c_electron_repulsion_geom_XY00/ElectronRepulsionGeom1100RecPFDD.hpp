#ifndef ElectronRepulsionGeom1100RecPFDD_hpp
#define ElectronRepulsionGeom1100RecPFDD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(PF|1/|r-r'||DD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_pfdd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(3596, 1);
    ...
    CSimdArray<double> skbuffer(20225, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 186, pbuffer, 3370, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 276, pbuffer, 3640, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 426, pbuffer, 4090, 225, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 651, pbuffer, 985, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 687, pbuffer, 1165, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 747, pbuffer, 1465, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 837, pbuffer, 2025, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 897, pbuffer, 2265, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 997, pbuffer, 2665, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1147, pbuffer, 3370, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1237, pbuffer, 3640, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1387, pbuffer, 4090, 225, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 1612, pbuffer, 3370, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1702, pbuffer, 3640, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1852, pbuffer, 4090, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2077, pbuffer, 4828, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2203, pbuffer, 5080, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2413, pbuffer, 5500, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2728, pbuffer, 6130, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2896, pbuffer, 6298, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3176, pbuffer, 6578, 420, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1584, cbuffer, 186, 276, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1854, cbuffer, 276, 426, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 2304, 1584, 1854, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 2844, cbuffer, 651, 687, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2952, cbuffer, 687, 747, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 3132, 2844, 2952, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 3348, cbuffer, 837, 897, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 3528, cbuffer, 897, 997, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 3828, 3348, 3528, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 5268, cbuffer, 1147, 1237, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 5538, cbuffer, 1237, 1387, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 5988, 5268, 5538, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 10416, cbuffer, 1612, 1702, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 10686, cbuffer, 1702, 1852, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 11136, 10416, 10686, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 11676, cbuffer, 2077, 2203, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 12054, cbuffer, 2203, 2413, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 12684, 11676, 12054, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 13440, cbuffer, 2728, 2896, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 13944, cbuffer, 2896, 3176, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 14784, 13440, 13944, cfactors, 6, 0, 6);

            t4cfunc::ket_transform<2, 2>(skbuffer, 0, ckbuffer, 288, 0, 2);

            t4cfunc::ket_transform<2, 2>(skbuffer, 13275, ckbuffer, 2304, 0, 4);

            t4cfunc::ket_transform<2, 2>(skbuffer, 13650, ckbuffer, 3132, 0, 2);

            t4cfunc::ket_transform<2, 2>(skbuffer, 13800, ckbuffer, 3828, 0, 3);

            t4cfunc::ket_transform<2, 2>(skbuffer, 14800, ckbuffer, 5988, 0, 4);

            t4cfunc::ket_transform<2, 2>(skbuffer, 18625, ckbuffer, 11136, 0, 4);

            t4cfunc::ket_transform<2, 2>(skbuffer, 19000, ckbuffer, 12684, 0, 5);

            t4cfunc::ket_transform<2, 2>(skbuffer, 19525, ckbuffer, 14784, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 17875, 13800, 14800, r_ab, 2, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 150, 0, 13275, 2, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 14050, 13650, 18625, 2, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 15175, 13800, 19000, 2, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_shxx(skbuffer, 16300, 14800, 19525, 2, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 900, 13800, 14050, 15175, r_ab, 2, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sgxx(skbuffer, 3150, 14800, 15175, 16300, r_ab, 2, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pfxx(skbuffer, 6525, 150, 17875, 900, 3150, r_ab, 2, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 6525, 2, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 525, skbuffer, 7275, 2, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1050, skbuffer, 8025, 2, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1575, skbuffer, 8775, 2, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 2100, skbuffer, 9525, 2, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 2625, skbuffer, 10275, 2, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 3150, skbuffer, 11025, 2, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 3675, skbuffer, 11775, 2, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 4200, skbuffer, 12525, 2, 2);
...
#endif /* ElectronRepulsionGeom1100RecPFDD_hpp */
