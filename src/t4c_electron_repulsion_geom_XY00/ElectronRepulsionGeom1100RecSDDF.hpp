#ifndef ElectronRepulsionGeom1100RecSDDF_hpp
#define ElectronRepulsionGeom1100RecSDDF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SD|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_sddf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(1564, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(7470, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(4760, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 414, pbuffer, 1955, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 514, pbuffer, 2155, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 664, pbuffer, 2455, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 874, pbuffer, 2875, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1024, pbuffer, 3025, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1249, pbuffer, 3250, 315, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<2, 3>(skbuffer, 1995, ckbuffer, 855, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 3885, ckbuffer, 4845, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 4235, ckbuffer, 6570, 0, 4);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 2205, 1890, 3885, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 2835, 1995, 4235, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 0, 1995, 2205, 2835, r_ab, 2, 3);
...
#endif /* ElectronRepulsionGeom1100RecSDDF_hpp */
