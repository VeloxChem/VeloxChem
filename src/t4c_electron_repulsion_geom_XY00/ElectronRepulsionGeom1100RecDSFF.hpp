#ifndef ElectronRepulsionGeom1100RecDSFF_hpp
#define ElectronRepulsionGeom1100RecDSFF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(DS|1/|r-r'||FF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_dsff(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(3996, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(27552, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(19698, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * b_exp, {621, 651});

                pbuffer.scale(2.0 * b_exp, {741, 786});

                pbuffer.scale(2.0 * b_exp, {921, 984});

                pbuffer.scale(2.0 * b_exp, {1173, 1257});
                ...
                t2cfunc::reduce(cbuffer, 74, pbuffer, 621, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 104, pbuffer, 741, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 149, pbuffer, 921, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 212, pbuffer, 1173, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 296, pbuffer, 1599, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 356, pbuffer, 1779, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 446, pbuffer, 2049, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 572, pbuffer, 2427, 168, ket_width, ket_npgtos);
                ...
                pbuffer.scale(a_exp / b_exp, {621, 651});

                pbuffer.scale(a_exp / b_exp, {741, 786});

                pbuffer.scale(a_exp / b_exp, {921, 984});

                pbuffer.scale(a_exp / b_exp, {1173, 1257});
                ...
                t2cfunc::reduce(cbuffer, 740, pbuffer, 95, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 750, pbuffer, 175, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 765, pbuffer, 280, 21, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 786, pbuffer, 406, 28, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 814, pbuffer, 621, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 844, pbuffer, 741, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 889, pbuffer, 921, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 952, pbuffer, 1173, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1036, pbuffer, 1599, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1096, pbuffer, 1779, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1186, pbuffer, 2049, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1312, pbuffer, 2427, 168, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 1480, pbuffer, 621, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1510, pbuffer, 741, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1555, pbuffer, 921, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1618, pbuffer, 1173, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1702, pbuffer, 1599, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1762, pbuffer, 1779, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1852, pbuffer, 2049, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1978, pbuffer, 2427, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2146, pbuffer, 2991, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2246, pbuffer, 3191, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2396, pbuffer, 3491, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2606, pbuffer, 3911, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2886, pbuffer, 4471, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3036, pbuffer, 4621, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3261, pbuffer, 4846, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3576, pbuffer, 5161, 420, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1288, cbuffer, 74, 104, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 1378, cbuffer, 104, 149, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 1513, cbuffer, 149, 212, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 1702, 1288, 1378, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 1882, 1378, 1513, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 2152, 1702, 1882, cfactors, 6, 0, 1);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 4780, cbuffer, 740, 750, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 4810, cbuffer, 750, 765, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 4855, cbuffer, 765, 786, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 4918, 4780, 4810, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 4978, 4810, 4855, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 5068, 4918, 4978, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 5168, cbuffer, 814, 844, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 5258, cbuffer, 844, 889, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 5393, cbuffer, 889, 952, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 5582, 5168, 5258, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 5762, 5258, 5393, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 6032, 5582, 5762, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 7232, cbuffer, 1036, 1096, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 7412, cbuffer, 1096, 1186, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 7682, cbuffer, 1186, 1312, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 8060, 7232, 7412, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 8420, 7412, 7682, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 8960, 8060, 8420, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 14360, cbuffer, 1480, 1510, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 14450, cbuffer, 1510, 1555, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 14585, cbuffer, 1555, 1618, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 14774, 14360, 14450, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 14954, 14450, 14585, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 15224, 14774, 14954, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 15524, cbuffer, 1702, 1762, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 15704, cbuffer, 1762, 1852, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 15974, cbuffer, 1852, 1978, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 16352, 15524, 15704, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 16712, 15704, 15974, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 17252, 16352, 16712, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 17852, cbuffer, 2146, 2246, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 18152, cbuffer, 2246, 2396, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 18602, cbuffer, 2396, 2606, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 19232, 17852, 18152, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 19832, 18152, 18602, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 20732, 19232, 19832, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 21732, cbuffer, 2886, 3036, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 22182, cbuffer, 3036, 3261, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 22857, cbuffer, 3261, 3576, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 23802, 21732, 22182, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 24702, 22182, 22857, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 26052, 23802, 24702, cfactors, 6, 0, 4);

            t4cfunc::ket_transform<3, 3>(skbuffer, 0, ckbuffer, 288, 0, 0);

            t4cfunc::ket_transform<3, 3>(skbuffer, 13279, ckbuffer, 2152, 0, 1);

            t4cfunc::ket_transform<3, 3>(skbuffer, 13426, ckbuffer, 4180, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 13720, ckbuffer, 5068, 0, 0);

            t4cfunc::ket_transform<3, 3>(skbuffer, 13769, ckbuffer, 6032, 0, 1);

            t4cfunc::ket_transform<3, 3>(skbuffer, 14357, ckbuffer, 8960, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 17591, ckbuffer, 15224, 0, 1);

            t4cfunc::ket_transform<3, 3>(skbuffer, 17738, ckbuffer, 17252, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 18032, ckbuffer, 20732, 0, 3);

            t4cfunc::ket_transform<3, 3>(skbuffer, 18522, ckbuffer, 26052, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 17003, 13720, 13769, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 17150, 13769, 14357, r_ab, 3, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 19257, 17591, 17738, r_ab, 3, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_psxx(skbuffer, 4900, 0, 17003, 17150, r_ab, 3, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 49, 0, 13426, 3, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_psxx(skbuffer, 4459, 0, 13279, 49, r_ab, 3, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 13916, 13720, 17738, 3, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 14651, 13769, 18032, 3, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 15533, 14357, 18522, 3, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 490, 13769, 13916, 14651, r_ab, 3, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 1813, 14357, 14651, 15533, r_ab, 3, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_psxx(skbuffer, 5341, 13279, 17003, 19257, 490, r_ab, 3, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ppxx(skbuffer, 6664, 49, 17150, 490, 1813, r_ab, 3, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dsxx(skbuffer, 10633, 4459, 4900, 5341, 6664, r_ab, 3, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 0, skbuffer, 10633, 3, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 245, skbuffer, 10927, 3, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 490, skbuffer, 11221, 3, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 735, skbuffer, 11515, 3, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 980, skbuffer, 11809, 3, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 1225, skbuffer, 12103, 3, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 1470, skbuffer, 12397, 3, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 1715, skbuffer, 12691, 3, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 1960, skbuffer, 12985, 3, 3);
...
#endif /* ElectronRepulsionGeom1100RecDSFF_hpp */
