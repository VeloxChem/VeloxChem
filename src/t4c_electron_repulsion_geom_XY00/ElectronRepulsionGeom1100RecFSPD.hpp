#ifndef ElectronRepulsionGeom1100RecFSPD_hpp
#define ElectronRepulsionGeom1100RecFSPD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(FS|1/|r-r'||PD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_fspd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(1568, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(4086, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(16860, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * b_exp, {180, 198});

                pbuffer.scale(2.0 * b_exp, {270, 300});
                ...
                pbuffer.scale(a_exp / b_exp, {180, 198});

                pbuffer.scale(a_exp / b_exp, {270, 300});
                ...
                t2cfunc::reduce(cbuffer, 368, pbuffer, 33, 6, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 374, pbuffer, 75, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 384, pbuffer, 180, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 402, pbuffer, 270, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 432, pbuffer, 486, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 468, pbuffer, 630, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 528, pbuffer, 940, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 588, pbuffer, 1120, 100, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 688, pbuffer, 180, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 706, pbuffer, 270, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 736, pbuffer, 486, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 772, pbuffer, 630, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 832, pbuffer, 940, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 892, pbuffer, 1120, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 992, pbuffer, 1465, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1082, pbuffer, 1645, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1232, pbuffer, 1945, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1358, pbuffer, 2071, 210, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 612, cbuffer, 112, 148, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 720, cbuffer, 208, 268, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 900, cbuffer, 368, 374, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 918, cbuffer, 384, 402, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1134, cbuffer, 432, 468, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1566, cbuffer, 528, 588, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 3096, cbuffer, 688, 706, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 3150, cbuffer, 736, 772, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 3258, cbuffer, 832, 892, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 3438, cbuffer, 992, 1082, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 3708, cbuffer, 1232, 1358, cfactors, 6, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 0, ckbuffer, 0, 0, 0);

            t4cfunc::ket_transform<1, 2>(skbuffer, 15, ckbuffer, 18, 0, 1);

            t4cfunc::ket_transform<1, 2>(skbuffer, 13335, ckbuffer, 558, 0, 1);

            t4cfunc::ket_transform<1, 2>(skbuffer, 13380, ckbuffer, 612, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 13470, ckbuffer, 720, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 13620, ckbuffer, 900, 0, 0);

            t4cfunc::ket_transform<1, 2>(skbuffer, 13635, ckbuffer, 918, 0, 1);

            t4cfunc::ket_transform<1, 2>(skbuffer, 13815, ckbuffer, 1134, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 14175, ckbuffer, 1566, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 15900, ckbuffer, 3096, 0, 1);

            t4cfunc::ket_transform<1, 2>(skbuffer, 15945, ckbuffer, 3150, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 16035, ckbuffer, 3258, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 16185, ckbuffer, 3438, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 16410, ckbuffer, 3708, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 3030, 0, 15, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 15450, 13620, 13635, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 15495, 13635, 13815, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 15630, 13815, 14175, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 16725, 15900, 15945, r_ab, 1, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_psxx(skbuffer, 3210, 0, 15450, 15495, r_ab, 1, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 4155, 15, 15495, 15630, r_ab, 1, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dsxx(skbuffer, 8475, 3030, 3210, 4155, r_ab, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 60, 0, 13380, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 600, 15, 13470, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_psxx(skbuffer, 3075, 0, 13335, 60, r_ab, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_ppxx(skbuffer, 3750, 15, 60, 600, r_ab, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_dsxx(skbuffer, 8205, 3030, 3075, 3750, r_ab, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 13680, 13620, 15945, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 13905, 13635, 16035, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 14325, 13815, 16185, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 14775, 14175, 16410, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 195, 13635, 13680, 13905, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 870, 13815, 13905, 14325, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 1680, 14175, 14325, 14775, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_psxx(skbuffer, 3345, 13335, 15450, 16725, 195, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ppxx(skbuffer, 4560, 60, 15495, 195, 870, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pdxx(skbuffer, 5775, 600, 15630, 870, 1680, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dsxx(skbuffer, 8745, 3075, 3210, 3345, 4560, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dpxx(skbuffer, 9555, 3750, 4155, 4560, 5775, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_fsxx(skbuffer, 11985, 8205, 8475, 8745, 9555, r_ab, 1, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 0, skbuffer, 11985, 1, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 105, skbuffer, 12135, 1, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 210, skbuffer, 12285, 1, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 315, skbuffer, 12435, 1, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 420, skbuffer, 12585, 1, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 525, skbuffer, 12735, 1, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 630, skbuffer, 12885, 1, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 735, skbuffer, 13035, 1, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 840, skbuffer, 13185, 1, 2);
...
#endif /* ElectronRepulsionGeom1100RecFSPD_hpp */
