#ifndef ElectronRepulsionGeom1100RecSDPP_hpp
#define ElectronRepulsionGeom1100RecSDPP_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SD|1/|r-r'||PP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_sdpp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(738, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(1224, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 81, pbuffer, 356, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 111, pbuffer, 416, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 171, pbuffer, 536, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 216, pbuffer, 581, 90, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<1, 1>(skbuffer, 486, ckbuffer, 0, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 513, ckbuffer, 27, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 999, ckbuffer, 513, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 1089, ckbuffer, 603, 0, 4);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 567, 486, 999, 1, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 729, 513, 1089, 1, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 0, 513, 567, 729, r_ab, 1, 1);
...
#endif /* ElectronRepulsionGeom1100RecSDPP_hpp */
