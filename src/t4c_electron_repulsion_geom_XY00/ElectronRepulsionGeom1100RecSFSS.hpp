#ifndef ElectronRepulsionGeom1100RecSFSS_hpp
#define ElectronRepulsionGeom1100RecSFSS_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SF|1/|r-r'||SS)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_sfss(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(52, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(217, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 16, pbuffer, 75, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 31, pbuffer, 105, 21, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 0>(skbuffer, 90, cbuffer, 0, 0, 2);

            t4cfunc::ket_transform<0, 0>(skbuffer, 96, cbuffer, 6, 0, 3);

            t4cfunc::ket_transform<0, 0>(skbuffer, 181, cbuffer, 16, 0, 4);

            t4cfunc::ket_transform<0, 0>(skbuffer, 196, cbuffer, 31, 0, 5);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 106, 90, 181, 0, 0);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 136, 96, 196, 0, 0);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 0, 96, 106, 136, r_ab, 0, 0);
...
#endif /* ElectronRepulsionGeom1100RecSFSS_hpp */
