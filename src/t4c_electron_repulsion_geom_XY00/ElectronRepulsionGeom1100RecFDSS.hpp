#ifndef ElectronRepulsionGeom1100RecFDSS_hpp
#define ElectronRepulsionGeom1100RecFDSS_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(FD|1/|r-r'||SS)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_fdss(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(230, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(3887, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * b_exp, {65, 75});

                pbuffer.scale(2.0 * b_exp, {115, 130});

                pbuffer.scale(2.0 * b_exp, {175, 196});

                t2cfunc::reduce(cbuffer, 19, pbuffer, 65, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 29, pbuffer, 115, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 44, pbuffer, 175, 21, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {8, 11});

                pbuffer.scale(2.0 * a_exp, {29, 35});

                pbuffer.scale(a_exp / b_exp, {65, 75});

                pbuffer.scale(a_exp / b_exp, {115, 130});

                pbuffer.scale(a_exp / b_exp, {175, 196});

                t2cfunc::reduce(cbuffer, 65, pbuffer, 8, 3, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 120, pbuffer, 65, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 130, pbuffer, 115, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 145, pbuffer, 175, 21, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 166, pbuffer, 238, 28, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 194, pbuffer, 294, 36, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<0, 0>(skbuffer, 81, cbuffer, 9, 0, 3);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3343, cbuffer, 19, 0, 3);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3353, cbuffer, 29, 0, 4);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3368, cbuffer, 44, 0, 5);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3389, cbuffer, 65, 0, 1);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3392, cbuffer, 68, 0, 2);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3416, cbuffer, 74, 0, 3);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3456, cbuffer, 84, 0, 4);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3516, cbuffer, 99, 0, 5);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3777, cbuffer, 120, 0, 3);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3787, cbuffer, 130, 0, 4);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3802, cbuffer, 145, 0, 5);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3823, cbuffer, 166, 0, 6);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3851, cbuffer, 194, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 580, 3, 81, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 3684, 3392, 3416, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 3702, 3416, 3456, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 3732, 3456, 3516, r_ab, 0, 0);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 652, 3, 3684, 3702, r_ab, 0, 0);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 958, 81, 3702, 3732, r_ab, 0, 0);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ddxx(skbuffer, 1831, 580, 652, 958, r_ab, 0, 0);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 9, 0, 3343, 0, 0);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 91, 3, 3353, 0, 0);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 211, 81, 3368, 0, 0);

            erirec::comp_bra_geom01_hrr_electron_repulsion_pdxx(skbuffer, 598, 3, 9, 91, r_ab, 0, 0);

            erirec::comp_bra_geom01_hrr_electron_repulsion_pfxx(skbuffer, 868, 81, 91, 211, r_ab, 0, 0);

            erirec::comp_bra_geom01_hrr_electron_repulsion_ddxx(skbuffer, 1723, 580, 598, 868, r_ab, 0, 0);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 3398, 3389, 3777, 0, 0);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 3426, 3392, 3787, 0, 0);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 3471, 3416, 3802, 0, 0);

            erirec::comp_bra_geom01_hrr_electron_repulsion_shxx(skbuffer, 3537, 3456, 3823, 0, 0);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sixx(skbuffer, 3600, 3516, 3851, 0, 0);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 27, 3392, 3398, 3426, r_ab, 0, 0);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 121, 3416, 3426, 3471, r_ab, 0, 0);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sgxx(skbuffer, 256, 3456, 3471, 3537, r_ab, 0, 0);

            erirec::comp_bra_geom11_hrr_electron_repulsion_shxx(skbuffer, 391, 3516, 3537, 3600, r_ab, 0, 0);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pdxx(skbuffer, 706, 9, 3684, 27, 121, r_ab, 0, 0);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pfxx(skbuffer, 1048, 91, 3702, 121, 256, r_ab, 0, 0);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pgxx(skbuffer, 1318, 211, 3732, 256, 391, r_ab, 0, 0);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ddxx(skbuffer, 1939, 598, 652, 706, 1048, r_ab, 0, 0);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dfxx(skbuffer, 2263, 868, 958, 1048, 1318, r_ab, 0, 0);

            erirec::comp_bra_geom11_hrr_electron_repulsion_fdxx(skbuffer, 2803, 1723, 1831, 1939, 2263, r_ab, 0, 0);

            t4cfunc::bra_transform<3, 2>(sbuffer, 0, skbuffer, 2803, 0, 0);

            t4cfunc::bra_transform<3, 2>(sbuffer, 35, skbuffer, 2863, 0, 0);

            t4cfunc::bra_transform<3, 2>(sbuffer, 70, skbuffer, 2923, 0, 0);

            t4cfunc::bra_transform<3, 2>(sbuffer, 105, skbuffer, 2983, 0, 0);

            t4cfunc::bra_transform<3, 2>(sbuffer, 140, skbuffer, 3043, 0, 0);

            t4cfunc::bra_transform<3, 2>(sbuffer, 175, skbuffer, 3103, 0, 0);

            t4cfunc::bra_transform<3, 2>(sbuffer, 210, skbuffer, 3163, 0, 0);

            t4cfunc::bra_transform<3, 2>(sbuffer, 245, skbuffer, 3223, 0, 0);

            t4cfunc::bra_transform<3, 2>(sbuffer, 280, skbuffer, 3283, 0, 0);
...
#endif /* ElectronRepulsionGeom1100RecFDSS_hpp */
