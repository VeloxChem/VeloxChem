#ifndef ElectronRepulsionGeom1100RecSPPP_hpp
#define ElectronRepulsionGeom1100RecSPPP_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SP|1/|r-r'||PP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_sppp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(423, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(666, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(4.0 * a_exp * b_exp, {138, 156});

                pbuffer.scale(4.0 * a_exp * b_exp, {174, 210});

                pbuffer.scale(4.0 * a_exp * b_exp, {246, 276});

                pbuffer.scale(4.0 * a_exp * b_exp, {276, 336});

                t2cfunc::reduce(cbuffer, 36, pbuffer, 138, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 54, pbuffer, 174, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 90, pbuffer, 246, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 120, pbuffer, 276, 60, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<1, 1>(skbuffer, 243, ckbuffer, 0, 0, 0);

            t4cfunc::ket_transform<1, 1>(skbuffer, 252, ckbuffer, 9, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 522, ckbuffer, 279, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 576, ckbuffer, 333, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 279, 243, 522, 1, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 360, 252, 576, 1, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 0, 252, 279, 360, r_ab, 1, 1);

            t4cfunc::bra_transform<0, 1>(sbuffer, 0, skbuffer, 0, 1, 1);

            t4cfunc::bra_transform<0, 1>(sbuffer, 27, skbuffer, 27, 1, 1);

            t4cfunc::bra_transform<0, 1>(sbuffer, 54, skbuffer, 54, 1, 1);

            t4cfunc::bra_transform<0, 1>(sbuffer, 81, skbuffer, 81, 1, 1);

            t4cfunc::bra_transform<0, 1>(sbuffer, 108, skbuffer, 108, 1, 1);

            t4cfunc::bra_transform<0, 1>(sbuffer, 135, skbuffer, 135, 1, 1);

            t4cfunc::bra_transform<0, 1>(sbuffer, 162, skbuffer, 162, 1, 1);

            t4cfunc::bra_transform<0, 1>(sbuffer, 189, skbuffer, 189, 1, 1);

            t4cfunc::bra_transform<0, 1>(sbuffer, 216, skbuffer, 216, 1, 1);
...
#endif /* ElectronRepulsionGeom1100RecSPPP_hpp */
