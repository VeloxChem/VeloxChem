#ifndef ElectronRepulsionGeom1100RecFDSF_hpp
#define ElectronRepulsionGeom1100RecFDSF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(FD|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_fdsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(27209, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 340, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 30, pbuffer, 820, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 90, pbuffer, 1530, 100, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * b_exp, {1530, 1630});

                pbuffer.scale(2.0 * b_exp, {2405, 2555});

                pbuffer.scale(2.0 * b_exp, {3320, 3530});

                t2cfunc::reduce(cbuffer, 190, pbuffer, 1530, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 290, pbuffer, 2405, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 440, pbuffer, 3320, 210, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {340, 370});

                pbuffer.scale(2.0 * a_exp, {820, 880});

                pbuffer.scale(a_exp / b_exp, {1530, 1630});

                pbuffer.scale(a_exp / b_exp, {2405, 2555});

                pbuffer.scale(a_exp / b_exp, {3320, 3530});

                t2cfunc::reduce(cbuffer, 650, pbuffer, 340, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 680, pbuffer, 820, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 740, pbuffer, 1530, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 840, pbuffer, 2405, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 990, pbuffer, 3320, 210, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 1200, pbuffer, 1530, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1300, pbuffer, 2405, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1450, pbuffer, 3320, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1660, pbuffer, 4118, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1940, pbuffer, 4678, 360, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<0, 3>(skbuffer, 567, cbuffer, 90, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 23401, cbuffer, 190, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 23471, cbuffer, 290, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 23576, cbuffer, 440, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 23723, cbuffer, 650, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 23744, cbuffer, 680, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 23912, cbuffer, 740, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 24192, cbuffer, 840, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 24612, cbuffer, 990, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 26439, cbuffer, 1200, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 26509, cbuffer, 1300, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 26614, cbuffer, 1450, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 26761, cbuffer, 1660, 0, 6);

            t4cfunc::ket_transform<0, 3>(skbuffer, 26957, cbuffer, 1940, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 4060, 21, 567, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 25788, 23744, 23912, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 25914, 23912, 24192, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 26124, 24192, 24612, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 4564, 21, 25788, 25914, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 6706, 567, 25914, 26124, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ddxx(skbuffer, 12817, 4060, 4564, 6706, r_ab, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 63, 0, 23401, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 637, 21, 23471, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 1477, 567, 23576, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_pdxx(skbuffer, 4186, 21, 63, 637, r_ab, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_pfxx(skbuffer, 6076, 567, 637, 1477, r_ab, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_ddxx(skbuffer, 12061, 4060, 4186, 6076, r_ab, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 23786, 23723, 26439, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 23982, 23744, 26509, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 24297, 23912, 26614, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_shxx(skbuffer, 24759, 24192, 26761, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sixx(skbuffer, 25200, 24612, 26957, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 189, 23744, 23786, 23982, r_ab, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 847, 23912, 23982, 24297, r_ab, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sgxx(skbuffer, 1792, 24192, 24297, 24759, r_ab, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_shxx(skbuffer, 2737, 24612, 24759, 25200, r_ab, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pdxx(skbuffer, 4942, 63, 25788, 189, 847, r_ab, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pfxx(skbuffer, 7336, 637, 25914, 847, 1792, r_ab, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pgxx(skbuffer, 9226, 1477, 26124, 1792, 2737, r_ab, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ddxx(skbuffer, 13573, 4186, 4564, 4942, 7336, r_ab, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dfxx(skbuffer, 15841, 6076, 6706, 7336, 9226, r_ab, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_fdxx(skbuffer, 19621, 12061, 12817, 13573, 15841, r_ab, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 0, skbuffer, 19621, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 245, skbuffer, 20041, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 490, skbuffer, 20461, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 735, skbuffer, 20881, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 980, skbuffer, 21301, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1225, skbuffer, 21721, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1470, skbuffer, 22141, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1715, skbuffer, 22561, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1960, skbuffer, 22981, 0, 3);
...
#endif /* ElectronRepulsionGeom1100RecFDSF_hpp */
