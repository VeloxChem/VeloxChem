#ifndef ElectronRepulsionGeom1100RecPFPF_hpp
#define ElectronRepulsionGeom1100RecPFPF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(PF|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_pfpf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(2900, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(8520, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(16989, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 150, pbuffer, 3300, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 300, pbuffer, 3750, 225, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 525, pbuffer, 1075, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 585, pbuffer, 1375, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 675, pbuffer, 2075, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 775, pbuffer, 2475, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 925, pbuffer, 3300, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1075, pbuffer, 3750, 225, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 1300, pbuffer, 3300, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1450, pbuffer, 3750, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1675, pbuffer, 4551, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1885, pbuffer, 4971, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2200, pbuffer, 5601, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2480, pbuffer, 5881, 420, ket_width, ket_npgtos);

            }

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 0, cbuffer, 0, 60, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1080, cbuffer, 150, 300, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1530, cbuffer, 525, 585, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1710, cbuffer, 675, 775, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2910, cbuffer, 925, 1075, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 6600, cbuffer, 1300, 1450, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 7050, cbuffer, 1675, 1885, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 7680, cbuffer, 2200, 2480, cfactors, 6, 0, 6);

            t4cfunc::ket_transform<1, 3>(skbuffer, 0, ckbuffer, 0, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 11151, ckbuffer, 1080, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 11466, ckbuffer, 1530, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 11592, ckbuffer, 1710, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 12432, ckbuffer, 2910, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 15645, ckbuffer, 6600, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 15960, ckbuffer, 7050, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 16401, ckbuffer, 7680, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 15015, 11592, 12432, r_ab, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 126, 0, 11151, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 11802, 11466, 15645, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 12747, 11592, 15960, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_shxx(skbuffer, 13692, 12432, 16401, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 756, 11592, 11802, 12747, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sgxx(skbuffer, 2646, 12432, 12747, 13692, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pfxx(skbuffer, 5481, 126, 15015, 756, 2646, r_ab, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 5481, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 441, skbuffer, 6111, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 882, skbuffer, 6741, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1323, skbuffer, 7371, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1764, skbuffer, 8001, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 2205, skbuffer, 8631, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 2646, skbuffer, 9261, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 3087, skbuffer, 9891, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 3528, skbuffer, 10521, 1, 3);
...
#endif /* ElectronRepulsionGeom1100RecPFPF_hpp */
