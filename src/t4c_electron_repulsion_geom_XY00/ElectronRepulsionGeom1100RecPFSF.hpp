#ifndef ElectronRepulsionGeom1100RecPFSF_hpp
#define ElectronRepulsionGeom1100RecPFSF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(PF|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_pfsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(1160, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(5663, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 60, pbuffer, 1855, 150, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 210, pbuffer, 680, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 270, pbuffer, 1230, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 370, pbuffer, 1855, 150, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 520, pbuffer, 1855, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 670, pbuffer, 2431, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 880, pbuffer, 2851, 280, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 3>(skbuffer, 0, cbuffer, 0, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 3717, cbuffer, 60, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 3822, cbuffer, 210, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 3864, cbuffer, 270, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 4144, cbuffer, 370, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 5215, cbuffer, 520, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 5320, cbuffer, 670, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 5467, cbuffer, 880, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 5005, 3864, 4144, r_ab, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 42, 0, 3717, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 3934, 3822, 5215, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 4249, 3864, 5320, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_shxx(skbuffer, 4564, 4144, 5467, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 252, 3864, 3934, 4249, r_ab, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sgxx(skbuffer, 882, 4144, 4249, 4564, r_ab, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pfxx(skbuffer, 1827, 42, 5005, 252, 882, r_ab, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 1827, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 147, skbuffer, 2037, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 294, skbuffer, 2247, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 441, skbuffer, 2457, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 588, skbuffer, 2667, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 735, skbuffer, 2877, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 882, skbuffer, 3087, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1029, skbuffer, 3297, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1176, skbuffer, 3507, 0, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 3, 0, 3, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1100RecPFSF_hpp */
