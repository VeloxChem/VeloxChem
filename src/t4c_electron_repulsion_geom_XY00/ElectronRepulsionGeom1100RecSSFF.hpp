#ifndef ElectronRepulsionGeom1100RecSSFF_hpp
#define ElectronRepulsionGeom1100RecSSFF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SS|1/|r-r'||FF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_ssff(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(4.0 * a_exp * b_exp, {396, 426});

                pbuffer.scale(4.0 * a_exp * b_exp, {456, 501});

                pbuffer.scale(4.0 * a_exp * b_exp, {546, 609});

                pbuffer.scale(4.0 * a_exp * b_exp, {672, 756});

                pbuffer.scale(4.0 * a_exp * b_exp, {840, 900});

                pbuffer.scale(4.0 * a_exp * b_exp, {900, 990});

                pbuffer.scale(4.0 * a_exp * b_exp, {990, 1116});

                pbuffer.scale(4.0 * a_exp * b_exp, {1116, 1284});
            ...
            t4cfunc::bra_transform<0, 0>(sbuffer, 0, skbuffer, 441, 3, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 49, skbuffer, 490, 3, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 98, skbuffer, 539, 3, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 147, skbuffer, 588, 3, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 196, skbuffer, 637, 3, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 245, skbuffer, 686, 3, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 294, skbuffer, 735, 3, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 343, skbuffer, 784, 3, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 392, skbuffer, 833, 3, 3);
...
#endif /* ElectronRepulsionGeom1100RecSSFF_hpp */
