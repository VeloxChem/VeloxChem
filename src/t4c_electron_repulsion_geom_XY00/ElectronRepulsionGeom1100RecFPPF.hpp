#ifndef ElectronRepulsionGeom1100RecFPPF_hpp
#define ElectronRepulsionGeom1100RecFPPF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(FP|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_fppf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(11340, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(48069, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 25, pbuffer, 415, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 55, pbuffer, 595, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 100, pbuffer, 1075, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 160, pbuffer, 1375, 90, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * b_exp, {1075, 1135});

                pbuffer.scale(2.0 * b_exp, {1375, 1465});
                ...
                t2cfunc::reduce(cbuffer, 250, pbuffer, 1075, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 310, pbuffer, 1375, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 400, pbuffer, 2075, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 500, pbuffer, 2475, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 650, pbuffer, 3300, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 800, pbuffer, 3750, 225, ket_width, ket_npgtos);
                ...
                pbuffer.scale(a_exp / b_exp, {1075, 1135});

                pbuffer.scale(a_exp / b_exp, {1375, 1465});
                ...
                t2cfunc::reduce(cbuffer, 1025, pbuffer, 95, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1035, pbuffer, 175, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1050, pbuffer, 415, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1080, pbuffer, 595, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1125, pbuffer, 1075, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1185, pbuffer, 1375, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1275, pbuffer, 2075, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1375, pbuffer, 2475, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1525, pbuffer, 3300, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1675, pbuffer, 3750, 225, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 1900, pbuffer, 1075, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1960, pbuffer, 1375, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2050, pbuffer, 2075, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2150, pbuffer, 2475, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2300, pbuffer, 3300, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2450, pbuffer, 3750, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2675, pbuffer, 4551, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2885, pbuffer, 4971, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3200, pbuffer, 5601, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3480, pbuffer, 5881, 420, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 390, cbuffer, 100, 160, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2010, cbuffer, 250, 310, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2190, cbuffer, 400, 500, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2490, cbuffer, 650, 800, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2940, cbuffer, 1025, 1035, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2970, cbuffer, 1050, 1080, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 3330, cbuffer, 1125, 1185, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 4050, cbuffer, 1275, 1375, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 5250, cbuffer, 1525, 1675, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 8940, cbuffer, 1900, 1960, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 9120, cbuffer, 2050, 2150, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 9420, cbuffer, 2300, 2450, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 9870, cbuffer, 2675, 2885, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 10500, cbuffer, 3200, 3480, cfactors, 6, 0, 6);

            t4cfunc::ket_transform<1, 3>(skbuffer, 0, ckbuffer, 0, 0, 0);

            t4cfunc::ket_transform<1, 3>(skbuffer, 21, ckbuffer, 30, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 840, ckbuffer, 390, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 40341, ckbuffer, 2010, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 40467, ckbuffer, 2190, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 40677, ckbuffer, 2490, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 40992, ckbuffer, 2940, 0, 0);

            t4cfunc::ket_transform<1, 3>(skbuffer, 41013, ckbuffer, 2970, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 41265, ckbuffer, 3330, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 41769, ckbuffer, 4050, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 42609, ckbuffer, 5250, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 46389, ckbuffer, 8940, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 46515, ckbuffer, 9120, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 46725, ckbuffer, 9420, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 47040, ckbuffer, 9870, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 47481, ckbuffer, 10500, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 7833, 21, 840, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 45192, 41013, 41265, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 45381, 41265, 41769, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 45759, 41769, 42609, r_ab, 1, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 8589, 21, 45192, 45381, r_ab, 1, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 11991, 840, 45381, 45759, r_ab, 1, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dpxx(skbuffer, 23331, 7833, 8589, 11991, r_ab, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 84, 0, 40341, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 966, 21, 40467, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 2478, 840, 40677, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_ppxx(skbuffer, 8022, 21, 84, 966, r_ab, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_pdxx(skbuffer, 10857, 840, 966, 2478, r_ab, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_dpxx(skbuffer, 22197, 7833, 8022, 10857, r_ab, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 41076, 40992, 46389, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 41391, 41013, 46515, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 41979, 41265, 46725, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 42924, 41769, 47040, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_shxx(skbuffer, 43869, 42609, 47481, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 273, 41013, 41076, 41391, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 1344, 41265, 41391, 41979, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 3108, 41769, 41979, 42924, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sgxx(skbuffer, 4998, 42609, 42924, 43869, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ppxx(skbuffer, 9156, 84, 45192, 273, 1344, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pdxx(skbuffer, 13125, 966, 45381, 1344, 3108, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pfxx(skbuffer, 16527, 2478, 45759, 3108, 4998, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dpxx(skbuffer, 24465, 8022, 8589, 9156, 13125, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ddxx(skbuffer, 27867, 10857, 11991, 13125, 16527, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_fpxx(skbuffer, 34671, 22197, 23331, 24465, 27867, r_ab, 1, 3);

            t4cfunc::bra_transform<3, 1>(sbuffer, 0, skbuffer, 34671, 1, 3);

            t4cfunc::bra_transform<3, 1>(sbuffer, 441, skbuffer, 35301, 1, 3);

            t4cfunc::bra_transform<3, 1>(sbuffer, 882, skbuffer, 35931, 1, 3);

            t4cfunc::bra_transform<3, 1>(sbuffer, 1323, skbuffer, 36561, 1, 3);

            t4cfunc::bra_transform<3, 1>(sbuffer, 1764, skbuffer, 37191, 1, 3);

            t4cfunc::bra_transform<3, 1>(sbuffer, 2205, skbuffer, 37821, 1, 3);

            t4cfunc::bra_transform<3, 1>(sbuffer, 2646, skbuffer, 38451, 1, 3);

            t4cfunc::bra_transform<3, 1>(sbuffer, 3087, skbuffer, 39081, 1, 3);

            t4cfunc::bra_transform<3, 1>(sbuffer, 3528, skbuffer, 39711, 1, 3);
...
#endif /* ElectronRepulsionGeom1100RecFPPF_hpp */
