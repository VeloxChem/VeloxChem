#ifndef ElectronRepulsionGeom1100RecFSSP_hpp
#define ElectronRepulsionGeom1100RecFSSP_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(FS|1/|r-r'||SP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_fssp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(294, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(3372, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * b_exp, {37, 46});

                pbuffer.scale(2.0 * b_exp, {100, 118});

                pbuffer.scale(2.0 * b_exp, {192, 222});

                t2cfunc::reduce(cbuffer, 12, pbuffer, 37, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 21, pbuffer, 100, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 39, pbuffer, 192, 30, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {7, 10});

                pbuffer.scale(a_exp / b_exp, {37, 46});

                pbuffer.scale(a_exp / b_exp, {100, 118});

                pbuffer.scale(a_exp / b_exp, {192, 222});

                t2cfunc::reduce(cbuffer, 69, pbuffer, 7, 3, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 72, pbuffer, 37, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 81, pbuffer, 100, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 99, pbuffer, 192, 30, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 129, pbuffer, 37, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 138, pbuffer, 100, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 156, pbuffer, 192, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 186, pbuffer, 297, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 231, pbuffer, 387, 63, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<0, 1>(skbuffer, 2667, cbuffer, 12, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2676, cbuffer, 21, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2694, cbuffer, 39, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2724, cbuffer, 69, 0, 0);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2727, cbuffer, 72, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2763, cbuffer, 81, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2835, cbuffer, 99, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 3180, cbuffer, 129, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 3189, cbuffer, 138, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 3207, cbuffer, 156, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 3237, cbuffer, 186, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 3282, cbuffer, 231, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 606, 0, 3, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 3090, 2724, 2727, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 3099, 2727, 2763, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 3126, 2763, 2835, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 3345, 3180, 3189, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_psxx(skbuffer, 642, 0, 3090, 3099, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 831, 3, 3099, 3126, r_ab, 0, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dsxx(skbuffer, 1695, 606, 642, 831, r_ab, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 12, 0, 2676, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 120, 3, 2694, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_psxx(skbuffer, 615, 0, 2667, 12, r_ab, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_ppxx(skbuffer, 750, 3, 12, 120, r_ab, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_dsxx(skbuffer, 1641, 606, 615, 750, r_ab, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 2736, 2724, 3189, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 2781, 2727, 3207, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 2865, 2763, 3237, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 2955, 2835, 3282, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 39, 2727, 2736, 2781, r_ab, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 174, 2763, 2781, 2865, r_ab, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 336, 2835, 2865, 2955, r_ab, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_psxx(skbuffer, 669, 2667, 3090, 3345, 39, r_ab, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ppxx(skbuffer, 912, 12, 3099, 39, 174, r_ab, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pdxx(skbuffer, 1155, 120, 3126, 174, 336, r_ab, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dsxx(skbuffer, 1749, 615, 642, 669, 912, r_ab, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dpxx(skbuffer, 1911, 750, 831, 912, 1155, r_ab, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_fsxx(skbuffer, 2397, 1641, 1695, 1749, 1911, r_ab, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 0, skbuffer, 2397, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 21, skbuffer, 2427, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 42, skbuffer, 2457, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 63, skbuffer, 2487, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 84, skbuffer, 2517, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 105, skbuffer, 2547, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 126, skbuffer, 2577, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 147, skbuffer, 2607, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 168, skbuffer, 2637, 0, 1);
...
#endif /* ElectronRepulsionGeom1100RecFSSP_hpp */
