...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SD|1/|r-r'||DD)  integral derivatives.
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(1054, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(4584, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(3400, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 279, pbuffer, 1245, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 339, pbuffer, 1365, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 439, pbuffer, 1565, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 589, pbuffer, 1865, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 679, pbuffer, 1955, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 829, pbuffer, 2105, 225, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<2, 2>(skbuffer, 1425, ckbuffer, 540, 0, 2);

            t4cfunc::ket_transform<2, 2>(skbuffer, 2775, ckbuffer, 2964, 0, 3);

            t4cfunc::ket_transform<2, 2>(skbuffer, 3025, ckbuffer, 4044, 0, 4);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 1575, 1350, 2775, 2, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 2025, 1425, 3025, 2, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 0, 1425, 1575, 2025, r_ab, 2, 2);
