#ifndef ElectronRepulsionGeom1100RecSPDF_hpp
#define ElectronRepulsionGeom1100RecSPDF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SP|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_spdf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(2590, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 184, pbuffer, 789, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 244, pbuffer, 909, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 334, pbuffer, 1089, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 460, pbuffer, 1341, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 560, pbuffer, 1441, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 710, pbuffer, 1591, 210, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<2, 3>(skbuffer, 945, ckbuffer, 75, 0, 0);

            t4cfunc::ket_transform<2, 3>(skbuffer, 980, ckbuffer, 360, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 2030, ckbuffer, 2610, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 2240, ckbuffer, 3720, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 1085, 945, 2030, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 1400, 980, 2240, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 0, 980, 1085, 1400, r_ab, 2, 3);
...
#endif /* ElectronRepulsionGeom1100RecSPDF_hpp */
