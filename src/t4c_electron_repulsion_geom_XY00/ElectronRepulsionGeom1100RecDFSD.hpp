#ifndef ElectronRepulsionGeom1100RecDFSD_hpp
#define ElectronRepulsionGeom1100RecDFSD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(DF|1/|r-r'||SD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_dfsd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(1224, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(11925, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 96, pbuffer, 1225, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 186, pbuffer, 1732, 126, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 312, pbuffer, 394, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 348, pbuffer, 760, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 408, pbuffer, 1225, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 498, pbuffer, 1732, 126, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 624, pbuffer, 1225, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 714, pbuffer, 1732, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 840, pbuffer, 2194, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1008, pbuffer, 2530, 216, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<0, 2>(skbuffer, 9500, cbuffer, 96, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9575, cbuffer, 186, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9680, cbuffer, 312, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9710, cbuffer, 348, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9910, cbuffer, 408, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 10210, cbuffer, 498, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 11425, cbuffer, 624, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 11500, cbuffer, 714, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 11605, cbuffer, 840, 0, 6);

            t4cfunc::ket_transform<0, 2>(skbuffer, 11745, cbuffer, 1008, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 11050, 9710, 9910, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 11200, 9910, 10210, r_ab, 0, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 2975, 30, 11050, 11200, r_ab, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 80, 0, 9500, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 680, 30, 9575, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_pfxx(skbuffer, 2525, 30, 80, 680, r_ab, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 9760, 9680, 11425, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 9985, 9710, 11500, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_shxx(skbuffer, 10315, 9910, 11605, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sixx(skbuffer, 10630, 10210, 11745, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 230, 9710, 9760, 9985, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sgxx(skbuffer, 905, 9910, 9985, 10315, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_shxx(skbuffer, 1580, 10210, 10315, 10630, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pfxx(skbuffer, 3425, 80, 11050, 230, 905, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pgxx(skbuffer, 4775, 680, 11200, 905, 1580, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dfxx(skbuffer, 6800, 2525, 2975, 3425, 4775, r_ab, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 6800, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 175, skbuffer, 7100, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 350, skbuffer, 7400, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 525, skbuffer, 7700, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 700, skbuffer, 8000, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 875, skbuffer, 8300, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1050, skbuffer, 8600, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1225, skbuffer, 8900, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1400, skbuffer, 9200, 0, 2);
...
#endif /* ElectronRepulsionGeom1100RecDFSD_hpp */
