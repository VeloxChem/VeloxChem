#ifndef ElectronRepulsionGeom1100RecFPSD_hpp
#define ElectronRepulsionGeom1100RecFPSD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(FP|1/|r-r'||SD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_fpsd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(11445, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 6, pbuffer, 132, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 24, pbuffer, 330, 36, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * b_exp, {330, 366});

                pbuffer.scale(2.0 * b_exp, {620, 680});

                pbuffer.scale(2.0 * b_exp, {965, 1055});

                t2cfunc::reduce(cbuffer, 60, pbuffer, 330, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 96, pbuffer, 620, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 156, pbuffer, 965, 90, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {33, 39});

                pbuffer.scale(2.0 * a_exp, {132, 150});

                pbuffer.scale(a_exp / b_exp, {330, 366});

                pbuffer.scale(a_exp / b_exp, {620, 680});

                pbuffer.scale(a_exp / b_exp, {965, 1055});

                t2cfunc::reduce(cbuffer, 246, pbuffer, 33, 6, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 252, pbuffer, 132, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 270, pbuffer, 330, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 306, pbuffer, 620, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 366, pbuffer, 965, 90, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 456, pbuffer, 330, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 492, pbuffer, 620, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 552, pbuffer, 965, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 642, pbuffer, 1298, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 768, pbuffer, 1550, 168, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<0, 2>(skbuffer, 200, cbuffer, 24, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9605, cbuffer, 60, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9635, cbuffer, 96, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9685, cbuffer, 156, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9760, cbuffer, 246, 0, 0);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9765, cbuffer, 252, 0, 1);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9825, cbuffer, 270, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9945, cbuffer, 306, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 10145, cbuffer, 366, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 11045, cbuffer, 456, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 11075, cbuffer, 492, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 11125, cbuffer, 552, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 11200, cbuffer, 642, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 11305, cbuffer, 768, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 1865, 5, 200, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 10760, 9765, 9825, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 10805, 9825, 9945, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 10895, 9945, 10145, r_ab, 0, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 2045, 5, 10760, 10805, r_ab, 0, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 2855, 200, 10805, 10895, r_ab, 0, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dpxx(skbuffer, 5555, 1865, 2045, 2855, r_ab, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 20, 0, 9605, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 230, 5, 9635, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 590, 200, 9685, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_ppxx(skbuffer, 1910, 5, 20, 230, r_ab, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_pdxx(skbuffer, 2585, 200, 230, 590, r_ab, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_dpxx(skbuffer, 5285, 1865, 1910, 2585, r_ab, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 9780, 9760, 11045, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 9855, 9765, 11075, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 9995, 9825, 11125, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 10220, 9945, 11200, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_shxx(skbuffer, 10445, 10145, 11305, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 65, 9765, 9780, 9855, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 320, 9825, 9855, 9995, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 740, 9945, 9995, 10220, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sgxx(skbuffer, 1190, 10145, 10220, 10445, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ppxx(skbuffer, 2180, 20, 10760, 65, 320, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pdxx(skbuffer, 3125, 230, 10805, 320, 740, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pfxx(skbuffer, 3935, 590, 10895, 740, 1190, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dpxx(skbuffer, 5825, 1910, 2045, 2180, 3125, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ddxx(skbuffer, 6635, 2585, 2855, 3125, 3935, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_fpxx(skbuffer, 8255, 5285, 5555, 5825, 6635, r_ab, 0, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 0, skbuffer, 8255, 0, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 105, skbuffer, 8405, 0, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 210, skbuffer, 8555, 0, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 315, skbuffer, 8705, 0, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 420, skbuffer, 8855, 0, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 525, skbuffer, 9005, 0, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 630, skbuffer, 9155, 0, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 735, skbuffer, 9305, 0, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 840, skbuffer, 9455, 0, 2);
...
#endif /* ElectronRepulsionGeom1100RecFPSD_hpp */
