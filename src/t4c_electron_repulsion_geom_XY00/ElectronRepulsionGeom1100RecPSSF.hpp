#ifndef ElectronRepulsionGeom1100RecPSSF_hpp
#define ElectronRepulsionGeom1100RecPSSF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(PS|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_pssf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(260, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(833, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * b_exp, {140, 170});

                t2cfunc::reduce(cbuffer, 0, pbuffer, 140, 30, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {55, 65});

                pbuffer.scale(a_exp / b_exp, {140, 170});

                t2cfunc::reduce(cbuffer, 30, pbuffer, 55, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 40, pbuffer, 140, 30, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * b_exp, {140, 170});

                pbuffer.scale(4.0 * a_exp * b_exp, {266, 326});

                pbuffer.scale(4.0 * a_exp * b_exp, {386, 486});

                t2cfunc::reduce(cbuffer, 70, pbuffer, 140, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 100, pbuffer, 266, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 160, pbuffer, 386, 100, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 3>(skbuffer, 378, cbuffer, 0, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 399, cbuffer, 30, 0, 0);

            t4cfunc::ket_transform<0, 3>(skbuffer, 406, cbuffer, 40, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 637, cbuffer, 70, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 658, cbuffer, 100, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 700, cbuffer, 160, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 616, 399, 406, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 770, 637, 658, r_ab, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 427, 399, 658, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 490, 406, 700, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 0, 406, 427, 490, r_ab, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_psxx(skbuffer, 189, 378, 616, 770, 0, r_ab, 0, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 0, skbuffer, 189, 0, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 21, skbuffer, 210, 0, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 42, skbuffer, 231, 0, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 63, skbuffer, 252, 0, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 84, skbuffer, 273, 0, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 105, skbuffer, 294, 0, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 126, skbuffer, 315, 0, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 147, skbuffer, 336, 0, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 168, skbuffer, 357, 0, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 0, 0, 3, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1100RecPSSF_hpp */
