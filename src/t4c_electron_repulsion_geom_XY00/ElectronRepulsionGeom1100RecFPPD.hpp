#ifndef ElectronRepulsionGeom1100RecFPPD_hpp
#define ElectronRepulsionGeom1100RecFPPD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(FP|1/|r-r'||PD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_fppd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(6804, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(34335, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 16, pbuffer, 212, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 34, pbuffer, 320, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 64, pbuffer, 590, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 100, pbuffer, 770, 60, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * b_exp, {590, 626});

                pbuffer.scale(2.0 * b_exp, {770, 830});
                ...
                t2cfunc::reduce(cbuffer, 160, pbuffer, 590, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 196, pbuffer, 770, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 256, pbuffer, 1180, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 316, pbuffer, 1420, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 416, pbuffer, 1925, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 506, pbuffer, 2195, 150, ket_width, ket_npgtos);
                ...
                pbuffer.scale(a_exp / b_exp, {590, 626});

                pbuffer.scale(a_exp / b_exp, {770, 830});

                pbuffer.scale(a_exp / b_exp, {1180, 1240});

                pbuffer.scale(a_exp / b_exp, {1420, 1520});

                pbuffer.scale(a_exp / b_exp, {1925, 2015});

                pbuffer.scale(a_exp / b_exp, {2195, 2345});

                t2cfunc::reduce(cbuffer, 656, pbuffer, 37, 6, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 662, pbuffer, 85, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 672, pbuffer, 212, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 690, pbuffer, 320, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 720, pbuffer, 590, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 756, pbuffer, 770, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 816, pbuffer, 1180, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 876, pbuffer, 1420, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 976, pbuffer, 1925, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1066, pbuffer, 2195, 150, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 1216, pbuffer, 590, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1252, pbuffer, 770, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1312, pbuffer, 1180, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1372, pbuffer, 1420, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1472, pbuffer, 1925, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1562, pbuffer, 2195, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1712, pbuffer, 2708, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1838, pbuffer, 2960, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2048, pbuffer, 3380, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2216, pbuffer, 3548, 280, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1206, cbuffer, 160, 196, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1314, cbuffer, 256, 316, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1494, cbuffer, 416, 506, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1764, cbuffer, 656, 662, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1782, cbuffer, 672, 690, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 1998, cbuffer, 720, 756, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 2430, cbuffer, 816, 876, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 3150, cbuffer, 976, 1066, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 5364, cbuffer, 1216, 1252, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 5472, cbuffer, 1312, 1372, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 5652, cbuffer, 1472, 1562, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 5922, cbuffer, 1712, 1838, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 6300, cbuffer, 2048, 2216, cfactors, 6, 0, 6);

            t4cfunc::ket_transform<1, 2>(skbuffer, 0, ckbuffer, 0, 0, 0);

            t4cfunc::ket_transform<1, 2>(skbuffer, 15, ckbuffer, 18, 0, 1);

            t4cfunc::ket_transform<1, 2>(skbuffer, 600, ckbuffer, 234, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 28815, ckbuffer, 1206, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 28905, ckbuffer, 1314, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 29055, ckbuffer, 1494, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 29280, ckbuffer, 1764, 0, 0);

            t4cfunc::ket_transform<1, 2>(skbuffer, 29295, ckbuffer, 1782, 0, 1);

            t4cfunc::ket_transform<1, 2>(skbuffer, 29475, ckbuffer, 1998, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 29835, ckbuffer, 2430, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 30435, ckbuffer, 3150, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 33135, ckbuffer, 5364, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 33225, ckbuffer, 5472, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 33375, ckbuffer, 5652, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 33600, ckbuffer, 5922, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 33915, ckbuffer, 6300, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 5595, 15, 600, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 32280, 29295, 29475, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 32415, 29475, 29835, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 32685, 29835, 30435, r_ab, 1, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 6135, 15, 32280, 32415, r_ab, 1, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pdxx(skbuffer, 8565, 600, 32415, 32685, r_ab, 1, 2);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dpxx(skbuffer, 16665, 5595, 6135, 8565, r_ab, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 60, 0, 28815, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 690, 15, 28905, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 1770, 600, 29055, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_ppxx(skbuffer, 5730, 15, 60, 690, r_ab, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_pdxx(skbuffer, 7755, 600, 690, 1770, r_ab, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_dpxx(skbuffer, 15855, 5595, 5730, 7755, r_ab, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 29340, 29280, 33135, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 29565, 29295, 33225, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 29985, 29475, 33375, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 30660, 29835, 33600, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_shxx(skbuffer, 31335, 30435, 33915, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 195, 29295, 29340, 29565, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 960, 29475, 29565, 29985, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 2220, 29835, 29985, 30660, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sgxx(skbuffer, 3570, 30435, 30660, 31335, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ppxx(skbuffer, 6540, 60, 32280, 195, 960, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pdxx(skbuffer, 9375, 690, 32415, 960, 2220, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pfxx(skbuffer, 11805, 1770, 32685, 2220, 3570, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dpxx(skbuffer, 17475, 5730, 6135, 6540, 9375, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ddxx(skbuffer, 19905, 7755, 8565, 9375, 11805, r_ab, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_fpxx(skbuffer, 24765, 15855, 16665, 17475, 19905, r_ab, 1, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 0, skbuffer, 24765, 1, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 315, skbuffer, 25215, 1, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 630, skbuffer, 25665, 1, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 945, skbuffer, 26115, 1, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 1260, skbuffer, 26565, 1, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 1575, skbuffer, 27015, 1, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 1890, skbuffer, 27465, 1, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 2205, skbuffer, 27915, 1, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 2520, skbuffer, 28365, 1, 2);
...
#endif /* ElectronRepulsionGeom1100RecFPPD_hpp */
