#ifndef ElectronRepulsionGeom1100RecSFSP_hpp
#define ElectronRepulsionGeom1100RecSFSP_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SF|1/|r-r'||SP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_sfsp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(651, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 48, pbuffer, 297, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 93, pbuffer, 387, 63, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 1>(skbuffer, 270, cbuffer, 0, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 288, cbuffer, 18, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 543, cbuffer, 48, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 588, cbuffer, 93, 0, 5);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 318, 270, 543, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 408, 288, 588, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 0, 288, 318, 408, r_ab, 0, 1);

            t4cfunc::bra_transform<0, 3>(sbuffer, 0, skbuffer, 0, 0, 1);

            t4cfunc::bra_transform<0, 3>(sbuffer, 21, skbuffer, 30, 0, 1);

            t4cfunc::bra_transform<0, 3>(sbuffer, 42, skbuffer, 60, 0, 1);

            t4cfunc::bra_transform<0, 3>(sbuffer, 63, skbuffer, 90, 0, 1);

            t4cfunc::bra_transform<0, 3>(sbuffer, 84, skbuffer, 120, 0, 1);

            t4cfunc::bra_transform<0, 3>(sbuffer, 105, skbuffer, 150, 0, 1);

            t4cfunc::bra_transform<0, 3>(sbuffer, 126, skbuffer, 180, 0, 1);

            t4cfunc::bra_transform<0, 3>(sbuffer, 147, skbuffer, 210, 0, 1);

            t4cfunc::bra_transform<0, 3>(sbuffer, 168, skbuffer, 240, 0, 1);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 3, 0, 1, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1100RecSFSP_hpp */
