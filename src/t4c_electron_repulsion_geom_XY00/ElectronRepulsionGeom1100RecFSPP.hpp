#ifndef ElectronRepulsionGeom1100RecFSPP_hpp
#define ElectronRepulsionGeom1100RecFSPP_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(FS|1/|r-r'||PP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_fspp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(882, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(2043, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(10116, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(a_exp / b_exp, {77, 86});
                ...
                t2cfunc::reduce(cbuffer, 207, pbuffer, 8, 3, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 210, pbuffer, 29, 6, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 216, pbuffer, 77, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 225, pbuffer, 122, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 243, pbuffer, 230, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 261, pbuffer, 302, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 297, pbuffer, 466, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 327, pbuffer, 556, 60, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 387, pbuffer, 77, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 396, pbuffer, 122, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 414, pbuffer, 230, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 432, pbuffer, 302, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 468, pbuffer, 466, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 498, pbuffer, 556, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 558, pbuffer, 751, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 603, pbuffer, 841, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 693, pbuffer, 1021, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 756, pbuffer, 1084, 126, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 306, cbuffer, 63, 81, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 360, cbuffer, 117, 147, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 450, cbuffer, 207, 210, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 459, cbuffer, 216, 225, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 567, cbuffer, 243, 261, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 783, cbuffer, 297, 327, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 1548, cbuffer, 387, 396, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 1575, cbuffer, 414, 432, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 1629, cbuffer, 468, 498, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 1719, cbuffer, 558, 603, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 1854, cbuffer, 693, 756, cfactors, 6, 0, 5);

            t4cfunc::ket_transform<1, 1>(skbuffer, 0, ckbuffer, 0, 0, 0);

            t4cfunc::ket_transform<1, 1>(skbuffer, 9, ckbuffer, 9, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 8001, ckbuffer, 279, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 8028, ckbuffer, 306, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 8082, ckbuffer, 360, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 8172, ckbuffer, 450, 0, 0);

            t4cfunc::ket_transform<1, 1>(skbuffer, 8181, ckbuffer, 459, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 8289, ckbuffer, 567, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 8505, ckbuffer, 783, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 9540, ckbuffer, 1548, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 9567, ckbuffer, 1575, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 9621, ckbuffer, 1629, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 9711, ckbuffer, 1719, 0, 4);

            t4cfunc::ket_transform<1, 1>(skbuffer, 9846, ckbuffer, 1854, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 1818, 0, 9, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 9270, 8172, 8181, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 9297, 8181, 8289, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 9378, 8289, 8505, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 10035, 9540, 9567, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_psxx(skbuffer, 1926, 0, 9270, 9297, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 2493, 9, 9297, 9378, r_ab, 1, 1);

            erirec::comp_bra_geom10_hrr_electron_repulsion_dsxx(skbuffer, 5085, 1818, 1926, 2493, r_ab, 1, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 36, 0, 8028, 1, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 360, 9, 8082, 1, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_psxx(skbuffer, 1845, 0, 8001, 36, r_ab, 1, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_ppxx(skbuffer, 2250, 9, 36, 360, r_ab, 1, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_dsxx(skbuffer, 4923, 1818, 1845, 2250, r_ab, 1, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 8208, 8172, 9567, 1, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 8343, 8181, 9621, 1, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 8595, 8289, 9711, 1, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 8865, 8505, 9846, 1, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 117, 8181, 8208, 8343, r_ab, 1, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 522, 8289, 8343, 8595, r_ab, 1, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 1008, 8505, 8595, 8865, r_ab, 1, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_psxx(skbuffer, 2007, 8001, 9270, 10035, 117, r_ab, 1, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ppxx(skbuffer, 2736, 36, 9297, 117, 522, r_ab, 1, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pdxx(skbuffer, 3465, 360, 9378, 522, 1008, r_ab, 1, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dsxx(skbuffer, 5247, 1845, 1926, 2007, 2736, r_ab, 1, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dpxx(skbuffer, 5733, 2250, 2493, 2736, 3465, r_ab, 1, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_fsxx(skbuffer, 7191, 4923, 5085, 5247, 5733, r_ab, 1, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 0, skbuffer, 7191, 1, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 63, skbuffer, 7281, 1, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 126, skbuffer, 7371, 1, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 189, skbuffer, 7461, 1, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 252, skbuffer, 7551, 1, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 315, skbuffer, 7641, 1, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 378, skbuffer, 7731, 1, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 441, skbuffer, 7821, 1, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 504, skbuffer, 7911, 1, 1);
...
#endif /* ElectronRepulsionGeom1100RecFSPP_hpp */
