#ifndef ElectronRepulsionGeom1100RecSDPD_hpp
#define ElectronRepulsionGeom1100RecSDPD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SD|1/|r-r'||PD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_sdpd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(544, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(1476, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(2040, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 144, pbuffer, 700, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 204, pbuffer, 820, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 304, pbuffer, 1020, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 394, pbuffer, 1110, 150, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<1, 2>(skbuffer, 855, ckbuffer, 54, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 1665, ckbuffer, 1026, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 1815, ckbuffer, 1206, 0, 4);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 945, 810, 1665, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 1215, 855, 1815, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 0, 855, 945, 1215, r_ab, 1, 2);
...
#endif /* ElectronRepulsionGeom1100RecSDPD_hpp */
