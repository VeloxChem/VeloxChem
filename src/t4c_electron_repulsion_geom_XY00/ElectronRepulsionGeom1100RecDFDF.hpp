#ifndef ElectronRepulsionGeom1100RecDFDF_hpp
#define ElectronRepulsionGeom1100RecDFDF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(DF|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_dfdf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(9384, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(45360, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(83475, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 1919, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 60, pbuffer, 2279, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 2819, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 276, pbuffer, 3925, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 376, pbuffer, 4425, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 526, pbuffer, 5175, 210, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * b_exp, {6600, 6750});

                pbuffer.scale(2.0 * b_exp, {7200, 7425});

                pbuffer.scale(2.0 * b_exp, {8100, 8415});

                pbuffer.scale(2.0 * b_exp, {9675, 9885});

                pbuffer.scale(2.0 * b_exp, {10305, 10620});

                pbuffer.scale(2.0 * b_exp, {11250, 11691});

                t2cfunc::reduce(cbuffer, 736, pbuffer, 6600, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 886, pbuffer, 7200, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1111, pbuffer, 8100, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1426, pbuffer, 9675, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1636, pbuffer, 10305, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1951, pbuffer, 11250, 441, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {1919, 1979});

                pbuffer.scale(2.0 * a_exp, {2279, 2369});

                pbuffer.scale(2.0 * a_exp, {2819, 2945});
                ...
                pbuffer.scale(a_exp / b_exp, {6600, 6750});

                pbuffer.scale(a_exp / b_exp, {7200, 7425});

                pbuffer.scale(a_exp / b_exp, {8100, 8415});

                pbuffer.scale(a_exp / b_exp, {9675, 9885});

                pbuffer.scale(a_exp / b_exp, {10305, 10620});

                pbuffer.scale(a_exp / b_exp, {11250, 11691});

                t2cfunc::reduce(cbuffer, 2392, pbuffer, 1919, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2452, pbuffer, 2279, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2542, pbuffer, 2819, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2668, pbuffer, 3925, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2768, pbuffer, 4425, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2918, pbuffer, 5175, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3128, pbuffer, 6600, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3278, pbuffer, 7200, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3503, pbuffer, 8100, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3818, pbuffer, 9675, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4028, pbuffer, 10305, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4343, pbuffer, 11250, 441, ket_width, ket_npgtos);
                ...
                pbuffer.scale(4.0 * a_exp * b_exp, {12741, 13021});

                pbuffer.scale(4.0 * a_exp * b_exp, {13301, 13721});

                pbuffer.scale(4.0 * a_exp * b_exp, {14141, 14729});

                pbuffer.scale(4.0 * a_exp * b_exp, {15317, 15677});

                pbuffer.scale(4.0 * a_exp * b_exp, {15677, 16217});

                pbuffer.scale(4.0 * a_exp * b_exp, {16217, 16973});

                t2cfunc::reduce(cbuffer, 4784, pbuffer, 6600, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4934, pbuffer, 7200, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5159, pbuffer, 8100, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5474, pbuffer, 9675, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5684, pbuffer, 10305, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5999, pbuffer, 11250, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 6440, pbuffer, 12741, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 6720, pbuffer, 13301, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 7140, pbuffer, 14141, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 7728, pbuffer, 15317, 360, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 8088, pbuffer, 15677, 540, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 8628, pbuffer, 16217, 756, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 6660, cbuffer, 736, 886, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 7110, cbuffer, 886, 1111, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 7785, 6660, 7110, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 8685, cbuffer, 1426, 1636, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 9315, cbuffer, 1636, 1951, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 10260, 8685, 9315, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 11520, cbuffer, 2392, 2452, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 11700, cbuffer, 2452, 2542, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 11970, 11520, 11700, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 12330, cbuffer, 2668, 2768, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 12630, cbuffer, 2768, 2918, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 13080, 12330, 12630, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 15480, cbuffer, 3128, 3278, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 15930, cbuffer, 3278, 3503, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 16605, 15480, 15930, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 20205, cbuffer, 3818, 4028, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 20835, cbuffer, 4028, 4343, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 21780, 20205, 20835, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 31860, cbuffer, 4784, 4934, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 32310, cbuffer, 4934, 5159, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 32985, 31860, 32310, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 33885, cbuffer, 5474, 5684, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 34515, cbuffer, 5684, 5999, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 35460, 33885, 34515, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 36720, cbuffer, 6440, 6720, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 37560, cbuffer, 6720, 7140, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 38820, 36720, 37560, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 40500, cbuffer, 7728, 8088, cfactors, 6, 0, 7);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 41580, cbuffer, 8088, 8628, cfactors, 6, 0, 7);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 43200, 40500, 41580, cfactors, 6, 0, 7);

            t4cfunc::ket_transform<2, 3>(skbuffer, 0, ckbuffer, 450, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 210, ckbuffer, 1560, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 66500, ckbuffer, 7785, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 67025, ckbuffer, 10260, 0, 5);

            t4cfunc::ket_transform<2, 3>(skbuffer, 67760, ckbuffer, 11970, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 67970, ckbuffer, 13080, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 69370, ckbuffer, 16605, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 71470, ckbuffer, 21780, 0, 5);

            t4cfunc::ket_transform<2, 3>(skbuffer, 79975, ckbuffer, 32985, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 80500, ckbuffer, 35460, 0, 5);

            t4cfunc::ket_transform<2, 3>(skbuffer, 81235, ckbuffer, 38820, 0, 6);

            t4cfunc::ket_transform<2, 3>(skbuffer, 82215, ckbuffer, 43200, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 77350, 67970, 69370, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 78400, 69370, 71470, r_ab, 2, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_pfxx(skbuffer, 20825, 210, 77350, 78400, r_ab, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 560, 0, 66500, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 4760, 210, 67025, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_pfxx(skbuffer, 17675, 210, 560, 4760, r_ab, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 68320, 67760, 79975, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 69895, 67970, 80500, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_shxx(skbuffer, 72205, 69370, 81235, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sixx(skbuffer, 74410, 71470, 82215, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 1610, 67970, 68320, 69895, r_ab, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sgxx(skbuffer, 6335, 69370, 69895, 72205, r_ab, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_shxx(skbuffer, 11060, 71470, 72205, 74410, r_ab, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pfxx(skbuffer, 23975, 560, 77350, 1610, 6335, r_ab, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pgxx(skbuffer, 33425, 4760, 78400, 6335, 11060, r_ab, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dfxx(skbuffer, 47600, 17675, 20825, 23975, 33425, r_ab, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 47600, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1225, skbuffer, 49700, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 2450, skbuffer, 51800, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 3675, skbuffer, 53900, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 4900, skbuffer, 56000, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 6125, skbuffer, 58100, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 7350, skbuffer, 60200, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 8575, skbuffer, 62300, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 9800, skbuffer, 64400, 2, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 2, 3, 2, 3, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1100RecDFDF_hpp */
