#ifndef ElectronRepulsionGeom1100RecPDSF_hpp
#define ElectronRepulsionGeom1100RecPDSF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(PD|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_pdsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(780, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(3591, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 30, pbuffer, 930, 100, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 130, pbuffer, 240, 30, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 320, pbuffer, 930, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 420, pbuffer, 1320, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 570, pbuffer, 1620, 210, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 3>(skbuffer, 0, cbuffer, 0, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2289, cbuffer, 30, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2359, cbuffer, 130, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2380, cbuffer, 160, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2548, cbuffer, 220, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 3269, cbuffer, 320, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 3339, cbuffer, 420, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 3444, cbuffer, 570, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 3143, 2380, 2548, r_ab, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 21, 0, 2289, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 2422, 2359, 3269, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 2618, 2380, 3339, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 2828, 2548, 3444, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 147, 2380, 2422, 2618, r_ab, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 525, 2548, 2618, 2828, r_ab, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pdxx(skbuffer, 1155, 21, 3143, 147, 525, r_ab, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 0, skbuffer, 1155, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 105, skbuffer, 1281, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 210, skbuffer, 1407, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 315, skbuffer, 1533, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 420, skbuffer, 1659, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 525, skbuffer, 1785, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 630, skbuffer, 1911, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 735, skbuffer, 2037, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 840, skbuffer, 2163, 0, 3);
...
#endif /* ElectronRepulsionGeom1100RecPDSF_hpp */
