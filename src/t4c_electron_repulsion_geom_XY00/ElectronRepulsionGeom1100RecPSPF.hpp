#ifndef ElectronRepulsionGeom1100RecPSPF_hpp
#define ElectronRepulsionGeom1100RecPSPF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(PS|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_pspf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(650, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(1590, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(2499, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * b_exp, {310, 355});

                t2cfunc::reduce(cbuffer, 0, pbuffer, 220, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 30, pbuffer, 310, 45, ket_width, ket_npgtos);
                ...
                pbuffer.scale(a_exp / b_exp, {310, 355});

                t2cfunc::reduce(cbuffer, 75, pbuffer, 65, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 85, pbuffer, 115, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 100, pbuffer, 220, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 130, pbuffer, 310, 45, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * b_exp, {220, 250});

                pbuffer.scale(2.0 * b_exp, {310, 355});

                pbuffer.scale(4.0 * a_exp * b_exp, {481, 541});

                pbuffer.scale(4.0 * a_exp * b_exp, {601, 691});

                pbuffer.scale(4.0 * a_exp * b_exp, {781, 881});

                pbuffer.scale(4.0 * a_exp * b_exp, {881, 1031});

                t2cfunc::reduce(cbuffer, 175, pbuffer, 220, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 205, pbuffer, 310, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 250, pbuffer, 481, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 310, pbuffer, 601, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 400, pbuffer, 781, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 500, pbuffer, 881, 150, ket_width, ket_npgtos);

            }

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 0, cbuffer, 0, 30, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 90, cbuffer, 75, 85, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 120, cbuffer, 100, 130, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1020, cbuffer, 175, 205, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1110, cbuffer, 250, 310, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1290, cbuffer, 400, 500, cfactors, 6, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1134, ckbuffer, 0, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1197, ckbuffer, 90, 0, 0);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1218, ckbuffer, 120, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1911, ckbuffer, 1020, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1974, ckbuffer, 1110, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 2100, ckbuffer, 1290, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 1848, 1197, 1218, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 2310, 1911, 1974, r_ab, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 1281, 1197, 1974, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 1470, 1218, 2100, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 0, 1218, 1281, 1470, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_psxx(skbuffer, 567, 1134, 1848, 2310, 0, r_ab, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 0, skbuffer, 567, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 63, skbuffer, 630, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 126, skbuffer, 693, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 189, skbuffer, 756, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 252, skbuffer, 819, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 315, skbuffer, 882, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 378, skbuffer, 945, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 441, skbuffer, 1008, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 504, skbuffer, 1071, 1, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 0, 1, 3, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1100RecPSPF_hpp */
