#ifndef ElectronRepulsionGeom1100RecPFDF_hpp
#define ElectronRepulsionGeom1100RecPFDF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(PF|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_pfdf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(5336, 1);
    ...
    CSimdArray<double> skbuffer(28315, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 276, pbuffer, 5330, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 426, pbuffer, 5780, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 651, pbuffer, 6455, 315, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 966, pbuffer, 1635, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1026, pbuffer, 1935, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1116, pbuffer, 2385, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1242, pbuffer, 3265, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1342, pbuffer, 3665, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1492, pbuffer, 4265, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1702, pbuffer, 5330, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1852, pbuffer, 5780, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2077, pbuffer, 6455, 315, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 2392, pbuffer, 5330, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2542, pbuffer, 5780, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2767, pbuffer, 6455, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3082, pbuffer, 7526, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3292, pbuffer, 7946, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3607, pbuffer, 8576, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4048, pbuffer, 9458, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4328, pbuffer, 9738, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4748, pbuffer, 10158, 588, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2610, cbuffer, 276, 426, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 3060, cbuffer, 426, 651, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 3735, 2610, 3060, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 4635, cbuffer, 966, 1026, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 4815, cbuffer, 1026, 1116, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 5085, 4635, 4815, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 5445, cbuffer, 1242, 1342, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 5745, cbuffer, 1342, 1492, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 6195, 5445, 5745, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 8595, cbuffer, 1702, 1852, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 9045, cbuffer, 1852, 2077, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 9720, 8595, 9045, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 17100, cbuffer, 2392, 2542, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 17550, cbuffer, 2542, 2767, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 18225, 17100, 17550, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 19125, cbuffer, 3082, 3292, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 19755, cbuffer, 3292, 3607, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 20700, 19125, 19755, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 21960, cbuffer, 4048, 4328, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 22800, cbuffer, 4328, 4748, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 24060, 21960, 22800, cfactors, 6, 0, 6);

            t4cfunc::ket_transform<2, 3>(skbuffer, 0, ckbuffer, 450, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 18585, ckbuffer, 3735, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 19110, ckbuffer, 5085, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 19320, ckbuffer, 6195, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 20720, ckbuffer, 9720, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 26075, ckbuffer, 18225, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 26600, ckbuffer, 20700, 0, 5);

            t4cfunc::ket_transform<2, 3>(skbuffer, 27335, ckbuffer, 24060, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 25025, 19320, 20720, r_ab, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 210, 0, 18585, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 19670, 19110, 26075, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 21245, 19320, 26600, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_shxx(skbuffer, 22820, 20720, 27335, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 1260, 19320, 19670, 21245, r_ab, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sgxx(skbuffer, 4410, 20720, 21245, 22820, r_ab, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pfxx(skbuffer, 9135, 210, 25025, 1260, 4410, r_ab, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 9135, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 735, skbuffer, 10185, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1470, skbuffer, 11235, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 2205, skbuffer, 12285, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 2940, skbuffer, 13335, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 3675, skbuffer, 14385, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 4410, skbuffer, 15435, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 5145, skbuffer, 16485, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 5880, skbuffer, 17535, 2, 3);
...
#endif /* ElectronRepulsionGeom1100RecPFDF_hpp */
