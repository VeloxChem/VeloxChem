#ifndef ElectronRepulsionGeom1100RecPFSD_hpp
#define ElectronRepulsionGeom1100RecPFSD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(PF|1/|r-r'||SD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_pfsd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(696, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(4045, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 36, pbuffer, 965, 90, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 126, pbuffer, 330, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 162, pbuffer, 620, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 222, pbuffer, 965, 90, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 312, pbuffer, 965, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 402, pbuffer, 1298, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 528, pbuffer, 1550, 168, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 2>(skbuffer, 0, cbuffer, 0, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 2655, cbuffer, 36, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 2730, cbuffer, 126, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 2760, cbuffer, 162, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 2960, cbuffer, 222, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 3725, cbuffer, 312, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 3800, cbuffer, 402, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 3905, cbuffer, 528, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 3575, 2760, 2960, r_ab, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 30, 0, 2655, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 2810, 2730, 3725, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 3035, 2760, 3800, 0, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_shxx(skbuffer, 3260, 2960, 3905, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 180, 2760, 2810, 3035, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sgxx(skbuffer, 630, 2960, 3035, 3260, r_ab, 0, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pfxx(skbuffer, 1305, 30, 3575, 180, 630, r_ab, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 1305, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 105, skbuffer, 1455, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 210, skbuffer, 1605, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 315, skbuffer, 1755, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 420, skbuffer, 1905, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 525, skbuffer, 2055, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 630, skbuffer, 2205, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 735, skbuffer, 2355, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 840, skbuffer, 2505, 0, 2);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 3, 0, 2, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1100RecPFSD_hpp */
