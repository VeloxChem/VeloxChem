#ifndef ElectronRepulsionGeom1100RecDSPF_hpp
#define ElectronRepulsionGeom1100RecDSPF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(DS|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_dspf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(3600, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(8442, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * b_exp, {285, 315});

                pbuffer.scale(2.0 * b_exp, {405, 450});

                pbuffer.scale(2.0 * b_exp, {675, 735});

                pbuffer.scale(2.0 * b_exp, {855, 945});
                ...
                pbuffer.scale(a_exp / b_exp, {285, 315});

                pbuffer.scale(a_exp / b_exp, {405, 450});

                pbuffer.scale(a_exp / b_exp, {675, 735});

                pbuffer.scale(a_exp / b_exp, {855, 945});
                ...
                t2cfunc::reduce(cbuffer, 500, pbuffer, 285, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 530, pbuffer, 405, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 575, pbuffer, 675, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 635, pbuffer, 855, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 725, pbuffer, 1185, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 825, pbuffer, 1385, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 975, pbuffer, 1685, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1125, pbuffer, 1835, 225, ket_width, ket_npgtos);

            }

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 0, cbuffer, 0, 10, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 300, cbuffer, 25, 55, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 390, cbuffer, 100, 160, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 570, cbuffer, 250, 260, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 600, cbuffer, 275, 305, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 960, cbuffer, 350, 410, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2580, cbuffer, 500, 530, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2670, cbuffer, 575, 635, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2850, cbuffer, 725, 825, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 3150, cbuffer, 975, 1125, cfactors, 6, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 0, ckbuffer, 0, 0, 0);

            t4cfunc::ket_transform<1, 3>(skbuffer, 5691, ckbuffer, 300, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 5754, ckbuffer, 390, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 5880, ckbuffer, 570, 0, 0);

            t4cfunc::ket_transform<1, 3>(skbuffer, 5901, ckbuffer, 600, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 6153, ckbuffer, 960, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 7539, ckbuffer, 2580, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 7602, ckbuffer, 2670, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 7728, ckbuffer, 2850, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 7938, ckbuffer, 3150, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 7287, 5880, 5901, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 7350, 5901, 6153, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 8253, 7539, 7602, r_ab, 1, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_psxx(skbuffer, 2100, 0, 7287, 7350, r_ab, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 21, 0, 5754, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_psxx(skbuffer, 1911, 0, 5691, 21, r_ab, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 5964, 5880, 7602, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 6279, 5901, 7728, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 6657, 6153, 7938, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 210, 5901, 5964, 6279, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 777, 6153, 6279, 6657, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_psxx(skbuffer, 2289, 5691, 7287, 8253, 210, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ppxx(skbuffer, 2856, 21, 7350, 210, 777, r_ab, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dsxx(skbuffer, 4557, 1911, 2100, 2289, 2856, r_ab, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 0, skbuffer, 4557, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 105, skbuffer, 4683, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 210, skbuffer, 4809, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 315, skbuffer, 4935, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 420, skbuffer, 5061, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 525, skbuffer, 5187, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 630, skbuffer, 5313, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 735, skbuffer, 5439, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 840, skbuffer, 5565, 1, 3);
...
#endif /* ElectronRepulsionGeom1100RecDSPF_hpp */
