#ifndef ElectronRepulsionGeom1100RecDPSF_hpp
#define ElectronRepulsionGeom1100RecDPSF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(DP|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_dpsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(920, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(6146, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * b_exp, {540, 600});

                pbuffer.scale(2.0 * b_exp, {930, 1030});

                t2cfunc::reduce(cbuffer, 40, pbuffer, 540, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 100, pbuffer, 930, 100, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {75, 85});

                pbuffer.scale(2.0 * a_exp, {240, 270});

                pbuffer.scale(a_exp / b_exp, {540, 600});

                pbuffer.scale(a_exp / b_exp, {930, 1030});

                t2cfunc::reduce(cbuffer, 200, pbuffer, 75, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 210, pbuffer, 240, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 240, pbuffer, 540, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 300, pbuffer, 930, 100, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * b_exp, {540, 600});

                pbuffer.scale(2.0 * b_exp, {930, 1030});

                pbuffer.scale(4.0 * a_exp * b_exp, {1320, 1470});

                pbuffer.scale(4.0 * a_exp * b_exp, {1620, 1830});

                t2cfunc::reduce(cbuffer, 400, pbuffer, 540, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 460, pbuffer, 930, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 560, pbuffer, 1320, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 710, pbuffer, 1620, 210, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<0, 3>(skbuffer, 4627, cbuffer, 40, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 4669, cbuffer, 100, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 4739, cbuffer, 200, 0, 0);

            t4cfunc::ket_transform<0, 3>(skbuffer, 4746, cbuffer, 210, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 4830, cbuffer, 240, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 4998, cbuffer, 300, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 5782, cbuffer, 400, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 5824, cbuffer, 460, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 5894, cbuffer, 560, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 5999, cbuffer, 710, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 5593, 4746, 4830, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 5656, 4830, 4998, r_ab, 0, 3);

            erirec::comp_bra_geom10_hrr_electron_repulsion_ppxx(skbuffer, 1603, 7, 5593, 5656, r_ab, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 28, 0, 4627, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 280, 7, 4669, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_ppxx(skbuffer, 1414, 7, 28, 280, r_ab, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 4767, 4739, 5782, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 4872, 4746, 5824, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 5068, 4830, 5894, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 5278, 4998, 5999, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 91, 4746, 4767, 4872, r_ab, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sdxx(skbuffer, 406, 4830, 4872, 5068, r_ab, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 784, 4998, 5068, 5278, r_ab, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_ppxx(skbuffer, 1792, 28, 5593, 91, 406, r_ab, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_pdxx(skbuffer, 2359, 280, 5656, 406, 784, r_ab, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_dpxx(skbuffer, 3493, 1414, 1603, 1792, 2359, r_ab, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 0, skbuffer, 3493, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 105, skbuffer, 3619, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 210, skbuffer, 3745, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 315, skbuffer, 3871, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 420, skbuffer, 3997, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 525, skbuffer, 4123, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 630, skbuffer, 4249, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 735, skbuffer, 4375, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 840, skbuffer, 4501, 0, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 2, 1, 0, 3, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1100RecDPSF_hpp */
