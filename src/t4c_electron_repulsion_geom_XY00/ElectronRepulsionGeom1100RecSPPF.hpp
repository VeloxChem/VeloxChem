#ifndef ElectronRepulsionGeom1100RecSPPF_hpp
#define ElectronRepulsionGeom1100RecSPPF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SP|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_sppf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(500, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(1410, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(1554, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 100, pbuffer, 481, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 160, pbuffer, 601, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 250, pbuffer, 781, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 350, pbuffer, 881, 150, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<1, 3>(skbuffer, 567, ckbuffer, 0, 0, 0);

            t4cfunc::ket_transform<1, 3>(skbuffer, 588, ckbuffer, 30, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1218, ckbuffer, 930, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1344, ckbuffer, 1110, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 651, 567, 1218, 1, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 840, 588, 1344, 1, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 0, 588, 651, 840, r_ab, 1, 3);
...
#endif /* ElectronRepulsionGeom1100RecSPPF_hpp */
