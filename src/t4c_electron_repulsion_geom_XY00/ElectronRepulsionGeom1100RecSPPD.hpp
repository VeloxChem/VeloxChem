...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SP|1/|r-r'||PD)  integral derivatives.
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(846, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(1110, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 64, pbuffer, 278, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 100, pbuffer, 350, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 160, pbuffer, 470, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 220, pbuffer, 530, 100, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<1, 2>(skbuffer, 405, ckbuffer, 0, 0, 0);

            t4cfunc::ket_transform<1, 2>(skbuffer, 420, ckbuffer, 18, 0, 1);

            t4cfunc::ket_transform<1, 2>(skbuffer, 870, ckbuffer, 558, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 960, ckbuffer, 666, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 465, 405, 870, 1, 2);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 600, 420, 960, 1, 2);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 0, 420, 465, 600, r_ab, 1, 2);

            t4cfunc::bra_transform<0, 1>(sbuffer, 0, skbuffer, 0, 1, 2);

            t4cfunc::bra_transform<0, 1>(sbuffer, 45, skbuffer, 45, 1, 2);

            t4cfunc::bra_transform<0, 1>(sbuffer, 90, skbuffer, 90, 1, 2);

            t4cfunc::bra_transform<0, 1>(sbuffer, 135, skbuffer, 135, 1, 2);

            t4cfunc::bra_transform<0, 1>(sbuffer, 180, skbuffer, 180, 1, 2);

            t4cfunc::bra_transform<0, 1>(sbuffer, 225, skbuffer, 225, 1, 2);

            t4cfunc::bra_transform<0, 1>(sbuffer, 270, skbuffer, 270, 1, 2);

            t4cfunc::bra_transform<0, 1>(sbuffer, 315, skbuffer, 315, 1, 2);

            t4cfunc::bra_transform<0, 1>(sbuffer, 360, skbuffer, 360, 1, 2);
