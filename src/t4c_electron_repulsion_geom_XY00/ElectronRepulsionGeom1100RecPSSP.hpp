#ifndef ElectronRepulsionGeom1100RecPSSP_hpp
#define ElectronRepulsionGeom1100RecPSSP_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(PS|1/|r-r'||SP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_pssp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> skbuffer(357, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 21, pbuffer, 23, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 30, pbuffer, 56, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 48, pbuffer, 92, 30, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 1>(skbuffer, 162, cbuffer, 0, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 171, cbuffer, 9, 0, 0);

            t4cfunc::ket_transform<0, 1>(skbuffer, 174, cbuffer, 12, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 273, cbuffer, 21, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 282, cbuffer, 30, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 300, cbuffer, 48, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 264, 171, 174, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 330, 273, 282, r_ab, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 183, 171, 282, 0, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 210, 174, 300, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 0, 174, 183, 210, r_ab, 0, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_psxx(skbuffer, 81, 162, 264, 330, 0, r_ab, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 0, skbuffer, 81, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 9, skbuffer, 90, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 18, skbuffer, 99, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 27, skbuffer, 108, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 36, skbuffer, 117, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 45, skbuffer, 126, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 54, skbuffer, 135, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 63, skbuffer, 144, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 72, skbuffer, 153, 0, 1);
...
#endif /* ElectronRepulsionGeom1100RecPSSP_hpp */
