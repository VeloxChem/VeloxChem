#ifndef ElectronRepulsionGeom1100RecSFPP_hpp
#define ElectronRepulsionGeom1100RecSFPP_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SF|1/|r-r'||PP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_sfpp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(1143, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(1953, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 144, pbuffer, 751, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 189, pbuffer, 841, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 279, pbuffer, 1021, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 342, pbuffer, 1084, 126, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 819, cbuffer, 144, 189, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 954, cbuffer, 279, 342, cfactors, 6, 0, 5);

            t4cfunc::ket_transform<1, 1>(skbuffer, 810, ckbuffer, 0, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 864, ckbuffer, 54, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 1629, ckbuffer, 819, 0, 4);

            t4cfunc::ket_transform<1, 1>(skbuffer, 1764, ckbuffer, 954, 0, 5);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 954, 810, 1629, 1, 1);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 1224, 864, 1764, 1, 1);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 0, 864, 954, 1224, r_ab, 1, 1);
...
#endif /* ElectronRepulsionGeom1100RecSFPP_hpp */
