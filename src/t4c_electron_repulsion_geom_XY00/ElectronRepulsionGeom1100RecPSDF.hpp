#ifndef ElectronRepulsionGeom1100RecPSDF_hpp
#define ElectronRepulsionGeom1100RecPSDF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(PS|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_psdf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(1196, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * b_exp, {339, 369});

                pbuffer.scale(2.0 * b_exp, {429, 474});

                pbuffer.scale(2.0 * b_exp, {564, 627});
                ...
                pbuffer.scale(a_exp / b_exp, {339, 369});

                pbuffer.scale(a_exp / b_exp, {429, 474});

                pbuffer.scale(a_exp / b_exp, {564, 627});

                t2cfunc::reduce(cbuffer, 138, pbuffer, 75, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 148, pbuffer, 135, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 163, pbuffer, 210, 21, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 184, pbuffer, 339, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 214, pbuffer, 429, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 259, pbuffer, 564, 63, ket_width, ket_npgtos);
                ...
                pbuffer.scale(4.0 * a_exp * b_exp, {789, 849});

                pbuffer.scale(4.0 * a_exp * b_exp, {909, 999});

                pbuffer.scale(4.0 * a_exp * b_exp, {1089, 1215});

                pbuffer.scale(4.0 * a_exp * b_exp, {1341, 1441});

                pbuffer.scale(4.0 * a_exp * b_exp, {1441, 1591});

                pbuffer.scale(4.0 * a_exp * b_exp, {1591, 1801});

                t2cfunc::reduce(cbuffer, 322, pbuffer, 339, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 352, pbuffer, 429, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 397, pbuffer, 564, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 460, pbuffer, 789, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 520, pbuffer, 909, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 610, pbuffer, 1089, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 736, pbuffer, 1341, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 836, pbuffer, 1441, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 986, pbuffer, 1591, 210, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 405, cbuffer, 138, 148, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 435, cbuffer, 148, 163, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 480, 405, 435, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 540, cbuffer, 184, 214, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 630, cbuffer, 214, 259, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 765, 540, 630, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2565, cbuffer, 322, 352, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 2655, cbuffer, 352, 397, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 2790, 2565, 2655, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2970, cbuffer, 460, 520, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 3150, cbuffer, 520, 610, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 3420, 2970, 3150, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 3780, cbuffer, 736, 836, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 4080, cbuffer, 836, 986, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 4530, 3780, 4080, cfactors, 6, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 1890, ckbuffer, 225, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 1995, ckbuffer, 480, 0, 0);

            t4cfunc::ket_transform<2, 3>(skbuffer, 2030, ckbuffer, 765, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 3185, ckbuffer, 2790, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 3290, ckbuffer, 3420, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 3500, ckbuffer, 4530, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 3080, 1995, 2030, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 3850, 3185, 3290, r_ab, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 2135, 1995, 3290, 2, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 2450, 2030, 3500, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 0, 2030, 2135, 2450, r_ab, 2, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_psxx(skbuffer, 945, 1890, 3080, 3850, 0, r_ab, 2, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 0, skbuffer, 945, 2, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 105, skbuffer, 1050, 2, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 210, skbuffer, 1155, 2, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 315, skbuffer, 1260, 2, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 420, skbuffer, 1365, 2, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 525, skbuffer, 1470, 2, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 630, skbuffer, 1575, 2, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 735, skbuffer, 1680, 2, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 840, skbuffer, 1785, 2, 3);
...
#endif /* ElectronRepulsionGeom1100RecPSDF_hpp */
