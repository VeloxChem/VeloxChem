#ifndef ElectronRepulsionGeom1100RecSFSF_hpp
#define ElectronRepulsionGeom1100RecSFSF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SF|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_sfsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(520, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(1519, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 160, pbuffer, 1320, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 310, pbuffer, 1620, 210, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 3>(skbuffer, 630, cbuffer, 0, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 672, cbuffer, 60, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1267, cbuffer, 160, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1372, cbuffer, 310, 0, 5);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sfxx(skbuffer, 742, 630, 1267, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sgxx(skbuffer, 952, 672, 1372, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_sfxx(skbuffer, 0, 672, 742, 952, r_ab, 0, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 0, skbuffer, 0, 0, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 49, skbuffer, 70, 0, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 98, skbuffer, 140, 0, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 147, skbuffer, 210, 0, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 196, skbuffer, 280, 0, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 245, skbuffer, 350, 0, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 294, skbuffer, 420, 0, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 343, skbuffer, 490, 0, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 392, skbuffer, 560, 0, 3);
...
#endif /* ElectronRepulsionGeom1100RecSFSF_hpp */
