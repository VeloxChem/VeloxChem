#ifndef ElectronRepulsionGeom1100RecSPSF_hpp
#define ElectronRepulsionGeom1100RecSPSF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dB^(1)(SP|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1100_spsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(200, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(518, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 40, pbuffer, 266, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 100, pbuffer, 386, 100, ket_width, ket_npgtos);

            }

            t4cfunc::ket_transform<0, 3>(skbuffer, 189, cbuffer, 0, 0, 0);

            t4cfunc::ket_transform<0, 3>(skbuffer, 196, cbuffer, 10, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 406, cbuffer, 40, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 448, cbuffer, 100, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_spxx(skbuffer, 217, 189, 406, 0, 3);

            erirec::comp_bra_geom01_hrr_electron_repulsion_sdxx(skbuffer, 280, 196, 448, 0, 3);

            erirec::comp_bra_geom11_hrr_electron_repulsion_spxx(skbuffer, 0, 196, 217, 280, r_ab, 0, 3);
            ...
            t4cfunc::bra_transform<0, 1>(sbuffer, 126, skbuffer, 126, 0, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 147, skbuffer, 147, 0, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 168, skbuffer, 168, 0, 3);
...
#endif /* ElectronRepulsionGeom1100RecSPSF_hpp */
