#ifndef ThreeCenterElectronRepulsionRecSFF_hpp
#define ThreeCenterElectronRepulsionRecSFF_hpp
...
/// @brief Computes (S|1/|r-r'||FF)  integrals for basis functions block and basis function pairs block.
...
template <class T>
inline auto
comp_electron_repulsion_sff(T& distributor,
                            ...
                            const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 104, 10, 25, cfactors, 6, 0);

            t3ceri::comp_hrr_electron_repulsion_xph(skbuffer, 149, 25, 46, cfactors, 6, 0);

            t3ceri::comp_hrr_electron_repulsion_xdf(skbuffer, 212, 74, 104, cfactors, 6, 0);

            t3ceri::comp_hrr_electron_repulsion_xdg(skbuffer, 272, 104, 149, cfactors, 6, 0);

            t3ceri::comp_hrr_electron_repulsion_xff(skbuffer, 362, 212, 272, cfactors, 6, 0);

            t3cfunc::ket_transform<3, 3>(sbuffer, 0, skbuffer, 362, 0);
...
#endif /* ThreeCenterElectronRepulsionRecSFF_hpp */
