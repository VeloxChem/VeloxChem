#ifndef ThreeCenterElectronRepulsionRecGPF_hpp
#define ThreeCenterElectronRepulsionRecGPF_hpp
...
/// @brief Computes (G|1/|r-r'||PF)  integrals for basis functions block and basis function pairs block.
...
template <class T>
inline auto
comp_electron_repulsion_gpf(T& distributor,
                            ...
                            const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            t3cfunc::ket_transform<1, 3>(sbuffer, 0, skbuffer, 225, 4);

            distributor.distribute(sbuffer, 0, bra_gto_indices, c_indices, d_indices, 4, 1, 3, j, ket_range);
        }
    }
...
#endif /* ThreeCenterElectronRepulsionRecGPF_hpp */
