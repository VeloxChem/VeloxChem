#ifndef ThreeCenterElectronRepulsionRecIPG_hpp
#define ThreeCenterElectronRepulsionRecIPG_hpp
...
/// @brief Computes (I|1/|r-r'||PG)  integrals for basis functions block and basis function pairs block.
...
template <class T>
inline auto
comp_electron_repulsion_ipg(T& distributor,
                            ...
                            const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    CSimdArray<double> skbuffer(1053, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 6924, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 420, pbuffer, 7344, 588, ket_width, ket_npgtos);
            ...
            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 468, 0, 195, cfactors, 6, 6);

            t3cfunc::ket_transform<1, 4>(sbuffer, 0, skbuffer, 468, 6);
...
#endif /* ThreeCenterElectronRepulsionRecIPG_hpp */
