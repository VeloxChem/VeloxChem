#ifndef ThreeCenterElectronRepulsionRecIGG_hpp
#define ThreeCenterElectronRepulsionRecIGG_hpp
...
/// @brief Computes (I|1/|r-r'||GG)  integrals for basis functions block and basis function pairs block.
...
template <class T>
inline auto
comp_electron_repulsion_igg(T& distributor,
                            ...
                            const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    CSimdArray<double> cbuffer(4060, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(18382, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 23316, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 420, pbuffer, 23736, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1008, pbuffer, 24324, 784, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1792, pbuffer, 25108, 1008, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2800, pbuffer, 26116, 1260, ket_width, ket_npgtos);
            ...
            t3cfunc::bra_transform<6>(skbuffer, 1300, cbuffer, 2800, 0, 8);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 1885, 0, 195, cfactors, 6, 6);

            t3ceri::comp_hrr_electron_repulsion_xph(skbuffer, 2470, 195, 468, cfactors, 6, 6);

            t3ceri::comp_hrr_electron_repulsion_xpi(skbuffer, 3289, 468, 832, cfactors, 6, 6);

            t3ceri::comp_hrr_electron_repulsion_xpk(skbuffer, 4381, 832, 1300, cfactors, 6, 6);

            t3ceri::comp_hrr_electron_repulsion_xdg(skbuffer, 5785, 1885, 2470, cfactors, 6, 6);

            t3ceri::comp_hrr_electron_repulsion_xdh(skbuffer, 6955, 2470, 3289, cfactors, 6, 6);

            t3ceri::comp_hrr_electron_repulsion_xdi(skbuffer, 8593, 3289, 4381, cfactors, 6, 6);

            t3ceri::comp_hrr_electron_repulsion_xfg(skbuffer, 10777, 5785, 6955, cfactors, 6, 6);

            t3ceri::comp_hrr_electron_repulsion_xfh(skbuffer, 12727, 6955, 8593, cfactors, 6, 6);

            t3ceri::comp_hrr_electron_repulsion_xgg(skbuffer, 15457, 10777, 12727, cfactors, 6, 6);

            t3cfunc::ket_transform<4, 4>(sbuffer, 0, skbuffer, 15457, 6);
...
#endif /* ThreeCenterElectronRepulsionRecIGG_hpp */
