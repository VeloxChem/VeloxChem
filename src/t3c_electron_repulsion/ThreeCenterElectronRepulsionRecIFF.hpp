#ifndef ThreeCenterElectronRepulsionRecIFF_hpp
#define ThreeCenterElectronRepulsionRecIFF_hpp
...
/// @brief Computes (I|1/|r-r'||FF)  integrals for basis functions block and basis function pairs block.
...
template <class T>
inline auto
comp_electron_repulsion_iff(T& distributor,
                            ...
                            const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    CSimdArray<double> skbuffer(6006, 1);

    // allocate aligned spherical integrals

    CSimdArray<double> sbuffer(637, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 11923, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 280, pbuffer, 12203, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 700, pbuffer, 12623, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1288, pbuffer, 13211, 784, ket_width, ket_npgtos);
            ...
            t3cfunc::bra_transform<6>(skbuffer, 598, cbuffer, 1288, 0, 6);

            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 962, 0, 130, cfactors, 6, 6);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 1352, 130, 325, cfactors, 6, 6);

            t3ceri::comp_hrr_electron_repulsion_xph(skbuffer, 1937, 325, 598, cfactors, 6, 6);

            t3ceri::comp_hrr_electron_repulsion_xdf(skbuffer, 2756, 962, 1352, cfactors, 6, 6);

            t3ceri::comp_hrr_electron_repulsion_xdg(skbuffer, 3536, 1352, 1937, cfactors, 6, 6);

            t3ceri::comp_hrr_electron_repulsion_xff(skbuffer, 4706, 2756, 3536, cfactors, 6, 6);

            t3cfunc::ket_transform<3, 3>(sbuffer, 0, skbuffer, 4706, 6);
...
#endif /* ThreeCenterElectronRepulsionRecIFF_hpp */
