#ifndef ThreeCenterElectronRepulsionRecSDG_hpp
#define ThreeCenterElectronRepulsionRecSDG_hpp
...
/// @brief Computes (S|1/|r-r'||DG)  integrals for basis functions block and basis function pairs block.
...
template <class T>
inline auto
comp_electron_repulsion_sdg(T& distributor,
                            ...
                            const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            t3ceri::comp_hrr_electron_repulsion_xph(skbuffer, 109, 15, 36, cfactors, 6, 0);

            t3ceri::comp_hrr_electron_repulsion_xdg(skbuffer, 172, 64, 109, cfactors, 6, 0);

            t3cfunc::ket_transform<2, 4>(sbuffer, 0, skbuffer, 172, 0);
...
#endif /* ThreeCenterElectronRepulsionRecSDG_hpp */
