#ifndef ThreeCenterElectronRepulsionRecHFG_hpp
#define ThreeCenterElectronRepulsionRecHFG_hpp
...
/// @brief Computes (H|1/|r-r'||FG)  integrals for basis functions block and basis function pairs block.
...
template <class T>
inline auto
comp_electron_repulsion_hfg(T& distributor,
                            ...
                            const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    CSimdArray<double> cbuffer(2100, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(7238, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 9310, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 315, pbuffer, 9625, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 756, pbuffer, 10066, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1344, pbuffer, 10654, 756, ket_width, ket_npgtos);
            ...
            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 1100, 0, 165, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xph(skbuffer, 1595, 165, 396, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xpi(skbuffer, 2288, 396, 704, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xdg(skbuffer, 3212, 1100, 1595, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xdh(skbuffer, 4202, 1595, 2288, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xfg(skbuffer, 5588, 3212, 4202, cfactors, 6, 5);

            t3cfunc::ket_transform<3, 4>(sbuffer, 0, skbuffer, 5588, 5);
...
#endif /* ThreeCenterElectronRepulsionRecHFG_hpp */
