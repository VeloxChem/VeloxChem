#ifndef ThreeCenterElectronRepulsionRecHFF_hpp
#define ThreeCenterElectronRepulsionRecHFF_hpp
...
/// @brief Computes (H|1/|r-r'||FF)  integrals for basis functions block and basis function pairs block.
...
template <class T>
inline auto
comp_electron_repulsion_hff(T& distributor,
                            ...
                            const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    CSimdArray<double> cbuffer(1554, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(5082, 1);

    // allocate aligned spherical integrals

    CSimdArray<double> sbuffer(539, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 6714, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 210, pbuffer, 6924, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 525, pbuffer, 7239, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 966, pbuffer, 7680, 588, ket_width, ket_npgtos);
            ...
            t3cfunc::bra_transform<5>(skbuffer, 506, cbuffer, 966, 0, 6);

            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 814, 0, 110, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 1144, 110, 275, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xph(skbuffer, 1639, 275, 506, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xdf(skbuffer, 2332, 814, 1144, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xdg(skbuffer, 2992, 1144, 1639, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xff(skbuffer, 3982, 2332, 2992, cfactors, 6, 5);

            t3cfunc::ket_transform<3, 3>(sbuffer, 0, skbuffer, 3982, 5);
...
#endif /* ThreeCenterElectronRepulsionRecHFF_hpp */
