#ifndef ThreeCenterElectronRepulsionRecSDF_hpp
#define ThreeCenterElectronRepulsionRecSDF_hpp
...
/// @brief Computes (S|1/|r-r'||DF)  integrals for basis functions block and basis function pairs block.
...
template <class T>
inline auto
comp_electron_repulsion_sdf(T& distributor,
                            ...
                            const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 76, 10, 25, cfactors, 6, 0);

            t3ceri::comp_hrr_electron_repulsion_xdf(skbuffer, 121, 46, 76, cfactors, 6, 0);

            t3cfunc::ket_transform<2, 3>(sbuffer, 0, skbuffer, 121, 0);
...
#endif /* ThreeCenterElectronRepulsionRecSDF_hpp */
