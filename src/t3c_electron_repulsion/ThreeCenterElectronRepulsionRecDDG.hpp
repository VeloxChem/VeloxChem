#ifndef ThreeCenterElectronRepulsionRecDDG_hpp
#define ThreeCenterElectronRepulsionRecDDG_hpp
...
/// @brief Computes (D|1/|r-r'||DG)  integrals for basis functions block and basis function pairs block.
...
template <class T>
inline auto
comp_electron_repulsion_ddg(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 90, pbuffer, 690, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 216, pbuffer, 816, 168, ket_width, ket_npgtos);
            ...
            t3ceri::comp_hrr_electron_repulsion_xph(skbuffer, 545, 75, 180, cfactors, 6, 2);

            t3ceri::comp_hrr_electron_repulsion_xdg(skbuffer, 860, 320, 545, cfactors, 6, 2);

            t3cfunc::ket_transform<2, 4>(sbuffer, 0, skbuffer, 860, 2);
...
#endif /* ThreeCenterElectronRepulsionRecDDG_hpp */
