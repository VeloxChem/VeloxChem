#ifndef ThreeCenterElectronRepulsionRecHGG_hpp
#define ThreeCenterElectronRepulsionRecHGG_hpp
...
/// @brief Computes (H|1/|r-r'||GG)  integrals for basis functions block and basis function pairs block.
...
template <class T>
inline auto
comp_electron_repulsion_hgg(T& distributor,
                            ...
                            const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    CSimdArray<double> cbuffer(3045, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(15554, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 13165, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 315, pbuffer, 13480, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 756, pbuffer, 13921, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1344, pbuffer, 14509, 756, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2100, pbuffer, 15265, 945, ket_width, ket_npgtos);
            ...
            t3cfunc::bra_transform<5>(skbuffer, 396, cbuffer, 756, 0, 6);

            t3cfunc::bra_transform<5>(skbuffer, 704, cbuffer, 1344, 0, 7);

            t3cfunc::bra_transform<5>(skbuffer, 1100, cbuffer, 2100, 0, 8);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 1595, 0, 165, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xph(skbuffer, 2090, 165, 396, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xpi(skbuffer, 2783, 396, 704, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xpk(skbuffer, 3707, 704, 1100, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xdg(skbuffer, 4895, 1595, 2090, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xdh(skbuffer, 5885, 2090, 2783, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xdi(skbuffer, 7271, 2783, 3707, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xfg(skbuffer, 9119, 4895, 5885, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xfh(skbuffer, 10769, 5885, 7271, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xgg(skbuffer, 13079, 9119, 10769, cfactors, 6, 5);

            t3cfunc::ket_transform<4, 4>(sbuffer, 0, skbuffer, 13079, 5);
...
#endif /* ThreeCenterElectronRepulsionRecHGG_hpp */
