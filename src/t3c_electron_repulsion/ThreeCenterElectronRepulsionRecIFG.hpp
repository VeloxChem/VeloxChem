#ifndef ThreeCenterElectronRepulsionRecIFG_hpp
#define ThreeCenterElectronRepulsionRecIFG_hpp
...
/// @brief Computes (I|1/|r-r'||FG)  integrals for basis functions block and basis function pairs block.
...
template <class T>
inline auto
comp_electron_repulsion_ifg(T& distributor,
                            ...
                            const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    CSimdArray<double> cbuffer(2800, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(8554, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 16536, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 420, pbuffer, 16956, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1008, pbuffer, 17544, 784, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1792, pbuffer, 18328, 1008, ket_width, ket_npgtos);

            }

            t3cfunc::bra_transform<6>(skbuffer, 0, cbuffer, 0, 0, 4);

            t3cfunc::bra_transform<6>(skbuffer, 195, cbuffer, 420, 0, 5);

            t3cfunc::bra_transform<6>(skbuffer, 468, cbuffer, 1008, 0, 6);

            t3cfunc::bra_transform<6>(skbuffer, 832, cbuffer, 1792, 0, 7);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 1300, 0, 195, cfactors, 6, 6);

            t3ceri::comp_hrr_electron_repulsion_xph(skbuffer, 1885, 195, 468, cfactors, 6, 6);

            t3ceri::comp_hrr_electron_repulsion_xpi(skbuffer, 2704, 468, 832, cfactors, 6, 6);

            t3ceri::comp_hrr_electron_repulsion_xdg(skbuffer, 3796, 1300, 1885, cfactors, 6, 6);

            t3ceri::comp_hrr_electron_repulsion_xdh(skbuffer, 4966, 1885, 2704, cfactors, 6, 6);

            t3ceri::comp_hrr_electron_repulsion_xfg(skbuffer, 6604, 3796, 4966, cfactors, 6, 6);

            t3cfunc::ket_transform<3, 4>(sbuffer, 0, skbuffer, 6604, 6);
...
#endif /* ThreeCenterElectronRepulsionRecIFG_hpp */
