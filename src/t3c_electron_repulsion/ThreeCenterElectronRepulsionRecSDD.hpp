#ifndef ThreeCenterElectronRepulsionRecSDD_hpp
#define ThreeCenterElectronRepulsionRecSDD_hpp
...
/// @brief Computes (S|1/|r-r'||DD)  integrals for basis functions block and basis function pairs block.
...
template <class T>
inline auto
comp_electron_repulsion_sdd(T& distributor,
                            ...
                            const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 49, 6, 16, cfactors, 6, 0);

            t3ceri::comp_hrr_electron_repulsion_xdd(skbuffer, 79, 31, 49, cfactors, 6, 0);

            t3cfunc::ket_transform<2, 2>(sbuffer, 0, skbuffer, 79, 0);
...
#endif /* ThreeCenterElectronRepulsionRecSDD_hpp */
