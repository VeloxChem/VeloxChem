#ifndef ThreeCenterElectronRepulsionRecDDF_hpp
#define ThreeCenterElectronRepulsionRecDDF_hpp
...
/// @brief Computes (D|1/|r-r'||DF)  integrals for basis functions block and basis function pairs block.
...
template <class T>
inline auto
comp_electron_repulsion_ddf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 60, pbuffer, 454, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 544, 126, ket_width, ket_npgtos);
            ...
            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 380, 50, 125, cfactors, 6, 2);

            t3ceri::comp_hrr_electron_repulsion_xdf(skbuffer, 605, 230, 380, cfactors, 6, 2);

            t3cfunc::ket_transform<2, 3>(sbuffer, 0, skbuffer, 605, 2);
...
#endif /* ThreeCenterElectronRepulsionRecDDF_hpp */
