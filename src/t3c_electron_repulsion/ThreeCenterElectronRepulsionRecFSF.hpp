#ifndef ThreeCenterElectronRepulsionRecFSF_hpp
#define ThreeCenterElectronRepulsionRecFSF_hpp
...
/// @brief Computes (F|1/|r-r'||SF)  integrals for basis functions block and basis function pairs block.
...
template <class T>
inline auto
comp_electron_repulsion_fsf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 288, 100, ket_width, ket_npgtos);

            }

            t3cfunc::bra_transform<3>(skbuffer, 0, cbuffer, 0, 0, 3);

            t3cfunc::ket_transform<0, 3>(sbuffer, 0, skbuffer, 0, 3);

            distributor.distribute(sbuffer, 0, bra_gto_indices, c_indices, d_indices, 3, 0, 3, j, ket_range);
        }
    }
...
#endif /* ThreeCenterElectronRepulsionRecFSF_hpp */
