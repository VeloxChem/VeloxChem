#ifndef ThreeCenterElectronRepulsionRecPPF_hpp
#define ThreeCenterElectronRepulsionRecPPF_hpp
...
/// @brief Computes (P|1/|r-r'||PF)  integrals for basis functions block and basis function pairs block.
...
template <class T>
inline auto
comp_electron_repulsion_ppf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t3ceri::comp_prim_electron_repulsion_psf(pbuffer, 70, 17, 35, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_psg(pbuffer, 100, 35, 55, pfactors, 26, a_exp);

                t2cfunc::reduce(cbuffer, 0, pbuffer, 70, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 30, pbuffer, 100, 45, ket_width, ket_npgtos);
            ...
            t3cfunc::ket_transform<1, 3>(sbuffer, 0, skbuffer, 75, 1);

            distributor.distribute(sbuffer, 0, bra_gto_indices, c_indices, d_indices, 1, 1, 3, j, ket_range);
        }
    }
...
#endif /* ThreeCenterElectronRepulsionRecPPF_hpp */
