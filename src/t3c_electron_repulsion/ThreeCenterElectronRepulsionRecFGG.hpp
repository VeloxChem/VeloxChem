#ifndef ThreeCenterElectronRepulsionRecFGG_hpp
#define ThreeCenterElectronRepulsionRecFGG_hpp
...
/// @brief Computes (F|1/|r-r'||GG)  integrals for basis functions block and basis function pairs block.
...
template <class T>
inline auto
comp_electron_repulsion_fgg(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 640, pbuffer, 3748, 360, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1000, pbuffer, 4108, 450, ket_width, ket_npgtos);
            ...
            t3cfunc::bra_transform<3>(skbuffer, 700, cbuffer, 1000, 0, 8);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 1015, 0, 105, cfactors, 6, 3);

            t3ceri::comp_hrr_electron_repulsion_xph(skbuffer, 1330, 105, 252, cfactors, 6, 3);

            t3ceri::comp_hrr_electron_repulsion_xpi(skbuffer, 1771, 252, 448, cfactors, 6, 3);

            t3ceri::comp_hrr_electron_repulsion_xpk(skbuffer, 2359, 448, 700, cfactors, 6, 3);

            t3ceri::comp_hrr_electron_repulsion_xdg(skbuffer, 3115, 1015, 1330, cfactors, 6, 3);

            t3ceri::comp_hrr_electron_repulsion_xdh(skbuffer, 3745, 1330, 1771, cfactors, 6, 3);

            t3ceri::comp_hrr_electron_repulsion_xdi(skbuffer, 4627, 1771, 2359, cfactors, 6, 3);

            t3ceri::comp_hrr_electron_repulsion_xfg(skbuffer, 5803, 3115, 3745, cfactors, 6, 3);

            t3ceri::comp_hrr_electron_repulsion_xfh(skbuffer, 6853, 3745, 4627, cfactors, 6, 3);

            t3ceri::comp_hrr_electron_repulsion_xgg(skbuffer, 8323, 5803, 6853, cfactors, 6, 3);

            t3cfunc::ket_transform<4, 4>(sbuffer, 0, skbuffer, 8323, 3);
...
#endif /* ThreeCenterElectronRepulsionRecFGG_hpp */
