#ifndef ThreeCenterOverlapGradientGeom001SumRecFP_hpp
#define ThreeCenterOverlapGradientGeom001SumRecFP_hpp
...
/// @brief Computes (F|GX(r)|d^(1)/dB^(1)P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_01_fp(T& distributor,
                                     ...
                                     const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(681, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    g3ovlrec::comp_prim_overlap_gradient_fs(pbuffer, 111, 41, 101, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fp(pbuffer, 141, 14, 41, 47, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fd(pbuffer, 171, 23, 47, 65, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_fd(pbuffer, 231, 65, 141, 171, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_geom_deriv_0x1_fp(pbuffer, 411, 111, 231, factors, 3);

                    t2cfunc::reduce(cbuffer, pbuffer, 411, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGradientGeom001SumRecFP_hpp */
