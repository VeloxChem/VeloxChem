#ifndef ThreeCenterOverlapGradientGeom001SumRecFD_hpp
#define ThreeCenterOverlapGradientGeom001SumRecFD_hpp
...
/// @brief Computes (F|GX(r)|d^(1)/dB^(1)D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_01_fd(T& distributor,
                                     ...
                                     const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(1331, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    g3ovlrec::comp_prim_overlap_gradient_fp(pbuffer, 241, 87, 201, 211, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fd(pbuffer, 331, 33, 87, 105, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ff(pbuffer, 391, 51, 105, 141, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_ff(pbuffer, 491, 141, 331, 391, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_geom_deriv_0x1_fd(pbuffer, 791, 241, 491, factors, 3);

                    t2cfunc::reduce(cbuffer, pbuffer, 791, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGradientGeom001SumRecFD_hpp */
