#ifndef ThreeCenterOverlapGradientGeom001SumRecDG_hpp
#define ThreeCenterOverlapGradientGeom001SumRecDG_hpp
...
/// @brief Computes (D|GX(r)|d^(1)/dB^(1)G)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_01_dg(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3ovlrec::comp_prim_overlap_pp(pbuffer, 57, 1, 2, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pd(pbuffer, 66, 2, 5, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pf(pbuffer, 84, 5, 11, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pg(pbuffer, 114, 11, 21, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ph(pbuffer, 159, 21, 36, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dd(pbuffer, 222, 5, 57, 66, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_df(pbuffer, 258, 11, 66, 84, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_df(pbuffer, 318, 84, 222, 258, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dg(pbuffer, 498, 21, 84, 114, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dh(pbuffer, 588, 36, 114, 159, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_dh(pbuffer, 714, 159, 498, 588, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_geom_deriv_0x1_dg(pbuffer, 1092, 318, 714, factors, 3);

                    t2cfunc::reduce(cbuffer, pbuffer, 1092, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGradientGeom001SumRecDG_hpp */
