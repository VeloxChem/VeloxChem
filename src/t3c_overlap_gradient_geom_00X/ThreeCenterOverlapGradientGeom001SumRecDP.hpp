#ifndef ThreeCenterOverlapGradientGeom001SumRecDP_hpp
#define ThreeCenterOverlapGradientGeom001SumRecDP_hpp
...
/// @brief Computes (D|GX(r)|d^(1)/dB^(1)P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_01_dp(T& distributor,
                                     ...
                                     const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(389, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    g3ovlrec::comp_prim_overlap_gradient_ds(pbuffer, 47, 11, 41, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dp(pbuffer, 65, 2, 11, 14, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dd(pbuffer, 83, 5, 14, 23, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_dd(pbuffer, 119, 23, 65, 83, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_geom_deriv_0x1_dp(pbuffer, 227, 47, 119, factors, 3);

                    t2cfunc::reduce(cbuffer, pbuffer, 227, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGradientGeom001SumRecDP_hpp */
