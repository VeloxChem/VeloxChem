#ifndef ThreeCenterOverlapGradientGeom001SumRecDF_hpp
#define ThreeCenterOverlapGradientGeom001SumRecDF_hpp
...
/// @brief Computes (D|GX(r)|d^(1)/dB^(1)F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_01_df(T& distributor,
                                     ...
                                     const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(1263, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    g3ovlrec::comp_prim_overlap_gradient_dd(pbuffer, 195, 48, 141, 159, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_df(pbuffer, 303, 11, 48, 66, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dg(pbuffer, 363, 21, 66, 96, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_dg(pbuffer, 453, 96, 303, 363, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_geom_deriv_0x1_df(pbuffer, 723, 195, 453, factors, 3);

                    t2cfunc::reduce(cbuffer, pbuffer, 723, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGradientGeom001SumRecDF_hpp */
