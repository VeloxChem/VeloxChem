#ifndef ThreeCenterOverlapGradientGeom001SumRecPD_hpp
#define ThreeCenterOverlapGradientGeom001SumRecPD_hpp
...
/// @brief Computes (P|GX(r)|d^(1)/dB^(1)D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_01_pd(T& distributor,
                                     ...
                                     const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(360, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    g3ovlrec::comp_prim_overlap_gradient_pp(pbuffer, 33, 2, 21, 24, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pd(pbuffer, 60, 2, 5, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pf(pbuffer, 78, 5, 11, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_pf(pbuffer, 108, 11, 60, 78, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_geom_deriv_0x1_pd(pbuffer, 198, 33, 108, factors, 3);

                    t2cfunc::reduce(cbuffer, pbuffer, 198, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGradientGeom001SumRecPD_hpp */
