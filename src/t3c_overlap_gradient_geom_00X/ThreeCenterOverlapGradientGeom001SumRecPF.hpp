#ifndef ThreeCenterOverlapGradientGeom001SumRecPF_hpp
#define ThreeCenterOverlapGradientGeom001SumRecPF_hpp
...
/// @brief Computes (P|GX(r)|d^(1)/dB^(1)F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_01_pf(T& distributor,
                                     ...
                                     const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(597, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    g3ovlrec::comp_prim_overlap_gradient_pd(pbuffer, 63, 5, 36, 45, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pf(pbuffer, 117, 5, 11, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pg(pbuffer, 147, 11, 21, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_pg(pbuffer, 192, 21, 117, 147, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_geom_deriv_0x1_pf(pbuffer, 327, 63, 192, factors, 3);

                    t2cfunc::reduce(cbuffer, pbuffer, 327, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGradientGeom001SumRecPF_hpp */
