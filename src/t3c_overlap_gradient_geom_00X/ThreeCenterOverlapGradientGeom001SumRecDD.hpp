#ifndef ThreeCenterOverlapGradientGeom001SumRecDD_hpp
#define ThreeCenterOverlapGradientGeom001SumRecDD_hpp
...
/// @brief Computes (D|GX(r)|d^(1)/dB^(1)D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_01_dd(T& distributor,
                                     ...
                                     const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(759, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    g3ovlrec::comp_prim_overlap_gradient_dp(pbuffer, 105, 24, 81, 87, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dd(pbuffer, 159, 5, 24, 33, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_df(pbuffer, 195, 11, 33, 51, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_df(pbuffer, 255, 51, 159, 195, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_geom_deriv_0x1_dd(pbuffer, 435, 105, 255, factors, 3);

                    t2cfunc::reduce(cbuffer, pbuffer, 435, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGradientGeom001SumRecDD_hpp */
