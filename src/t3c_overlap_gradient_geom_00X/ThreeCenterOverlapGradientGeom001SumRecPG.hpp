#ifndef ThreeCenterOverlapGradientGeom001SumRecPG_hpp
#define ThreeCenterOverlapGradientGeom001SumRecPG_hpp
...
/// @brief Computes (P|GX(r)|d^(1)/dB^(1)G)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_01_pg(T& distributor,
                                     ...
                                     const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(897, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3ovlrec::comp_prim_overlap_pd(pbuffer, 57, 2, 5, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pf(pbuffer, 75, 5, 11, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_pf(pbuffer, 105, 11, 57, 75, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pg(pbuffer, 195, 11, 21, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ph(pbuffer, 240, 21, 36, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_ph(pbuffer, 303, 36, 195, 240, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_geom_deriv_0x1_pg(pbuffer, 492, 105, 303, factors, 3);

                    t2cfunc::reduce(cbuffer, pbuffer, 492, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGradientGeom001SumRecPG_hpp */
