#ifndef ThreeCenterOverlapGradientGeom001SumRecPP_hpp
#define ThreeCenterOverlapGradientGeom001SumRecPP_hpp
...
/// @brief Computes (P|GX(r)|d^(1)/dB^(1)P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_01_pp(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    g3ovlrec::comp_prim_overlap_gradient_ps(pbuffer, 14, 1, 11, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pp(pbuffer, 23, 1, 2, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pd(pbuffer, 32, 2, 5, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_pd(pbuffer, 50, 5, 23, 32, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_geom_deriv_0x1_pp(pbuffer, 104, 14, 50, factors, 3);
...
#endif /* ThreeCenterOverlapGradientGeom001SumRecPP_hpp */
