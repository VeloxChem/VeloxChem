#ifndef ThreeCenterOverlapGradientGeom00XDriver_hpp
#define ThreeCenterOverlapGradientGeom00XDriver_hpp
...
#include "ThreeCenterOverlapGradientGeom001Func.hpp"
...
template <int N>
class CThreeCenterOverlapGradientGeom00XDriver
{
   public:
    /// @brief Creates an overlap derivative integrals driver.
    CThreeCenterOverlapGradientGeom00XDriver() = default;

    /// @brief The default copy constructor.
    /// @param other The overlap derivative integrals driver to be copied.
    CThreeCenterOverlapGradientGeom00XDriver(const CThreeCenterOverlapGradientGeom00XDriver &other) = delete;

    /// @brief The default move constructor.
    /// @param other The overlap derivative integrals driver  to be moved.
    CThreeCenterOverlapGradientGeom00XDriver(CThreeCenterOverlapGradientGeom00XDriver &&other) noexcept = delete;
    ...
    auto operator=(const CThreeCenterOverlapGradientGeom00XDriver &other) -> CThreeCenterOverlapGradientGeom00XDriver & = delete;

    /// @brief The default move assignment operator.
    /// @param other The overlap derivative integrals driver to be move assigned.
    /// @return The assigned overlap derivative integrals driver .
    auto operator=(CThreeCenterOverlapGradientGeom00XDriver &&other) noexcept -> CThreeCenterOverlapGradientGeom00XDriver & = delete;
    ...
    auto operator==(const CThreeCenterOverlapGradientGeom00XDriver &other) const -> bool = delete;
...
template <int N>
auto
CThreeCenterOverlapGradientGeom00XDriver<N>::compute(const std::vector<double>         &exponents,
                                                     const std::vector<double>         &factors,
                                                     const std::vector<TPoint<double>> &coordinates,
                                                     const CMolecularBasis             &basis,
                                                     const CMolecule                   &molecule,
...
#pragma omp task firstprivate(bra_gtos, ket_gtos, bra_indices, ket_indices)
                {
                    ...
                    if constexpr (N == 1)
                    {
                        g3ovlfunc::compute_geom_001(distributor, bra_gtos, ket_gtos, bra_indices, ket_indices, false);
                    }
                }
            });
        }
...
#endif /* ThreeCenterOverlapGradientGeom00XDriver_hpp */
