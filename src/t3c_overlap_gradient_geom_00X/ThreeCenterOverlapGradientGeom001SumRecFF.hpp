#ifndef ThreeCenterOverlapGradientGeom001SumRecFF_hpp
#define ThreeCenterOverlapGradientGeom001SumRecFF_hpp
...
/// @brief Computes (F|GX(r)|d^(1)/dB^(1)F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_gradient_geom_01_ff(T& distributor,
                                     ...
                                     const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(2221, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    g3ovlrec::comp_prim_overlap_gradient_fd(pbuffer, 441, 165, 351, 381, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ff(pbuffer, 621, 66, 165, 201, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fg(pbuffer, 721, 96, 201, 261, factors, 11, a_exp, exgtos[l]);

                    g3ovlrec::comp_prim_overlap_gradient_fg(pbuffer, 871, 261, 621, 721, factors, 20, a_exp, exgtos[l]);

                    t2cgeom::comp_geom_deriv_0x1_ff(pbuffer, 1321, 441, 871, factors, 3);

                    t2cfunc::reduce(cbuffer, pbuffer, 1321, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGradientGeom001SumRecFF_hpp */
