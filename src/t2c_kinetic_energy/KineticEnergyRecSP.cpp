...
auto
compKineticEnergySP(CSubMatrix*      matrix,
        ...
        for (int64_t j = bra_first; j < bra_last; j++)
        {
            ...
            for (int64_t k = 0; k < ket_npgtos; k++)
            {
                ...
                for (int64_t l = 0; l < bra_npgtos; l++)
                {
                    ...
                    kinrec::compPrimitiveKineticEnergySP(buffer_x,
                                                         buffer_y,
                                                         buffer_z,
                                                         bra_exp,
                                                         bra_norm,
...
auto
compPrimitiveKineticEnergySP(TDoubleArray&       buffer_x,
...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += fss * 2.0 * rpb_x * fz_0;

        fints_x[i] += ftt * rpb_x;

        fints_y[i] += fss * 2.0 * rpb_y * fz_0;

        fints_y[i] += ftt * rpb_y;

        fints_z[i] += fss * 2.0 * rpb_z * fz_0;
