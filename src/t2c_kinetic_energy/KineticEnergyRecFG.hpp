...
/// @brief Computes (F|T|G)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_kinetic_energy_fg(T&                               distributor,
                       ...
                       const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(856, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_pd(pbuffer, 70, 2, 8, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_pd(pbuffer, 88, 5, 14, 70, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_pf(pbuffer, 106, 8, 20, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_pf(pbuffer, 136, 14, 30, 106, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_pg(pbuffer, 166, 20, 40, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_pg(pbuffer, 211, 30, 55, 166, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_df(pbuffer, 256, 20, 70, 106, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_df(pbuffer, 316, 20, 30, 88, 136, 256, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_dg(pbuffer, 376, 40, 106, 166, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_dg(pbuffer, 466, 40, 55, 136, 211, 376, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_fg(pbuffer, 556, 166, 256, 376, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_fg(pbuffer, 706, 166, 211, 316, 466, 556, factors, 8, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 706, ket_width, ket_npgtos);
            }
