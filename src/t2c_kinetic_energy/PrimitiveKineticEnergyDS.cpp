#include "PrimitiveKineticEnergyDS.hpp"
...
auto
compPrimitiveKineticEnergyDS(TDoubleArray&       buffer_xx,
...
#pragma omp simd aligned(fints_xx, fints_xy, fints_xz, fints_yy, fints_yz, fints_zz, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += fss * (-fbe_0 * fz_0 + fe_0 * fz_0 + 4.0 * rpa_x * rpa_x * fz_0);

        fints_xx[i] += ftt * ((1.0 / 2.0) * fe_0 + rpa_x * rpa_x);

        fints_xy[i] += fss * 4.0 * rpa_y * rpa_x * fz_0;

        fints_xy[i] += ftt * rpa_y * rpa_x;

        fints_xz[i] += fss * 4.0 * rpa_z * rpa_x * fz_0;

        fints_xz[i] += ftt * rpa_z * rpa_x;

        fints_yy[i] += fss * (-fbe_0 * fz_0 + fe_0 * fz_0 + 4.0 * rpa_y * rpa_y * fz_0);

        fints_yy[i] += ftt * ((1.0 / 2.0) * fe_0 + rpa_y * rpa_y);

        fints_yz[i] += fss * 4.0 * rpa_z * rpa_y * fz_0;

        fints_yz[i] += ftt * rpa_z * rpa_y;

        fints_zz[i] += fss * (-fbe_0 * fz_0 + fe_0 * fz_0 + 4.0 * rpa_z * rpa_z * fz_0);

        fints_zz[i] += ftt * ((1.0 / 2.0) * fe_0 + rpa_z * rpa_z);
    }
}
