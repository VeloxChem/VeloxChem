#include "PrimitiveKineticEnergySD.hpp"
...
auto
compPrimitiveKineticEnergySD(TDoubleArray&       buffer_xx,
...
#pragma omp simd aligned(fints_xx, fints_xy, fints_xz, fints_yy, fints_yz, fints_zz, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += fss * (fe_0 * fz_0 - fke_0 * fz_0 + 4.0 * rpb_x * rpb_x * fz_0);

        fints_xx[i] += ftt * ((1.0 / 2.0) * fe_0 + rpb_x * rpb_x);

        fints_xy[i] += fss * 4.0 * rpb_y * rpb_x * fz_0;

        fints_xy[i] += ftt * rpb_y * rpb_x;

        fints_xz[i] += fss * 4.0 * rpb_z * rpb_x * fz_0;

        fints_xz[i] += ftt * rpb_z * rpb_x;

        fints_yy[i] += fss * (fe_0 * fz_0 - fke_0 * fz_0 + 4.0 * rpb_y * rpb_y * fz_0);

        fints_yy[i] += ftt * ((1.0 / 2.0) * fe_0 + rpb_y * rpb_y);

        fints_yz[i] += fss * 4.0 * rpb_z * rpb_y * fz_0;

        fints_yz[i] += ftt * rpb_z * rpb_y;

        fints_zz[i] += fss * (fe_0 * fz_0 - fke_0 * fz_0 + 4.0 * rpb_z * rpb_z * fz_0);

        fints_zz[i] += ftt * ((1.0 / 2.0) * fe_0 + rpb_z * rpb_z);
    }
}
