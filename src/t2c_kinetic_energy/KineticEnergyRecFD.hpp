...
/// @brief Computes (F|T|D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_kinetic_energy_fd(T&                               distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, pbuffer, 248, ket_width, ket_npgtos);
            }
