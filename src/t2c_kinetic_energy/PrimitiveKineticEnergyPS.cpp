#include "PrimitiveKineticEnergyPS.hpp"
...
auto
compPrimitiveKineticEnergyPS(TDoubleArray&       buffer_x,
...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += fss * 2.0 * rpa_x * fz_0;

        fints_x[i] += ftt * rpa_x;

        fints_y[i] += fss * 2.0 * rpa_y * fz_0;

        fints_y[i] += ftt * rpa_y;

        fints_z[i] += fss * 2.0 * rpa_z * fz_0;
