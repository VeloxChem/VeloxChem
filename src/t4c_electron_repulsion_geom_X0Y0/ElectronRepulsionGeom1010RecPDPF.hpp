#ifndef ElectronRepulsionGeom1010RecPDPF_hpp
#define ElectronRepulsionGeom1010RecPDPF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(PD|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_pdpf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(6105, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 336, pbuffer, 1067, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 396, pbuffer, 1955, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 496, pbuffer, 2875, 150, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 60, pbuffer, 1067, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 120, pbuffer, 1247, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 210, pbuffer, 1517, 126, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 646, pbuffer, 1067, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 706, pbuffer, 1247, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 796, pbuffer, 1517, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 922, pbuffer, 1955, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1022, pbuffer, 2155, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1172, pbuffer, 2455, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1382, pbuffer, 2875, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1532, pbuffer, 3025, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1757, pbuffer, 3250, 315, ket_width, ket_npgtos);

            }

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 0, cbuffer, 60, 120, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 180, cbuffer, 120, 210, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 450, cbuffer, 0, 0, 180, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 990, cbuffer, 646, 706, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 1170, cbuffer, 706, 796, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 1440, cbuffer, 336, 990, 1170, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1980, cbuffer, 922, 1022, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 2280, cbuffer, 1022, 1172, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 2730, cbuffer, 396, 1980, 2280, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 3630, cbuffer, 1382, 1532, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 4080, cbuffer, 1532, 1757, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 4755, cbuffer, 496, 3630, 4080, cfactors, 6, 0, 4);
            ...
            t4cfunc::ket_transform<1, 3>(skbuffer, 6804, ckbuffer, 1440, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 6930, ckbuffer, 1620, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 7056, ckbuffer, 1800, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 7182, ckbuffer, 2730, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 7392, ckbuffer, 3030, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 7602, ckbuffer, 3330, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 7812, ckbuffer, 4755, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 8127, ckbuffer, 5205, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 8442, ckbuffer, 5655, 0, 4);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 378, 6804, 7182, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 1512, 7182, 7812, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pdxx(skbuffer, 3402, 0, 378, 1512, r_ab, 1, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 0, skbuffer, 3402, 1, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 315, skbuffer, 3780, 1, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 630, skbuffer, 4158, 1, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 945, skbuffer, 4536, 1, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 1260, skbuffer, 4914, 1, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 1575, skbuffer, 5292, 1, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 1890, skbuffer, 5670, 1, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 2205, skbuffer, 6048, 1, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 2520, skbuffer, 6426, 1, 3);
...
#endif /* ElectronRepulsionGeom1010RecPDPF_hpp */
