#ifndef ElectronRepulsionGeom1010RecSSPF_hpp
#define ElectronRepulsionGeom1010RecSSPF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SS|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_sspf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                erirec::comp_prim_electron_repulsion_spsg(pbuffer, 212, 65, 95, 110, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsh(pbuffer, 257, 110, 140, 161, pfactors, 26, r_pb, a_exp, b_exp);
                ...
                t2cfunc::reduce(cbuffer, 10, pbuffer, 182, 30, ket_width, ket_npgtos);
                ...
                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {212, 257});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {257, 320});
                ...
                t2cfunc::reduce(cbuffer, 86, pbuffer, 182, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 116, pbuffer, 212, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 161, pbuffer, 257, 63, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<1, 3>(skbuffer, 189, ckbuffer, 75, 0, 0);

            t4cfunc::ket_transform<1, 3>(skbuffer, 210, ckbuffer, 105, 0, 0);

            t4cfunc::ket_transform<1, 3>(skbuffer, 231, ckbuffer, 135, 0, 0);
            ...
            erirec::comp_bra_geom1010_hrr_electron_repulsion_ssxx(skbuffer, 0, 189, 252, r_ab, 1, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 0, skbuffer, 0, 1, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 21, skbuffer, 21, 1, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 42, skbuffer, 42, 1, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 63, skbuffer, 63, 1, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 84, skbuffer, 84, 1, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 105, skbuffer, 105, 1, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 126, skbuffer, 126, 1, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 147, skbuffer, 147, 1, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 168, skbuffer, 168, 1, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 0, 1, 3, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1010RecSSPF_hpp */
