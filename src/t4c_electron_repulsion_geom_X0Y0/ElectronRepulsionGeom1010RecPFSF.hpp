#ifndef ElectronRepulsionGeom1010RecPFSF_hpp
#define ElectronRepulsionGeom1010RecPFSF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(PF|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_pfsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(1400, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(1680, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 1625, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 100, pbuffer, 1925, 150, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 500, pbuffer, 2465, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 650, pbuffer, 2765, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 875, pbuffer, 3215, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1085, pbuffer, 3425, 315, ket_width, ket_npgtos);

            }

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 0, cbuffer, 0, 100, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 300, cbuffer, 250, 350, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 600, cbuffer, 500, 650, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1050, cbuffer, 875, 1085, cfactors, 6, 0, 5);
            ...
            t4cfunc::ket_transform<0, 3>(skbuffer, 3675, ckbuffer, 300, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 3745, ckbuffer, 400, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 3815, ckbuffer, 500, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 3885, ckbuffer, 600, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 3990, ckbuffer, 750, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 4095, ckbuffer, 900, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 4200, ckbuffer, 1050, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 4347, ckbuffer, 1260, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 4494, ckbuffer, 1470, 0, 5);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 210, 3675, 3885, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sgxx(skbuffer, 840, 3885, 4200, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pfxx(skbuffer, 1785, 0, 210, 840, r_ab, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 1785, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 147, skbuffer, 1995, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 294, skbuffer, 2205, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 441, skbuffer, 2415, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 588, skbuffer, 2625, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 735, skbuffer, 2835, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 882, skbuffer, 3045, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1029, skbuffer, 3255, 0, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1176, skbuffer, 3465, 0, 3);
...
#endif /* ElectronRepulsionGeom1010RecPFSF_hpp */
