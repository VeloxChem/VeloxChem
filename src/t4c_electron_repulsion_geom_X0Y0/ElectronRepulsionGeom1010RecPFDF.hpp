#ifndef ElectronRepulsionGeom1010RecPFDF_hpp
#define ElectronRepulsionGeom1010RecPFDF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(PF|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_pfdf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(32088, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 990, pbuffer, 3921, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1090, pbuffer, 4221, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1240, pbuffer, 6231, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1390, pbuffer, 6531, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1615, pbuffer, 8451, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1825, pbuffer, 8661, 315, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 250, pbuffer, 3921, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 350, pbuffer, 4221, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 500, pbuffer, 4671, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 710, pbuffer, 5301, 280, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 2140, pbuffer, 3921, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2240, pbuffer, 4221, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2390, pbuffer, 4671, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2600, pbuffer, 5301, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2880, pbuffer, 6231, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3030, pbuffer, 6531, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3255, pbuffer, 6981, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3570, pbuffer, 7611, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3990, pbuffer, 8451, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4200, pbuffer, 8661, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4515, pbuffer, 8976, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4956, pbuffer, 9417, 588, ket_width, ket_npgtos);

            }

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1380, cbuffer, 0, 100, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 7110, cbuffer, 990, 1090, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 13530, cbuffer, 1240, 1390, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 22953, cbuffer, 1615, 1825, cfactors, 6, 0, 5);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 750, cbuffer, 500, 710, cfactors, 6, 0, 3);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 5730, cbuffer, 2140, 2240, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 6030, cbuffer, 2240, 2390, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 6480, cbuffer, 2390, 2600, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 7410, cbuffer, 990, 5730, 6030, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpg(ckbuffer, 8310, cbuffer, 1090, 6030, 6480, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxdf(ckbuffer, 9660, 7110, 7410, 8310, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 11460, cbuffer, 2880, 3030, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 11910, cbuffer, 3030, 3255, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 12585, cbuffer, 3255, 3570, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 13980, cbuffer, 1240, 11460, 11910, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpg(ckbuffer, 15330, cbuffer, 1390, 11910, 12585, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxdf(ckbuffer, 17355, 13530, 13980, 15330, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 20055, cbuffer, 3990, 4200, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 20685, cbuffer, 4200, 4515, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 21630, cbuffer, 4515, 4956, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 23583, cbuffer, 1615, 20055, 20685, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpg(ckbuffer, 25473, cbuffer, 1825, 20685, 21630, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxdf(ckbuffer, 28308, 22953, 23583, 25473, cfactors, 6, 0, 5);
            ...
            t4cfunc::ket_transform<2, 3>(skbuffer, 18375, ckbuffer, 9660, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 18725, ckbuffer, 10260, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 19075, ckbuffer, 10860, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 19425, ckbuffer, 17355, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 19950, ckbuffer, 18255, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 20475, ckbuffer, 19155, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 21000, ckbuffer, 28308, 0, 5);

            t4cfunc::ket_transform<2, 3>(skbuffer, 21735, ckbuffer, 29568, 0, 5);

            t4cfunc::ket_transform<2, 3>(skbuffer, 22470, ckbuffer, 30828, 0, 5);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 1050, 18375, 19425, r_ab, 2, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sgxx(skbuffer, 4200, 19425, 21000, r_ab, 2, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pfxx(skbuffer, 8925, 0, 1050, 4200, r_ab, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 8925, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 735, skbuffer, 9975, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1470, skbuffer, 11025, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 2205, skbuffer, 12075, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 2940, skbuffer, 13125, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 3675, skbuffer, 14175, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 4410, skbuffer, 15225, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 5145, skbuffer, 16275, 2, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 5880, skbuffer, 17325, 2, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 3, 2, 3, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1010RecPFDF_hpp */
