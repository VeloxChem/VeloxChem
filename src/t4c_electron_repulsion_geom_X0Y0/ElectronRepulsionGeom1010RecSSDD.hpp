...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SS|1/|r-r'||DD)  integral derivatives.
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(1452, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                erirec::comp_prim_electron_repulsion_spsd(pbuffer, 182, 10, 25, 31, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsf(pbuffer, 200, 31, 55, 65, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsg(pbuffer, 230, 65, 95, 110, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsh(pbuffer, 275, 110, 140, 161, pfactors, 26, r_pb, a_exp, b_exp);

                pbuffer.scale(2.0 * a_exp, {25, 31});

                pbuffer.scale(2.0 * a_exp, {55, 65});

                pbuffer.scale(2.0 * a_exp, {182, 200});

                pbuffer.scale(2.0 * a_exp, {200, 230});

                t2cfunc::reduce(cbuffer, 0, pbuffer, 25, 6, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 6, pbuffer, 55, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 16, pbuffer, 182, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 34, pbuffer, 200, 30, ket_width, ket_npgtos);
                ...
                pbuffer.scale(pfactors, 0, 2.0, {182, 200});

                pbuffer.scale(pfactors, 0, 2.0, {200, 230});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {230, 275});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {275, 338});

                t2cfunc::reduce(cbuffer, 64, pbuffer, 25, 6, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 70, pbuffer, 55, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 80, pbuffer, 95, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 95, pbuffer, 140, 21, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 116, pbuffer, 182, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 134, pbuffer, 200, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 164, pbuffer, 230, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 209, pbuffer, 275, 63, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<2, 2>(skbuffer, 225, ckbuffer, 255, 0, 0);

            t4cfunc::ket_transform<2, 2>(skbuffer, 250, ckbuffer, 291, 0, 0);

            t4cfunc::ket_transform<2, 2>(skbuffer, 275, ckbuffer, 327, 0, 0);
            ...
            erirec::comp_bra_geom1010_hrr_electron_repulsion_ssxx(skbuffer, 0, 225, 300, r_ab, 2, 2);

            t4cfunc::bra_transform<0, 0>(sbuffer, 0, skbuffer, 0, 2, 2);

            t4cfunc::bra_transform<0, 0>(sbuffer, 25, skbuffer, 25, 2, 2);

            t4cfunc::bra_transform<0, 0>(sbuffer, 50, skbuffer, 50, 2, 2);

            t4cfunc::bra_transform<0, 0>(sbuffer, 75, skbuffer, 75, 2, 2);

            t4cfunc::bra_transform<0, 0>(sbuffer, 100, skbuffer, 100, 2, 2);

            t4cfunc::bra_transform<0, 0>(sbuffer, 125, skbuffer, 125, 2, 2);

            t4cfunc::bra_transform<0, 0>(sbuffer, 150, skbuffer, 150, 2, 2);

            t4cfunc::bra_transform<0, 0>(sbuffer, 175, skbuffer, 175, 2, 2);

            t4cfunc::bra_transform<0, 0>(sbuffer, 200, skbuffer, 200, 2, 2);
