#ifndef ElectronRepulsionGeom1010RecSDSF_hpp
#define ElectronRepulsionGeom1010RecSDSF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SD|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_sdsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(400, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(480, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 481, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 60, pbuffer, 601, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 781, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 250, pbuffer, 881, 150, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<0, 3>(skbuffer, 378, ckbuffer, 0, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 420, ckbuffer, 60, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 462, ckbuffer, 120, 0, 2);
            ...
            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 0, 378, 504, r_ab, 0, 3);
...
#endif /* ElectronRepulsionGeom1010RecSDSF_hpp */
