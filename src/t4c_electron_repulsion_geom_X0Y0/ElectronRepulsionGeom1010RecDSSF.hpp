#ifndef ElectronRepulsionGeom1010RecDSSF_hpp
#define ElectronRepulsionGeom1010RecDSSF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(DS|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_dssf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(pfactors, 0, 2.0, {310, 355});
                ...
                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {481, 541});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {601, 691});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {781, 881});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {881, 1031});
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 120, cbuffer, 100, 110, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 150, cbuffer, 125, 155, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 240, cbuffer, 200, 260, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 420, cbuffer, 350, 450, cfactors, 6, 0, 3);
            ...
            t4cfunc::ket_transform<0, 3>(skbuffer, 1911, ckbuffer, 120, 0, 0);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1918, ckbuffer, 130, 0, 0);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1925, ckbuffer, 140, 0, 0);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1932, ckbuffer, 150, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1953, ckbuffer, 180, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1974, ckbuffer, 210, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1995, ckbuffer, 240, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2037, ckbuffer, 300, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2079, ckbuffer, 360, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2121, ckbuffer, 420, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2191, ckbuffer, 520, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2261, ckbuffer, 620, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 714, 0, 84, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 735, 7, 105, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 756, 14, 126, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ssxx(skbuffer, 21, 1911, 1932, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 147, 1932, 1995, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 336, 1995, 2121, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_psxx(skbuffer, 777, 0, 21, 147, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ppxx(skbuffer, 966, 84, 147, 336, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dsxx(skbuffer, 1533, 714, 777, 966, r_ab, 0, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 0, skbuffer, 1533, 0, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 35, skbuffer, 1575, 0, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 70, skbuffer, 1617, 0, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 105, skbuffer, 1659, 0, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 140, skbuffer, 1701, 0, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 175, skbuffer, 1743, 0, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 210, skbuffer, 1785, 0, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 245, skbuffer, 1827, 0, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 280, skbuffer, 1869, 0, 3);
...
#endif /* ElectronRepulsionGeom1010RecDSSF_hpp */
