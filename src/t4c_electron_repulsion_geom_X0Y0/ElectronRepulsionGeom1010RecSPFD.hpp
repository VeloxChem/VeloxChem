#ifndef ElectronRepulsionGeom1010RecSPFD_hpp
#define ElectronRepulsionGeom1010RecSPFD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SP|1/|r-r'||FD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_spfd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> pbuffer(1347, ket_npgtos);
    ...
    CSimdArray<double> ckbuffer(8883, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(1890, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                erirec::comp_prim_electron_repulsion_spsp(pbuffer, 378, 2, 12, 15, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsd(pbuffer, 387, 12, 33, 39, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsd(pbuffer, 405, 15, 39, 45, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsf(pbuffer, 423, 39, 75, 85, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsf(pbuffer, 453, 45, 85, 95, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsg(pbuffer, 483, 85, 135, 150, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsg(pbuffer, 528, 95, 150, 165, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsh(pbuffer, 573, 150, 210, 231, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsh(pbuffer, 636, 165, 231, 252, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsi(pbuffer, 699, 231, 294, 322, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsi(pbuffer, 783, 252, 322, 350, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_sdsd(pbuffer, 867, 33, 39, 378, 387, 405, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_sdsf(pbuffer, 903, 75, 85, 405, 423, 453, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_sdsg(pbuffer, 963, 135, 150, 453, 483, 528, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_sdsh(pbuffer, 1053, 210, 231, 528, 573, 636, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_sdsi(pbuffer, 1179, 294, 322, 636, 699, 783, pfactors, 26, r_pb, a_exp, b_exp);

                pbuffer.scale(2.0 * a_exp, {387, 405});

                pbuffer.scale(2.0 * a_exp, {423, 453});

                pbuffer.scale(2.0 * a_exp, {483, 528});

                pbuffer.scale(2.0 * a_exp, {867, 903});

                pbuffer.scale(2.0 * a_exp, {903, 963});

                pbuffer.scale(2.0 * a_exp, {963, 1053});

                t2cfunc::reduce(cbuffer, 0, pbuffer, 387, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 18, pbuffer, 423, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 48, pbuffer, 483, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 93, pbuffer, 867, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 129, pbuffer, 903, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 189, pbuffer, 963, 90, ket_width, ket_npgtos);

                pbuffer.scale(pfactors, 0, 2.0, {387, 405});

                pbuffer.scale(pfactors, 0, 2.0, {423, 453});

                pbuffer.scale(pfactors, 0, 2.0, {483, 528});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {573, 636});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {699, 783});

                pbuffer.scale(pfactors, 0, 2.0, {867, 903});

                pbuffer.scale(pfactors, 0, 2.0, {903, 963});

                pbuffer.scale(pfactors, 0, 2.0, {963, 1053});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {1053, 1179});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {1179, 1347});

                t2cfunc::reduce(cbuffer, 279, pbuffer, 387, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 297, pbuffer, 423, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 327, pbuffer, 483, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 372, pbuffer, 573, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 435, pbuffer, 699, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 519, pbuffer, 867, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 555, pbuffer, 903, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 615, pbuffer, 963, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 705, pbuffer, 1053, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 831, pbuffer, 1179, 168, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<3, 2>(skbuffer, 945, ckbuffer, 2421, 0, 1);

            t4cfunc::ket_transform<3, 2>(skbuffer, 1050, ckbuffer, 2601, 0, 1);

            t4cfunc::ket_transform<3, 2>(skbuffer, 1155, ckbuffer, 2781, 0, 1);
            ...
            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 0, 945, 1260, r_ab, 3, 2);

            t4cfunc::bra_transform<0, 1>(sbuffer, 0, skbuffer, 0, 3, 2);

            t4cfunc::bra_transform<0, 1>(sbuffer, 105, skbuffer, 105, 3, 2);

            t4cfunc::bra_transform<0, 1>(sbuffer, 210, skbuffer, 210, 3, 2);

            t4cfunc::bra_transform<0, 1>(sbuffer, 315, skbuffer, 315, 3, 2);

            t4cfunc::bra_transform<0, 1>(sbuffer, 420, skbuffer, 420, 3, 2);

            t4cfunc::bra_transform<0, 1>(sbuffer, 525, skbuffer, 525, 3, 2);

            t4cfunc::bra_transform<0, 1>(sbuffer, 630, skbuffer, 630, 3, 2);

            t4cfunc::bra_transform<0, 1>(sbuffer, 735, skbuffer, 735, 3, 2);

            t4cfunc::bra_transform<0, 1>(sbuffer, 840, skbuffer, 840, 3, 2);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 1, 3, 2, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1010RecSPFD_hpp */
