#ifndef ElectronRepulsionGeom1010RecPPSF_hpp
#define ElectronRepulsionGeom1010RecPPSF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(PP|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_ppsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(660, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 75, pbuffer, 220, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 105, pbuffer, 310, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 481, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 210, pbuffer, 601, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 300, pbuffer, 781, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 400, pbuffer, 881, 150, ket_width, ket_npgtos);

            }

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 0, cbuffer, 0, 30, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 90, cbuffer, 75, 105, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 180, cbuffer, 150, 210, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 360, cbuffer, 300, 400, cfactors, 6, 0, 3);
            ...
            t4cfunc::ket_transform<0, 3>(skbuffer, 1197, ckbuffer, 90, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1218, ckbuffer, 120, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1239, ckbuffer, 150, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1260, ckbuffer, 180, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1302, ckbuffer, 240, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1344, ckbuffer, 300, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1386, ckbuffer, 360, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1456, ckbuffer, 460, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1526, ckbuffer, 560, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 63, 1197, 1260, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 252, 1260, 1386, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ppxx(skbuffer, 630, 0, 63, 252, r_ab, 0, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 630, 0, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 63, skbuffer, 693, 0, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 126, skbuffer, 756, 0, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 189, skbuffer, 819, 0, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 252, skbuffer, 882, 0, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 315, skbuffer, 945, 0, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 378, skbuffer, 1008, 0, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 441, skbuffer, 1071, 0, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 504, skbuffer, 1134, 0, 3);
...
#endif /* ElectronRepulsionGeom1010RecPPSF_hpp */
