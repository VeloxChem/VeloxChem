...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SP|1/|r-r'||PP)  integral derivatives.
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(486, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 78, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 9, pbuffer, 192, 18, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 27, pbuffer, 78, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 36, pbuffer, 96, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 54, pbuffer, 132, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 84, pbuffer, 192, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 102, pbuffer, 210, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 138, pbuffer, 246, 60, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<1, 1>(skbuffer, 243, ckbuffer, 81, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 270, ckbuffer, 108, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 297, ckbuffer, 135, 0, 1);
            ...
            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 0, 243, 324, r_ab, 1, 1);
