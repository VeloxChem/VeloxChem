#ifndef ElectronRepulsionGeom1010RecSSFD_hpp
#define ElectronRepulsionGeom1010RecSSFD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SS|1/|r-r'||FD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_ssfd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> pbuffer(534, ket_npgtos);
    ...
    CSimdArray<double> ckbuffer(3948, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                erirec::comp_prim_electron_repulsion_spsd(pbuffer, 294, 11, 29, 35, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsf(pbuffer, 312, 35, 65, 75, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsg(pbuffer, 342, 75, 115, 130, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsh(pbuffer, 387, 130, 175, 196, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsi(pbuffer, 450, 196, 238, 266, pfactors, 26, r_pb, a_exp, b_exp);
                ...
                pbuffer.scale(2.0 * a_exp, {294, 312});

                pbuffer.scale(2.0 * a_exp, {312, 342});

                pbuffer.scale(2.0 * a_exp, {342, 387});
                ...
                t2cfunc::reduce(cbuffer, 31, pbuffer, 294, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 49, pbuffer, 312, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 79, pbuffer, 342, 45, ket_width, ket_npgtos);
                ...
                pbuffer.scale(pfactors, 0, 2.0, {294, 312});

                pbuffer.scale(pfactors, 0, 2.0, {312, 342});

                pbuffer.scale(pfactors, 0, 2.0, {342, 387});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {387, 450});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {450, 534});
                ...
                t2cfunc::reduce(cbuffer, 155, pbuffer, 175, 21, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 176, pbuffer, 238, 28, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 204, pbuffer, 294, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 222, pbuffer, 312, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 252, pbuffer, 342, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 297, pbuffer, 387, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 360, pbuffer, 450, 84, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<3, 2>(skbuffer, 315, ckbuffer, 807, 0, 0);

            t4cfunc::ket_transform<3, 2>(skbuffer, 350, ckbuffer, 867, 0, 0);

            t4cfunc::ket_transform<3, 2>(skbuffer, 385, ckbuffer, 927, 0, 0);
            ...
            erirec::comp_bra_geom1010_hrr_electron_repulsion_ssxx(skbuffer, 0, 315, 420, r_ab, 3, 2);

            t4cfunc::bra_transform<0, 0>(sbuffer, 0, skbuffer, 0, 3, 2);

            t4cfunc::bra_transform<0, 0>(sbuffer, 35, skbuffer, 35, 3, 2);

            t4cfunc::bra_transform<0, 0>(sbuffer, 70, skbuffer, 70, 3, 2);

            t4cfunc::bra_transform<0, 0>(sbuffer, 105, skbuffer, 105, 3, 2);

            t4cfunc::bra_transform<0, 0>(sbuffer, 140, skbuffer, 140, 3, 2);

            t4cfunc::bra_transform<0, 0>(sbuffer, 175, skbuffer, 175, 3, 2);

            t4cfunc::bra_transform<0, 0>(sbuffer, 210, skbuffer, 210, 3, 2);

            t4cfunc::bra_transform<0, 0>(sbuffer, 245, skbuffer, 245, 3, 2);

            t4cfunc::bra_transform<0, 0>(sbuffer, 280, skbuffer, 280, 3, 2);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 0, 3, 2, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1010RecSSFD_hpp */
