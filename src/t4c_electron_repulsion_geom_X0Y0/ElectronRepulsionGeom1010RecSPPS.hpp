#ifndef ElectronRepulsionGeom1010RecSPPS_hpp
#define ElectronRepulsionGeom1010RecSPPS_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SP|1/|r-r'||PS)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_spps(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(189, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 3, pbuffer, 95, 6, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 9, pbuffer, 35, 3, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 12, pbuffer, 41, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 21, pbuffer, 59, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 39, pbuffer, 95, 6, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 45, pbuffer, 101, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 63, pbuffer, 119, 36, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<1, 0>(skbuffer, 81, ckbuffer, 36, 0, 1);

            t4cfunc::ket_transform<1, 0>(skbuffer, 90, ckbuffer, 45, 0, 1);

            t4cfunc::ket_transform<1, 0>(skbuffer, 99, ckbuffer, 54, 0, 1);
            ...
            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 0, 81, 108, r_ab, 1, 0);

            t4cfunc::bra_transform<0, 1>(sbuffer, 0, skbuffer, 0, 1, 0);

            t4cfunc::bra_transform<0, 1>(sbuffer, 9, skbuffer, 9, 1, 0);

            t4cfunc::bra_transform<0, 1>(sbuffer, 18, skbuffer, 18, 1, 0);

            t4cfunc::bra_transform<0, 1>(sbuffer, 27, skbuffer, 27, 1, 0);

            t4cfunc::bra_transform<0, 1>(sbuffer, 36, skbuffer, 36, 1, 0);

            t4cfunc::bra_transform<0, 1>(sbuffer, 45, skbuffer, 45, 1, 0);

            t4cfunc::bra_transform<0, 1>(sbuffer, 54, skbuffer, 54, 1, 0);

            t4cfunc::bra_transform<0, 1>(sbuffer, 63, skbuffer, 63, 1, 0);

            t4cfunc::bra_transform<0, 1>(sbuffer, 72, skbuffer, 72, 1, 0);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 1, 1, 0, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1010RecSPPS_hpp */
