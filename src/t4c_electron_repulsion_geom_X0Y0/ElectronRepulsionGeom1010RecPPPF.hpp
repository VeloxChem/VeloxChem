#ifndef ElectronRepulsionGeom1010RecPPPF_hpp
#define ElectronRepulsionGeom1010RecPPPF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(PP|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_pppf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(3630, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 168, pbuffer, 339, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 198, pbuffer, 789, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 258, pbuffer, 1341, 100, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 30, pbuffer, 339, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 60, pbuffer, 429, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 105, pbuffer, 564, 63, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 358, pbuffer, 339, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 388, pbuffer, 429, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 433, pbuffer, 564, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 496, pbuffer, 789, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 556, pbuffer, 909, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 646, pbuffer, 1089, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 772, pbuffer, 1341, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 872, pbuffer, 1441, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1022, pbuffer, 1591, 210, ket_width, ket_npgtos);

            }

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 0, cbuffer, 30, 60, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 90, cbuffer, 60, 105, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 225, cbuffer, 0, 0, 90, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 495, cbuffer, 358, 388, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 585, cbuffer, 388, 433, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 720, cbuffer, 168, 495, 585, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 990, cbuffer, 496, 556, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 1170, cbuffer, 556, 646, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 1440, cbuffer, 198, 990, 1170, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1980, cbuffer, 772, 872, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 2280, cbuffer, 872, 1022, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 2730, cbuffer, 258, 1980, 2280, cfactors, 6, 0, 3);
            ...
            t4cfunc::ket_transform<1, 3>(skbuffer, 3591, ckbuffer, 720, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 3654, ckbuffer, 810, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 3717, ckbuffer, 900, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 3780, ckbuffer, 1440, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 3906, ckbuffer, 1620, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 4032, ckbuffer, 1800, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 4158, ckbuffer, 2730, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 4368, ckbuffer, 3030, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 4578, ckbuffer, 3330, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 189, 3591, 3780, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 756, 3780, 4158, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ppxx(skbuffer, 1890, 0, 189, 756, r_ab, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 1890, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 189, skbuffer, 2079, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 378, skbuffer, 2268, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 567, skbuffer, 2457, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 756, skbuffer, 2646, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 945, skbuffer, 2835, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 1134, skbuffer, 3024, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 1323, skbuffer, 3213, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 1512, skbuffer, 3402, 1, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 1, 1, 3, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1010RecPPPF_hpp */
