#ifndef ElectronRepulsionGeom1010RecPDSD_hpp
#define ElectronRepulsionGeom1010RecPDSD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(PD|1/|r-r'||SD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_pdsd(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 96, pbuffer, 382, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 132, pbuffer, 490, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 192, pbuffer, 700, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 252, pbuffer, 820, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 352, pbuffer, 1020, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 442, pbuffer, 1110, 150, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 396, cbuffer, 352, 442, cfactors, 6, 0, 4);
            ...
            t4cfunc::ket_transform<0, 2>(skbuffer, 1620, ckbuffer, 108, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1650, ckbuffer, 144, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1680, ckbuffer, 180, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1710, ckbuffer, 216, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1760, ckbuffer, 276, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1810, ckbuffer, 336, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1860, ckbuffer, 396, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1935, ckbuffer, 486, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 2010, ckbuffer, 576, 0, 4);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 90, 1620, 1710, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 360, 1710, 1860, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pdxx(skbuffer, 810, 0, 90, 360, r_ab, 0, 2);
...
#endif /* ElectronRepulsionGeom1010RecPDSD_hpp */
