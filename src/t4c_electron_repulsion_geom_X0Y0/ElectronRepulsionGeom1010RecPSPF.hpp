#ifndef ElectronRepulsionGeom1010RecPSPF_hpp
#define ElectronRepulsionGeom1010RecPSPF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(PS|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_pspf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(1815, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 66, pbuffer, 256, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 96, pbuffer, 532, 60, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 156, pbuffer, 65, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 166, pbuffer, 115, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 181, pbuffer, 175, 21, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 202, pbuffer, 256, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 232, pbuffer, 316, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 277, pbuffer, 406, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 340, pbuffer, 532, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 400, pbuffer, 592, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 490, pbuffer, 682, 126, ket_width, ket_npgtos);

            }

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 0, cbuffer, 10, 20, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 30, cbuffer, 20, 35, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 75, cbuffer, 0, 0, 30, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 165, cbuffer, 156, 166, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 195, cbuffer, 166, 181, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 240, cbuffer, 56, 165, 195, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 330, cbuffer, 202, 232, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 420, cbuffer, 232, 277, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 555, cbuffer, 66, 330, 420, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 825, cbuffer, 340, 400, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 1005, cbuffer, 400, 490, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 1275, cbuffer, 96, 825, 1005, cfactors, 6, 0, 2);
            ...
            t4cfunc::ket_transform<1, 3>(skbuffer, 1386, ckbuffer, 240, 0, 0);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1407, ckbuffer, 270, 0, 0);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1428, ckbuffer, 300, 0, 0);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1449, ckbuffer, 555, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1512, ckbuffer, 645, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1575, ckbuffer, 735, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1638, ckbuffer, 1275, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1764, ckbuffer, 1455, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 1890, ckbuffer, 1635, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ssxx(skbuffer, 63, 1386, 1449, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 252, 1449, 1638, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_psxx(skbuffer, 819, 0, 63, 252, r_ab, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 0, skbuffer, 819, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 63, skbuffer, 882, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 126, skbuffer, 945, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 189, skbuffer, 1008, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 252, skbuffer, 1071, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 315, skbuffer, 1134, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 378, skbuffer, 1197, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 441, skbuffer, 1260, 1, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 504, skbuffer, 1323, 1, 3);
...
#endif /* ElectronRepulsionGeom1010RecPSPF_hpp */
