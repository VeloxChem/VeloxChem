#ifndef ElectronRepulsionGeom1010RecDDSF_hpp
#define ElectronRepulsionGeom1010RecDDSF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(DD|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_ddsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(1700, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(2040, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 875, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 60, pbuffer, 1115, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 1625, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 250, pbuffer, 1925, 150, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 800, pbuffer, 2465, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 950, pbuffer, 2765, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1175, pbuffer, 3215, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1385, pbuffer, 3425, 315, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 480, cbuffer, 400, 460, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 660, cbuffer, 550, 650, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 960, cbuffer, 800, 950, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1410, cbuffer, 1175, 1385, cfactors, 6, 0, 5);
            ...
            t4cfunc::ket_transform<0, 3>(skbuffer, 7959, ckbuffer, 480, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 8001, ckbuffer, 540, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 8043, ckbuffer, 600, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 8085, ckbuffer, 660, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 8155, ckbuffer, 760, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 8225, ckbuffer, 860, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 8295, ckbuffer, 960, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 8400, ckbuffer, 1110, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 8505, ckbuffer, 1260, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 8610, ckbuffer, 1410, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 8757, ckbuffer, 1620, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 8904, ckbuffer, 1830, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 2289, 0, 504, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 2415, 42, 574, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 2541, 84, 644, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 126, 7959, 8085, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 714, 8085, 8295, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sgxx(skbuffer, 1344, 8295, 8610, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pdxx(skbuffer, 2667, 0, 126, 714, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pfxx(skbuffer, 3801, 504, 714, 1344, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ddxx(skbuffer, 5691, 2289, 2667, 3801, r_ab, 0, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 0, skbuffer, 5691, 0, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 175, skbuffer, 5943, 0, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 350, skbuffer, 6195, 0, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 525, skbuffer, 6447, 0, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 700, skbuffer, 6699, 0, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 875, skbuffer, 6951, 0, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 1050, skbuffer, 7203, 0, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 1225, skbuffer, 7455, 0, 3);

            t4cfunc::bra_transform<2, 2>(sbuffer, 1400, skbuffer, 7707, 0, 3);
...
#endif /* ElectronRepulsionGeom1010RecDDSF_hpp */
