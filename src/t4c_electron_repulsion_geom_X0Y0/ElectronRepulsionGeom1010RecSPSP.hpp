                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<0, 1>(skbuffer, 84, ckbuffer, 3, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 87, ckbuffer, 6, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 108, ckbuffer, 27, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 113, ckbuffer, 33, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 118, ckbuffer, 39, 0, 2);
