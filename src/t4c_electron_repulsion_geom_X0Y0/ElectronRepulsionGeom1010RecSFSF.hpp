#ifndef ElectronRepulsionGeom1010RecSFSF_hpp
#define ElectronRepulsionGeom1010RecSFSF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SF|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_sfsf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 1185, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 100, pbuffer, 1385, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 250, pbuffer, 1685, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 400, pbuffer, 1835, 225, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<0, 3>(skbuffer, 630, ckbuffer, 0, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 700, ckbuffer, 100, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 770, ckbuffer, 200, 0, 3);
            ...
            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 0, 630, 840, r_ab, 0, 3);
...
#endif /* ElectronRepulsionGeom1010RecSFSF_hpp */
