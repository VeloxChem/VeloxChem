#ifndef ElectronRepulsionGeom1010RecPFSD_hpp
#define ElectronRepulsionGeom1010RecPFSD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(PF|1/|r-r'||SD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_pfsd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 180, cbuffer, 160, 220, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 360, cbuffer, 320, 410, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 630, cbuffer, 560, 686, cfactors, 6, 0, 5);
            ...
            t4cfunc::ket_transform<0, 2>(skbuffer, 2625, ckbuffer, 180, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 2675, ckbuffer, 240, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 2725, ckbuffer, 300, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 2775, ckbuffer, 360, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 2850, ckbuffer, 450, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 2925, ckbuffer, 540, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 3000, ckbuffer, 630, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 3105, ckbuffer, 756, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 3210, ckbuffer, 882, 0, 5);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 150, 2625, 2775, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sgxx(skbuffer, 600, 2775, 3000, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pfxx(skbuffer, 1275, 0, 150, 600, r_ab, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 1275, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 105, skbuffer, 1425, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 210, skbuffer, 1575, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 315, skbuffer, 1725, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 420, skbuffer, 1875, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 525, skbuffer, 2025, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 630, skbuffer, 2175, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 735, skbuffer, 2325, 0, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 840, skbuffer, 2475, 0, 2);
...
#endif /* ElectronRepulsionGeom1010RecPFSD_hpp */
