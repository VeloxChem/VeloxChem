#ifndef ElectronRepulsionGeom1010RecSFSP_hpp
#define ElectronRepulsionGeom1010RecSFSP_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SF|1/|r-r'||SP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_sfsp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(225, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 356, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 30, pbuffer, 416, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 90, pbuffer, 536, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 135, pbuffer, 581, 90, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<0, 1>(skbuffer, 270, ckbuffer, 0, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 300, ckbuffer, 30, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 330, ckbuffer, 60, 0, 3);
            ...
            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 0, 270, 360, r_ab, 0, 1);
...
#endif /* ElectronRepulsionGeom1010RecSFSP_hpp */
