#ifndef ElectronRepulsionGeom1010RecSDFF_hpp
#define ElectronRepulsionGeom1010RecSDFF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SD|1/|r-r'||FF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_sdff(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(24336, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(4998, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 1761, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 60, pbuffer, 1881, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 2061, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 276, pbuffer, 3081, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 376, pbuffer, 3181, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 526, pbuffer, 3331, 210, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 736, pbuffer, 1761, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 796, pbuffer, 1881, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 886, pbuffer, 2061, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1012, pbuffer, 2313, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1180, pbuffer, 2649, 216, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1396, pbuffer, 3081, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1496, pbuffer, 3181, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1646, pbuffer, 3331, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1856, pbuffer, 3541, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2136, pbuffer, 3821, 360, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<3, 3>(skbuffer, 2646, ckbuffer, 7326, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 2940, ckbuffer, 7926, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 3234, ckbuffer, 8526, 0, 2);
            ...
            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 0, 2646, 3528, r_ab, 3, 3);
...
#endif /* ElectronRepulsionGeom1010RecSDFF_hpp */
