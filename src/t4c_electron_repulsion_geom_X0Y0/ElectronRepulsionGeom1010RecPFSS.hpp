#ifndef ElectronRepulsionGeom1010RecPFSS_hpp
#define ElectronRepulsionGeom1010RecPFSS_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(PF|1/|r-r'||SS)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_pfss(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(224, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(168, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 181, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 10, pbuffer, 211, 30, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 40, pbuffer, 181, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 50, pbuffer, 211, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 80, pbuffer, 301, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 95, pbuffer, 331, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 140, pbuffer, 421, 21, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 161, pbuffer, 442, 63, ket_width, ket_npgtos);

            }

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxss(ckbuffer, 0, cbuffer, 0, 10, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxss(ckbuffer, 30, cbuffer, 40, 50, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxss(ckbuffer, 60, cbuffer, 80, 95, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxss(ckbuffer, 105, cbuffer, 140, 161, cfactors, 6, 0, 5);
            ...
            t4cfunc::ket_transform<0, 0>(skbuffer, 525, ckbuffer, 30, 0, 3);

            t4cfunc::ket_transform<0, 0>(skbuffer, 535, ckbuffer, 40, 0, 3);

            t4cfunc::ket_transform<0, 0>(skbuffer, 545, ckbuffer, 50, 0, 3);

            t4cfunc::ket_transform<0, 0>(skbuffer, 555, ckbuffer, 60, 0, 4);

            t4cfunc::ket_transform<0, 0>(skbuffer, 570, ckbuffer, 75, 0, 4);

            t4cfunc::ket_transform<0, 0>(skbuffer, 585, ckbuffer, 90, 0, 4);

            t4cfunc::ket_transform<0, 0>(skbuffer, 600, ckbuffer, 105, 0, 5);

            t4cfunc::ket_transform<0, 0>(skbuffer, 621, ckbuffer, 126, 0, 5);

            t4cfunc::ket_transform<0, 0>(skbuffer, 642, ckbuffer, 147, 0, 5);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 30, 525, 555, r_ab, 0, 0);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sgxx(skbuffer, 120, 555, 600, r_ab, 0, 0);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pfxx(skbuffer, 255, 0, 30, 120, r_ab, 0, 0);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 255, 0, 0);

            t4cfunc::bra_transform<1, 3>(sbuffer, 21, skbuffer, 285, 0, 0);

            t4cfunc::bra_transform<1, 3>(sbuffer, 42, skbuffer, 315, 0, 0);

            t4cfunc::bra_transform<1, 3>(sbuffer, 63, skbuffer, 345, 0, 0);

            t4cfunc::bra_transform<1, 3>(sbuffer, 84, skbuffer, 375, 0, 0);

            t4cfunc::bra_transform<1, 3>(sbuffer, 105, skbuffer, 405, 0, 0);

            t4cfunc::bra_transform<1, 3>(sbuffer, 126, skbuffer, 435, 0, 0);

            t4cfunc::bra_transform<1, 3>(sbuffer, 147, skbuffer, 465, 0, 0);

            t4cfunc::bra_transform<1, 3>(sbuffer, 168, skbuffer, 495, 0, 0);
...
#endif /* ElectronRepulsionGeom1010RecPFSS_hpp */
