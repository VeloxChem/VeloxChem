#ifndef ElectronRepulsionGeom1010RecFDSF_hpp
#define ElectronRepulsionGeom1010RecFDSF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(FD|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_fdsf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(pfactors, 0, 2.0, {1075, 1135});

                pbuffer.scale(pfactors, 0, 2.0, {1375, 1465});

                pbuffer.scale(pfactors, 0, 2.0, {2075, 2175});

                pbuffer.scale(pfactors, 0, 2.0, {2475, 2625});

                pbuffer.scale(pfactors, 0, 2.0, {3300, 3450});

                pbuffer.scale(pfactors, 0, 2.0, {3750, 3975});

                t2cfunc::reduce(cbuffer, 0, pbuffer, 1075, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 60, pbuffer, 1375, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 2075, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 250, pbuffer, 2475, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 400, pbuffer, 3300, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 550, pbuffer, 3750, 225, ket_width, ket_npgtos);
                ...
                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {4551, 4761});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {4971, 5286});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {5601, 5881});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {5881, 6301});

                t2cfunc::reduce(cbuffer, 775, pbuffer, 1075, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 835, pbuffer, 1375, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 925, pbuffer, 2075, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1025, pbuffer, 2475, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1175, pbuffer, 3300, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1325, pbuffer, 3750, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1550, pbuffer, 4551, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1760, pbuffer, 4971, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2075, pbuffer, 5601, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2355, pbuffer, 5881, 420, ket_width, ket_npgtos);

            }

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 0, cbuffer, 0, 60, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 180, cbuffer, 150, 250, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 480, cbuffer, 400, 550, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 930, cbuffer, 775, 835, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1110, cbuffer, 925, 1025, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1410, cbuffer, 1175, 1325, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1860, cbuffer, 1550, 1760, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 2490, cbuffer, 2075, 2355, cfactors, 6, 0, 6);

            t4cfunc::ket_transform<0, 3>(skbuffer, 0, ckbuffer, 0, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 42, ckbuffer, 60, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 84, ckbuffer, 120, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 504, ckbuffer, 180, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 574, ckbuffer, 280, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 644, ckbuffer, 380, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1344, ckbuffer, 480, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1449, ckbuffer, 630, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 1554, ckbuffer, 780, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 21378, ckbuffer, 930, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 21420, ckbuffer, 990, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 21462, ckbuffer, 1050, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 21504, ckbuffer, 1110, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 21574, ckbuffer, 1210, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 21644, ckbuffer, 1310, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 21714, ckbuffer, 1410, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 21819, ckbuffer, 1560, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 21924, ckbuffer, 1710, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 22029, ckbuffer, 1860, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 22176, ckbuffer, 2070, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 22323, ckbuffer, 2280, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 22470, ckbuffer, 2490, 0, 6);

            t4cfunc::ket_transform<0, 3>(skbuffer, 22666, ckbuffer, 2770, 0, 6);

            t4cfunc::ket_transform<0, 3>(skbuffer, 22862, ckbuffer, 3050, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 3927, 0, 504, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 4053, 42, 574, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 4179, 84, 644, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 5439, 504, 1344, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 5649, 574, 1449, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 5859, 644, 1554, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 10794, 3927, 5439, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 11046, 4053, 5649, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 11298, 4179, 5859, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 126, 21378, 21504, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 714, 21504, 21714, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sgxx(skbuffer, 1659, 21714, 22029, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_shxx(skbuffer, 2604, 22029, 22470, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pdxx(skbuffer, 4305, 0, 126, 714, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pfxx(skbuffer, 6069, 504, 714, 1659, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pgxx(skbuffer, 7959, 1344, 1659, 2604, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ddxx(skbuffer, 11550, 3927, 4305, 6069, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dfxx(skbuffer, 13818, 5439, 6069, 7959, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_fdxx(skbuffer, 17598, 10794, 11550, 13818, r_ab, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 0, skbuffer, 17598, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 245, skbuffer, 18018, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 490, skbuffer, 18438, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 735, skbuffer, 18858, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 980, skbuffer, 19278, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1225, skbuffer, 19698, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1470, skbuffer, 20118, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1715, skbuffer, 20538, 0, 3);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1960, skbuffer, 20958, 0, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 3, 2, 0, 3, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1010RecFDSF_hpp */
