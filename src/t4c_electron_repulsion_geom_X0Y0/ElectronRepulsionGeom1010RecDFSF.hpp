#ifndef ElectronRepulsionGeom1010RecDFSF_hpp
#define ElectronRepulsionGeom1010RecDFSF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(DF|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_dfsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(2475, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 2075, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 100, pbuffer, 2475, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 250, pbuffer, 3300, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 400, pbuffer, 3750, 225, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 625, pbuffer, 2075, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 725, pbuffer, 2475, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 875, pbuffer, 3300, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1025, pbuffer, 3750, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1250, pbuffer, 4551, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1460, pbuffer, 4971, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1775, pbuffer, 5601, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2055, pbuffer, 5881, 420, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 750, cbuffer, 625, 725, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1050, cbuffer, 875, 1025, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1500, cbuffer, 1250, 1460, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 2130, cbuffer, 1775, 2055, cfactors, 6, 0, 6);
            ...
            t4cfunc::ket_transform<0, 3>(skbuffer, 12558, ckbuffer, 750, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 12628, ckbuffer, 850, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 12698, ckbuffer, 950, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 12768, ckbuffer, 1050, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 12873, ckbuffer, 1200, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 12978, ckbuffer, 1350, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 13083, ckbuffer, 1500, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 13230, ckbuffer, 1710, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 13377, ckbuffer, 1920, 0, 5);

            t4cfunc::ket_transform<0, 3>(skbuffer, 13524, ckbuffer, 2130, 0, 6);

            t4cfunc::ket_transform<0, 3>(skbuffer, 13720, ckbuffer, 2410, 0, 6);

            t4cfunc::ket_transform<0, 3>(skbuffer, 13916, ckbuffer, 2690, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 3423, 0, 840, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 3633, 70, 945, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 3843, 140, 1050, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 210, 12558, 12768, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sgxx(skbuffer, 1155, 12768, 13083, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_shxx(skbuffer, 2100, 13083, 13524, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pfxx(skbuffer, 4053, 0, 210, 1155, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pgxx(skbuffer, 5943, 840, 1155, 2100, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dfxx(skbuffer, 8778, 3423, 4053, 5943, r_ab, 0, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 8778, 0, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 245, skbuffer, 9198, 0, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 490, skbuffer, 9618, 0, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 735, skbuffer, 10038, 0, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 980, skbuffer, 10458, 0, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1225, skbuffer, 10878, 0, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1470, skbuffer, 11298, 0, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1715, skbuffer, 11718, 0, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1960, skbuffer, 12138, 0, 3);
...
#endif /* ElectronRepulsionGeom1010RecDFSF_hpp */
