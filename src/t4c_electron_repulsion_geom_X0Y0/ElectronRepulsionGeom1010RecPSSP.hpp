#ifndef ElectronRepulsionGeom1010RecPSSP_hpp
#define ElectronRepulsionGeom1010RecPSSP_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(PS|1/|r-r'||SP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_pssp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 9, cbuffer, 9, 12, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 18, cbuffer, 18, 27, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 45, cbuffer, 45, 63, cfactors, 6, 0, 2);
            ...
            t4cfunc::ket_transform<0, 1>(skbuffer, 198, ckbuffer, 9, 0, 0);

            t4cfunc::ket_transform<0, 1>(skbuffer, 201, ckbuffer, 12, 0, 0);

            t4cfunc::ket_transform<0, 1>(skbuffer, 204, ckbuffer, 15, 0, 0);

            t4cfunc::ket_transform<0, 1>(skbuffer, 207, ckbuffer, 18, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 216, ckbuffer, 27, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 225, ckbuffer, 36, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 234, ckbuffer, 45, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 252, ckbuffer, 63, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 270, ckbuffer, 81, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ssxx(skbuffer, 9, 198, 207, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 36, 207, 234, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_psxx(skbuffer, 117, 0, 9, 36, r_ab, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 0, skbuffer, 117, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 9, skbuffer, 126, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 18, skbuffer, 135, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 27, skbuffer, 144, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 36, skbuffer, 153, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 45, skbuffer, 162, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 54, skbuffer, 171, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 63, skbuffer, 180, 0, 1);

            t4cfunc::bra_transform<1, 0>(sbuffer, 72, skbuffer, 189, 0, 1);
...
#endif /* ElectronRepulsionGeom1010RecPSSP_hpp */
