#ifndef ElectronRepulsionGeom1010RecPPPP_hpp
#define ElectronRepulsionGeom1010RecPPPP_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(PP|1/|r-r'||PP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_pppp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(484, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(1188, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(2052, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 101, 9, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 66, pbuffer, 101, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 75, pbuffer, 278, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 93, pbuffer, 506, 30, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 9, pbuffer, 101, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 18, pbuffer, 128, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 36, pbuffer, 182, 30, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 123, pbuffer, 101, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 132, pbuffer, 128, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 182, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 180, pbuffer, 278, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 198, pbuffer, 314, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 234, pbuffer, 386, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 294, pbuffer, 506, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 324, pbuffer, 536, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 384, pbuffer, 596, 100, ket_width, ket_npgtos);

            }

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 0, cbuffer, 9, 18, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 27, cbuffer, 18, 36, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpp(ckbuffer, 81, cbuffer, 0, 0, 27, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 162, cbuffer, 123, 132, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 189, cbuffer, 132, 150, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpp(ckbuffer, 243, cbuffer, 66, 162, 189, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 324, cbuffer, 180, 198, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 378, cbuffer, 198, 234, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpp(ckbuffer, 486, cbuffer, 75, 324, 378, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 648, cbuffer, 294, 324, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 738, cbuffer, 324, 384, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpp(ckbuffer, 918, cbuffer, 93, 648, 738, cfactors, 6, 0, 3);
            ...
            t4cfunc::ket_transform<1, 1>(skbuffer, 1539, ckbuffer, 243, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 1566, ckbuffer, 270, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 1593, ckbuffer, 297, 0, 1);

            t4cfunc::ket_transform<1, 1>(skbuffer, 1620, ckbuffer, 486, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 1674, ckbuffer, 540, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 1728, ckbuffer, 594, 0, 2);

            t4cfunc::ket_transform<1, 1>(skbuffer, 1782, ckbuffer, 918, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 1872, ckbuffer, 1008, 0, 3);

            t4cfunc::ket_transform<1, 1>(skbuffer, 1962, ckbuffer, 1098, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 81, 1539, 1620, r_ab, 1, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 324, 1620, 1782, r_ab, 1, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ppxx(skbuffer, 810, 0, 81, 324, r_ab, 1, 1);

            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 810, 1, 1);

            t4cfunc::bra_transform<1, 1>(sbuffer, 81, skbuffer, 891, 1, 1);

            t4cfunc::bra_transform<1, 1>(sbuffer, 162, skbuffer, 972, 1, 1);

            t4cfunc::bra_transform<1, 1>(sbuffer, 243, skbuffer, 1053, 1, 1);

            t4cfunc::bra_transform<1, 1>(sbuffer, 324, skbuffer, 1134, 1, 1);

            t4cfunc::bra_transform<1, 1>(sbuffer, 405, skbuffer, 1215, 1, 1);

            t4cfunc::bra_transform<1, 1>(sbuffer, 486, skbuffer, 1296, 1, 1);

            t4cfunc::bra_transform<1, 1>(sbuffer, 567, skbuffer, 1377, 1, 1);

            t4cfunc::bra_transform<1, 1>(sbuffer, 648, skbuffer, 1458, 1, 1);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 1, 1, 1, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1010RecPPPP_hpp */
