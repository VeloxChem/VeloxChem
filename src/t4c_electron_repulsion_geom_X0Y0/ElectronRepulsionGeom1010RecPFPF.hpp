#ifndef ElectronRepulsionGeom1010RecPFPF_hpp
#define ElectronRepulsionGeom1010RecPFPF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(PF|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_pfpf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(3136, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(9240, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 560, pbuffer, 2605, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 660, pbuffer, 4075, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 810, pbuffer, 5455, 210, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 100, pbuffer, 2605, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 200, pbuffer, 2905, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 350, pbuffer, 3355, 210, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 1020, pbuffer, 2605, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1120, pbuffer, 2905, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1270, pbuffer, 3355, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1480, pbuffer, 4075, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1630, pbuffer, 4375, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1855, pbuffer, 4825, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2170, pbuffer, 5455, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2380, pbuffer, 5665, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2695, pbuffer, 5980, 441, ket_width, ket_npgtos);

            }

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 0, cbuffer, 100, 200, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 300, cbuffer, 200, 350, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 750, cbuffer, 0, 0, 300, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1650, cbuffer, 1020, 1120, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 1950, cbuffer, 1120, 1270, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 2400, cbuffer, 560, 1650, 1950, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 3300, cbuffer, 1480, 1630, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 3750, cbuffer, 1630, 1855, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 4425, cbuffer, 660, 3300, 3750, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 5775, cbuffer, 2170, 2380, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 6405, cbuffer, 2380, 2695, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 7350, cbuffer, 810, 5775, 6405, cfactors, 6, 0, 5);
            ...
            t4cfunc::ket_transform<1, 3>(skbuffer, 11025, ckbuffer, 2400, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 11235, ckbuffer, 2700, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 11445, ckbuffer, 3000, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 11655, ckbuffer, 4425, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 11970, ckbuffer, 4875, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 12285, ckbuffer, 5325, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 12600, ckbuffer, 7350, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 13041, ckbuffer, 7980, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 13482, ckbuffer, 8610, 0, 5);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 630, 11025, 11655, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sgxx(skbuffer, 2520, 11655, 12600, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pfxx(skbuffer, 5355, 0, 630, 2520, r_ab, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 5355, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 441, skbuffer, 5985, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 882, skbuffer, 6615, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1323, skbuffer, 7245, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1764, skbuffer, 7875, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 2205, skbuffer, 8505, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 2646, skbuffer, 9135, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 3087, skbuffer, 9765, 1, 3);

            t4cfunc::bra_transform<1, 3>(sbuffer, 3528, skbuffer, 10395, 1, 3);
...
#endif /* ElectronRepulsionGeom1010RecPFPF_hpp */
