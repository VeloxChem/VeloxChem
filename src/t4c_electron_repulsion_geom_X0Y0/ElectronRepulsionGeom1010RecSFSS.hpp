#ifndef ElectronRepulsionGeom1010RecSFSS_hpp
#define ElectronRepulsionGeom1010RecSFSS_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SF|1/|r-r'||SS)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_sfss(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(75, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 141, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 10, pbuffer, 161, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 40, pbuffer, 221, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 55, pbuffer, 236, 45, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<0, 0>(skbuffer, 90, ckbuffer, 0, 0, 3);

            t4cfunc::ket_transform<0, 0>(skbuffer, 100, ckbuffer, 10, 0, 3);

            t4cfunc::ket_transform<0, 0>(skbuffer, 110, ckbuffer, 20, 0, 3);
            ...
            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 0, 90, 120, r_ab, 0, 0);

            t4cfunc::bra_transform<0, 3>(sbuffer, 0, skbuffer, 0, 0, 0);

            t4cfunc::bra_transform<0, 3>(sbuffer, 7, skbuffer, 10, 0, 0);

            t4cfunc::bra_transform<0, 3>(sbuffer, 14, skbuffer, 20, 0, 0);

            t4cfunc::bra_transform<0, 3>(sbuffer, 21, skbuffer, 30, 0, 0);

            t4cfunc::bra_transform<0, 3>(sbuffer, 28, skbuffer, 40, 0, 0);

            t4cfunc::bra_transform<0, 3>(sbuffer, 35, skbuffer, 50, 0, 0);

            t4cfunc::bra_transform<0, 3>(sbuffer, 42, skbuffer, 60, 0, 0);

            t4cfunc::bra_transform<0, 3>(sbuffer, 49, skbuffer, 70, 0, 0);

            t4cfunc::bra_transform<0, 3>(sbuffer, 56, skbuffer, 80, 0, 0);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 3, 0, 0, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1010RecSFSS_hpp */
