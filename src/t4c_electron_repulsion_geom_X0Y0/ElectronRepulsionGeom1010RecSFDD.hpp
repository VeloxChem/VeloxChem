#ifndef ElectronRepulsionGeom1010RecSFDD_hpp
#define ElectronRepulsionGeom1010RecSFDD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SF|1/|r-r'||DD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_sfdd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(9075, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(4125, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 2015, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 60, pbuffer, 2135, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 160, pbuffer, 3055, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 250, pbuffer, 3145, 150, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 400, pbuffer, 2015, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 460, pbuffer, 2135, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 560, pbuffer, 2335, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 710, pbuffer, 2635, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 920, pbuffer, 3055, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1010, pbuffer, 3145, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1160, pbuffer, 3295, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1385, pbuffer, 3520, 315, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<2, 2>(skbuffer, 2250, ckbuffer, 2550, 0, 3);

            t4cfunc::ket_transform<2, 2>(skbuffer, 2500, ckbuffer, 2910, 0, 3);

            t4cfunc::ket_transform<2, 2>(skbuffer, 2750, ckbuffer, 3270, 0, 3);
            ...
            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 0, 2250, 3000, r_ab, 2, 2);
...
#endif /* ElectronRepulsionGeom1010RecSFDD_hpp */
