#ifndef ElectronRepulsionGeom1010RecPDSF_hpp
#define ElectronRepulsionGeom1010RecPDSF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(PD|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_pdsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 660, cbuffer, 550, 700, cfactors, 6, 0, 4);
            ...
            t4cfunc::ket_transform<0, 3>(skbuffer, 2268, ckbuffer, 180, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2310, ckbuffer, 240, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2352, ckbuffer, 300, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2394, ckbuffer, 360, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2464, ckbuffer, 460, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2534, ckbuffer, 560, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2604, ckbuffer, 660, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2709, ckbuffer, 810, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 2814, ckbuffer, 960, 0, 4);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 126, 2268, 2394, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 504, 2394, 2604, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pdxx(skbuffer, 1134, 0, 126, 504, r_ab, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 0, skbuffer, 1134, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 105, skbuffer, 1260, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 210, skbuffer, 1386, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 315, skbuffer, 1512, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 420, skbuffer, 1638, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 525, skbuffer, 1764, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 630, skbuffer, 1890, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 735, skbuffer, 2016, 0, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 840, skbuffer, 2142, 0, 3);
...
#endif /* ElectronRepulsionGeom1010RecPDSF_hpp */
