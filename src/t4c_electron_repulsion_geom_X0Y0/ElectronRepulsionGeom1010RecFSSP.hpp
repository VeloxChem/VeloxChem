#ifndef ElectronRepulsionGeom1010RecFSSP_hpp
#define ElectronRepulsionGeom1010RecFSSP_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(FS|1/|r-r'||SP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_fssp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 36, cbuffer, 36, 54, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 90, cbuffer, 90, 93, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 99, cbuffer, 99, 108, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 126, cbuffer, 126, 144, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 180, cbuffer, 180, 210, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 270, cbuffer, 270, 315, cfactors, 6, 0, 4);
            ...
            t4cfunc::ket_transform<0, 1>(skbuffer, 144, ckbuffer, 36, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 162, ckbuffer, 54, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 180, ckbuffer, 72, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2520, ckbuffer, 90, 0, 0);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2523, ckbuffer, 93, 0, 0);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2526, ckbuffer, 96, 0, 0);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2529, ckbuffer, 99, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2538, ckbuffer, 108, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2547, ckbuffer, 117, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2556, ckbuffer, 126, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2574, ckbuffer, 144, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2592, ckbuffer, 162, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2610, ckbuffer, 180, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2640, ckbuffer, 210, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2670, ckbuffer, 240, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2700, ckbuffer, 270, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2745, ckbuffer, 315, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 2790, ckbuffer, 360, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 630, 0, 36, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 639, 3, 45, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 648, 6, 54, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 738, 36, 144, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 765, 45, 162, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 792, 54, 180, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dsxx(skbuffer, 1548, 630, 738, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dsxx(skbuffer, 1566, 639, 765, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dsxx(skbuffer, 1584, 648, 792, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ssxx(skbuffer, 9, 2520, 2529, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 63, 2529, 2556, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 198, 2556, 2610, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 360, 2610, 2700, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_psxx(skbuffer, 657, 0, 9, 63, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ppxx(skbuffer, 819, 36, 63, 198, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pdxx(skbuffer, 1062, 144, 198, 360, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dsxx(skbuffer, 1602, 630, 657, 819, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dpxx(skbuffer, 1764, 738, 819, 1062, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_fsxx(skbuffer, 2250, 1548, 1602, 1764, r_ab, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 0, skbuffer, 2250, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 21, skbuffer, 2280, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 42, skbuffer, 2310, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 63, skbuffer, 2340, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 84, skbuffer, 2370, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 105, skbuffer, 2400, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 126, skbuffer, 2430, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 147, skbuffer, 2460, 0, 1);

            t4cfunc::bra_transform<3, 0>(sbuffer, 168, skbuffer, 2490, 0, 1);
...
#endif /* ElectronRepulsionGeom1010RecFSSP_hpp */
