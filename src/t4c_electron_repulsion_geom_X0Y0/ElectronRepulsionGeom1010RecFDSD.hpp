#ifndef ElectronRepulsionGeom1010RecFDSD_hpp
#define ElectronRepulsionGeom1010RecFDSD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(FD|1/|r-r'||SD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_fdsd(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(pfactors, 0, 2.0, {590, 626});

                pbuffer.scale(pfactors, 0, 2.0, {770, 830});

                pbuffer.scale(pfactors, 0, 2.0, {1180, 1240});

                pbuffer.scale(pfactors, 0, 2.0, {1420, 1520});

                pbuffer.scale(pfactors, 0, 2.0, {1925, 2015});

                pbuffer.scale(pfactors, 0, 2.0, {2195, 2345});
                ...
                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {2708, 2834});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {2960, 3170});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {3380, 3548});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {3548, 3828});
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 288, cbuffer, 256, 346, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 558, cbuffer, 496, 532, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 666, cbuffer, 592, 652, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 846, cbuffer, 752, 842, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 1116, cbuffer, 992, 1118, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 1494, cbuffer, 1328, 1496, cfactors, 6, 0, 6);
            ...
            t4cfunc::ket_transform<0, 2>(skbuffer, 960, ckbuffer, 288, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1035, ckbuffer, 378, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1110, ckbuffer, 468, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 15270, ckbuffer, 558, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 15300, ckbuffer, 594, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 15330, ckbuffer, 630, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 15360, ckbuffer, 666, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 15410, ckbuffer, 726, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 15460, ckbuffer, 786, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 15510, ckbuffer, 846, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 15585, ckbuffer, 936, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 15660, ckbuffer, 1026, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 15735, ckbuffer, 1116, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 15840, ckbuffer, 1242, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 15945, ckbuffer, 1368, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 16050, ckbuffer, 1494, 0, 6);

            t4cfunc::ket_transform<0, 2>(skbuffer, 16190, ckbuffer, 1662, 0, 6);

            t4cfunc::ket_transform<0, 2>(skbuffer, 16330, ckbuffer, 1830, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 2805, 0, 360, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 2895, 30, 410, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 2985, 60, 460, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 3885, 360, 960, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 4035, 410, 1035, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 4185, 460, 1110, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 7710, 2805, 3885, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 7890, 2895, 4035, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 8070, 2985, 4185, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 90, 15270, 15360, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 510, 15360, 15510, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sgxx(skbuffer, 1185, 15510, 15735, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_shxx(skbuffer, 1860, 15735, 16050, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pdxx(skbuffer, 3075, 0, 90, 510, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pfxx(skbuffer, 4335, 360, 510, 1185, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pgxx(skbuffer, 5685, 960, 1185, 1860, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ddxx(skbuffer, 8250, 2805, 3075, 4335, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dfxx(skbuffer, 9870, 3885, 4335, 5685, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_fdxx(skbuffer, 12570, 7710, 8250, 9870, r_ab, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 0, skbuffer, 12570, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 175, skbuffer, 12870, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 350, skbuffer, 13170, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 525, skbuffer, 13470, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 700, skbuffer, 13770, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 875, skbuffer, 14070, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1050, skbuffer, 14370, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1225, skbuffer, 14670, 0, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1400, skbuffer, 14970, 0, 2);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 3, 2, 0, 2, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1010RecFDSD_hpp */
