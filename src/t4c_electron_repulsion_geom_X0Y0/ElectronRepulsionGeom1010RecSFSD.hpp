#ifndef ElectronRepulsionGeom1010RecSFSD_hpp
#define ElectronRepulsionGeom1010RecSFSD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SF|1/|r-r'||SD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_sfsd(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 700, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 60, pbuffer, 820, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 160, pbuffer, 1020, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 250, pbuffer, 1110, 150, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<0, 2>(skbuffer, 450, ckbuffer, 0, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 500, ckbuffer, 60, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 550, ckbuffer, 120, 0, 3);
            ...
            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 0, 450, 600, r_ab, 0, 2);
...
#endif /* ElectronRepulsionGeom1010RecSFSD_hpp */
