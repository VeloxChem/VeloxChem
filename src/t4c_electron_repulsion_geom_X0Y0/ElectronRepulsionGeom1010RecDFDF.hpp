#ifndef ElectronRepulsionGeom1010RecDFDF_hpp
#define ElectronRepulsionGeom1010RecDFDF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(DF|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_dfdf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(9801, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(56727, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 250, pbuffer, 8046, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 400, pbuffer, 8496, 225, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 2475, pbuffer, 4861, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2575, pbuffer, 5261, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2725, pbuffer, 8046, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2875, pbuffer, 8496, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3100, pbuffer, 11502, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3310, pbuffer, 11922, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3625, pbuffer, 14610, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3905, pbuffer, 14890, 420, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 625, pbuffer, 4861, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 725, pbuffer, 5261, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 875, pbuffer, 5861, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1085, pbuffer, 6701, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1365, pbuffer, 8046, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1515, pbuffer, 8496, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1740, pbuffer, 9171, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2055, pbuffer, 10116, 420, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 4325, pbuffer, 4861, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4425, pbuffer, 5261, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4575, pbuffer, 5861, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4785, pbuffer, 6701, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5065, pbuffer, 8046, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5215, pbuffer, 8496, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5440, pbuffer, 9171, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5755, pbuffer, 10116, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 6175, pbuffer, 11502, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 6385, pbuffer, 11922, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 6700, pbuffer, 12552, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 7141, pbuffer, 13434, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 7729, pbuffer, 14610, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 8009, pbuffer, 14890, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 8429, pbuffer, 15310, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 9017, pbuffer, 15898, 784, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 15705, cbuffer, 2475, 2575, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 22125, cbuffer, 2725, 2875, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 31548, cbuffer, 3100, 3310, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 44547, cbuffer, 3625, 3905, cfactors, 6, 0, 6);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 0, cbuffer, 625, 725, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 300, cbuffer, 725, 875, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 750, cbuffer, 875, 1085, cfactors, 6, 0, 3);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 5730, cbuffer, 1365, 1515, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 6180, cbuffer, 1515, 1740, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 6855, cbuffer, 1740, 2055, cfactors, 6, 0, 4);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 14325, cbuffer, 4325, 4425, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 14625, cbuffer, 4425, 4575, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 15075, cbuffer, 4575, 4785, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 16005, cbuffer, 2475, 14325, 14625, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpg(ckbuffer, 16905, cbuffer, 2575, 14625, 15075, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxdf(ckbuffer, 18255, 15705, 16005, 16905, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 20055, cbuffer, 5065, 5215, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 20505, cbuffer, 5215, 5440, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 21180, cbuffer, 5440, 5755, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 22575, cbuffer, 2725, 20055, 20505, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpg(ckbuffer, 23925, cbuffer, 2875, 20505, 21180, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxdf(ckbuffer, 25950, 22125, 22575, 23925, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 28650, cbuffer, 6175, 6385, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 29280, cbuffer, 6385, 6700, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 30225, cbuffer, 6700, 7141, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 32178, cbuffer, 3100, 28650, 29280, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpg(ckbuffer, 34068, cbuffer, 3310, 29280, 30225, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxdf(ckbuffer, 36903, 31548, 32178, 34068, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 40683, cbuffer, 7729, 8009, cfactors, 6, 0, 6);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 41523, cbuffer, 8009, 8429, cfactors, 6, 0, 6);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 42783, cbuffer, 8429, 9017, cfactors, 6, 0, 6);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 45387, cbuffer, 3625, 40683, 41523, cfactors, 6, 0, 6);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpg(ckbuffer, 47907, cbuffer, 3905, 41523, 42783, cfactors, 6, 0, 6);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxdf(ckbuffer, 51687, 44547, 45387, 47907, cfactors, 6, 0, 6);
            ...
            t4cfunc::ket_transform<2, 3>(skbuffer, 62790, ckbuffer, 18255, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 63140, ckbuffer, 18855, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 63490, ckbuffer, 19455, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 63840, ckbuffer, 25950, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 64365, ckbuffer, 26850, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 64890, ckbuffer, 27750, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 65415, ckbuffer, 36903, 0, 5);

            t4cfunc::ket_transform<2, 3>(skbuffer, 66150, ckbuffer, 38163, 0, 5);

            t4cfunc::ket_transform<2, 3>(skbuffer, 66885, ckbuffer, 39423, 0, 5);

            t4cfunc::ket_transform<2, 3>(skbuffer, 67620, ckbuffer, 51687, 0, 6);

            t4cfunc::ket_transform<2, 3>(skbuffer, 68600, ckbuffer, 53367, 0, 6);

            t4cfunc::ket_transform<2, 3>(skbuffer, 69580, ckbuffer, 55047, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 17115, 0, 4200, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 18165, 350, 4725, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 19215, 700, 5250, r_ab, 2, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 1050, 62790, 63840, r_ab, 2, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sgxx(skbuffer, 5775, 63840, 65415, r_ab, 2, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_shxx(skbuffer, 10500, 65415, 67620, r_ab, 2, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pfxx(skbuffer, 20265, 0, 1050, 5775, r_ab, 2, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pgxx(skbuffer, 29715, 4200, 5775, 10500, r_ab, 2, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dfxx(skbuffer, 43890, 17115, 20265, 29715, r_ab, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 43890, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1225, skbuffer, 45990, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 2450, skbuffer, 48090, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 3675, skbuffer, 50190, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 4900, skbuffer, 52290, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 6125, skbuffer, 54390, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 7350, skbuffer, 56490, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 8575, skbuffer, 58590, 2, 3);

            t4cfunc::bra_transform<2, 3>(sbuffer, 9800, skbuffer, 60690, 2, 3);
...
#endif /* ElectronRepulsionGeom1010RecDFDF_hpp */
