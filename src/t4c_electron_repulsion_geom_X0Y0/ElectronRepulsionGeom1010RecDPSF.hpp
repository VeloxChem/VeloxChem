#ifndef ElectronRepulsionGeom1010RecDPSF_hpp
#define ElectronRepulsionGeom1010RecDPSF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(DP|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_dpsf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(1075, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(1290, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 225, pbuffer, 285, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 255, pbuffer, 405, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 300, pbuffer, 675, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 360, pbuffer, 855, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 450, pbuffer, 1185, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 550, pbuffer, 1385, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 700, pbuffer, 1685, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 850, pbuffer, 1835, 225, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 270, cbuffer, 225, 255, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 360, cbuffer, 300, 360, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 540, cbuffer, 450, 550, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 840, cbuffer, 700, 850, cfactors, 6, 0, 4);
            ...
            t4cfunc::ket_transform<0, 3>(skbuffer, 4410, ckbuffer, 270, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 4431, ckbuffer, 300, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 4452, ckbuffer, 330, 0, 1);

            t4cfunc::ket_transform<0, 3>(skbuffer, 4473, ckbuffer, 360, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 4515, ckbuffer, 420, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 4557, ckbuffer, 480, 0, 2);

            t4cfunc::ket_transform<0, 3>(skbuffer, 4599, ckbuffer, 540, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 4669, ckbuffer, 640, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 4739, ckbuffer, 740, 0, 3);

            t4cfunc::ket_transform<0, 3>(skbuffer, 4809, ckbuffer, 840, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 4914, ckbuffer, 990, 0, 4);

            t4cfunc::ket_transform<0, 3>(skbuffer, 5019, ckbuffer, 1140, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 1386, 0, 252, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 1449, 21, 294, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 1512, 42, 336, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 63, 4410, 4473, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 378, 4473, 4599, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 756, 4599, 4809, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ppxx(skbuffer, 1575, 0, 63, 378, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pdxx(skbuffer, 2142, 252, 378, 756, r_ab, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dpxx(skbuffer, 3276, 1386, 1575, 2142, r_ab, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 0, skbuffer, 3276, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 105, skbuffer, 3402, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 210, skbuffer, 3528, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 315, skbuffer, 3654, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 420, skbuffer, 3780, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 525, skbuffer, 3906, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 630, skbuffer, 4032, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 735, skbuffer, 4158, 0, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 840, skbuffer, 4284, 0, 3);
...
#endif /* ElectronRepulsionGeom1010RecDPSF_hpp */
