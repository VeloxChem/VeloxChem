#ifndef ElectronRepulsionGeom1010RecPFPD_hpp
#define ElectronRepulsionGeom1010RecPFPD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(PF|1/|r-r'||PD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_pfpd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(2072, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(5712, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 370, pbuffer, 1635, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 430, pbuffer, 2610, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 520, pbuffer, 3540, 126, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 60, pbuffer, 1635, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 120, pbuffer, 1815, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 220, pbuffer, 2115, 150, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 646, pbuffer, 1635, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 706, pbuffer, 1815, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 806, pbuffer, 2115, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 956, pbuffer, 2610, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1046, pbuffer, 2790, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1196, pbuffer, 3090, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1421, pbuffer, 3540, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1547, pbuffer, 3666, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1757, pbuffer, 3876, 315, ket_width, ket_npgtos);

            }

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 0, cbuffer, 60, 120, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 180, cbuffer, 120, 220, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 480, cbuffer, 0, 0, 180, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 1020, cbuffer, 646, 706, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1200, cbuffer, 706, 806, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 1500, cbuffer, 370, 1020, 1200, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 2040, cbuffer, 956, 1046, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 2310, cbuffer, 1046, 1196, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 2760, cbuffer, 430, 2040, 2310, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 3570, cbuffer, 1421, 1547, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 3948, cbuffer, 1547, 1757, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 4578, cbuffer, 520, 3570, 3948, cfactors, 6, 0, 5);
            ...
            t4cfunc::ket_transform<1, 2>(skbuffer, 7875, ckbuffer, 1500, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 8025, ckbuffer, 1680, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 8175, ckbuffer, 1860, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 8325, ckbuffer, 2760, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 8550, ckbuffer, 3030, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 8775, ckbuffer, 3300, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 9000, ckbuffer, 4578, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 9315, ckbuffer, 4956, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 9630, ckbuffer, 5334, 0, 5);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 450, 7875, 8325, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sgxx(skbuffer, 1800, 8325, 9000, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pfxx(skbuffer, 3825, 0, 450, 1800, r_ab, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 3825, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 315, skbuffer, 4275, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 630, skbuffer, 4725, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 945, skbuffer, 5175, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1260, skbuffer, 5625, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1575, skbuffer, 6075, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 1890, skbuffer, 6525, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 2205, skbuffer, 6975, 1, 2);

            t4cfunc::bra_transform<1, 3>(sbuffer, 2520, skbuffer, 7425, 1, 2);
...
#endif /* ElectronRepulsionGeom1010RecPFPD_hpp */
