#ifndef ElectronRepulsionGeom1010RecSFPD_hpp
#define ElectronRepulsionGeom1010RecSFPD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SF|1/|r-r'||PD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_sfpd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(2550, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 60, pbuffer, 1865, 90, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 150, pbuffer, 1245, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 210, pbuffer, 1365, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 310, pbuffer, 1565, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 460, pbuffer, 1865, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 550, pbuffer, 1955, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 700, pbuffer, 2105, 225, ket_width, ket_npgtos);

            }

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 0, cbuffer, 150, 210, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 180, cbuffer, 210, 310, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 480, cbuffer, 0, 0, 180, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 1020, cbuffer, 460, 550, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1290, cbuffer, 550, 700, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 1740, cbuffer, 60, 1020, 1290, cfactors, 6, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 1350, ckbuffer, 480, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 1500, ckbuffer, 660, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 1650, ckbuffer, 840, 0, 3);
            ...
            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 0, 1350, 1800, r_ab, 1, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 0, skbuffer, 0, 1, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 105, skbuffer, 150, 1, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 210, skbuffer, 300, 1, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 315, skbuffer, 450, 1, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 420, skbuffer, 600, 1, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 525, skbuffer, 750, 1, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 630, skbuffer, 900, 1, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 735, skbuffer, 1050, 1, 2);

            t4cfunc::bra_transform<0, 3>(sbuffer, 840, skbuffer, 1200, 1, 2);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 3, 1, 2, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1010RecSFPD_hpp */
