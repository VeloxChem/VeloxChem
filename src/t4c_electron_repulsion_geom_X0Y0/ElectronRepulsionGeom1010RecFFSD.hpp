#ifndef ElectronRepulsionGeom1010RecFFSD_hpp
#define ElectronRepulsionGeom1010RecFFSD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(FF|1/|r-r'||SD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_ffsd(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(pfactors, 0, 2.0, {1420, 1480});

                pbuffer.scale(pfactors, 0, 2.0, {1720, 1820});

                pbuffer.scale(pfactors, 0, 2.0, {2385, 2475});

                pbuffer.scale(pfactors, 0, 2.0, {2745, 2895});

                pbuffer.scale(pfactors, 0, 2.0, {3492, 3618});

                pbuffer.scale(pfactors, 0, 2.0, {3870, 4080});
                ...
                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {4584, 4752});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {4920, 5200});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {5480, 5696});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {5696, 6056});
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 0, cbuffer, 0, 60, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 180, cbuffer, 160, 250, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 450, cbuffer, 400, 526, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 828, cbuffer, 736, 796, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 1008, cbuffer, 896, 986, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 1278, cbuffer, 1136, 1262, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 1656, cbuffer, 1472, 1640, cfactors, 6, 0, 6);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 2160, cbuffer, 1920, 2136, cfactors, 6, 0, 7);

            t4cfunc::ket_transform<0, 2>(skbuffer, 0, ckbuffer, 0, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 50, ckbuffer, 60, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 100, ckbuffer, 120, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 600, ckbuffer, 180, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 675, ckbuffer, 270, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 750, ckbuffer, 360, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1500, ckbuffer, 450, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1605, ckbuffer, 576, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1710, ckbuffer, 702, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 23505, ckbuffer, 828, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 23555, ckbuffer, 888, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 23605, ckbuffer, 948, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 23655, ckbuffer, 1008, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 23730, ckbuffer, 1098, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 23805, ckbuffer, 1188, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 23880, ckbuffer, 1278, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 23985, ckbuffer, 1404, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 24090, ckbuffer, 1530, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 24195, ckbuffer, 1656, 0, 6);

            t4cfunc::ket_transform<0, 2>(skbuffer, 24335, ckbuffer, 1824, 0, 6);

            t4cfunc::ket_transform<0, 2>(skbuffer, 24475, ckbuffer, 1992, 0, 6);

            t4cfunc::ket_transform<0, 2>(skbuffer, 24615, ckbuffer, 2160, 0, 7);

            t4cfunc::ket_transform<0, 2>(skbuffer, 24795, ckbuffer, 2376, 0, 7);

            t4cfunc::ket_transform<0, 2>(skbuffer, 24975, ckbuffer, 2592, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 4020, 0, 600, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 4170, 50, 675, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 4320, 100, 750, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 5820, 600, 1500, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 6045, 675, 1605, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 6270, 750, 1710, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 11355, 4020, 5820, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 11655, 4170, 6045, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 11955, 4320, 6270, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 150, 23505, 23655, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sgxx(skbuffer, 825, 23655, 23880, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_shxx(skbuffer, 1815, 23880, 24195, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sixx(skbuffer, 2760, 24195, 24615, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pfxx(skbuffer, 4470, 0, 150, 825, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pgxx(skbuffer, 6495, 600, 825, 1815, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_phxx(skbuffer, 8520, 1500, 1815, 2760, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dfxx(skbuffer, 12255, 4020, 4470, 6495, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dgxx(skbuffer, 14955, 5820, 6495, 8520, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ffxx(skbuffer, 19005, 11355, 12255, 14955, r_ab, 0, 2);

            t4cfunc::bra_transform<3, 3>(sbuffer, 0, skbuffer, 19005, 0, 2);

            t4cfunc::bra_transform<3, 3>(sbuffer, 245, skbuffer, 19505, 0, 2);

            t4cfunc::bra_transform<3, 3>(sbuffer, 490, skbuffer, 20005, 0, 2);

            t4cfunc::bra_transform<3, 3>(sbuffer, 735, skbuffer, 20505, 0, 2);

            t4cfunc::bra_transform<3, 3>(sbuffer, 980, skbuffer, 21005, 0, 2);

            t4cfunc::bra_transform<3, 3>(sbuffer, 1225, skbuffer, 21505, 0, 2);

            t4cfunc::bra_transform<3, 3>(sbuffer, 1470, skbuffer, 22005, 0, 2);

            t4cfunc::bra_transform<3, 3>(sbuffer, 1715, skbuffer, 22505, 0, 2);

            t4cfunc::bra_transform<3, 3>(sbuffer, 1960, skbuffer, 23005, 0, 2);
...
#endif /* ElectronRepulsionGeom1010RecFFSD_hpp */
