#ifndef ElectronRepulsionGeom1010RecPDDF_hpp
#define ElectronRepulsionGeom1010RecPDDF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(PD|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_pddf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(3663, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(21201, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 1599, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 60, pbuffer, 1779, 90, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 594, pbuffer, 1599, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 654, pbuffer, 1779, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 744, pbuffer, 2991, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 844, pbuffer, 3191, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 994, pbuffer, 4471, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1144, pbuffer, 4621, 225, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 150, pbuffer, 1599, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 210, pbuffer, 1779, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 300, pbuffer, 2049, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 426, pbuffer, 2427, 168, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 1369, pbuffer, 1599, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1429, pbuffer, 1779, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1519, pbuffer, 2049, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1645, pbuffer, 2427, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1813, pbuffer, 2991, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1913, pbuffer, 3191, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2063, pbuffer, 3491, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2273, pbuffer, 3911, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2553, pbuffer, 4471, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2703, pbuffer, 4621, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2928, pbuffer, 4846, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3243, pbuffer, 5161, 420, ket_width, ket_npgtos);

            }

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 828, cbuffer, 0, 60, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 4266, cbuffer, 594, 654, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 8256, cbuffer, 744, 844, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 14676, cbuffer, 994, 1144, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 0, cbuffer, 150, 210, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 180, cbuffer, 210, 300, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 450, cbuffer, 300, 426, cfactors, 6, 0, 2);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 3438, cbuffer, 1369, 1429, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 3618, cbuffer, 1429, 1519, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 3888, cbuffer, 1519, 1645, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 4446, cbuffer, 594, 3438, 3618, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpg(ckbuffer, 4986, cbuffer, 654, 3618, 3888, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxdf(ckbuffer, 5796, 4266, 4446, 4986, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 6876, cbuffer, 1813, 1913, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 7176, cbuffer, 1913, 2063, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 7626, cbuffer, 2063, 2273, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 8556, cbuffer, 744, 6876, 7176, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpg(ckbuffer, 9456, cbuffer, 844, 7176, 7626, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxdf(ckbuffer, 10806, 8256, 8556, 9456, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 12606, cbuffer, 2553, 2703, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 13056, cbuffer, 2703, 2928, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 13731, cbuffer, 2928, 3243, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 15126, cbuffer, 994, 12606, 13056, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpg(ckbuffer, 16476, cbuffer, 1144, 13056, 13731, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxdf(ckbuffer, 18501, 14676, 15126, 16476, cfactors, 6, 0, 4);
            ...
            t4cfunc::ket_transform<2, 3>(skbuffer, 11340, ckbuffer, 5796, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 11550, ckbuffer, 6156, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 11760, ckbuffer, 6516, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 11970, ckbuffer, 10806, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 12320, ckbuffer, 11406, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 12670, ckbuffer, 12006, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 13020, ckbuffer, 18501, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 13545, ckbuffer, 19401, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 14070, ckbuffer, 20301, 0, 4);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 630, 11340, 11970, r_ab, 2, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 2520, 11970, 13020, r_ab, 2, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pdxx(skbuffer, 5670, 0, 630, 2520, r_ab, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 0, skbuffer, 5670, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 525, skbuffer, 6300, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 1050, skbuffer, 6930, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 1575, skbuffer, 7560, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 2100, skbuffer, 8190, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 2625, skbuffer, 8820, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 3150, skbuffer, 9450, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 3675, skbuffer, 10080, 2, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 4200, skbuffer, 10710, 2, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 2, 2, 3, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1010RecPDDF_hpp */
