#ifndef ElectronRepulsionGeom1010RecSFDF_hpp
#define ElectronRepulsionGeom1010RecSFDF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SF|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_sfdf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(14325, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 2991, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 100, pbuffer, 3191, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 250, pbuffer, 4471, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 400, pbuffer, 4621, 225, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 625, pbuffer, 2991, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 725, pbuffer, 3191, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 875, pbuffer, 3491, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1085, pbuffer, 3911, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1365, pbuffer, 4471, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1515, pbuffer, 4621, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1740, pbuffer, 4846, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2055, pbuffer, 5161, 420, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<2, 3>(skbuffer, 3150, ckbuffer, 3930, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 3500, ckbuffer, 4530, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 3850, ckbuffer, 5130, 0, 3);
            ...
            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 0, 3150, 4200, r_ab, 2, 3);
            ...
            t4cfunc::bra_transform<0, 3>(sbuffer, 1470, skbuffer, 2100, 2, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 1715, skbuffer, 2450, 2, 3);

            t4cfunc::bra_transform<0, 3>(sbuffer, 1960, skbuffer, 2800, 2, 3);
...
#endif /* ElectronRepulsionGeom1010RecSFDF_hpp */
