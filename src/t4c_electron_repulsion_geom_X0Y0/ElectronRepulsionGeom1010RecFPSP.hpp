#ifndef ElectronRepulsionGeom1010RecFPSP_hpp
#define ElectronRepulsionGeom1010RecFPSP_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(FP|1/|r-r'||SP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_fpsp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(666, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(pfactors, 0, 2.0, {77, 86});

                pbuffer.scale(pfactors, 0, 2.0, {122, 140});

                pbuffer.scale(pfactors, 0, 2.0, {230, 248});

                pbuffer.scale(pfactors, 0, 2.0, {302, 338});
                ...
                t2cfunc::reduce(cbuffer, 27, pbuffer, 230, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 45, pbuffer, 302, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 81, pbuffer, 466, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 111, pbuffer, 556, 60, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 171, pbuffer, 77, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 180, pbuffer, 122, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 198, pbuffer, 230, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 216, pbuffer, 302, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 252, pbuffer, 466, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 282, pbuffer, 556, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 342, pbuffer, 751, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 387, pbuffer, 841, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 477, pbuffer, 1021, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 540, pbuffer, 1084, 126, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 81, cbuffer, 81, 111, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 171, cbuffer, 171, 180, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 198, cbuffer, 198, 216, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 252, cbuffer, 252, 282, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 342, cbuffer, 342, 387, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 477, cbuffer, 477, 540, cfactors, 6, 0, 5);
            ...
            t4cfunc::ket_transform<0, 1>(skbuffer, 324, ckbuffer, 81, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 354, ckbuffer, 111, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 384, ckbuffer, 141, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5301, ckbuffer, 171, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5310, ckbuffer, 180, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5319, ckbuffer, 189, 0, 1);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5328, ckbuffer, 198, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5346, ckbuffer, 216, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5364, ckbuffer, 234, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5382, ckbuffer, 252, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5412, ckbuffer, 282, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5442, ckbuffer, 312, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5472, ckbuffer, 342, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5517, ckbuffer, 387, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5562, ckbuffer, 432, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5607, ckbuffer, 477, 0, 5);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5670, ckbuffer, 540, 0, 5);

            t4cfunc::ket_transform<0, 1>(skbuffer, 5733, ckbuffer, 603, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 1089, 0, 108, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 1116, 9, 126, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 1143, 18, 144, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 1413, 108, 324, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 1467, 126, 354, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 1521, 144, 384, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 2871, 1089, 1413, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 2925, 1116, 1467, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 2979, 1143, 1521, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 27, 5301, 5328, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 162, 5328, 5382, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 414, 5382, 5472, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sgxx(skbuffer, 684, 5472, 5607, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ppxx(skbuffer, 1170, 0, 27, 162, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pdxx(skbuffer, 1575, 108, 162, 414, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pfxx(skbuffer, 2061, 324, 414, 684, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dpxx(skbuffer, 3033, 1089, 1170, 1575, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ddxx(skbuffer, 3519, 1413, 1575, 2061, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_fpxx(skbuffer, 4491, 2871, 3033, 3519, r_ab, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 0, skbuffer, 4491, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 63, skbuffer, 4581, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 126, skbuffer, 4671, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 189, skbuffer, 4761, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 252, skbuffer, 4851, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 315, skbuffer, 4941, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 378, skbuffer, 5031, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 441, skbuffer, 5121, 0, 1);

            t4cfunc::bra_transform<3, 1>(sbuffer, 504, skbuffer, 5211, 0, 1);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 3, 1, 0, 1, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1010RecFPSP_hpp */
