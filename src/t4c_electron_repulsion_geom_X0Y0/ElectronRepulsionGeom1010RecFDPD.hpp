#ifndef ElectronRepulsionGeom1010RecFDPD_hpp
#define ElectronRepulsionGeom1010RecFDPD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(FD|1/|r-r'||PD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_fdpd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(11322, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 36, pbuffer, 2025, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 96, pbuffer, 3370, 90, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 1183, pbuffer, 2025, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1243, pbuffer, 3370, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1333, pbuffer, 4828, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1459, pbuffer, 6130, 168, ket_width, ket_npgtos);

                pbuffer.scale(pfactors, 0, 1.0 / a_exp, {985, 1021});

                pbuffer.scale(pfactors, 0, 2.0, {1165, 1225});

                pbuffer.scale(pfactors, 0, 2.0, {1465, 1555});

                pbuffer.scale(pfactors, 0, 1.0 / a_exp, {2025, 2085});

                pbuffer.scale(pfactors, 0, 2.0, {2265, 2365});

                pbuffer.scale(pfactors, 0, 2.0, {2665, 2815});

                pbuffer.scale(pfactors, 0, 1.0 / a_exp, {3370, 3460});

                pbuffer.scale(pfactors, 0, 2.0, {3640, 3790});

                pbuffer.scale(pfactors, 0, 2.0, {4090, 4315});
                ...
                pbuffer.scale(pfactors, 0, 2.0, {4828, 4954});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {5080, 5290});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {5500, 5815});

                pbuffer.scale(pfactors, 0, 2.0, {6130, 6298});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {6298, 6578});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {6578, 6998});

                t2cfunc::reduce(cbuffer, 1627, pbuffer, 985, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1663, pbuffer, 1165, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1723, pbuffer, 1465, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1813, pbuffer, 2025, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1873, pbuffer, 2265, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1973, pbuffer, 2665, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2123, pbuffer, 3370, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2213, pbuffer, 3640, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2363, pbuffer, 4090, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2588, pbuffer, 4828, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2714, pbuffer, 5080, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2924, pbuffer, 5500, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3239, pbuffer, 6130, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3407, pbuffer, 6298, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3687, pbuffer, 6578, 420, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 612, cbuffer, 372, 432, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 792, cbuffer, 432, 532, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 1092, cbuffer, 36, 612, 792, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 1632, cbuffer, 682, 772, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1902, cbuffer, 772, 922, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 2352, cbuffer, 96, 1632, 1902, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 3162, cbuffer, 1627, 1663, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 3270, cbuffer, 1663, 1723, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 3450, cbuffer, 1147, 3162, 3270, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 3774, cbuffer, 1813, 1873, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 3954, cbuffer, 1873, 1973, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 4254, cbuffer, 1183, 3774, 3954, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 4794, cbuffer, 2123, 2213, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 5064, cbuffer, 2213, 2363, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 5514, cbuffer, 1243, 4794, 5064, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 6324, cbuffer, 2588, 2714, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 6702, cbuffer, 2714, 2924, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 7332, cbuffer, 1333, 6324, 6702, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 8466, cbuffer, 3239, 3407, cfactors, 6, 0, 6);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 8970, cbuffer, 3407, 3687, cfactors, 6, 0, 6);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 9810, cbuffer, 1459, 8466, 8970, cfactors, 6, 0, 6);
            ...
            t4cfunc::ket_transform<1, 2>(skbuffer, 2880, ckbuffer, 2352, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 3105, ckbuffer, 2622, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 3330, ckbuffer, 2892, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 45810, ckbuffer, 3450, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 45900, ckbuffer, 3558, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 45990, ckbuffer, 3666, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 46080, ckbuffer, 4254, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 46230, ckbuffer, 4434, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 46380, ckbuffer, 4614, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 46530, ckbuffer, 5514, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 46755, ckbuffer, 5784, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 46980, ckbuffer, 6054, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 47205, ckbuffer, 7332, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 47520, ckbuffer, 7710, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 47835, ckbuffer, 8088, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 48150, ckbuffer, 9810, 0, 6);

            t4cfunc::ket_transform<1, 2>(skbuffer, 48570, ckbuffer, 10314, 0, 6);

            t4cfunc::ket_transform<1, 2>(skbuffer, 48990, ckbuffer, 10818, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 8415, 0, 1080, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 8685, 90, 1230, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 8955, 180, 1380, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 11655, 1080, 2880, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 12105, 1230, 3105, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 12555, 1380, 3330, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 23130, 8415, 11655, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 23670, 8685, 12105, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 24210, 8955, 12555, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 270, 45810, 46080, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 1530, 46080, 46530, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sgxx(skbuffer, 3555, 46530, 47205, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_shxx(skbuffer, 5580, 47205, 48150, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pdxx(skbuffer, 9225, 0, 270, 1530, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pfxx(skbuffer, 13005, 1080, 1530, 3555, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pgxx(skbuffer, 17055, 2880, 3555, 5580, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ddxx(skbuffer, 24750, 8415, 9225, 13005, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dfxx(skbuffer, 29610, 11655, 13005, 17055, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_fdxx(skbuffer, 37710, 23130, 24750, 29610, r_ab, 1, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 0, skbuffer, 37710, 1, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 525, skbuffer, 38610, 1, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1050, skbuffer, 39510, 1, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 1575, skbuffer, 40410, 1, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 2100, skbuffer, 41310, 1, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 2625, skbuffer, 42210, 1, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 3150, skbuffer, 43110, 1, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 3675, skbuffer, 44010, 1, 2);

            t4cfunc::bra_transform<3, 2>(sbuffer, 4200, skbuffer, 44910, 1, 2);
...
#endif /* ElectronRepulsionGeom1010RecFDPD_hpp */
