...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(PP|1/|r-r'||SD)  integral derivatives.
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(396, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 48, pbuffer, 116, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 66, pbuffer, 170, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 96, pbuffer, 278, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 132, pbuffer, 350, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 192, pbuffer, 470, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 252, pbuffer, 530, 100, ket_width, ket_npgtos);

            }

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 0, cbuffer, 0, 18, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 54, cbuffer, 48, 66, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 108, cbuffer, 96, 132, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 216, cbuffer, 192, 252, cfactors, 6, 0, 3);
            ...
            t4cfunc::ket_transform<0, 2>(skbuffer, 855, ckbuffer, 54, 0, 1);

            t4cfunc::ket_transform<0, 2>(skbuffer, 870, ckbuffer, 72, 0, 1);

            t4cfunc::ket_transform<0, 2>(skbuffer, 885, ckbuffer, 90, 0, 1);

            t4cfunc::ket_transform<0, 2>(skbuffer, 900, ckbuffer, 108, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 930, ckbuffer, 144, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 960, ckbuffer, 180, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 990, ckbuffer, 216, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1040, ckbuffer, 276, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 1090, ckbuffer, 336, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 45, 855, 900, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 180, 900, 990, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ppxx(skbuffer, 450, 0, 45, 180, r_ab, 0, 2);

            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 450, 0, 2);

            t4cfunc::bra_transform<1, 1>(sbuffer, 45, skbuffer, 495, 0, 2);

            t4cfunc::bra_transform<1, 1>(sbuffer, 90, skbuffer, 540, 0, 2);

            t4cfunc::bra_transform<1, 1>(sbuffer, 135, skbuffer, 585, 0, 2);

            t4cfunc::bra_transform<1, 1>(sbuffer, 180, skbuffer, 630, 0, 2);

            t4cfunc::bra_transform<1, 1>(sbuffer, 225, skbuffer, 675, 0, 2);

            t4cfunc::bra_transform<1, 1>(sbuffer, 270, skbuffer, 720, 0, 2);

            t4cfunc::bra_transform<1, 1>(sbuffer, 315, skbuffer, 765, 0, 2);

            t4cfunc::bra_transform<1, 1>(sbuffer, 360, skbuffer, 810, 0, 2);
