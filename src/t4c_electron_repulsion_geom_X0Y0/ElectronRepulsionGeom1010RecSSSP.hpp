#ifndef ElectronRepulsionGeom1010RecSSSP_hpp
#define ElectronRepulsionGeom1010RecSSSP_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SS|1/|r-r'||SP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_sssp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(36, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                erirec::comp_prim_electron_repulsion_sssd(pbuffer, 13, 0, 1, 4, 7, pfactors, 20, 23, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_sssd(pbuffer, 19, 1, 2, 7, 10, pfactors, 20, 23, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsp(pbuffer, 25, 1, 4, 7, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsd(pbuffer, 34, 7, 13, 19, pfactors, 26, r_pb, a_exp, b_exp);

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {4, 7});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {13, 19});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {25, 34});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {34, 52});

                t2cfunc::reduce(cbuffer, 0, pbuffer, 4, 3, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3, pbuffer, 13, 6, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 9, pbuffer, 25, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 18, pbuffer, 34, 18, ket_width, ket_npgtos);
            ...
            t4cfunc::bra_transform<0, 0>(sbuffer, 18, skbuffer, 18, 0, 1);

            t4cfunc::bra_transform<0, 0>(sbuffer, 21, skbuffer, 21, 0, 1);

            t4cfunc::bra_transform<0, 0>(sbuffer, 24, skbuffer, 24, 0, 1);
...
#endif /* ElectronRepulsionGeom1010RecSSSP_hpp */
