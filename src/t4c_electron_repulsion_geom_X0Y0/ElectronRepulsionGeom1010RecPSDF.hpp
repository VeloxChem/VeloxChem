#ifndef ElectronRepulsionGeom1010RecPSDF_hpp
#define ElectronRepulsionGeom1010RecPSDF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(PS|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_psdf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(6303, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 124, pbuffer, 396, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 154, pbuffer, 456, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 199, pbuffer, 840, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 259, pbuffer, 900, 90, ket_width, ket_npgtos);

                pbuffer.scale(pfactors, 0, 1.0 / a_exp, {75, 85});

                pbuffer.scale(pfactors, 0, 1.0 / a_exp, {135, 150});
                ...
                pbuffer.scale(pfactors, 0, 2.0, {396, 426});

                pbuffer.scale(pfactors, 0, 2.0, {456, 501});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {546, 609});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {672, 756});

                pbuffer.scale(pfactors, 0, 2.0, {840, 900});

                pbuffer.scale(pfactors, 0, 2.0, {900, 990});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {990, 1116});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {1116, 1284});

                t2cfunc::reduce(cbuffer, 349, pbuffer, 75, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 359, pbuffer, 135, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 374, pbuffer, 210, 21, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 395, pbuffer, 294, 28, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 423, pbuffer, 396, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 453, pbuffer, 456, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 498, pbuffer, 546, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 561, pbuffer, 672, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 645, pbuffer, 840, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 705, pbuffer, 900, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 795, pbuffer, 990, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 921, pbuffer, 1116, 168, ket_width, ket_npgtos);

            }

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 138, cbuffer, 0, 10, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 711, cbuffer, 99, 109, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 1560, cbuffer, 124, 154, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 3693, cbuffer, 199, 259, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 0, cbuffer, 25, 35, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 30, cbuffer, 35, 50, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 75, cbuffer, 50, 71, cfactors, 6, 0, 0);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 573, cbuffer, 349, 359, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 603, cbuffer, 359, 374, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 648, cbuffer, 374, 395, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 741, cbuffer, 99, 573, 603, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpg(ckbuffer, 831, cbuffer, 109, 603, 648, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxdf(ckbuffer, 966, 711, 741, 831, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1146, cbuffer, 423, 453, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 1236, cbuffer, 453, 498, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 1371, cbuffer, 498, 561, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 1650, cbuffer, 124, 1146, 1236, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpg(ckbuffer, 1920, cbuffer, 154, 1236, 1371, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxdf(ckbuffer, 2325, 1560, 1650, 1920, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 2865, cbuffer, 645, 705, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 3045, cbuffer, 705, 795, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 3315, cbuffer, 795, 921, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 3873, cbuffer, 199, 2865, 3045, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpg(ckbuffer, 4413, cbuffer, 259, 3045, 3315, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxdf(ckbuffer, 5223, 3693, 3873, 4413, cfactors, 6, 0, 2);
            ...
            t4cfunc::ket_transform<2, 3>(skbuffer, 2310, ckbuffer, 966, 0, 0);

            t4cfunc::ket_transform<2, 3>(skbuffer, 2345, ckbuffer, 1026, 0, 0);

            t4cfunc::ket_transform<2, 3>(skbuffer, 2380, ckbuffer, 1086, 0, 0);

            t4cfunc::ket_transform<2, 3>(skbuffer, 2415, ckbuffer, 2325, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 2520, ckbuffer, 2505, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 2625, ckbuffer, 2685, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 2730, ckbuffer, 5223, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 2940, ckbuffer, 5583, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 3150, ckbuffer, 5943, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ssxx(skbuffer, 105, 2310, 2415, r_ab, 2, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 420, 2415, 2730, r_ab, 2, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_psxx(skbuffer, 1365, 0, 105, 420, r_ab, 2, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 0, skbuffer, 1365, 2, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 105, skbuffer, 1470, 2, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 210, skbuffer, 1575, 2, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 315, skbuffer, 1680, 2, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 420, skbuffer, 1785, 2, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 525, skbuffer, 1890, 2, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 630, skbuffer, 1995, 2, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 735, skbuffer, 2100, 2, 3);

            t4cfunc::bra_transform<1, 0>(sbuffer, 840, skbuffer, 2205, 2, 3);
...
#endif /* ElectronRepulsionGeom1010RecPSDF_hpp */
