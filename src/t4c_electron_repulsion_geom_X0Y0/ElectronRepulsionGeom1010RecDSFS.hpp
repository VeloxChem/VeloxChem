#ifndef ElectronRepulsionGeom1010RecDSFS_hpp
#define ElectronRepulsionGeom1010RecDSFS_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(DS|1/|r-r'||FS)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_dsfs(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(6480, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 4, pbuffer, 29, 6, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 10, pbuffer, 175, 3, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 13, pbuffer, 184, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 22, pbuffer, 211, 18, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 180, pbuffer, 0, 1, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 181, pbuffer, 8, 3, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 184, pbuffer, 29, 6, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 190, pbuffer, 175, 3, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 193, pbuffer, 184, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 202, pbuffer, 211, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 220, pbuffer, 490, 6, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 226, pbuffer, 502, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 244, pbuffer, 538, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 280, pbuffer, 910, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 290, pbuffer, 920, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 320, pbuffer, 950, 60, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 60, pbuffer, 115, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 75, pbuffer, 175, 3, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 78, pbuffer, 184, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 87, pbuffer, 211, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 105, pbuffer, 265, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 135, pbuffer, 355, 45, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 400, pbuffer, 115, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 415, pbuffer, 175, 3, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 418, pbuffer, 184, 9, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 427, pbuffer, 211, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 445, pbuffer, 265, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 475, pbuffer, 355, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 520, pbuffer, 490, 6, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 526, pbuffer, 502, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 544, pbuffer, 538, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 580, pbuffer, 610, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 640, pbuffer, 730, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 730, pbuffer, 910, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 740, pbuffer, 920, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 770, pbuffer, 950, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 830, pbuffer, 1010, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 930, pbuffer, 1110, 150, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxps(ckbuffer, 1140, cbuffer, 180, 181, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 1152, cbuffer, 181, 184, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxds(ckbuffer, 1242, 1140, 1152, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxps(ckbuffer, 1530, cbuffer, 190, 193, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 1566, cbuffer, 193, 202, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxds(ckbuffer, 1836, 1530, 1566, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxps(ckbuffer, 2520, cbuffer, 220, 226, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 2592, cbuffer, 226, 244, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxds(ckbuffer, 3132, 2520, 2592, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxps(ckbuffer, 4380, cbuffer, 280, 290, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 4500, cbuffer, 290, 320, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxds(ckbuffer, 5400, 4380, 4500, cfactors, 6, 0, 3);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxss(ckbuffer, 1080, cbuffer, 380, 381, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 1083, cbuffer, 381, 384, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 1092, cbuffer, 384, 390, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1110, cbuffer, 390, 400, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxps(ckbuffer, 1143, cbuffer, 180, 1080, 1083, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpp(ckbuffer, 1161, cbuffer, 181, 1083, 1092, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 1188, cbuffer, 184, 1092, 1110, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxds(ckbuffer, 1248, 1140, 1143, 1161, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxdp(ckbuffer, 1266, 1152, 1161, 1188, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxfs(ckbuffer, 1320, 1242, 1248, 1266, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxss(ckbuffer, 1350, cbuffer, 415, 418, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 1359, cbuffer, 418, 427, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 1386, cbuffer, 427, 445, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1440, cbuffer, 445, 475, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxps(ckbuffer, 1539, cbuffer, 190, 1350, 1359, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpp(ckbuffer, 1593, cbuffer, 193, 1359, 1386, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 1674, cbuffer, 202, 1386, 1440, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxds(ckbuffer, 1854, 1530, 1539, 1593, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxdp(ckbuffer, 1908, 1566, 1593, 1674, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxfs(ckbuffer, 2070, 1836, 1854, 1908, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxss(ckbuffer, 2160, cbuffer, 520, 526, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 2178, cbuffer, 526, 544, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 2232, cbuffer, 544, 580, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 2340, cbuffer, 580, 640, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxps(ckbuffer, 2538, cbuffer, 220, 2160, 2178, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpp(ckbuffer, 2646, cbuffer, 226, 2178, 2232, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 2808, cbuffer, 244, 2232, 2340, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxds(ckbuffer, 3168, 2520, 2538, 2646, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxdp(ckbuffer, 3276, 2592, 2646, 2808, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxfs(ckbuffer, 3600, 3132, 3168, 3276, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxss(ckbuffer, 3780, cbuffer, 730, 740, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 3810, cbuffer, 740, 770, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 3900, cbuffer, 770, 830, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 4080, cbuffer, 830, 930, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxps(ckbuffer, 4410, cbuffer, 280, 3780, 3810, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpp(ckbuffer, 4590, cbuffer, 290, 3810, 3900, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 4860, cbuffer, 320, 3900, 4080, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxds(ckbuffer, 5460, 4380, 4410, 4590, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxdp(ckbuffer, 5640, 4500, 4590, 4860, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxfs(ckbuffer, 6180, 5400, 5460, 5640, cfactors, 6, 0, 3);
            ...
            t4cfunc::ket_transform<3, 0>(skbuffer, 1911, ckbuffer, 1320, 0, 0);

            t4cfunc::ket_transform<3, 0>(skbuffer, 1918, ckbuffer, 1330, 0, 0);

            t4cfunc::ket_transform<3, 0>(skbuffer, 1925, ckbuffer, 1340, 0, 0);

            t4cfunc::ket_transform<3, 0>(skbuffer, 1932, ckbuffer, 2070, 0, 1);

            t4cfunc::ket_transform<3, 0>(skbuffer, 1953, ckbuffer, 2100, 0, 1);

            t4cfunc::ket_transform<3, 0>(skbuffer, 1974, ckbuffer, 2130, 0, 1);

            t4cfunc::ket_transform<3, 0>(skbuffer, 1995, ckbuffer, 3600, 0, 2);

            t4cfunc::ket_transform<3, 0>(skbuffer, 2037, ckbuffer, 3660, 0, 2);

            t4cfunc::ket_transform<3, 0>(skbuffer, 2079, ckbuffer, 3720, 0, 2);

            t4cfunc::ket_transform<3, 0>(skbuffer, 2121, ckbuffer, 6180, 0, 3);

            t4cfunc::ket_transform<3, 0>(skbuffer, 2191, ckbuffer, 6280, 0, 3);

            t4cfunc::ket_transform<3, 0>(skbuffer, 2261, ckbuffer, 6380, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 714, 0, 84, r_ab, 3, 0);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 735, 7, 105, r_ab, 3, 0);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 756, 14, 126, r_ab, 3, 0);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ssxx(skbuffer, 21, 1911, 1932, r_ab, 3, 0);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 147, 1932, 1995, r_ab, 3, 0);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 336, 1995, 2121, r_ab, 3, 0);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_psxx(skbuffer, 777, 0, 21, 147, r_ab, 3, 0);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ppxx(skbuffer, 966, 84, 147, 336, r_ab, 3, 0);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dsxx(skbuffer, 1533, 714, 777, 966, r_ab, 3, 0);

            t4cfunc::bra_transform<2, 0>(sbuffer, 0, skbuffer, 1533, 3, 0);

            t4cfunc::bra_transform<2, 0>(sbuffer, 35, skbuffer, 1575, 3, 0);

            t4cfunc::bra_transform<2, 0>(sbuffer, 70, skbuffer, 1617, 3, 0);

            t4cfunc::bra_transform<2, 0>(sbuffer, 105, skbuffer, 1659, 3, 0);

            t4cfunc::bra_transform<2, 0>(sbuffer, 140, skbuffer, 1701, 3, 0);

            t4cfunc::bra_transform<2, 0>(sbuffer, 175, skbuffer, 1743, 3, 0);

            t4cfunc::bra_transform<2, 0>(sbuffer, 210, skbuffer, 1785, 3, 0);

            t4cfunc::bra_transform<2, 0>(sbuffer, 245, skbuffer, 1827, 3, 0);

            t4cfunc::bra_transform<2, 0>(sbuffer, 280, skbuffer, 1869, 3, 0);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 2, 0, 3, 0, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1010RecDSFS_hpp */
