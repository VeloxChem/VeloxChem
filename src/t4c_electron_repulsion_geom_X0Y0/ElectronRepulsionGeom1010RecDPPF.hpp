#ifndef ElectronRepulsionGeom1010RecDPPF_hpp
#define ElectronRepulsionGeom1010RecDPPF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(DP|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_dppf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(7095, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 30, pbuffer, 1067, 60, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 504, pbuffer, 425, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 534, pbuffer, 1067, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 594, pbuffer, 1955, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 694, pbuffer, 2875, 150, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 90, pbuffer, 425, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 120, pbuffer, 545, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 165, pbuffer, 725, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 228, pbuffer, 1067, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 288, pbuffer, 1247, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 378, pbuffer, 1517, 126, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 844, pbuffer, 425, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 874, pbuffer, 545, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 919, pbuffer, 725, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 982, pbuffer, 1067, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1042, pbuffer, 1247, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1132, pbuffer, 1517, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1258, pbuffer, 1955, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1358, pbuffer, 2155, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1508, pbuffer, 2455, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1718, pbuffer, 2875, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1868, pbuffer, 3025, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2093, pbuffer, 3250, 315, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1485, cbuffer, 844, 874, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 1575, cbuffer, 874, 919, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 1710, cbuffer, 504, 1485, 1575, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1980, cbuffer, 982, 1042, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 2160, cbuffer, 1042, 1132, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 2430, cbuffer, 534, 1980, 2160, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 2970, cbuffer, 1258, 1358, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 3270, cbuffer, 1358, 1508, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 3720, cbuffer, 594, 2970, 3270, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 4620, cbuffer, 1718, 1868, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 5070, cbuffer, 1868, 2093, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 5745, cbuffer, 694, 4620, 5070, cfactors, 6, 0, 4);
            ...
            t4cfunc::ket_transform<1, 3>(skbuffer, 13230, ckbuffer, 1710, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 13293, ckbuffer, 1800, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 13356, ckbuffer, 1890, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 13419, ckbuffer, 2430, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 13545, ckbuffer, 2610, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 13671, ckbuffer, 2790, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 13797, ckbuffer, 3720, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 14007, ckbuffer, 4020, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 14217, ckbuffer, 4320, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 14427, ckbuffer, 5745, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 14742, ckbuffer, 6195, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 15057, ckbuffer, 6645, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 4158, 0, 756, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 4347, 63, 882, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 4536, 126, 1008, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 189, 13230, 13419, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 1134, 13419, 13797, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 2268, 13797, 14427, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ppxx(skbuffer, 4725, 0, 189, 1134, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pdxx(skbuffer, 6426, 756, 1134, 2268, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dpxx(skbuffer, 9828, 4158, 4725, 6426, r_ab, 1, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 0, skbuffer, 9828, 1, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 315, skbuffer, 10206, 1, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 630, skbuffer, 10584, 1, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 945, skbuffer, 10962, 1, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 1260, skbuffer, 11340, 1, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 1575, skbuffer, 11718, 1, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 1890, skbuffer, 12096, 1, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 2205, skbuffer, 12474, 1, 3);

            t4cfunc::bra_transform<2, 1>(sbuffer, 2520, skbuffer, 12852, 1, 3);
...
#endif /* ElectronRepulsionGeom1010RecDPPF_hpp */
