#ifndef ElectronRepulsionGeom1010RecFDSS_hpp
#define ElectronRepulsionGeom1010RecFDSS_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(FD|1/|r-r'||SS)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_fdss(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(333, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(pfactors, 0, 2.0, {101, 107});
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 101, 6, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 6, pbuffer, 131, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 24, pbuffer, 221, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 34, pbuffer, 261, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 64, pbuffer, 381, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 79, pbuffer, 426, 45, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {101, 107});
                ...
                t2cfunc::reduce(cbuffer, 124, pbuffer, 101, 6, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 130, pbuffer, 131, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 148, pbuffer, 221, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 158, pbuffer, 261, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 188, pbuffer, 381, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 203, pbuffer, 426, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 248, pbuffer, 561, 21, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 269, pbuffer, 603, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 332, pbuffer, 729, 28, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 360, pbuffer, 757, 84, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxss(ckbuffer, 48, cbuffer, 64, 79, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxss(ckbuffer, 93, cbuffer, 124, 130, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxss(ckbuffer, 111, cbuffer, 148, 158, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxss(ckbuffer, 141, cbuffer, 188, 203, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxss(ckbuffer, 186, cbuffer, 248, 269, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxss(ckbuffer, 249, cbuffer, 332, 360, cfactors, 6, 0, 6);
            ...
            t4cfunc::ket_transform<0, 0>(skbuffer, 192, ckbuffer, 48, 0, 4);

            t4cfunc::ket_transform<0, 0>(skbuffer, 207, ckbuffer, 63, 0, 4);

            t4cfunc::ket_transform<0, 0>(skbuffer, 222, ckbuffer, 78, 0, 4);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3054, ckbuffer, 93, 0, 2);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3060, ckbuffer, 99, 0, 2);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3066, ckbuffer, 105, 0, 2);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3072, ckbuffer, 111, 0, 3);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3082, ckbuffer, 121, 0, 3);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3092, ckbuffer, 131, 0, 3);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3102, ckbuffer, 141, 0, 4);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3117, ckbuffer, 156, 0, 4);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3132, ckbuffer, 171, 0, 4);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3147, ckbuffer, 186, 0, 5);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3168, ckbuffer, 207, 0, 5);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3189, ckbuffer, 228, 0, 5);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3210, ckbuffer, 249, 0, 6);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3238, ckbuffer, 277, 0, 6);

            t4cfunc::ket_transform<0, 0>(skbuffer, 3266, ckbuffer, 305, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 561, 0, 72, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 579, 6, 82, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 597, 12, 92, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 777, 72, 192, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 807, 82, 207, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 837, 92, 222, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 1542, 561, 777, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 1578, 579, 807, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 1614, 597, 837, r_ab, 0, 0);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 18, 3054, 3072, r_ab, 0, 0);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 102, 3072, 3102, r_ab, 0, 0);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sgxx(skbuffer, 237, 3102, 3147, r_ab, 0, 0);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_shxx(skbuffer, 372, 3147, 3210, r_ab, 0, 0);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pdxx(skbuffer, 615, 0, 18, 102, r_ab, 0, 0);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pfxx(skbuffer, 867, 72, 102, 237, r_ab, 0, 0);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pgxx(skbuffer, 1137, 192, 237, 372, r_ab, 0, 0);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ddxx(skbuffer, 1650, 561, 615, 867, r_ab, 0, 0);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dfxx(skbuffer, 1974, 777, 867, 1137, r_ab, 0, 0);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_fdxx(skbuffer, 2514, 1542, 1650, 1974, r_ab, 0, 0);

            t4cfunc::bra_transform<3, 2>(sbuffer, 0, skbuffer, 2514, 0, 0);

            t4cfunc::bra_transform<3, 2>(sbuffer, 35, skbuffer, 2574, 0, 0);

            t4cfunc::bra_transform<3, 2>(sbuffer, 70, skbuffer, 2634, 0, 0);

            t4cfunc::bra_transform<3, 2>(sbuffer, 105, skbuffer, 2694, 0, 0);

            t4cfunc::bra_transform<3, 2>(sbuffer, 140, skbuffer, 2754, 0, 0);

            t4cfunc::bra_transform<3, 2>(sbuffer, 175, skbuffer, 2814, 0, 0);

            t4cfunc::bra_transform<3, 2>(sbuffer, 210, skbuffer, 2874, 0, 0);

            t4cfunc::bra_transform<3, 2>(sbuffer, 245, skbuffer, 2934, 0, 0);

            t4cfunc::bra_transform<3, 2>(sbuffer, 280, skbuffer, 2994, 0, 0);
...
#endif /* ElectronRepulsionGeom1010RecFDSS_hpp */
