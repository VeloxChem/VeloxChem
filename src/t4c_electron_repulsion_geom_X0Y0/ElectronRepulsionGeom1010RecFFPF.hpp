#ifndef ElectronRepulsionGeom1010RecFFPF_hpp
#define ElectronRepulsionGeom1010RecFFPF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(FF|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_ffpf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(25740, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 100, pbuffer, 6600, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 250, pbuffer, 9675, 210, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 2576, pbuffer, 3925, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2676, pbuffer, 6600, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2826, pbuffer, 9675, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3036, pbuffer, 12741, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3316, pbuffer, 15317, 360, ket_width, ket_npgtos);

                pbuffer.scale(pfactors, 0, 1.0 / a_exp, {3925, 4025});

                pbuffer.scale(pfactors, 0, 2.0, {4425, 4575});

                pbuffer.scale(pfactors, 0, 2.0, {5175, 5385});

                pbuffer.scale(pfactors, 0, 1.0 / a_exp, {6600, 6750});

                pbuffer.scale(pfactors, 0, 2.0, {7200, 7425});

                pbuffer.scale(pfactors, 0, 2.0, {8100, 8415});

                pbuffer.scale(pfactors, 0, 1.0 / a_exp, {9675, 9885});

                pbuffer.scale(pfactors, 0, 2.0, {10305, 10620});

                pbuffer.scale(pfactors, 0, 2.0, {11250, 11691});

                t2cfunc::reduce(cbuffer, 460, pbuffer, 3925, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 560, pbuffer, 4425, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 710, pbuffer, 5175, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 920, pbuffer, 6600, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1070, pbuffer, 7200, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1295, pbuffer, 8100, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1610, pbuffer, 9675, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1820, pbuffer, 10305, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2135, pbuffer, 11250, 441, ket_width, ket_npgtos);
                ...
                pbuffer.scale(pfactors, 0, 2.0, {12741, 13021});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {13301, 13721});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {14141, 14729});

                pbuffer.scale(pfactors, 0, 2.0, {15317, 15677});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {15677, 16217});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {16217, 16973});

                t2cfunc::reduce(cbuffer, 3676, pbuffer, 3925, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3776, pbuffer, 4425, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3926, pbuffer, 5175, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4136, pbuffer, 6600, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4286, pbuffer, 7200, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4511, pbuffer, 8100, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4826, pbuffer, 9675, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5036, pbuffer, 10305, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5351, pbuffer, 11250, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5792, pbuffer, 12741, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 6072, pbuffer, 13301, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 6492, pbuffer, 14141, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 7080, pbuffer, 15317, 360, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 7440, pbuffer, 15677, 540, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 7980, pbuffer, 16217, 756, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1650, cbuffer, 920, 1070, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 2100, cbuffer, 1070, 1295, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 2775, cbuffer, 100, 1650, 2100, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 4125, cbuffer, 1610, 1820, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 4755, cbuffer, 1820, 2135, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 5700, cbuffer, 250, 4125, 4755, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 7590, cbuffer, 3676, 3776, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 7890, cbuffer, 3776, 3926, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 8340, cbuffer, 2576, 7590, 7890, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 9240, cbuffer, 4136, 4286, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 9690, cbuffer, 4286, 4511, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 10365, cbuffer, 2676, 9240, 9690, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 11715, cbuffer, 4826, 5036, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 12345, cbuffer, 5036, 5351, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 13290, cbuffer, 2826, 11715, 12345, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 15180, cbuffer, 5792, 6072, cfactors, 6, 0, 6);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 16020, cbuffer, 6072, 6492, cfactors, 6, 0, 6);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 17280, cbuffer, 3036, 15180, 16020, cfactors, 6, 0, 6);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 19800, cbuffer, 7080, 7440, cfactors, 6, 0, 7);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 20880, cbuffer, 7440, 7980, cfactors, 6, 0, 7);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 22500, cbuffer, 3316, 19800, 20880, cfactors, 6, 0, 7);
            ...
            t4cfunc::ket_transform<1, 3>(skbuffer, 6300, ckbuffer, 5700, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 6741, ckbuffer, 6330, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 7182, ckbuffer, 6960, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 98721, ckbuffer, 8340, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 98931, ckbuffer, 8640, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 99141, ckbuffer, 8940, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 99351, ckbuffer, 10365, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 99666, ckbuffer, 10815, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 99981, ckbuffer, 11265, 0, 4);

            t4cfunc::ket_transform<1, 3>(skbuffer, 100296, ckbuffer, 13290, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 100737, ckbuffer, 13920, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 101178, ckbuffer, 14550, 0, 5);

            t4cfunc::ket_transform<1, 3>(skbuffer, 101619, ckbuffer, 17280, 0, 6);

            t4cfunc::ket_transform<1, 3>(skbuffer, 102207, ckbuffer, 18120, 0, 6);

            t4cfunc::ket_transform<1, 3>(skbuffer, 102795, ckbuffer, 18960, 0, 6);

            t4cfunc::ket_transform<1, 3>(skbuffer, 103383, ckbuffer, 22500, 0, 7);

            t4cfunc::ket_transform<1, 3>(skbuffer, 104139, ckbuffer, 23580, 0, 7);

            t4cfunc::ket_transform<1, 3>(skbuffer, 104895, ckbuffer, 24660, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 16884, 0, 2520, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 17514, 210, 2835, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 18144, 420, 3150, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 24444, 2520, 6300, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 25389, 2835, 6741, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 26334, 3150, 7182, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 47691, 16884, 24444, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 48951, 17514, 25389, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 50211, 18144, 26334, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 630, 98721, 99351, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sgxx(skbuffer, 3465, 99351, 100296, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_shxx(skbuffer, 7623, 100296, 101619, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sixx(skbuffer, 11592, 101619, 103383, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pfxx(skbuffer, 18774, 0, 630, 3465, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pgxx(skbuffer, 27279, 2520, 3465, 7623, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_phxx(skbuffer, 35784, 6300, 7623, 11592, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dfxx(skbuffer, 51471, 16884, 18774, 27279, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dgxx(skbuffer, 62811, 24444, 27279, 35784, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ffxx(skbuffer, 79821, 47691, 51471, 62811, r_ab, 1, 3);

            t4cfunc::bra_transform<3, 3>(sbuffer, 0, skbuffer, 79821, 1, 3);

            t4cfunc::bra_transform<3, 3>(sbuffer, 1029, skbuffer, 81921, 1, 3);

            t4cfunc::bra_transform<3, 3>(sbuffer, 2058, skbuffer, 84021, 1, 3);

            t4cfunc::bra_transform<3, 3>(sbuffer, 3087, skbuffer, 86121, 1, 3);

            t4cfunc::bra_transform<3, 3>(sbuffer, 4116, skbuffer, 88221, 1, 3);

            t4cfunc::bra_transform<3, 3>(sbuffer, 5145, skbuffer, 90321, 1, 3);

            t4cfunc::bra_transform<3, 3>(sbuffer, 6174, skbuffer, 92421, 1, 3);

            t4cfunc::bra_transform<3, 3>(sbuffer, 7203, skbuffer, 94521, 1, 3);

            t4cfunc::bra_transform<3, 3>(sbuffer, 8232, skbuffer, 96621, 1, 3);
...
#endif /* ElectronRepulsionGeom1010RecFFPF_hpp */
