#ifndef ElectronRepulsionGeom1010RecSSSF_hpp
#define ElectronRepulsionGeom1010RecSSSF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SS|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_sssf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> pbuffer(180, ket_npgtos);
    ...
    CSimdArray<double> ckbuffer(120, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                erirec::comp_prim_electron_repulsion_spsf(pbuffer, 105, 27, 45, 55, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsg(pbuffer, 135, 55, 75, 90, pfactors, 26, r_pb, a_exp, b_exp);

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {45, 55});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {75, 90});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {105, 135});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {135, 180});

                t2cfunc::reduce(cbuffer, 0, pbuffer, 45, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 10, pbuffer, 75, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 25, pbuffer, 105, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 55, pbuffer, 135, 45, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<0, 3>(skbuffer, 63, ckbuffer, 0, 0, 0);

            t4cfunc::ket_transform<0, 3>(skbuffer, 70, ckbuffer, 10, 0, 0);

            t4cfunc::ket_transform<0, 3>(skbuffer, 77, ckbuffer, 20, 0, 0);
            ...
            erirec::comp_bra_geom1010_hrr_electron_repulsion_ssxx(skbuffer, 0, 63, 84, r_ab, 0, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 0, skbuffer, 0, 0, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 7, skbuffer, 7, 0, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 14, skbuffer, 14, 0, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 21, skbuffer, 21, 0, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 28, skbuffer, 28, 0, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 35, skbuffer, 35, 0, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 42, skbuffer, 42, 0, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 49, skbuffer, 49, 0, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 56, skbuffer, 56, 0, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 0, 0, 3, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1010RecSSSF_hpp */
