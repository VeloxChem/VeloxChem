#ifndef ElectronRepulsionGeom1010RecDFPD_hpp
#define ElectronRepulsionGeom1010RecDFPD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(DF|1/|r-r'||PD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_dfpd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(10098, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 60, pbuffer, 3370, 90, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 925, pbuffer, 2025, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 985, pbuffer, 3370, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1075, pbuffer, 4828, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1201, pbuffer, 6130, 168, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 150, pbuffer, 2025, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 210, pbuffer, 2265, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 310, pbuffer, 2665, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 460, pbuffer, 3370, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 550, pbuffer, 3640, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 700, pbuffer, 4090, 225, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 1369, pbuffer, 2025, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1429, pbuffer, 2265, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1529, pbuffer, 2665, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1679, pbuffer, 3370, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1769, pbuffer, 3640, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1919, pbuffer, 4090, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2144, pbuffer, 4828, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2270, pbuffer, 5080, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2480, pbuffer, 5500, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2795, pbuffer, 6130, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2963, pbuffer, 6298, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3243, pbuffer, 6578, 420, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 2550, cbuffer, 1369, 1429, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 2730, cbuffer, 1429, 1529, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 3030, cbuffer, 925, 2550, 2730, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 3570, cbuffer, 1679, 1769, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 3840, cbuffer, 1769, 1919, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 4290, cbuffer, 985, 3570, 3840, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 5100, cbuffer, 2144, 2270, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 5478, cbuffer, 2270, 2480, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 6108, cbuffer, 1075, 5100, 5478, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 7242, cbuffer, 2795, 2963, cfactors, 6, 0, 6);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 7746, cbuffer, 2963, 3243, cfactors, 6, 0, 6);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 8586, cbuffer, 1201, 7242, 7746, cfactors, 6, 0, 6);
            ...
            t4cfunc::ket_transform<1, 2>(skbuffer, 26910, ckbuffer, 3030, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 27060, ckbuffer, 3210, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 27210, ckbuffer, 3390, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 27360, ckbuffer, 4290, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 27585, ckbuffer, 4560, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 27810, ckbuffer, 4830, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 28035, ckbuffer, 6108, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 28350, ckbuffer, 6486, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 28665, ckbuffer, 6864, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 28980, ckbuffer, 8586, 0, 6);

            t4cfunc::ket_transform<1, 2>(skbuffer, 29400, ckbuffer, 9090, 0, 6);

            t4cfunc::ket_transform<1, 2>(skbuffer, 29820, ckbuffer, 9594, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 7335, 0, 1800, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 7785, 150, 2025, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 8235, 300, 2250, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 450, 26910, 27360, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sgxx(skbuffer, 2475, 27360, 28035, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_shxx(skbuffer, 4500, 28035, 28980, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pfxx(skbuffer, 8685, 0, 450, 2475, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pgxx(skbuffer, 12735, 1800, 2475, 4500, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dfxx(skbuffer, 18810, 7335, 8685, 12735, r_ab, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 18810, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 525, skbuffer, 19710, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1050, skbuffer, 20610, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1575, skbuffer, 21510, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 2100, skbuffer, 22410, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 2625, skbuffer, 23310, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 3150, skbuffer, 24210, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 3675, skbuffer, 25110, 1, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 4200, skbuffer, 26010, 1, 2);
...
#endif /* ElectronRepulsionGeom1010RecDFPD_hpp */
