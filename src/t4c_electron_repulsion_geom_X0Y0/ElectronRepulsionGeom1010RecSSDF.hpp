#ifndef ElectronRepulsionGeom1010RecSSDF_hpp
#define ElectronRepulsionGeom1010RecSSDF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SS|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_ssdf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(2292, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(735, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                erirec::comp_prim_electron_repulsion_sssi(pbuffer, 238, 115, 130, 175, 196, pfactors, 20, 23, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_sssi(pbuffer, 266, 130, 145, 196, 217, pfactors, 20, 23, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsf(pbuffer, 294, 35, 65, 75, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsg(pbuffer, 324, 75, 115, 130, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsh(pbuffer, 369, 130, 175, 196, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsi(pbuffer, 432, 196, 238, 266, pfactors, 26, r_pb, a_exp, b_exp);

                pbuffer.scale(2.0 * a_exp, {65, 75});

                pbuffer.scale(2.0 * a_exp, {115, 130});

                pbuffer.scale(2.0 * a_exp, {294, 324});

                pbuffer.scale(2.0 * a_exp, {324, 369});

                t2cfunc::reduce(cbuffer, 0, pbuffer, 65, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 10, pbuffer, 115, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 25, pbuffer, 294, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 55, pbuffer, 324, 45, ket_width, ket_npgtos);

                pbuffer.scale(pfactors, 0, 2.0, {65, 75});

                pbuffer.scale(pfactors, 0, 2.0, {115, 130});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {175, 196});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {238, 266});

                pbuffer.scale(pfactors, 0, 2.0, {294, 324});

                pbuffer.scale(pfactors, 0, 2.0, {324, 369});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {369, 432});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {432, 516});
                ...
                t2cfunc::reduce(cbuffer, 125, pbuffer, 175, 21, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 146, pbuffer, 238, 28, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 174, pbuffer, 294, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 204, pbuffer, 324, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 249, pbuffer, 369, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 312, pbuffer, 432, 84, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<2, 3>(skbuffer, 315, ckbuffer, 393, 0, 0);

            t4cfunc::ket_transform<2, 3>(skbuffer, 350, ckbuffer, 453, 0, 0);

            t4cfunc::ket_transform<2, 3>(skbuffer, 385, ckbuffer, 513, 0, 0);
            ...
            erirec::comp_bra_geom1010_hrr_electron_repulsion_ssxx(skbuffer, 0, 315, 420, r_ab, 2, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 0, skbuffer, 0, 2, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 35, skbuffer, 35, 2, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 70, skbuffer, 70, 2, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 105, skbuffer, 105, 2, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 140, skbuffer, 140, 2, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 175, skbuffer, 175, 2, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 210, skbuffer, 210, 2, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 245, skbuffer, 245, 2, 3);

            t4cfunc::bra_transform<0, 0>(sbuffer, 280, skbuffer, 280, 2, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 0, 2, 3, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1010RecSSDF_hpp */
