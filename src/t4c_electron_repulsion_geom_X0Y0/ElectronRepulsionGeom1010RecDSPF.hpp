#ifndef ElectronRepulsionGeom1010RecDSPF_hpp
#define ElectronRepulsionGeom1010RecDSPF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(DS|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_dspf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(3960, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 10, pbuffer, 339, 30, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {75, 85});

                pbuffer.scale(2.0 * a_exp, {339, 369});

                pbuffer.scale(2.0 * a_exp, {789, 849});

                pbuffer.scale(2.0 * a_exp, {1341, 1441});

                t2cfunc::reduce(cbuffer, 224, pbuffer, 75, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 234, pbuffer, 339, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 264, pbuffer, 789, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 324, pbuffer, 1341, 100, ket_width, ket_npgtos);
                ...
                pbuffer.scale(pfactors, 0, 1.0 / a_exp, {339, 369});

                pbuffer.scale(pfactors, 0, 2.0, {429, 474});

                pbuffer.scale(pfactors, 0, 2.0, {564, 627});

                t2cfunc::reduce(cbuffer, 40, pbuffer, 75, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 50, pbuffer, 135, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 65, pbuffer, 210, 21, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 86, pbuffer, 339, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 116, pbuffer, 429, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 161, pbuffer, 564, 63, ket_width, ket_npgtos);
                ...
                pbuffer.scale(pfactors, 0, 2.0, {789, 849});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {909, 999});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {1089, 1215});

                pbuffer.scale(pfactors, 0, 2.0, {1341, 1441});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {1441, 1591});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {1591, 1801});

                t2cfunc::reduce(cbuffer, 424, pbuffer, 75, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 434, pbuffer, 135, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 449, pbuffer, 210, 21, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 470, pbuffer, 339, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 500, pbuffer, 429, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 545, pbuffer, 564, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 608, pbuffer, 789, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 668, pbuffer, 909, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 758, pbuffer, 1089, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 884, pbuffer, 1341, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 984, pbuffer, 1441, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1134, pbuffer, 1591, 210, ket_width, ket_npgtos);

            }

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 0, cbuffer, 40, 50, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 30, cbuffer, 50, 65, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 75, cbuffer, 0, 0, 30, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 165, cbuffer, 86, 116, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 255, cbuffer, 116, 161, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 390, cbuffer, 10, 165, 255, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 660, cbuffer, 424, 434, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 690, cbuffer, 434, 449, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 735, cbuffer, 224, 660, 690, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 825, cbuffer, 470, 500, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 915, cbuffer, 500, 545, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 1050, cbuffer, 234, 825, 915, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1320, cbuffer, 608, 668, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 1500, cbuffer, 668, 758, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 1770, cbuffer, 264, 1320, 1500, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 2310, cbuffer, 884, 984, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 2610, cbuffer, 984, 1134, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 3060, cbuffer, 324, 2310, 2610, cfactors, 6, 0, 3);
            ...
            t4cfunc::ket_transform<1, 3>(skbuffer, 5733, ckbuffer, 735, 0, 0);

            t4cfunc::ket_transform<1, 3>(skbuffer, 5754, ckbuffer, 765, 0, 0);

            t4cfunc::ket_transform<1, 3>(skbuffer, 5775, ckbuffer, 795, 0, 0);

            t4cfunc::ket_transform<1, 3>(skbuffer, 5796, ckbuffer, 1050, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 5859, ckbuffer, 1140, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 5922, ckbuffer, 1230, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 5985, ckbuffer, 1770, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 6111, ckbuffer, 1950, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 6237, ckbuffer, 2130, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 6363, ckbuffer, 3060, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 6573, ckbuffer, 3360, 0, 3);

            t4cfunc::ket_transform<1, 3>(skbuffer, 6783, ckbuffer, 3660, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 2142, 0, 252, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 2205, 21, 315, r_ab, 1, 3);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 2268, 42, 378, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ssxx(skbuffer, 63, 5733, 5796, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 441, 5796, 5985, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 1008, 5985, 6363, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_psxx(skbuffer, 2331, 0, 63, 441, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ppxx(skbuffer, 2898, 252, 441, 1008, r_ab, 1, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dsxx(skbuffer, 4599, 2142, 2331, 2898, r_ab, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 0, skbuffer, 4599, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 105, skbuffer, 4725, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 210, skbuffer, 4851, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 315, skbuffer, 4977, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 420, skbuffer, 5103, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 525, skbuffer, 5229, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 630, skbuffer, 5355, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 735, skbuffer, 5481, 1, 3);

            t4cfunc::bra_transform<2, 0>(sbuffer, 840, skbuffer, 5607, 1, 3);
...
#endif /* ElectronRepulsionGeom1010RecDSPF_hpp */
