#ifndef ElectronRepulsionGeom1010RecSPPF_hpp
#define ElectronRepulsionGeom1010RecSPPF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SP|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_sppf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 256, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 30, pbuffer, 532, 60, ket_width, ket_npgtos);

                pbuffer.scale(pfactors, 0, 2.0, {256, 286});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {316, 361});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {406, 469});

                pbuffer.scale(pfactors, 0, 2.0, {532, 592});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {592, 682});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {682, 808});

                t2cfunc::reduce(cbuffer, 90, pbuffer, 256, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 120, pbuffer, 316, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 165, pbuffer, 406, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 228, pbuffer, 532, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 288, pbuffer, 592, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 378, pbuffer, 682, 126, ket_width, ket_npgtos);

            }

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 0, cbuffer, 90, 120, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 90, cbuffer, 120, 165, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 225, cbuffer, 0, 0, 90, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 495, cbuffer, 228, 288, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 675, cbuffer, 288, 378, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 945, cbuffer, 30, 495, 675, cfactors, 6, 0, 2);

            t4cfunc::ket_transform<1, 3>(skbuffer, 567, ckbuffer, 225, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 630, ckbuffer, 315, 0, 1);

            t4cfunc::ket_transform<1, 3>(skbuffer, 693, ckbuffer, 405, 0, 1);
            ...
            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 0, 567, 756, r_ab, 1, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 0, skbuffer, 0, 1, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 63, skbuffer, 63, 1, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 126, skbuffer, 126, 1, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 189, skbuffer, 189, 1, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 252, skbuffer, 252, 1, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 315, skbuffer, 315, 1, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 378, skbuffer, 378, 1, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 441, skbuffer, 441, 1, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 504, skbuffer, 504, 1, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 1, 1, 3, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1010RecSPPF_hpp */
