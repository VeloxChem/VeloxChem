#ifndef ElectronRepulsionGeom1010RecFPPD_hpp
#define ElectronRepulsionGeom1010RecFPPD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(FP|1/|r-r'||PD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_fppd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(7548, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 18, pbuffer, 821, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 54, pbuffer, 1635, 60, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 703, pbuffer, 290, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 721, pbuffer, 821, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 757, pbuffer, 1635, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 817, pbuffer, 2610, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 907, pbuffer, 3540, 126, ket_width, ket_npgtos);

                pbuffer.scale(pfactors, 0, 1.0 / a_exp, {290, 308});

                pbuffer.scale(pfactors, 0, 2.0, {380, 410});

                pbuffer.scale(pfactors, 0, 2.0, {530, 575});
                ...
                t2cfunc::reduce(cbuffer, 114, pbuffer, 290, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 132, pbuffer, 380, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 162, pbuffer, 530, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 207, pbuffer, 821, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 243, pbuffer, 965, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 303, pbuffer, 1205, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 393, pbuffer, 1635, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 453, pbuffer, 1815, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 553, pbuffer, 2115, 150, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 1033, pbuffer, 290, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1051, pbuffer, 380, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1081, pbuffer, 530, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1126, pbuffer, 821, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1162, pbuffer, 965, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1222, pbuffer, 1205, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1312, pbuffer, 1635, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1372, pbuffer, 1815, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1472, pbuffer, 2115, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1622, pbuffer, 2610, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1712, pbuffer, 2790, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1862, pbuffer, 3090, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2087, pbuffer, 3540, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2213, pbuffer, 3666, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2423, pbuffer, 3876, 315, ket_width, ket_npgtos);

            }

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 0, cbuffer, 114, 132, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 54, cbuffer, 132, 162, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 144, cbuffer, 0, 0, 54, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 306, cbuffer, 207, 243, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 414, cbuffer, 243, 303, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 594, cbuffer, 18, 306, 414, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 918, cbuffer, 393, 453, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1098, cbuffer, 453, 553, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 1398, cbuffer, 54, 918, 1098, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 1938, cbuffer, 1033, 1051, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1992, cbuffer, 1051, 1081, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 2082, cbuffer, 703, 1938, 1992, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 2244, cbuffer, 1126, 1162, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 2352, cbuffer, 1162, 1222, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 2532, cbuffer, 721, 2244, 2352, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 2856, cbuffer, 1312, 1372, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 3036, cbuffer, 1372, 1472, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 3336, cbuffer, 757, 2856, 3036, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 3876, cbuffer, 1622, 1712, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 4146, cbuffer, 1712, 1862, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 4596, cbuffer, 817, 3876, 4146, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 5406, cbuffer, 2087, 2213, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 5784, cbuffer, 2213, 2423, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 6414, cbuffer, 907, 5406, 5784, cfactors, 6, 0, 5);
            ...
            t4cfunc::ket_transform<1, 2>(skbuffer, 1620, ckbuffer, 1398, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 1770, ckbuffer, 1578, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 1920, ckbuffer, 1758, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 26505, ckbuffer, 2082, 0, 1);

            t4cfunc::ket_transform<1, 2>(skbuffer, 26550, ckbuffer, 2136, 0, 1);

            t4cfunc::ket_transform<1, 2>(skbuffer, 26595, ckbuffer, 2190, 0, 1);

            t4cfunc::ket_transform<1, 2>(skbuffer, 26640, ckbuffer, 2532, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 26730, ckbuffer, 2640, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 26820, ckbuffer, 2748, 0, 2);

            t4cfunc::ket_transform<1, 2>(skbuffer, 26910, ckbuffer, 3336, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 27060, ckbuffer, 3516, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 27210, ckbuffer, 3696, 0, 3);

            t4cfunc::ket_transform<1, 2>(skbuffer, 27360, ckbuffer, 4596, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 27585, ckbuffer, 4866, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 27810, ckbuffer, 5136, 0, 4);

            t4cfunc::ket_transform<1, 2>(skbuffer, 28035, ckbuffer, 6414, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 28350, ckbuffer, 6792, 0, 5);

            t4cfunc::ket_transform<1, 2>(skbuffer, 28665, ckbuffer, 7170, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 5445, 0, 540, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 5580, 45, 630, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 5715, 90, 720, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 7065, 540, 1620, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 7335, 630, 1770, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 7605, 720, 1920, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 14355, 5445, 7065, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 14625, 5580, 7335, r_ab, 1, 2);

            erirec::comp_bra_hrr_electron_repulsion_dpxx(skbuffer, 14895, 5715, 7605, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 135, 26505, 26640, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 810, 26640, 26910, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 2070, 26910, 27360, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sgxx(skbuffer, 3420, 27360, 28035, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ppxx(skbuffer, 5850, 0, 135, 810, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pdxx(skbuffer, 7875, 540, 810, 2070, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pfxx(skbuffer, 10305, 1620, 2070, 3420, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dpxx(skbuffer, 15165, 5445, 5850, 7875, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ddxx(skbuffer, 17595, 7065, 7875, 10305, r_ab, 1, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_fpxx(skbuffer, 22455, 14355, 15165, 17595, r_ab, 1, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 0, skbuffer, 22455, 1, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 315, skbuffer, 22905, 1, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 630, skbuffer, 23355, 1, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 945, skbuffer, 23805, 1, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 1260, skbuffer, 24255, 1, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 1575, skbuffer, 24705, 1, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 1890, skbuffer, 25155, 1, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 2205, skbuffer, 25605, 1, 2);

            t4cfunc::bra_transform<3, 1>(sbuffer, 2520, skbuffer, 26055, 1, 2);
...
#endif /* ElectronRepulsionGeom1010RecFPPD_hpp */
