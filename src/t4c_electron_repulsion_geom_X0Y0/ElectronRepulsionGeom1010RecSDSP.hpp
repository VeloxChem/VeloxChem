...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SD|1/|r-r'||SP)  integral derivatives.
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(144, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 138, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 18, pbuffer, 174, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 54, pbuffer, 246, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 84, pbuffer, 276, 60, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<0, 1>(skbuffer, 162, ckbuffer, 0, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 180, ckbuffer, 18, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 198, ckbuffer, 36, 0, 2);
            ...
            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 0, 162, 216, r_ab, 0, 1);
