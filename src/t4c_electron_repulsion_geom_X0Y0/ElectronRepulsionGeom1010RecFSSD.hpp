#ifndef ElectronRepulsionGeom1010RecFSSD_hpp
#define ElectronRepulsionGeom1010RecFSSD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(FS|1/|r-r'||SD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_fssd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 72, cbuffer, 64, 100, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 180, cbuffer, 160, 166, cfactors, 6, 0, 0);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 198, cbuffer, 176, 194, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 252, cbuffer, 224, 260, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 360, cbuffer, 320, 380, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 540, cbuffer, 480, 570, cfactors, 6, 0, 4);
            ...
            t4cfunc::ket_transform<0, 2>(skbuffer, 240, ckbuffer, 72, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 270, ckbuffer, 108, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 300, ckbuffer, 144, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 4200, ckbuffer, 180, 0, 0);

            t4cfunc::ket_transform<0, 2>(skbuffer, 4205, ckbuffer, 186, 0, 0);

            t4cfunc::ket_transform<0, 2>(skbuffer, 4210, ckbuffer, 192, 0, 0);

            t4cfunc::ket_transform<0, 2>(skbuffer, 4215, ckbuffer, 198, 0, 1);

            t4cfunc::ket_transform<0, 2>(skbuffer, 4230, ckbuffer, 216, 0, 1);

            t4cfunc::ket_transform<0, 2>(skbuffer, 4245, ckbuffer, 234, 0, 1);

            t4cfunc::ket_transform<0, 2>(skbuffer, 4260, ckbuffer, 252, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 4290, ckbuffer, 288, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 4320, ckbuffer, 324, 0, 2);

            t4cfunc::ket_transform<0, 2>(skbuffer, 4350, ckbuffer, 360, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 4400, ckbuffer, 420, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 4450, ckbuffer, 480, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 4500, ckbuffer, 540, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 4575, ckbuffer, 630, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 4650, ckbuffer, 720, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 1050, 0, 60, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 1065, 5, 75, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_psxx(skbuffer, 1080, 10, 90, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 1230, 60, 240, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 1275, 75, 270, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 1320, 90, 300, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_dsxx(skbuffer, 2580, 1050, 1230, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_dsxx(skbuffer, 2610, 1065, 1275, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_dsxx(skbuffer, 2640, 1080, 1320, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ssxx(skbuffer, 15, 4200, 4215, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 105, 4215, 4260, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 330, 4260, 4350, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 600, 4350, 4500, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_psxx(skbuffer, 1095, 0, 15, 105, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ppxx(skbuffer, 1365, 60, 105, 330, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pdxx(skbuffer, 1770, 240, 330, 600, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dsxx(skbuffer, 2670, 1050, 1095, 1365, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dpxx(skbuffer, 2940, 1230, 1365, 1770, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_fsxx(skbuffer, 3750, 2580, 2670, 2940, r_ab, 0, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 0, skbuffer, 3750, 0, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 35, skbuffer, 3800, 0, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 70, skbuffer, 3850, 0, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 105, skbuffer, 3900, 0, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 140, skbuffer, 3950, 0, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 175, skbuffer, 4000, 0, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 210, skbuffer, 4050, 0, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 245, skbuffer, 4100, 0, 2);

            t4cfunc::bra_transform<3, 0>(sbuffer, 280, skbuffer, 4150, 0, 2);
...
#endif /* ElectronRepulsionGeom1010RecFSSD_hpp */
