...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(PD|1/|r-r'||SP)  integral derivatives.
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 198, cbuffer, 198, 243, cfactors, 6, 0, 4);
            ...
            t4cfunc::ket_transform<0, 1>(skbuffer, 972, ckbuffer, 54, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 990, ckbuffer, 72, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1008, ckbuffer, 90, 0, 2);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1026, ckbuffer, 108, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1056, ckbuffer, 138, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1086, ckbuffer, 168, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1116, ckbuffer, 198, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1161, ckbuffer, 243, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1206, ckbuffer, 288, 0, 4);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 54, 972, 1026, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 216, 1026, 1116, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pdxx(skbuffer, 486, 0, 54, 216, r_ab, 0, 1);
