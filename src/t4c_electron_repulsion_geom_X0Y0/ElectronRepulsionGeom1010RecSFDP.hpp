#ifndef ElectronRepulsionGeom1010RecSFDP_hpp
#define ElectronRepulsionGeom1010RecSFDP_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SF|1/|r-r'||DP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_sfdp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(5025, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 1261, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 30, pbuffer, 1321, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 90, pbuffer, 1941, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 135, pbuffer, 1986, 90, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 225, pbuffer, 1261, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 255, pbuffer, 1321, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 315, pbuffer, 1441, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 415, pbuffer, 1641, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 565, pbuffer, 1941, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 610, pbuffer, 1986, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 700, pbuffer, 2076, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 850, pbuffer, 2226, 225, ket_width, ket_npgtos);
            ...
            t4cfunc::ket_transform<2, 1>(skbuffer, 1350, ckbuffer, 1470, 0, 3);

            t4cfunc::ket_transform<2, 1>(skbuffer, 1500, ckbuffer, 1650, 0, 3);

            t4cfunc::ket_transform<2, 1>(skbuffer, 1650, ckbuffer, 1830, 0, 3);
            ...
            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 0, 1350, 1800, r_ab, 2, 1);

            t4cfunc::bra_transform<0, 3>(sbuffer, 0, skbuffer, 0, 2, 1);

            t4cfunc::bra_transform<0, 3>(sbuffer, 105, skbuffer, 150, 2, 1);

            t4cfunc::bra_transform<0, 3>(sbuffer, 210, skbuffer, 300, 2, 1);

            t4cfunc::bra_transform<0, 3>(sbuffer, 315, skbuffer, 450, 2, 1);

            t4cfunc::bra_transform<0, 3>(sbuffer, 420, skbuffer, 600, 2, 1);

            t4cfunc::bra_transform<0, 3>(sbuffer, 525, skbuffer, 750, 2, 1);

            t4cfunc::bra_transform<0, 3>(sbuffer, 630, skbuffer, 900, 2, 1);

            t4cfunc::bra_transform<0, 3>(sbuffer, 735, skbuffer, 1050, 2, 1);

            t4cfunc::bra_transform<0, 3>(sbuffer, 840, skbuffer, 1200, 2, 1);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 3, 2, 1, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1010RecSFDP_hpp */
