#ifndef ElectronRepulsionGeom1010RecDFSD_hpp
#define ElectronRepulsionGeom1010RecDFSD_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(DF|1/|r-r'||SD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_dfsd(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(1782, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 400, pbuffer, 1180, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 460, pbuffer, 1420, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 560, pbuffer, 1925, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 650, pbuffer, 2195, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 800, pbuffer, 2708, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 926, pbuffer, 2960, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1136, pbuffer, 3380, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1304, pbuffer, 3548, 280, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 450, cbuffer, 400, 460, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 630, cbuffer, 560, 650, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 900, cbuffer, 800, 926, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 1278, cbuffer, 1136, 1304, cfactors, 6, 0, 6);
            ...
            t4cfunc::ket_transform<0, 2>(skbuffer, 8970, ckbuffer, 450, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9020, ckbuffer, 510, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9070, ckbuffer, 570, 0, 3);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9120, ckbuffer, 630, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9195, ckbuffer, 720, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9270, ckbuffer, 810, 0, 4);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9345, ckbuffer, 900, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9450, ckbuffer, 1026, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9555, ckbuffer, 1152, 0, 5);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9660, ckbuffer, 1278, 0, 6);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9800, ckbuffer, 1446, 0, 6);

            t4cfunc::ket_transform<0, 2>(skbuffer, 9940, ckbuffer, 1614, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 2445, 0, 600, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 2595, 50, 675, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 2745, 100, 750, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 150, 8970, 9120, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sgxx(skbuffer, 825, 9120, 9345, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_shxx(skbuffer, 1500, 9345, 9660, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pfxx(skbuffer, 2895, 0, 150, 825, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pgxx(skbuffer, 4245, 600, 825, 1500, r_ab, 0, 2);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dfxx(skbuffer, 6270, 2445, 2895, 4245, r_ab, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 6270, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 175, skbuffer, 6570, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 350, skbuffer, 6870, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 525, skbuffer, 7170, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 700, skbuffer, 7470, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 875, skbuffer, 7770, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1050, skbuffer, 8070, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1225, skbuffer, 8370, 0, 2);

            t4cfunc::bra_transform<2, 3>(sbuffer, 1400, skbuffer, 8670, 0, 2);
...
#endif /* ElectronRepulsionGeom1010RecDFSD_hpp */
