#ifndef ElectronRepulsionGeom1010RecPDFP_hpp
#define ElectronRepulsionGeom1010RecPDFP_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(PD|1/|r-r'||FP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_pdfp(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> cbuffer(2738, 1);

    // allocate aligned contracted integrals

    CSimdArray<double> ckbuffer(21090, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(8757, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 1031, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 18, pbuffer, 1085, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 54, pbuffer, 1193, 60, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 444, pbuffer, 1031, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 462, pbuffer, 1085, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 498, pbuffer, 1193, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 558, pbuffer, 2031, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 588, pbuffer, 2091, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 648, pbuffer, 2211, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 748, pbuffer, 3131, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 793, pbuffer, 3176, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 883, pbuffer, 3266, 150, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 114, pbuffer, 1031, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 132, pbuffer, 1085, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 168, pbuffer, 1193, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 228, pbuffer, 1373, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 318, pbuffer, 1643, 126, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 1033, pbuffer, 1031, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1051, pbuffer, 1085, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1087, pbuffer, 1193, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1147, pbuffer, 1373, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1237, pbuffer, 1643, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1363, pbuffer, 2031, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1393, pbuffer, 2091, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1453, pbuffer, 2211, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1553, pbuffer, 2411, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1703, pbuffer, 2711, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1913, pbuffer, 3131, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1958, pbuffer, 3176, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2048, pbuffer, 3266, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2198, pbuffer, 3416, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2423, pbuffer, 3641, 315, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 4032, cbuffer, 444, 462, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 4248, cbuffer, 462, 498, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdp(ckbuffer, 5220, 4032, 4248, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 7860, cbuffer, 558, 588, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 8220, cbuffer, 588, 648, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdp(ckbuffer, 9840, 7860, 8220, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 14070, cbuffer, 748, 793, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 14610, cbuffer, 793, 883, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdp(ckbuffer, 17040, 14070, 14610, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 0, cbuffer, 114, 132, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 54, cbuffer, 132, 168, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 162, cbuffer, 168, 228, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 342, cbuffer, 228, 318, cfactors, 6, 0, 2);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 3420, cbuffer, 1033, 1051, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 3474, cbuffer, 1051, 1087, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 3582, cbuffer, 1087, 1147, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 3762, cbuffer, 1147, 1237, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpp(ckbuffer, 4086, cbuffer, 444, 3420, 3474, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 4356, cbuffer, 462, 3474, 3582, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 4680, cbuffer, 498, 3582, 3762, cfactors, 6, 0, 2);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 6840, cbuffer, 1363, 1393, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 6930, cbuffer, 1393, 1453, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 7110, cbuffer, 1453, 1553, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 7410, cbuffer, 1553, 1703, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpp(ckbuffer, 7950, cbuffer, 558, 6840, 6930, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 8400, cbuffer, 588, 6930, 7110, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 8940, cbuffer, 648, 7110, 7410, cfactors, 6, 0, 3);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 12540, cbuffer, 1913, 1958, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsd(ckbuffer, 12675, cbuffer, 1958, 2048, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 12945, cbuffer, 2048, 2198, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 13395, cbuffer, 2198, 2423, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpp(ckbuffer, 14205, cbuffer, 748, 12540, 12675, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpd(ckbuffer, 14880, cbuffer, 793, 12675, 12945, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 15690, cbuffer, 883, 12945, 13395, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxdp(ckbuffer, 17310, 14070, 14205, 14880, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxdd(ckbuffer, 18120, 14610, 14880, 15690, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxfp(ckbuffer, 19740, 17040, 17310, 18120, cfactors, 6, 0, 4);
            ...
            t4cfunc::ket_transform<3, 1>(skbuffer, 6804, ckbuffer, 6300, 0, 2);

            t4cfunc::ket_transform<3, 1>(skbuffer, 6930, ckbuffer, 6480, 0, 2);

            t4cfunc::ket_transform<3, 1>(skbuffer, 7056, ckbuffer, 6660, 0, 2);

            t4cfunc::ket_transform<3, 1>(skbuffer, 7182, ckbuffer, 11640, 0, 3);

            t4cfunc::ket_transform<3, 1>(skbuffer, 7392, ckbuffer, 11940, 0, 3);

            t4cfunc::ket_transform<3, 1>(skbuffer, 7602, ckbuffer, 12240, 0, 3);

            t4cfunc::ket_transform<3, 1>(skbuffer, 7812, ckbuffer, 19740, 0, 4);

            t4cfunc::ket_transform<3, 1>(skbuffer, 8127, ckbuffer, 20190, 0, 4);

            t4cfunc::ket_transform<3, 1>(skbuffer, 8442, ckbuffer, 20640, 0, 4);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 378, 6804, 7182, r_ab, 3, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 1512, 7182, 7812, r_ab, 3, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pdxx(skbuffer, 3402, 0, 378, 1512, r_ab, 3, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 0, skbuffer, 3402, 3, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 315, skbuffer, 3780, 3, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 630, skbuffer, 4158, 3, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 945, skbuffer, 4536, 3, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 1260, skbuffer, 4914, 3, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 1575, skbuffer, 5292, 3, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 1890, skbuffer, 5670, 3, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 2205, skbuffer, 6048, 3, 1);

            t4cfunc::bra_transform<1, 2>(sbuffer, 2520, skbuffer, 6426, 3, 1);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 2, 3, 1, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1010RecPDFP_hpp */
