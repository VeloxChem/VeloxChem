#ifndef ElectronRepulsionGeom1010RecSPDF_hpp
#define ElectronRepulsionGeom1010RecSPDF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(SP|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_spdf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(5157, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 75, pbuffer, 840, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 135, pbuffer, 900, 90, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 225, pbuffer, 396, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 255, pbuffer, 456, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 300, pbuffer, 546, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 363, pbuffer, 672, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 447, pbuffer, 840, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 507, pbuffer, 900, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 597, pbuffer, 990, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 723, pbuffer, 1116, 168, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 0, cbuffer, 225, 255, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 90, cbuffer, 255, 300, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 225, cbuffer, 300, 363, cfactors, 6, 0, 1);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1719, cbuffer, 447, 507, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 1899, cbuffer, 507, 597, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 2169, cbuffer, 597, 723, cfactors, 6, 0, 2);
            ...
            t4cfunc::ket_transform<2, 3>(skbuffer, 945, ckbuffer, 1179, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 1050, ckbuffer, 1359, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 1155, ckbuffer, 1539, 0, 1);
            ...
            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 0, 945, 1260, r_ab, 2, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 0, skbuffer, 0, 2, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 105, skbuffer, 105, 2, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 210, skbuffer, 210, 2, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 315, skbuffer, 315, 2, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 420, skbuffer, 420, 2, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 525, skbuffer, 525, 2, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 630, skbuffer, 630, 2, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 735, skbuffer, 735, 2, 3);

            t4cfunc::bra_transform<0, 1>(sbuffer, 840, skbuffer, 840, 2, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 1, 2, 3, j, ket_range);
        }
    }
...
#endif /* ElectronRepulsionGeom1010RecSPDF_hpp */
