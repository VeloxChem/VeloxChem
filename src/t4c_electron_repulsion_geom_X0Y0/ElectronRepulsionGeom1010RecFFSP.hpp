#ifndef ElectronRepulsionGeom1010RecFFSP_hpp
#define ElectronRepulsionGeom1010RecFFSP_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(FF|1/|r-r'||SP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_ffsp(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(pfactors, 0, 2.0, {686, 716});

                pbuffer.scale(pfactors, 0, 2.0, {836, 896});

                pbuffer.scale(pfactors, 0, 2.0, {1181, 1226});

                pbuffer.scale(pfactors, 0, 2.0, {1361, 1451});

                pbuffer.scale(pfactors, 0, 2.0, {1763, 1826});

                pbuffer.scale(pfactors, 0, 2.0, {1952, 2078});
                ...
                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {2358, 2442});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {2526, 2694});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {2862, 2970});

                pbuffer.scale(pfactors, 0, 4.0 * a_exp, {2970, 3186});
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 225, cbuffer, 225, 288, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 414, cbuffer, 414, 444, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 504, cbuffer, 504, 549, cfactors, 6, 0, 4);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 639, cbuffer, 639, 702, cfactors, 6, 0, 5);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 828, cbuffer, 828, 912, cfactors, 6, 0, 6);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsp(ckbuffer, 1080, cbuffer, 1080, 1188, cfactors, 6, 0, 7);
            ...
            t4cfunc::ket_transform<0, 1>(skbuffer, 900, ckbuffer, 225, 0, 5);

            t4cfunc::ket_transform<0, 1>(skbuffer, 963, ckbuffer, 288, 0, 5);

            t4cfunc::ket_transform<0, 1>(skbuffer, 1026, ckbuffer, 351, 0, 5);

            t4cfunc::ket_transform<0, 1>(skbuffer, 14103, ckbuffer, 414, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 14133, ckbuffer, 444, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 14163, ckbuffer, 474, 0, 3);

            t4cfunc::ket_transform<0, 1>(skbuffer, 14193, ckbuffer, 504, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 14238, ckbuffer, 549, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 14283, ckbuffer, 594, 0, 4);

            t4cfunc::ket_transform<0, 1>(skbuffer, 14328, ckbuffer, 639, 0, 5);

            t4cfunc::ket_transform<0, 1>(skbuffer, 14391, ckbuffer, 702, 0, 5);

            t4cfunc::ket_transform<0, 1>(skbuffer, 14454, ckbuffer, 765, 0, 5);

            t4cfunc::ket_transform<0, 1>(skbuffer, 14517, ckbuffer, 828, 0, 6);

            t4cfunc::ket_transform<0, 1>(skbuffer, 14601, ckbuffer, 912, 0, 6);

            t4cfunc::ket_transform<0, 1>(skbuffer, 14685, ckbuffer, 996, 0, 6);

            t4cfunc::ket_transform<0, 1>(skbuffer, 14769, ckbuffer, 1080, 0, 7);

            t4cfunc::ket_transform<0, 1>(skbuffer, 14877, ckbuffer, 1188, 0, 7);

            t4cfunc::ket_transform<0, 1>(skbuffer, 14985, ckbuffer, 1296, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 2412, 0, 360, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 2502, 30, 405, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 2592, 60, 450, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 3492, 360, 900, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 3627, 405, 963, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 3762, 450, 1026, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 6813, 2412, 3492, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 6993, 2502, 3627, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 7173, 2592, 3762, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sfxx(skbuffer, 90, 14103, 14193, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sgxx(skbuffer, 495, 14193, 14328, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_shxx(skbuffer, 1089, 14328, 14517, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sixx(skbuffer, 1656, 14517, 14769, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pfxx(skbuffer, 2682, 0, 90, 495, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_pgxx(skbuffer, 3897, 360, 495, 1089, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_phxx(skbuffer, 5112, 900, 1089, 1656, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dfxx(skbuffer, 7353, 2412, 2682, 3897, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_dgxx(skbuffer, 8973, 3492, 3897, 5112, r_ab, 0, 1);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ffxx(skbuffer, 11403, 6813, 7353, 8973, r_ab, 0, 1);

            t4cfunc::bra_transform<3, 3>(sbuffer, 0, skbuffer, 11403, 0, 1);

            t4cfunc::bra_transform<3, 3>(sbuffer, 147, skbuffer, 11703, 0, 1);

            t4cfunc::bra_transform<3, 3>(sbuffer, 294, skbuffer, 12003, 0, 1);

            t4cfunc::bra_transform<3, 3>(sbuffer, 441, skbuffer, 12303, 0, 1);

            t4cfunc::bra_transform<3, 3>(sbuffer, 588, skbuffer, 12603, 0, 1);

            t4cfunc::bra_transform<3, 3>(sbuffer, 735, skbuffer, 12903, 0, 1);

            t4cfunc::bra_transform<3, 3>(sbuffer, 882, skbuffer, 13203, 0, 1);

            t4cfunc::bra_transform<3, 3>(sbuffer, 1029, skbuffer, 13503, 0, 1);

            t4cfunc::bra_transform<3, 3>(sbuffer, 1176, skbuffer, 13803, 0, 1);
...
#endif /* ElectronRepulsionGeom1010RecFFSP_hpp */
