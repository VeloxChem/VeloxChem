#ifndef ElectronRepulsionGeom1010RecPPDF_hpp
#define ElectronRepulsionGeom1010RecPPDF_hpp
...
/// @brief Computes d^(1)/dA^(1)d^(1)/dC^(1)(PP|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom1010_ppdf(T& distributor,
                                      ...
                                      const std::pair<size_t, size_t>& ket_indices) -> void
{
    ...
    CSimdArray<double> ckbuffer(12606, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 297, pbuffer, 507, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 327, pbuffer, 597, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 372, pbuffer, 1209, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 432, pbuffer, 1329, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 522, pbuffer, 2097, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 622, pbuffer, 2197, 150, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 75, pbuffer, 507, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 105, pbuffer, 597, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 732, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 213, pbuffer, 921, 84, ket_width, ket_npgtos);
                ...
                t2cfunc::reduce(cbuffer, 772, pbuffer, 507, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 802, pbuffer, 597, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 847, pbuffer, 732, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 910, pbuffer, 921, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 994, pbuffer, 1209, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1054, pbuffer, 1329, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1144, pbuffer, 1509, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1270, pbuffer, 1761, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1438, pbuffer, 2097, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1538, pbuffer, 2197, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1688, pbuffer, 2347, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1898, pbuffer, 2557, 280, ket_width, ket_npgtos);

            }

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 414, cbuffer, 0, 30, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 2133, cbuffer, 297, 327, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 4266, cbuffer, 372, 432, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 8256, cbuffer, 522, 622, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 0, cbuffer, 75, 105, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 90, cbuffer, 105, 150, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 225, cbuffer, 150, 213, cfactors, 6, 0, 1);
            ...
            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 1719, cbuffer, 772, 802, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 1809, cbuffer, 802, 847, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 1944, cbuffer, 847, 910, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 2223, cbuffer, 297, 1719, 1809, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpg(ckbuffer, 2493, cbuffer, 327, 1809, 1944, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxdf(ckbuffer, 2898, 2133, 2223, 2493, cfactors, 6, 0, 1);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 3438, cbuffer, 994, 1054, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 3618, cbuffer, 1054, 1144, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 3888, cbuffer, 1144, 1270, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 4446, cbuffer, 372, 3438, 3618, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpg(ckbuffer, 4986, cbuffer, 432, 3618, 3888, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxdf(ckbuffer, 5796, 4266, 4446, 4986, cfactors, 6, 0, 2);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsf(ckbuffer, 6876, cbuffer, 1438, 1538, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsg(ckbuffer, 7176, cbuffer, 1538, 1688, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxsh(ckbuffer, 7626, cbuffer, 1688, 1898, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpf(ckbuffer, 8556, cbuffer, 522, 6876, 7176, cfactors, 6, 0, 3);

            erirec::comp_ket_geom10_hrr_electron_repulsion_xxpg(ckbuffer, 9456, cbuffer, 622, 7176, 7626, cfactors, 6, 0, 3);
            ...
            t4cfunc::ket_transform<2, 3>(skbuffer, 5985, ckbuffer, 2898, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 6090, ckbuffer, 3078, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 6195, ckbuffer, 3258, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 6300, ckbuffer, 5796, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 6510, ckbuffer, 6156, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 6720, ckbuffer, 6516, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 6930, ckbuffer, 10806, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 7280, ckbuffer, 11406, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 7630, ckbuffer, 12006, 0, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_spxx(skbuffer, 315, 5985, 6300, r_ab, 2, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_sdxx(skbuffer, 1260, 6300, 6930, r_ab, 2, 3);

            erirec::comp_bra_geom1010_hrr_electron_repulsion_ppxx(skbuffer, 3150, 0, 315, 1260, r_ab, 2, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 3150, 2, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 315, skbuffer, 3465, 2, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 630, skbuffer, 3780, 2, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 945, skbuffer, 4095, 2, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 1260, skbuffer, 4410, 2, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 1575, skbuffer, 4725, 2, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 1890, skbuffer, 5040, 2, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 2205, skbuffer, 5355, 2, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 2520, skbuffer, 5670, 2, 3);
...
#endif /* ElectronRepulsionGeom1010RecPPDF_hpp */
