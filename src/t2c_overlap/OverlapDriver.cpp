...
auto
COverlapDriver::compute(const CMolecularBasis& basis,
                ...
                #pragma omp task firstprivate(i)
                {
                    for (const auto& task : ptr_work_groups[i])
                    {
                        ...
                        else
                        {
                            ...
                            ovlfunc::compute(ptr_submatrix, bra_gto_block, ket_gto_block,
                                             bra_angmom, ket_angmom, ang_order,
                                             task[2], task[3], mat_type);
                        }
                    }
                }
            }
