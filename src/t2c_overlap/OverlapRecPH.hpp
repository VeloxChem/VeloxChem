#ifndef OverlapRecPH_hpp
#define OverlapRecPH_hpp
...
/// @brief Computes (P|H)  integrals for pair of basis functions blocks.
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_ph(pbuffer, 56, 20, 35, factors, 8, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 56, ket_width, ket_npgtos);
            }

            t2cfunc::transform<1, 5>(sbuffer, cbuffer);

            distributor.distribute(sbuffer, bra_gto_indices, ket_gto_indices, 1, 5, j, ket_range, bra_eq_ket);
        }
    }
}
