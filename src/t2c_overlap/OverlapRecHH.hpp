#ifndef OverlapRecHH_hpp
#define OverlapRecHH_hpp
...
/// @brief Computes (H|H)  integrals for pair of basis functions blocks.
                ...
                const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(1974, ket_npgtos);

    // allocate aligned contracted integrals

    CSimdArray<double> cbuffer(441, 1);

    CSimdArray<double> sbuffer(121, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_fg(pbuffer, 633, 113, 257, 317, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_fh(pbuffer, 783, 158, 317, 407, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_gg(pbuffer, 993, 317, 533, 633, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_gh(pbuffer, 1218, 407, 633, 783, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_hh(pbuffer, 1533, 783, 993, 1218, factors, 8, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 1533, ket_width, ket_npgtos);
            }
