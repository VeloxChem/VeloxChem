#include "PrimitiveOverlapDS.hpp"
...
auto
compPrimitiveOverlapDS(TDoubleArray&       buffer_xx,
...
#pragma omp simd aligned(fints_xx, fints_xy, fints_xz, fints_yy, fints_yz, fints_zz, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += fss * ((1.0 / 2.0) * fe_0 + rpa_x * rpa_x);

        fints_xy[i] += fss * rpa_y * rpa_x;

        fints_xz[i] += fss * rpa_z * rpa_x;

        fints_yy[i] += fss * ((1.0 / 2.0) * fe_0 + rpa_y * rpa_y);

        fints_yz[i] += fss * rpa_z * rpa_y;

        fints_zz[i] += fss * ((1.0 / 2.0) * fe_0 + rpa_z * rpa_z);
    }
}
