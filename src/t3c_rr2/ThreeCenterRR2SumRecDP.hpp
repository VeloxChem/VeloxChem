#ifndef ThreeCenterRR2SumRecDP_hpp
#define ThreeCenterRR2SumRecDP_hpp
...
/// @brief Computes (D|GR.R2(r)|P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_r_r2_dp(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3r2rec::comp_prim_r2_pp(pbuffer, 17, 1, 2, 5, 8, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ds(pbuffer, 26, 1, 5, factors, 11, a_exp, exgtos[l]);

                    t3r2rec::comp_prim_r2_ds(pbuffer, 32, 1, 5, 26, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dp(pbuffer, 38, 2, 5, 8, factors, 11, a_exp, exgtos[l]);

                    t3r2rec::comp_prim_r2_dp(pbuffer, 110, 2, 5, 8, 26, 38, factors, 20, a_exp, exgtos[l]);

                    t3rr2rec::comp_prim_r_r2_dp(pbuffer, 56, 8, 17, 26, 32, 38, 110, factors, 20, a_exp, exgtos[l]);
...
#endif /* ThreeCenterRR2SumRecDP_hpp */
