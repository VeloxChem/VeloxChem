#ifndef ThreeCenterRR2SumRecFP_hpp
#define ThreeCenterRR2SumRecFP_hpp
...
/// @brief Computes (F|GR.R2(r)|P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_r_r2_fp(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3r2rec::comp_prim_r2_dp(pbuffer, 41, 2, 5, 8, 17, 23, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fs(pbuffer, 59, 5, 17, factors, 11, a_exp, exgtos[l]);

                    t3r2rec::comp_prim_r2_fs(pbuffer, 69, 5, 17, 59, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fp(pbuffer, 79, 8, 17, 23, factors, 11, a_exp, exgtos[l]);

                    t3r2rec::comp_prim_r2_fp(pbuffer, 199, 8, 17, 23, 59, 79, factors, 20, a_exp, exgtos[l]);

                    t3rr2rec::comp_prim_r_r2_fp(pbuffer, 109, 23, 41, 59, 69, 79, 199, factors, 20, a_exp, exgtos[l]);
...
#endif /* ThreeCenterRR2SumRecFP_hpp */
