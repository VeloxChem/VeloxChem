#ifndef ThreeCenterRR2SumRecDD_hpp
#define ThreeCenterRR2SumRecDD_hpp
...
/// @brief Computes (D|GR.R2(r)|D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_r_r2_dd(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3r2rec::comp_prim_r2_pd(pbuffer, 41, 2, 5, 11, 14, 23, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ds(pbuffer, 59, 1, 11, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dp(pbuffer, 65, 2, 11, 14, factors, 11, a_exp, exgtos[l]);

                    t3r2rec::comp_prim_r2_dp(pbuffer, 83, 2, 11, 14, 59, 65, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dd(pbuffer, 101, 5, 14, 23, factors, 11, a_exp, exgtos[l]);

                    t3r2rec::comp_prim_r2_dd(pbuffer, 245, 5, 14, 23, 59, 65, 101, factors, 20, a_exp, exgtos[l]);

                    t3rr2rec::comp_prim_r_r2_dd(pbuffer, 137, 23, 41, 65, 83, 101, 245, factors, 20, a_exp, exgtos[l]);

                    t2cfunc::reduce(cbuffer, pbuffer, 137, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterRR2SumRecDD_hpp */
