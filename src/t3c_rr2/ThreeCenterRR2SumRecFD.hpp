#ifndef ThreeCenterRR2SumRecFD_hpp
#define ThreeCenterRR2SumRecFD_hpp
...
/// @brief Computes (F|GR.R2(r)|D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_r_r2_fd(T& distributor,
                 ...
                 const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(507, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3r2rec::comp_prim_r2_dd(pbuffer, 101, 5, 14, 23, 41, 47, 65, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fs(pbuffer, 137, 11, 41, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fp(pbuffer, 147, 14, 41, 47, factors, 11, a_exp, exgtos[l]);

                    t3r2rec::comp_prim_r2_fp(pbuffer, 177, 14, 41, 47, 137, 147, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fd(pbuffer, 207, 23, 47, 65, factors, 11, a_exp, exgtos[l]);

                    t3r2rec::comp_prim_r2_fd(pbuffer, 447, 23, 47, 65, 137, 147, 207, factors, 20, a_exp, exgtos[l]);

                    t3rr2rec::comp_prim_r_r2_fd(pbuffer, 267, 65, 101, 147, 177, 207, 447, factors, 20, a_exp, exgtos[l]);

                    t2cfunc::reduce(cbuffer, pbuffer, 267, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterRR2SumRecFD_hpp */
