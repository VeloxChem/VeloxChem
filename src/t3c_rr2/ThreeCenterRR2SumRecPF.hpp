#ifndef ThreeCenterRR2SumRecPF_hpp
#define ThreeCenterRR2SumRecPF_hpp
...
/// @brief Computes (P|GR.R2(r)|F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_r_r2_pf(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3r2rec::comp_prim_r2_sf(pbuffer, 21, 2, 5, 11, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ps(pbuffer, 31, 1, factors, 11);

                    t3ovlrec::comp_prim_overlap_pp(pbuffer, 34, 1, 2, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pd(pbuffer, 43, 2, 5, factors, 11, a_exp, exgtos[l]);

                    t3r2rec::comp_prim_r2_pd(pbuffer, 61, 2, 5, 31, 34, 43, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pf(pbuffer, 79, 5, 11, factors, 11, a_exp, exgtos[l]);

                    t3r2rec::comp_prim_r2_pf(pbuffer, 199, 5, 11, 34, 43, 79, factors, 20, a_exp, exgtos[l]);

                    t3rr2rec::comp_prim_r_r2_pf(pbuffer, 109, 11, 21, 43, 61, 79, 199, factors, 20, a_exp, exgtos[l]);

                    t2cfunc::reduce(cbuffer, pbuffer, 109, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterRR2SumRecPF_hpp */
