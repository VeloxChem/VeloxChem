#ifndef ThreeCenterRR2SumRecFG_hpp
#define ThreeCenterRR2SumRecFG_hpp
...
/// @brief Computes (F|GR.R2(r)|G)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_r_r2_fg(T& distributor,
                 ...
                 const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(1481, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3r2rec::comp_prim_r2_dg(pbuffer, 351, 21, 66, 96, 165, 201, 261, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fp(pbuffer, 441, 39, 141, 147, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fd(pbuffer, 471, 48, 147, 165, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ff(pbuffer, 531, 66, 165, 201, factors, 11, a_exp, exgtos[l]);

                    t3r2rec::comp_prim_r2_ff(pbuffer, 631, 66, 165, 201, 441, 471, 531, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fg(pbuffer, 731, 96, 201, 261, factors, 11, a_exp, exgtos[l]);

                    t3r2rec::comp_prim_r2_fg(pbuffer, 1331, 96, 201, 261, 471, 531, 731, factors, 20, a_exp, exgtos[l]);

                    t3rr2rec::comp_prim_r_r2_fg(pbuffer, 881, 261, 351, 531, 631, 731, 1331, factors, 20, a_exp, exgtos[l]);

                    t2cfunc::reduce(cbuffer, pbuffer, 881, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterRR2SumRecFG_hpp */
