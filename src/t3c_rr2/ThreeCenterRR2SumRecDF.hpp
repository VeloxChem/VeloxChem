#ifndef ThreeCenterRR2SumRecDF_hpp
#define ThreeCenterRR2SumRecDF_hpp
...
/// @brief Computes (D|GR.R2(r)|F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_r_r2_df(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3r2rec::comp_prim_r2_pf(pbuffer, 81, 5, 11, 24, 33, 51, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ds(pbuffer, 111, 1, 21, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dp(pbuffer, 117, 2, 21, 24, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dd(pbuffer, 135, 5, 24, 33, factors, 11, a_exp, exgtos[l]);

                    t3r2rec::comp_prim_r2_dd(pbuffer, 171, 5, 24, 33, 111, 117, 135, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_df(pbuffer, 207, 11, 33, 51, factors, 11, a_exp, exgtos[l]);

                    t3r2rec::comp_prim_r2_df(pbuffer, 447, 11, 33, 51, 117, 135, 207, factors, 20, a_exp, exgtos[l]);

                    t3rr2rec::comp_prim_r_r2_df(pbuffer, 267, 51, 81, 135, 171, 207, 447, factors, 20, a_exp, exgtos[l]);
...
#endif /* ThreeCenterRR2SumRecDF_hpp */
