#ifndef ThreeCenterRR2SumRecDG_hpp
#define ThreeCenterRR2SumRecDG_hpp
...
/// @brief Computes (D|GR.R2(r)|G)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_r_r2_dg(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3r2rec::comp_prim_r2_pg(pbuffer, 141, 11, 21, 48, 66, 96, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dp(pbuffer, 186, 2, 36, 39, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dd(pbuffer, 204, 5, 39, 48, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_df(pbuffer, 240, 11, 48, 66, factors, 11, a_exp, exgtos[l]);

                    t3r2rec::comp_prim_r2_df(pbuffer, 300, 11, 48, 66, 186, 204, 240, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dg(pbuffer, 360, 21, 66, 96, factors, 11, a_exp, exgtos[l]);

                    t3r2rec::comp_prim_r2_dg(pbuffer, 720, 21, 66, 96, 204, 240, 360, factors, 20, a_exp, exgtos[l]);

                    t3rr2rec::comp_prim_r_r2_dg(pbuffer, 450, 96, 141, 240, 300, 360, 720, factors, 20, a_exp, exgtos[l]);
...
#endif /* ThreeCenterRR2SumRecDG_hpp */
