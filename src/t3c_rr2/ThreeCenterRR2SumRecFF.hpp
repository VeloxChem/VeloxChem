#ifndef ThreeCenterRR2SumRecFF_hpp
#define ThreeCenterRR2SumRecFF_hpp
...
/// @brief Computes (F|GR.R2(r)|F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_r_r2_ff(T& distributor,
                 ...
                 const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(921, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3r2rec::comp_prim_r2_df(pbuffer, 201, 11, 33, 51, 87, 105, 141, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fs(pbuffer, 261, 21, 81, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fp(pbuffer, 271, 24, 81, 87, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fd(pbuffer, 301, 33, 87, 105, factors, 11, a_exp, exgtos[l]);

                    t3r2rec::comp_prim_r2_fd(pbuffer, 361, 33, 87, 105, 261, 271, 301, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ff(pbuffer, 421, 51, 105, 141, factors, 11, a_exp, exgtos[l]);

                    t3r2rec::comp_prim_r2_ff(pbuffer, 821, 51, 105, 141, 271, 301, 421, factors, 20, a_exp, exgtos[l]);

                    t3rr2rec::comp_prim_r_r2_ff(pbuffer, 521, 141, 201, 301, 361, 421, 821, factors, 20, a_exp, exgtos[l]);

                    t2cfunc::reduce(cbuffer, pbuffer, 521, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterRR2SumRecFF_hpp */
