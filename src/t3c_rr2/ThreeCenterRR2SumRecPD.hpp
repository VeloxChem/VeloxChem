#ifndef ThreeCenterRR2SumRecPD_hpp
#define ThreeCenterRR2SumRecPD_hpp
...
/// @brief Computes (P|GR.R2(r)|D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_r_r2_pd(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3r2rec::comp_prim_r2_sd(pbuffer, 11, 1, 2, 5, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ps(pbuffer, 17, 1, factors, 11);

                    t3ovlrec::comp_prim_overlap_pp(pbuffer, 20, 1, 2, factors, 11, a_exp, exgtos[l]);

                    t3r2rec::comp_prim_r2_pp(pbuffer, 29, 1, 2, 17, 20, factors, 20, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pd(pbuffer, 38, 2, 5, factors, 11, a_exp, exgtos[l]);

                    t3r2rec::comp_prim_r2_pd(pbuffer, 110, 2, 5, 17, 20, 38, factors, 20, a_exp, exgtos[l]);

                    t3rr2rec::comp_prim_r_r2_pd(pbuffer, 56, 5, 11, 20, 29, 38, 110, factors, 20, a_exp, exgtos[l]);
...
#endif /* ThreeCenterRR2SumRecPD_hpp */
