#ifndef NuclearPotentialGeom110SumRecPP_hpp
#define NuclearPotentialGeom110SumRecPP_hpp
...
/// @brief Computes (d^(1)/dA^(1)P|AG(1)|P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_nuclear_potential_geom_110_pp(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    npotrec::comp_prim_nuclear_potential_geom_010_pp(pbuffer, 77, 5, 8, 17, 23, 32, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_pp(pbuffer, 104, 8, 11, 20, 32, 41, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_dp(pbuffer, 131, 23, 32, 50, 59, 68, 77, 104, factors, 11, 17, a_exp);

                    t2cgeom::comp_prim_op_geom_10_px(pbuffer, 185, 23, 131, 3, 3, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 185, dipoles, 3, l, ket_width, ket_npgtos);
                }
            }
...
#endif /* NuclearPotentialGeom110SumRecPP_hpp */
