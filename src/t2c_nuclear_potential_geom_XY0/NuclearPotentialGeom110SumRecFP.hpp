#ifndef NuclearPotentialGeom110SumRecFP_hpp
#define NuclearPotentialGeom110SumRecFP_hpp
...
/// @brief Computes (d^(1)/dA^(1)F|AG(1)|P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_nuclear_potential_geom_110_fp(T&                               distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    t2cgeom::comp_prim_op_geom_10_fx(pbuffer, 931, 364, 796, 3, 3, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 931, dipoles, 3, l, ket_width, ket_npgtos);
                }
            }
...
#endif /* NuclearPotentialGeom110SumRecFP_hpp */
