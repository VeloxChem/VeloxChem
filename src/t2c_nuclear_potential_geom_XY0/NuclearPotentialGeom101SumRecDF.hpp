#ifndef NuclearPotentialGeom101SumRecDF_hpp
#define NuclearPotentialGeom101SumRecDF_hpp
...
/// @brief Computes (d^(1)/dA^(1)D|A|d^(1)/dB^(1)F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_nuclear_potential_geom_11_df(T& distributor,
                                      ...
                                      const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(1649, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    npotrec::comp_prim_nuclear_potential_ps(pbuffer, 176, 1, 2, factors, 11, 17);

                    npotrec::comp_prim_nuclear_potential_ps(pbuffer, 179, 2, 3, factors, 11, 17);

                    npotrec::comp_prim_nuclear_potential_ps(pbuffer, 182, 3, 4, factors, 11, 17);

                    npotrec::comp_prim_nuclear_potential_pp(pbuffer, 185, 1, 2, 9, 12, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_pp(pbuffer, 194, 2, 3, 12, 15, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_pp(pbuffer, 203, 3, 4, 15, 18, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_pd(pbuffer, 212, 9, 12, 30, 36, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_pd(pbuffer, 230, 12, 15, 36, 42, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_pd(pbuffer, 248, 15, 18, 42, 48, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_pf(pbuffer, 266, 30, 36, 66, 76, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_pf(pbuffer, 296, 36, 42, 76, 86, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_pf(pbuffer, 326, 42, 48, 86, 96, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_pg(pbuffer, 356, 66, 76, 116, 131, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_pg(pbuffer, 401, 76, 86, 131, 146, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_pg(pbuffer, 446, 86, 96, 146, 161, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_dp(pbuffer, 491, 9, 12, 176, 179, 185, 194, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_dp(pbuffer, 509, 12, 15, 179, 182, 194, 203, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_dd(pbuffer, 527, 30, 36, 185, 194, 212, 230, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_dd(pbuffer, 563, 36, 42, 194, 203, 230, 248, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_df(pbuffer, 599, 66, 76, 212, 230, 266, 296, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_df(pbuffer, 659, 76, 86, 230, 248, 296, 326, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_dg(pbuffer, 719, 116, 131, 266, 296, 356, 401, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_dg(pbuffer, 809, 131, 146, 296, 326, 401, 446, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_fd(pbuffer, 899, 212, 230, 491, 509, 527, 563, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_fg(pbuffer, 959, 356, 401, 599, 659, 719, 809, factors, 11, 17, a_exp);

                    t2cgeom::comp_prim_op_geom_11_df(pbuffer, 1109, 212, 356, 899, 959, 1, factors, a_exp);
...
#endif /* NuclearPotentialGeom101SumRecDF_hpp */
