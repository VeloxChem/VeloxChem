                                      ...
                                      const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> cart_buffer_0_geom2000_pdpd(1944, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_geom2000_pdpd(1350, ket_dim);
    ...
    {
        cart_buffer_0_geom2000_pdpd.zero();

        spher_buffer_0_geom2000_pdpd.zero();
        ...
        for (int j = 0; j < bra_npgtos; j++)
        {
            ...
            t4c_geom::comp_geom2000_pdpd_0(prim_buffer_0_geom2000_pdpd, prim_buffer_0_pdpd, prim_buffer_0_fdpd, a_exp );

            t2cfunc::reduce(cart_buffer_0_geom2000_pdpd, prim_buffer_0_geom2000_pdpd, ket_dim, ket_npgtos);
        }

        t4cfunc::full_transform<1, 2, 1, 2>(spher_buffer_0_geom2000_pdpd, cart_spher_buffer_0_geom2000_pdpd);

        distributor->distribute(spher_buffer_0_geom2000_pdpd, a_indices, b_indices, c_indices, d_indices, 1, 2, 1, 2, 2, 0, 0, 0, i, ket_indices);
    }
}
