...
#include "ElectronRepulsionPrimRecDSSP.hpp"
...
/// Computes (SS|1/|r-r'||SP)  integrals for two GTOs pair blocks.
                                      ...
                                      const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> prim_buffer_0_dssp(18, ket_pdim);

    // allocate aligned Cartesian integrals

    CSimdArray<double> cart_buffer_0_geom2000_sssp(18, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_geom2000_sssp(18, ket_dim);
    ...
    {
        cart_buffer_0_geom2000_sssp.zero();

        spher_buffer_0_geom2000_sssp.zero();
        ...
        for (int j = 0; j < bra_npgtos; j++)
        {
            ...
            erirec::comp_prim_electron_repulsion_dssp(prim_buffer_0_dssp, prim_buffer_0_sssp, prim_buffer_1_sssp, prim_buffer_1_psss, prim_buffer_0_pssp, prim_buffer_1_pssp, pa_x, pa_y, pa_z, wp_x[0], wp_y[0], wp_z[0], a_exp, b_exp, c_exps[0], d_exps[0]);

            t4c_geom::comp_geom2000_sssp_0(prim_buffer_0_geom2000_sssp, prim_buffer_0_sssp, prim_buffer_0_dssp, a_exp );

            t2cfunc::reduce(cart_buffer_0_geom2000_sssp, prim_buffer_0_geom2000_sssp, ket_dim, ket_npgtos);
        }

        t4cfunc::full_transform<0, 0, 0, 1>(spher_buffer_0_geom2000_sssp, cart_spher_buffer_0_geom2000_sssp);

        distributor->distribute(spher_buffer_0_geom2000_sssp, a_indices, b_indices, c_indices, d_indices, 0, 0, 0, 1, 2, 0, 0, 0, i, ket_indices);
    }
}
