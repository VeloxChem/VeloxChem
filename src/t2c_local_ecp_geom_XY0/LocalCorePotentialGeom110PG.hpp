#ifndef LocalCorePotentialGeom110PG_hpp
#define LocalCorePotentialGeom110PG_hpp
...
template <class T>
auto
comp_local_core_potential_geom_110_pg(T& distributor,
                                      ...
                                      const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(1043, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < ecp_nppt; l++)
                {
                    ...
                    t2lecp::comp_prim_local_core_potential_pd(pbuffer, 56, 1, 4, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_pf(pbuffer, 74, 4, 10, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_pg(pbuffer, 104, 10, 20, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_ph(pbuffer, 149, 20, 35, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_df(pbuffer, 212, 10, 56, 74, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dg(pbuffer, 272, 20, 74, 104, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dh(pbuffer, 362, 35, 104, 149, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_fg(pbuffer, 488, 104, 212, 272, pfactors, 8, 14);

                    t2cgeom::comp_prim_op_geom_110_pg(pbuffer, 638, 10, 35, 104, 212, 362, 488, pfactors, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 638, ket_width, ket_npgtos);
                }
            }
...
#endif /* LocalCorePotentialGeom110PG_hpp */
