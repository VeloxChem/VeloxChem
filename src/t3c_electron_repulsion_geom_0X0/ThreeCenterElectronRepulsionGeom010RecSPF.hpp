#ifndef ThreeCenterElectronRepulsionGeom010RecSPF_hpp
#define ThreeCenterElectronRepulsionGeom010RecSPF_hpp
...
/// @brief Computes d^(1)/dC^(1)(S|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom010_spf(T& distributor,
                                    ...
                                    const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            t3cfunc::bra_transform<0>(skbuffer, 100, cbuffer, 10, 0, 3);

            t3cfunc::bra_transform<0>(skbuffer, 110, cbuffer, 20, 0, 4);

            t3cfunc::bra_transform<0>(skbuffer, 125, cbuffer, 35, 0, 5);

            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 146, 100, 110, cfactors, 6, 0);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 176, 110, 125, cfactors, 6, 0);

            t3ceri::comp_ket_geom010_electron_repulsion_xpf(skbuffer, 10, 0, 146, 176, cfactors, 6, 0);

            t3cfunc::ket_transform<1, 3>(sbuffer, 0, skbuffer, 10, 0);

            t3cfunc::ket_transform<1, 3>(sbuffer, 21, skbuffer, 40, 0);

            t3cfunc::ket_transform<1, 3>(sbuffer, 42, skbuffer, 70, 0);

            distributor.distribute(sbuffer, 0, bra_gto_indices, c_indices, d_indices, 0, 1, 3, j, ket_range);
        }
    }
...
#endif /* ThreeCenterElectronRepulsionGeom010RecSPF_hpp */
