#ifndef ThreeCenterElectronRepulsionGeom010RecFDG_hpp
#define ThreeCenterElectronRepulsionGeom010RecFDG_hpp
...
/// @brief Computes d^(1)/dC^(1)(F|1/|r-r'||DG)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom010_fdg(T& distributor,
                                    ...
                                    const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    CSimdArray<double> cbuffer(1360, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(6769, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(pfactors, 0, 2.0, {2178, 2328});

                pbuffer.scale(pfactors, 0, 2.0, {2328, 2538});

                pbuffer.scale(pfactors, 0, 2.0, {2538, 2818});

                pbuffer.scale(pfactors, 0, 2.0, {2818, 3178});

                t2cfunc::reduce(cbuffer, 360, pbuffer, 2178, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 510, pbuffer, 2328, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 720, pbuffer, 2538, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1000, pbuffer, 2818, 360, ket_width, ket_npgtos);
            ...
            t3cfunc::bra_transform<3>(skbuffer, 4725, cbuffer, 360, 0, 4);

            t3cfunc::bra_transform<3>(skbuffer, 4830, cbuffer, 510, 0, 5);

            t3cfunc::bra_transform<3>(skbuffer, 4977, cbuffer, 720, 0, 6);

            t3cfunc::bra_transform<3>(skbuffer, 5173, cbuffer, 1000, 0, 7);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 252, 0, 105, cfactors, 6, 3);

            t3ceri::comp_ket_geom010_electron_repulsion_xsg(skbuffer, 5425, 4725, 4830, cfactors, 6, 3);

            t3ceri::comp_ket_geom010_electron_repulsion_xsh(skbuffer, 5740, 4830, 4977, cfactors, 6, 3);

            t3ceri::comp_ket_geom010_electron_repulsion_xsi(skbuffer, 6181, 4977, 5173, cfactors, 6, 3);

            t3ceri::comp_ket_geom010_electron_repulsion_xpg(skbuffer, 567, 0, 5425, 5740, cfactors, 6, 3);

            t3ceri::comp_ket_geom010_electron_repulsion_xph(skbuffer, 1512, 105, 5740, 6181, cfactors, 6, 3);

            t3ceri::comp_ket_geom010_electron_repulsion_xdg(skbuffer, 2835, 252, 567, 1512, cfactors, 6, 3);

            t3cfunc::ket_transform<2, 4>(sbuffer, 0, skbuffer, 2835, 3);

            t3cfunc::ket_transform<2, 4>(sbuffer, 315, skbuffer, 3465, 3);

            t3cfunc::ket_transform<2, 4>(sbuffer, 630, skbuffer, 4095, 3);
...
#endif /* ThreeCenterElectronRepulsionGeom010RecFDG_hpp */
