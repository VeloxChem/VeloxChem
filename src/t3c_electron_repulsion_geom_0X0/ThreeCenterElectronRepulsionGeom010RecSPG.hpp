#ifndef ThreeCenterElectronRepulsionGeom010RecSPG_hpp
#define ThreeCenterElectronRepulsionGeom010RecSPG_hpp
...
/// @brief Computes d^(1)/dC^(1)(S|1/|r-r'||PG)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom010_spg(T& distributor,
                                    ...
                                    const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    CSimdArray<double> cbuffer(79, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(322, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 15, pbuffer, 95, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 30, pbuffer, 140, 21, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 51, pbuffer, 182, 28, ket_width, ket_npgtos);

            }

            t3cfunc::bra_transform<0>(skbuffer, 0, cbuffer, 0, 0, 4);

            t3cfunc::bra_transform<0>(skbuffer, 150, cbuffer, 15, 0, 4);

            t3cfunc::bra_transform<0>(skbuffer, 165, cbuffer, 30, 0, 5);

            t3cfunc::bra_transform<0>(skbuffer, 186, cbuffer, 51, 0, 6);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 214, 150, 165, cfactors, 6, 0);

            t3ceri::comp_hrr_electron_repulsion_xph(skbuffer, 259, 165, 186, cfactors, 6, 0);

            t3ceri::comp_ket_geom010_electron_repulsion_xpg(skbuffer, 15, 0, 214, 259, cfactors, 6, 0);

            t3cfunc::ket_transform<1, 4>(sbuffer, 0, skbuffer, 15, 0);

            t3cfunc::ket_transform<1, 4>(sbuffer, 27, skbuffer, 60, 0);

            t3cfunc::ket_transform<1, 4>(sbuffer, 54, skbuffer, 105, 0);

            distributor.distribute(sbuffer, 0, bra_gto_indices, c_indices, d_indices, 0, 1, 4, j, ket_range);
        }
    }
...
#endif /* ThreeCenterElectronRepulsionGeom010RecSPG_hpp */
