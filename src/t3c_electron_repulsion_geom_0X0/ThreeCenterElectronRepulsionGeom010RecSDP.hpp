#ifndef ThreeCenterElectronRepulsionGeom010RecSDP_hpp
#define ThreeCenterElectronRepulsionGeom010RecSDP_hpp
...
/// @brief Computes d^(1)/dC^(1)(S|1/|r-r'||DP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom010_sdp(T& distributor,
                                    ...
                                    const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            t3cfunc::bra_transform<0>(skbuffer, 153, cbuffer, 9, 0, 1);

            t3cfunc::bra_transform<0>(skbuffer, 156, cbuffer, 12, 0, 2);

            t3cfunc::bra_transform<0>(skbuffer, 162, cbuffer, 18, 0, 3);

            t3cfunc::bra_transform<0>(skbuffer, 172, cbuffer, 28, 0, 4);

            t3ceri::comp_hrr_electron_repulsion_xpp(skbuffer, 9, 0, 3, cfactors, 6, 0);

            t3ceri::comp_hrr_electron_repulsion_xpp(skbuffer, 187, 153, 156, cfactors, 6, 0);

            t3ceri::comp_hrr_electron_repulsion_xpd(skbuffer, 196, 156, 162, cfactors, 6, 0);

            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 214, 162, 172, cfactors, 6, 0);

            t3ceri::comp_ket_geom010_electron_repulsion_xpp(skbuffer, 18, 0, 187, 196, cfactors, 6, 0);

            t3ceri::comp_ket_geom010_electron_repulsion_xpd(skbuffer, 45, 3, 196, 214, cfactors, 6, 0);

            t3ceri::comp_ket_geom010_electron_repulsion_xdp(skbuffer, 99, 9, 18, 45, cfactors, 6, 0);

            t3cfunc::ket_transform<2, 1>(sbuffer, 0, skbuffer, 99, 0);

            t3cfunc::ket_transform<2, 1>(sbuffer, 15, skbuffer, 117, 0);

            t3cfunc::ket_transform<2, 1>(sbuffer, 30, skbuffer, 135, 0);

            distributor.distribute(sbuffer, 0, bra_gto_indices, c_indices, d_indices, 0, 2, 1, j, ket_range);
        }
    }
...
#endif /* ThreeCenterElectronRepulsionGeom010RecSDP_hpp */
