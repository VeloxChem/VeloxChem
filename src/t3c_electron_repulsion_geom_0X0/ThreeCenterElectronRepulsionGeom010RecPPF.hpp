#ifndef ThreeCenterElectronRepulsionGeom010RecPPF_hpp
#define ThreeCenterElectronRepulsionGeom010RecPPF_hpp
...
/// @brief Computes d^(1)/dC^(1)(P|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom010_ppf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(pfactors, 0, 2.0, {126, 156});

                pbuffer.scale(pfactors, 0, 2.0, {156, 201});

                pbuffer.scale(pfactors, 0, 2.0, {201, 264});

                t2cfunc::reduce(cbuffer, 30, pbuffer, 126, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 60, pbuffer, 156, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 105, pbuffer, 201, 63, ket_width, ket_npgtos);

            }

            t3cfunc::bra_transform<1>(skbuffer, 0, cbuffer, 0, 0, 3);

            t3cfunc::bra_transform<1>(skbuffer, 300, cbuffer, 30, 0, 3);

            t3cfunc::bra_transform<1>(skbuffer, 330, cbuffer, 60, 0, 4);

            t3cfunc::bra_transform<1>(skbuffer, 375, cbuffer, 105, 0, 5);

            t3ceri::comp_ket_geom010_electron_repulsion_xsf(skbuffer, 438, 300, 330, cfactors, 6, 1);

            t3ceri::comp_ket_geom010_electron_repulsion_xsg(skbuffer, 528, 330, 375, cfactors, 6, 1);

            t3ceri::comp_ket_geom010_electron_repulsion_xpf(skbuffer, 30, 0, 438, 528, cfactors, 6, 1);

            t3cfunc::ket_transform<1, 3>(sbuffer, 0, skbuffer, 30, 1);

            t3cfunc::ket_transform<1, 3>(sbuffer, 63, skbuffer, 120, 1);

            t3cfunc::ket_transform<1, 3>(sbuffer, 126, skbuffer, 210, 1);
...
#endif /* ThreeCenterElectronRepulsionGeom010RecPPF_hpp */
