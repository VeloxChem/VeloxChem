#ifndef ThreeCenterElectronRepulsionGeom010RecFDF_hpp
#define ThreeCenterElectronRepulsionGeom010RecFDF_hpp
...
/// @brief Computes d^(1)/dC^(1)(F|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom010_fdf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(pfactors, 0, 2.0, {1651, 1801});

                pbuffer.scale(pfactors, 0, 2.0, {1801, 2011});

                pbuffer.scale(pfactors, 0, 2.0, {2011, 2291});

                t2cfunc::reduce(cbuffer, 250, pbuffer, 1551, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 350, pbuffer, 1651, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 500, pbuffer, 1801, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 710, pbuffer, 2011, 280, ket_width, ket_npgtos);
            ...
            t3cfunc::bra_transform<3>(skbuffer, 3220, cbuffer, 250, 0, 3);

            t3cfunc::bra_transform<3>(skbuffer, 3290, cbuffer, 350, 0, 4);

            t3cfunc::bra_transform<3>(skbuffer, 3395, cbuffer, 500, 0, 5);

            t3cfunc::bra_transform<3>(skbuffer, 3542, cbuffer, 710, 0, 6);

            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 175, 0, 70, cfactors, 6, 3);

            t3ceri::comp_ket_geom010_electron_repulsion_xsf(skbuffer, 3738, 3220, 3290, cfactors, 6, 3);

            t3ceri::comp_ket_geom010_electron_repulsion_xsg(skbuffer, 3948, 3290, 3395, cfactors, 6, 3);

            t3ceri::comp_ket_geom010_electron_repulsion_xsh(skbuffer, 4263, 3395, 3542, cfactors, 6, 3);

            t3ceri::comp_ket_geom010_electron_repulsion_xpf(skbuffer, 385, 0, 3738, 3948, cfactors, 6, 3);

            t3ceri::comp_ket_geom010_electron_repulsion_xpg(skbuffer, 1015, 70, 3948, 4263, cfactors, 6, 3);

            t3ceri::comp_ket_geom010_electron_repulsion_xdf(skbuffer, 1960, 175, 385, 1015, cfactors, 6, 3);

            t3cfunc::ket_transform<2, 3>(sbuffer, 0, skbuffer, 1960, 3);

            t3cfunc::ket_transform<2, 3>(sbuffer, 245, skbuffer, 2380, 3);

            t3cfunc::ket_transform<2, 3>(sbuffer, 490, skbuffer, 2800, 3);
...
#endif /* ThreeCenterElectronRepulsionGeom010RecFDF_hpp */
