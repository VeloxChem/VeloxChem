#ifndef ThreeCenterElectronRepulsionGeom010RecSFF_hpp
#define ThreeCenterElectronRepulsionGeom010RecSFF_hpp
...
/// @brief Computes d^(1)/dC^(1)(S|1/|r-r'||FF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom010_sff(T& distributor,
                                    ...
                                    const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            t3cfunc::bra_transform<0>(skbuffer, 1345, cbuffer, 46, 0, 3);

            t3cfunc::bra_transform<0>(skbuffer, 1355, cbuffer, 56, 0, 4);

            t3cfunc::bra_transform<0>(skbuffer, 1370, cbuffer, 71, 0, 5);

            t3cfunc::bra_transform<0>(skbuffer, 1391, cbuffer, 92, 0, 6);

            t3cfunc::bra_transform<0>(skbuffer, 1419, cbuffer, 120, 0, 7);

            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 46, 0, 10, cfactors, 6, 0);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 166, 10, 25, cfactors, 6, 0);

            t3ceri::comp_hrr_electron_repulsion_xdf(skbuffer, 535, 46, 166, cfactors, 6, 0);

            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 1455, 1345, 1355, cfactors, 6, 0);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 1485, 1355, 1370, cfactors, 6, 0);

            t3ceri::comp_hrr_electron_repulsion_xph(skbuffer, 1530, 1370, 1391, cfactors, 6, 0);

            t3ceri::comp_hrr_electron_repulsion_xpi(skbuffer, 1593, 1391, 1419, cfactors, 6, 0);

            t3ceri::comp_ket_geom010_electron_repulsion_xpf(skbuffer, 76, 0, 1455, 1485, cfactors, 6, 0);

            t3ceri::comp_ket_geom010_electron_repulsion_xpg(skbuffer, 211, 10, 1485, 1530, cfactors, 6, 0);

            t3ceri::comp_ket_geom010_electron_repulsion_xph(skbuffer, 346, 25, 1530, 1593, cfactors, 6, 0);

            t3ceri::comp_ket_geom010_electron_repulsion_xdf(skbuffer, 595, 46, 76, 211, cfactors, 6, 0);

            t3ceri::comp_ket_geom010_electron_repulsion_xdg(skbuffer, 775, 166, 211, 346, cfactors, 6, 0);

            t3ceri::comp_ket_geom010_electron_repulsion_xff(skbuffer, 1045, 535, 595, 775, cfactors, 6, 0);

            t3cfunc::ket_transform<3, 3>(sbuffer, 0, skbuffer, 1045, 0);

            t3cfunc::ket_transform<3, 3>(sbuffer, 49, skbuffer, 1145, 0);

            t3cfunc::ket_transform<3, 3>(sbuffer, 98, skbuffer, 1245, 0);

            distributor.distribute(sbuffer, 0, bra_gto_indices, c_indices, d_indices, 0, 3, 3, j, ket_range);
        }
    }
...
#endif /* ThreeCenterElectronRepulsionGeom010RecSFF_hpp */
