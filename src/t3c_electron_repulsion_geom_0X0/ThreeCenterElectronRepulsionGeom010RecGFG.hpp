#ifndef ThreeCenterElectronRepulsionGeom010RecGFG_hpp
#define ThreeCenterElectronRepulsionGeom010RecGFG_hpp
...
/// @brief Computes d^(1)/dC^(1)(G|1/|r-r'||FG)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom010_gfg(T& distributor,
                                    ...
                                    const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    CSimdArray<double> cbuffer(3135, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(21429, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(pfactors, 0, 2.0, {6833, 7058});

                pbuffer.scale(pfactors, 0, 2.0, {7058, 7373});

                pbuffer.scale(pfactors, 0, 2.0, {7373, 7793});

                pbuffer.scale(pfactors, 0, 2.0, {7793, 8333});

                pbuffer.scale(pfactors, 0, 2.0, {8333, 9008});

                t2cfunc::reduce(cbuffer, 960, pbuffer, 6833, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1185, pbuffer, 7058, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1500, pbuffer, 7373, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1920, pbuffer, 7793, 540, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2460, pbuffer, 8333, 675, ket_width, ket_npgtos);
            ...
            t3cfunc::bra_transform<4>(skbuffer, 17424, cbuffer, 960, 0, 4);

            t3cfunc::bra_transform<4>(skbuffer, 17559, cbuffer, 1185, 0, 5);

            t3cfunc::bra_transform<4>(skbuffer, 17748, cbuffer, 1500, 0, 6);

            t3cfunc::bra_transform<4>(skbuffer, 18000, cbuffer, 1920, 0, 7);

            t3cfunc::bra_transform<4>(skbuffer, 18324, cbuffer, 2460, 0, 8);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 576, 0, 135, cfactors, 6, 4);

            t3ceri::comp_hrr_electron_repulsion_xph(skbuffer, 2196, 135, 324, cfactors, 6, 4);

            t3ceri::comp_hrr_electron_repulsion_xdg(skbuffer, 6732, 576, 2196, cfactors, 6, 4);

            t3ceri::comp_ket_geom010_electron_repulsion_xsg(skbuffer, 18729, 17424, 17559, cfactors, 6, 4);

            t3ceri::comp_ket_geom010_electron_repulsion_xsh(skbuffer, 19134, 17559, 17748, cfactors, 6, 4);

            t3ceri::comp_ket_geom010_electron_repulsion_xsi(skbuffer, 19701, 17748, 18000, cfactors, 6, 4);

            t3ceri::comp_ket_geom010_electron_repulsion_xsk(skbuffer, 20457, 18000, 18324, cfactors, 6, 4);

            t3ceri::comp_ket_geom010_electron_repulsion_xpg(skbuffer, 981, 0, 18729, 19134, cfactors, 6, 4);

            t3ceri::comp_ket_geom010_electron_repulsion_xph(skbuffer, 2763, 135, 19134, 19701, cfactors, 6, 4);

            t3ceri::comp_ket_geom010_electron_repulsion_xpi(skbuffer, 4464, 324, 19701, 20457, cfactors, 6, 4);

            t3ceri::comp_ket_geom010_electron_repulsion_xdg(skbuffer, 7542, 576, 981, 2763, cfactors, 6, 4);

            t3ceri::comp_ket_geom010_electron_repulsion_xdh(skbuffer, 9972, 2196, 2763, 4464, cfactors, 6, 4);

            t3ceri::comp_ket_geom010_electron_repulsion_xfg(skbuffer, 13374, 6732, 7542, 9972, cfactors, 6, 4);

            t3cfunc::ket_transform<3, 4>(sbuffer, 0, skbuffer, 13374, 4);

            t3cfunc::ket_transform<3, 4>(sbuffer, 567, skbuffer, 14724, 4);

            t3cfunc::ket_transform<3, 4>(sbuffer, 1134, skbuffer, 16074, 4);
...
#endif /* ThreeCenterElectronRepulsionGeom010RecGFG_hpp */
