#ifndef ThreeCenterElectronRepulsionGeom010RecFPG_hpp
#define ThreeCenterElectronRepulsionGeom010RecFPG_hpp
...
/// @brief Computes d^(1)/dC^(1)(F|1/|r-r'||PG)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom010_fpg(T& distributor,
                                    ...
                                    const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    CSimdArray<double> cbuffer(790, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(2254, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 1446, 150, ket_width, ket_npgtos);

                pbuffer.scale(pfactors, 0, 2.0, {1446, 1596});

                pbuffer.scale(pfactors, 0, 2.0, {1596, 1806});

                pbuffer.scale(pfactors, 0, 2.0, {1806, 2086});

                t2cfunc::reduce(cbuffer, 150, pbuffer, 1446, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 300, pbuffer, 1596, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 510, pbuffer, 1806, 280, ket_width, ket_npgtos);

            }

            t3cfunc::bra_transform<3>(skbuffer, 0, cbuffer, 0, 0, 4);

            t3cfunc::bra_transform<3>(skbuffer, 1050, cbuffer, 150, 0, 4);

            t3cfunc::bra_transform<3>(skbuffer, 1155, cbuffer, 300, 0, 5);

            t3cfunc::bra_transform<3>(skbuffer, 1302, cbuffer, 510, 0, 6);

            t3ceri::comp_ket_geom010_electron_repulsion_xsg(skbuffer, 1498, 1050, 1155, cfactors, 6, 3);

            t3ceri::comp_ket_geom010_electron_repulsion_xsh(skbuffer, 1813, 1155, 1302, cfactors, 6, 3);

            t3ceri::comp_ket_geom010_electron_repulsion_xpg(skbuffer, 105, 0, 1498, 1813, cfactors, 6, 3);

            t3cfunc::ket_transform<1, 4>(sbuffer, 0, skbuffer, 105, 3);

            t3cfunc::ket_transform<1, 4>(sbuffer, 189, skbuffer, 420, 3);

            t3cfunc::ket_transform<1, 4>(sbuffer, 378, skbuffer, 735, 3);
...
#endif /* ThreeCenterElectronRepulsionGeom010RecFPG_hpp */
