#ifndef ThreeCenterElectronRepulsionGeom010RecGSG_hpp
#define ThreeCenterElectronRepulsionGeom010RecGSG_hpp
...
/// @brief Computes d^(1)/dC^(1)(G|1/|r-r'||SG)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom010_gsg(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(pfactors, 0, 2.0, {1995, 2220});

                pbuffer.scale(pfactors, 0, 2.0, {2220, 2535});

                t2cfunc::reduce(cbuffer, 0, pbuffer, 1995, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 225, pbuffer, 2220, 315, ket_width, ket_npgtos);
            ...
            t3ceri::comp_ket_geom010_electron_repulsion_xsg(skbuffer, 324, 0, 135, cfactors, 6, 4);

            t3cfunc::ket_transform<0, 4>(sbuffer, 0, skbuffer, 324, 4);

            t3cfunc::ket_transform<0, 4>(sbuffer, 81, skbuffer, 459, 4);

            t3cfunc::ket_transform<0, 4>(sbuffer, 162, skbuffer, 594, 4);

            distributor.distribute(sbuffer, 0, bra_gto_indices, c_indices, d_indices, 4, 0, 4, j, ket_range);
        }
    }
...
#endif /* ThreeCenterElectronRepulsionGeom010RecGSG_hpp */
