#ifndef ThreeCenterElectronRepulsionGeom010RecPSF_hpp
#define ThreeCenterElectronRepulsionGeom010RecPSF_hpp
...
/// @brief Computes d^(1)/dC^(1)(P|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom010_psf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(pfactors, 0, 2.0, {70, 100});

                pbuffer.scale(pfactors, 0, 2.0, {100, 145});
            ...
            t3ceri::comp_ket_geom010_electron_repulsion_xsf(skbuffer, 75, 0, 30, cfactors, 6, 1);

            t3cfunc::ket_transform<0, 3>(sbuffer, 0, skbuffer, 75, 1);

            t3cfunc::ket_transform<0, 3>(sbuffer, 21, skbuffer, 105, 1);

            t3cfunc::ket_transform<0, 3>(sbuffer, 42, skbuffer, 135, 1);
...
#endif /* ThreeCenterElectronRepulsionGeom010RecPSF_hpp */
