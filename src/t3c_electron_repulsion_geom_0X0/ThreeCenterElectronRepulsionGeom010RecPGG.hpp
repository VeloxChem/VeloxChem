#ifndef ThreeCenterElectronRepulsionGeom010RecPGG_hpp
#define ThreeCenterElectronRepulsionGeom010RecPGG_hpp
...
/// @brief Computes d^(1)/dC^(1)(P|1/|r-r'||GG)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom010_pgg(T& distributor,
                                    ...
                                    const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    CSimdArray<double> pbuffer(1315, ket_npgtos);
    ...
    CSimdArray<double> skbuffer(15300, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t3ceri::comp_prim_electron_repulsion_psg(pbuffer, 715, 85, 155, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_psh(pbuffer, 760, 155, 245, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_psi(pbuffer, 823, 245, 350, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_psk(pbuffer, 907, 350, 462, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_psl(pbuffer, 1015, 462, 570, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_psm(pbuffer, 1150, 570, 660, pfactors, 26, a_exp);

                t2cfunc::reduce(cbuffer, 0, pbuffer, 715, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 45, pbuffer, 760, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 108, pbuffer, 823, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 192, pbuffer, 907, 108, ket_width, ket_npgtos);

                pbuffer.scale(pfactors, 0, 2.0, {715, 760});

                pbuffer.scale(pfactors, 0, 2.0, {760, 823});

                pbuffer.scale(pfactors, 0, 2.0, {823, 907});

                pbuffer.scale(pfactors, 0, 2.0, {907, 1015});

                pbuffer.scale(pfactors, 0, 2.0, {1015, 1150});

                pbuffer.scale(pfactors, 0, 2.0, {1150, 1315});

                t2cfunc::reduce(cbuffer, 300, pbuffer, 715, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 345, pbuffer, 760, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 408, pbuffer, 823, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 492, pbuffer, 907, 108, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 600, pbuffer, 1015, 135, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 735, pbuffer, 1150, 165, ket_width, ket_npgtos);
            ...
            t3cfunc::bra_transform<1>(skbuffer, 192, cbuffer, 192, 0, 7);

            t3cfunc::bra_transform<1>(skbuffer, 13395, cbuffer, 300, 0, 4);

            t3cfunc::bra_transform<1>(skbuffer, 13440, cbuffer, 345, 0, 5);

            t3cfunc::bra_transform<1>(skbuffer, 13503, cbuffer, 408, 0, 6);

            t3cfunc::bra_transform<1>(skbuffer, 13587, cbuffer, 492, 0, 7);

            t3cfunc::bra_transform<1>(skbuffer, 13695, cbuffer, 600, 0, 8);

            t3cfunc::bra_transform<1>(skbuffer, 13830, cbuffer, 735, 0, 9);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 300, 0, 45, cfactors, 6, 1);

            t3ceri::comp_hrr_electron_repulsion_xph(skbuffer, 840, 45, 108, cfactors, 6, 1);

            t3ceri::comp_hrr_electron_repulsion_xpi(skbuffer, 1596, 108, 192, cfactors, 6, 1);

            t3ceri::comp_hrr_electron_repulsion_xdg(skbuffer, 3576, 300, 840, cfactors, 6, 1);

            t3ceri::comp_hrr_electron_repulsion_xdh(skbuffer, 4656, 840, 1596, cfactors, 6, 1);

            t3ceri::comp_hrr_electron_repulsion_xfg(skbuffer, 7680, 3576, 4656, cfactors, 6, 1);

            t3ceri::comp_ket_geom010_electron_repulsion_xsg(skbuffer, 13995, 13395, 13440, cfactors, 6, 1);

            t3ceri::comp_ket_geom010_electron_repulsion_xsh(skbuffer, 14130, 13440, 13503, cfactors, 6, 1);

            t3ceri::comp_ket_geom010_electron_repulsion_xsi(skbuffer, 14319, 13503, 13587, cfactors, 6, 1);

            t3ceri::comp_ket_geom010_electron_repulsion_xsk(skbuffer, 14571, 13587, 13695, cfactors, 6, 1);

            t3ceri::comp_ket_geom010_electron_repulsion_xsl(skbuffer, 14895, 13695, 13830, cfactors, 6, 1);

            t3ceri::comp_ket_geom010_electron_repulsion_xpg(skbuffer, 435, 0, 13995, 14130, cfactors, 6, 1);

            t3ceri::comp_ket_geom010_electron_repulsion_xph(skbuffer, 1029, 45, 14130, 14319, cfactors, 6, 1);

            t3ceri::comp_ket_geom010_electron_repulsion_xpi(skbuffer, 1848, 108, 14319, 14571, cfactors, 6, 1);

            t3ceri::comp_ket_geom010_electron_repulsion_xpk(skbuffer, 2604, 192, 14571, 14895, cfactors, 6, 1);

            t3ceri::comp_ket_geom010_electron_repulsion_xdg(skbuffer, 3846, 300, 435, 1029, cfactors, 6, 1);

            t3ceri::comp_ket_geom010_electron_repulsion_xdh(skbuffer, 5034, 840, 1029, 1848, cfactors, 6, 1);

            t3ceri::comp_ket_geom010_electron_repulsion_xdi(skbuffer, 6168, 1596, 1848, 2604, cfactors, 6, 1);

            t3ceri::comp_ket_geom010_electron_repulsion_xfg(skbuffer, 8130, 3576, 3846, 5034, cfactors, 6, 1);

            t3ceri::comp_ket_geom010_electron_repulsion_xfh(skbuffer, 9480, 4656, 5034, 6168, cfactors, 6, 1);

            t3ceri::comp_ket_geom010_electron_repulsion_xgg(skbuffer, 11370, 7680, 8130, 9480, cfactors, 6, 1);

            t3cfunc::ket_transform<4, 4>(sbuffer, 0, skbuffer, 11370, 1);

            t3cfunc::ket_transform<4, 4>(sbuffer, 243, skbuffer, 12045, 1);

            t3cfunc::ket_transform<4, 4>(sbuffer, 486, skbuffer, 12720, 1);
...
#endif /* ThreeCenterElectronRepulsionGeom010RecPGG_hpp */
