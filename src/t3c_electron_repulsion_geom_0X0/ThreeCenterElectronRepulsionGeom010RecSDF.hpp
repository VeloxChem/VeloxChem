#ifndef ThreeCenterElectronRepulsionGeom010RecSDF_hpp
#define ThreeCenterElectronRepulsionGeom010RecSDF_hpp
...
/// @brief Computes d^(1)/dC^(1)(S|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom010_sdf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 25, pbuffer, 55, 10, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 35, pbuffer, 95, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 50, pbuffer, 140, 21, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 71, pbuffer, 182, 28, ket_width, ket_npgtos);
            ...
            t3cfunc::bra_transform<0>(skbuffer, 460, cbuffer, 25, 0, 3);

            t3cfunc::bra_transform<0>(skbuffer, 470, cbuffer, 35, 0, 4);

            t3cfunc::bra_transform<0>(skbuffer, 485, cbuffer, 50, 0, 5);

            t3cfunc::bra_transform<0>(skbuffer, 506, cbuffer, 71, 0, 6);

            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 25, 0, 10, cfactors, 6, 0);

            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 534, 460, 470, cfactors, 6, 0);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 564, 470, 485, cfactors, 6, 0);

            t3ceri::comp_hrr_electron_repulsion_xph(skbuffer, 609, 485, 506, cfactors, 6, 0);

            t3ceri::comp_ket_geom010_electron_repulsion_xpf(skbuffer, 55, 0, 534, 564, cfactors, 6, 0);

            t3ceri::comp_ket_geom010_electron_repulsion_xpg(skbuffer, 145, 10, 564, 609, cfactors, 6, 0);

            t3ceri::comp_ket_geom010_electron_repulsion_xdf(skbuffer, 280, 25, 55, 145, cfactors, 6, 0);

            t3cfunc::ket_transform<2, 3>(sbuffer, 0, skbuffer, 280, 0);

            t3cfunc::ket_transform<2, 3>(sbuffer, 35, skbuffer, 340, 0);

            t3cfunc::ket_transform<2, 3>(sbuffer, 70, skbuffer, 400, 0);

            distributor.distribute(sbuffer, 0, bra_gto_indices, c_indices, d_indices, 0, 2, 3, j, ket_range);
        }
    }
...
#endif /* ThreeCenterElectronRepulsionGeom010RecSDF_hpp */
