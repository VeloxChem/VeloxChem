#ifndef ThreeCenterElectronRepulsionGeom010RecFPD_hpp
#define ThreeCenterElectronRepulsionGeom010RecFPD_hpp
...
/// @brief Computes d^(1)/dC^(1)(F|1/|r-r'||PD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom010_fpd(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(pfactors, 0, 2.0, {635, 695});

                pbuffer.scale(pfactors, 0, 2.0, {695, 795});

                pbuffer.scale(pfactors, 0, 2.0, {795, 945});

                t2cfunc::reduce(cbuffer, 60, pbuffer, 635, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 120, pbuffer, 695, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 220, pbuffer, 795, 150, ket_width, ket_npgtos);

            }

            t3cfunc::bra_transform<3>(skbuffer, 0, cbuffer, 0, 0, 2);

            t3cfunc::bra_transform<3>(skbuffer, 420, cbuffer, 60, 0, 2);

            t3cfunc::bra_transform<3>(skbuffer, 462, cbuffer, 120, 0, 3);

            t3cfunc::bra_transform<3>(skbuffer, 532, cbuffer, 220, 0, 4);

            t3ceri::comp_ket_geom010_electron_repulsion_xsd(skbuffer, 637, 420, 462, cfactors, 6, 3);

            t3ceri::comp_ket_geom010_electron_repulsion_xsf(skbuffer, 763, 462, 532, cfactors, 6, 3);

            t3ceri::comp_ket_geom010_electron_repulsion_xpd(skbuffer, 42, 0, 637, 763, cfactors, 6, 3);

            t3cfunc::ket_transform<1, 2>(sbuffer, 0, skbuffer, 42, 3);

            t3cfunc::ket_transform<1, 2>(sbuffer, 105, skbuffer, 168, 3);

            t3cfunc::ket_transform<1, 2>(sbuffer, 210, skbuffer, 294, 3);
...
#endif /* ThreeCenterElectronRepulsionGeom010RecFPD_hpp */
