#ifndef ThreeCenterElectronRepulsionGeom010RecFSG_hpp
#define ThreeCenterElectronRepulsionGeom010RecFSG_hpp
...
/// @brief Computes d^(1)/dC^(1)(F|1/|r-r'||SG)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom010_fsg(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(pfactors, 0, 2.0, {886, 1036});

                pbuffer.scale(pfactors, 0, 2.0, {1036, 1246});
            ...
            t3ceri::comp_ket_geom010_electron_repulsion_xsg(skbuffer, 252, 0, 105, cfactors, 6, 3);

            t3cfunc::ket_transform<0, 4>(sbuffer, 0, skbuffer, 252, 3);

            t3cfunc::ket_transform<0, 4>(sbuffer, 63, skbuffer, 357, 3);

            t3cfunc::ket_transform<0, 4>(sbuffer, 126, skbuffer, 462, 3);
...
#endif /* ThreeCenterElectronRepulsionGeom010RecFSG_hpp */
