...
auto
compPrimitiveDipoleFS_YYY_0(TDoubleArray&       buffer_x,
...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += faa_x * ((3.0 / 2.0) * fe_0 * rpa_y + rpa_y * rpa_y * rpa_y);

        fints_y[i] += fss * ((3.0 / 2.0) * fe_0 * rpa_y * rpa_y + (3.0 / 4.0) * fe_0 * fe_0);

        fints_y[i] += faa_y * ((3.0 / 2.0) * fe_0 * rpa_y + rpa_y * rpa_y * rpa_y);

        fints_z[i] += faa_z * ((3.0 / 2.0) * fe_0 * rpa_y + rpa_y * rpa_y * rpa_y);
    }
}
