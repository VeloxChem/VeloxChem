...
auto
compPrimitiveDipolePF_Y_XZZ(TDoubleArray&       buffer_x,
...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += fss * ((1.0 / 2.0) * fe_0 * rpa_y * rpb_z * rpb_z + (1.0 / 4.0) * fe_0 * fe_0 * rpa_y);

        fints_x[i] += faa_x * ((1.0 / 2.0) * fe_0 * rpa_y * rpb_x + rpa_y * rpb_z * rpb_z * rpb_x);

        fints_y[i] += fss * ((1.0 / 2.0) * fe_0 * rpb_z * rpb_z * rpb_x + (1.0 / 4.0) * fe_0 * fe_0 * rpb_x);

        fints_y[i] += faa_y * ((1.0 / 2.0) * fe_0 * rpa_y * rpb_x + rpa_y * rpb_z * rpb_z * rpb_x);

        fints_z[i] += fss * fe_0 * rpa_y * rpb_z * rpb_x;

        fints_z[i] += faa_z * ((1.0 / 2.0) * fe_0 * rpa_y * rpb_x + rpa_y * rpb_z * rpb_z * rpb_x);
    }
}
