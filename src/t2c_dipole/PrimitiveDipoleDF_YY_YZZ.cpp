...
auto
compPrimitiveDipoleDF_YY_YZZ(TDoubleArray&       buffer_x,
...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += faa_x * (fe_0 * rpa_y * rpb_z * rpb_z + (1.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpb_y +
                               (1.0 / 2.0) * fe_0 * rpb_z * rpb_z * rpb_y + (1.0 / 2.0) * fe_0 * fe_0 * rpa_y + (1.0 / 4.0) * fe_0 * fe_0 * rpb_y);

        fints_x[i] += faa_x * rpa_y * rpa_y * rpb_z * rpb_z * rpb_y;

        fints_y[i] +=
            fss * (fe_0 * rpa_y * rpb_z * rpb_z * rpb_y + (1.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpb_z * rpb_z +
                   (1.0 / 2.0) * fe_0 * fe_0 * rpa_y * rpb_y + (1.0 / 4.0) * fe_0 * fe_0 * rpa_y * rpa_y + (3.0 / 4.0) * fe_0 * fe_0 * rpb_z * rpb_z);

        fints_y[i] += fss * (3.0 / 8.0) * fe_0 * fe_0 * fe_0;

        fints_y[i] += faa_y * (fe_0 * rpa_y * rpb_z * rpb_z + (1.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpb_y +
                               (1.0 / 2.0) * fe_0 * rpb_z * rpb_z * rpb_y + (1.0 / 2.0) * fe_0 * fe_0 * rpa_y + (1.0 / 4.0) * fe_0 * fe_0 * rpb_y);

        fints_y[i] += faa_y * rpa_y * rpa_y * rpb_z * rpb_z * rpb_y;

        fints_z[i] += fss * (fe_0 * rpa_y * rpa_y * rpb_z * rpb_y + fe_0 * fe_0 * rpa_y * rpb_z + (1.0 / 2.0) * fe_0 * fe_0 * rpb_z * rpb_y);

        fints_z[i] += faa_z * (fe_0 * rpa_y * rpb_z * rpb_z + (1.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpb_y +
                               (1.0 / 2.0) * fe_0 * rpb_z * rpb_z * rpb_y + (1.0 / 2.0) * fe_0 * fe_0 * rpa_y + (1.0 / 4.0) * fe_0 * fe_0 * rpb_y);

        fints_z[i] += faa_z * rpa_y * rpa_y * rpb_z * rpb_z * rpb_y;
    }
}
