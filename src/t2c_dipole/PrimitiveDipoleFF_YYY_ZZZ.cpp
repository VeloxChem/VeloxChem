...
auto
compPrimitiveDipoleFF_YYY_ZZZ(TDoubleArray&       buffer_x,
...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += faa_x * ((3.0 / 2.0) * fe_0 * rpa_y * rpb_z * rpb_z * rpb_z + (3.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpa_y * rpb_z +
                               (9.0 / 4.0) * fe_0 * fe_0 * rpa_y * rpb_z + rpa_y * rpa_y * rpa_y * rpb_z * rpb_z * rpb_z);

        fints_y[i] += fss * ((3.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpb_z * rpb_z * rpb_z + (9.0 / 4.0) * fe_0 * fe_0 * rpa_y * rpa_y * rpb_z +
                             (3.0 / 4.0) * fe_0 * fe_0 * rpb_z * rpb_z * rpb_z + (9.0 / 8.0) * fe_0 * fe_0 * fe_0 * rpb_z);

        fints_y[i] += faa_y * ((3.0 / 2.0) * fe_0 * rpa_y * rpb_z * rpb_z * rpb_z + (3.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpa_y * rpb_z +
                               (9.0 / 4.0) * fe_0 * fe_0 * rpa_y * rpb_z + rpa_y * rpa_y * rpa_y * rpb_z * rpb_z * rpb_z);

        fints_z[i] += fss * ((3.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpa_y * rpb_z * rpb_z + (9.0 / 4.0) * fe_0 * fe_0 * rpa_y * rpb_z * rpb_z +
                             (3.0 / 4.0) * fe_0 * fe_0 * rpa_y * rpa_y * rpa_y + (9.0 / 8.0) * fe_0 * fe_0 * fe_0 * rpa_y);

        fints_z[i] += faa_z * ((3.0 / 2.0) * fe_0 * rpa_y * rpb_z * rpb_z * rpb_z + (3.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpa_y * rpb_z +
                               (9.0 / 4.0) * fe_0 * fe_0 * rpa_y * rpb_z + rpa_y * rpa_y * rpa_y * rpb_z * rpb_z * rpb_z);
    }
}
