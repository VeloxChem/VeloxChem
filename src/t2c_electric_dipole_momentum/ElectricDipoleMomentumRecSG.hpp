#ifndef ElectricDipoleMomentumRecSG_hpp
#define ElectricDipoleMomentumRecSG_hpp
...
/// @brief Computes (S|r|G)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_electric_dipole_momentum_sg(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_sp(pbuffer, 4, 0, factors, 11);

                diprec::comp_prim_electric_dipole_momentum_sp(pbuffer, 7, 0, 1, factors, 11, a_exp);

                ovlrec::comp_prim_overlap_sd(pbuffer, 16, 0, 4, factors, 11, a_exp);

                diprec::comp_prim_electric_dipole_momentum_sd(pbuffer, 22, 1, 4, 7, factors, 11, a_exp);

                ovlrec::comp_prim_overlap_sf(pbuffer, 40, 4, 16, factors, 11, a_exp);

                diprec::comp_prim_electric_dipole_momentum_sf(pbuffer, 50, 7, 16, 22, factors, 11, a_exp);

                diprec::comp_prim_electric_dipole_momentum_sg(pbuffer, 80, 22, 40, 50, factors, 11, a_exp);
...
#endif /* ElectricDipoleMomentumRecSG_hpp */
