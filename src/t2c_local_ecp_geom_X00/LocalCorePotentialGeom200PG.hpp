#ifndef LocalCorePotentialGeom200PG_hpp
#define LocalCorePotentialGeom200PG_hpp
...
template <class T>
auto
comp_local_core_potential_geom_200_pg(T& distributor,
                                      ...
                                      const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(698, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < ecp_nppt; l++)
                {
                    ...
                    t2lecp::comp_prim_local_core_potential_pd(pbuffer, 35, 1, 4, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_pf(pbuffer, 53, 4, 10, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_pg(pbuffer, 83, 10, 20, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_df(pbuffer, 128, 10, 35, 53, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dg(pbuffer, 188, 20, 53, 83, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_fg(pbuffer, 278, 83, 128, 188, pfactors, 8, 14);

                    t2cgeom::comp_prim_op_geom_20_px(pbuffer, 428, 83, 278, 1, 15, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 428, ket_width, ket_npgtos);
                }
            }
...
#endif /* LocalCorePotentialGeom200PG_hpp */
