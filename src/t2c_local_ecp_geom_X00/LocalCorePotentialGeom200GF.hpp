#ifndef LocalCorePotentialGeom200GF_hpp
#define LocalCorePotentialGeom200GF_hpp
...
template <class T>
auto
comp_local_core_potential_geom_200_gf(T& distributor,
                                      ...
                                      const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(2201, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < ecp_nppt; l++)
                {
                    ...
                    t2lecp::comp_prim_local_core_potential_gp(pbuffer, 400, 86, 200, 210, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_gd(pbuffer, 445, 104, 210, 240, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_gf(pbuffer, 535, 140, 240, 300, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_hd(pbuffer, 685, 240, 400, 445, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_hf(pbuffer, 811, 300, 445, 535, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_if(pbuffer, 1021, 535, 685, 811, pfactors, 8, 14);

                    t2cgeom::comp_prim_op_geom_20_gx(pbuffer, 1301, 140, 535, 1021, 1, 10, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 1301, ket_width, ket_npgtos);
                }
            }
...
#endif /* LocalCorePotentialGeom200GF_hpp */
