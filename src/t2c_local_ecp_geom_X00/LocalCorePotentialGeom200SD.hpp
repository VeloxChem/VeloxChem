#ifndef LocalCorePotentialGeom200SD_hpp
#define LocalCorePotentialGeom200SD_hpp
...
template <class T>
auto
comp_local_core_potential_geom_200_sd(T& distributor,
                                      ...
                                      const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(109, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < ecp_nppt; l++)
                {
                    ...
                    t2lecp::comp_prim_local_core_potential_pp(pbuffer, 10, 0, 1, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_pd(pbuffer, 19, 1, 4, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dd(pbuffer, 37, 4, 10, 19, pfactors, 8, 14);

                    t2cgeom::comp_prim_op_geom_20_sx(pbuffer, 73, 4, 37, 1, 6, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 73, ket_width, ket_npgtos);
                }
            }
...
#endif /* LocalCorePotentialGeom200SD_hpp */
