#ifndef LocalCorePotentialGeom200FG_hpp
#define LocalCorePotentialGeom200FG_hpp
...
template <class T>
auto
comp_local_core_potential_geom_200_fg(T& distributor,
                                      ...
                                      const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(2244, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < ecp_nppt; l++)
                {
                    ...
                    t2lecp::comp_prim_local_core_potential_dp(pbuffer, 140, 1, 35, 38, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dd(pbuffer, 158, 4, 38, 47, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_df(pbuffer, 194, 10, 47, 65, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dg(pbuffer, 254, 20, 65, 95, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_fd(pbuffer, 344, 47, 140, 158, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_ff(pbuffer, 404, 65, 158, 194, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_fg(pbuffer, 504, 95, 194, 254, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_gf(pbuffer, 654, 194, 344, 404, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_gg(pbuffer, 804, 254, 404, 504, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_hg(pbuffer, 1029, 504, 654, 804, pfactors, 8, 14);

                    t2cgeom::comp_prim_op_geom_20_fx(pbuffer, 1344, 95, 504, 1029, 1, 15, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 1344, ket_width, ket_npgtos);
                }
            }
...
#endif /* LocalCorePotentialGeom200FG_hpp */
