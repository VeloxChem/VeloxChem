#ifndef LocalCorePotentialGeom200GP_hpp
#define LocalCorePotentialGeom200GP_hpp
...
template <class T>
auto
comp_local_core_potential_geom_200_gp(T& distributor,
                                      ...
                                      const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(578, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < ecp_nppt; l++)
                {
                    ...
                    t2lecp::comp_prim_local_core_potential_ps(pbuffer, 4, 0, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_pp(pbuffer, 7, 0, 1, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_ds(pbuffer, 16, 0, 4, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dp(pbuffer, 22, 1, 4, 7, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_fs(pbuffer, 40, 4, 16, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_fp(pbuffer, 50, 7, 16, 22, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_gs(pbuffer, 80, 16, 40, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_gp(pbuffer, 95, 22, 40, 50, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_hs(pbuffer, 140, 40, 80, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_hp(pbuffer, 161, 50, 80, 95, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_ip(pbuffer, 224, 95, 140, 161, pfactors, 8, 14);

                    t2cgeom::comp_prim_op_geom_20_gx(pbuffer, 308, 22, 95, 224, 1, 3, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 308, ket_width, ket_npgtos);
                }
            }
...
#endif /* LocalCorePotentialGeom200GP_hpp */
