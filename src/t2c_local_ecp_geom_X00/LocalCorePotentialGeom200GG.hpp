#ifndef LocalCorePotentialGeom200GG_hpp
#define LocalCorePotentialGeom200GG_hpp
...
template <class T>
auto
comp_local_core_potential_geom_200_gg(T& distributor,
                                      ...
                                      const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(3450, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < ecp_nppt; l++)
                {
                    ...
                    t2lecp::comp_prim_local_core_potential_fp(pbuffer, 350, 38, 140, 146, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_fd(pbuffer, 380, 47, 146, 164, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_ff(pbuffer, 440, 65, 164, 200, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_fg(pbuffer, 540, 95, 200, 260, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_gd(pbuffer, 690, 164, 350, 380, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_gf(pbuffer, 780, 200, 380, 440, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_gg(pbuffer, 930, 260, 440, 540, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_hf(pbuffer, 1155, 440, 690, 780, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_hg(pbuffer, 1365, 540, 780, 930, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_ig(pbuffer, 1680, 930, 1155, 1365, pfactors, 8, 14);

                    t2cgeom::comp_prim_op_geom_20_gx(pbuffer, 2100, 260, 930, 1680, 1, 15, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 2100, ket_width, ket_npgtos);
                }
            }
...
#endif /* LocalCorePotentialGeom200GG_hpp */
