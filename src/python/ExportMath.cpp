...
export_math(py::module &m) -> void
{
    ...
    m.def(
        ...
        [](const std::array<int, 2> orders,
           ...
           const mat_t              mtype) -> std::shared_ptr<CMatrices> {
    ...
    m.def(
        ...
        [](const int              order,
           ...
           const CMolecularBasis &ket_basis) -> std::shared_ptr<CMatrices> {
    ...
    PyClass<CSubMatrix>(m, "SubMatrix")
        ...
        .def(py::pickle(
            [](const CSubMatrix &submat) { return py::make_tuple(submat.get_values(), submat.get_dimensions()); },
            [](py::tuple t) {
                return CSubMatrix(t[0].cast<std::vector<double>>(), t[1].cast<std::array<size_t, 4>>());
            }))
        .def(
            "at",
            [](CSubMatrix &self, const std::pair<size_t, size_t> &index, const double value) -> void {
                self.at(index) = value;
            },
        ...
        .def(
            ...
            [](CSubMatrix &self, const py::array_t<double> &values) -> void {
                if (values.ndim() == 2)
                {
                    ...
                    {
                        std::ranges::for_each(views::rectangular(nrows, ncols), [&](const auto &index) {
                            self.at(index) = values.at(index.first, index.second);
                        });
                    }
                }
            },
        ...
        .def("__setitem__",
             [](CSubMatrix &self, const std::pair<size_t, size_t> &index, const double value) { self[index] = value; })
    ...
    PyClass<CMatrix>(m, "Matrix")
        ...
        .def(py::pickle(
            [](const CMatrix &mat) { return py::make_tuple(mat.get_type(), mat.angular_pairs(), mat.sub_matrices()); },
            [](py::tuple t) {
                return CMatrix(t[1].cast<std::vector<std::pair<int, int>>>(),
                               t[2].cast<std::vector<CSubMatrix>>(),
                               t[0].cast<mat_t>());
            }))
        .def("add",
             py::overload_cast<const CSubMatrix &, const std::pair<int, int> &>(&CMatrix::add),
             "Adds submatrix to matrix.")
        .def("add",
             py::overload_cast<const std::array<size_t, 4> &, const std::pair<int, int> &>(&CMatrix::add),
             "Adds submatrix to matrix.")
        ...
        .def(
            ...
            [](const CMatrix &self) -> std::shared_ptr<CSubMatrix> {
                return std::make_shared<CSubMatrix>(self.full_matrix());
            },
