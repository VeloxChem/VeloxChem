...
export_oneeints(py::module& m)
{
    ...
    m.def("compute_electric_field_gradient",
            [](const CMolecule&           molecule,
               const CMolecularBasis&     basis,
               const py::array_t<double>& dipole_coords,
               const py::array_t<double>& dipole_moments,
               const py::array_t<double>& D) -> py::array_t<double> {
                std::string errstyle("compute_electric_field_gradient: Expecting contiguous numpy arrays");
                ...
                std::string errsize("compute_electric_field_gradient: Inconsistent dimension of dipole coordinates/moments");
                errors::assertMsgCritical(dipole_coords.shape(1) == 3, errsize);
                errors::assertMsgCritical(dipole_moments.shape(1) == 3, errsize);
                std::string errshape("compute_electric_field_gradient: Expecting square matrix D");
                errors::assertMsgCritical(D.shape(0) == D.shape(1), errshape);
                auto ndipoles = static_cast<int>(dipole_coords.shape(0));
                auto naos = static_cast<int>(D.shape(0));
                auto ef_grad = onee::computeElectricFieldGradients(molecule, basis, dipole_coords.data(), dipole_moments.data(), ndipoles, D.data(), naos);
