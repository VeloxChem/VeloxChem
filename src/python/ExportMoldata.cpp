...
export_moldata(py::module &m)
{
    ...
    PyClass<CMolecule>(m, "Molecule")
        ...
        .def(py::pickle(
            [](const CMolecule &mol) { return py::make_tuple(mol.identifiers(), mol.coordinates("au"), mol.get_charge(), mol.get_multiplicity()); },
            [](py::tuple t) {
                auto mol = CMolecule(t[0].cast<std::vector<int>>(), t[1].cast<std::vector<TPoint<double>>>(), std::string("au"));
