...
auto
compPrimitiveQuadrupolePS_Y_0(TDoubleArray&       buffer_xx,
...
#pragma omp simd aligned(fints_xx, fints_xy, fints_xz, fints_yy, fints_yz, fints_zz, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += faa_xx * rpa_y;

        fints_xy[i] += faa_x * (1.0 / 2.0) * fe_0;

        fints_xy[i] += faa_xy * rpa_y;

        fints_xz[i] += faa_xz * rpa_y;

        fints_yy[i] += faa_y * fe_0;

        fints_yy[i] += faa_yy * rpa_y;

        fints_yz[i] += faa_z * (1.0 / 2.0) * fe_0;

        fints_yz[i] += faa_yz * rpa_y;

        fints_zz[i] += faa_zz * rpa_y;
    }
}
