...
auto
compPrimitiveQuadrupolePD_Y_ZZ(TDoubleArray&       buffer_xx,
...
#pragma omp simd aligned(fints_xx, fints_xy, fints_xz, fints_yy, fints_yz, fints_zz, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += faa_xx * ((1.0 / 2.0) * fe_0 * rpa_y + rpa_y * rpb_z * rpb_z);

        fints_xy[i] += faa_x * ((1.0 / 2.0) * fe_0 * rpb_z * rpb_z + (1.0 / 4.0) * fe_0 * fe_0);

        fints_xy[i] += faa_xy * ((1.0 / 2.0) * fe_0 * rpa_y + rpa_y * rpb_z * rpb_z);

        fints_xz[i] += faa_x * fe_0 * rpa_y * rpb_z;

        fints_xz[i] += faa_xz * ((1.0 / 2.0) * fe_0 * rpa_y + rpa_y * rpb_z * rpb_z);

        fints_yy[i] += faa_y * (fe_0 * rpb_z * rpb_z + (1.0 / 2.0) * fe_0 * fe_0);

        fints_yy[i] += faa_yy * ((1.0 / 2.0) * fe_0 * rpa_y + rpa_y * rpb_z * rpb_z);

        fints_yz[i] += fss * (1.0 / 2.0) * fe_0 * fe_0 * rpb_z;

        fints_yz[i] += faa_z * ((1.0 / 2.0) * fe_0 * rpb_z * rpb_z + (1.0 / 4.0) * fe_0 * fe_0);

        fints_yz[i] += faa_y * fe_0 * rpa_y * rpb_z;

        fints_yz[i] += faa_yz * ((1.0 / 2.0) * fe_0 * rpa_y + rpa_y * rpb_z * rpb_z);

        fints_zz[i] += fss * (1.0 / 2.0) * fe_0 * fe_0 * rpa_y;

        fints_zz[i] += faa_z * 2.0 * fe_0 * rpa_y * rpb_z;

        fints_zz[i] += faa_zz * ((1.0 / 2.0) * fe_0 * rpa_y + rpa_y * rpb_z * rpb_z);
    }
}
