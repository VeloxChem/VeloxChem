...
auto
compPrimitiveQuadrupoleSP_0_Z(TDoubleArray&       buffer_xx,
...
#pragma omp simd aligned(fints_xx, fints_xy, fints_xz, fints_yy, fints_yz, fints_zz, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += faa_xx * rpb_z;

        fints_xy[i] += faa_xy * rpb_z;

        fints_xz[i] += faa_x * (1.0 / 2.0) * fe_0;

        fints_xz[i] += faa_xz * rpb_z;

        fints_yy[i] += faa_yy * rpb_z;

        fints_yz[i] += faa_y * (1.0 / 2.0) * fe_0;

        fints_yz[i] += faa_yz * rpb_z;

        fints_zz[i] += faa_z * fe_0;

        fints_zz[i] += faa_zz * rpb_z;
    }
}
