...
auto
compPrimitiveQuadrupolePF_Z_YYY(TDoubleArray&       buffer_xx,
...
#pragma omp simd aligned(fints_xx, fints_xy, fints_xz, fints_yy, fints_yz, fints_zz, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += faa_xx * ((3.0 / 2.0) * fe_0 * rpa_z * rpb_y + rpa_z * rpb_y * rpb_y * rpb_y);

        fints_xy[i] += faa_x * ((3.0 / 2.0) * fe_0 * rpa_z * rpb_y * rpb_y + (3.0 / 4.0) * fe_0 * fe_0 * rpa_z);

        fints_xy[i] += faa_xy * ((3.0 / 2.0) * fe_0 * rpa_z * rpb_y + rpa_z * rpb_y * rpb_y * rpb_y);

        fints_xz[i] += faa_x * ((1.0 / 2.0) * fe_0 * rpb_y * rpb_y * rpb_y + (3.0 / 4.0) * fe_0 * fe_0 * rpb_y);

        fints_xz[i] += faa_xz * ((3.0 / 2.0) * fe_0 * rpa_z * rpb_y + rpa_z * rpb_y * rpb_y * rpb_y);

        fints_yy[i] += fss * (3.0 / 2.0) * fe_0 * fe_0 * rpa_z * rpb_y;

        fints_yy[i] += faa_y * (3.0 * fe_0 * rpa_z * rpb_y * rpb_y + (3.0 / 2.0) * fe_0 * fe_0 * rpa_z);

        fints_yy[i] += faa_yy * ((3.0 / 2.0) * fe_0 * rpa_z * rpb_y + rpa_z * rpb_y * rpb_y * rpb_y);

        fints_yz[i] += fss * ((3.0 / 4.0) * fe_0 * fe_0 * rpb_y * rpb_y + (3.0 / 8.0) * fe_0 * fe_0 * fe_0);

        fints_yz[i] += faa_z * ((3.0 / 2.0) * fe_0 * rpa_z * rpb_y * rpb_y + (3.0 / 4.0) * fe_0 * fe_0 * rpa_z);

        fints_yz[i] += faa_y * ((1.0 / 2.0) * fe_0 * rpb_y * rpb_y * rpb_y + (3.0 / 4.0) * fe_0 * fe_0 * rpb_y);

        fints_yz[i] += faa_yz * ((3.0 / 2.0) * fe_0 * rpa_z * rpb_y + rpa_z * rpb_y * rpb_y * rpb_y);

        fints_zz[i] += faa_z * (fe_0 * rpb_y * rpb_y * rpb_y + (3.0 / 2.0) * fe_0 * fe_0 * rpb_y);

        fints_zz[i] += faa_zz * ((3.0 / 2.0) * fe_0 * rpa_z * rpb_y + rpa_z * rpb_y * rpb_y * rpb_y);
    }
}
