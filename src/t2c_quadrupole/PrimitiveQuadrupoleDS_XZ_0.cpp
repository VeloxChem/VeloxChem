#include "PrimitiveQuadrupoleDS_XZ_0.hpp"
...
auto
compPrimitiveQuadrupoleDS_XZ_0(TDoubleArray&       buffer_xx,
...
#pragma omp simd aligned(fints_xx, fints_xy, fints_xz, fints_yy, fints_yz, fints_zz, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += faa_x * fe_0 * rpa_z;

        fints_xx[i] += faa_xx * rpa_z * rpa_x;

        fints_xy[i] += faa_y * (1.0 / 2.0) * fe_0 * rpa_z;

        fints_xy[i] += faa_xy * rpa_z * rpa_x;

        fints_xz[i] += fss * (1.0 / 4.0) * fe_0 * fe_0;

        fints_xz[i] += faa_z * (1.0 / 2.0) * fe_0 * rpa_z;

        fints_xz[i] += faa_x * (1.0 / 2.0) * fe_0 * rpa_x;

        fints_xz[i] += faa_xz * rpa_z * rpa_x;

        fints_yy[i] += faa_yy * rpa_z * rpa_x;

        fints_yz[i] += faa_y * (1.0 / 2.0) * fe_0 * rpa_x;

        fints_yz[i] += faa_yz * rpa_z * rpa_x;

        fints_zz[i] += faa_z * fe_0 * rpa_x;

        fints_zz[i] += faa_zz * rpa_z * rpa_x;
    }
}
