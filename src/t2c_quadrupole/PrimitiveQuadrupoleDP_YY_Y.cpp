...
auto
compPrimitiveQuadrupoleDP_YY_Y(TDoubleArray&       buffer_xx,
...
#pragma omp simd aligned(fints_xx, fints_xy, fints_xz, fints_yy, fints_yz, fints_zz, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += faa_xx * (fe_0 * rpa_y + (1.0 / 2.0) * fe_0 * rpb_y + rpa_y * rpa_y * rpb_y);

        fints_xy[i] += faa_x * (fe_0 * rpa_y * rpb_y + (1.0 / 2.0) * fe_0 * rpa_y * rpa_y + (3.0 / 4.0) * fe_0 * fe_0);

        fints_xy[i] += faa_xy * (fe_0 * rpa_y + (1.0 / 2.0) * fe_0 * rpb_y + rpa_y * rpa_y * rpb_y);

        fints_xz[i] += faa_xz * (fe_0 * rpa_y + (1.0 / 2.0) * fe_0 * rpb_y + rpa_y * rpa_y * rpb_y);

        fints_yy[i] += fss * (fe_0 * fe_0 * rpa_y + (1.0 / 2.0) * fe_0 * fe_0 * rpb_y);

        fints_yy[i] += faa_y * (2.0 * fe_0 * rpa_y * rpb_y + fe_0 * rpa_y * rpa_y + (3.0 / 2.0) * fe_0 * fe_0);

        fints_yy[i] += faa_yy * (fe_0 * rpa_y + (1.0 / 2.0) * fe_0 * rpb_y + rpa_y * rpa_y * rpb_y);

        fints_yz[i] += faa_z * (fe_0 * rpa_y * rpb_y + (1.0 / 2.0) * fe_0 * rpa_y * rpa_y + (3.0 / 4.0) * fe_0 * fe_0);

        fints_yz[i] += faa_yz * (fe_0 * rpa_y + (1.0 / 2.0) * fe_0 * rpb_y + rpa_y * rpa_y * rpb_y);

        fints_zz[i] += faa_zz * (fe_0 * rpa_y + (1.0 / 2.0) * fe_0 * rpb_y + rpa_y * rpa_y * rpb_y);
    }
}
