...
auto
compPrimitiveQuadrupoleFS_XXZ_0(TDoubleArray&       buffer_xx,
...
#pragma omp simd aligned(fints_xx, fints_xy, fints_xz, fints_yy, fints_yz, fints_zz, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += faa_x * 2.0 * fe_0 * rpa_z * rpa_x;

        fints_xx[i] += faa_xx * ((1.0 / 2.0) * fe_0 * rpa_z + rpa_z * rpa_x * rpa_x);

        fints_xy[i] += faa_y * fe_0 * rpa_z * rpa_x;

        fints_xy[i] += faa_xy * ((1.0 / 2.0) * fe_0 * rpa_z + rpa_z * rpa_x * rpa_x);

        fints_xz[i] += fss * (1.0 / 2.0) * fe_0 * fe_0 * rpa_x;

        fints_xz[i] += faa_z * fe_0 * rpa_z * rpa_x;

        fints_xz[i] += faa_x * ((1.0 / 2.0) * fe_0 * rpa_x * rpa_x + (1.0 / 4.0) * fe_0 * fe_0);

        fints_xz[i] += faa_xz * ((1.0 / 2.0) * fe_0 * rpa_z + rpa_z * rpa_x * rpa_x);

        fints_yy[i] += faa_yy * ((1.0 / 2.0) * fe_0 * rpa_z + rpa_z * rpa_x * rpa_x);

        fints_yz[i] += faa_y * ((1.0 / 2.0) * fe_0 * rpa_x * rpa_x + (1.0 / 4.0) * fe_0 * fe_0);

        fints_yz[i] += faa_yz * ((1.0 / 2.0) * fe_0 * rpa_z + rpa_z * rpa_x * rpa_x);

        fints_zz[i] += faa_z * (fe_0 * rpa_x * rpa_x + (1.0 / 2.0) * fe_0 * fe_0);

        fints_zz[i] += faa_zz * ((1.0 / 2.0) * fe_0 * rpa_z + rpa_z * rpa_x * rpa_x);
    }
}
