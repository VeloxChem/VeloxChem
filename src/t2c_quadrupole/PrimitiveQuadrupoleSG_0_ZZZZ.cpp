...
auto
compPrimitiveQuadrupoleSG_0_ZZZZ(TDoubleArray&       buffer_xx,
...
#pragma omp simd aligned(fints_xx, fints_xy, fints_xz, fints_yy, fints_yz, fints_zz, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += faa_xx * (3.0 * fe_0 * rpb_z * rpb_z + (3.0 / 4.0) * fe_0 * fe_0 + rpb_z * rpb_z * rpb_z * rpb_z);

        fints_xy[i] += faa_xy * (3.0 * fe_0 * rpb_z * rpb_z + (3.0 / 4.0) * fe_0 * fe_0 + rpb_z * rpb_z * rpb_z * rpb_z);

        fints_xz[i] += faa_x * (2.0 * fe_0 * rpb_z * rpb_z * rpb_z + 3.0 * fe_0 * fe_0 * rpb_z);

        fints_xz[i] += faa_xz * (3.0 * fe_0 * rpb_z * rpb_z + (3.0 / 4.0) * fe_0 * fe_0 + rpb_z * rpb_z * rpb_z * rpb_z);

        fints_yy[i] += faa_yy * (3.0 * fe_0 * rpb_z * rpb_z + (3.0 / 4.0) * fe_0 * fe_0 + rpb_z * rpb_z * rpb_z * rpb_z);

        fints_yz[i] += faa_y * (2.0 * fe_0 * rpb_z * rpb_z * rpb_z + 3.0 * fe_0 * fe_0 * rpb_z);

        fints_yz[i] += faa_yz * (3.0 * fe_0 * rpb_z * rpb_z + (3.0 / 4.0) * fe_0 * fe_0 + rpb_z * rpb_z * rpb_z * rpb_z);

        fints_zz[i] += fss * (3.0 * fe_0 * fe_0 * rpb_z * rpb_z + (3.0 / 2.0) * fe_0 * fe_0 * fe_0);

        fints_zz[i] += faa_z * (4.0 * fe_0 * rpb_z * rpb_z * rpb_z + 6.0 * fe_0 * fe_0 * rpb_z);

        fints_zz[i] += faa_zz * (3.0 * fe_0 * rpb_z * rpb_z + (3.0 / 4.0) * fe_0 * fe_0 + rpb_z * rpb_z * rpb_z * rpb_z);
    }
}
