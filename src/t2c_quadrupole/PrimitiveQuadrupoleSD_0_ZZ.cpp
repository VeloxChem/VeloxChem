...
auto
compPrimitiveQuadrupoleSD_0_ZZ(TDoubleArray&       buffer_xx,
...
#pragma omp simd aligned(fints_xx, fints_xy, fints_xz, fints_yy, fints_yz, fints_zz, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += faa_xx * ((1.0 / 2.0) * fe_0 + rpb_z * rpb_z);

        fints_xy[i] += faa_xy * ((1.0 / 2.0) * fe_0 + rpb_z * rpb_z);

        fints_xz[i] += faa_x * fe_0 * rpb_z;

        fints_xz[i] += faa_xz * ((1.0 / 2.0) * fe_0 + rpb_z * rpb_z);

        fints_yy[i] += faa_yy * ((1.0 / 2.0) * fe_0 + rpb_z * rpb_z);

        fints_yz[i] += faa_y * fe_0 * rpb_z;

        fints_yz[i] += faa_yz * ((1.0 / 2.0) * fe_0 + rpb_z * rpb_z);

        fints_zz[i] += fss * (1.0 / 2.0) * fe_0 * fe_0;

        fints_zz[i] += faa_z * 2.0 * fe_0 * rpb_z;

        fints_zz[i] += faa_zz * ((1.0 / 2.0) * fe_0 + rpb_z * rpb_z);
    }
}
