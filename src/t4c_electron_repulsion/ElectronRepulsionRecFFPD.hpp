...
/// Computes (FF|1/|r-r'||PD)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_ffpd(1500, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ffpd(735, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_ffpd.zero();

        spher_buffer_0_ffpd.zero();
        ...
        erirec::comp_bra_hrr_electron_repulsion_ffxx(ket_spher_buffer_0_ffpd, ket_spher_buffer_0_dfpd, ket_spher_buffer_0_dgpd, ab_x, ab_y, ab_z, 1, 2);

        t4cfunc::bra_transform<3, 3>(spher_buffer_0_ffpd, ket_spher_buffer_0_ffpd, 1, 2);

        t4cfunc::store_values(buffer, spher_buffer_0_ffpd, 735 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 3, 3, 1, 2, bra_indices, ket_indices);
}
