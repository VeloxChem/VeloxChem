#ifndef ElectronRepulsionRecDFSP_hpp
#define ElectronRepulsionRecDFSP_hpp
...
/// Computes (DF|1/|r-r'||SP)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_dfsp(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_sfsp(30, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_sgsp(45, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_shsp(63, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_pfsp(90, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_pgsp(135, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_dfsp(180, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_dfsp(105, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_sfsp.zero();

        ket_spher_buffer_0_sgsp.zero();

        ket_spher_buffer_0_shsp.zero();

        ket_spher_buffer_0_pfsp.zero();

        ket_spher_buffer_0_pgsp.zero();

        ket_spher_buffer_0_dfsp.zero();

        spher_buffer_0_dfsp.zero();
        ...
        t4cfunc::ket_transform<0, 1>(ket_spher_buffer_0_sfsp, cart_buffer_0_sfsp, 0, 3);
        ...
        erirec::comp_bra_hrr_electron_repulsion_pfxx(ket_spher_buffer_0_pfsp, ket_spher_buffer_0_sfsp, ket_spher_buffer_0_sgsp, ab_x, ab_y, ab_z, 0, 1);
        ...
        erirec::comp_bra_hrr_electron_repulsion_dfxx(ket_spher_buffer_0_dfsp, ket_spher_buffer_0_pfsp, ket_spher_buffer_0_pgsp, ab_x, ab_y, ab_z, 0, 1);

        t4cfunc::bra_transform<2, 3>(spher_buffer_0_dfsp, ket_spher_buffer_0_dfsp, 0, 1);

        t4cfunc::store_values(buffer, spher_buffer_0_dfsp, 105 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 2, 3, 0, 1, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecDFSP_hpp */
