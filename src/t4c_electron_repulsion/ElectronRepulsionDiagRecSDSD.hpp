#ifndef ElectronRepulsionDiagRecSDSD_hpp
#define ElectronRepulsionDiagRecSDSD_hpp
...
/// @brief Computes (SD|1/|r-r'||SD)  integrals for GTOs pair block.
...
template <class T>
auto
comp_diag_electron_repulsion_sdsd(T& distributor, const CGtoPairBlock& gto_pair_block, const std::pair<size_t, size_t>& gto_indices) -> void
{
    ...
    CSimdArray<double> pbuffer(116, npgtos);
    ...
    CSimdArray<double> skbuffer(30, 1);
    ...
    for (auto i = gto_indices.first; i < gto_indices.second; i++)
    {
        ...
        for (int j = 0; j < npgtos; j++)
        {
            ...
            t2cfunc::reduce(cbuffer, 0, pbuffer, 80, 36, ket_width, npgtos);
        }

        t4cfunc::ket_transform<0, 2>(skbuffer, 0, cbuffer, 0, 0, 2);

        t4cfunc::bra_transform<0, 2>(sbuffer, 0, skbuffer, 0, 0, 2);
...
template <class T>
auto
comp_diag_electron_repulsion_dsds(T& distributor, const CGtoPairBlock& gto_pair_block, const std::pair<size_t, size_t>& gto_indices) -> void
{
    // intialize GTOs pair data
...
#endif /* ElectronRepulsionDiagRecSDSD_hpp */
