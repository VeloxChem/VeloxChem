#ifndef ElectronRepulsionRecSSPP_hpp
#define ElectronRepulsionRecSSPP_hpp
...
/// Computes (SS|1/|r-r'||PP)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_sspp(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> cart_buffer_0_sssp(3, ket_dim);

    CSimdArray<double> cart_buffer_0_sssd(6, ket_dim);

    // allocate aligned contracted integrals

    CSimdArray<double> contr_buffer_0_sspp(9, ket_dim);

    // allocate aligned half transformed integrals

    CSimdArray<double> ket_spher_buffer_0_sspp(9, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_sspp(9, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        cart_buffer_0_sssp.zero();

        cart_buffer_0_sssd.zero();

        ket_spher_buffer_0_sspp.zero();

        spher_buffer_0_sspp.zero();
        ...
        for (int j = 0; j < bra_npgtos; j++)
        {
            ...
            t2cfunc::reduce(cart_buffer_0_sssp, prim_buffer_0_sssp, ket_dim, ket_npgtos);

            t2cfunc::reduce(cart_buffer_0_sssd, prim_buffer_0_sssd, ket_dim, ket_npgtos);

        }

        erirec::comp_ket_hrr_electron_repulsion_xxpp(contr_buffer_0_sspp, cart_buffer_0_sssp, cart_buffer_0_sssd, cd_x[0], cd_y[0], cd_z[0], 0, 0);

        t4cfunc::ket_transform<1, 1>(ket_spher_buffer_0_sspp, contr_buffer_0_sspp, 0, 0);

        t4cfunc::bra_transform<0, 0>(spher_buffer_0_sspp, ket_spher_buffer_0_sspp, 1, 1);

        t4cfunc::store_values(buffer, spher_buffer_0_sspp, 9 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 0, 1, 1, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSSPP_hpp */
