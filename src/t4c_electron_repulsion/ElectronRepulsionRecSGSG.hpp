#ifndef ElectronRepulsionRecSGSG_hpp
#define ElectronRepulsionRecSGSG_hpp
...
template <class T>
auto
comp_electron_repulsion_sgsg(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_sgsg(81, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::store_values(buffer, spher_buffer_0_sgsg, 81 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, 0, 4, bra_indices, ket_indices);
}

/// Computes (SG|1/|r-r'||SG)  integrals for two GTOs pair blocks.
/// - Parameter distributor: the pointer to Fock matrix/matrices distributor.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 4, 0, 4, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSGSG_hpp */
