...
/// @brief Computes (SG|1/|r-r'||SG)  integrals for two basis function pairs blocks.
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 1351, 225, ket_width, ket_npgtos);
            }
            ...
            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 4, 0, 4, j, ket_range, diagonal);
        }
    }
}
