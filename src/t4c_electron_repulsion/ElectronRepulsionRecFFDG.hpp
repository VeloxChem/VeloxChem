...
/// Computes (FF|1/|r-r'||DG)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_dfdg(2700, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_dgdg(4050, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_ffdg(4500, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ffdg(2205, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_dfdg.zero();

        ket_spher_buffer_0_dgdg.zero();

        ket_spher_buffer_0_ffdg.zero();

        spher_buffer_0_ffdg.zero();
        ...
        erirec::comp_bra_hrr_electron_repulsion_dfxx(ket_spher_buffer_0_dfdg, ket_spher_buffer_0_pfdg, ket_spher_buffer_0_pgdg, ab_x, ab_y, ab_z, 2, 4);
        ...
        erirec::comp_bra_hrr_electron_repulsion_ffxx(ket_spher_buffer_0_ffdg, ket_spher_buffer_0_dfdg, ket_spher_buffer_0_dgdg, ab_x, ab_y, ab_z, 2, 4);

        t4cfunc::bra_transform<3, 3>(spher_buffer_0_ffdg, ket_spher_buffer_0_ffdg, 2, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_ffdg, 2205 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 3, 3, 2, 4, bra_indices, ket_indices);
}
