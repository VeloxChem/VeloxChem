#ifndef ElectronRepulsionRecFFDG_hpp
#define ElectronRepulsionRecFFDG_hpp
...
/// @brief Computes (FF|1/|r-r'||DG)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_ffdg(T& distributor,
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(14652, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(20790, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 4881, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 5481, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 360, pbuffer, 6321, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 640, pbuffer, 7831, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 865, pbuffer, 8506, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1180, pbuffer, 9451, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1600, pbuffer, 10921, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1915, pbuffer, 11551, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2356, pbuffer, 12433, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2944, pbuffer, 13609, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3364, pbuffer, 14029, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3952, pbuffer, 14617, 784, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 7218, cbuffer, 0, 0, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 9108, cbuffer, 2944, 3364, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 10368, cbuffer, 3364, 3952, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 12132, cbuffer, 0, 0, cfactors, 6, 0, 6);

            t4cfunc::ket_transform<2, 4>(skbuffer, 0, ckbuffer, 1080, 0, 3);

            t4cfunc::ket_transform<2, 4>(skbuffer, 900, ckbuffer, 3600, 0, 4);

            t4cfunc::ket_transform<2, 4>(skbuffer, 2250, ckbuffer, 7218, 0, 5);

            t4cfunc::ket_transform<2, 4>(skbuffer, 4140, ckbuffer, 12132, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 3330, 0, 450, r_ab, 2, 4);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 4680, 450, 1125, r_ab, 2, 4);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 6705, 1125, 2070, r_ab, 2, 4);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 9540, 3330, 4680, r_ab, 2, 4);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 12240, 4680, 6705, r_ab, 2, 4);

            erirec::comp_bra_hrr_electron_repulsion_ffxx(skbuffer, 16290, 9540, 12240, r_ab, 2, 4);

            t4cfunc::bra_transform<3, 3>(sbuffer, 0, skbuffer, 16290, 2, 4);

            const bool diagonal = bra_eq_ket && (j >= ket_range.first) && (j < ket_range.second);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 3, 3, 2, 4, j, ket_range, diagonal);
        }
    }
...
#endif /* ElectronRepulsionRecFFDG_hpp */
