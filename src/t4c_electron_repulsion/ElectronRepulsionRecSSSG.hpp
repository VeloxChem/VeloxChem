#ifndef ElectronRepulsionRecSSSG_hpp
#define ElectronRepulsionRecSSSG_hpp
...
/// Computes (SS|1/|r-r'||SG)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_sssg(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_sssg(9, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_sssg(9, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_sssg.zero();

        spher_buffer_0_sssg.zero();
        ...
        t4cfunc::ket_transform<0, 4>(ket_spher_buffer_0_sssg, cart_buffer_0_sssg, 0, 0);

        t4cfunc::bra_transform<0, 0>(spher_buffer_0_sssg, ket_spher_buffer_0_sssg, 0, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_sssg, 9 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 0, 0, 4, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSSSG_hpp */
