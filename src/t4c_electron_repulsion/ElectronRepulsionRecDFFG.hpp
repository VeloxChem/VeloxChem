#ifndef ElectronRepulsionRecDFFG_hpp
#define ElectronRepulsionRecDFFG_hpp
...
/// @brief Computes (DF|1/|r-r'||FG)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_dffg(T& distributor,
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(25668, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(11403, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 5555, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 6005, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 360, pbuffer, 6635, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 640, pbuffer, 7475, 360, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1000, pbuffer, 8705, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1225, pbuffer, 9155, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1540, pbuffer, 9785, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1960, pbuffer, 10625, 540, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2500, pbuffer, 11705, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2815, pbuffer, 12020, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3256, pbuffer, 12461, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3844, pbuffer, 13049, 756, ket_width, ket_npgtos);
            ...
            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 2898, 0, 630, r_ab, 3, 4);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 4788, 630, 1575, r_ab, 3, 4);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 7623, 2898, 4788, r_ab, 3, 4);

            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 7623, 3, 4);

            const bool diagonal = bra_eq_ket && (j >= ket_range.first) && (j < ket_range.second);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 2, 3, 3, 4, j, ket_range, diagonal);
        }
    }
...
#endif /* ElectronRepulsionRecDFFG_hpp */
