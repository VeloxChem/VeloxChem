...
/// Computes (FF|1/|r-r'||SG)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_ffsg(900, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ffsg(441, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_ffsg.zero();

        spher_buffer_0_ffsg.zero();
        ...
        erirec::comp_bra_hrr_electron_repulsion_ffxx(ket_spher_buffer_0_ffsg, ket_spher_buffer_0_dfsg, ket_spher_buffer_0_dgsg, ab_x, ab_y, ab_z, 0, 4);

        t4cfunc::bra_transform<3, 3>(spher_buffer_0_ffsg, ket_spher_buffer_0_ffsg, 0, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_ffsg, 441 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 3, 3, 0, 4, bra_indices, ket_indices);
}
