#ifndef ElectronRepulsionRecSDPG_hpp
#define ElectronRepulsionRecSDPG_hpp
...
/// @brief Computes (SD|1/|r-r'||PG)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_sdpg(T&                               distributor,
                             ...
                             const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(270, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 484, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 90, pbuffer, 574, 126, ket_width, ket_npgtos);
            }

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 0, cbuffer, 0, 90, cfactors, 6, 0, 2);

            t4cfunc::ket_transform<1, 4>(skbuffer, 0, ckbuffer, 0, 0, 2);

            t4cfunc::bra_transform<0, 2>(sbuffer, 0, skbuffer, 0, 1, 4);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 2, 1, 4, j, ket_range, bra_eq_ket);
        }
    }
}
...
#endif /* ElectronRepulsionRecSDPG_hpp */
