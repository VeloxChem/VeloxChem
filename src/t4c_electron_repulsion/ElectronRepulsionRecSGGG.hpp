#ifndef ElectronRepulsionRecSGGG_hpp
#define ElectronRepulsionRecSGGG_hpp
...
/// @brief Computes (SG|1/|r-r'||GG)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_sggg(T& distributor,
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(19035, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 8796, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 225, pbuffer, 9021, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 540, pbuffer, 9336, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 960, pbuffer, 9756, 540, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1500, pbuffer, 10296, 675, ket_width, ket_npgtos);
            ...
            t4cfunc::bra_transform<0, 4>(sbuffer, 0, skbuffer, 0, 4, 4);

            const bool diagonal = bra_eq_ket && (j >= ket_range.first) && (j < ket_range.second);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 4, 4, 4, j, ket_range, diagonal);
        }
    }
...
#endif /* ElectronRepulsionRecSGGG_hpp */
