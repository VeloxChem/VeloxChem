#ifndef ElectronRepulsionRecSFSF_hpp
#define ElectronRepulsionRecSFSF_hpp
...
template <class T>
auto
comp_electron_repulsion_sfsf(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_sfsf(49, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::store_values(buffer, spher_buffer_0_sfsf, 49 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, 0, 3, bra_indices, ket_indices);
}

/// Computes (SF|1/|r-r'||SF)  integrals for two GTOs pair blocks.
/// - Parameter distributor: the pointer to Fock matrix/matrices distributor.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 3, 0, 3, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSFSF_hpp */
