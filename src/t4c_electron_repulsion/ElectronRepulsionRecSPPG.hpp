#ifndef ElectronRepulsionRecSPPG_hpp
#define ElectronRepulsionRecSPPG_hpp
...
/// Computes (SP|1/|r-r'||PG)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_sppg(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_sppg(81, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<0, 1>(spher_buffer_0_sppg, ket_spher_buffer_0_sppg, 1, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_sppg, 81 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 1, 1, 4, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSPPG_hpp */
