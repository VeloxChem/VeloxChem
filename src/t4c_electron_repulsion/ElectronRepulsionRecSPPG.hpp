...
/// @brief Computes (SP|1/|r-r'||PG)  integrals for two basis function pairs blocks.
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 182, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 45, pbuffer, 227, 63, ket_width, ket_npgtos);
            ...
            t4cfunc::bra_transform<0, 1>(sbuffer, 0, skbuffer, 0, 1, 4);
