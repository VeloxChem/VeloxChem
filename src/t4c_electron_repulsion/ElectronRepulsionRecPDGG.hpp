#ifndef ElectronRepulsionRecPDGG_hpp
#define ElectronRepulsionRecPDGG_hpp
...
/// @brief Computes (PD|1/|r-r'||GG)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_pdgg(T&                               distributor,
                             ...
                             const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(20304, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(2754, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 2433, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 90, pbuffer, 2613, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 216, pbuffer, 2865, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 384, pbuffer, 3201, 216, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 600, pbuffer, 3633, 270, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 870, pbuffer, 4173, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1020, pbuffer, 4323, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1230, pbuffer, 4533, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1510, pbuffer, 4813, 360, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1870, pbuffer, 5173, 450, ket_width, ket_npgtos);
            }
            ...
            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 1296, 0, 486, r_ab, 4, 4);

            t4cfunc::bra_transform<1, 2>(sbuffer, 0, skbuffer, 1296, 4, 4);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 2, 4, 4, j, ket_range, bra_eq_ket);
        }
    }
}
...
#endif /* ElectronRepulsionRecPDGG_hpp */
