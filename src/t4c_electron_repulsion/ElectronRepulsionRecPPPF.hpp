#ifndef ElectronRepulsionRecPPPF_hpp
#define ElectronRepulsionRecPPPF_hpp
...
/// Computes (PP|1/|r-r'||PF)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_pppf(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_sppf(63, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_sdpf(126, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_pppf(189, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_pppf(189, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_sppf.zero();

        ket_spher_buffer_0_sdpf.zero();

        ket_spher_buffer_0_pppf.zero();

        spher_buffer_0_pppf.zero();
        ...
        t4cfunc::ket_transform<1, 3>(ket_spher_buffer_0_sppf, contr_buffer_0_sppf, 0, 1);

        t4cfunc::ket_transform<1, 3>(ket_spher_buffer_0_sdpf, contr_buffer_0_sdpf, 0, 2);

        erirec::comp_bra_hrr_electron_repulsion_ppxx(ket_spher_buffer_0_pppf, ket_spher_buffer_0_sppf, ket_spher_buffer_0_sdpf, ab_x, ab_y, ab_z, 1, 3);

        t4cfunc::bra_transform<1, 1>(spher_buffer_0_pppf, ket_spher_buffer_0_pppf, 1, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_pppf, 189 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 1, 1, 3, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecPPPF_hpp */
