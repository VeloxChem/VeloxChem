...
/// @brief Computes (PP|1/|r-r'||PF)  integrals for two basis function pairs blocks.
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t4cfunc::ket_transform<1, 3>(skbuffer, 90, ckbuffer, 90, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 189, 0, 63, r_ab, 1, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 189, 1, 3);

            const bool diagonal = bra_eq_ket && (j >= ket_range.first) && (j < ket_range.second);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 1, 1, 3, j, ket_range, diagonal);
        }
    }
