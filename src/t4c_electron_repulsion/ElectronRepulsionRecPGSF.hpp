#ifndef ElectronRepulsionRecPGSF_hpp
#define ElectronRepulsionRecPGSF_hpp
...
/// @brief Computes (PG|1/|r-r'||SF)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_pgsf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 1320, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 1620, 210, ket_width, ket_npgtos);
            ...
            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 252, 0, 105, r_ab, 0, 3);

            t4cfunc::bra_transform<1, 4>(sbuffer, 0, skbuffer, 252, 0, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 4, 0, 3, j, ket_range, bra_eq_ket);
        }
    }
...
#endif /* ElectronRepulsionRecPGSF_hpp */
