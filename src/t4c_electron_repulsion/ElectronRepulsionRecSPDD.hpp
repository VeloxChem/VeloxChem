                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_spdd(75, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<0, 1>(spher_buffer_0_spdd, ket_spher_buffer_0_spdd, 2, 2);

        t4cfunc::store_values(buffer, spher_buffer_0_spdd, 75 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 1, 2, 2, bra_indices, ket_indices);
}
