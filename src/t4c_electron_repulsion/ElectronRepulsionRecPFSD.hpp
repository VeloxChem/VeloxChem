#ifndef ElectronRepulsionRecPFSD_hpp
#define ElectronRepulsionRecPFSD_hpp
...
/// @brief Computes (PF|1/|r-r'||SD)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_pfsd(T&                               distributor,
                             ...
                             const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> skbuffer(275, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 340, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 60, pbuffer, 460, 90, ket_width, ket_npgtos);
            }
            ...
            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 125, 0, 2);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 3, 0, 2, j, ket_range, bra_eq_ket);
        }
    }
}
...
#endif /* ElectronRepulsionRecPFSD_hpp */
