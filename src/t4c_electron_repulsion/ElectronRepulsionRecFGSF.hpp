#ifndef ElectronRepulsionRecFGSF_hpp
#define ElectronRepulsionRecFGSF_hpp
...
/// @brief Computes (FG|1/|r-r'||SF)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_fgsf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 2405, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 3320, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 360, pbuffer, 4118, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 640, pbuffer, 4678, 360, ket_width, ket_npgtos);
            ...
            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 700, 0, 105, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 1015, 105, 252, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pixx(skbuffer, 1456, 252, 448, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 2044, 700, 1015, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_dhxx(skbuffer, 2674, 1015, 1456, r_ab, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_fgxx(skbuffer, 3556, 2044, 2674, r_ab, 0, 3);

            t4cfunc::bra_transform<3, 4>(sbuffer, 0, skbuffer, 3556, 0, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 3, 4, 0, 3, j, ket_range, bra_eq_ket);
        }
    }
...
#endif /* ElectronRepulsionRecFGSF_hpp */
