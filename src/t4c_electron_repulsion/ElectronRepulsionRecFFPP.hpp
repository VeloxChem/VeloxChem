#ifndef ElectronRepulsionRecFFPP_hpp
#define ElectronRepulsionRecFFPP_hpp
...
/// @brief Computes (FF|1/|r-r'||PP)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_ffpp(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 225, pbuffer, 1392, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 288, pbuffer, 1518, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 414, pbuffer, 1770, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 498, pbuffer, 1854, 168, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpp(ckbuffer, 414, cbuffer, 414, 498, cfactors, 6, 0, 6);
            ...
            t4cfunc::ket_transform<1, 1>(skbuffer, 414, ckbuffer, 414, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 666, 0, 90, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 936, 90, 225, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 1341, 225, 414, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 1908, 666, 936, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 2448, 936, 1341, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_ffxx(skbuffer, 3258, 1908, 2448, r_ab, 1, 1);

            t4cfunc::bra_transform<3, 3>(sbuffer, 0, skbuffer, 3258, 1, 1);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 3, 3, 1, 1, j, ket_range, bra_eq_ket);
        }
    }
...
#endif /* ElectronRepulsionRecFFPP_hpp */
