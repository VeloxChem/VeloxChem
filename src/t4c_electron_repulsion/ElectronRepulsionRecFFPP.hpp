...
/// Computes (FF|1/|r-r'||PP)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_dfpp(540, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_dgpp(810, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_ffpp(900, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ffpp(441, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_dfpp.zero();

        ket_spher_buffer_0_dgpp.zero();

        ket_spher_buffer_0_ffpp.zero();

        spher_buffer_0_ffpp.zero();
        ...
        erirec::comp_bra_hrr_electron_repulsion_dfxx(ket_spher_buffer_0_dfpp, ket_spher_buffer_0_pfpp, ket_spher_buffer_0_pgpp, ab_x, ab_y, ab_z, 1, 1);
        ...
        erirec::comp_bra_hrr_electron_repulsion_ffxx(ket_spher_buffer_0_ffpp, ket_spher_buffer_0_dfpp, ket_spher_buffer_0_dgpp, ab_x, ab_y, ab_z, 1, 1);

        t4cfunc::bra_transform<3, 3>(spher_buffer_0_ffpp, ket_spher_buffer_0_ffpp, 1, 1);

        t4cfunc::store_values(buffer, spher_buffer_0_ffpp, 441 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 3, 3, 1, 1, bra_indices, ket_indices);
}
