#ifndef ElectronRepulsionRecSPFF_hpp
#define ElectronRepulsionRecSPFF_hpp
...
/// @brief Computes (SP|1/|r-r'||FF)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_spff(T& distributor,
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(1164, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 294, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 30, pbuffer, 324, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 75, pbuffer, 369, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 138, pbuffer, 432, 84, ket_width, ket_npgtos);
            ...
            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 1, 3, 3, j, ket_range, bra_eq_ket);
        }
    }
...
#endif /* ElectronRepulsionRecSPFF_hpp */
