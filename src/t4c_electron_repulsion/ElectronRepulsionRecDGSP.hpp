#ifndef ElectronRepulsionRecDGSP_hpp
#define ElectronRepulsionRecDGSP_hpp
...
/// @brief Computes (DG|1/|r-r'||SP)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_dgsp(T&                               distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 392, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 45, pbuffer, 548, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 108, pbuffer, 674, 84, ket_width, ket_npgtos);
            }
            ...
            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 192, 0, 45, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 327, 45, 108, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 516, 192, 327, r_ab, 0, 1);

            t4cfunc::bra_transform<2, 4>(sbuffer, 0, skbuffer, 516, 0, 1);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 2, 4, 0, 1, j, ket_range, bra_eq_ket);
        }
    }
}
...
#endif /* ElectronRepulsionRecDGSP_hpp */
