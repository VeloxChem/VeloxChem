#ifndef ElectronRepulsionRecSPSG_hpp
#define ElectronRepulsionRecSPSG_hpp
...
/// Computes (SP|1/|r-r'||SG)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_spsg(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_spsg(27, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_spsg(27, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 27, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_spsg.zero();

        spher_buffer_0_spsg.zero();
        ...
        t4cfunc::ket_transform<0, 4>(ket_spher_buffer_0_spsg, cart_buffer_0_spsg, 0, 1);

        t4cfunc::bra_transform<0, 1>(spher_buffer_0_spsg, ket_spher_buffer_0_spsg, 0, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_spsg, 27 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 1, 0, 4, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSPSG_hpp */
