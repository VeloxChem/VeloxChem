...
/// Computes (FF|1/|r-r'||SD)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_dfsd(300, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_dgsd(450, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_ffsd(500, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ffsd(245, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_dfsd.zero();

        ket_spher_buffer_0_dgsd.zero();

        ket_spher_buffer_0_ffsd.zero();

        spher_buffer_0_ffsd.zero();
        ...
        erirec::comp_bra_hrr_electron_repulsion_dfxx(ket_spher_buffer_0_dfsd, ket_spher_buffer_0_pfsd, ket_spher_buffer_0_pgsd, ab_x, ab_y, ab_z, 0, 2);
        ...
        erirec::comp_bra_hrr_electron_repulsion_ffxx(ket_spher_buffer_0_ffsd, ket_spher_buffer_0_dfsd, ket_spher_buffer_0_dgsd, ab_x, ab_y, ab_z, 0, 2);

        t4cfunc::bra_transform<3, 3>(spher_buffer_0_ffsd, ket_spher_buffer_0_ffsd, 0, 2);

        t4cfunc::store_values(buffer, spher_buffer_0_ffsd, 245 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 3, 3, 0, 2, bra_indices, ket_indices);
}
