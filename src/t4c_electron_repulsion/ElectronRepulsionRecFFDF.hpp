...
/// Computes (FF|1/|r-r'||DF)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_ffdf(3500, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ffdf(1715, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_ffdf.zero();

        spher_buffer_0_ffdf.zero();
        ...
        erirec::comp_bra_hrr_electron_repulsion_ffxx(ket_spher_buffer_0_ffdf, ket_spher_buffer_0_dfdf, ket_spher_buffer_0_dgdf, ab_x, ab_y, ab_z, 2, 3);

        t4cfunc::bra_transform<3, 3>(spher_buffer_0_ffdf, ket_spher_buffer_0_ffdf, 2, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_ffdf, 1715 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 3, 3, 2, 3, bra_indices, ket_indices);
}
