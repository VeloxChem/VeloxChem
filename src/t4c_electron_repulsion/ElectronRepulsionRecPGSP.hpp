#ifndef ElectronRepulsionRecPGSP_hpp
#define ElectronRepulsionRecPGSP_hpp
...
/// Computes (PG|1/|r-r'||SP)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_pgsp(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_pgsp(81, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<1, 4>(spher_buffer_0_pgsp, ket_spher_buffer_0_pgsp, 0, 1);

        t4cfunc::store_values(buffer, spher_buffer_0_pgsp, 81 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 4, 0, 1, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecPGSP_hpp */
