...
/// @brief Computes (PG|1/|r-r'||SP)  integrals for two basis function pairs blocks.
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 297, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 45, pbuffer, 387, 63, ket_width, ket_npgtos);
            }
            ...
            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 108, 0, 45, r_ab, 0, 1);

            t4cfunc::bra_transform<1, 4>(sbuffer, 0, skbuffer, 108, 0, 1);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 4, 0, 1, j, ket_range, bra_eq_ket);
        }
    }
}
