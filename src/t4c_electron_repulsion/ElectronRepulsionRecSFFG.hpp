#ifndef ElectronRepulsionRecSFFG_hpp
#define ElectronRepulsionRecSFFG_hpp
...
/// @brief Computes (SF|1/|r-r'||FG)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_sffg(T& distributor,
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(5580, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 2928, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 3078, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 360, pbuffer, 3288, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 640, pbuffer, 3568, 360, ket_width, ket_npgtos);
            ...
            t4cfunc::bra_transform<0, 3>(sbuffer, 0, skbuffer, 0, 3, 4);

            const bool diagonal = bra_eq_ket && (j >= ket_range.first) && (j < ket_range.second);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 3, 3, 4, j, ket_range, diagonal);
        }
    }
...
#endif /* ElectronRepulsionRecSFFG_hpp */
