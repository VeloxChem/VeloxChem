#ifndef ElectronRepulsionRecFFFG_hpp
#define ElectronRepulsionRecFFFG_hpp
...
/// @brief Computes (FF|1/|r-r'||FG)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_fffg(T& distributor,
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(41292, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(29106, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 6945, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 7545, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 360, pbuffer, 8385, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 640, pbuffer, 9505, 360, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1000, pbuffer, 11335, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1225, pbuffer, 12010, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1540, pbuffer, 12955, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1960, pbuffer, 14215, 540, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2500, pbuffer, 16045, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2815, pbuffer, 16675, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3256, pbuffer, 17557, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3844, pbuffer, 18733, 756, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4600, pbuffer, 20245, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5020, pbuffer, 20665, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5608, pbuffer, 21253, 784, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 6392, pbuffer, 22037, 1008, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 13950, cbuffer, 2500, 2815, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 14895, cbuffer, 2815, 3256, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpi(ckbuffer, 16218, cbuffer, 3256, 3844, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 17982, cbuffer, 0, 0, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdh(ckbuffer, 19872, cbuffer, 0, 0, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxfg(ckbuffer, 22518, cbuffer, 0, 0, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 25668, cbuffer, 4600, 5020, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 26928, cbuffer, 5020, 5608, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpi(ckbuffer, 28692, cbuffer, 5608, 6392, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 31044, cbuffer, 0, 0, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxdh(ckbuffer, 33564, cbuffer, 0, 0, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxfg(ckbuffer, 37092, cbuffer, 0, 0, cfactors, 6, 0, 6);

            t4cfunc::ket_transform<3, 4>(skbuffer, 0, ckbuffer, 4080, 0, 3);

            t4cfunc::ket_transform<3, 4>(skbuffer, 1500, ckbuffer, 11700, 0, 4);

            t4cfunc::ket_transform<3, 4>(skbuffer, 3750, ckbuffer, 22518, 0, 5);

            t4cfunc::ket_transform<3, 4>(skbuffer, 6900, ckbuffer, 37092, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 4662, 0, 630, r_ab, 3, 4);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 6552, 630, 1575, r_ab, 3, 4);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 9387, 1575, 2898, r_ab, 3, 4);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 13356, 4662, 6552, r_ab, 3, 4);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 17136, 6552, 9387, r_ab, 3, 4);

            erirec::comp_bra_hrr_electron_repulsion_ffxx(skbuffer, 22806, 13356, 17136, r_ab, 3, 4);

            t4cfunc::bra_transform<3, 3>(sbuffer, 0, skbuffer, 22806, 3, 4);

            const bool diagonal = bra_eq_ket && (j >= ket_range.first) && (j < ket_range.second);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 3, 3, 3, 4, j, ket_range, diagonal);
        }
    }
...
#endif /* ElectronRepulsionRecFFFG_hpp */
