...
/// Computes (PD|1/|r-r'||SG)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_sdsg(54, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_sfsg(90, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_pdsg(162, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_pdsg(135, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 135, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_sdsg.zero();

        ket_spher_buffer_0_sfsg.zero();

        ket_spher_buffer_0_pdsg.zero();

        spher_buffer_0_pdsg.zero();
        ...
        t4cfunc::ket_transform<0, 4>(ket_spher_buffer_0_sdsg, cart_buffer_0_sdsg, 0, 2);

        t4cfunc::ket_transform<0, 4>(ket_spher_buffer_0_sfsg, cart_buffer_0_sfsg, 0, 3);

        erirec::comp_bra_hrr_electron_repulsion_pdxx(ket_spher_buffer_0_pdsg, ket_spher_buffer_0_sdsg, ket_spher_buffer_0_sfsg, ab_x, ab_y, ab_z, 0, 4);

        t4cfunc::bra_transform<1, 2>(spher_buffer_0_pdsg, ket_spher_buffer_0_pdsg, 0, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_pdsg, 135 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 2, 0, 4, bra_indices, ket_indices);
}
