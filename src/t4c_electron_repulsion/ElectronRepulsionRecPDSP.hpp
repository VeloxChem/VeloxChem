#ifndef ElectronRepulsionRecPDSP_hpp
#define ElectronRepulsionRecPDSP_hpp
...
template <class T>
auto
comp_electron_repulsion_pdsp(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_pdsp(45, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 45, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<1, 2>(spher_buffer_0_pdsp, ket_spher_buffer_0_pdsp, 0, 1);

        t4cfunc::store_values(buffer, spher_buffer_0_pdsp, 45 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 2, 0, 1, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecPDSP_hpp */
