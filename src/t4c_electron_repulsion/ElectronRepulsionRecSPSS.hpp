                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_spss(3, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 3, ket_dim);
    ...
    CSimdArray<double> bf_values(2, ket_pdim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<0, 1>(spher_buffer_0_spss, ket_spher_buffer_0_spss, 0, 0);

        t4cfunc::store_values(buffer, spher_buffer_0_spss, 3 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 1, 0, 0, bra_indices, ket_indices);
}
