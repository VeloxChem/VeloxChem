#ifndef ElectronRepulsionRecSFDD_hpp
#define ElectronRepulsionRecSFDD_hpp
...
/// Computes (SF|1/|r-r'||DD)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_sfdd(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> contr_buffer_0_sfdd(360, ket_dim);

    // allocate aligned half transformed integrals

    CSimdArray<double> ket_spher_buffer_0_sfdd(250, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_sfdd(175, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_sfdd.zero();

        spher_buffer_0_sfdd.zero();
        ...
        erirec::comp_ket_hrr_electron_repulsion_xxdd(contr_buffer_0_sfdd, contr_buffer_0_sfpd, contr_buffer_0_sfpf, cd_x[0], cd_y[0], cd_z[0], 0, 3);

        t4cfunc::ket_transform<2, 2>(ket_spher_buffer_0_sfdd, contr_buffer_0_sfdd, 0, 3);

        t4cfunc::bra_transform<0, 3>(spher_buffer_0_sfdd, ket_spher_buffer_0_sfdd, 2, 2);

        t4cfunc::store_values(buffer, spher_buffer_0_sfdd, 175 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 3, 2, 2, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSFDD_hpp */
