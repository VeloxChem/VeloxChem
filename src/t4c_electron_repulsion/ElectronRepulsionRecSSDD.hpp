#ifndef ElectronRepulsionRecSSDD_hpp
#define ElectronRepulsionRecSSDD_hpp
...
template <class T>
auto
comp_electron_repulsion_ssdd(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> contr_buffer_0_ssdd(36, ket_dim);

    // allocate aligned half transformed integrals

    CSimdArray<double> ket_spher_buffer_0_ssdd(25, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ssdd(25, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_ssdd.zero();

        spher_buffer_0_ssdd.zero();
        ...
        erirec::comp_ket_hrr_electron_repulsion_xxdd(contr_buffer_0_ssdd, contr_buffer_0_sspd, contr_buffer_0_sspf, cd_x[0], cd_y[0], cd_z[0], 0, 0);

        t4cfunc::ket_transform<2, 2>(ket_spher_buffer_0_ssdd, contr_buffer_0_ssdd, 0, 0);

        t4cfunc::bra_transform<0, 0>(spher_buffer_0_ssdd, ket_spher_buffer_0_ssdd, 2, 2);

        t4cfunc::store_values(buffer, spher_buffer_0_ssdd, 25 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 0, 2, 2, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSSDD_hpp */
