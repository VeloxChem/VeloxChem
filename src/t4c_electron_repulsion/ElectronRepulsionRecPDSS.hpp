#ifndef ElectronRepulsionRecPDSS_hpp
#define ElectronRepulsionRecPDSS_hpp
...
template <class T>
auto
comp_electron_repulsion_pdss(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_pdss(15, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 15, ket_dim);
    ...
    CSimdArray<double> bf_values(4, ket_pdim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<1, 2>(spher_buffer_0_pdss, ket_spher_buffer_0_pdss, 0, 0);

        t4cfunc::store_values(buffer, spher_buffer_0_pdss, 15 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 2, 0, 0, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecPDSS_hpp */
