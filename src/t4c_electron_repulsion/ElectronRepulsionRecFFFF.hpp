#ifndef ElectronRepulsionRecFFFF_hpp
#define ElectronRepulsionRecFFFF_hpp
...
/// @brief Computes (FF|1/|r-r'||FF)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_ffff(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 1850, pbuffer, 11502, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2060, pbuffer, 11922, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2375, pbuffer, 12552, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2816, pbuffer, 13434, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3404, pbuffer, 14610, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3684, pbuffer, 14890, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4104, pbuffer, 15310, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4692, pbuffer, 15898, 784, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 21712, cbuffer, 0, 0, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 23392, cbuffer, 0, 0, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 25912, cbuffer, 0, 0, cfactors, 6, 0, 6);
            ...
            t4cfunc::ket_transform<3, 3>(skbuffer, 4600, ckbuffer, 25912, 0, 6);
            ...
            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 3, 3, 3, 3, j, ket_range, diagonal);
        }
    }
...
#endif /* ElectronRepulsionRecFFFF_hpp */
