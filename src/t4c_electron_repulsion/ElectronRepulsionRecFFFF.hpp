                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_ffff(4900, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ffff(2401, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 2401, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        for (int j = 0; j < npgtos; j++)
        {
            ...
            t2cfunc::reduce(cart_buffer_0_sfsi, prim_buffer_0_sfsi, ket_dim, npgtos);
            ...
            t2cfunc::reduce(cart_buffer_0_sisf, prim_buffer_0_sisf, ket_dim, npgtos);
        ...
        t4cfunc::store_values(buffer, spher_buffer_0_ffff, 2401 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, 3, 3, bra_indices, ket_indices);
}

/// Computes (FF|1/|r-r'||FF)  integrals for two GTOs pair blocks.
/// - Parameter distributor: the pointer to Fock matrix/matrices distributor.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 3, 3, 3, 3, bra_indices, ket_indices);
}
