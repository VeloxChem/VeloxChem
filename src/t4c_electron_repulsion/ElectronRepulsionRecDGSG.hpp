#ifndef ElectronRepulsionRecDGSG_hpp
#define ElectronRepulsionRecDGSG_hpp
...
/// Computes (DG|1/|r-r'||SG)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_dgsg(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_dgsg(405, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<2, 4>(spher_buffer_0_dgsg, ket_spher_buffer_0_dgsg, 0, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_dgsg, 405 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 2, 4, 0, 4, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecDGSG_hpp */
