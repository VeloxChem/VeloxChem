#ifndef ElectronRepulsionRecSGDD_hpp
#define ElectronRepulsionRecSGDD_hpp
...
/// Computes (SG|1/|r-r'||DD)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_sgdd(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_sgdd(225, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<0, 4>(spher_buffer_0_sgdd, ket_spher_buffer_0_sgdd, 2, 2);

        t4cfunc::store_values(buffer, spher_buffer_0_sgdd, 225 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 4, 2, 2, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSGDD_hpp */
