#ifndef ElectronRepulsionRecFFSP_hpp
#define ElectronRepulsionRecFFSP_hpp
...
/// @brief Computes (FF|1/|r-r'||SP)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_ffsp(T&                               distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 75, pbuffer, 548, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 138, pbuffer, 674, 84, ket_width, ket_npgtos);
            }
            ...
            t4cfunc::ket_transform<0, 1>(skbuffer, 138, cbuffer, 138, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 222, 0, 30, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 312, 30, 75, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 447, 75, 138, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 636, 222, 312, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 816, 312, 447, r_ab, 0, 1);

            erirec::comp_bra_hrr_electron_repulsion_ffxx(skbuffer, 1086, 636, 816, r_ab, 0, 1);

            t4cfunc::bra_transform<3, 3>(sbuffer, 0, skbuffer, 1086, 0, 1);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 3, 3, 0, 1, j, ket_range, bra_eq_ket);
        }
    }
}
...
#endif /* ElectronRepulsionRecFFSP_hpp */
