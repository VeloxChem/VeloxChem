#ifndef ElectronRepulsionRecPGDD_hpp
#define ElectronRepulsionRecPGDD_hpp
...
/// Computes (PG|1/|r-r'||DD)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_pgdd(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_pgdd(675, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 675, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<1, 4>(spher_buffer_0_pgdd, ket_spher_buffer_0_pgdd, 2, 2);

        t4cfunc::store_values(buffer, spher_buffer_0_pgdd, 675 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 4, 2, 2, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecPGDD_hpp */
