...
/// Computes (DD|1/|r-r'||SF)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_sdsf(42, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_sfsf(70, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_sgsf(105, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_pdsf(126, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_pfsf(210, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_ddsf(252, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ddsf(175, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 175, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_sdsf.zero();

        ket_spher_buffer_0_sfsf.zero();

        ket_spher_buffer_0_sgsf.zero();

        ket_spher_buffer_0_pdsf.zero();

        ket_spher_buffer_0_pfsf.zero();

        ket_spher_buffer_0_ddsf.zero();

        spher_buffer_0_ddsf.zero();
        ...
        t4cfunc::ket_transform<0, 3>(ket_spher_buffer_0_sdsf, cart_buffer_0_sdsf, 0, 2);

        t4cfunc::ket_transform<0, 3>(ket_spher_buffer_0_sfsf, cart_buffer_0_sfsf, 0, 3);

        t4cfunc::ket_transform<0, 3>(ket_spher_buffer_0_sgsf, cart_buffer_0_sgsf, 0, 4);

        erirec::comp_bra_hrr_electron_repulsion_pdxx(ket_spher_buffer_0_pdsf, ket_spher_buffer_0_sdsf, ket_spher_buffer_0_sfsf, ab_x, ab_y, ab_z, 0, 3);

        erirec::comp_bra_hrr_electron_repulsion_pfxx(ket_spher_buffer_0_pfsf, ket_spher_buffer_0_sfsf, ket_spher_buffer_0_sgsf, ab_x, ab_y, ab_z, 0, 3);

        erirec::comp_bra_hrr_electron_repulsion_ddxx(ket_spher_buffer_0_ddsf, ket_spher_buffer_0_pdsf, ket_spher_buffer_0_pfsf, ab_x, ab_y, ab_z, 0, 3);

        t4cfunc::bra_transform<2, 2>(spher_buffer_0_ddsf, ket_spher_buffer_0_ddsf, 0, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_ddsf, 175 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 2, 2, 0, 3, bra_indices, ket_indices);
}
