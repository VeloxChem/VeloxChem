...
/// @brief Computes (SS|1/|r-r'||GG)  integrals for two basis function pairs blocks.
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(1269, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                erirec::comp_prim_electron_repulsion_sssl(pbuffer, 450, 294, 322, 378, 414, pfactors, 20, 23, a_exp, b_exp);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 0, cbuffer, 0, 15, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 45, cbuffer, 15, 36, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpi(ckbuffer, 108, cbuffer, 36, 64, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxpk(ckbuffer, 192, cbuffer, 64, 100, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 300, cbuffer, 0, 0, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxdh(ckbuffer, 390, cbuffer, 0, 0, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxdi(ckbuffer, 516, cbuffer, 0, 0, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxfg(ckbuffer, 684, cbuffer, 0, 0, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxfh(ckbuffer, 834, cbuffer, 0, 0, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxgg(ckbuffer, 1044, cbuffer, 0, 0, cfactors, 6, 0, 0);

            t4cfunc::ket_transform<4, 4>(skbuffer, 0, ckbuffer, 1044, 0, 0);

            t4cfunc::bra_transform<0, 0>(sbuffer, 0, skbuffer, 0, 4, 4);

            const bool diagonal = bra_eq_ket && (j >= ket_range.first) && (j < ket_range.second);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 0, 4, 4, j, ket_range, diagonal);
        }
    }
