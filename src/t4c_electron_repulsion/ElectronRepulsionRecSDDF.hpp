#ifndef ElectronRepulsionRecSDDF_hpp
#define ElectronRepulsionRecSDDF_hpp
...
/// Computes (SD|1/|r-r'||DF)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_sddf(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_sddf(175, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<0, 2>(spher_buffer_0_sddf, ket_spher_buffer_0_sddf, 2, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_sddf, 175 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 2, 2, 3, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSDDF_hpp */
