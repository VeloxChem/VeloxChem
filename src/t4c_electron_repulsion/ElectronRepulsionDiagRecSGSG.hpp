...
/// Computes (SG|1/|r-r'||SG)  integrals for GTOs pair block.
                                  ...
                                  const std::array<int, 2>& gto_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_sgsg(135, 1);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_sgsg(81, 1);
    ...
    for (auto i = gto_indices[0]; i < gto_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<0, 4>(spher_buffer_0_sgsg, ket_spher_buffer_0_sgsg, 0, 4);

        t4cfunc::update_max_values(max_values, spher_buffer_0_sgsg, i - gto_indices[0]); 
    }
