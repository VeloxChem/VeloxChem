#ifndef ElectronRepulsionRecPPSF_hpp
#define ElectronRepulsionRecPPSF_hpp
...
/// Computes (PP|1/|r-r'||SF)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_ppsf(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_spsf(21, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_sdsf(42, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_ppsf(63, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ppsf(63, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_spsf.zero();

        ket_spher_buffer_0_sdsf.zero();

        ket_spher_buffer_0_ppsf.zero();

        spher_buffer_0_ppsf.zero();
        ...
        t4cfunc::ket_transform<0, 3>(ket_spher_buffer_0_spsf, cart_buffer_0_spsf, 0, 1);

        t4cfunc::ket_transform<0, 3>(ket_spher_buffer_0_sdsf, cart_buffer_0_sdsf, 0, 2);

        erirec::comp_bra_hrr_electron_repulsion_ppxx(ket_spher_buffer_0_ppsf, ket_spher_buffer_0_spsf, ket_spher_buffer_0_sdsf, ab_x, ab_y, ab_z, 0, 3);

        t4cfunc::bra_transform<1, 1>(spher_buffer_0_ppsf, ket_spher_buffer_0_ppsf, 0, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_ppsf, 63 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 1, 0, 3, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecPPSF_hpp */
