#ifndef ElectronRepulsionRecSSPF_hpp
#define ElectronRepulsionRecSSPF_hpp
...
/// Computes (SS|1/|r-r'||PF)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_sspf(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_sspf(21, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_sspf(21, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 21, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_sspf.zero();

        spher_buffer_0_sspf.zero();
        ...
        t4cfunc::ket_transform<1, 3>(ket_spher_buffer_0_sspf, contr_buffer_0_sspf, 0, 0);

        t4cfunc::bra_transform<0, 0>(spher_buffer_0_sspf, ket_spher_buffer_0_sspf, 1, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_sspf, 21 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 0, 1, 3, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSSPF_hpp */
