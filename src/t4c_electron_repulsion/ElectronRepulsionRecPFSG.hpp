#ifndef ElectronRepulsionRecPFSG_hpp
#define ElectronRepulsionRecPFSG_hpp
...
/// @brief Computes (PF|1/|r-r'||SG)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_pfsg(T&                               distributor,
                             ...
                             const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(1576, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 1051, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 1351, 225, ket_width, ket_npgtos);
            }
            ...
            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 225, 0, 90, r_ab, 0, 4);

            t4cfunc::bra_transform<1, 3>(sbuffer, 0, skbuffer, 225, 0, 4);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 3, 0, 4, j, ket_range, bra_eq_ket);
        }
    }
}
...
#endif /* ElectronRepulsionRecPFSG_hpp */
