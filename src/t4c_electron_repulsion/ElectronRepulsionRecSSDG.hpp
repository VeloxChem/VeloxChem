...
/// Computes (SS|1/|r-r'||DG)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_ssdg(45, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ssdg(45, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_ssdg.zero();

        spher_buffer_0_ssdg.zero();
        ...
        t4cfunc::ket_transform<2, 4>(ket_spher_buffer_0_ssdg, contr_buffer_0_ssdg, 0, 0);

        t4cfunc::bra_transform<0, 0>(spher_buffer_0_ssdg, ket_spher_buffer_0_ssdg, 2, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_ssdg, 45 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 0, 2, 4, bra_indices, ket_indices);
}
