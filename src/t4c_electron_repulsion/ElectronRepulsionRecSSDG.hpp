                             ...
                             const bool                       bra_eq_ket) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 108, 0, 45, cfactors, 6, 0, 0);

            t4cfunc::ket_transform<2, 4>(skbuffer, 0, ckbuffer, 108, 0, 0);

            t4cfunc::bra_transform<0, 0>(sbuffer, 0, skbuffer, 0, 2, 4);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 0, 2, 4, j, ket_range, bra_eq_ket);
        }
    }
}
