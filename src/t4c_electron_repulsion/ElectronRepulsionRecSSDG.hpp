#ifndef ElectronRepulsionRecSSDG_hpp
#define ElectronRepulsionRecSSDG_hpp
...
/// @brief Computes (SS|1/|r-r'||DG)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_ssdg(T& distributor,
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> skbuffer(45, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 36, pbuffer, 182, 28, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 108, cbuffer, 0, 0, cfactors, 6, 0, 0);

            t4cfunc::ket_transform<2, 4>(skbuffer, 0, ckbuffer, 108, 0, 0);

            t4cfunc::bra_transform<0, 0>(sbuffer, 0, skbuffer, 0, 2, 4);

            const bool diagonal = bra_eq_ket && (j >= ket_range.first) && (j < ket_range.second);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 0, 2, 4, j, ket_range, diagonal);
        }
    }
...
#endif /* ElectronRepulsionRecSSDG_hpp */
