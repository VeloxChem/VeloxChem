...
/// Computes (DF|1/|r-r'||FF)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_pfff(1470, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_pgff(2205, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_dfff(2940, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_dfff(1715, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 1715, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<2, 3>(spher_buffer_0_dfff, ket_spher_buffer_0_dfff, 3, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_dfff, 1715 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 2, 3, 3, 3, bra_indices, ket_indices);
}
