...
/// Computes (FG|1/|r-r'||SG)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_fgsg(567, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<3, 4>(spher_buffer_0_fgsg, ket_spher_buffer_0_fgsg, 0, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_fgsg, 567 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 3, 4, 0, 4, bra_indices, ket_indices);
}
