...
/// Computes (DG|1/|r-r'||PF)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_dgpf(945, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<2, 4>(spher_buffer_0_dgpf, ket_spher_buffer_0_dgpf, 1, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_dgpf, 945 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 2, 4, 1, 3, bra_indices, ket_indices);
}
