#ifndef ElectronRepulsionRecDGDF_hpp
#define ElectronRepulsionRecDGDF_hpp
...
/// @brief Computes (DG|1/|r-r'||DF)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_dgdf(T& distributor,
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(8640, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(9170, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 5330, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 5780, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 375, pbuffer, 6455, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 690, pbuffer, 7526, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 900, pbuffer, 7946, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1215, pbuffer, 8576, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1656, pbuffer, 9458, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1936, pbuffer, 9738, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2356, pbuffer, 10158, 588, ket_width, ket_npgtos);
            ...
            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 2240, 0, 525, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 3815, 525, 1260, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 6020, 2240, 3815, r_ab, 2, 3);

            t4cfunc::bra_transform<2, 4>(sbuffer, 0, skbuffer, 6020, 2, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 2, 4, 2, 3, j, ket_range, bra_eq_ket);
        }
    }
...
#endif /* ElectronRepulsionRecDGDF_hpp */
