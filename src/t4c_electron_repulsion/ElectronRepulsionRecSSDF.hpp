#ifndef ElectronRepulsionRecSSDF_hpp
#define ElectronRepulsionRecSSDF_hpp
...
/// Computes (SS|1/|r-r'||DF)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_ssdf(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_ssdf(35, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ssdf(35, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_ssdf.zero();

        spher_buffer_0_ssdf.zero();
        ...
        t4cfunc::ket_transform<2, 3>(ket_spher_buffer_0_ssdf, contr_buffer_0_ssdf, 0, 0);

        t4cfunc::bra_transform<0, 0>(spher_buffer_0_ssdf, ket_spher_buffer_0_ssdf, 2, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_ssdf, 35 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 0, 2, 3, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSSDF_hpp */
