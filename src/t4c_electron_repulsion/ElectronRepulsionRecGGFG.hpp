    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                if (use_rs)
                {
                    t4cfunc::comp_boys_args(bf_data, 16, pfactors, 13, a_exp, b_exp, omega);

                    bf_table.compute(bf_data, 0, 16, pfactors, a_exp, b_exp, omega);
                }
                else
                {
            ...
            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 4, 4, 3, 4, j, ket_range, bra_eq_ket);
        }
    }
}
