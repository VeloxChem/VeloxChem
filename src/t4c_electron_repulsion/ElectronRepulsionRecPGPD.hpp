#ifndef ElectronRepulsionRecPGPD_hpp
#define ElectronRepulsionRecPGPD_hpp
...
/// @brief Computes (PG|1/|r-r'||PD)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_pgpd(T&                               distributor,
                             ...
                             const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(648, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 1465, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 90, pbuffer, 1645, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 240, pbuffer, 1945, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 366, pbuffer, 2071, 210, ket_width, ket_npgtos);
            }
            ...
            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 540, 0, 225, r_ab, 1, 2);

            t4cfunc::bra_transform<1, 4>(sbuffer, 0, skbuffer, 540, 1, 2);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 4, 1, 2, j, ket_range, bra_eq_ket);
        }
    }
}
...
#endif /* ElectronRepulsionRecPGPD_hpp */
