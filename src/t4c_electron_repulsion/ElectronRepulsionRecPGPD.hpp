...
/// Computes (PG|1/|r-r'||PD)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_pgpd(405, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<1, 4>(spher_buffer_0_pgpd, ket_spher_buffer_0_pgpd, 1, 2);

        t4cfunc::store_values(buffer, spher_buffer_0_pgpd, 405 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 4, 1, 2, bra_indices, ket_indices);
}
