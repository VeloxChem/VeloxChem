#ifndef ElectronRepulsionRecDDDF_hpp
#define ElectronRepulsionRecDDDF_hpp
...
/// @brief Computes (DD|1/|r-r'||DF)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_dddf(T& distributor,
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(4185, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(4025, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 1067, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 60, pbuffer, 1247, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 1517, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 276, pbuffer, 1955, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 376, pbuffer, 2155, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 526, pbuffer, 2455, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 736, pbuffer, 2875, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 886, pbuffer, 3025, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1111, pbuffer, 3250, 315, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 450, cbuffer, 0, 0, cfactors, 6, 0, 2);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 1560, cbuffer, 0, 0, cfactors, 6, 0, 3);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 3285, cbuffer, 0, 0, cfactors, 6, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 0, ckbuffer, 450, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 360, ckbuffer, 1560, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 960, ckbuffer, 3285, 0, 4);
            ...
            t4cfunc::bra_transform<2, 2>(sbuffer, 0, skbuffer, 2765, 2, 3);

            const bool diagonal = bra_eq_ket && (j >= ket_range.first) && (j < ket_range.second);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 2, 2, 2, 3, j, ket_range, diagonal);
        }
    }
...
#endif /* ElectronRepulsionRecDDDF_hpp */
