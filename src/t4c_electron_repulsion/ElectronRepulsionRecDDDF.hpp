...
/// Computes (DD|1/|r-r'||DF)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_sddf(210, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_sfdf(350, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_sgdf(525, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_pddf(630, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_pfdf(1050, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_dddf(1260, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_dddf(875, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 875, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_sddf.zero();

        ket_spher_buffer_0_sfdf.zero();

        ket_spher_buffer_0_sgdf.zero();

        ket_spher_buffer_0_pddf.zero();

        ket_spher_buffer_0_pfdf.zero();

        ket_spher_buffer_0_dddf.zero();

        spher_buffer_0_dddf.zero();
        ...
        t4cfunc::ket_transform<2, 3>(ket_spher_buffer_0_sddf, contr_buffer_0_sddf, 0, 2);
        ...
        erirec::comp_bra_hrr_electron_repulsion_pdxx(ket_spher_buffer_0_pddf, ket_spher_buffer_0_sddf, ket_spher_buffer_0_sfdf, ab_x, ab_y, ab_z, 2, 3);
        ...
        erirec::comp_bra_hrr_electron_repulsion_ddxx(ket_spher_buffer_0_dddf, ket_spher_buffer_0_pddf, ket_spher_buffer_0_pfdf, ab_x, ab_y, ab_z, 2, 3);

        t4cfunc::bra_transform<2, 2>(spher_buffer_0_dddf, ket_spher_buffer_0_dddf, 2, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_dddf, 875 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 2, 2, 2, 3, bra_indices, ket_indices);
}
