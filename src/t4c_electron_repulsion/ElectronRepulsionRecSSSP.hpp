#ifndef ElectronRepulsionRecSSSP_hpp
#define ElectronRepulsionRecSSSP_hpp
...
template <class T>
auto
comp_electron_repulsion_sssp(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_sssp(3, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_sssp(3, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_sssp.zero();

        spher_buffer_0_sssp.zero();
        ...
        t4cfunc::ket_transform<0, 1>(ket_spher_buffer_0_sssp, cart_buffer_0_sssp, 0, 0);

        t4cfunc::bra_transform<0, 0>(spher_buffer_0_sssp, ket_spher_buffer_0_sssp, 0, 1);

        t4cfunc::store_values(buffer, spher_buffer_0_sssp, 3 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 0, 0, 1, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSSSP_hpp */
