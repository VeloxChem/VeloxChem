#ifndef ElectronRepulsionRecSFSS_hpp
#define ElectronRepulsionRecSFSS_hpp
...
/// Computes (SF|1/|r-r'||SS)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_sfss(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_sfss(7, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 7, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<0, 3>(spher_buffer_0_sfss, ket_spher_buffer_0_sfss, 0, 0);

        t4cfunc::store_values(buffer, spher_buffer_0_sfss, 7 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 3, 0, 0, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSFSS_hpp */
