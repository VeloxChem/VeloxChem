...
/// Computes (PG|1/|r-r'||FG)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_pgfg(1701, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<1, 4>(spher_buffer_0_pgfg, ket_spher_buffer_0_pgfg, 3, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_pgfg, 1701 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 4, 3, 4, bra_indices, ket_indices);
}
