                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_pfpf(441, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::store_values(buffer, spher_buffer_0_pfpf, 441 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, 1, 3, bra_indices, ket_indices);
}

/// Computes (PF|1/|r-r'||PF)  integrals for two GTOs pair blocks.
/// - Parameter distributor: the pointer to Fock matrix/matrices distributor.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 3, 1, 3, bra_indices, ket_indices);
}
