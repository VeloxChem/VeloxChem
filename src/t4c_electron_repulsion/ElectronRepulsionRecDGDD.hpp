...
/// @brief Computes (DG|1/|r-r'||DD)  integrals for two basis function pairs blocks.
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(5376, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(6550, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 3370, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 90, pbuffer, 3640, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 240, pbuffer, 4090, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 465, pbuffer, 4828, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 591, pbuffer, 5080, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 801, pbuffer, 5500, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1116, pbuffer, 6130, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1284, pbuffer, 6298, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1564, pbuffer, 6578, 420, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 720, cbuffer, 0, 0, cfactors, 6, 0, 4);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 2268, cbuffer, 0, 0, cfactors, 6, 0, 5);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 4368, cbuffer, 0, 0, cfactors, 6, 0, 6);

            t4cfunc::ket_transform<2, 2>(skbuffer, 0, ckbuffer, 720, 0, 4);

            t4cfunc::ket_transform<2, 2>(skbuffer, 540, ckbuffer, 2268, 0, 5);

            t4cfunc::ket_transform<2, 2>(skbuffer, 1296, ckbuffer, 4368, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 1600, 0, 375, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 2725, 375, 900, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 4300, 1600, 2725, r_ab, 2, 2);

            t4cfunc::bra_transform<2, 4>(sbuffer, 0, skbuffer, 4300, 2, 2);

            const bool diagonal = bra_eq_ket && (j >= ket_range.first) && (j < ket_range.second);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 2, 4, 2, 2, j, ket_range, diagonal);
        }
    }
