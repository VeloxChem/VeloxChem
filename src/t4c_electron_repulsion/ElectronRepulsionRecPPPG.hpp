...
/// Computes (PP|1/|r-r'||PG)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_sppg(81, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_sdpg(162, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_pppg(243, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_pppg(243, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 243, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_sppg.zero();

        ket_spher_buffer_0_sdpg.zero();

        ket_spher_buffer_0_pppg.zero();

        spher_buffer_0_pppg.zero();
        ...
        t4cfunc::ket_transform<1, 4>(ket_spher_buffer_0_sppg, contr_buffer_0_sppg, 0, 1);

        t4cfunc::ket_transform<1, 4>(ket_spher_buffer_0_sdpg, contr_buffer_0_sdpg, 0, 2);

        erirec::comp_bra_hrr_electron_repulsion_ppxx(ket_spher_buffer_0_pppg, ket_spher_buffer_0_sppg, ket_spher_buffer_0_sdpg, ab_x, ab_y, ab_z, 1, 4);

        t4cfunc::bra_transform<1, 1>(spher_buffer_0_pppg, ket_spher_buffer_0_pppg, 1, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_pppg, 243 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 1, 1, 4, bra_indices, ket_indices);
}
