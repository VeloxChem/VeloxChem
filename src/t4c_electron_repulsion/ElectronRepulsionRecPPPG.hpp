#ifndef ElectronRepulsionRecPPPG_hpp
#define ElectronRepulsionRecPPPG_hpp
...
/// @brief Computes (PP|1/|r-r'||PG)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_pppg(T&                               distributor,
                             ...
                             const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(700, ket_npgtos);
    ...
    CSimdArray<double> skbuffer(486, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                erirec::comp_prim_electron_repulsion_spsf(pbuffer, 238, 41, 75, 85, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsg(pbuffer, 268, 75, 115, 130, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsg(pbuffer, 313, 85, 130, 145, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsh(pbuffer, 358, 130, 175, 196, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsh(pbuffer, 421, 145, 196, 217, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_sdsg(pbuffer, 484, 115, 130, 238, 268, 313, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_sdsh(pbuffer, 574, 175, 196, 313, 358, 421, pfactors, 26, r_pb, a_exp, b_exp);

                t2cfunc::reduce(cbuffer, 0, pbuffer, 268, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 45, pbuffer, 358, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 108, pbuffer, 484, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 198, pbuffer, 574, 126, ket_width, ket_npgtos);
            }

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 0, cbuffer, 0, 45, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 135, cbuffer, 108, 198, cfactors, 6, 0, 2);

            t4cfunc::ket_transform<1, 4>(skbuffer, 0, ckbuffer, 0, 0, 1);

            t4cfunc::ket_transform<1, 4>(skbuffer, 81, ckbuffer, 135, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 243, 0, 81, r_ab, 1, 4);

            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 243, 1, 4);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 1, 1, 4, j, ket_range, bra_eq_ket);
        }
    }
}
...
#endif /* ElectronRepulsionRecPPPG_hpp */
