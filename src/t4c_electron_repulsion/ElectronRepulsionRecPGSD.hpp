#ifndef ElectronRepulsionRecPGSD_hpp
#define ElectronRepulsionRecPGSD_hpp
...
/// @brief Computes (PG|1/|r-r'||SD)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_pgsd(T&                               distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 705, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 90, pbuffer, 885, 126, ket_width, ket_npgtos);
            }
            ...
            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 180, 0, 75, r_ab, 0, 2);

            t4cfunc::bra_transform<1, 4>(sbuffer, 0, skbuffer, 180, 0, 2);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 4, 0, 2, j, ket_range, bra_eq_ket);
        }
    }
}
...
#endif /* ElectronRepulsionRecPGSD_hpp */
