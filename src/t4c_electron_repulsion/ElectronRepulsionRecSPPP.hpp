#ifndef ElectronRepulsionRecSPPP_hpp
#define ElectronRepulsionRecSPPP_hpp
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_sppp(27, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<0, 1>(spher_buffer_0_sppp, ket_spher_buffer_0_sppp, 1, 1);

        t4cfunc::store_values(buffer, spher_buffer_0_sppp, 27 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 1, 1, 1, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSPPP_hpp */
