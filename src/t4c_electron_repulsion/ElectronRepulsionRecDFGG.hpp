...
/// Computes (DF|1/|r-r'||GG)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_dfgg(4860, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_dfgg(2835, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_dfgg.zero();

        spher_buffer_0_dfgg.zero();
        ...
        erirec::comp_bra_hrr_electron_repulsion_dfxx(ket_spher_buffer_0_dfgg, ket_spher_buffer_0_pfgg, ket_spher_buffer_0_pggg, ab_x, ab_y, ab_z, 4, 4);

        t4cfunc::bra_transform<2, 3>(spher_buffer_0_dfgg, ket_spher_buffer_0_dfgg, 4, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_dfgg, 2835 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 2, 3, 4, 4, bra_indices, ket_indices);
}
