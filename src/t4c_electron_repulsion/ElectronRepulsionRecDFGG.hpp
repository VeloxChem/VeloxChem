#ifndef ElectronRepulsionRecDFGG_hpp
#define ElectronRepulsionRecDFGG_hpp
...
/// @brief Computes (DF|1/|r-r'||GG)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_dfgg(T& distributor,
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(58374, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(14661, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 7700, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 8150, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 360, pbuffer, 8780, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 640, pbuffer, 9620, 360, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1000, pbuffer, 10700, 450, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1450, pbuffer, 12200, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1675, pbuffer, 12650, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1990, pbuffer, 13280, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2410, pbuffer, 14120, 540, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2950, pbuffer, 15200, 675, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3625, pbuffer, 16550, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3940, pbuffer, 16865, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4381, pbuffer, 17306, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4969, pbuffer, 17894, 756, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5725, pbuffer, 18650, 945, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 3000, cbuffer, 0, 0, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdh(ckbuffer, 3900, cbuffer, 0, 0, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdi(ckbuffer, 5160, cbuffer, 0, 0, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxfg(ckbuffer, 6840, cbuffer, 0, 0, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxfh(ckbuffer, 8340, cbuffer, 0, 0, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxgg(ckbuffer, 10440, cbuffer, 0, 0, cfactors, 6, 0, 3);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 17190, cbuffer, 0, 0, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdh(ckbuffer, 18540, cbuffer, 0, 0, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxdi(ckbuffer, 20430, cbuffer, 0, 0, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxfg(ckbuffer, 22950, cbuffer, 0, 0, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxfh(ckbuffer, 25200, cbuffer, 0, 0, cfactors, 6, 0, 4);

            erirec::comp_ket_hrr_electron_repulsion_xxgg(ckbuffer, 28350, cbuffer, 0, 0, cfactors, 6, 0, 4);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 38025, cbuffer, 0, 0, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdh(ckbuffer, 39915, cbuffer, 0, 0, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxdi(ckbuffer, 42561, cbuffer, 0, 0, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxfg(ckbuffer, 46089, cbuffer, 0, 0, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxfh(ckbuffer, 49239, cbuffer, 0, 0, cfactors, 6, 0, 5);

            erirec::comp_ket_hrr_electron_repulsion_xxgg(ckbuffer, 53649, cbuffer, 0, 0, cfactors, 6, 0, 5);

            t4cfunc::ket_transform<4, 4>(skbuffer, 0, ckbuffer, 10440, 0, 3);

            t4cfunc::ket_transform<4, 4>(skbuffer, 2250, ckbuffer, 28350, 0, 4);

            t4cfunc::ket_transform<4, 4>(skbuffer, 5625, ckbuffer, 53649, 0, 5);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 3726, 0, 810, r_ab, 4, 4);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 6156, 810, 2025, r_ab, 4, 4);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 9801, 3726, 6156, r_ab, 4, 4);

            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 9801, 4, 4);

            const bool diagonal = bra_eq_ket && (j >= ket_range.first) && (j < ket_range.second);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 2, 3, 4, 4, j, ket_range, diagonal);
        }
    }
...
#endif /* ElectronRepulsionRecDFGG_hpp */
