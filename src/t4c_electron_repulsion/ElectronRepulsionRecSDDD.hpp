#ifndef ElectronRepulsionRecSDDD_hpp
#define ElectronRepulsionRecSDDD_hpp
...
template <class T>
auto
comp_electron_repulsion_sddd(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_sddd(125, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<0, 2>(spher_buffer_0_sddd, ket_spher_buffer_0_sddd, 2, 2);

        t4cfunc::store_values(buffer, spher_buffer_0_sddd, 125 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 2, 2, 2, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSDDD_hpp */
