#ifndef ElectronRepulsionRecSPPD_hpp
#define ElectronRepulsionRecSPPD_hpp
...
template <class T>
auto
comp_electron_repulsion_sppd(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_sppd(45, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_sppd(45, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_sppd.zero();

        spher_buffer_0_sppd.zero();
        ...
        t4cfunc::ket_transform<1, 2>(ket_spher_buffer_0_sppd, contr_buffer_0_sppd, 0, 1);

        t4cfunc::bra_transform<0, 1>(spher_buffer_0_sppd, ket_spher_buffer_0_sppd, 1, 2);

        t4cfunc::store_values(buffer, spher_buffer_0_sppd, 45 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 1, 1, 2, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSPPD_hpp */
