...
/// @brief Computes (PD|1/|r-r'||FF)  integrals for two basis function pairs blocks.
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(6208, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(1666, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 150, pbuffer, 1509, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 276, pbuffer, 1761, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 444, pbuffer, 2097, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 544, pbuffer, 2197, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 694, pbuffer, 2347, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 904, pbuffer, 2557, 280, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 828, cbuffer, 0, 0, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 1188, cbuffer, 0, 0, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 1728, cbuffer, 0, 0, cfactors, 6, 0, 2);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 3708, cbuffer, 0, 0, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 4308, cbuffer, 0, 0, cfactors, 6, 0, 3);

            erirec::comp_ket_hrr_electron_repulsion_xxff(ckbuffer, 5208, cbuffer, 0, 0, cfactors, 6, 0, 3);

            t4cfunc::ket_transform<3, 3>(skbuffer, 0, ckbuffer, 1728, 0, 2);

            t4cfunc::ket_transform<3, 3>(skbuffer, 600, ckbuffer, 5208, 0, 3);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 784, 0, 294, r_ab, 3, 3);

            t4cfunc::bra_transform<1, 2>(sbuffer, 0, skbuffer, 784, 3, 3);

            const bool diagonal = bra_eq_ket && (j >= ket_range.first) && (j < ket_range.second);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 2, 3, 3, j, ket_range, diagonal);
        }
    }
