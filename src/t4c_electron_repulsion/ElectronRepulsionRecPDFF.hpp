#ifndef ElectronRepulsionRecPDFF_hpp
#define ElectronRepulsionRecPDFF_hpp
...
/// Computes (PD|1/|r-r'||FF)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_pdff(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> contr_buffer_0_sfff(1000, ket_dim);

    // allocate aligned half transformed integrals

    CSimdArray<double> ket_spher_buffer_0_sdff(294, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_sfff(490, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_pdff(882, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_pdff(735, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 735, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_pdff.zero();

        spher_buffer_0_pdff.zero();
        ...
        erirec::comp_bra_hrr_electron_repulsion_pdxx(ket_spher_buffer_0_pdff, ket_spher_buffer_0_sdff, ket_spher_buffer_0_sfff, ab_x, ab_y, ab_z, 3, 3);

        t4cfunc::bra_transform<1, 2>(spher_buffer_0_pdff, ket_spher_buffer_0_pdff, 3, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_pdff, 735 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 2, 3, 3, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecPDFF_hpp */
