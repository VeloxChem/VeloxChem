#ifndef ElectronRepulsionRecSFSG_hpp
#define ElectronRepulsionRecSFSG_hpp
...
/// Computes (SF|1/|r-r'||SG)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_sfsg(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_sfsg(63, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<0, 3>(spher_buffer_0_sfsg, ket_spher_buffer_0_sfsg, 0, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_sfsg, 63 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 3, 0, 4, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSFSG_hpp */
