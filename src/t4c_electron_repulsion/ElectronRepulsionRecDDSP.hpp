...
/// @brief Computes (DD|1/|r-r'||SP)  integrals for two basis function pairs blocks.
                             ...
                             const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> cbuffer(93, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(345, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 78, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 18, pbuffer, 142, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 48, pbuffer, 202, 45, ket_width, ket_npgtos);
            }
            ...
            t4cfunc::bra_transform<2, 2>(sbuffer, 0, skbuffer, 237, 0, 1);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 2, 2, 0, 1, j, ket_range, bra_eq_ket);
        }
    }
}
