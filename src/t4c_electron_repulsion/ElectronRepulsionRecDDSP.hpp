#ifndef ElectronRepulsionRecDDSP_hpp
#define ElectronRepulsionRecDDSP_hpp
...
/// Computes (DD|1/|r-r'||SP)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_ddsp(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_sdsp(18, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_sfsp(30, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_sgsp(45, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_pdsp(54, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_pfsp(90, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_ddsp(108, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ddsp(75, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 75, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_sdsp.zero();

        ket_spher_buffer_0_sfsp.zero();

        ket_spher_buffer_0_sgsp.zero();

        ket_spher_buffer_0_pdsp.zero();

        ket_spher_buffer_0_pfsp.zero();

        ket_spher_buffer_0_ddsp.zero();

        spher_buffer_0_ddsp.zero();
        ...
        t4cfunc::ket_transform<0, 1>(ket_spher_buffer_0_sdsp, cart_buffer_0_sdsp, 0, 2);
        ...
        erirec::comp_bra_hrr_electron_repulsion_pdxx(ket_spher_buffer_0_pdsp, ket_spher_buffer_0_sdsp, ket_spher_buffer_0_sfsp, ab_x, ab_y, ab_z, 0, 1);
        ...
        erirec::comp_bra_hrr_electron_repulsion_ddxx(ket_spher_buffer_0_ddsp, ket_spher_buffer_0_pdsp, ket_spher_buffer_0_pfsp, ab_x, ab_y, ab_z, 0, 1);

        t4cfunc::bra_transform<2, 2>(spher_buffer_0_ddsp, ket_spher_buffer_0_ddsp, 0, 1);

        t4cfunc::store_values(buffer, spher_buffer_0_ddsp, 75 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 2, 2, 0, 1, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecDDSP_hpp */
