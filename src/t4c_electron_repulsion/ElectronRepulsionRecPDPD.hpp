#ifndef ElectronRepulsionRecPDPD_hpp
#define ElectronRepulsionRecPDPD_hpp
...
template <class T>
auto
comp_electron_repulsion_pdpd(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_pdpd(225, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        for (int j = 0; j < npgtos; j++)
        {
            ...
            t2cfunc::reduce(cart_buffer_0_sdsf, prim_buffer_0_sdsf, ket_dim, npgtos);

            t2cfunc::reduce(cart_buffer_0_sfsd, prim_buffer_0_sfsd, ket_dim, npgtos);
        ...
        t4cfunc::store_values(buffer, spher_buffer_0_pdpd, 225 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, 1, 2, bra_indices, ket_indices);
}

/// Computes (PD|1/|r-r'||PD)  integrals for two GTOs pair blocks.
/// - Parameter distributor: the pointer to Fock matrix/matrices distributor.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 2, 1, 2, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecPDPD_hpp */
