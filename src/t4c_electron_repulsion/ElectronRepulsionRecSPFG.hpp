#ifndef ElectronRepulsionRecSPFG_hpp
#define ElectronRepulsionRecSPFG_hpp
...
/// @brief Computes (SP|1/|r-r'||FG)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_spfg(T&                               distributor,
                             ...
                             const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(750, ket_npgtos);
    ...
    CSimdArray<double> ckbuffer(1674, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                erirec::comp_prim_electron_repulsion_spsh(pbuffer, 495, 150, 210, 231, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsi(pbuffer, 558, 231, 294, 322, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsk(pbuffer, 642, 322, 378, 414, pfactors, 26, r_pb, a_exp, b_exp);

                t2cfunc::reduce(cbuffer, 0, pbuffer, 450, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 45, pbuffer, 495, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 108, pbuffer, 558, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 192, pbuffer, 642, 108, ket_width, ket_npgtos);
            }
            ...
            t4cfunc::bra_transform<0, 1>(sbuffer, 0, skbuffer, 0, 3, 4);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 1, 3, 4, j, ket_range, bra_eq_ket);
        }
    }
}
...
#endif /* ElectronRepulsionRecSPFG_hpp */
