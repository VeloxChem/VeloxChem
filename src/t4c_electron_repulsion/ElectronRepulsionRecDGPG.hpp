#ifndef ElectronRepulsionRecDGPG_hpp
#define ElectronRepulsionRecDGPG_hpp
...
/// Computes (DG|1/|r-r'||PG)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_dgpg(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_dgpg(1215, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 1215, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<2, 4>(spher_buffer_0_dgpg, ket_spher_buffer_0_dgpg, 1, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_dgpg, 1215 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 2, 4, 1, 4, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecDGPG_hpp */
