#ifndef ElectronRepulsionRecSGSF_hpp
#define ElectronRepulsionRecSGSF_hpp
...
/// Computes (SG|1/|r-r'||SF)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_sgsf(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_sgsf(63, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<0, 4>(spher_buffer_0_sgsf, ket_spher_buffer_0_sgsf, 0, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_sgsf, 63 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 4, 0, 3, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSGSF_hpp */
