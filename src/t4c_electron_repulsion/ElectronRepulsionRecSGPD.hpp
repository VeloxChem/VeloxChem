#ifndef ElectronRepulsionRecSGPD_hpp
#define ElectronRepulsionRecSGPD_hpp
...
/// Computes (SG|1/|r-r'||PD)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_sgpd(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_sgpd(135, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<0, 4>(spher_buffer_0_sgpd, ket_spher_buffer_0_sgpd, 1, 2);

        t4cfunc::store_values(buffer, spher_buffer_0_sgpd, 135 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 4, 1, 2, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSGPD_hpp */
