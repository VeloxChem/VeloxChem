#ifndef ElectronRepulsionRecPPDG_hpp
#define ElectronRepulsionRecPPDG_hpp
...
/// @brief Computes (PP|1/|r-r'||DG)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_ppdg(T& distributor,
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(1176, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                erirec::comp_prim_electron_repulsion_spsf(pbuffer, 378, 45, 85, 95, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsg(pbuffer, 408, 85, 135, 150, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsg(pbuffer, 453, 95, 150, 165, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsh(pbuffer, 498, 150, 210, 231, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsh(pbuffer, 561, 165, 231, 252, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsi(pbuffer, 624, 231, 294, 322, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_spsi(pbuffer, 708, 252, 322, 350, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_sdsg(pbuffer, 792, 135, 150, 378, 408, 453, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_sdsh(pbuffer, 882, 210, 231, 453, 498, 561, pfactors, 26, r_pb, a_exp, b_exp);

                erirec::comp_prim_electron_repulsion_sdsi(pbuffer, 1008, 294, 322, 561, 624, 708, pfactors, 26, r_pb, a_exp, b_exp);

                t2cfunc::reduce(cbuffer, 0, pbuffer, 408, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 45, pbuffer, 498, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 108, pbuffer, 624, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 192, pbuffer, 792, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 282, pbuffer, 882, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 408, pbuffer, 1008, 168, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 324, cbuffer, 0, 0, cfactors, 6, 0, 1);

            erirec::comp_ket_hrr_electron_repulsion_xxpg(ckbuffer, 594, cbuffer, 192, 282, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxph(ckbuffer, 864, cbuffer, 282, 408, cfactors, 6, 0, 2);

            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 1242, cbuffer, 0, 0, cfactors, 6, 0, 2);

            t4cfunc::ket_transform<2, 4>(skbuffer, 0, ckbuffer, 324, 0, 1);

            t4cfunc::ket_transform<2, 4>(skbuffer, 270, ckbuffer, 1242, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 405, 0, 135, r_ab, 2, 4);

            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 405, 2, 4);

            const bool diagonal = bra_eq_ket && (j >= ket_range.first) && (j < ket_range.second);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 1, 2, 4, j, ket_range, diagonal);
        }
    }
...
#endif /* ElectronRepulsionRecPPDG_hpp */
