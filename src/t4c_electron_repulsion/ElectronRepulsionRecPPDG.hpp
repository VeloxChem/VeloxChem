...
/// Computes (PP|1/|r-r'||DG)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_spdg(135, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_sddg(270, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_ppdg(405, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ppdg(405, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_spdg.zero();

        ket_spher_buffer_0_sddg.zero();

        ket_spher_buffer_0_ppdg.zero();

        spher_buffer_0_ppdg.zero();
        ...
        t4cfunc::ket_transform<2, 4>(ket_spher_buffer_0_spdg, contr_buffer_0_spdg, 0, 1);

        t4cfunc::ket_transform<2, 4>(ket_spher_buffer_0_sddg, contr_buffer_0_sddg, 0, 2);

        erirec::comp_bra_hrr_electron_repulsion_ppxx(ket_spher_buffer_0_ppdg, ket_spher_buffer_0_spdg, ket_spher_buffer_0_sddg, ab_x, ab_y, ab_z, 2, 4);

        t4cfunc::bra_transform<1, 1>(spher_buffer_0_ppdg, ket_spher_buffer_0_ppdg, 2, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_ppdg, 405 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 1, 2, 4, bra_indices, ket_indices);
}
