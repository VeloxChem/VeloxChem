                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_spsp(9, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::store_values(buffer, spher_buffer_0_spsp, 9 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, 0, 1, bra_indices, ket_indices);
}
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 1, 0, 1, bra_indices, ket_indices);
}
