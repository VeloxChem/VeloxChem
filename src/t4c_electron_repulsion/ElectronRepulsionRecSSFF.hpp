...
/// Computes (SS|1/|r-r'||FF)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> contr_buffer_0_sspf(30, ket_dim);

    CSimdArray<double> contr_buffer_0_sspg(45, ket_dim);

    CSimdArray<double> contr_buffer_0_ssph(63, ket_dim);

    CSimdArray<double> contr_buffer_0_ssdf(60, ket_dim);

    CSimdArray<double> contr_buffer_0_ssdg(90, ket_dim);

    CSimdArray<double> contr_buffer_0_ssff(100, ket_dim);

    // allocate aligned half transformed integrals

    CSimdArray<double> ket_spher_buffer_0_ssff(49, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ssff(49, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 49, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_ssff.zero();

        spher_buffer_0_ssff.zero();
        ...
        erirec::comp_ket_hrr_electron_repulsion_xxpf(contr_buffer_0_sspf, cart_buffer_0_sssf, cart_buffer_0_sssg, cd_x[0], cd_y[0], cd_z[0], 0, 0);
        ...
        erirec::comp_ket_hrr_electron_repulsion_xxdf(contr_buffer_0_ssdf, contr_buffer_0_sspf, contr_buffer_0_sspg, cd_x[0], cd_y[0], cd_z[0], 0, 0);
        ...
        erirec::comp_ket_hrr_electron_repulsion_xxff(contr_buffer_0_ssff, contr_buffer_0_ssdf, contr_buffer_0_ssdg, cd_x[0], cd_y[0], cd_z[0], 0, 0);

        t4cfunc::ket_transform<3, 3>(ket_spher_buffer_0_ssff, contr_buffer_0_ssff, 0, 0);

        t4cfunc::bra_transform<0, 0>(spher_buffer_0_ssff, ket_spher_buffer_0_ssff, 3, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_ssff, 49 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 0, 3, 3, bra_indices, ket_indices);
}
