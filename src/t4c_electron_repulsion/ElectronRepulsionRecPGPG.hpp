...
/// Computes (PG|1/|r-r'||PG)  integrals for GTOs pair block.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_pgpg(729, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<1, 4>(spher_buffer_0_pgpg, ket_spher_buffer_0_pgpg, 1, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_pgpg, 729 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, 1, 4, bra_indices, ket_indices);
}

/// Computes (PG|1/|r-r'||PG)  integrals for two GTOs pair blocks.
/// - Parameter distributor: the pointer to Fock matrix/matrices distributor.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 4, 1, 4, bra_indices, ket_indices);
}
