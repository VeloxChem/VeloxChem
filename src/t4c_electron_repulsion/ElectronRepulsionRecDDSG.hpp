#ifndef ElectronRepulsionRecDDSG_hpp
#define ElectronRepulsionRecDDSG_hpp
...
/// @brief Computes (DD|1/|r-r'||SG)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_ddsg(T&                               distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 681, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 90, pbuffer, 1051, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 240, pbuffer, 1351, 225, ket_width, ket_npgtos);
            }
            ...
            t4cfunc::ket_transform<0, 4>(skbuffer, 144, cbuffer, 240, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_pdxx(skbuffer, 279, 0, 54, r_ab, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 441, 54, 144, r_ab, 0, 4);

            erirec::comp_bra_hrr_electron_repulsion_ddxx(skbuffer, 711, 279, 441, r_ab, 0, 4);

            t4cfunc::bra_transform<2, 2>(sbuffer, 0, skbuffer, 711, 0, 4);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 2, 2, 0, 4, j, ket_range, bra_eq_ket);
        }
    }
}
...
#endif /* ElectronRepulsionRecDDSG_hpp */
