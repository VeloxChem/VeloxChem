#ifndef ElectronRepulsionRecDDSS_hpp
#define ElectronRepulsionRecDDSS_hpp
...
template <class T>
auto
comp_electron_repulsion_ddss(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> cart_buffer_0_sdss(6, ket_dim);

    CSimdArray<double> cart_buffer_0_sfss(10, ket_dim);

    CSimdArray<double> cart_buffer_0_sgss(15, ket_dim);

    // allocate aligned half transformed integrals

    CSimdArray<double> ket_spher_buffer_0_sdss(6, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_sfss(10, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_sgss(15, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_pdss(18, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_pfss(30, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_ddss(36, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ddss(25, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 25, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        cart_buffer_0_sdss.zero();

        cart_buffer_0_sfss.zero();

        cart_buffer_0_sgss.zero();

        ket_spher_buffer_0_sdss.zero();

        ket_spher_buffer_0_sfss.zero();

        ket_spher_buffer_0_sgss.zero();

        ket_spher_buffer_0_pdss.zero();

        ket_spher_buffer_0_pfss.zero();

        ket_spher_buffer_0_ddss.zero();

        spher_buffer_0_ddss.zero();
        ...
        for (int j = 0; j < bra_npgtos; j++)
        {
            ...
            t2cfunc::reduce(cart_buffer_0_sdss, prim_buffer_0_sdss, ket_dim, ket_npgtos);

            t2cfunc::reduce(cart_buffer_0_sfss, prim_buffer_0_sfss, ket_dim, ket_npgtos);

            t2cfunc::reduce(cart_buffer_0_sgss, prim_buffer_0_sgss, ket_dim, ket_npgtos);

        }

        t4cfunc::ket_transform<0, 0>(ket_spher_buffer_0_sdss, cart_buffer_0_sdss, 0, 2);

        t4cfunc::ket_transform<0, 0>(ket_spher_buffer_0_sfss, cart_buffer_0_sfss, 0, 3);

        t4cfunc::ket_transform<0, 0>(ket_spher_buffer_0_sgss, cart_buffer_0_sgss, 0, 4);

        erirec::comp_bra_hrr_electron_repulsion_pdxx(ket_spher_buffer_0_pdss, ket_spher_buffer_0_sdss, ket_spher_buffer_0_sfss, ab_x, ab_y, ab_z, 0, 0);

        erirec::comp_bra_hrr_electron_repulsion_pfxx(ket_spher_buffer_0_pfss, ket_spher_buffer_0_sfss, ket_spher_buffer_0_sgss, ab_x, ab_y, ab_z, 0, 0);

        erirec::comp_bra_hrr_electron_repulsion_ddxx(ket_spher_buffer_0_ddss, ket_spher_buffer_0_pdss, ket_spher_buffer_0_pfss, ab_x, ab_y, ab_z, 0, 0);

        t4cfunc::bra_transform<2, 2>(spher_buffer_0_ddss, ket_spher_buffer_0_ddss, 0, 0);

        t4cfunc::store_values(buffer, spher_buffer_0_ddss, 25 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 2, 2, 0, 0, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecDDSS_hpp */
