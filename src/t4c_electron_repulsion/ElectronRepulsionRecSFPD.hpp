#ifndef ElectronRepulsionRecSFPD_hpp
#define ElectronRepulsionRecSFPD_hpp
...
/// Computes (SF|1/|r-r'||PD)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_sfpd(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_sfpd(105, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<0, 3>(spher_buffer_0_sfpd, ket_spher_buffer_0_sfpd, 1, 2);

        t4cfunc::store_values(buffer, spher_buffer_0_sfpd, 105 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 3, 1, 2, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSFPD_hpp */
