...
/// @brief Computes (DF|1/|r-r'||DF)  integrals for two basis function pairs blocks.
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 460, pbuffer, 4075, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 610, pbuffer, 4375, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 835, pbuffer, 4825, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1150, pbuffer, 5455, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1360, pbuffer, 5665, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1675, pbuffer, 5980, 441, ket_width, ket_npgtos);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 750, cbuffer, 0, 0, cfactors, 6, 0, 3);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 2475, cbuffer, 0, 0, cfactors, 6, 0, 4);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 4950, cbuffer, 0, 0, cfactors, 6, 0, 5);

            t4cfunc::ket_transform<2, 3>(skbuffer, 0, ckbuffer, 750, 0, 3);

            t4cfunc::ket_transform<2, 3>(skbuffer, 600, ckbuffer, 2475, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 1500, ckbuffer, 4950, 0, 5);
            ...
            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 2, 3, 2, 3, j, ket_range, diagonal);
        }
    }
