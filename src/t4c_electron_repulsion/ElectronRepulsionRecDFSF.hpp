...
/// @brief Computes (DF|1/|r-r'||SF)  integrals for two basis function pairs blocks.
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 100, pbuffer, 1320, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 250, pbuffer, 1620, 210, ket_width, ket_npgtos);
            }
            ...
            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 847, 0, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 2, 3, 0, 3, j, ket_range, bra_eq_ket);
        }
    }
}
