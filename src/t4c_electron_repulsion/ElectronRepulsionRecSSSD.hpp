#ifndef ElectronRepulsionRecSSSD_hpp
#define ElectronRepulsionRecSSSD_hpp
...
template <class T>
auto
comp_electron_repulsion_sssd(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_sssd(5, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_sssd(5, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 5, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_sssd.zero();

        spher_buffer_0_sssd.zero();
        ...
        t4cfunc::ket_transform<0, 2>(ket_spher_buffer_0_sssd, cart_buffer_0_sssd, 0, 0);

        t4cfunc::bra_transform<0, 0>(spher_buffer_0_sssd, ket_spher_buffer_0_sssd, 0, 2);

        t4cfunc::store_values(buffer, spher_buffer_0_sssd, 5 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 0, 0, 2, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSSSD_hpp */
