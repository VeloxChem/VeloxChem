#ifndef ElectronRepulsionRecDDSD_hpp
#define ElectronRepulsionRecDDSD_hpp
...
template <class T>
auto
comp_electron_repulsion_ddsd(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_sdsd(30, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_sfsd(50, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_sgsd(75, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_pdsd(90, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_pfsd(150, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_ddsd(180, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ddsd(125, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 125, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_sdsd.zero();

        ket_spher_buffer_0_sfsd.zero();

        ket_spher_buffer_0_sgsd.zero();

        ket_spher_buffer_0_pdsd.zero();

        ket_spher_buffer_0_pfsd.zero();

        ket_spher_buffer_0_ddsd.zero();

        spher_buffer_0_ddsd.zero();
        ...
        t4cfunc::ket_transform<0, 2>(ket_spher_buffer_0_sdsd, cart_buffer_0_sdsd, 0, 2);

        t4cfunc::ket_transform<0, 2>(ket_spher_buffer_0_sfsd, cart_buffer_0_sfsd, 0, 3);

        t4cfunc::ket_transform<0, 2>(ket_spher_buffer_0_sgsd, cart_buffer_0_sgsd, 0, 4);

        erirec::comp_bra_hrr_electron_repulsion_pdxx(ket_spher_buffer_0_pdsd, ket_spher_buffer_0_sdsd, ket_spher_buffer_0_sfsd, ab_x, ab_y, ab_z, 0, 2);

        erirec::comp_bra_hrr_electron_repulsion_pfxx(ket_spher_buffer_0_pfsd, ket_spher_buffer_0_sfsd, ket_spher_buffer_0_sgsd, ab_x, ab_y, ab_z, 0, 2);

        erirec::comp_bra_hrr_electron_repulsion_ddxx(ket_spher_buffer_0_ddsd, ket_spher_buffer_0_pdsd, ket_spher_buffer_0_pfsd, ab_x, ab_y, ab_z, 0, 2);

        t4cfunc::bra_transform<2, 2>(spher_buffer_0_ddsd, ket_spher_buffer_0_ddsd, 0, 2);

        t4cfunc::store_values(buffer, spher_buffer_0_ddsd, 125 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 2, 2, 0, 2, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecDDSD_hpp */
