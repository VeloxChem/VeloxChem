...
/// @brief Computes (DD|1/|r-r'||SD)  integrals for two basis function pairs blocks.
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> cbuffer(186, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(575, 1);

    // allocate aligned spherical integrals

    CSimdArray<double> sbuffer(125, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 202, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 36, pbuffer, 340, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 96, pbuffer, 460, 90, ket_width, ket_npgtos);
            ...
            t4cfunc::bra_transform<2, 2>(sbuffer, 0, skbuffer, 395, 0, 2);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 2, 2, 0, 2, j, ket_range, bra_eq_ket);
        }
    }
