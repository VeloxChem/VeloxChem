#ifndef ElectronRepulsionRecSFSD_hpp
#define ElectronRepulsionRecSFSD_hpp
...
/// Computes (SF|1/|r-r'||SD)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_sfsd(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_sfsd(35, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 35, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<0, 3>(spher_buffer_0_sfsd, ket_spher_buffer_0_sfsd, 0, 2);

        t4cfunc::store_values(buffer, spher_buffer_0_sfsd, 35 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 3, 0, 2, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSFSD_hpp */
