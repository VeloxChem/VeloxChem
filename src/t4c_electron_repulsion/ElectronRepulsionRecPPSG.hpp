#ifndef ElectronRepulsionRecPPSG_hpp
#define ElectronRepulsionRecPPSG_hpp
...
/// Computes (PP|1/|r-r'||SG)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_ppsg(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_ppsg(81, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ppsg(81, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_ppsg.zero();

        spher_buffer_0_ppsg.zero();
        ...
        erirec::comp_bra_hrr_electron_repulsion_ppxx(ket_spher_buffer_0_ppsg, ket_spher_buffer_0_spsg, ket_spher_buffer_0_sdsg, ab_x, ab_y, ab_z, 0, 4);

        t4cfunc::bra_transform<1, 1>(spher_buffer_0_ppsg, ket_spher_buffer_0_ppsg, 0, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_ppsg, 81 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 1, 0, 4, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecPPSG_hpp */
