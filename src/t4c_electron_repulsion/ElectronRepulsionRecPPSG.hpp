...
/// @brief Computes (PP|1/|r-r'||SG)  integrals for two basis function pairs blocks.
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 170, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 45, pbuffer, 260, 90, ket_width, ket_npgtos);
            }

            t4cfunc::ket_transform<0, 4>(skbuffer, 0, cbuffer, 0, 0, 1);

            t4cfunc::ket_transform<0, 4>(skbuffer, 27, cbuffer, 45, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 81, 0, 27, r_ab, 0, 4);

            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 81, 0, 4);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 1, 0, 4, j, ket_range, bra_eq_ket);
        }
    }
}
