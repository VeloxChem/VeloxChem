                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_dgdg(2025, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::store_values(buffer, spher_buffer_0_dgdg, 2025 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, 2, 4, bra_indices, ket_indices);
}

/// Computes (DG|1/|r-r'||DG)  integrals for two GTOs pair blocks.
/// - Parameter distributor: the pointer to Fock matrix/matrices distributor.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 2, 4, 2, 4, bra_indices, ket_indices);
}
