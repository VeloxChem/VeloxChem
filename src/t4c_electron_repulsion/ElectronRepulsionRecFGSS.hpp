...
/// @brief Computes (FG|1/|r-r'||SS)  integrals for two basis function pairs blocks.
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 100, 0, 15, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 145, 15, 36, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_pixx(skbuffer, 208, 36, 64, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 292, 100, 145, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_dhxx(skbuffer, 382, 145, 208, r_ab, 0, 0);

            erirec::comp_bra_hrr_electron_repulsion_fgxx(skbuffer, 508, 292, 382, r_ab, 0, 0);

            t4cfunc::bra_transform<3, 4>(sbuffer, 0, skbuffer, 508, 0, 0);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 3, 4, 0, 0, j, ket_range, bra_eq_ket);
        }
    }
