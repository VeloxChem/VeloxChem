                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_spsp(9, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_sdsp(18, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_ppsp(27, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ppsp(27, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_ppsp.zero();

        spher_buffer_0_ppsp.zero();
        ...
        erirec::comp_bra_hrr_electron_repulsion_ppxx(ket_spher_buffer_0_ppsp, ket_spher_buffer_0_spsp, ket_spher_buffer_0_sdsp, ab_x, ab_y, ab_z, 0, 1);

        t4cfunc::bra_transform<1, 1>(spher_buffer_0_ppsp, ket_spher_buffer_0_ppsp, 0, 1);

        t4cfunc::store_values(buffer, spher_buffer_0_ppsp, 27 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 1, 0, 1, bra_indices, ket_indices);
}
