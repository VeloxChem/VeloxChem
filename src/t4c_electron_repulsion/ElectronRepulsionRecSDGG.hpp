#ifndef ElectronRepulsionRecSDGG_hpp
#define ElectronRepulsionRecSDGG_hpp
...
/// @brief Computes (SD|1/|r-r'||GG)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_sdgg(T& distributor,
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(7614, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 1725, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 90, pbuffer, 1815, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 216, pbuffer, 1941, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 384, pbuffer, 2109, 216, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 600, pbuffer, 2325, 270, ket_width, ket_npgtos);
            ...
            t4cfunc::bra_transform<0, 2>(sbuffer, 0, skbuffer, 0, 4, 4);

            const bool diagonal = bra_eq_ket && (j >= ket_range.first) && (j < ket_range.second);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 2, 4, 4, j, ket_range, diagonal);
        }
    }
...
#endif /* ElectronRepulsionRecSDGG_hpp */
