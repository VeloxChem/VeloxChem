...
/// Computes (SD|1/|r-r'||GG)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_sdgg(405, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<0, 2>(spher_buffer_0_sdgg, ket_spher_buffer_0_sdgg, 4, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_sdgg, 405 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 2, 4, 4, bra_indices, ket_indices);
}
