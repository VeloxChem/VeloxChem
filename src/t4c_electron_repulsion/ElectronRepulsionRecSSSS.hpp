                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> cart_buffer_0_ssss(1, ket_dim);

    // allocate aligned half transformed integrals

    CSimdArray<double> ket_spher_buffer_0_ssss(1, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ssss(1, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim, ket_dim);
    ...
    CSimdArray<double> bf_values(1, ket_pdim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        for (int j = 0; j < npgtos; j++)
        {
            ...
            t2cfunc::reduce(cart_buffer_0_ssss, prim_buffer_0_ssss, ket_dim, npgtos);
        ...
        t4cfunc::store_values(buffer, spher_buffer_0_ssss, i - bra_indices[0]);
    }

    distributor->distribute(buffer, a_indices, b_indices, 0, 0, bra_indices, ket_indices);
}
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        for (int j = 0; j < bra_npgtos; j++)
        {
            ...
            t2cfunc::reduce(cart_buffer_0_ssss, prim_buffer_0_ssss, ket_dim, ket_npgtos);
    ...
    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 0, 0, 0, bra_indices, ket_indices);
}
