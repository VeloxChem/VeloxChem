#ifndef ElectronRepulsionRecDGSD_hpp
#define ElectronRepulsionRecDGSD_hpp
...
/// @brief Computes (DG|1/|r-r'||SD)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_dgsd(T&                               distributor,
                             ...
                             const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> skbuffer(1310, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 965, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 90, pbuffer, 1298, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 216, pbuffer, 1550, 168, ket_width, ket_npgtos);
            }
            ...
            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 320, 0, 75, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 545, 75, 180, r_ab, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 860, 320, 545, r_ab, 0, 2);

            t4cfunc::bra_transform<2, 4>(sbuffer, 0, skbuffer, 860, 0, 2);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 2, 4, 0, 2, j, ket_range, bra_eq_ket);
        }
    }
}
...
#endif /* ElectronRepulsionRecDGSD_hpp */
