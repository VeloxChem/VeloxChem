#ifndef ElectronRepulsionRecPGDG_hpp
#define ElectronRepulsionRecPGDG_hpp
...
/// @brief Computes (PG|1/|r-r'||DG)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_pgdg(T&                               distributor,
                             ...
                             const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(7128, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(3645, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 5921, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 225, pbuffer, 6371, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 540, pbuffer, 7001, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 960, pbuffer, 7841, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1275, pbuffer, 8156, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1716, pbuffer, 8597, 588, ket_width, ket_npgtos);
            }
            ...
            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 1620, 0, 675, r_ab, 2, 4);

            t4cfunc::bra_transform<1, 4>(sbuffer, 0, skbuffer, 1620, 2, 4);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 4, 2, 4, j, ket_range, bra_eq_ket);
        }
    }
}
...
#endif /* ElectronRepulsionRecPGDG_hpp */
