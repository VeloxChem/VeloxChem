#ifndef ElectronRepulsionRecSDSS_hpp
#define ElectronRepulsionRecSDSS_hpp
...
template <class T>
auto
comp_electron_repulsion_sdss(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_sdss(5, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<0, 2>(spher_buffer_0_sdss, ket_spher_buffer_0_sdss, 0, 0);

        t4cfunc::store_values(buffer, spher_buffer_0_sdss, 5 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 2, 0, 0, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSDSS_hpp */
