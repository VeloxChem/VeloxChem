#ifndef ElectronRepulsionRecPPPD_hpp
#define ElectronRepulsionRecPPPD_hpp
...
template <class T>
auto
comp_electron_repulsion_pppd(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_pppd(135, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_pppd(135, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_pppd.zero();

        spher_buffer_0_pppd.zero();
        ...
        erirec::comp_bra_hrr_electron_repulsion_ppxx(ket_spher_buffer_0_pppd, ket_spher_buffer_0_sppd, ket_spher_buffer_0_sdpd, ab_x, ab_y, ab_z, 1, 2);

        t4cfunc::bra_transform<1, 1>(spher_buffer_0_pppd, ket_spher_buffer_0_pppd, 1, 2);

        t4cfunc::store_values(buffer, spher_buffer_0_pppd, 135 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 1, 1, 2, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecPPPD_hpp */
