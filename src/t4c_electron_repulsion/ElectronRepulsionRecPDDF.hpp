...
/// Computes (PD|1/|r-r'||DF)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_pddf(525, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<1, 2>(spher_buffer_0_pddf, ket_spher_buffer_0_pddf, 2, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_pddf, 525 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 2, 2, 3, bra_indices, ket_indices);
}
