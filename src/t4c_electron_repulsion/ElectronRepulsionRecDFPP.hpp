#ifndef ElectronRepulsionRecDFPP_hpp
#define ElectronRepulsionRecDFPP_hpp
...
/// Computes (DF|1/|r-r'||PP)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_dfpp(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_dfpp(315, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<2, 3>(spher_buffer_0_dfpp, ket_spher_buffer_0_dfpp, 1, 1);

        t4cfunc::store_values(buffer, spher_buffer_0_dfpp, 315 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 2, 3, 1, 1, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecDFPP_hpp */
