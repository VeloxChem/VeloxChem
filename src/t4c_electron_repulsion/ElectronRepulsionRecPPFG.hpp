...
/// Computes (PP|1/|r-r'||FG)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_spfg(189, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_sdfg(378, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_ppfg(567, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ppfg(567, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_spfg.zero();

        ket_spher_buffer_0_sdfg.zero();

        ket_spher_buffer_0_ppfg.zero();

        spher_buffer_0_ppfg.zero();
        ...
        t4cfunc::ket_transform<3, 4>(ket_spher_buffer_0_spfg, contr_buffer_0_spfg, 0, 1);

        t4cfunc::ket_transform<3, 4>(ket_spher_buffer_0_sdfg, contr_buffer_0_sdfg, 0, 2);

        erirec::comp_bra_hrr_electron_repulsion_ppxx(ket_spher_buffer_0_ppfg, ket_spher_buffer_0_spfg, ket_spher_buffer_0_sdfg, ab_x, ab_y, ab_z, 3, 4);

        t4cfunc::bra_transform<1, 1>(spher_buffer_0_ppfg, ket_spher_buffer_0_ppfg, 3, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_ppfg, 567 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 1, 3, 4, bra_indices, ket_indices);
}
