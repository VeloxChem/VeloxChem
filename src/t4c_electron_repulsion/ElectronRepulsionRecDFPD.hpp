...
/// @brief Computes (DF|1/|r-r'||PD)  integrals for two basis function pairs blocks.
                             ...
                             const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(828, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(2715, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 160, pbuffer, 1465, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 250, pbuffer, 1645, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 400, pbuffer, 1945, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 526, pbuffer, 2071, 210, ket_width, ket_npgtos);
            }
            ...
            t4cfunc::bra_transform<2, 3>(sbuffer, 0, skbuffer, 1815, 1, 2);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 2, 3, 1, 2, j, ket_range, bra_eq_ket);
        }
    }
}
