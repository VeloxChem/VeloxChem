#ifndef ElectronRepulsionRecDFPD_hpp
#define ElectronRepulsionRecDFPD_hpp
...
/// Computes (DF|1/|r-r'||PD)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_dfpd(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> contr_buffer_0_sfpd(180, ket_dim);
    ...
    CSimdArray<double> ket_spher_buffer_0_sfpd(150, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_sgpd(225, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_shpd(315, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_pfpd(450, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_pgpd(675, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_dfpd(900, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_dfpd(525, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 525, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_pfpd.zero();

        ket_spher_buffer_0_pgpd.zero();

        ket_spher_buffer_0_dfpd.zero();

        spher_buffer_0_dfpd.zero();
        ...
        erirec::comp_bra_hrr_electron_repulsion_pfxx(ket_spher_buffer_0_pfpd, ket_spher_buffer_0_sfpd, ket_spher_buffer_0_sgpd, ab_x, ab_y, ab_z, 1, 2);
        ...
        erirec::comp_bra_hrr_electron_repulsion_dfxx(ket_spher_buffer_0_dfpd, ket_spher_buffer_0_pfpd, ket_spher_buffer_0_pgpd, ab_x, ab_y, ab_z, 1, 2);

        t4cfunc::bra_transform<2, 3>(spher_buffer_0_dfpd, ket_spher_buffer_0_dfpd, 1, 2);

        t4cfunc::store_values(buffer, spher_buffer_0_dfpd, 525 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 2, 3, 1, 2, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecDFPD_hpp */
