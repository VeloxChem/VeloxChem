...
/// @brief Computes (PP|1/|r-r'||DF)  integrals for two basis function pairs blocks.
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 855, cbuffer, 0, 0, cfactors, 6, 0, 2);

            t4cfunc::ket_transform<2, 3>(skbuffer, 0, ckbuffer, 225, 0, 1);

            t4cfunc::ket_transform<2, 3>(skbuffer, 180, ckbuffer, 855, 0, 2);

            erirec::comp_bra_hrr_electron_repulsion_ppxx(skbuffer, 315, 0, 105, r_ab, 2, 3);

            t4cfunc::bra_transform<1, 1>(sbuffer, 0, skbuffer, 315, 2, 3);

            const bool diagonal = bra_eq_ket && (j >= ket_range.first) && (j < ket_range.second);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 1, 2, 3, j, ket_range, diagonal);
        }
    }
