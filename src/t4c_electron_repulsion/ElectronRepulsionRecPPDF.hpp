#ifndef ElectronRepulsionRecPPDF_hpp
#define ElectronRepulsionRecPPDF_hpp
...
/// Computes (PP|1/|r-r'||DF)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_ppdf(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_ppdf(315, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ppdf(315, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_ppdf.zero();

        spher_buffer_0_ppdf.zero();
        ...
        erirec::comp_bra_hrr_electron_repulsion_ppxx(ket_spher_buffer_0_ppdf, ket_spher_buffer_0_spdf, ket_spher_buffer_0_sddf, ab_x, ab_y, ab_z, 2, 3);

        t4cfunc::bra_transform<1, 1>(spher_buffer_0_ppdf, ket_spher_buffer_0_ppdf, 2, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_ppdf, 315 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 1, 2, 3, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecPPDF_hpp */
