#ifndef ElectronRepulsionRecSDFF_hpp
#define ElectronRepulsionRecSDFF_hpp
...
/// @brief Computes (SD|1/|r-r'||FF)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_sdff(T&                               distributor,
                             ...
                             const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(2328, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(294, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 840, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 60, pbuffer, 900, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 990, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 276, pbuffer, 1116, 168, ket_width, ket_npgtos);
            }
            ...
            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 2, 3, 3, j, ket_range, bra_eq_ket);
        }
    }
}
...
#endif /* ElectronRepulsionRecSDFF_hpp */
