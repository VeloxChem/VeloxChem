#ifndef ElectronRepulsionRecSSSF_hpp
#define ElectronRepulsionRecSSSF_hpp
...
/// Computes (SS|1/|r-r'||SF)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_sssf(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_sssf(7, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_sssf(7, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_sssf.zero();

        spher_buffer_0_sssf.zero();
        ...
        t4cfunc::ket_transform<0, 3>(ket_spher_buffer_0_sssf, cart_buffer_0_sssf, 0, 0);

        t4cfunc::bra_transform<0, 0>(spher_buffer_0_sssf, ket_spher_buffer_0_sssf, 0, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_sssf, 7 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 0, 0, 3, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSSSF_hpp */
