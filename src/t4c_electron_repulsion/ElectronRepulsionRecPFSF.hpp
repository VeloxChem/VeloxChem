#ifndef ElectronRepulsionRecPFSF_hpp
#define ElectronRepulsionRecPFSF_hpp
...
/// Computes (PF|1/|r-r'||SF)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_pfsf(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_pfsf(147, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<1, 3>(spher_buffer_0_pfsf, ket_spher_buffer_0_pfsf, 0, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_pfsf, 147 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 3, 0, 3, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecPFSF_hpp */
