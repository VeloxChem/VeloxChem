...
/// Computes (FF|1/|r-r'||DD)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_ffdd(2500, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ffdd(1225, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_ffdd.zero();

        spher_buffer_0_ffdd.zero();
        ...
        erirec::comp_bra_hrr_electron_repulsion_ffxx(ket_spher_buffer_0_ffdd, ket_spher_buffer_0_dfdd, ket_spher_buffer_0_dgdd, ab_x, ab_y, ab_z, 2, 2);

        t4cfunc::bra_transform<3, 3>(spher_buffer_0_ffdd, ket_spher_buffer_0_ffdd, 2, 2);

        t4cfunc::store_values(buffer, spher_buffer_0_ffdd, 1225 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 3, 3, 2, 2, bra_indices, ket_indices);
}
