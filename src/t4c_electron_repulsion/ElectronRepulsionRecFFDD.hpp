#ifndef ElectronRepulsionRecFFDD_hpp
#define ElectronRepulsionRecFFDD_hpp
...
/// @brief Computes (FF|1/|r-r'||DD)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_ffdd(T&                               distributor,
                             ...
                             const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> skbuffer(11550, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 775, pbuffer, 4828, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 901, pbuffer, 5080, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1111, pbuffer, 5500, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1426, pbuffer, 6130, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1594, pbuffer, 6298, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1874, pbuffer, 6578, 420, ket_width, ket_npgtos);
            }
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxpd(ckbuffer, 3864, cbuffer, 1426, 1594, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxpf(ckbuffer, 4368, cbuffer, 1594, 1874, cfactors, 6, 0, 6);

            erirec::comp_ket_hrr_electron_repulsion_xxdd(ckbuffer, 5208, 3864, 4368, cfactors, 6, 0, 6);
            ...
            t4cfunc::ket_transform<2, 2>(skbuffer, 1150, ckbuffer, 5208, 0, 6);

            erirec::comp_bra_hrr_electron_repulsion_pfxx(skbuffer, 1850, 0, 250, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 2600, 250, 625, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 3725, 625, 1150, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_dfxx(skbuffer, 5300, 1850, 2600, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 6800, 2600, 3725, r_ab, 2, 2);

            erirec::comp_bra_hrr_electron_repulsion_ffxx(skbuffer, 9050, 5300, 6800, r_ab, 2, 2);

            t4cfunc::bra_transform<3, 3>(sbuffer, 0, skbuffer, 9050, 2, 2);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 3, 3, 2, 2, j, ket_range, bra_eq_ket);
        }
    }
}
...
#endif /* ElectronRepulsionRecFFDD_hpp */
