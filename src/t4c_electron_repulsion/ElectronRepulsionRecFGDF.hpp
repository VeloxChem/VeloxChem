...
/// @brief Computes (FG|1/|r-r'||DF)  integrals for two basis function pairs blocks.
                             ...
                             const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(13500, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(23030, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 6600, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 7200, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 375, pbuffer, 8100, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 690, pbuffer, 9675, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 900, pbuffer, 10305, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1215, pbuffer, 11250, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1656, pbuffer, 12741, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1936, pbuffer, 13301, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2356, pbuffer, 14141, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2944, pbuffer, 15317, 360, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3304, pbuffer, 15677, 540, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3844, pbuffer, 16217, 756, ket_width, ket_npgtos);
            }
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 1125, 0, 450, cfactors, 6, 0, 4);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 3600, 2025, 2655, cfactors, 6, 0, 5);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 6960, 4860, 5700, cfactors, 6, 0, 6);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdf(ckbuffer, 11340, 8640, 9720, cfactors, 6, 0, 7);

            t4cfunc::ket_transform<2, 3>(skbuffer, 0, ckbuffer, 1125, 0, 4);

            t4cfunc::ket_transform<2, 3>(skbuffer, 525, ckbuffer, 3600, 0, 5);

            t4cfunc::ket_transform<2, 3>(skbuffer, 1260, ckbuffer, 6960, 0, 6);

            t4cfunc::ket_transform<2, 3>(skbuffer, 2240, ckbuffer, 11340, 0, 7);

            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 3500, 0, 525, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 5075, 525, 1260, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_pixx(skbuffer, 7280, 1260, 2240, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 10220, 3500, 5075, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_dhxx(skbuffer, 13370, 5075, 7280, r_ab, 2, 3);

            erirec::comp_bra_hrr_electron_repulsion_fgxx(skbuffer, 17780, 10220, 13370, r_ab, 2, 3);

            t4cfunc::bra_transform<3, 4>(sbuffer, 0, skbuffer, 17780, 2, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 3, 4, 2, 3, j, ket_range, bra_eq_ket);
        }
    }
}
