...
/// @brief Computes (FG|1/|r-r'||FG)  integrals for two basis function pairs blocks.
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(36346, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 14020, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 225, pbuffer, 14920, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 540, pbuffer, 16180, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 960, pbuffer, 17860, 540, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1500, pbuffer, 20566, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1815, pbuffer, 21511, 441, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2256, pbuffer, 22834, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2844, pbuffer, 24598, 756, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3600, pbuffer, 27146, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4020, pbuffer, 27986, 588, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4608, pbuffer, 29162, 784, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5392, pbuffer, 30730, 1008, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 6400, pbuffer, 32746, 540, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 6940, pbuffer, 33286, 756, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 7696, pbuffer, 34042, 1008, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 8704, pbuffer, 35050, 1296, ket_width, ket_npgtos);
            ...
            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 3, 4, 3, 4, j, ket_range, diagonal);
        }
    }
