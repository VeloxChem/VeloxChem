#ifndef ElectronRepulsionRecFGFG_hpp
#define ElectronRepulsionRecFGFG_hpp
...
/// Computes (FG|1/|r-r'||FG)  integrals for GTOs pair block.
...
template <class T>
auto
comp_electron_repulsion_fgfg(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_fgfg(3969, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 3969, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        for (int j = 0; j < npgtos; j++)
        {
            ...
            t2cfunc::reduce(cart_buffer_0_sgsi, prim_buffer_0_sgsi, ket_dim, npgtos);

            t2cfunc::reduce(cart_buffer_0_sgsk, prim_buffer_0_sgsk, ket_dim, npgtos);
            ...
            t2cfunc::reduce(cart_buffer_0_shsi, prim_buffer_0_shsi, ket_dim, npgtos);

            t2cfunc::reduce(cart_buffer_0_shsk, prim_buffer_0_shsk, ket_dim, npgtos);

            t2cfunc::reduce(cart_buffer_0_sisg, prim_buffer_0_sisg, ket_dim, npgtos);

            t2cfunc::reduce(cart_buffer_0_sish, prim_buffer_0_sish, ket_dim, npgtos);

            t2cfunc::reduce(cart_buffer_0_sisi, prim_buffer_0_sisi, ket_dim, npgtos);

            t2cfunc::reduce(cart_buffer_0_sisk, prim_buffer_0_sisk, ket_dim, npgtos);

            t2cfunc::reduce(cart_buffer_0_sksg, prim_buffer_0_sksg, ket_dim, npgtos);

            t2cfunc::reduce(cart_buffer_0_sksh, prim_buffer_0_sksh, ket_dim, npgtos);

            t2cfunc::reduce(cart_buffer_0_sksi, prim_buffer_0_sksi, ket_dim, npgtos);

            t2cfunc::reduce(cart_buffer_0_sksk, prim_buffer_0_sksk, ket_dim, npgtos);
        ...
        t4cfunc::bra_transform<3, 4>(spher_buffer_0_fgfg, ket_spher_buffer_0_fgfg, 3, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_fgfg, 3969 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, 3, 4, bra_indices, ket_indices);
}

/// Computes (FG|1/|r-r'||FG)  integrals for two GTOs pair blocks.
/// - Parameter distributor: the pointer to Fock matrix/matrices distributor.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 3, 4, 3, 4, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecFGFG_hpp */
