...
/// Computes (DD|1/|r-r'||FF)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_ddff(1764, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ddff(1225, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_ddff.zero();

        spher_buffer_0_ddff.zero();
        ...
        erirec::comp_bra_hrr_electron_repulsion_ddxx(ket_spher_buffer_0_ddff, ket_spher_buffer_0_pdff, ket_spher_buffer_0_pfff, ab_x, ab_y, ab_z, 3, 3);

        t4cfunc::bra_transform<2, 2>(spher_buffer_0_ddff, ket_spher_buffer_0_ddff, 3, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_ddff, 1225 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 2, 2, 3, 3, bra_indices, ket_indices);
}
