#ifndef ElectronRepulsionRecPDPF_hpp
#define ElectronRepulsionRecPDPF_hpp
...
/// Computes (PD|1/|r-r'||PF)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_pdpf(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_pdpf(315, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<1, 2>(spher_buffer_0_pdpf, ket_spher_buffer_0_pdpf, 1, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_pdpf, 315 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 2, 1, 3, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecPDPF_hpp */
