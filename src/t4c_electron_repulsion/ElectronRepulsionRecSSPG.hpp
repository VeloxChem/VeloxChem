#ifndef ElectronRepulsionRecSSPG_hpp
#define ElectronRepulsionRecSSPG_hpp
...
/// @brief Computes (SS|1/|r-r'||PG)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_sspg(T&                               distributor,
                             ...
                             const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(45, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                erirec::comp_prim_electron_repulsion_sssh(pbuffer, 105, 45, 55, 75, 90, pfactors, 20, 23, a_exp, b_exp);

                t2cfunc::reduce(cbuffer, 0, pbuffer, 75, 15, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 15, pbuffer, 105, 21, ket_width, ket_npgtos);
            }
            ...
            t4cfunc::bra_transform<0, 0>(sbuffer, 0, skbuffer, 0, 1, 4);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 0, 1, 4, j, ket_range, bra_eq_ket);
        }
    }
}
...
#endif /* ElectronRepulsionRecSSPG_hpp */
