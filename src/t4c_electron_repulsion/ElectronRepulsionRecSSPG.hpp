...
/// Computes (SS|1/|r-r'||PG)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_sspg(27, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_sspg(27, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_sspg.zero();

        spher_buffer_0_sspg.zero();
        ...
        t4cfunc::ket_transform<1, 4>(ket_spher_buffer_0_sspg, contr_buffer_0_sspg, 0, 0);

        t4cfunc::bra_transform<0, 0>(spher_buffer_0_sspg, ket_spher_buffer_0_sspg, 1, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_sspg, 27 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 0, 1, 4, bra_indices, ket_indices);
}
