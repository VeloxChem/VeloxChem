#ifndef ElectronRepulsionRecSFSP_hpp
#define ElectronRepulsionRecSFSP_hpp
...
/// Computes (SF|1/|r-r'||SP)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_sfsp(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_sfsp(21, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<0, 3>(spher_buffer_0_sfsp, ket_spher_buffer_0_sfsp, 0, 1);

        t4cfunc::store_values(buffer, spher_buffer_0_sfsp, 21 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 3, 0, 1, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSFSP_hpp */
