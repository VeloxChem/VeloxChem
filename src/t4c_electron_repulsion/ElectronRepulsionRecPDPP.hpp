#ifndef ElectronRepulsionRecPDPP_hpp
#define ElectronRepulsionRecPDPP_hpp
...
template <class T>
auto
comp_electron_repulsion_pdpp(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_pdpp(135, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<1, 2>(spher_buffer_0_pdpp, ket_spher_buffer_0_pdpp, 1, 1);

        t4cfunc::store_values(buffer, spher_buffer_0_pdpp, 135 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 2, 1, 1, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecPDPP_hpp */
