...
/// @brief Computes (GG|1/|r-r'||GG)  integrals for GTOs pair block.
...
template <class T>
auto
comp_diag_electron_repulsion_gggg(T& distributor, const CGtoPairBlock& gto_pair_block, const std::pair<size_t, size_t>& gto_indices) -> void
{
    ...
    CSimdArray<double> pbuffer(77148, npgtos);
    ...
    CSimdArray<double> bf_data(18, npgtos);
    ...
    for (auto i = gto_indices.first; i < gto_indices.second; i++)
    {
        ...
        for (int j = 0; j < npgtos; j++)
        {
            ...
            t2cfunc::reduce(cbuffer, 0, pbuffer, 22915, 225, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 225, pbuffer, 24040, 315, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 540, pbuffer, 25615, 420, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 960, pbuffer, 27715, 540, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 1500, pbuffer, 30415, 675, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 2175, pbuffer, 34735, 315, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 2490, pbuffer, 35995, 441, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 2931, pbuffer, 37759, 588, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 3519, pbuffer, 40111, 756, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 4275, pbuffer, 43135, 945, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 5220, pbuffer, 47643, 420, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 5640, pbuffer, 48903, 588, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 6228, pbuffer, 50667, 784, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 7012, pbuffer, 53019, 1008, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 8020, pbuffer, 56043, 1260, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 9280, pbuffer, 60183, 540, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 9820, pbuffer, 61263, 756, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 10576, pbuffer, 62775, 1008, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 11584, pbuffer, 64791, 1296, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 12880, pbuffer, 67383, 1620, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 14500, pbuffer, 70623, 675, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 15175, pbuffer, 71298, 945, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 16120, pbuffer, 72243, 1260, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 17380, pbuffer, 73503, 1620, ket_width, npgtos);

            t2cfunc::reduce(cbuffer, 19000, pbuffer, 75123, 2025, ket_width, npgtos);
        }
