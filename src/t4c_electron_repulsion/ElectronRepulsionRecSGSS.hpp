#ifndef ElectronRepulsionRecSGSS_hpp
#define ElectronRepulsionRecSGSS_hpp
...
/// Computes (SG|1/|r-r'||SS)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_sgss(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_sgss(9, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<0, 4>(spher_buffer_0_sgss, ket_spher_buffer_0_sgss, 0, 0);

        t4cfunc::store_values(buffer, spher_buffer_0_sgss, 9 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 4, 0, 0, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSGSS_hpp */
