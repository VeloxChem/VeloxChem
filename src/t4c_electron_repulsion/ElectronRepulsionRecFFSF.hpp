...
/// Computes (FF|1/|r-r'||SF)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_dfsf(420, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_dgsf(630, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_ffsf(700, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ffsf(343, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 343, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_dfsf.zero();

        ket_spher_buffer_0_dgsf.zero();

        ket_spher_buffer_0_ffsf.zero();

        spher_buffer_0_ffsf.zero();
        ...
        erirec::comp_bra_hrr_electron_repulsion_dfxx(ket_spher_buffer_0_dfsf, ket_spher_buffer_0_pfsf, ket_spher_buffer_0_pgsf, ab_x, ab_y, ab_z, 0, 3);
        ...
        erirec::comp_bra_hrr_electron_repulsion_ffxx(ket_spher_buffer_0_ffsf, ket_spher_buffer_0_dfsf, ket_spher_buffer_0_dgsf, ab_x, ab_y, ab_z, 0, 3);

        t4cfunc::bra_transform<3, 3>(spher_buffer_0_ffsf, ket_spher_buffer_0_ffsf, 0, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_ffsf, 343 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 3, 3, 0, 3, bra_indices, ket_indices);
}
