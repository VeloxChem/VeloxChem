#ifndef ElectronRepulsionRecSDPP_hpp
#define ElectronRepulsionRecSDPP_hpp
...
template <class T>
auto
comp_electron_repulsion_sdpp(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_sdpp(45, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<0, 2>(spher_buffer_0_sdpp, ket_spher_buffer_0_sdpp, 1, 1);

        t4cfunc::store_values(buffer, spher_buffer_0_sdpp, 45 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 2, 1, 1, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSDPP_hpp */
