#ifndef ElectronRepulsionRecPGPF_hpp
#define ElectronRepulsionRecPGPF_hpp
...
/// @brief Computes (PG|1/|r-r'||PF)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_pgpf(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 2465, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 2765, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 375, pbuffer, 3215, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 585, pbuffer, 3425, 315, ket_width, ket_npgtos);
            ...
            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 756, 0, 315, r_ab, 1, 3);

            t4cfunc::bra_transform<1, 4>(sbuffer, 0, skbuffer, 756, 1, 3);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 1, 4, 1, 3, j, ket_range, bra_eq_ket);
        }
    }
...
#endif /* ElectronRepulsionRecPGPF_hpp */
