...
/// Computes (DF|1/|r-r'||DG)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_dfdg(1575, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<2, 3>(spher_buffer_0_dfdg, ket_spher_buffer_0_dfdg, 2, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_dfdg, 1575 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 2, 3, 2, 4, bra_indices, ket_indices);
}
