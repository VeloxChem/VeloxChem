#ifndef ElectronRepulsionRecSSPD_hpp
#define ElectronRepulsionRecSSPD_hpp
...
/// Computes (SS|1/|r-r'||PD)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_sspd(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> cart_buffer_0_sssd(6, ket_dim);

    CSimdArray<double> cart_buffer_0_sssf(10, ket_dim);

    // allocate aligned contracted integrals

    CSimdArray<double> contr_buffer_0_sspd(18, ket_dim);

    // allocate aligned half transformed integrals

    CSimdArray<double> ket_spher_buffer_0_sspd(15, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_sspd(15, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        cart_buffer_0_sssd.zero();

        cart_buffer_0_sssf.zero();

        ket_spher_buffer_0_sspd.zero();

        spher_buffer_0_sspd.zero();
        ...
        for (int j = 0; j < bra_npgtos; j++)
        {
            ...
            t2cfunc::reduce(cart_buffer_0_sssd, prim_buffer_0_sssd, ket_dim, ket_npgtos);

            t2cfunc::reduce(cart_buffer_0_sssf, prim_buffer_0_sssf, ket_dim, ket_npgtos);

        }

        erirec::comp_ket_hrr_electron_repulsion_xxpd(contr_buffer_0_sspd, cart_buffer_0_sssd, cart_buffer_0_sssf, cd_x[0], cd_y[0], cd_z[0], 0, 0);

        t4cfunc::ket_transform<1, 2>(ket_spher_buffer_0_sspd, contr_buffer_0_sspd, 0, 0);

        t4cfunc::bra_transform<0, 0>(spher_buffer_0_sspd, ket_spher_buffer_0_sspd, 1, 2);

        t4cfunc::store_values(buffer, spher_buffer_0_sspd, 15 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 0, 1, 2, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSSPD_hpp */
