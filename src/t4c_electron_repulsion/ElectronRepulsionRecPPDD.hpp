#ifndef ElectronRepulsionRecPPDD_hpp
#define ElectronRepulsionRecPPDD_hpp
...
template <class T>
auto
comp_electron_repulsion_ppdd(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> contr_buffer_0_sppd(54, ket_dim);

    CSimdArray<double> contr_buffer_0_sppf(90, ket_dim);

    CSimdArray<double> contr_buffer_0_spdd(108, ket_dim);

    CSimdArray<double> contr_buffer_0_sdpd(108, ket_dim);

    CSimdArray<double> contr_buffer_0_sdpf(180, ket_dim);

    CSimdArray<double> contr_buffer_0_sddd(216, ket_dim);

    // allocate aligned half transformed integrals

    CSimdArray<double> ket_spher_buffer_0_spdd(75, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_sddd(150, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_ppdd(225, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ppdd(225, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_spdd.zero();

        ket_spher_buffer_0_sddd.zero();

        ket_spher_buffer_0_ppdd.zero();

        spher_buffer_0_ppdd.zero();
        ...
        erirec::comp_ket_hrr_electron_repulsion_xxpd(contr_buffer_0_sppd, cart_buffer_0_spsd, cart_buffer_0_spsf, cd_x[0], cd_y[0], cd_z[0], 0, 1);
        ...
        erirec::comp_ket_hrr_electron_repulsion_xxdd(contr_buffer_0_spdd, contr_buffer_0_sppd, contr_buffer_0_sppf, cd_x[0], cd_y[0], cd_z[0], 0, 1);
        ...
        erirec::comp_ket_hrr_electron_repulsion_xxdd(contr_buffer_0_sddd, contr_buffer_0_sdpd, contr_buffer_0_sdpf, cd_x[0], cd_y[0], cd_z[0], 0, 2);

        t4cfunc::ket_transform<2, 2>(ket_spher_buffer_0_spdd, contr_buffer_0_spdd, 0, 1);

        t4cfunc::ket_transform<2, 2>(ket_spher_buffer_0_sddd, contr_buffer_0_sddd, 0, 2);

        erirec::comp_bra_hrr_electron_repulsion_ppxx(ket_spher_buffer_0_ppdd, ket_spher_buffer_0_spdd, ket_spher_buffer_0_sddd, ab_x, ab_y, ab_z, 2, 2);

        t4cfunc::bra_transform<1, 1>(spher_buffer_0_ppdd, ket_spher_buffer_0_ppdd, 2, 2);

        t4cfunc::store_values(buffer, spher_buffer_0_ppdd, 225 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 1, 1, 2, 2, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecPPDD_hpp */
