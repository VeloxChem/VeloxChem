#ifndef ElectronRepulsionRecSPDF_hpp
#define ElectronRepulsionRecSPDF_hpp
...
/// Computes (SP|1/|r-r'||DF)  integrals for two GTOs pair blocks.
...
template <class T>
auto
comp_electron_repulsion_spdf(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_spdf(105, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_spdf(105, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 105, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_spdf.zero();

        spher_buffer_0_spdf.zero();
        ...
        t4cfunc::ket_transform<2, 3>(ket_spher_buffer_0_spdf, contr_buffer_0_spdf, 0, 1);

        t4cfunc::bra_transform<0, 1>(spher_buffer_0_spdf, ket_spher_buffer_0_spdf, 2, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_spdf, 105 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 1, 2, 3, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecSPDF_hpp */
