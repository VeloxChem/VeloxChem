...
/// Computes (DF|1/|r-r'||SD)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> spher_buffer_0_dfsd(175, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        t4cfunc::bra_transform<2, 3>(spher_buffer_0_dfsd, ket_spher_buffer_0_dfsd, 0, 2);

        t4cfunc::store_values(buffer, spher_buffer_0_dfsd, 175 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 2, 3, 0, 2, bra_indices, ket_indices);
}
