...
/// @brief Computes (SG|1/|r-r'||SD)  integrals for two basis function pairs blocks.
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 460, 90, ket_width, ket_npgtos);
            }

            t4cfunc::ket_transform<0, 2>(skbuffer, 0, cbuffer, 0, 0, 4);

            t4cfunc::bra_transform<0, 4>(sbuffer, 0, skbuffer, 0, 0, 2);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 4, 0, 2, j, ket_range, bra_eq_ket);
        }
    }
}
