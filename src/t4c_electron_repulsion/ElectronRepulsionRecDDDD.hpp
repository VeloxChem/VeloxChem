#ifndef ElectronRepulsionRecDDDD_hpp
#define ElectronRepulsionRecDDDD_hpp
...
template <class T>
auto
comp_electron_repulsion_dddd(T* distributor,
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_pddd(450, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_pfdd(750, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_dddd(900, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_dddd(625, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 625, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        for (int j = 0; j < npgtos; j++)
        {
            ...
            t2cfunc::reduce(cart_buffer_0_sdsg, prim_buffer_0_sdsg, ket_dim, npgtos);
            ...
            t2cfunc::reduce(cart_buffer_0_sgsd, prim_buffer_0_sgsd, ket_dim, npgtos);
        ...
        t4cfunc::store_values(buffer, spher_buffer_0_dddd, 625 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, 2, 2, bra_indices, ket_indices);
}
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 2, 2, 2, 2, bra_indices, ket_indices);
}

} // erirec namespace

#endif /* ElectronRepulsionRecDDDD_hpp */
