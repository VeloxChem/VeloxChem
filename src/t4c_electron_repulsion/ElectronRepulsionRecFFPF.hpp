...
/// Computes (FF|1/|r-r'||PF)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_dfpf(1260, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_dgpf(1890, ket_dim);

    CSimdArray<double> ket_spher_buffer_0_ffpf(2100, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ffpf(1029, ket_dim);

    // allocate accumulation buffer for integrals

    CSimdArray<double> buffer(bra_dim * 1029, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_dfpf.zero();

        ket_spher_buffer_0_dgpf.zero();

        ket_spher_buffer_0_ffpf.zero();

        spher_buffer_0_ffpf.zero();
        ...
        erirec::comp_bra_hrr_electron_repulsion_dfxx(ket_spher_buffer_0_dfpf, ket_spher_buffer_0_pfpf, ket_spher_buffer_0_pgpf, ab_x, ab_y, ab_z, 1, 3);
        ...
        erirec::comp_bra_hrr_electron_repulsion_ffxx(ket_spher_buffer_0_ffpf, ket_spher_buffer_0_dfpf, ket_spher_buffer_0_dgpf, ab_x, ab_y, ab_z, 1, 3);

        t4cfunc::bra_transform<3, 3>(spher_buffer_0_ffpf, ket_spher_buffer_0_ffpf, 1, 3);

        t4cfunc::store_values(buffer, spher_buffer_0_ffpf, 1029 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 3, 3, 1, 3, bra_indices, ket_indices);
}
