#ifndef ElectronRepulsionRecSSFG_hpp
#define ElectronRepulsionRecSSFG_hpp
...
/// @brief Computes (SS|1/|r-r'||FG)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_ssfg(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                erirec::comp_prim_electron_repulsion_sssk(pbuffer, 294, 175, 196, 238, 266, pfactors, 20, 23, a_exp, b_exp);
            ...
            erirec::comp_ket_hrr_electron_repulsion_xxdg(ckbuffer, 192, cbuffer, 0, 0, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxdh(ckbuffer, 282, cbuffer, 0, 0, cfactors, 6, 0, 0);

            erirec::comp_ket_hrr_electron_repulsion_xxfg(ckbuffer, 408, cbuffer, 0, 0, cfactors, 6, 0, 0);

            t4cfunc::ket_transform<3, 4>(skbuffer, 0, ckbuffer, 408, 0, 0);

            t4cfunc::bra_transform<0, 0>(sbuffer, 0, skbuffer, 0, 3, 4);

            const bool diagonal = bra_eq_ket && (j >= ket_range.first) && (j < ket_range.second);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 0, 0, 3, 4, j, ket_range, diagonal);
        }
    }
...
#endif /* ElectronRepulsionRecSSFG_hpp */
