...
/// Computes (SS|1/|r-r'||FG)  integrals for two GTOs pair blocks.
                             ...
                             const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_ssfg(63, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_ssfg(63, ket_dim);
    ...
    for (auto i = bra_indices[0]; i < bra_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_ssfg.zero();

        spher_buffer_0_ssfg.zero();
        ...
        t4cfunc::ket_transform<3, 4>(ket_spher_buffer_0_ssfg, contr_buffer_0_ssfg, 0, 0);

        t4cfunc::bra_transform<0, 0>(spher_buffer_0_ssfg, ket_spher_buffer_0_ssfg, 3, 4);

        t4cfunc::store_values(buffer, spher_buffer_0_ssfg, 63 * (i - bra_indices[0]));
    }

    distributor->distribute(buffer, a_indices, b_indices, c_indices, d_indices, 0, 0, 3, 4, bra_indices, ket_indices);
}
