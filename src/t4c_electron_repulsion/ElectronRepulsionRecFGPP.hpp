#ifndef ElectronRepulsionRecFGPP_hpp
#define ElectronRepulsionRecFGPP_hpp
...
/// @brief Computes (FG|1/|r-r'||PP)  integrals for two basis function pairs blocks.
...
template <class T>
inline auto
comp_electron_repulsion_fgpp(T&                               distributor,
                             ...
                             const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> ckbuffer(900, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 1181, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 45, pbuffer, 1361, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 135, pbuffer, 1763, 63, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 198, pbuffer, 1952, 126, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 324, pbuffer, 2358, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 408, pbuffer, 2526, 168, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 576, pbuffer, 2862, 108, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 684, pbuffer, 2970, 216, ket_width, ket_npgtos);
            }
            ...
            erirec::comp_bra_hrr_electron_repulsion_pgxx(skbuffer, 900, 0, 135, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_phxx(skbuffer, 1305, 135, 324, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_pixx(skbuffer, 1872, 324, 576, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_dgxx(skbuffer, 2628, 900, 1305, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_dhxx(skbuffer, 3438, 1305, 1872, r_ab, 1, 1);

            erirec::comp_bra_hrr_electron_repulsion_fgxx(skbuffer, 4572, 2628, 3438, r_ab, 1, 1);

            t4cfunc::bra_transform<3, 4>(sbuffer, 0, skbuffer, 4572, 1, 1);

            distributor.distribute(sbuffer, 0, a_indices, b_indices, c_indices, d_indices, 3, 4, 1, 1, j, ket_range, bra_eq_ket);
        }
    }
}
...
#endif /* ElectronRepulsionRecFGPP_hpp */
