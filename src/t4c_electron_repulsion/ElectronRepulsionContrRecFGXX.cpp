...
auto
comp_bra_hrr_electron_repulsion_fgxx(CSimdArray<double>& contr_buffer_fgxx,
                                     const CSimdArray<double>& contr_buffer_dgxx,
                                     const CSimdArray<double>& contr_buffer_dhxx,
                                     const double ab_x,
                                     const double ab_y,
                                     const double ab_z,
                                     const int c_angmom,
...
{
    const auto ndims = contr_buffer_fgxx.number_of_columns();
    ...
    for (int i = 0; i < ccomps; i++)
    {
        for (int j = 0; j < dcomps; j++)
        {
            ...
            /// Set up components of auxilary buffer : contr_buffer_dhxx
            ...
            const auto fg_off = i * dcomps + j;

            /// Set up 0-15 components of targeted buffer : contr_buffer_fgxx

            auto g_xxx_xxxx = contr_buffer_fgxx[fg_off + 0 * ccomps * dcomps];

            auto g_xxx_xxxy = contr_buffer_fgxx[fg_off + 1 * ccomps * dcomps];

            auto g_xxx_xxxz = contr_buffer_fgxx[fg_off + 2 * ccomps * dcomps];

            auto g_xxx_xxyy = contr_buffer_fgxx[fg_off + 3 * ccomps * dcomps];

            auto g_xxx_xxyz = contr_buffer_fgxx[fg_off + 4 * ccomps * dcomps];

            auto g_xxx_xxzz = contr_buffer_fgxx[fg_off + 5 * ccomps * dcomps];

            auto g_xxx_xyyy = contr_buffer_fgxx[fg_off + 6 * ccomps * dcomps];

            auto g_xxx_xyyz = contr_buffer_fgxx[fg_off + 7 * ccomps * dcomps];

            auto g_xxx_xyzz = contr_buffer_fgxx[fg_off + 8 * ccomps * dcomps];

            auto g_xxx_xzzz = contr_buffer_fgxx[fg_off + 9 * ccomps * dcomps];

            auto g_xxx_yyyy = contr_buffer_fgxx[fg_off + 10 * ccomps * dcomps];

            auto g_xxx_yyyz = contr_buffer_fgxx[fg_off + 11 * ccomps * dcomps];

            auto g_xxx_yyzz = contr_buffer_fgxx[fg_off + 12 * ccomps * dcomps];

            auto g_xxx_yzzz = contr_buffer_fgxx[fg_off + 13 * ccomps * dcomps];

            auto g_xxx_zzzz = contr_buffer_fgxx[fg_off + 14 * ccomps * dcomps];

            #pragma omp simd aligned(g_xx_xxxx, g_xx_xxxxx, g_xx_xxxxy, g_xx_xxxxz, g_xx_xxxy, g_xx_xxxyy, g_xx_xxxyz, g_xx_xxxz, g_xx_xxxzz, g_xx_xxyy, g_xx_xxyyy, g_xx_xxyyz, g_xx_xxyz, g_xx_xxyzz, g_xx_xxzz, g_xx_xxzzz, g_xx_xyyy, g_xx_xyyyy, g_xx_xyyyz, g_xx_xyyz, g_xx_xyyzz, g_xx_xyzz, g_xx_xyzzz, g_xx_xzzz, g_xx_xzzzz, g_xx_yyyy, g_xx_yyyz, g_xx_yyzz, g_xx_yzzz, g_xx_zzzz, g_xxx_xxxx, g_xxx_xxxy, g_xxx_xxxz, g_xxx_xxyy, g_xxx_xxyz, g_xxx_xxzz, g_xxx_xyyy, g_xxx_xyyz, g_xxx_xyzz, g_xxx_xzzz, g_xxx_yyyy, g_xxx_yyyz, g_xxx_yyzz, g_xxx_yzzz, g_xxx_zzzz  : 64)
            ...
            /// Set up 15-30 components of targeted buffer : contr_buffer_fgxx

            auto g_xxy_xxxx = contr_buffer_fgxx[fg_off + 15 * ccomps * dcomps];

            auto g_xxy_xxxy = contr_buffer_fgxx[fg_off + 16 * ccomps * dcomps];

            auto g_xxy_xxxz = contr_buffer_fgxx[fg_off + 17 * ccomps * dcomps];

            auto g_xxy_xxyy = contr_buffer_fgxx[fg_off + 18 * ccomps * dcomps];

            auto g_xxy_xxyz = contr_buffer_fgxx[fg_off + 19 * ccomps * dcomps];

            auto g_xxy_xxzz = contr_buffer_fgxx[fg_off + 20 * ccomps * dcomps];

            auto g_xxy_xyyy = contr_buffer_fgxx[fg_off + 21 * ccomps * dcomps];

            auto g_xxy_xyyz = contr_buffer_fgxx[fg_off + 22 * ccomps * dcomps];

            auto g_xxy_xyzz = contr_buffer_fgxx[fg_off + 23 * ccomps * dcomps];

            auto g_xxy_xzzz = contr_buffer_fgxx[fg_off + 24 * ccomps * dcomps];

            auto g_xxy_yyyy = contr_buffer_fgxx[fg_off + 25 * ccomps * dcomps];

            auto g_xxy_yyyz = contr_buffer_fgxx[fg_off + 26 * ccomps * dcomps];

            auto g_xxy_yyzz = contr_buffer_fgxx[fg_off + 27 * ccomps * dcomps];

            auto g_xxy_yzzz = contr_buffer_fgxx[fg_off + 28 * ccomps * dcomps];

            auto g_xxy_zzzz = contr_buffer_fgxx[fg_off + 29 * ccomps * dcomps];

            #pragma omp simd aligned(g_xxy_xxxx, g_xxy_xxxy, g_xxy_xxxz, g_xxy_xxyy, g_xxy_xxyz, g_xxy_xxzz, g_xxy_xyyy, g_xxy_xyyz, g_xxy_xyzz, g_xxy_xzzz, g_xxy_yyyy, g_xxy_yyyz, g_xxy_yyzz, g_xxy_yzzz, g_xxy_zzzz, g_xy_xxxx, g_xy_xxxxx, g_xy_xxxxy, g_xy_xxxxz, g_xy_xxxy, g_xy_xxxyy, g_xy_xxxyz, g_xy_xxxz, g_xy_xxxzz, g_xy_xxyy, g_xy_xxyyy, g_xy_xxyyz, g_xy_xxyz, g_xy_xxyzz, g_xy_xxzz, g_xy_xxzzz, g_xy_xyyy, g_xy_xyyyy, g_xy_xyyyz, g_xy_xyyz, g_xy_xyyzz, g_xy_xyzz, g_xy_xyzzz, g_xy_xzzz, g_xy_xzzzz, g_xy_yyyy, g_xy_yyyz, g_xy_yyzz, g_xy_yzzz, g_xy_zzzz  : 64)
            for (int k = 0; k < ndims; k++)
            {
            ...
            /// Set up 30-45 components of targeted buffer : contr_buffer_fgxx

            auto g_xxz_xxxx = contr_buffer_fgxx[fg_off + 30 * ccomps * dcomps];

            auto g_xxz_xxxy = contr_buffer_fgxx[fg_off + 31 * ccomps * dcomps];

            auto g_xxz_xxxz = contr_buffer_fgxx[fg_off + 32 * ccomps * dcomps];

            auto g_xxz_xxyy = contr_buffer_fgxx[fg_off + 33 * ccomps * dcomps];

            auto g_xxz_xxyz = contr_buffer_fgxx[fg_off + 34 * ccomps * dcomps];

            auto g_xxz_xxzz = contr_buffer_fgxx[fg_off + 35 * ccomps * dcomps];

            auto g_xxz_xyyy = contr_buffer_fgxx[fg_off + 36 * ccomps * dcomps];

            auto g_xxz_xyyz = contr_buffer_fgxx[fg_off + 37 * ccomps * dcomps];

            auto g_xxz_xyzz = contr_buffer_fgxx[fg_off + 38 * ccomps * dcomps];

            auto g_xxz_xzzz = contr_buffer_fgxx[fg_off + 39 * ccomps * dcomps];

            auto g_xxz_yyyy = contr_buffer_fgxx[fg_off + 40 * ccomps * dcomps];

            auto g_xxz_yyyz = contr_buffer_fgxx[fg_off + 41 * ccomps * dcomps];

            auto g_xxz_yyzz = contr_buffer_fgxx[fg_off + 42 * ccomps * dcomps];

            auto g_xxz_yzzz = contr_buffer_fgxx[fg_off + 43 * ccomps * dcomps];

            auto g_xxz_zzzz = contr_buffer_fgxx[fg_off + 44 * ccomps * dcomps];

            #pragma omp simd aligned(g_xxz_xxxx, g_xxz_xxxy, g_xxz_xxxz, g_xxz_xxyy, g_xxz_xxyz, g_xxz_xxzz, g_xxz_xyyy, g_xxz_xyyz, g_xxz_xyzz, g_xxz_xzzz, g_xxz_yyyy, g_xxz_yyyz, g_xxz_yyzz, g_xxz_yzzz, g_xxz_zzzz, g_xz_xxxx, g_xz_xxxxx, g_xz_xxxxy, g_xz_xxxxz, g_xz_xxxy, g_xz_xxxyy, g_xz_xxxyz, g_xz_xxxz, g_xz_xxxzz, g_xz_xxyy, g_xz_xxyyy, g_xz_xxyyz, g_xz_xxyz, g_xz_xxyzz, g_xz_xxzz, g_xz_xxzzz, g_xz_xyyy, g_xz_xyyyy, g_xz_xyyyz, g_xz_xyyz, g_xz_xyyzz, g_xz_xyzz, g_xz_xyzzz, g_xz_xzzz, g_xz_xzzzz, g_xz_yyyy, g_xz_yyyz, g_xz_yyzz, g_xz_yzzz, g_xz_zzzz  : 64)
            for (int k = 0; k < ndims; k++)
            {
            ...
            /// Set up 45-60 components of targeted buffer : contr_buffer_fgxx

            auto g_xyy_xxxx = contr_buffer_fgxx[fg_off + 45 * ccomps * dcomps];

            auto g_xyy_xxxy = contr_buffer_fgxx[fg_off + 46 * ccomps * dcomps];

            auto g_xyy_xxxz = contr_buffer_fgxx[fg_off + 47 * ccomps * dcomps];

            auto g_xyy_xxyy = contr_buffer_fgxx[fg_off + 48 * ccomps * dcomps];

            auto g_xyy_xxyz = contr_buffer_fgxx[fg_off + 49 * ccomps * dcomps];

            auto g_xyy_xxzz = contr_buffer_fgxx[fg_off + 50 * ccomps * dcomps];

            auto g_xyy_xyyy = contr_buffer_fgxx[fg_off + 51 * ccomps * dcomps];

            auto g_xyy_xyyz = contr_buffer_fgxx[fg_off + 52 * ccomps * dcomps];

            auto g_xyy_xyzz = contr_buffer_fgxx[fg_off + 53 * ccomps * dcomps];

            auto g_xyy_xzzz = contr_buffer_fgxx[fg_off + 54 * ccomps * dcomps];

            auto g_xyy_yyyy = contr_buffer_fgxx[fg_off + 55 * ccomps * dcomps];

            auto g_xyy_yyyz = contr_buffer_fgxx[fg_off + 56 * ccomps * dcomps];

            auto g_xyy_yyzz = contr_buffer_fgxx[fg_off + 57 * ccomps * dcomps];

            auto g_xyy_yzzz = contr_buffer_fgxx[fg_off + 58 * ccomps * dcomps];

            auto g_xyy_zzzz = contr_buffer_fgxx[fg_off + 59 * ccomps * dcomps];

            #pragma omp simd aligned(g_xyy_xxxx, g_xyy_xxxy, g_xyy_xxxz, g_xyy_xxyy, g_xyy_xxyz, g_xyy_xxzz, g_xyy_xyyy, g_xyy_xyyz, g_xyy_xyzz, g_xyy_xzzz, g_xyy_yyyy, g_xyy_yyyz, g_xyy_yyzz, g_xyy_yzzz, g_xyy_zzzz, g_yy_xxxx, g_yy_xxxxx, g_yy_xxxxy, g_yy_xxxxz, g_yy_xxxy, g_yy_xxxyy, g_yy_xxxyz, g_yy_xxxz, g_yy_xxxzz, g_yy_xxyy, g_yy_xxyyy, g_yy_xxyyz, g_yy_xxyz, g_yy_xxyzz, g_yy_xxzz, g_yy_xxzzz, g_yy_xyyy, g_yy_xyyyy, g_yy_xyyyz, g_yy_xyyz, g_yy_xyyzz, g_yy_xyzz, g_yy_xyzzz, g_yy_xzzz, g_yy_xzzzz, g_yy_yyyy, g_yy_yyyz, g_yy_yyzz, g_yy_yzzz, g_yy_zzzz  : 64)
            for (int k = 0; k < ndims; k++)
            {
            ...
            /// Set up 60-75 components of targeted buffer : contr_buffer_fgxx

            auto g_xyz_xxxx = contr_buffer_fgxx[fg_off + 60 * ccomps * dcomps];

            auto g_xyz_xxxy = contr_buffer_fgxx[fg_off + 61 * ccomps * dcomps];

            auto g_xyz_xxxz = contr_buffer_fgxx[fg_off + 62 * ccomps * dcomps];

            auto g_xyz_xxyy = contr_buffer_fgxx[fg_off + 63 * ccomps * dcomps];

            auto g_xyz_xxyz = contr_buffer_fgxx[fg_off + 64 * ccomps * dcomps];

            auto g_xyz_xxzz = contr_buffer_fgxx[fg_off + 65 * ccomps * dcomps];

            auto g_xyz_xyyy = contr_buffer_fgxx[fg_off + 66 * ccomps * dcomps];

            auto g_xyz_xyyz = contr_buffer_fgxx[fg_off + 67 * ccomps * dcomps];

            auto g_xyz_xyzz = contr_buffer_fgxx[fg_off + 68 * ccomps * dcomps];

            auto g_xyz_xzzz = contr_buffer_fgxx[fg_off + 69 * ccomps * dcomps];

            auto g_xyz_yyyy = contr_buffer_fgxx[fg_off + 70 * ccomps * dcomps];

            auto g_xyz_yyyz = contr_buffer_fgxx[fg_off + 71 * ccomps * dcomps];

            auto g_xyz_yyzz = contr_buffer_fgxx[fg_off + 72 * ccomps * dcomps];

            auto g_xyz_yzzz = contr_buffer_fgxx[fg_off + 73 * ccomps * dcomps];

            auto g_xyz_zzzz = contr_buffer_fgxx[fg_off + 74 * ccomps * dcomps];

            #pragma omp simd aligned(g_xyz_xxxx, g_xyz_xxxy, g_xyz_xxxz, g_xyz_xxyy, g_xyz_xxyz, g_xyz_xxzz, g_xyz_xyyy, g_xyz_xyyz, g_xyz_xyzz, g_xyz_xzzz, g_xyz_yyyy, g_xyz_yyyz, g_xyz_yyzz, g_xyz_yzzz, g_xyz_zzzz, g_yz_xxxx, g_yz_xxxxx, g_yz_xxxxy, g_yz_xxxxz, g_yz_xxxy, g_yz_xxxyy, g_yz_xxxyz, g_yz_xxxz, g_yz_xxxzz, g_yz_xxyy, g_yz_xxyyy, g_yz_xxyyz, g_yz_xxyz, g_yz_xxyzz, g_yz_xxzz, g_yz_xxzzz, g_yz_xyyy, g_yz_xyyyy, g_yz_xyyyz, g_yz_xyyz, g_yz_xyyzz, g_yz_xyzz, g_yz_xyzzz, g_yz_xzzz, g_yz_xzzzz, g_yz_yyyy, g_yz_yyyz, g_yz_yyzz, g_yz_yzzz, g_yz_zzzz  : 64)
            for (int k = 0; k < ndims; k++)
            {
            ...
            /// Set up 75-90 components of targeted buffer : contr_buffer_fgxx

            auto g_xzz_xxxx = contr_buffer_fgxx[fg_off + 75 * ccomps * dcomps];

            auto g_xzz_xxxy = contr_buffer_fgxx[fg_off + 76 * ccomps * dcomps];

            auto g_xzz_xxxz = contr_buffer_fgxx[fg_off + 77 * ccomps * dcomps];

            auto g_xzz_xxyy = contr_buffer_fgxx[fg_off + 78 * ccomps * dcomps];

            auto g_xzz_xxyz = contr_buffer_fgxx[fg_off + 79 * ccomps * dcomps];

            auto g_xzz_xxzz = contr_buffer_fgxx[fg_off + 80 * ccomps * dcomps];

            auto g_xzz_xyyy = contr_buffer_fgxx[fg_off + 81 * ccomps * dcomps];

            auto g_xzz_xyyz = contr_buffer_fgxx[fg_off + 82 * ccomps * dcomps];

            auto g_xzz_xyzz = contr_buffer_fgxx[fg_off + 83 * ccomps * dcomps];

            auto g_xzz_xzzz = contr_buffer_fgxx[fg_off + 84 * ccomps * dcomps];

            auto g_xzz_yyyy = contr_buffer_fgxx[fg_off + 85 * ccomps * dcomps];

            auto g_xzz_yyyz = contr_buffer_fgxx[fg_off + 86 * ccomps * dcomps];

            auto g_xzz_yyzz = contr_buffer_fgxx[fg_off + 87 * ccomps * dcomps];

            auto g_xzz_yzzz = contr_buffer_fgxx[fg_off + 88 * ccomps * dcomps];

            auto g_xzz_zzzz = contr_buffer_fgxx[fg_off + 89 * ccomps * dcomps];

            #pragma omp simd aligned(g_xzz_xxxx, g_xzz_xxxy, g_xzz_xxxz, g_xzz_xxyy, g_xzz_xxyz, g_xzz_xxzz, g_xzz_xyyy, g_xzz_xyyz, g_xzz_xyzz, g_xzz_xzzz, g_xzz_yyyy, g_xzz_yyyz, g_xzz_yyzz, g_xzz_yzzz, g_xzz_zzzz, g_zz_xxxx, g_zz_xxxxx, g_zz_xxxxy, g_zz_xxxxz, g_zz_xxxy, g_zz_xxxyy, g_zz_xxxyz, g_zz_xxxz, g_zz_xxxzz, g_zz_xxyy, g_zz_xxyyy, g_zz_xxyyz, g_zz_xxyz, g_zz_xxyzz, g_zz_xxzz, g_zz_xxzzz, g_zz_xyyy, g_zz_xyyyy, g_zz_xyyyz, g_zz_xyyz, g_zz_xyyzz, g_zz_xyzz, g_zz_xyzzz, g_zz_xzzz, g_zz_xzzzz, g_zz_yyyy, g_zz_yyyz, g_zz_yyzz, g_zz_yzzz, g_zz_zzzz  : 64)
            for (int k = 0; k < ndims; k++)
            {
            ...
            /// Set up 90-105 components of targeted buffer : contr_buffer_fgxx

            auto g_yyy_xxxx = contr_buffer_fgxx[fg_off + 90 * ccomps * dcomps];

            auto g_yyy_xxxy = contr_buffer_fgxx[fg_off + 91 * ccomps * dcomps];

            auto g_yyy_xxxz = contr_buffer_fgxx[fg_off + 92 * ccomps * dcomps];

            auto g_yyy_xxyy = contr_buffer_fgxx[fg_off + 93 * ccomps * dcomps];

            auto g_yyy_xxyz = contr_buffer_fgxx[fg_off + 94 * ccomps * dcomps];

            auto g_yyy_xxzz = contr_buffer_fgxx[fg_off + 95 * ccomps * dcomps];

            auto g_yyy_xyyy = contr_buffer_fgxx[fg_off + 96 * ccomps * dcomps];

            auto g_yyy_xyyz = contr_buffer_fgxx[fg_off + 97 * ccomps * dcomps];

            auto g_yyy_xyzz = contr_buffer_fgxx[fg_off + 98 * ccomps * dcomps];

            auto g_yyy_xzzz = contr_buffer_fgxx[fg_off + 99 * ccomps * dcomps];

            auto g_yyy_yyyy = contr_buffer_fgxx[fg_off + 100 * ccomps * dcomps];

            auto g_yyy_yyyz = contr_buffer_fgxx[fg_off + 101 * ccomps * dcomps];

            auto g_yyy_yyzz = contr_buffer_fgxx[fg_off + 102 * ccomps * dcomps];

            auto g_yyy_yzzz = contr_buffer_fgxx[fg_off + 103 * ccomps * dcomps];

            auto g_yyy_zzzz = contr_buffer_fgxx[fg_off + 104 * ccomps * dcomps];

            #pragma omp simd aligned(g_yy_xxxx, g_yy_xxxxy, g_yy_xxxy, g_yy_xxxyy, g_yy_xxxyz, g_yy_xxxz, g_yy_xxyy, g_yy_xxyyy, g_yy_xxyyz, g_yy_xxyz, g_yy_xxyzz, g_yy_xxzz, g_yy_xyyy, g_yy_xyyyy, g_yy_xyyyz, g_yy_xyyz, g_yy_xyyzz, g_yy_xyzz, g_yy_xyzzz, g_yy_xzzz, g_yy_yyyy, g_yy_yyyyy, g_yy_yyyyz, g_yy_yyyz, g_yy_yyyzz, g_yy_yyzz, g_yy_yyzzz, g_yy_yzzz, g_yy_yzzzz, g_yy_zzzz, g_yyy_xxxx, g_yyy_xxxy, g_yyy_xxxz, g_yyy_xxyy, g_yyy_xxyz, g_yyy_xxzz, g_yyy_xyyy, g_yyy_xyyz, g_yyy_xyzz, g_yyy_xzzz, g_yyy_yyyy, g_yyy_yyyz, g_yyy_yyzz, g_yyy_yzzz, g_yyy_zzzz  : 64)
            for (int k = 0; k < ndims; k++)
            {
            ...
            /// Set up 105-120 components of targeted buffer : contr_buffer_fgxx

            auto g_yyz_xxxx = contr_buffer_fgxx[fg_off + 105 * ccomps * dcomps];

            auto g_yyz_xxxy = contr_buffer_fgxx[fg_off + 106 * ccomps * dcomps];

            auto g_yyz_xxxz = contr_buffer_fgxx[fg_off + 107 * ccomps * dcomps];

            auto g_yyz_xxyy = contr_buffer_fgxx[fg_off + 108 * ccomps * dcomps];

            auto g_yyz_xxyz = contr_buffer_fgxx[fg_off + 109 * ccomps * dcomps];

            auto g_yyz_xxzz = contr_buffer_fgxx[fg_off + 110 * ccomps * dcomps];

            auto g_yyz_xyyy = contr_buffer_fgxx[fg_off + 111 * ccomps * dcomps];

            auto g_yyz_xyyz = contr_buffer_fgxx[fg_off + 112 * ccomps * dcomps];

            auto g_yyz_xyzz = contr_buffer_fgxx[fg_off + 113 * ccomps * dcomps];

            auto g_yyz_xzzz = contr_buffer_fgxx[fg_off + 114 * ccomps * dcomps];

            auto g_yyz_yyyy = contr_buffer_fgxx[fg_off + 115 * ccomps * dcomps];

            auto g_yyz_yyyz = contr_buffer_fgxx[fg_off + 116 * ccomps * dcomps];

            auto g_yyz_yyzz = contr_buffer_fgxx[fg_off + 117 * ccomps * dcomps];

            auto g_yyz_yzzz = contr_buffer_fgxx[fg_off + 118 * ccomps * dcomps];

            auto g_yyz_zzzz = contr_buffer_fgxx[fg_off + 119 * ccomps * dcomps];

            #pragma omp simd aligned(g_yyz_xxxx, g_yyz_xxxy, g_yyz_xxxz, g_yyz_xxyy, g_yyz_xxyz, g_yyz_xxzz, g_yyz_xyyy, g_yyz_xyyz, g_yyz_xyzz, g_yyz_xzzz, g_yyz_yyyy, g_yyz_yyyz, g_yyz_yyzz, g_yyz_yzzz, g_yyz_zzzz, g_yz_xxxx, g_yz_xxxxy, g_yz_xxxy, g_yz_xxxyy, g_yz_xxxyz, g_yz_xxxz, g_yz_xxyy, g_yz_xxyyy, g_yz_xxyyz, g_yz_xxyz, g_yz_xxyzz, g_yz_xxzz, g_yz_xyyy, g_yz_xyyyy, g_yz_xyyyz, g_yz_xyyz, g_yz_xyyzz, g_yz_xyzz, g_yz_xyzzz, g_yz_xzzz, g_yz_yyyy, g_yz_yyyyy, g_yz_yyyyz, g_yz_yyyz, g_yz_yyyzz, g_yz_yyzz, g_yz_yyzzz, g_yz_yzzz, g_yz_yzzzz, g_yz_zzzz  : 64)
            for (int k = 0; k < ndims; k++)
            {
            ...
            /// Set up 120-135 components of targeted buffer : contr_buffer_fgxx

            auto g_yzz_xxxx = contr_buffer_fgxx[fg_off + 120 * ccomps * dcomps];

            auto g_yzz_xxxy = contr_buffer_fgxx[fg_off + 121 * ccomps * dcomps];

            auto g_yzz_xxxz = contr_buffer_fgxx[fg_off + 122 * ccomps * dcomps];

            auto g_yzz_xxyy = contr_buffer_fgxx[fg_off + 123 * ccomps * dcomps];

            auto g_yzz_xxyz = contr_buffer_fgxx[fg_off + 124 * ccomps * dcomps];

            auto g_yzz_xxzz = contr_buffer_fgxx[fg_off + 125 * ccomps * dcomps];

            auto g_yzz_xyyy = contr_buffer_fgxx[fg_off + 126 * ccomps * dcomps];

            auto g_yzz_xyyz = contr_buffer_fgxx[fg_off + 127 * ccomps * dcomps];

            auto g_yzz_xyzz = contr_buffer_fgxx[fg_off + 128 * ccomps * dcomps];

            auto g_yzz_xzzz = contr_buffer_fgxx[fg_off + 129 * ccomps * dcomps];

            auto g_yzz_yyyy = contr_buffer_fgxx[fg_off + 130 * ccomps * dcomps];

            auto g_yzz_yyyz = contr_buffer_fgxx[fg_off + 131 * ccomps * dcomps];

            auto g_yzz_yyzz = contr_buffer_fgxx[fg_off + 132 * ccomps * dcomps];

            auto g_yzz_yzzz = contr_buffer_fgxx[fg_off + 133 * ccomps * dcomps];

            auto g_yzz_zzzz = contr_buffer_fgxx[fg_off + 134 * ccomps * dcomps];

            #pragma omp simd aligned(g_yzz_xxxx, g_yzz_xxxy, g_yzz_xxxz, g_yzz_xxyy, g_yzz_xxyz, g_yzz_xxzz, g_yzz_xyyy, g_yzz_xyyz, g_yzz_xyzz, g_yzz_xzzz, g_yzz_yyyy, g_yzz_yyyz, g_yzz_yyzz, g_yzz_yzzz, g_yzz_zzzz, g_zz_xxxx, g_zz_xxxxy, g_zz_xxxy, g_zz_xxxyy, g_zz_xxxyz, g_zz_xxxz, g_zz_xxyy, g_zz_xxyyy, g_zz_xxyyz, g_zz_xxyz, g_zz_xxyzz, g_zz_xxzz, g_zz_xyyy, g_zz_xyyyy, g_zz_xyyyz, g_zz_xyyz, g_zz_xyyzz, g_zz_xyzz, g_zz_xyzzz, g_zz_xzzz, g_zz_yyyy, g_zz_yyyyy, g_zz_yyyyz, g_zz_yyyz, g_zz_yyyzz, g_zz_yyzz, g_zz_yyzzz, g_zz_yzzz, g_zz_yzzzz, g_zz_zzzz  : 64)
            for (int k = 0; k < ndims; k++)
            {
            ...
            /// Set up 135-150 components of targeted buffer : contr_buffer_fgxx

            auto g_zzz_xxxx = contr_buffer_fgxx[fg_off + 135 * ccomps * dcomps];

            auto g_zzz_xxxy = contr_buffer_fgxx[fg_off + 136 * ccomps * dcomps];

            auto g_zzz_xxxz = contr_buffer_fgxx[fg_off + 137 * ccomps * dcomps];

            auto g_zzz_xxyy = contr_buffer_fgxx[fg_off + 138 * ccomps * dcomps];

            auto g_zzz_xxyz = contr_buffer_fgxx[fg_off + 139 * ccomps * dcomps];

            auto g_zzz_xxzz = contr_buffer_fgxx[fg_off + 140 * ccomps * dcomps];

            auto g_zzz_xyyy = contr_buffer_fgxx[fg_off + 141 * ccomps * dcomps];

            auto g_zzz_xyyz = contr_buffer_fgxx[fg_off + 142 * ccomps * dcomps];

            auto g_zzz_xyzz = contr_buffer_fgxx[fg_off + 143 * ccomps * dcomps];

            auto g_zzz_xzzz = contr_buffer_fgxx[fg_off + 144 * ccomps * dcomps];

            auto g_zzz_yyyy = contr_buffer_fgxx[fg_off + 145 * ccomps * dcomps];

            auto g_zzz_yyyz = contr_buffer_fgxx[fg_off + 146 * ccomps * dcomps];

            auto g_zzz_yyzz = contr_buffer_fgxx[fg_off + 147 * ccomps * dcomps];

            auto g_zzz_yzzz = contr_buffer_fgxx[fg_off + 148 * ccomps * dcomps];

            auto g_zzz_zzzz = contr_buffer_fgxx[fg_off + 149 * ccomps * dcomps];

            #pragma omp simd aligned(g_zz_xxxx, g_zz_xxxxz, g_zz_xxxy, g_zz_xxxyz, g_zz_xxxz, g_zz_xxxzz, g_zz_xxyy, g_zz_xxyyz, g_zz_xxyz, g_zz_xxyzz, g_zz_xxzz, g_zz_xxzzz, g_zz_xyyy, g_zz_xyyyz, g_zz_xyyz, g_zz_xyyzz, g_zz_xyzz, g_zz_xyzzz, g_zz_xzzz, g_zz_xzzzz, g_zz_yyyy, g_zz_yyyyz, g_zz_yyyz, g_zz_yyyzz, g_zz_yyzz, g_zz_yyzzz, g_zz_yzzz, g_zz_yzzzz, g_zz_zzzz, g_zz_zzzzz, g_zzz_xxxx, g_zzz_xxxy, g_zzz_xxxz, g_zzz_xxyy, g_zzz_xxyz, g_zzz_xxzz, g_zzz_xyyy, g_zzz_xyyz, g_zzz_xyzz, g_zzz_xzzz, g_zzz_yyyy, g_zzz_yyyz, g_zzz_yyzz, g_zzz_yzzz, g_zzz_zzzz  : 64)
            for (int k = 0; k < ndims; k++)
            {
