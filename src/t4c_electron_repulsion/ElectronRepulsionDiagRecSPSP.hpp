...
/// Computes (SP|1/|r-r'||SP)  integrals for GTOs pair block.
                                  ...
                                  const std::array<int, 2>& gto_indices) -> void
{
    ...
    CSimdArray<double> ket_spher_buffer_0_spsp(9, 1);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_spsp(9, 1);
    ...
    CSimdArray<double> bf_values(3, npgtos);
    ...
    for (auto i = gto_indices[0]; i < gto_indices[1]; i++)
    {
        ...
        ket_spher_buffer_0_spsp.zero();

        spher_buffer_0_spsp.zero();
        ...
        t4cfunc::ket_transform<0, 1>(ket_spher_buffer_0_spsp, cart_buffer_0_spsp, 0, 1);

        t4cfunc::bra_transform<0, 1>(spher_buffer_0_spsp, ket_spher_buffer_0_spsp, 0, 1);

        t4cfunc::update_max_values(max_values, spher_buffer_0_spsp, i - gto_indices[0]); 
    }
