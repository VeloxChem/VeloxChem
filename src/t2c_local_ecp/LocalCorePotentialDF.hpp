#ifndef LocalCorePotentialDF_hpp
#define LocalCorePotentialDF_hpp
...
template <class T>
auto
comp_local_core_potential_df(T& distributor,
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t2chrr::comp_hrr_pf(cbuffer, 46, 0, 10, cfactors);

            t2chrr::comp_hrr_pg(cbuffer, 76, 10, 25, cfactors);

            t2chrr::comp_hrr_df(cbuffer, 121, 46, 76, cfactors);

            t2cfunc::transform<2, 3>(sbuffer, cbuffer, 121);
...
#endif /* LocalCorePotentialDF_hpp */
