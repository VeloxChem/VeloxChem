#ifndef LocalCorePotentialFG_hpp
#define LocalCorePotentialFG_hpp
...
template <class T>
auto
comp_local_core_potential_fg(T& distributor,
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            t2chrr::comp_hrr_pg(cbuffer, 100, 0, 15, cfactors);

            t2chrr::comp_hrr_ph(cbuffer, 145, 15, 36, cfactors);

            t2chrr::comp_hrr_pi(cbuffer, 208, 36, 64, cfactors);

            t2chrr::comp_hrr_dg(cbuffer, 292, 100, 145, cfactors);

            t2chrr::comp_hrr_dh(cbuffer, 382, 145, 208, cfactors);

            t2chrr::comp_hrr_fg(cbuffer, 508, 292, 382, cfactors);

            t2cfunc::transform<3, 4>(sbuffer, cbuffer, 508);
...
#endif /* LocalCorePotentialFG_hpp */
