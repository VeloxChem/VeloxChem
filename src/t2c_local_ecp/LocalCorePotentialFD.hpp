#ifndef LocalCorePotentialFD_hpp
#define LocalCorePotentialFD_hpp
...
template <class T>
auto
comp_local_core_potential_fd(T& distributor,
                             ...
                             const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(56, ket_npgtos);

    // allocate aligned contracted integrals

    CSimdArray<double> cbuffer(181, 1);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < ecp_nppt; l++)
                {
                    ...
                    t2cfunc::reduce(cbuffer, 100, pbuffer, 20, 15, ket_width, ket_npgtos);

                    t2cfunc::reduce(cbuffer, 160, pbuffer, 35, 21, ket_width, ket_npgtos);

                }
            }

            t2chrr::comp_hrr_fp(cbuffer, 10, 0, 100, cfactors);

            t2chrr::comp_hrr_fd(cbuffer, 40, 10, 115, cfactors);

            t2chrr::comp_hrr_gp(cbuffer, 115, 100, 160, cfactors);

            t2cfunc::transform<3, 2>(sbuffer, cbuffer, 40);
...
#endif /* LocalCorePotentialFD_hpp */
