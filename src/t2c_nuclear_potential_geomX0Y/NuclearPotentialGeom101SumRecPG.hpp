#ifndef NuclearPotentialGeom101SumRecPG_hpp
#define NuclearPotentialGeom101SumRecPG_hpp
...
/// @brief Computes (d^(1)/dA^(1)P|A|d^(1)/dB^(1)G)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_nuclear_potential_geom_11_pg(T& distributor,
                                      ...
                                      const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(1142, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    npotrec::comp_prim_nuclear_potential_sh(pbuffer, 176, 66, 76, 116, 131, factors, 14, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_sh(pbuffer, 197, 76, 86, 131, 146, factors, 14, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_sh(pbuffer, 218, 86, 96, 146, 161, factors, 14, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_pd(pbuffer, 239, 9, 12, 30, 36, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_pd(pbuffer, 257, 12, 15, 36, 42, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_pf(pbuffer, 275, 30, 36, 66, 76, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_pf(pbuffer, 305, 36, 42, 76, 86, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_pg(pbuffer, 335, 66, 76, 116, 131, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_pg(pbuffer, 380, 76, 86, 131, 146, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_ph(pbuffer, 425, 116, 131, 176, 197, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_ph(pbuffer, 488, 131, 146, 197, 218, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_df(pbuffer, 551, 66, 76, 239, 257, 275, 305, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_dh(pbuffer, 611, 176, 197, 335, 380, 425, 488, factors, 11, 17, a_exp);

                    t2cgeom::comp_prim_op_geom_11_pg(pbuffer, 737, 66, 176, 551, 611, 1, factors, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 737, charges[l], ket_width, ket_npgtos);
                }
            }
...
#endif /* NuclearPotentialGeom101SumRecPG_hpp */
