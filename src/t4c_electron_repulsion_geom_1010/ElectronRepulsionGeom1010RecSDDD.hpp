                                      ...
                                      const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> cart_buffer_0_geom1010_sddd(1944, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_geom1010_sddd(1125, ket_dim);
    ...
    {
        cart_buffer_0_geom1010_sddd.zero();

        spher_buffer_0_geom1010_sddd.zero();
        ...
        for (int j = 0; j < bra_npgtos; j++)
        {
            ...
            t4c_geom::comp_geom1010_sddd_0(prim_buffer_0_geom1010_sddd, prim_buffer_0_pdpd, prim_buffer_0_pdfd, a_exp, c_exps[0] );

            t2cfunc::reduce(cart_buffer_0_geom1010_sddd, prim_buffer_0_geom1010_sddd, ket_dim, ket_npgtos);
        }

        t4cfunc::full_transform<0, 2, 2, 2>(spher_buffer_0_geom1010_sddd, cart_spher_buffer_0_geom1010_sddd);

        distributor->distribute(spher_buffer_0_geom1010_sddd, a_indices, b_indices, c_indices, d_indices, 0, 2, 2, 2, 1, 0, 1, 0, i, ket_indices);
    }
}
