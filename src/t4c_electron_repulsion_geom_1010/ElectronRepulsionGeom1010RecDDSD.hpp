                                      ...
                                      const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> cart_buffer_0_geom1010_ddsd(1944, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_geom1010_ddsd(1125, ket_dim);
    ...
    {
        cart_buffer_0_geom1010_ddsd.zero();

        spher_buffer_0_geom1010_ddsd.zero();
        ...
        for (int j = 0; j < bra_npgtos; j++)
        {
            ...
            t4c_geom::comp_geom1010_ddsd_0(prim_buffer_0_geom1010_ddsd, prim_buffer_0_pdpd, prim_buffer_0_fdpd, a_exp, c_exps[0] );

            t2cfunc::reduce(cart_buffer_0_geom1010_ddsd, prim_buffer_0_geom1010_ddsd, ket_dim, ket_npgtos);
        }

        t4cfunc::full_transform<2, 2, 0, 2>(spher_buffer_0_geom1010_ddsd, cart_spher_buffer_0_geom1010_ddsd);

        distributor->distribute(spher_buffer_0_geom1010_ddsd, a_indices, b_indices, c_indices, d_indices, 2, 2, 0, 2, 1, 0, 1, 0, i, ket_indices);
    }
}
