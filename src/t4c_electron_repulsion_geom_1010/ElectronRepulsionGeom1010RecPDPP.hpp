                                      ...
                                      const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> cart_buffer_0_geom1010_pdpp(1458, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_geom1010_pdpp(1215, ket_dim);
    ...
    {
        cart_buffer_0_geom1010_pdpp.zero();

        spher_buffer_0_geom1010_pdpp.zero();
        ...
        for (int j = 0; j < bra_npgtos; j++)
        {
            ...
            t4c_geom::comp_geom1010_pdpp_0(prim_buffer_0_geom1010_pdpp, prim_buffer_0_sdsp, prim_buffer_0_sddp, prim_buffer_0_ddsp, prim_buffer_0_dddp, a_exp, c_exps[0] );

            t2cfunc::reduce(cart_buffer_0_geom1010_pdpp, prim_buffer_0_geom1010_pdpp, ket_dim, ket_npgtos);
        }

        t4cfunc::full_transform<1, 2, 1, 1>(spher_buffer_0_geom1010_pdpp, cart_spher_buffer_0_geom1010_pdpp);

        distributor->distribute(spher_buffer_0_geom1010_pdpp, a_indices, b_indices, c_indices, d_indices, 1, 2, 1, 1, 1, 0, 1, 0, i, ket_indices);
    }
}
