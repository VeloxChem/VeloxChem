                                      ...
                                      const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> cart_buffer_0_geom1010_pppd(1458, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_geom1010_pppd(1215, ket_dim);
    ...
    {
        cart_buffer_0_geom1010_pppd.zero();

        spher_buffer_0_geom1010_pppd.zero();
        ...
        for (int j = 0; j < bra_npgtos; j++)
        {
            ...
            t4c_geom::comp_geom1010_pppd_0(prim_buffer_0_geom1010_pppd, prim_buffer_0_spsd, prim_buffer_0_spdd, prim_buffer_0_dpsd, prim_buffer_0_dpdd, a_exp, c_exps[0] );

            t2cfunc::reduce(cart_buffer_0_geom1010_pppd, prim_buffer_0_geom1010_pppd, ket_dim, ket_npgtos);
        }

        t4cfunc::full_transform<1, 1, 1, 2>(spher_buffer_0_geom1010_pppd, cart_spher_buffer_0_geom1010_pppd);

        distributor->distribute(spher_buffer_0_geom1010_pppd, a_indices, b_indices, c_indices, d_indices, 1, 1, 1, 2, 1, 0, 1, 0, i, ket_indices);
    }
}
