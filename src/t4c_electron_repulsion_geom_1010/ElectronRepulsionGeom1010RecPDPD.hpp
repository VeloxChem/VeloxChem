                                      ...
                                      const std::array<int, 2>& ket_indices) -> void
{
    ...
    CSimdArray<double> cart_buffer_0_geom1010_pdpd(2916, ket_dim);

    // allocate aligned spherical integrals

    CSimdArray<double> spher_buffer_0_geom1010_pdpd(2025, ket_dim);
    ...
    {
        cart_buffer_0_geom1010_pdpd.zero();

        spher_buffer_0_geom1010_pdpd.zero();
        ...
        for (int j = 0; j < bra_npgtos; j++)
        {
            ...
            t4c_geom::comp_geom1010_pdpd_0(prim_buffer_0_geom1010_pdpd, prim_buffer_0_sdsd, prim_buffer_0_sddd, prim_buffer_0_ddsd, prim_buffer_0_dddd, a_exp, c_exps[0] );

            t2cfunc::reduce(cart_buffer_0_geom1010_pdpd, prim_buffer_0_geom1010_pdpd, ket_dim, ket_npgtos);
        }

        t4cfunc::full_transform<1, 2, 1, 2>(spher_buffer_0_geom1010_pdpd, cart_spher_buffer_0_geom1010_pdpd);

        distributor->distribute(spher_buffer_0_geom1010_pdpd, a_indices, b_indices, c_indices, d_indices, 1, 2, 1, 2, 1, 0, 1, 0, i, ket_indices);
    }
}
