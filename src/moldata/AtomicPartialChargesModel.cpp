...
getPartialCharges(const CMolecule& molecule, const double netcharge) -> std::vector<double>
{
    ...
    for (int i = 0; i < natoms; i++)
    {
        const auto rixyz = xyzcoord[i];

        auto Amat_row_i = Amat.row(i);

        for (int j = 0; j < i; j++)
        {
            const auto rjxyz = xyzcoord[j];
    ...
    std::vector<double> solution(Ainv.getNumberOfRows());

    gpu::computeMatrixMultiplication(solution.data(), Ainv.values(), vec_rhs.data(), "N", "N", Ainv.getNumberOfRows(), Ainv.getNumberOfColumns(), 1);

    auto solptr = solution.data();
