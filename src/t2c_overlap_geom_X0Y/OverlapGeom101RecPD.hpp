#ifndef OverlapGeom101RecPD_hpp
#define OverlapGeom101RecPD_hpp
...
/// @brief Computes (d^(1)/dA^(1)P|d^(1)/dB^(1)D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_overlap_geom_11_pd(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_df(pbuffer, 98, 10, 32, 50, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_11_pd(pbuffer, 158, 1, 10, 80, 98, 1, factors, a_exp);
