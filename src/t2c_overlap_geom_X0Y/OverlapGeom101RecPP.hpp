#ifndef OverlapGeom101RecPP_hpp
#define OverlapGeom101RecPP_hpp
...
/// @brief Computes (d^(1)/dA^(1)P|d^(1)/dB^(1)P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_overlap_geom_11_pp(T& distributor,
                        ...
                        const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(163, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_dd(pbuffer, 46, 4, 13, 22, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_11_pp(pbuffer, 82, 0, 4, 40, 46, 1, factors, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 82, ket_width, ket_npgtos);
            }
