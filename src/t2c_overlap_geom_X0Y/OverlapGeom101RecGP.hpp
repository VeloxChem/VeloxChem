#ifndef OverlapGeom101RecGP_hpp
#define OverlapGeom101RecGP_hpp
...
/// @brief Computes (d^(1)/dA^(1)G|d^(1)/dB^(1)P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_overlap_geom_11_gp(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_hs(pbuffer, 350, 100, 200, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_hd(pbuffer, 371, 140, 215, 260, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_11_gp(pbuffer, 497, 100, 140, 350, 371, 1, factors, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 497, ket_width, ket_npgtos);
            }
