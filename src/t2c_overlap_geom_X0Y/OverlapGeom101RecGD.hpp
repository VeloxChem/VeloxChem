#ifndef OverlapGeom101RecGD_hpp
#define OverlapGeom101RecGD_hpp
...
/// @brief Computes (d^(1)/dA^(1)G|d^(1)/dB^(1)D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_overlap_geom_11_gd(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_fp(pbuffer, 210, 23, 80, 86, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_fd(pbuffer, 240, 32, 86, 104, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_ff(pbuffer, 300, 50, 104, 140, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_gs(pbuffer, 400, 80, 200, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_gp(pbuffer, 415, 86, 200, 210, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_gd(pbuffer, 460, 104, 210, 240, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_gf(pbuffer, 550, 140, 240, 300, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_hp(pbuffer, 700, 210, 400, 415, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_hf(pbuffer, 763, 300, 460, 550, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_11_gd(pbuffer, 973, 210, 300, 700, 763, 1, factors, a_exp);
