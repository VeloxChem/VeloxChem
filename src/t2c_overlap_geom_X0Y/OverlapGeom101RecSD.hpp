#ifndef OverlapGeom101RecSD_hpp
#define OverlapGeom101RecSD_hpp
...
/// @brief Computes (d^(1)/dA^(1)S|d^(1)/dB^(1)D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_overlap_geom_11_sd(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_pf(pbuffer, 29, 4, 10, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_11_sd(pbuffer, 59, 20, 29, 1, factors, a_exp);
