#ifndef OverlapGeom101RecFP_hpp
#define OverlapGeom101RecFP_hpp
...
/// @brief Computes (d^(1)/dA^(1)F|d^(1)/dB^(1)P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_overlap_geom_11_fp(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_gd(pbuffer, 215, 64, 110, 140, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_11_fp(pbuffer, 305, 40, 64, 200, 215, 1, factors, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 305, ket_width, ket_npgtos);
            }
