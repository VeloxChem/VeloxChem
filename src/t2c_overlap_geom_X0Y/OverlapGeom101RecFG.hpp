#ifndef OverlapGeom101RecFG_hpp
#define OverlapGeom101RecFG_hpp
...
/// @brief Computes (d^(1)/dA^(1)F|d^(1)/dB^(1)G)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_overlap_geom_11_fg(T& distributor,
                        ...
                        const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(2889, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_gh(pbuffer, 1224, 428, 714, 864, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_11_fg(pbuffer, 1539, 278, 428, 1074, 1224, 1, factors, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 1539, ket_width, ket_npgtos);
            }
