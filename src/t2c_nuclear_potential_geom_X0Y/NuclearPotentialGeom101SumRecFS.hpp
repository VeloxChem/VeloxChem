#ifndef NuclearPotentialGeom101SumRecFS_hpp
#define NuclearPotentialGeom101SumRecFS_hpp
...
/// @brief Computes (d^(1)/dA^(1)F|A|d^(1)/dB^(1)S)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_nuclear_potential_geom_11_fs(T&                               distributor,
                                      ...
                                      const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(357, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    t2cgeom::comp_prim_op_geom_11_fs(pbuffer, 267, 88, 222, 1, factors, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 267, charges[l], ket_width, ket_npgtos);
                }
            }
...
#endif /* NuclearPotentialGeom101SumRecFS_hpp */
