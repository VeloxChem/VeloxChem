#ifndef NuclearPotentialGeom101SumRecGP_hpp
#define NuclearPotentialGeom101SumRecGP_hpp
...
/// @brief Computes (d^(1)/dA^(1)G|A|d^(1)/dB^(1)P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_nuclear_potential_geom_11_gp(T&                               distributor,
                                      ...
                                      const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(1608, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    npotrec::comp_prim_nuclear_potential_gs(pbuffer, 756, 216, 222, 456, 466, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_gs(pbuffer, 771, 222, 228, 466, 476, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_gp(pbuffer, 786, 240, 258, 456, 466, 486, 516, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_gp(pbuffer, 831, 258, 276, 466, 476, 516, 546, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_gd(pbuffer, 876, 312, 348, 486, 516, 576, 636, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_gd(pbuffer, 966, 348, 384, 516, 546, 636, 696, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_hs(pbuffer, 1056, 456, 466, 756, 771, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_hd(pbuffer, 1077, 576, 636, 786, 831, 876, 966, factors, 11, 17, a_exp);

                    t2cgeom::comp_prim_op_geom_11_gp(pbuffer, 1203, 456, 576, 1056, 1077, 1, factors, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 1203, charges[l], ket_width, ket_npgtos);
                }
            }
...
#endif /* NuclearPotentialGeom101SumRecGP_hpp */
