#ifndef NuclearPotentialGeom101SumRecDP_hpp
#define NuclearPotentialGeom101SumRecDP_hpp
...
/// @brief Computes (d^(1)/dA^(1)D|A|d^(1)/dB^(1)P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_nuclear_potential_geom_11_dp(T&                               distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    npotrec::comp_prim_nuclear_potential_ds(pbuffer, 136, 1, 2, 46, 49, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_ds(pbuffer, 142, 2, 3, 49, 52, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_dp(pbuffer, 148, 7, 10, 46, 49, 55, 64, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_dp(pbuffer, 166, 10, 13, 49, 52, 64, 73, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_dd(pbuffer, 184, 22, 28, 55, 64, 82, 100, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_dd(pbuffer, 220, 28, 34, 64, 73, 100, 118, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_fs(pbuffer, 256, 46, 49, 136, 142, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_fd(pbuffer, 266, 82, 100, 148, 166, 184, 220, factors, 11, 17, a_exp);

                    t2cgeom::comp_prim_op_geom_11_dp(pbuffer, 326, 46, 82, 256, 266, 1, factors, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 326, charges[l], ket_width, ket_npgtos);
                }
            }
...
#endif /* NuclearPotentialGeom101SumRecDP_hpp */
