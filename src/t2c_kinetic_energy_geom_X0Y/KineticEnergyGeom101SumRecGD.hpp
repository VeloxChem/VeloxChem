#ifndef KineticEnergyGeom101SumRecGD_hpp
#define KineticEnergyGeom101SumRecGD_hpp
...
/// @brief Computes (d^(1)/dA^(1)G|T|d^(1)/dB^(1)D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_kinetic_energy_geom_11_gd(T&                               distributor,
                                   ...
                                   const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(2756, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    ovlrec::comp_prim_overlap_gs(pbuffer, 800, 160, 400, factors, 8, a_exp);

                    kinrec::comp_prim_kinetic_energy_gs(pbuffer, 815, 160, 166, 410, 800, factors, 8, a_exp);

                    ovlrec::comp_prim_overlap_gp(pbuffer, 830, 172, 400, 420, factors, 8, a_exp);

                    kinrec::comp_prim_kinetic_energy_gp(pbuffer, 875, 172, 190, 410, 450, 830, factors, 8, a_exp);

                    ovlrec::comp_prim_overlap_gd(pbuffer, 920, 208, 420, 480, factors, 8, a_exp);

                    kinrec::comp_prim_kinetic_energy_gd(pbuffer, 1010, 208, 244, 450, 540, 920, factors, 8, a_exp);

                    ovlrec::comp_prim_overlap_gf(pbuffer, 1100, 280, 480, 600, factors, 8, a_exp);

                    kinrec::comp_prim_kinetic_energy_gf(pbuffer, 1250, 280, 340, 540, 700, 1100, factors, 8, a_exp);

                    ovlrec::comp_prim_overlap_hp(pbuffer, 1400, 420, 800, 830, factors, 8, a_exp);

                    kinrec::comp_prim_kinetic_energy_hp(pbuffer, 1463, 420, 450, 815, 875, 1400, factors, 8, a_exp);

                    ovlrec::comp_prim_overlap_hf(pbuffer, 1526, 600, 920, 1100, factors, 8, a_exp);

                    kinrec::comp_prim_kinetic_energy_hf(pbuffer, 1736, 600, 700, 1010, 1250, 1526, factors, 8, a_exp);

                    t2cgeom::comp_prim_op_geom_11_gd(pbuffer, 1946, 450, 700, 1463, 1736, 1, factors, a_exp);
...
#endif /* KineticEnergyGeom101SumRecGD_hpp */
