#ifndef KineticEnergyGeom101RecGS_hpp
#define KineticEnergyGeom101RecGS_hpp
...
/// @brief Computes (d^(1)/dA^(1)G|T|d^(1)/dB^(1)S)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_kinetic_energy_geom_11_gs(T&                               distributor,
                               ...
                               const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(541, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_gs(pbuffer, 160, 32, 80, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_gs(pbuffer, 175, 32, 38, 90, 160, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_gp(pbuffer, 190, 44, 80, 100, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_gp(pbuffer, 235, 44, 62, 90, 130, 190, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_hp(pbuffer, 280, 100, 160, 190, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_hp(pbuffer, 343, 100, 130, 175, 235, 280, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_11_gs(pbuffer, 406, 130, 343, 1, factors, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 406, ket_width, ket_npgtos);
            }
...
#endif /* KineticEnergyGeom101RecGS_hpp */
