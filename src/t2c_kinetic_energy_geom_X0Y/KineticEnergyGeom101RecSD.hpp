#ifndef KineticEnergyGeom101RecSD_hpp
#define KineticEnergyGeom101RecSD_hpp
...
/// @brief Computes (d^(1)/dA^(1)S|T|d^(1)/dB^(1)D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_kinetic_energy_geom_11_sd(T&                               distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_pf(pbuffer, 58, 8, 20, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_pf(pbuffer, 88, 14, 30, 58, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_11_sd(pbuffer, 118, 49, 88, 1, factors, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 118, ket_width, ket_npgtos);
            }
...
#endif /* KineticEnergyGeom101RecSD_hpp */
