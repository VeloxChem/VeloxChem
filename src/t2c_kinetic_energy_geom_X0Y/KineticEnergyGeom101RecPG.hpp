#ifndef KineticEnergyGeom101RecPG_hpp
#define KineticEnergyGeom101RecPG_hpp
...
/// @brief Computes (d^(1)/dA^(1)P|T|d^(1)/dB^(1)G)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_kinetic_energy_geom_11_pg(T& distributor,
                               ...
                               const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(1201, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_pd(pbuffer, 112, 2, 8, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_pd(pbuffer, 130, 5, 14, 112, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_pf(pbuffer, 148, 8, 20, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_pf(pbuffer, 178, 14, 30, 148, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_pg(pbuffer, 208, 20, 40, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_pg(pbuffer, 253, 30, 55, 208, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_ph(pbuffer, 298, 40, 70, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_ph(pbuffer, 361, 55, 91, 298, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_df(pbuffer, 424, 20, 112, 148, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_df(pbuffer, 484, 20, 30, 130, 178, 424, factors, 8, a_exp);

                ovlrec::comp_prim_overlap_dh(pbuffer, 544, 70, 208, 298, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_dh(pbuffer, 670, 70, 91, 253, 361, 544, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_11_pg(pbuffer, 796, 30, 91, 484, 670, 1, factors, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 796, ket_width, ket_npgtos);
            }
...
#endif /* KineticEnergyGeom101RecPG_hpp */
