#ifndef KineticEnergyGeom101RecDP_hpp
#define KineticEnergyGeom101RecDP_hpp
...
/// @brief Computes (d^(1)/dA^(1)D|T|d^(1)/dB^(1)P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_kinetic_energy_geom_11_dp(T& distributor,
                               ...
                               const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(502, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_fd(pbuffer, 220, 44, 92, 128, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_fd(pbuffer, 280, 44, 62, 110, 164, 220, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_11_dp(pbuffer, 340, 23, 62, 210, 280, 1, factors, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 340, ket_width, ket_npgtos);
            }
...
#endif /* KineticEnergyGeom101RecDP_hpp */
