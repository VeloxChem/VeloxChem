#ifndef KineticEnergyGeom101SumRecDS_hpp
#define KineticEnergyGeom101SumRecDS_hpp
...
/// @brief Computes (d^(1)/dA^(1)D|T|d^(1)/dB^(1)S)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_kinetic_energy_geom_11_ds(T&                               distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    t2cgeom::comp_prim_op_geom_11_ds(pbuffer, 140, 23, 110, 1, factors, a_exp);
...
#endif /* KineticEnergyGeom101SumRecDS_hpp */
