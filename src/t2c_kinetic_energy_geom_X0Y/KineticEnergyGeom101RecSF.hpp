#ifndef KineticEnergyGeom101RecSF_hpp
#define KineticEnergyGeom101RecSF_hpp
...
/// @brief Computes (d^(1)/dA^(1)S|T|d^(1)/dB^(1)F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_kinetic_energy_geom_11_sf(T& distributor,
                               ...
                               const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(286, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_pg(pbuffer, 106, 20, 40, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_pg(pbuffer, 151, 30, 55, 106, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_11_sf(pbuffer, 196, 88, 151, 1, factors, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 196, ket_width, ket_npgtos);
            }
...
#endif /* KineticEnergyGeom101RecSF_hpp */
