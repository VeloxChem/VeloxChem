#ifndef KineticEnergyGeom101RecSS_hpp
#define KineticEnergyGeom101RecSS_hpp
...
/// @brief Computes (d^(1)/dA^(1)S|T|d^(1)/dB^(1)S)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_kinetic_energy_geom_11_ss(T&                               distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_pp(pbuffer, 8, 0, 2, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_pp(pbuffer, 17, 1, 5, 8, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_11_ss(pbuffer, 26, 17, 1, factors, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 26, ket_width, ket_npgtos);
            }
...
#endif /* KineticEnergyGeom101RecSS_hpp */
