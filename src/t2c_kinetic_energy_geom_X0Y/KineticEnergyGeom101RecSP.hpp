#ifndef KineticEnergyGeom101RecSP_hpp
#define KineticEnergyGeom101RecSP_hpp
...
/// @brief Computes (d^(1)/dA^(1)S|T|d^(1)/dB^(1)P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_kinetic_energy_geom_11_sp(T& distributor,
                               ...
                               const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(89, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_pd(pbuffer, 26, 2, 8, factors, 8, a_exp);

                kinrec::comp_prim_kinetic_energy_pd(pbuffer, 44, 5, 14, 26, factors, 8, a_exp);

                t2cgeom::comp_prim_op_geom_11_sp(pbuffer, 62, 23, 44, 1, factors, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 62, ket_width, ket_npgtos);
            }
...
#endif /* KineticEnergyGeom101RecSP_hpp */
