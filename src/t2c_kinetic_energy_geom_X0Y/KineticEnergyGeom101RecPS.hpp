#ifndef KineticEnergyGeom101RecPS_hpp
#define KineticEnergyGeom101RecPS_hpp
...
/// @brief Computes (d^(1)/dA^(1)P|T|d^(1)/dB^(1)S)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_kinetic_energy_geom_11_ps(T&                               distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cgeom::comp_prim_op_geom_11_ps(pbuffer, 68, 5, 50, 1, factors, a_exp);
...
#endif /* KineticEnergyGeom101RecPS_hpp */
