                 ...
                 const std::vector<int64_t>& gtos_mask) -> CMatrix
{
    ...
    for (int64_t i = 0; i < ncgtos; i++)
    {
        if (gtos_mask[i] == 1)
        {
            ...
            dft::distribute(submat, buffer_xx, -1.0, 2 * nrows + irow);
            
            dft::distribute(submat, buffer_xx, 0.5 * f2_3, 4 * nrows + irow);
            
            dft::distribute(submat, buffer_xy, f2_3, irow);
            
            dft::distribute(submat, buffer_xz, f2_3, 3 * nrows + irow);
            
            dft::distribute(submat, buffer_yy, -1.0, 2 * nrows + irow);
            
            dft::distribute(submat, buffer_yy, -0.5 * f2_3, 4 * nrows + irow);
            
            dft::distribute(submat, buffer_yz, f2_3, nrows + irow);
            
            dft::distribute(submat, buffer_zz, 2.0, 2 * nrows + irow);
