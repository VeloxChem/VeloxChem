                 ...
                 const std::vector<int64_t>& gtos_mask) -> CMatrix
{
    ...
    for (int64_t i = 0; i < ncgtos; i++)
    {
        if (gtos_mask[i] == 1)
        {
            ...
            dft::distribute(submat, buffer_xxx, -f3_3, 4 * nrows + irow);

            dft::distribute(submat, buffer_xxx, f3_5, 6 * nrows + irow);

            dft::distribute(submat, buffer_xxy, 3.0 * f3_5, irow);

            dft::distribute(submat, buffer_xxy, -f3_3, 2 * nrows + irow);

            dft::distribute(submat, buffer_xxz, -3.0, 3 * nrows + irow);

            dft::distribute(submat, buffer_xxz, 0.5 * f3_15, 5 * nrows + irow);

            dft::distribute(submat, buffer_xyy, -f3_3, 4 * nrows + irow);

            dft::distribute(submat, buffer_xyy, -3.0 * f3_5, 6 * nrows + irow);

            dft::distribute(submat, buffer_xyz, f3_15, nrows + irow);

            dft::distribute(submat, buffer_xzz, 4.0 * f3_3, 4 * nrows + irow);

            dft::distribute(submat, buffer_yyy, -f3_5, irow);

            dft::distribute(submat, buffer_yyy, -f3_3, 2 * nrows + irow);

            dft::distribute(submat, buffer_yyz, -3.0, 3 * nrows + irow);

            dft::distribute(submat, buffer_yyz, -0.5 * f3_15, 5 * nrows + irow);

            dft::distribute(submat, buffer_yzz, 4.0 * f3_3, 2 * nrows + irow);

            dft::distribute(submat, buffer_zzz, 2.0, 3 * nrows + irow);
