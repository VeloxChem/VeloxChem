...
/**
 Computes LDA GTO values on given grid for D type GTOs.
...
/**
 Computes GGA GTO values on given grid for D type GTOs.
 ...
 @param gtos_mask the mask for GTOs (1 evaluate, 0 skip).
 */
auto getGgaValuesRecD(const CGtoBlock&            gto_block,
                      const std::vector<double>&  grid_coords_x,
                      const std::vector<double>&  grid_coords_y,
                      const std::vector<double>&  grid_coords_z,
                      const std::vector<int64_t>& gtos_mask) -> CMatrix;
