#ifndef ExpGeom2011Driver_hpp
#define ExpGeom2011Driver_hpp
...
#include "EriGeom2011Func.hpp"

/**
 Class CExpGeom2011Driver provides methods for computing Fock matrices
 using four center electron repulsion integrals..

 @author Z. Rinkevicius
 */
class CExpGeom2011Driver
{
   public:
    /**
     Creates a Fock matrices  driver.
     */
    CExpGeom2011Driver() = default;
...
#pragma omp task firstprivate(i)
                            {
                                for (const auto& task : ptr_work_groups[i])
                                {
                                    ...
                                    eri_g2011::compute(distributor, bra_gto_pair_block, ket_gto_pair_block, {task[4], task[5]}, {task[6], task[7]});
                                }
                            }
                        }
                    }
