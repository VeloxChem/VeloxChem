#ifndef ExpGeom1111Driver_hpp
#define ExpGeom1111Driver_hpp
...
#include "EriGeom1111Func.hpp"

/**
 Class CExpGeom1111Driver provides methods for computing Fock matrices
 using four center electron repulsion integrals..

 @author Z. Rinkevicius
 */
class CExpGeom1111Driver
{
   public:
    /**
     Creates a Fock matrices  driver.
     */
    CExpGeom1111Driver() = default;
        ...
        for (auto& [atoms, mats] : results[0].getMap())
        {
            for (auto& [tints, ctasks] : task.getIntegralTask(atoms))
            {
                ...
                const auto ket_gto_pair_blocks = gtofunc::makeGtoPairBlocks(gtofunc::makeGtoBlocks(basis, molecule, {c, }),
                                                                            gtofunc::makeGtoBlocks(basis, molecule, {d, }));
...
#pragma omp task firstprivate(i)
                            {
                                for (const auto& task : ptr_work_groups[i])
                                {
                                    ...
                                    eri_g1111::compute(distributor, bra_gto_pair_block, ket_gto_pair_block, {task[4], task[5]}, {task[6], task[7]});
                                }
                            }
                        }
                    }
