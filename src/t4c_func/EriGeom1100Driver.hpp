#ifndef ExpGeom1100Driver_hpp
#define ExpGeom1100Driver_hpp
...
#include "EriGeom1100Func.hpp"

/**
 Class CExpGeom1100Driver provides methods for computing Fock matrices
 using four center electron repulsion integrals..

 @author Z. Rinkevicius
 */
class CExpGeom1100Driver
{
   public:
    /**
     Creates a Fock matrices  driver.
     */
    CExpGeom1100Driver() = default;
...
#pragma omp task firstprivate(i)
                            {
                                for (const auto& task : ptr_work_groups[i])
                                {
                                    ...
                                    eri_g1100::compute(distributor, bra_gto_pair_block, ket_gto_pair_block, {task[4], task[5]}, {task[6], task[7]});
                                }
                            }
                        }
                    }
