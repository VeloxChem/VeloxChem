...
auto
accumulate_value(double*                    value_ptr,
        ...
        for (int j = 0; j < ket_comps; j++)
        {
            ...
            for (size_t k = 0; k < bra_loc_indices[0]; k++)
            {
                ...
                for (size_t l = 0; l < ket_loc_indices[0]; l++)
                {
                    ...
                    if (ang_order)
                    {
                        (*value_ptr) += density_matrix->operator[]({kg, lg}) * loc_matrix->at({bra_loff + k, ket_loff + l});
                    }
                    else
                    {
                        (*value_ptr) += density_matrix->operator[]({lg, kg}) * loc_matrix->at({bra_loff + k, ket_loff + l});
                    }
                }
            }
        }
                               ...
                               const std::pair<size_t, size_t>& ket_range) -> void
{
    ...
    if ((label == "k") || (label == "k_rs"))
    {
        ...
        {
            t4cfunc::local_distribute_k_geom_ket_gen(focks,
                                                     suffix,
                                                     density,
                                                     buffer,
                                                     offset,
    ...
    if ((label == "kx") || (label == "kx_rs"))
    {
        ...
        {
            t4cfunc::local_distribute_kx_geom_ket_gen(focks,
                                                      suffix,
                                                      density,
                                                      buffer,
                                                      offset,
                      ...
                      const std::pair<size_t, size_t>& ket_range) -> void
{
    ...
    else if (label == "2jkx")
    {
        if (density->get_type() == mat_t::symmetric)
        {
            t4cfunc::local_distribute_jkx_geom_sym(values,
                                                   cart_ind,
                                                   density,
                                                   density_2,
                                                   buffer,
    ...
    else if ((label == "j") || (label == "j_rs"))
    {
        if (density->get_type() == mat_t::symmetric)
        {
            t4cfunc::local_distribute_j_geom_sym(values,
                                                 cart_ind,
                                                 density,
                                                 density_2,
                                                 buffer,
...
auto
local_distribute_geom_no_symm(std::vector<double>&             values,
                      const int                        cart_ind,
                      const CMatrix*                   density,
                      const CMatrix*                   density_2,
                      const std::string&               label,
                      ...
                      const std::pair<size_t, size_t>& ket_range) -> void
{
    if (label == "2jk")
    {
        if (density->get_type() == mat_t::general)
        {
            t4cfunc::local_distribute_jk_geom_gen_no_symm(values,
                                                  cart_ind,
                                                  density,
                                                  density_2,
                                                  buffer,
    ...
    else if (label == "2jkx")
    {
        if (density->get_type() == mat_t::general)
        {
            t4cfunc::local_distribute_jkx_geom_gen_no_symm(values,
                                                   cart_ind,
                                                   density,
                                                   density_2,
                                                   buffer,
    ...
    else if ((label == "j") || (label == "j_rs"))
    {
        if (density->get_type() == mat_t::general)
        {
            t4cfunc::local_distribute_j_geom_gen_no_symm(values,
                                                 cart_ind,
                                                 density,
                                                 density_2,
                                                 buffer,
    ...
    else if ((label == "k") || (label == "k_rs"))
    {
        if (density->get_type() == mat_t::general)
        {
            t4cfunc::local_distribute_k_geom_gen_no_symm(values,
                                                 cart_ind,
                                                 density,
                                                 density_2,
                                                 buffer,
    ...
    else if ((label == "kx") || (label == "kx_rs"))
    {
        if (density->get_type() == mat_t::general)
        {
            t4cfunc::local_distribute_kx_geom_gen_no_symm(values,
                                                  cart_ind,
                                                  density,
                                                  density_2,
                                                  buffer,
