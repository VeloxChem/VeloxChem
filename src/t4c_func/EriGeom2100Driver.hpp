#ifndef ExpGeom2100Driver_hpp
#define ExpGeom2100Driver_hpp
...
#include "EriGeom2100Func.hpp"

/**
 Class CExpGeom2100Driver provides methods for computing Fock matrices
 using four center electron repulsion integrals..

 @author Z. Rinkevicius
 */
class CExpGeom2100Driver
{
   public:
    /**
     Creates a Fock matrices  driver.
     */
    CExpGeom2100Driver() = default;
...
#pragma omp task firstprivate(i)
                            {
                                for (const auto& task : ptr_work_groups[i])
                                {
                                    ...
                                    eri_g2100::compute(distributor, bra_gto_pair_block, ket_gto_pair_block, {task[4], task[5]}, {task[6], task[7]});
                                }
                            }
                        }
                    }
