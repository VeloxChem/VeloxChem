#ifndef ExpGeom4000Driver_hpp
#define ExpGeom4000Driver_hpp

#include <vector>

#include "EriGeom4000Func.hpp"
...
/**
 Class CExpGeom4000Driver provides methods for computing Fock matrices
 using four center electron repulsion integrals..

 @author Z. Rinkevicius
 */
class CExpGeom4000Driver
{
   public:
    /**
     Creates a Fock matrices  driver.
     */
    CExpGeom4000Driver() = default;
...
#pragma omp task firstprivate(i)
                            {
                                for (const auto& task : ptr_work_groups[i])
                                {
                                    ...
                                    eri_g4000::compute(distributor, bra_gto_pair_block, ket_gto_pair_block, {task[4], task[5]}, {task[6], task[7]});
                                }
                            }
                        }
                    }
