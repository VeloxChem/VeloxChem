#include "PrimitiveOctupoleSP_0_X.hpp"
...
auto
compPrimitiveOctupoleSP_0_X(TDoubleArray&       buffer_xxx,
...
#pragma omp simd aligned(fints_xxx,     \
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xxx[i] += faa_xx * (3.0 / 2.0) * fe_0;

        fints_xxx[i] += faa_xxx * rpb_x;

        fints_xxy[i] += faa_xy * fe_0;

        fints_xxy[i] += faa_xxy * rpb_x;

        fints_xxz[i] += faa_xz * fe_0;

        fints_xxz[i] += faa_xxz * rpb_x;

        fints_xyy[i] += faa_yy * (1.0 / 2.0) * fe_0;

        fints_xyy[i] += faa_xyy * rpb_x;

        fints_xyz[i] += faa_yz * (1.0 / 2.0) * fe_0;

        fints_xyz[i] += faa_xyz * rpb_x;

        fints_xzz[i] += faa_zz * (1.0 / 2.0) * fe_0;

        fints_xzz[i] += faa_xzz * rpb_x;

        fints_yyy[i] += faa_yyy * rpb_x;

        fints_yyz[i] += faa_yyz * rpb_x;

        fints_yzz[i] += faa_yzz * rpb_x;

        fints_zzz[i] += faa_zzz * rpb_x;
    }
}
