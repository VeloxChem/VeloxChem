...
auto
compPrimitiveOctupoleSD_0_YY(TDoubleArray&       buffer_xxx,
...
#pragma omp simd aligned(fints_xxx,     \
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xxx[i] += faa_xxx * ((1.0 / 2.0) * fe_0 + rpb_y * rpb_y);

        fints_xxy[i] += faa_xx * fe_0 * rpb_y;

        fints_xxy[i] += faa_xxy * ((1.0 / 2.0) * fe_0 + rpb_y * rpb_y);

        fints_xxz[i] += faa_xxz * ((1.0 / 2.0) * fe_0 + rpb_y * rpb_y);

        fints_xyy[i] += faa_x * (1.0 / 2.0) * fe_0 * fe_0;

        fints_xyy[i] += faa_xy * 2.0 * fe_0 * rpb_y;

        fints_xyy[i] += faa_xyy * ((1.0 / 2.0) * fe_0 + rpb_y * rpb_y);

        fints_xyz[i] += faa_xz * fe_0 * rpb_y;

        fints_xyz[i] += faa_xyz * ((1.0 / 2.0) * fe_0 + rpb_y * rpb_y);

        fints_xzz[i] += faa_xzz * ((1.0 / 2.0) * fe_0 + rpb_y * rpb_y);

        fints_yyy[i] += faa_y * (3.0 / 2.0) * fe_0 * fe_0;

        fints_yyy[i] += faa_yy * 3.0 * fe_0 * rpb_y;

        fints_yyy[i] += faa_yyy * ((1.0 / 2.0) * fe_0 + rpb_y * rpb_y);

        fints_yyz[i] += faa_z * (1.0 / 2.0) * fe_0 * fe_0;

        fints_yyz[i] += faa_yz * 2.0 * fe_0 * rpb_y;

        fints_yyz[i] += faa_yyz * ((1.0 / 2.0) * fe_0 + rpb_y * rpb_y);

        fints_yzz[i] += faa_zz * fe_0 * rpb_y;

        fints_yzz[i] += faa_yzz * ((1.0 / 2.0) * fe_0 + rpb_y * rpb_y);

        fints_zzz[i] += faa_zzz * ((1.0 / 2.0) * fe_0 + rpb_y * rpb_y);
    }
}
