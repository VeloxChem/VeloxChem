...
auto
compPrimitiveOctupolePS_X_0(TDoubleArray&       buffer_xxx,
...
#pragma omp simd aligned(fints_xxx,     \
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xxx[i] += faa_xxx * rpa_x;

        fints_xxy[i] += faa_xy * fe_0;

        fints_xxy[i] += faa_xxy * rpa_x;

        fints_xxz[i] += faa_xz * fe_0;

        fints_xxz[i] += faa_xxz * rpa_x;

        fints_xyy[i] += faa_yy * (1.0 / 2.0) * fe_0;

        fints_xyy[i] += faa_xyy * rpa_x;

        fints_xyz[i] += faa_yz * (1.0 / 2.0) * fe_0;

        fints_xyz[i] += faa_xyz * rpa_x;

        fints_xzz[i] += faa_zz * (1.0 / 2.0) * fe_0;

        fints_xzz[i] += faa_xzz * rpa_x;

        fints_yyy[i] += faa_yyy * rpa_x;

        fints_yyz[i] += faa_yyz * rpa_x;

        fints_yzz[i] += faa_yzz * rpa_x;

        fints_zzz[i] += faa_zzz * rpa_x;
    }
}
