...
auto
compPrimitiveOctupoleSP_0_Y(TDoubleArray&       buffer_xxx,
...
#pragma omp simd aligned(fints_xxx,     \
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xxx[i] += faa_xxx * rpb_y;

        fints_xxy[i] += faa_xx * (1.0 / 2.0) * fe_0;

        fints_xxy[i] += faa_xxy * rpb_y;

        fints_xxz[i] += faa_xxz * rpb_y;

        fints_xyy[i] += faa_xy * fe_0;

        fints_xyy[i] += faa_xyy * rpb_y;

        fints_xyz[i] += faa_xz * (1.0 / 2.0) * fe_0;

        fints_xyz[i] += faa_xyz * rpb_y;

        fints_xzz[i] += faa_xzz * rpb_y;

        fints_yyy[i] += faa_yy * (3.0 / 2.0) * fe_0;

        fints_yyy[i] += faa_yyy * rpb_y;

        fints_yyz[i] += faa_yz * fe_0;

        fints_yyz[i] += faa_yyz * rpb_y;

        fints_yzz[i] += faa_zz * (1.0 / 2.0) * fe_0;

        fints_yzz[i] += faa_yzz * rpb_y;

        fints_zzz[i] += faa_zzz * rpb_y;
    }
}
