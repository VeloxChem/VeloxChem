...
auto
compPrimitiveOctupoleSF_0_ZZZ(      TDoubleArray& buffer_xxx,
                              ...
                              const int64_t       ket_dim) -> void
{
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xxx[i] += faa_xxx * ((3.0 / 2.0) * fe_0 * rpb_z + rpb_z * rpb_z * rpb_z);

        fints_xxy[i] += faa_xxy * ((3.0 / 2.0) * fe_0 * rpb_z + rpb_z * rpb_z * rpb_z);

        fints_xxz[i] += faa_xx * ((3.0 / 2.0) * fe_0 * rpb_z * rpb_z + (3.0 / 4.0) * fe_0 * fe_0);

        fints_xxz[i] += faa_xxz * ((3.0 / 2.0) * fe_0 * rpb_z + rpb_z * rpb_z * rpb_z);

        fints_xyy[i] += faa_xyy * ((3.0 / 2.0) * fe_0 * rpb_z + rpb_z * rpb_z * rpb_z);

        fints_xyz[i] += faa_xy * ((3.0 / 2.0) * fe_0 * rpb_z * rpb_z + (3.0 / 4.0) * fe_0 * fe_0);

        fints_xyz[i] += faa_xyz * ((3.0 / 2.0) * fe_0 * rpb_z + rpb_z * rpb_z * rpb_z);

        fints_xzz[i] += faa_x * (3.0 / 2.0) * fe_0 * fe_0 * rpb_z;

        fints_xzz[i] += faa_xz * (3.0 * fe_0 * rpb_z * rpb_z + (3.0 / 2.0) * fe_0 * fe_0);

        fints_xzz[i] += faa_xzz * ((3.0 / 2.0) * fe_0 * rpb_z + rpb_z * rpb_z * rpb_z);

        fints_yyy[i] += faa_yyy * ((3.0 / 2.0) * fe_0 * rpb_z + rpb_z * rpb_z * rpb_z);

        fints_yyz[i] += faa_yy * ((3.0 / 2.0) * fe_0 * rpb_z * rpb_z + (3.0 / 4.0) * fe_0 * fe_0);

        fints_yyz[i] += faa_yyz * ((3.0 / 2.0) * fe_0 * rpb_z + rpb_z * rpb_z * rpb_z);

        fints_yzz[i] += faa_y * (3.0 / 2.0) * fe_0 * fe_0 * rpb_z;

        fints_yzz[i] += faa_yz * (3.0 * fe_0 * rpb_z * rpb_z + (3.0 / 2.0) * fe_0 * fe_0);

        fints_yzz[i] += faa_yzz * ((3.0 / 2.0) * fe_0 * rpb_z + rpb_z * rpb_z * rpb_z);

        fints_zzz[i] += fss * (3.0 / 4.0) * fe_0 * fe_0 * fe_0;

        fints_zzz[i] += faa_z * (9.0 / 2.0) * fe_0 * fe_0 * rpb_z;

        fints_zzz[i] += faa_zz * ((9.0 / 2.0) * fe_0 * rpb_z * rpb_z + (9.0 / 4.0) * fe_0 * fe_0);

        fints_zzz[i] += faa_zzz * ((3.0 / 2.0) * fe_0 * rpb_z + rpb_z * rpb_z * rpb_z);
