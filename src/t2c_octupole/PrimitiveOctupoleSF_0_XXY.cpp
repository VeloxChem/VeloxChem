...
auto
compPrimitiveOctupoleSF_0_XXY(TDoubleArray&       buffer_xxx,
...
#pragma omp simd aligned(fints_xxx,     \
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xxx[i] += faa_xx * 3.0 * fe_0 * rpb_y * rpb_x;

        fints_xxx[i] += faa_xxx * ((1.0 / 2.0) * fe_0 * rpb_y + rpb_y * rpb_x * rpb_x);
        ...
        fints_xxy[i] += faa_xy * 2.0 * fe_0 * rpb_y * rpb_x;

        fints_xxy[i] += faa_xx * ((1.0 / 2.0) * fe_0 * rpb_x * rpb_x + (1.0 / 4.0) * fe_0 * fe_0);

        fints_xxy[i] += faa_xxy * ((1.0 / 2.0) * fe_0 * rpb_y + rpb_y * rpb_x * rpb_x);

        fints_xxz[i] += faa_z * (1.0 / 2.0) * fe_0 * fe_0 * rpb_y;

        fints_xxz[i] += faa_xz * 2.0 * fe_0 * rpb_y * rpb_x;

        fints_xxz[i] += faa_xxz * ((1.0 / 2.0) * fe_0 * rpb_y + rpb_y * rpb_x * rpb_x);

        fints_xyy[i] += faa_y * fe_0 * fe_0 * rpb_x;

        fints_xyy[i] += faa_yy * fe_0 * rpb_y * rpb_x;

        fints_xyy[i] += faa_xy * (fe_0 * rpb_x * rpb_x + (1.0 / 2.0) * fe_0 * fe_0);

        fints_xyy[i] += faa_xyy * ((1.0 / 2.0) * fe_0 * rpb_y + rpb_y * rpb_x * rpb_x);

        fints_xyz[i] += faa_z * (1.0 / 2.0) * fe_0 * fe_0 * rpb_x;

        fints_xyz[i] += faa_yz * fe_0 * rpb_y * rpb_x;

        fints_xyz[i] += faa_xz * ((1.0 / 2.0) * fe_0 * rpb_x * rpb_x + (1.0 / 4.0) * fe_0 * fe_0);

        fints_xyz[i] += faa_xyz * ((1.0 / 2.0) * fe_0 * rpb_y + rpb_y * rpb_x * rpb_x);

        fints_xzz[i] += faa_zz * fe_0 * rpb_y * rpb_x;

        fints_xzz[i] += faa_xzz * ((1.0 / 2.0) * fe_0 * rpb_y + rpb_y * rpb_x * rpb_x);

        fints_yyy[i] += faa_yy * ((3.0 / 2.0) * fe_0 * rpb_x * rpb_x + (3.0 / 4.0) * fe_0 * fe_0);

        fints_yyy[i] += faa_yyy * ((1.0 / 2.0) * fe_0 * rpb_y + rpb_y * rpb_x * rpb_x);

        fints_yyz[i] += faa_yz * (fe_0 * rpb_x * rpb_x + (1.0 / 2.0) * fe_0 * fe_0);

        fints_yyz[i] += faa_yyz * ((1.0 / 2.0) * fe_0 * rpb_y + rpb_y * rpb_x * rpb_x);

        fints_yzz[i] += faa_zz * ((1.0 / 2.0) * fe_0 * rpb_x * rpb_x + (1.0 / 4.0) * fe_0 * fe_0);

        fints_yzz[i] += faa_yzz * ((1.0 / 2.0) * fe_0 * rpb_y + rpb_y * rpb_x * rpb_x);

        fints_zzz[i] += faa_zzz * ((1.0 / 2.0) * fe_0 * rpb_y + rpb_y * rpb_x * rpb_x);
    }
}
