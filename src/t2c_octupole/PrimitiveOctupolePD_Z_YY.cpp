...
#pragma omp simd aligned(fints_xxx,     \
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xxx[i] += faa_xxx * ((1.0 / 2.0) * fe_0 * rpa_z + rpa_z * rpb_y * rpb_y);

        fints_xxy[i] += faa_xx * fe_0 * rpa_z * rpb_y;

        fints_xxy[i] += faa_xxy * ((1.0 / 2.0) * fe_0 * rpa_z + rpa_z * rpb_y * rpb_y);

        fints_xxz[i] += faa_xx * ((1.0 / 2.0) * fe_0 * rpb_y * rpb_y + (1.0 / 4.0) * fe_0 * fe_0);

        fints_xxz[i] += faa_xxz * ((1.0 / 2.0) * fe_0 * rpa_z + rpa_z * rpb_y * rpb_y);

        fints_xyy[i] += faa_x * (1.0 / 2.0) * fe_0 * fe_0 * rpa_z;

        fints_xyy[i] += faa_xy * 2.0 * fe_0 * rpa_z * rpb_y;

        fints_xyy[i] += faa_xyy * ((1.0 / 2.0) * fe_0 * rpa_z + rpa_z * rpb_y * rpb_y);

        fints_xyz[i] += faa_x * (1.0 / 2.0) * fe_0 * fe_0 * rpb_y;

        fints_xyz[i] += faa_xz * fe_0 * rpa_z * rpb_y;

        fints_xyz[i] += faa_xy * ((1.0 / 2.0) * fe_0 * rpb_y * rpb_y + (1.0 / 4.0) * fe_0 * fe_0);

        fints_xyz[i] += faa_xyz * ((1.0 / 2.0) * fe_0 * rpa_z + rpa_z * rpb_y * rpb_y);

        fints_xzz[i] += faa_xz * (fe_0 * rpb_y * rpb_y + (1.0 / 2.0) * fe_0 * fe_0);

        fints_xzz[i] += faa_xzz * ((1.0 / 2.0) * fe_0 * rpa_z + rpa_z * rpb_y * rpb_y);

        fints_yyy[i] += faa_y * (3.0 / 2.0) * fe_0 * fe_0 * rpa_z;

        fints_yyy[i] += faa_yy * 3.0 * fe_0 * rpa_z * rpb_y;

        fints_yyy[i] += faa_yyy * ((1.0 / 2.0) * fe_0 * rpa_z + rpa_z * rpb_y * rpb_y);
        ...
        fints_yyz[i] += faa_yz * 2.0 * fe_0 * rpa_z * rpb_y;

        fints_yyz[i] += faa_yy * ((1.0 / 2.0) * fe_0 * rpb_y * rpb_y + (1.0 / 4.0) * fe_0 * fe_0);

        fints_yyz[i] += faa_yyz * ((1.0 / 2.0) * fe_0 * rpa_z + rpa_z * rpb_y * rpb_y);

        fints_yzz[i] += faa_z * fe_0 * fe_0 * rpb_y;

        fints_yzz[i] += faa_zz * fe_0 * rpa_z * rpb_y;

        fints_yzz[i] += faa_yz * (fe_0 * rpb_y * rpb_y + (1.0 / 2.0) * fe_0 * fe_0);

        fints_yzz[i] += faa_yzz * ((1.0 / 2.0) * fe_0 * rpa_z + rpa_z * rpb_y * rpb_y);

        fints_zzz[i] += faa_zz * ((3.0 / 2.0) * fe_0 * rpb_y * rpb_y + (3.0 / 4.0) * fe_0 * fe_0);

        fints_zzz[i] += faa_zzz * ((1.0 / 2.0) * fe_0 * rpa_z + rpa_z * rpb_y * rpb_y);
    }
}
