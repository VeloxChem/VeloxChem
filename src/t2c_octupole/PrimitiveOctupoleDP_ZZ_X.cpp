...
#pragma omp simd aligned(fints_xxx,     \
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xxx[i] += faa_xxx * ((1.0 / 2.0) * fe_0 * rpb_x + rpa_z * rpa_z * rpb_x);

        fints_xxy[i] += faa_xy * (fe_0 * rpa_z * rpa_z + (1.0 / 2.0) * fe_0 * fe_0);

        fints_xxy[i] += faa_xxy * ((1.0 / 2.0) * fe_0 * rpb_x + rpa_z * rpa_z * rpb_x);

        fints_xxz[i] += faa_x * fe_0 * fe_0 * rpa_z;

        fints_xxz[i] += faa_xz * (fe_0 * rpa_z * rpa_z + (1.0 / 2.0) * fe_0 * fe_0);

        fints_xxz[i] += faa_xx * fe_0 * rpa_z * rpb_x;

        fints_xxz[i] += faa_xxz * ((1.0 / 2.0) * fe_0 * rpb_x + rpa_z * rpa_z * rpb_x);

        fints_xyy[i] += faa_yy * ((1.0 / 2.0) * fe_0 * rpa_z * rpa_z + (1.0 / 4.0) * fe_0 * fe_0);

        fints_xyy[i] += faa_xyy * ((1.0 / 2.0) * fe_0 * rpb_x + rpa_z * rpa_z * rpb_x);

        fints_xyz[i] += faa_y * (1.0 / 2.0) * fe_0 * fe_0 * rpa_z;

        fints_xyz[i] += faa_yz * ((1.0 / 2.0) * fe_0 * rpa_z * rpa_z + (1.0 / 4.0) * fe_0 * fe_0);

        fints_xyz[i] += faa_xy * fe_0 * rpa_z * rpb_x;

        fints_xyz[i] += faa_xyz * ((1.0 / 2.0) * fe_0 * rpb_x + rpa_z * rpa_z * rpb_x);
        ...
        fints_xzz[i] += faa_xz * 2.0 * fe_0 * rpa_z * rpb_x;

        fints_xzz[i] += faa_xzz * ((1.0 / 2.0) * fe_0 * rpb_x + rpa_z * rpa_z * rpb_x);

        fints_yyy[i] += faa_yyy * ((1.0 / 2.0) * fe_0 * rpb_x + rpa_z * rpa_z * rpb_x);

        fints_yyz[i] += faa_yy * fe_0 * rpa_z * rpb_x;

        fints_yyz[i] += faa_yyz * ((1.0 / 2.0) * fe_0 * rpb_x + rpa_z * rpa_z * rpb_x);

        fints_yzz[i] += faa_y * (1.0 / 2.0) * fe_0 * fe_0 * rpb_x;

        fints_yzz[i] += faa_yz * 2.0 * fe_0 * rpa_z * rpb_x;

        fints_yzz[i] += faa_yzz * ((1.0 / 2.0) * fe_0 * rpb_x + rpa_z * rpa_z * rpb_x);

        fints_zzz[i] += faa_z * (3.0 / 2.0) * fe_0 * fe_0 * rpb_x;

        fints_zzz[i] += faa_zz * 3.0 * fe_0 * rpa_z * rpb_x;

        fints_zzz[i] += faa_zzz * ((1.0 / 2.0) * fe_0 * rpb_x + rpa_z * rpa_z * rpb_x);
    }
}
