...
auto
compPrimitiveOctupolePP_X_Z(TDoubleArray&       buffer_xxx,
...
#pragma omp simd aligned(fints_xxx,     \
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xxx[i] += faa_xx * (3.0 / 2.0) * fe_0 * rpb_z;

        fints_xxx[i] += faa_xxx * rpa_x * rpb_z;

        fints_xxy[i] += faa_xy * fe_0 * rpb_z;

        fints_xxy[i] += faa_xxy * rpa_x * rpb_z;

        fints_xxz[i] += faa_x * (1.0 / 2.0) * fe_0 * fe_0;

        fints_xxz[i] += faa_xz * fe_0 * rpb_z;

        fints_xxz[i] += faa_xx * (1.0 / 2.0) * fe_0 * rpa_x;

        fints_xxz[i] += faa_xxz * rpa_x * rpb_z;

        fints_xyy[i] += faa_yy * (1.0 / 2.0) * fe_0 * rpb_z;

        fints_xyy[i] += faa_xyy * rpa_x * rpb_z;

        fints_xyz[i] += faa_y * (1.0 / 4.0) * fe_0 * fe_0;

        fints_xyz[i] += faa_yz * (1.0 / 2.0) * fe_0 * rpb_z;

        fints_xyz[i] += faa_xy * (1.0 / 2.0) * fe_0 * rpa_x;

        fints_xyz[i] += faa_xyz * rpa_x * rpb_z;

        fints_xzz[i] += faa_z * (1.0 / 2.0) * fe_0 * fe_0;

        fints_xzz[i] += faa_zz * (1.0 / 2.0) * fe_0 * rpb_z;

        fints_xzz[i] += faa_xz * fe_0 * rpa_x;

        fints_xzz[i] += faa_xzz * rpa_x * rpb_z;

        fints_yyy[i] += faa_yyy * rpa_x * rpb_z;

        fints_yyz[i] += faa_yy * (1.0 / 2.0) * fe_0 * rpa_x;

        fints_yyz[i] += faa_yyz * rpa_x * rpb_z;

        fints_yzz[i] += faa_yz * fe_0 * rpa_x;

        fints_yzz[i] += faa_yzz * rpa_x * rpb_z;

        fints_zzz[i] += faa_zz * (3.0 / 2.0) * fe_0 * rpa_x;

        fints_zzz[i] += faa_zzz * rpa_x * rpb_z;
    }
}
