...
auto
compPrimitiveOctupolePS_Z_0(TDoubleArray&       buffer_xxx,
...
#pragma omp simd aligned(fints_xxx,     \
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xxx[i] += faa_xxx * rpa_z;

        fints_xxy[i] += faa_xxy * rpa_z;

        fints_xxz[i] += faa_xx * (1.0 / 2.0) * fe_0;

        fints_xxz[i] += faa_xxz * rpa_z;

        fints_xyy[i] += faa_xyy * rpa_z;

        fints_xyz[i] += faa_xy * (1.0 / 2.0) * fe_0;

        fints_xyz[i] += faa_xyz * rpa_z;

        fints_xzz[i] += faa_xz * fe_0;

        fints_xzz[i] += faa_xzz * rpa_z;

        fints_yyy[i] += faa_yyy * rpa_z;

        fints_yyz[i] += faa_yy * (1.0 / 2.0) * fe_0;

        fints_yyz[i] += faa_yyz * rpa_z;

        fints_yzz[i] += faa_yz * fe_0;

        fints_yzz[i] += faa_yzz * rpa_z;

        fints_zzz[i] += faa_zz * (3.0 / 2.0) * fe_0;

        fints_zzz[i] += faa_zzz * rpa_z;
    }
}
