...
auto
compPrimitiveOctupoleSD_0_ZZ(TDoubleArray&       buffer_xxx,
...
#pragma omp simd aligned(fints_xxx,     \
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xxx[i] += faa_xxx * ((1.0 / 2.0) * fe_0 + rpb_z * rpb_z);

        fints_xxy[i] += faa_xxy * ((1.0 / 2.0) * fe_0 + rpb_z * rpb_z);

        fints_xxz[i] += faa_xx * fe_0 * rpb_z;

        fints_xxz[i] += faa_xxz * ((1.0 / 2.0) * fe_0 + rpb_z * rpb_z);

        fints_xyy[i] += faa_xyy * ((1.0 / 2.0) * fe_0 + rpb_z * rpb_z);

        fints_xyz[i] += faa_xy * fe_0 * rpb_z;

        fints_xyz[i] += faa_xyz * ((1.0 / 2.0) * fe_0 + rpb_z * rpb_z);

        fints_xzz[i] += faa_x * (1.0 / 2.0) * fe_0 * fe_0;

        fints_xzz[i] += faa_xz * 2.0 * fe_0 * rpb_z;

        fints_xzz[i] += faa_xzz * ((1.0 / 2.0) * fe_0 + rpb_z * rpb_z);

        fints_yyy[i] += faa_yyy * ((1.0 / 2.0) * fe_0 + rpb_z * rpb_z);

        fints_yyz[i] += faa_yy * fe_0 * rpb_z;

        fints_yyz[i] += faa_yyz * ((1.0 / 2.0) * fe_0 + rpb_z * rpb_z);

        fints_yzz[i] += faa_y * (1.0 / 2.0) * fe_0 * fe_0;

        fints_yzz[i] += faa_yz * 2.0 * fe_0 * rpb_z;

        fints_yzz[i] += faa_yzz * ((1.0 / 2.0) * fe_0 + rpb_z * rpb_z);

        fints_zzz[i] += faa_z * (3.0 / 2.0) * fe_0 * fe_0;

        fints_zzz[i] += faa_zz * 3.0 * fe_0 * rpb_z;

        fints_zzz[i] += faa_zzz * ((1.0 / 2.0) * fe_0 + rpb_z * rpb_z);
    }
}
