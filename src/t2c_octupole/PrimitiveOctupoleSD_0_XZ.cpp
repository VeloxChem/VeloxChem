                             ...
                             const int64_t       ket_dim) -> void
{
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xxx[i] += faa_xxx * rpb_z * rpb_x;

        fints_xxy[i] += faa_xy * fe_0 * rpb_z;

        fints_xxy[i] += faa_xxy * rpb_z * rpb_x;

        fints_xxz[i] += faa_x * (1.0 / 2.0) * fe_0 * fe_0;

        fints_xxz[i] += faa_xz * fe_0 * rpb_z;

        fints_xxz[i] += faa_xx * (1.0 / 2.0) * fe_0 * rpb_x;

        fints_xxz[i] += faa_xxz * rpb_z * rpb_x;

        fints_xyy[i] += faa_yy * (1.0 / 2.0) * fe_0 * rpb_z;

        fints_xyy[i] += faa_xyy * rpb_z * rpb_x;

        fints_xyz[i] += faa_y * (1.0 / 4.0) * fe_0 * fe_0;

        fints_xyz[i] += faa_yz * (1.0 / 2.0) * fe_0 * rpb_z;

        fints_xyz[i] += faa_xy * (1.0 / 2.0) * fe_0 * rpb_x;

        fints_xyz[i] += faa_xyz * rpb_z * rpb_x;

        fints_xzz[i] += faa_z * (1.0 / 2.0) * fe_0 * fe_0;

        fints_xzz[i] += faa_zz * (1.0 / 2.0) * fe_0 * rpb_z;

        fints_xzz[i] += faa_xz * fe_0 * rpb_x;

        fints_xzz[i] += faa_xzz * rpb_z * rpb_x;

        fints_yyy[i] += faa_yyy * rpb_z * rpb_x;

        fints_yyz[i] += faa_yy * (1.0 / 2.0) * fe_0 * rpb_x;

        fints_yyz[i] += faa_yyz * rpb_z * rpb_x;

        fints_yzz[i] += faa_yz * fe_0 * rpb_x;

        fints_yzz[i] += faa_yzz * rpb_z * rpb_x;

        fints_zzz[i] += faa_zz * (3.0 / 2.0) * fe_0 * rpb_x;

        fints_zzz[i] += faa_zzz * rpb_z * rpb_x;
