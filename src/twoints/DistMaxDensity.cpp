...
namespace distmaxden { // distmaxden namespace
    ...
    void
    getMaxUnrestDenJ(      CMemBlock<double>& maxDensityElements,
                     const double*            densityMatrixAlpha,
                     const double*            densityMatrixBeta,
                     const int32_t            nDensityColumns,
                     const CGtoPairsBlock&    braGtoPairsBlock,
    ...
    void
    getMaxUnrestDenJKX(      CMemBlock<double>&   maxDensityElements,
                       const double*              densityMatrixAlpha,
                       const double*              densityMatrixBeta,
                       const int32_t              nDensityColumns,
                       const double               exchangeFactor,
                ...
                for (int32_t k = 0; k < ccomp; k++)
                {
                    ...
                    for (int32_t l = 0; l < dcomp; l++)
                    {
                        ...
                        for (int32_t m = 0; m < nKetContrPairs; m++)
                        {
                            ...
                            cden = exchangeFactor * std::fabs(densityMatrixAlpha[idq * nDensityColumns + ids]);
                            
                            if (cden > mden) mden = cden;
                            
                            cden = exchangeFactor * std::fabs(densityMatrixAlpha[idq * nDensityColumns + idr]);
                            
                            if (cden > mden) mden = cden;
                            
                            cden = exchangeFactor * std::fabs(densityMatrixAlpha[idp * nDensityColumns + ids]);
                            
                            if (cden > mden) mden = cden;
                            
                            cden = exchangeFactor * std::fabs(densityMatrixAlpha[idp * nDensityColumns + idr]);
                            
                            if (cden > mden) mden = cden;
                            
                            cden = exchangeFactor * std::fabs(densityMatrixBeta[idq * nDensityColumns + ids]);
                            
                            if (cden > mden) mden = cden;
                            
                            cden = exchangeFactor * std::fabs(densityMatrixBeta[idq * nDensityColumns + idr]);
                            
                            if (cden > mden) mden = cden;
                            
                            cden = exchangeFactor * std::fabs(densityMatrixBeta[idp * nDensityColumns + ids]);
                            
                            if (cden > mden) mden = cden;
                            
                            cden = exchangeFactor * std::fabs(densityMatrixBeta[idp * nDensityColumns + idr]);
