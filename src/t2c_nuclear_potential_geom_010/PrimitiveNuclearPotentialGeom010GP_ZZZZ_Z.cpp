...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_y[i] += faa_y * b4_vals[i] * (-6.0 * rpa_z * rpa_z * rpc_z * rpc_z * rpc_z);

        fints_y[i] += faa_y * b5_vals[i] *
                      (5.0 * fe_0 * rpc_z * rpc_z * rpc_z + 4.0 * rpa_z * rpc_z * rpc_z * rpc_z * rpc_z + rpb_z * rpc_z * rpc_z * rpc_z * rpc_z);

        fints_y[i] += faa_y * b6_vals[i] * (-rpc_z * rpc_z * rpc_z * rpc_z * rpc_z);

        fints_z[i] += dip_z * fss * b1_vals[i] *
                      (6.0 * fe_0 * rpa_z * rpb_z + 9.0 * fe_0 * rpa_z * rpa_z + (15.0 / 4.0) * fe_0 * fe_0 + 4.0 * rpa_z * rpa_z * rpa_z * rpb_z +
                       rpa_z * rpa_z * rpa_z * rpa_z);

        fints_z[i] += dip_z * fss * b2_vals[i] *
                      (-6.0 * fe_0 * rpa_z * rpb_z - 24.0 * fe_0 * rpa_z * rpc_z - 9.0 * fe_0 * rpa_z * rpa_z - 6.0 * fe_0 * rpb_z * rpc_z -
                       (15.0 / 2.0) * fe_0 * fe_0);

        fints_z[i] += dip_z * fss * b2_vals[i] * (-12.0 * rpa_z * rpa_z * rpb_z * rpc_z - 8.0 * rpa_z * rpa_z * rpa_z * rpc_z);

        fints_z[i] += dip_z * fss * b3_vals[i] *
                      (24.0 * fe_0 * rpa_z * rpc_z + 6.0 * fe_0 * rpb_z * rpc_z + 15.0 * fe_0 * rpc_z * rpc_z + (15.0 / 4.0) * fe_0 * fe_0 +
                       12.0 * rpa_z * rpb_z * rpc_z * rpc_z);

        fints_z[i] += dip_z * fss * b3_vals[i] * 18.0 * rpa_z * rpa_z * rpc_z * rpc_z;

        fints_z[i] +=
            dip_z * fss * b4_vals[i] * (-15.0 * fe_0 * rpc_z * rpc_z - 16.0 * rpa_z * rpc_z * rpc_z * rpc_z - 4.0 * rpb_z * rpc_z * rpc_z * rpc_z);
        ...
        fints_z[i] += faa_z * b4_vals[i] * (-6.0 * rpa_z * rpa_z * rpc_z * rpc_z * rpc_z);

        fints_z[i] += faa_z * b5_vals[i] *
                      (5.0 * fe_0 * rpc_z * rpc_z * rpc_z + 4.0 * rpa_z * rpc_z * rpc_z * rpc_z * rpc_z + rpb_z * rpc_z * rpc_z * rpc_z * rpc_z);

        fints_z[i] += faa_z * b6_vals[i] * (-rpc_z * rpc_z * rpc_z * rpc_z * rpc_z);
    }
}
