...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += faa_x * b2_vals[i] * (-(3.0 / 2.0) * fe_0 * fe_0 - 2.0 * rpa_z * rpb_z * rpb_z * rpc_z - 2.0 * rpa_z * rpa_z * rpb_z * rpc_z);
        ...
        fints_x[i] += faa_x * b3_vals[i] * (rpa_z * rpa_z * rpc_z * rpc_z + rpb_z * rpb_z * rpc_z * rpc_z);

        fints_x[i] += faa_x * b4_vals[i] * (-3.0 * fe_0 * rpc_z * rpc_z - 2.0 * rpa_z * rpc_z * rpc_z * rpc_z - 2.0 * rpb_z * rpc_z * rpc_z * rpc_z);

        fints_x[i] += faa_x * b5_vals[i] * rpc_z * rpc_z * rpc_z * rpc_z;
        ...
        fints_y[i] += faa_y * b2_vals[i] * (-(3.0 / 2.0) * fe_0 * fe_0 - 2.0 * rpa_z * rpb_z * rpb_z * rpc_z - 2.0 * rpa_z * rpa_z * rpb_z * rpc_z);

        fints_y[i] += faa_y * b3_vals[i] *
                      (3.0 * fe_0 * rpa_z * rpc_z + 3.0 * fe_0 * rpb_z * rpc_z + 3.0 * fe_0 * rpc_z * rpc_z + (3.0 / 4.0) * fe_0 * fe_0 +
                       4.0 * rpa_z * rpb_z * rpc_z * rpc_z);

        fints_y[i] += faa_y * b3_vals[i] * (rpa_z * rpa_z * rpc_z * rpc_z + rpb_z * rpb_z * rpc_z * rpc_z);

        fints_y[i] += faa_y * b4_vals[i] * (-3.0 * fe_0 * rpc_z * rpc_z - 2.0 * rpa_z * rpc_z * rpc_z * rpc_z - 2.0 * rpb_z * rpc_z * rpc_z * rpc_z);

        fints_y[i] += faa_y * b5_vals[i] * rpc_z * rpc_z * rpc_z * rpc_z;

        fints_z[i] +=
            dip_z * fss * b1_vals[i] * (3.0 * fe_0 * rpa_z + 3.0 * fe_0 * rpb_z + 2.0 * rpa_z * rpb_z * rpb_z + 2.0 * rpa_z * rpa_z * rpb_z);

        fints_z[i] += dip_z * fss * b2_vals[i] *
                      (-3.0 * fe_0 * rpa_z - 3.0 * fe_0 * rpb_z - 6.0 * fe_0 * rpc_z - 8.0 * rpa_z * rpb_z * rpc_z - 2.0 * rpa_z * rpa_z * rpc_z);
        ...
        fints_z[i] += faa_z * b2_vals[i] * (-(3.0 / 2.0) * fe_0 * fe_0 - 2.0 * rpa_z * rpb_z * rpb_z * rpc_z - 2.0 * rpa_z * rpa_z * rpb_z * rpc_z);

        fints_z[i] += faa_z * b3_vals[i] *
                      (3.0 * fe_0 * rpa_z * rpc_z + 3.0 * fe_0 * rpb_z * rpc_z + 3.0 * fe_0 * rpc_z * rpc_z + (3.0 / 4.0) * fe_0 * fe_0 +
                       4.0 * rpa_z * rpb_z * rpc_z * rpc_z);

        fints_z[i] += faa_z * b3_vals[i] * (rpa_z * rpa_z * rpc_z * rpc_z + rpb_z * rpb_z * rpc_z * rpc_z);

        fints_z[i] += faa_z * b4_vals[i] * (-3.0 * fe_0 * rpc_z * rpc_z - 2.0 * rpa_z * rpc_z * rpc_z * rpc_z - 2.0 * rpb_z * rpc_z * rpc_z * rpc_z);

        fints_z[i] += faa_z * b5_vals[i] * rpc_z * rpc_z * rpc_z * rpc_z;
    }
}
