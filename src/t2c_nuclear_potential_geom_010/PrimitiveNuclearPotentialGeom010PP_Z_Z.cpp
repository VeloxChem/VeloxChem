#include "PrimitiveNuclearPotentialGeom010PP_Z_Z.hpp"
...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += faa_x * b1_vals[i] * ((1.0 / 2.0) * fe_0 + rpa_z * rpb_z);

        fints_x[i] += faa_x * b2_vals[i] * (-(1.0 / 2.0) * fe_0 - rpa_z * rpc_z - rpb_z * rpc_z);

        fints_x[i] += faa_x * b3_vals[i] * rpc_z * rpc_z;

        fints_y[i] += faa_y * b1_vals[i] * ((1.0 / 2.0) * fe_0 + rpa_z * rpb_z);

        fints_y[i] += faa_y * b2_vals[i] * (-(1.0 / 2.0) * fe_0 - rpa_z * rpc_z - rpb_z * rpc_z);

        fints_y[i] += faa_y * b3_vals[i] * rpc_z * rpc_z;

        fints_z[i] += dip_z * fss * b1_vals[i] * (rpa_z + rpb_z);

        fints_z[i] += dip_z * fss * b2_vals[i] * (-2.0 * rpc_z);

        fints_z[i] += faa_z * b1_vals[i] * ((1.0 / 2.0) * fe_0 + rpa_z * rpb_z);

        fints_z[i] += faa_z * b2_vals[i] * (-(1.0 / 2.0) * fe_0 - rpa_z * rpc_z - rpb_z * rpc_z);

        fints_z[i] += faa_z * b3_vals[i] * rpc_z * rpc_z;
    }
}
