...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_y[i] += dip_y * fss * b1_vals[i] *
                      ((9.0 / 2.0) * fe_0 * rpa_z * rpa_y + (3.0 / 2.0) * fe_0 * rpa_z * rpb_y + 3.0 * rpa_z * rpa_y * rpa_y * rpb_y +
                       rpa_z * rpa_y * rpa_y * rpa_y);

        fints_y[i] += dip_y * fss * b2_vals[i] *
                      (-(9.0 / 2.0) * fe_0 * rpa_z * rpa_y - (3.0 / 2.0) * fe_0 * rpa_z * rpb_y - 6.0 * fe_0 * rpa_z * rpc_y -
                       (9.0 / 2.0) * fe_0 * rpa_y * rpc_z - (3.0 / 2.0) * fe_0 * rpb_y * rpc_z);

        fints_y[i] += dip_y * fss * b2_vals[i] *
                      (-6.0 * rpa_z * rpa_y * rpb_y * rpc_y - 6.0 * rpa_z * rpa_y * rpa_y * rpc_y - 3.0 * rpa_y * rpa_y * rpb_y * rpc_z -
                       rpa_y * rpa_y * rpa_y * rpc_z);

        fints_y[i] += dip_y * fss * b3_vals[i] *
                      (6.0 * fe_0 * rpa_z * rpc_y + (9.0 / 2.0) * fe_0 * rpa_y * rpc_z + (3.0 / 2.0) * fe_0 * rpb_y * rpc_z +
                       6.0 * fe_0 * rpc_z * rpc_y + 9.0 * rpa_z * rpa_y * rpc_y * rpc_y);

        fints_y[i] += dip_y * fss * b3_vals[i] *
                      (3.0 * rpa_z * rpb_y * rpc_y * rpc_y + 6.0 * rpa_y * rpb_y * rpc_z * rpc_y + 6.0 * rpa_y * rpa_y * rpc_z * rpc_y);

        fints_y[i] += dip_y * fss * b4_vals[i] *
                      (-6.0 * fe_0 * rpc_z * rpc_y - 4.0 * rpa_z * rpc_y * rpc_y * rpc_y - 9.0 * rpa_y * rpc_z * rpc_y * rpc_y -
