...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += faa_x * b1_vals[i] * ((1.0 / 2.0) * fe_0 * rpb_z + rpb_z * rpb_y * rpb_y);

        fints_x[i] +=
            faa_x * b2_vals[i] * (-(1.0 / 2.0) * fe_0 * rpb_z - (1.0 / 2.0) * fe_0 * rpc_z - 2.0 * rpb_z * rpb_y * rpc_y - rpb_y * rpb_y * rpc_z);

        fints_x[i] += faa_x * b3_vals[i] * ((1.0 / 2.0) * fe_0 * rpc_z + rpb_z * rpc_y * rpc_y + 2.0 * rpb_y * rpc_z * rpc_y);
        ...
        fints_y[i] += faa_y * b1_vals[i] * ((1.0 / 2.0) * fe_0 * rpb_z + rpb_z * rpb_y * rpb_y);

        fints_y[i] +=
            faa_y * b2_vals[i] * (-(1.0 / 2.0) * fe_0 * rpb_z - (1.0 / 2.0) * fe_0 * rpc_z - 2.0 * rpb_z * rpb_y * rpc_y - rpb_y * rpb_y * rpc_z);

        fints_y[i] += faa_y * b3_vals[i] * ((1.0 / 2.0) * fe_0 * rpc_z + rpb_z * rpc_y * rpc_y + 2.0 * rpb_y * rpc_z * rpc_y);
        ...
        fints_z[i] += faa_z * b1_vals[i] * ((1.0 / 2.0) * fe_0 * rpb_z + rpb_z * rpb_y * rpb_y);

        fints_z[i] +=
            faa_z * b2_vals[i] * (-(1.0 / 2.0) * fe_0 * rpb_z - (1.0 / 2.0) * fe_0 * rpc_z - 2.0 * rpb_z * rpb_y * rpc_y - rpb_y * rpb_y * rpc_z);

        fints_z[i] += faa_z * b3_vals[i] * ((1.0 / 2.0) * fe_0 * rpc_z + rpb_z * rpc_y * rpc_y + 2.0 * rpb_y * rpc_z * rpc_y);
