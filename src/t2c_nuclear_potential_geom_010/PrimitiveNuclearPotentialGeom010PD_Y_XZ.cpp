#include "PrimitiveNuclearPotentialGeom010PD_Y_XZ.hpp"
...
auto
compPrimitiveNuclearPotentialGeom010PD_Y_XZ(TDoubleArray&       buffer_x,
...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += dip_x * fss * b1_vals[i] * rpa_y * rpb_z;

        fints_x[i] += dip_x * fss * b2_vals[i] * (-rpa_y * rpc_z - rpb_z * rpc_y);

        fints_x[i] += dip_x * fss * b3_vals[i] * rpc_z * rpc_y;

        fints_x[i] += faa_x * b1_vals[i] * rpa_y * rpb_z * rpb_x;

        fints_x[i] += faa_x * b2_vals[i] * (-rpa_y * rpb_z * rpc_x - rpa_y * rpb_x * rpc_z - rpb_z * rpb_x * rpc_y);

        fints_x[i] += faa_x * b3_vals[i] * (rpa_y * rpc_z * rpc_x + rpb_z * rpc_y * rpc_x + rpb_x * rpc_z * rpc_y);

        fints_x[i] += faa_x * b4_vals[i] * (-rpc_z * rpc_y * rpc_x);
        ...
        fints_y[i] += faa_y * b1_vals[i] * rpa_y * rpb_z * rpb_x;

        fints_y[i] += faa_y * b2_vals[i] * (-rpa_y * rpb_z * rpc_x - rpa_y * rpb_x * rpc_z - rpb_z * rpb_x * rpc_y);

        fints_y[i] += faa_y * b3_vals[i] * (rpa_y * rpc_z * rpc_x + rpb_z * rpc_y * rpc_x + rpb_x * rpc_z * rpc_y);

        fints_y[i] += faa_y * b4_vals[i] * (-rpc_z * rpc_y * rpc_x);

        fints_z[i] += dip_z * fss * b1_vals[i] * rpa_y * rpb_x;

        fints_z[i] += dip_z * fss * b2_vals[i] * (-rpa_y * rpc_x - rpb_x * rpc_y);

        fints_z[i] += dip_z * fss * b3_vals[i] * rpc_y * rpc_x;

        fints_z[i] += faa_z * b1_vals[i] * rpa_y * rpb_z * rpb_x;

        fints_z[i] += faa_z * b2_vals[i] * (-rpa_y * rpb_z * rpc_x - rpa_y * rpb_x * rpc_z - rpb_z * rpb_x * rpc_y);

        fints_z[i] += faa_z * b3_vals[i] * (rpa_y * rpc_z * rpc_x + rpb_z * rpc_y * rpc_x + rpb_x * rpc_z * rpc_y);

        fints_z[i] += faa_z * b4_vals[i] * (-rpc_z * rpc_y * rpc_x);
    }
}
