...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_y[i] +=
            dip_y * fss * b3_vals[i] * (fe_0 * rpc_y + 2.0 * rpa_z * rpc_z * rpc_y + 2.0 * rpb_z * rpc_z * rpc_y + 2.0 * rpb_y * rpc_z * rpc_z);
        ...
        fints_z[i] +=
            dip_z * fss * b1_vals[i] * ((1.0 / 2.0) * fe_0 * rpa_z + (1.0 / 2.0) * fe_0 * rpb_z + rpa_z * rpb_y * rpb_y + rpb_z * rpb_y * rpb_y);
