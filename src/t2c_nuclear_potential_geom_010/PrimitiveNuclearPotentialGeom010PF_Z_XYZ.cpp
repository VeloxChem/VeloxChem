...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += faa_x * b1_vals[i] * ((1.0 / 2.0) * fe_0 * rpb_y * rpb_x + rpa_z * rpb_z * rpb_y * rpb_x);
        ...
        fints_x[i] += faa_x * b2_vals[i] * (-rpa_z * rpb_y * rpb_x * rpc_z - rpb_z * rpb_y * rpb_x * rpc_z);
        ...
        fints_y[i] += faa_y * b1_vals[i] * ((1.0 / 2.0) * fe_0 * rpb_y * rpb_x + rpa_z * rpb_z * rpb_y * rpb_x);

        fints_y[i] += faa_y * b2_vals[i] *
                      (-(1.0 / 2.0) * fe_0 * rpb_y * rpb_x - (1.0 / 2.0) * fe_0 * rpb_y * rpc_x - (1.0 / 2.0) * fe_0 * rpb_x * rpc_y -
                       rpa_z * rpb_z * rpb_y * rpc_x - rpa_z * rpb_z * rpb_x * rpc_y);

        fints_y[i] += faa_y * b2_vals[i] * (-rpa_z * rpb_y * rpb_x * rpc_z - rpb_z * rpb_y * rpb_x * rpc_z);
        ...
        fints_z[i] +=
            dip_z * fss * b3_vals[i] * (rpa_z * rpc_y * rpc_x + rpb_z * rpc_y * rpc_x + 2.0 * rpb_y * rpc_z * rpc_x + 2.0 * rpb_x * rpc_z * rpc_y);

        fints_z[i] += dip_z * fss * b4_vals[i] * (-2.0 * rpc_z * rpc_y * rpc_x);

        fints_z[i] += faa_z * b1_vals[i] * ((1.0 / 2.0) * fe_0 * rpb_y * rpb_x + rpa_z * rpb_z * rpb_y * rpb_x);

        fints_z[i] += faa_z * b2_vals[i] *
                      (-(1.0 / 2.0) * fe_0 * rpb_y * rpb_x - (1.0 / 2.0) * fe_0 * rpb_y * rpc_x - (1.0 / 2.0) * fe_0 * rpb_x * rpc_y -
                       rpa_z * rpb_z * rpb_y * rpc_x - rpa_z * rpb_z * rpb_x * rpc_y);

        fints_z[i] += faa_z * b2_vals[i] * (-rpa_z * rpb_y * rpb_x * rpc_z - rpb_z * rpb_y * rpb_x * rpc_z);
