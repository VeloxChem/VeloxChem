...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += faa_x * b1_vals[i] * ((1.0 / 2.0) * fe_0 * rpb_y + rpa_x * rpa_x * rpb_y);

        fints_x[i] +=
            faa_x * b2_vals[i] * (-(1.0 / 2.0) * fe_0 * rpb_y - (1.0 / 2.0) * fe_0 * rpc_y - 2.0 * rpa_x * rpb_y * rpc_x - rpa_x * rpa_x * rpc_y);

        fints_x[i] += faa_x * b3_vals[i] * ((1.0 / 2.0) * fe_0 * rpc_y + 2.0 * rpa_x * rpc_y * rpc_x + rpb_y * rpc_x * rpc_x);
        ...
        fints_y[i] += faa_y * b1_vals[i] * ((1.0 / 2.0) * fe_0 * rpb_y + rpa_x * rpa_x * rpb_y);

        fints_y[i] +=
            faa_y * b2_vals[i] * (-(1.0 / 2.0) * fe_0 * rpb_y - (1.0 / 2.0) * fe_0 * rpc_y - 2.0 * rpa_x * rpb_y * rpc_x - rpa_x * rpa_x * rpc_y);

        fints_y[i] += faa_y * b3_vals[i] * ((1.0 / 2.0) * fe_0 * rpc_y + 2.0 * rpa_x * rpc_y * rpc_x + rpb_y * rpc_x * rpc_x);

        fints_y[i] += faa_y * b4_vals[i] * (-rpc_y * rpc_x * rpc_x);

        fints_z[i] += faa_z * b1_vals[i] * ((1.0 / 2.0) * fe_0 * rpb_y + rpa_x * rpa_x * rpb_y);

        fints_z[i] +=
            faa_z * b2_vals[i] * (-(1.0 / 2.0) * fe_0 * rpb_y - (1.0 / 2.0) * fe_0 * rpc_y - 2.0 * rpa_x * rpb_y * rpc_x - rpa_x * rpa_x * rpc_y);

        fints_z[i] += faa_z * b3_vals[i] * ((1.0 / 2.0) * fe_0 * rpc_y + 2.0 * rpa_x * rpc_y * rpc_x + rpb_y * rpc_x * rpc_x);
