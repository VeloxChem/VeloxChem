#include "PrimitiveNuclearPotentialGeom010GF_YYYZ_YYZ.hpp"
...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += faa_x * b1_vals[i] * ((3.0 / 4.0) * fe_0 * fe_0 * fe_0 * rpb_y + rpa_z * rpa_y * rpa_y * rpa_y * rpb_z * rpb_y * rpb_y);
        ...
        fints_x[i] +=
            faa_x * b5_vals[i] * (rpa_y * rpa_y * rpa_y * rpc_z * rpc_z * rpc_y * rpc_y + rpb_z * rpb_y * rpb_y * rpc_z * rpc_y * rpc_y * rpc_y);
        ...
        fints_x[i] += faa_x * b7_vals[i] * 2.0 * rpb_y * rpc_z * rpc_z * rpc_y * rpc_y * rpc_y * rpc_y;

        fints_x[i] += faa_x * b8_vals[i] * (-rpc_z * rpc_z * rpc_y * rpc_y * rpc_y * rpc_y * rpc_y);

        fints_y[i] += dip_y * fss * b1_vals[i] *
                      (9.0 * fe_0 * rpa_z * rpa_y * rpb_z * rpb_y + (9.0 / 2.0) * fe_0 * rpa_z * rpa_y * rpa_y * rpb_z +
                       (3.0 / 2.0) * fe_0 * rpa_z * rpb_z * rpb_y * rpb_y + (3.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpb_y * rpb_y +
                       fe_0 * rpa_y * rpa_y * rpa_y * rpb_y);

        fints_y[i] += dip_y * fss * b1_vals[i] *
                      ((15.0 / 4.0) * fe_0 * fe_0 * rpa_z * rpb_z + (9.0 / 2.0) * fe_0 * fe_0 * rpa_y * rpb_y +
                       (9.0 / 4.0) * fe_0 * fe_0 * rpa_y * rpa_y + (3.0 / 4.0) * fe_0 * fe_0 * rpb_y * rpb_y + (15.0 / 8.0) * fe_0 * fe_0 * fe_0);

        fints_y[i] +=
            dip_y * fss * b1_vals[i] * (3.0 * rpa_z * rpa_y * rpa_y * rpb_z * rpb_y * rpb_y + 2.0 * rpa_z * rpa_y * rpa_y * rpa_y * rpb_z * rpb_y);

        fints_y[i] +=
            dip_y * fss * b2_vals[i] *
            (-9.0 * fe_0 * rpa_z * rpa_y * rpb_z * rpb_y - 18.0 * fe_0 * rpa_z * rpa_y * rpb_z * rpc_y - 9.0 * fe_0 * rpa_z * rpa_y * rpb_y * rpc_z -
             (9.0 / 2.0) * fe_0 * rpa_z * rpa_y * rpa_y * rpb_z - (9.0 / 2.0) * fe_0 * rpa_z * rpa_y * rpa_y * rpc_z);

        fints_y[i] += dip_y * fss * b2_vals[i] *
                      (-12.0 * fe_0 * rpa_z * rpb_z * rpb_y * rpc_y - (3.0 / 2.0) * fe_0 * rpa_z * rpb_z * rpb_y * rpb_y -
                       (3.0 / 2.0) * fe_0 * rpa_z * rpb_y * rpb_y * rpc_z - 9.0 * fe_0 * rpa_y * rpb_z * rpb_y * rpc_z -
                       3.0 * fe_0 * rpa_y * rpb_y * rpb_y * rpc_y);

        fints_y[i] +=
            dip_y * fss * b2_vals[i] *
            (-(9.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpb_z * rpc_z - 6.0 * fe_0 * rpa_y * rpa_y * rpb_y * rpc_y -
             (3.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpb_y * rpb_y - fe_0 * rpa_y * rpa_y * rpa_y * rpb_y - fe_0 * rpa_y * rpa_y * rpa_y * rpc_y);

        fints_y[i] += dip_y * fss * b2_vals[i] *
                      (-(3.0 / 2.0) * fe_0 * rpb_z * rpb_y * rpb_y * rpc_z - (15.0 / 2.0) * fe_0 * fe_0 * rpa_z * rpb_z -
                       (15.0 / 4.0) * fe_0 * fe_0 * rpa_z * rpc_z - 9.0 * fe_0 * fe_0 * rpa_y * rpb_y - 9.0 * fe_0 * fe_0 * rpa_y * rpc_y);

        fints_y[i] += dip_y * fss * b2_vals[i] *
                      (-(9.0 / 2.0) * fe_0 * fe_0 * rpa_y * rpa_y - (15.0 / 4.0) * fe_0 * fe_0 * rpb_z * rpc_z - 6.0 * fe_0 * fe_0 * rpb_y * rpc_y -
                       (3.0 / 2.0) * fe_0 * fe_0 * rpb_y * rpb_y - (45.0 / 8.0) * fe_0 * fe_0 * fe_0);

        fints_y[i] += dip_y * fss * b2_vals[i] *
                      (-6.0 * rpa_z * rpa_y * rpb_z * rpb_y * rpb_y * rpc_y - 12.0 * rpa_z * rpa_y * rpa_y * rpb_z * rpb_y * rpc_y -
                       3.0 * rpa_z * rpa_y * rpa_y * rpb_y * rpb_y * rpc_z - 2.0 * rpa_z * rpa_y * rpa_y * rpa_y * rpb_z * rpc_y -
                       2.0 * rpa_z * rpa_y * rpa_y * rpa_y * rpb_y * rpc_z);

        fints_y[i] +=
            dip_y * fss * b2_vals[i] * (-3.0 * rpa_y * rpa_y * rpb_z * rpb_y * rpb_y * rpc_z - 2.0 * rpa_y * rpa_y * rpa_y * rpb_z * rpb_y * rpc_z);

        fints_y[i] +=
            dip_y * fss * b3_vals[i] *
            (18.0 * fe_0 * rpa_z * rpa_y * rpb_z * rpc_y + 9.0 * fe_0 * rpa_z * rpa_y * rpb_y * rpc_z + 18.0 * fe_0 * rpa_z * rpa_y * rpc_z * rpc_y +
             (9.0 / 2.0) * fe_0 * rpa_z * rpa_y * rpa_y * rpc_z + 12.0 * fe_0 * rpa_z * rpb_z * rpb_y * rpc_y);

        fints_y[i] += dip_y * fss * b3_vals[i] *
                      (15.0 * fe_0 * rpa_z * rpb_z * rpc_y * rpc_y + 12.0 * fe_0 * rpa_z * rpb_y * rpc_z * rpc_y +
                       (3.0 / 2.0) * fe_0 * rpa_z * rpb_y * rpb_y * rpc_z + 9.0 * fe_0 * rpa_y * rpb_z * rpb_y * rpc_z +
                       18.0 * fe_0 * rpa_y * rpb_z * rpc_z * rpc_y);

        fints_y[i] +=
            dip_y * fss * b3_vals[i] *
            (9.0 * fe_0 * rpa_y * rpb_y * rpc_z * rpc_z + 9.0 * fe_0 * rpa_y * rpb_y * rpc_y * rpc_y + 3.0 * fe_0 * rpa_y * rpb_y * rpb_y * rpc_y +
             (9.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpb_z * rpc_z + 6.0 * fe_0 * rpa_y * rpa_y * rpb_y * rpc_y);

        fints_y[i] +=
            dip_y * fss * b3_vals[i] *
            ((9.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpc_z * rpc_z + (9.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpc_y * rpc_y +
             fe_0 * rpa_y * rpa_y * rpa_y * rpc_y + 12.0 * fe_0 * rpb_z * rpb_y * rpc_z * rpc_y + (3.0 / 2.0) * fe_0 * rpb_z * rpb_y * rpb_y * rpc_z);

        fints_y[i] +=
            dip_y * fss * b3_vals[i] *
            ((3.0 / 2.0) * fe_0 * rpb_y * rpb_y * rpc_z * rpc_z + (3.0 / 2.0) * fe_0 * rpb_y * rpb_y * rpc_y * rpc_y +
             (15.0 / 4.0) * fe_0 * fe_0 * rpa_z * rpb_z + (15.0 / 2.0) * fe_0 * fe_0 * rpa_z * rpc_z + (9.0 / 2.0) * fe_0 * fe_0 * rpa_y * rpb_y);

        fints_y[i] += dip_y * fss * b3_vals[i] *
                      (18.0 * fe_0 * fe_0 * rpa_y * rpc_y + (9.0 / 4.0) * fe_0 * fe_0 * rpa_y * rpa_y + (15.0 / 2.0) * fe_0 * fe_0 * rpb_z * rpc_z +
                       12.0 * fe_0 * fe_0 * rpb_y * rpc_y + (3.0 / 4.0) * fe_0 * fe_0 * rpb_y * rpb_y);

        fints_y[i] += dip_y * fss * b3_vals[i] *
                      ((15.0 / 4.0) * fe_0 * fe_0 * rpc_z * rpc_z + (15.0 / 2.0) * fe_0 * fe_0 * rpc_y * rpc_y + (45.0 / 8.0) * fe_0 * fe_0 * fe_0 +
                       18.0 * rpa_z * rpa_y * rpb_z * rpb_y * rpc_y * rpc_y + 6.0 * rpa_z * rpa_y * rpb_y * rpb_y * rpc_z * rpc_y);

        fints_y[i] += dip_y * fss * b3_vals[i] *
                      (9.0 * rpa_z * rpa_y * rpa_y * rpb_z * rpc_y * rpc_y + 12.0 * rpa_z * rpa_y * rpa_y * rpb_y * rpc_z * rpc_y +
                       2.0 * rpa_z * rpa_y * rpa_y * rpa_y * rpc_z * rpc_y + 3.0 * rpa_z * rpb_z * rpb_y * rpb_y * rpc_y * rpc_y +
                       6.0 * rpa_y * rpb_z * rpb_y * rpb_y * rpc_z * rpc_y);

        fints_y[i] += dip_y * fss * b3_vals[i] *
                      (12.0 * rpa_y * rpa_y * rpb_z * rpb_y * rpc_z * rpc_y + 3.0 * rpa_y * rpa_y * rpb_y * rpb_y * rpc_z * rpc_z +
                       2.0 * rpa_y * rpa_y * rpa_y * rpb_z * rpc_z * rpc_y + 2.0 * rpa_y * rpa_y * rpa_y * rpb_y * rpc_z * rpc_z);

        fints_y[i] +=
            dip_y * fss * b4_vals[i] *
            (-18.0 * fe_0 * rpa_z * rpa_y * rpc_z * rpc_y - 15.0 * fe_0 * rpa_z * rpb_z * rpc_y * rpc_y -
             12.0 * fe_0 * rpa_z * rpb_y * rpc_z * rpc_y - 15.0 * fe_0 * rpa_z * rpc_z * rpc_y * rpc_y - 18.0 * fe_0 * rpa_y * rpb_z * rpc_z * rpc_y);

        fints_y[i] +=
            dip_y * fss * b4_vals[i] *
            (-9.0 * fe_0 * rpa_y * rpb_y * rpc_z * rpc_z - 9.0 * fe_0 * rpa_y * rpb_y * rpc_y * rpc_y - 18.0 * fe_0 * rpa_y * rpc_z * rpc_z * rpc_y -
             6.0 * fe_0 * rpa_y * rpc_y * rpc_y * rpc_y - (9.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpc_z * rpc_z);

        fints_y[i] +=
            dip_y * fss * b4_vals[i] *
            (-(9.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpc_y * rpc_y - 12.0 * fe_0 * rpb_z * rpb_y * rpc_z * rpc_y -
             15.0 * fe_0 * rpb_z * rpc_z * rpc_y * rpc_y - 12.0 * fe_0 * rpb_y * rpc_z * rpc_z * rpc_y - 4.0 * fe_0 * rpb_y * rpc_y * rpc_y * rpc_y);

        fints_y[i] += dip_y * fss * b4_vals[i] *
                      (-(3.0 / 2.0) * fe_0 * rpb_y * rpb_y * rpc_z * rpc_z - (3.0 / 2.0) * fe_0 * rpb_y * rpb_y * rpc_y * rpc_y -
                       (15.0 / 4.0) * fe_0 * fe_0 * rpa_z * rpc_z - 9.0 * fe_0 * fe_0 * rpa_y * rpc_y - (15.0 / 4.0) * fe_0 * fe_0 * rpb_z * rpc_z);

        fints_y[i] += dip_y * fss * b4_vals[i] *
                      (-6.0 * fe_0 * fe_0 * rpb_y * rpc_y - (15.0 / 2.0) * fe_0 * fe_0 * rpc_z * rpc_z - 15.0 * fe_0 * fe_0 * rpc_y * rpc_y -
                       (15.0 / 8.0) * fe_0 * fe_0 * fe_0 - 12.0 * rpa_z * rpa_y * rpb_z * rpc_y * rpc_y * rpc_y);

        fints_y[i] += dip_y * fss * b4_vals[i] *
                      (-18.0 * rpa_z * rpa_y * rpb_y * rpc_z * rpc_y * rpc_y - 9.0 * rpa_z * rpa_y * rpa_y * rpc_z * rpc_y * rpc_y -
                       8.0 * rpa_z * rpb_z * rpb_y * rpc_y * rpc_y * rpc_y - 3.0 * rpa_z * rpb_y * rpb_y * rpc_z * rpc_y * rpc_y -
                       18.0 * rpa_y * rpb_z * rpb_y * rpc_z * rpc_y * rpc_y);

        fints_y[i] += dip_y * fss * b4_vals[i] *
                      (-6.0 * rpa_y * rpb_y * rpb_y * rpc_z * rpc_z * rpc_y - 9.0 * rpa_y * rpa_y * rpb_z * rpc_z * rpc_y * rpc_y -
                       12.0 * rpa_y * rpa_y * rpb_y * rpc_z * rpc_z * rpc_y - 2.0 * rpa_y * rpa_y * rpa_y * rpc_z * rpc_z * rpc_y -
                       3.0 * rpb_z * rpb_y * rpb_y * rpc_z * rpc_y * rpc_y);

        fints_y[i] +=
            dip_y * fss * b5_vals[i] *
            (15.0 * fe_0 * rpa_z * rpc_z * rpc_y * rpc_y + 18.0 * fe_0 * rpa_y * rpc_z * rpc_z * rpc_y + 6.0 * fe_0 * rpa_y * rpc_y * rpc_y * rpc_y +
             15.0 * fe_0 * rpb_z * rpc_z * rpc_y * rpc_y + 12.0 * fe_0 * rpb_y * rpc_z * rpc_z * rpc_y);

        fints_y[i] += dip_y * fss * b5_vals[i] *
                      (4.0 * fe_0 * rpb_y * rpc_y * rpc_y * rpc_y + 15.0 * fe_0 * rpc_z * rpc_z * rpc_y * rpc_y +
                       (5.0 / 2.0) * fe_0 * rpc_y * rpc_y * rpc_y * rpc_y + (15.0 / 4.0) * fe_0 * fe_0 * rpc_z * rpc_z +
                       (15.0 / 2.0) * fe_0 * fe_0 * rpc_y * rpc_y);

        fints_y[i] += dip_y * fss * b5_vals[i] *
                      (12.0 * rpa_z * rpa_y * rpc_z * rpc_y * rpc_y * rpc_y + 5.0 * rpa_z * rpb_z * rpc_y * rpc_y * rpc_y * rpc_y +
                       8.0 * rpa_z * rpb_y * rpc_z * rpc_y * rpc_y * rpc_y + 12.0 * rpa_y * rpb_z * rpc_z * rpc_y * rpc_y * rpc_y +
                       18.0 * rpa_y * rpb_y * rpc_z * rpc_z * rpc_y * rpc_y);

        fints_y[i] += dip_y * fss * b5_vals[i] *
                      (9.0 * rpa_y * rpa_y * rpc_z * rpc_z * rpc_y * rpc_y + 8.0 * rpb_z * rpb_y * rpc_z * rpc_y * rpc_y * rpc_y +
        ...
        fints_y[i] += dip_y * fss * b6_vals[i] *
                      (-15.0 * fe_0 * rpc_z * rpc_z * rpc_y * rpc_y - (5.0 / 2.0) * fe_0 * rpc_y * rpc_y * rpc_y * rpc_y -
                       5.0 * rpa_z * rpc_z * rpc_y * rpc_y * rpc_y * rpc_y - 12.0 * rpa_y * rpc_z * rpc_z * rpc_y * rpc_y * rpc_y -
                       5.0 * rpb_z * rpc_z * rpc_y * rpc_y * rpc_y * rpc_y);

        fints_y[i] += dip_y * fss * b6_vals[i] * (-8.0 * rpb_y * rpc_z * rpc_z * rpc_y * rpc_y * rpc_y);
        ...
        fints_y[i] += faa_y * b1_vals[i] * ((3.0 / 4.0) * fe_0 * fe_0 * fe_0 * rpb_y + rpa_z * rpa_y * rpa_y * rpa_y * rpb_z * rpb_y * rpb_y);
        ...
        fints_y[i] +=
            faa_y * b5_vals[i] * (rpa_y * rpa_y * rpa_y * rpc_z * rpc_z * rpc_y * rpc_y + rpb_z * rpb_y * rpb_y * rpc_z * rpc_y * rpc_y * rpc_y);
        ...
        fints_y[i] += faa_y * b7_vals[i] * 2.0 * rpb_y * rpc_z * rpc_z * rpc_y * rpc_y * rpc_y * rpc_y;
        ...
        fints_z[i] += dip_z * fss * b1_vals[i] *
                      ((3.0 / 2.0) * fe_0 * rpa_z * rpa_y * rpb_y * rpb_y + 3.0 * fe_0 * rpa_z * rpa_y * rpa_y * rpb_y +
                       (1.0 / 2.0) * fe_0 * rpa_z * rpa_y * rpa_y * rpa_y + (3.0 / 2.0) * fe_0 * rpa_y * rpb_z * rpb_y * rpb_y +
                       3.0 * fe_0 * rpa_y * rpa_y * rpb_z * rpb_y);

        fints_z[i] +=
            dip_z * fss * b1_vals[i] *
            ((1.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpa_y * rpb_z + (9.0 / 4.0) * fe_0 * fe_0 * rpa_z * rpa_y +
             (3.0 / 2.0) * fe_0 * fe_0 * rpa_z * rpb_y + (9.0 / 4.0) * fe_0 * fe_0 * rpa_y * rpb_z + (3.0 / 2.0) * fe_0 * fe_0 * rpb_z * rpb_y);

        fints_z[i] += dip_z * fss * b1_vals[i] * (rpa_z * rpa_y * rpa_y * rpa_y * rpb_y * rpb_y + rpa_y * rpa_y * rpa_y * rpb_z * rpb_y * rpb_y);
        ...
        fints_z[i] += dip_z * fss * b2_vals[i] *
                      (-(3.0 / 2.0) * fe_0 * rpa_z * rpb_y * rpb_y * rpc_y - 9.0 * fe_0 * rpa_y * rpb_z * rpb_y * rpc_y -
                       (3.0 / 2.0) * fe_0 * rpa_y * rpb_z * rpb_y * rpb_y - 3.0 * fe_0 * rpa_y * rpb_y * rpb_y * rpc_z -
                       3.0 * fe_0 * rpa_y * rpa_y * rpb_z * rpb_y);

        fints_z[i] += dip_z * fss * b2_vals[i] *
                      (-(9.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpb_z * rpc_y - 6.0 * fe_0 * rpa_y * rpa_y * rpb_y * rpc_z -
                       (1.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpa_y * rpb_z - fe_0 * rpa_y * rpa_y * rpa_y * rpc_z -
                       (3.0 / 2.0) * fe_0 * rpb_z * rpb_y * rpb_y * rpc_y);

        fints_z[i] += dip_z * fss * b2_vals[i] *
                      (-(9.0 / 2.0) * fe_0 * fe_0 * rpa_z * rpa_y - 3.0 * fe_0 * fe_0 * rpa_z * rpb_y - (15.0 / 4.0) * fe_0 * fe_0 * rpa_z * rpc_y -
                       (9.0 / 2.0) * fe_0 * fe_0 * rpa_y * rpb_z - (9.0 / 2.0) * fe_0 * fe_0 * rpa_y * rpc_z);

        fints_z[i] += dip_z * fss * b2_vals[i] *
                      (-3.0 * fe_0 * fe_0 * rpb_z * rpb_y - (15.0 / 4.0) * fe_0 * fe_0 * rpb_z * rpc_y - 3.0 * fe_0 * fe_0 * rpb_y * rpc_z -
                       3.0 * rpa_z * rpa_y * rpa_y * rpb_y * rpb_y * rpc_y - 2.0 * rpa_z * rpa_y * rpa_y * rpa_y * rpb_y * rpc_y);

        fints_z[i] += dip_z * fss * b2_vals[i] *
                      (-3.0 * rpa_y * rpa_y * rpb_z * rpb_y * rpb_y * rpc_y - 2.0 * rpa_y * rpa_y * rpa_y * rpb_z * rpb_y * rpc_y -
                       2.0 * rpa_y * rpa_y * rpa_y * rpb_y * rpb_y * rpc_z);
        ...
        fints_z[i] +=
            dip_z * fss * b3_vals[i] *
            (9.0 * fe_0 * rpa_y * rpb_z * rpb_y * rpc_y + 9.0 * fe_0 * rpa_y * rpb_z * rpc_y * rpc_y + 18.0 * fe_0 * rpa_y * rpb_y * rpc_z * rpc_y +
             3.0 * fe_0 * rpa_y * rpb_y * rpb_y * rpc_z + (9.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpb_z * rpc_y);

        fints_z[i] +=
            dip_z * fss * b3_vals[i] *
            (6.0 * fe_0 * rpa_y * rpa_y * rpb_y * rpc_z + 9.0 * fe_0 * rpa_y * rpa_y * rpc_z * rpc_y + fe_0 * rpa_y * rpa_y * rpa_y * rpc_z +
             6.0 * fe_0 * rpb_z * rpb_y * rpc_y * rpc_y + (3.0 / 2.0) * fe_0 * rpb_z * rpb_y * rpb_y * rpc_y);

        fints_z[i] +=
            dip_z * fss * b3_vals[i] *
            (3.0 * fe_0 * rpb_y * rpb_y * rpc_z * rpc_y + (9.0 / 4.0) * fe_0 * fe_0 * rpa_z * rpa_y + (3.0 / 2.0) * fe_0 * fe_0 * rpa_z * rpb_y +
             (15.0 / 2.0) * fe_0 * fe_0 * rpa_z * rpc_y + (9.0 / 4.0) * fe_0 * fe_0 * rpa_y * rpb_z);

        fints_z[i] += dip_z * fss * b3_vals[i] *
                      (9.0 * fe_0 * fe_0 * rpa_y * rpc_z + (3.0 / 2.0) * fe_0 * fe_0 * rpb_z * rpb_y + (15.0 / 2.0) * fe_0 * fe_0 * rpb_z * rpc_y +
                       6.0 * fe_0 * fe_0 * rpb_y * rpc_z + (15.0 / 2.0) * fe_0 * fe_0 * rpc_z * rpc_y);

        fints_z[i] += dip_z * fss * b3_vals[i] *
                      (3.0 * rpa_z * rpa_y * rpb_y * rpb_y * rpc_y * rpc_y + 6.0 * rpa_z * rpa_y * rpa_y * rpb_y * rpc_y * rpc_y +
                       rpa_z * rpa_y * rpa_y * rpa_y * rpc_y * rpc_y + 3.0 * rpa_y * rpb_z * rpb_y * rpb_y * rpc_y * rpc_y +
                       6.0 * rpa_y * rpa_y * rpb_z * rpb_y * rpc_y * rpc_y);

        fints_z[i] += dip_z * fss * b3_vals[i] *
                      (6.0 * rpa_y * rpa_y * rpb_y * rpb_y * rpc_z * rpc_y + rpa_y * rpa_y * rpa_y * rpb_z * rpc_y * rpc_y +
                       4.0 * rpa_y * rpa_y * rpa_y * rpb_y * rpc_z * rpc_y);

        fints_z[i] +=
            dip_z * fss * b4_vals[i] *
            (-9.0 * fe_0 * rpa_z * rpa_y * rpc_y * rpc_y - 6.0 * fe_0 * rpa_z * rpb_y * rpc_y * rpc_y - 5.0 * fe_0 * rpa_z * rpc_y * rpc_y * rpc_y -
             9.0 * fe_0 * rpa_y * rpb_z * rpc_y * rpc_y - 18.0 * fe_0 * rpa_y * rpb_y * rpc_z * rpc_y);

        fints_z[i] +=
            dip_z * fss * b4_vals[i] *
            (-18.0 * fe_0 * rpa_y * rpc_z * rpc_y * rpc_y - 9.0 * fe_0 * rpa_y * rpa_y * rpc_z * rpc_y - 6.0 * fe_0 * rpb_z * rpb_y * rpc_y * rpc_y -
             5.0 * fe_0 * rpb_z * rpc_y * rpc_y * rpc_y - 12.0 * fe_0 * rpb_y * rpc_z * rpc_y * rpc_y);

        fints_z[i] += dip_z * fss * b4_vals[i] *
                      (-3.0 * fe_0 * rpb_y * rpb_y * rpc_z * rpc_y - (15.0 / 4.0) * fe_0 * fe_0 * rpa_z * rpc_y -
                       (9.0 / 2.0) * fe_0 * fe_0 * rpa_y * rpc_z - (15.0 / 4.0) * fe_0 * fe_0 * rpb_z * rpc_y - 3.0 * fe_0 * fe_0 * rpb_y * rpc_z);

        fints_z[i] += dip_z * fss * b4_vals[i] *
                      (-15.0 * fe_0 * fe_0 * rpc_z * rpc_y - 6.0 * rpa_z * rpa_y * rpb_y * rpc_y * rpc_y * rpc_y -
                       3.0 * rpa_z * rpa_y * rpa_y * rpc_y * rpc_y * rpc_y - rpa_z * rpb_y * rpb_y * rpc_y * rpc_y * rpc_y -
                       6.0 * rpa_y * rpb_z * rpb_y * rpc_y * rpc_y * rpc_y);

        fints_z[i] += dip_z * fss * b4_vals[i] *
                      (-6.0 * rpa_y * rpb_y * rpb_y * rpc_z * rpc_y * rpc_y - 3.0 * rpa_y * rpa_y * rpb_z * rpc_y * rpc_y * rpc_y -
                       12.0 * rpa_y * rpa_y * rpb_y * rpc_z * rpc_y * rpc_y - 2.0 * rpa_y * rpa_y * rpa_y * rpc_z * rpc_y * rpc_y -
                       rpb_z * rpb_y * rpb_y * rpc_y * rpc_y * rpc_y);

        fints_z[i] +=
            dip_z * fss * b5_vals[i] *
            (5.0 * fe_0 * rpa_z * rpc_y * rpc_y * rpc_y + 18.0 * fe_0 * rpa_y * rpc_z * rpc_y * rpc_y + 5.0 * fe_0 * rpb_z * rpc_y * rpc_y * rpc_y +
             12.0 * fe_0 * rpb_y * rpc_z * rpc_y * rpc_y + 10.0 * fe_0 * rpc_z * rpc_y * rpc_y * rpc_y);

        fints_z[i] += dip_z * fss * b5_vals[i] *
                      ((15.0 / 2.0) * fe_0 * fe_0 * rpc_z * rpc_y + 3.0 * rpa_z * rpa_y * rpc_y * rpc_y * rpc_y * rpc_y +
                       2.0 * rpa_z * rpb_y * rpc_y * rpc_y * rpc_y * rpc_y + 3.0 * rpa_y * rpb_z * rpc_y * rpc_y * rpc_y * rpc_y +
                       12.0 * rpa_y * rpb_y * rpc_z * rpc_y * rpc_y * rpc_y);

        fints_z[i] += dip_z * fss * b5_vals[i] *
                      (6.0 * rpa_y * rpa_y * rpc_z * rpc_y * rpc_y * rpc_y + 2.0 * rpb_z * rpb_y * rpc_y * rpc_y * rpc_y * rpc_y +
        ...
        fints_z[i] += dip_z * fss * b6_vals[i] *
                      (-10.0 * fe_0 * rpc_z * rpc_y * rpc_y * rpc_y - rpa_z * rpc_y * rpc_y * rpc_y * rpc_y * rpc_y -
                       6.0 * rpa_y * rpc_z * rpc_y * rpc_y * rpc_y * rpc_y - rpb_z * rpc_y * rpc_y * rpc_y * rpc_y * rpc_y -
                       4.0 * rpb_y * rpc_z * rpc_y * rpc_y * rpc_y * rpc_y);
        ...
        fints_z[i] += faa_z * b1_vals[i] * ((3.0 / 4.0) * fe_0 * fe_0 * fe_0 * rpb_y + rpa_z * rpa_y * rpa_y * rpa_y * rpb_z * rpb_y * rpb_y);
        ...
        fints_z[i] +=
            faa_z * b5_vals[i] * (rpa_y * rpa_y * rpa_y * rpc_z * rpc_z * rpc_y * rpc_y + rpb_z * rpb_y * rpb_y * rpc_z * rpc_y * rpc_y * rpc_y);
        ...
        fints_z[i] += faa_z * b7_vals[i] * 2.0 * rpb_y * rpc_z * rpc_z * rpc_y * rpc_y * rpc_y * rpc_y;
