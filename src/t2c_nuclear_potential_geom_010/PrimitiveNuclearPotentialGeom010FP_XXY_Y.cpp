...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] +=
            dip_x * fss * b3_vals[i] * (fe_0 * rpc_x + 2.0 * rpa_y * rpc_y * rpc_x + 2.0 * rpa_x * rpc_y * rpc_y + 2.0 * rpb_y * rpc_y * rpc_x);
        ...
        fints_y[i] +=
            dip_y * fss * b1_vals[i] * ((1.0 / 2.0) * fe_0 * rpa_y + (1.0 / 2.0) * fe_0 * rpb_y + rpa_y * rpa_x * rpa_x + rpa_x * rpa_x * rpb_y);
