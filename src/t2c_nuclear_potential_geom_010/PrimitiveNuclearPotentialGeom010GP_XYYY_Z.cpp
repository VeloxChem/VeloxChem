    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += dip_x * fss * b2_vals[i] *
                      (-(3.0 / 2.0) * fe_0 * rpa_y * rpb_z - (3.0 / 2.0) * fe_0 * rpa_y * rpc_z - (3.0 / 2.0) * fe_0 * rpb_z * rpc_y -
                       3.0 * rpa_y * rpa_y * rpb_z * rpc_y - rpa_y * rpa_y * rpa_y * rpc_z);

        fints_x[i] += dip_x * fss * b3_vals[i] *
                      ((3.0 / 2.0) * fe_0 * rpa_y * rpc_z + (3.0 / 2.0) * fe_0 * rpb_z * rpc_y + (3.0 / 2.0) * fe_0 * rpc_z * rpc_y +
                       3.0 * rpa_y * rpb_z * rpc_y * rpc_y + 3.0 * rpa_y * rpa_y * rpc_z * rpc_y);

        fints_x[i] +=
            dip_x * fss * b4_vals[i] * (-(3.0 / 2.0) * fe_0 * rpc_z * rpc_y - 3.0 * rpa_y * rpc_z * rpc_y * rpc_y - rpb_z * rpc_y * rpc_y * rpc_y);

        fints_x[i] += dip_x * fss * b5_vals[i] * rpc_z * rpc_y * rpc_y * rpc_y;

        fints_x[i] += faa_x * b1_vals[i] * ((3.0 / 2.0) * fe_0 * rpa_y * rpa_x * rpb_z + rpa_y * rpa_y * rpa_y * rpa_x * rpb_z);

        fints_x[i] +=
            faa_x * b2_vals[i] *
            (-(3.0 / 2.0) * fe_0 * rpa_y * rpa_x * rpb_z - (3.0 / 2.0) * fe_0 * rpa_y * rpa_x * rpc_z - (3.0 / 2.0) * fe_0 * rpa_y * rpb_z * rpc_x -
             (3.0 / 2.0) * fe_0 * rpa_x * rpb_z * rpc_y - 3.0 * rpa_y * rpa_y * rpa_x * rpb_z * rpc_y);

        fints_x[i] += faa_x * b2_vals[i] * (-rpa_y * rpa_y * rpa_y * rpa_x * rpc_z - rpa_y * rpa_y * rpa_y * rpb_z * rpc_x);
        ...
        fints_x[i] +=
            faa_x * b3_vals[i] *
            ((3.0 / 2.0) * fe_0 * rpb_z * rpc_y * rpc_x + 3.0 * rpa_y * rpa_x * rpb_z * rpc_y * rpc_y + 3.0 * rpa_y * rpa_y * rpa_x * rpc_z * rpc_y +
             3.0 * rpa_y * rpa_y * rpb_z * rpc_y * rpc_x + rpa_y * rpa_y * rpa_y * rpc_z * rpc_x);

        fints_x[i] +=
            faa_x * b4_vals[i] *
            (-(3.0 / 2.0) * fe_0 * rpa_y * rpc_z * rpc_x - (3.0 / 2.0) * fe_0 * rpa_x * rpc_z * rpc_y - (3.0 / 2.0) * fe_0 * rpb_z * rpc_y * rpc_x -
             (3.0 / 2.0) * fe_0 * rpc_z * rpc_y * rpc_x - 3.0 * rpa_y * rpa_x * rpc_z * rpc_y * rpc_y);

        fints_x[i] +=
            faa_x * b4_vals[i] *
            (-3.0 * rpa_y * rpb_z * rpc_y * rpc_y * rpc_x - 3.0 * rpa_y * rpa_y * rpc_z * rpc_y * rpc_x - rpa_x * rpb_z * rpc_y * rpc_y * rpc_y);

        fints_x[i] += faa_x * b5_vals[i] *
                      ((3.0 / 2.0) * fe_0 * rpc_z * rpc_y * rpc_x + 3.0 * rpa_y * rpc_z * rpc_y * rpc_y * rpc_x +
                       rpa_x * rpc_z * rpc_y * rpc_y * rpc_y + rpb_z * rpc_y * rpc_y * rpc_y * rpc_x);
        ...
        fints_y[i] += dip_y * fss * b2_vals[i] *
                      (-(3.0 / 2.0) * fe_0 * rpa_x * rpb_z - (3.0 / 2.0) * fe_0 * rpa_x * rpc_z - (3.0 / 2.0) * fe_0 * rpb_z * rpc_x -
                       6.0 * rpa_y * rpa_x * rpb_z * rpc_y - 3.0 * rpa_y * rpa_y * rpa_x * rpc_z);
        ...
        fints_y[i] += dip_y * fss * b3_vals[i] *
                      ((3.0 / 2.0) * fe_0 * rpa_x * rpc_z + (3.0 / 2.0) * fe_0 * rpb_z * rpc_x + (3.0 / 2.0) * fe_0 * rpc_z * rpc_x +
                       6.0 * rpa_y * rpa_x * rpc_z * rpc_y + 6.0 * rpa_y * rpb_z * rpc_y * rpc_x);

        fints_y[i] += dip_y * fss * b3_vals[i] * (3.0 * rpa_y * rpa_y * rpc_z * rpc_x + 3.0 * rpa_x * rpb_z * rpc_y * rpc_y);

        fints_y[i] += dip_y * fss * b4_vals[i] *
                      (-(3.0 / 2.0) * fe_0 * rpc_z * rpc_x - 6.0 * rpa_y * rpc_z * rpc_y * rpc_x - 3.0 * rpa_x * rpc_z * rpc_y * rpc_y -
                       3.0 * rpb_z * rpc_y * rpc_y * rpc_x);

        fints_y[i] += dip_y * fss * b5_vals[i] * 3.0 * rpc_z * rpc_y * rpc_y * rpc_x;

        fints_y[i] += faa_y * b1_vals[i] * ((3.0 / 2.0) * fe_0 * rpa_y * rpa_x * rpb_z + rpa_y * rpa_y * rpa_y * rpa_x * rpb_z);
        ...
        fints_y[i] += faa_y * b2_vals[i] * (-rpa_y * rpa_y * rpa_y * rpa_x * rpc_z - rpa_y * rpa_y * rpa_y * rpb_z * rpc_x);
        ...
        fints_z[i] +=
            dip_z * fss * b4_vals[i] * (-(3.0 / 2.0) * fe_0 * rpc_y * rpc_x - 3.0 * rpa_y * rpc_y * rpc_y * rpc_x - rpa_x * rpc_y * rpc_y * rpc_y);

        fints_z[i] += dip_z * fss * b5_vals[i] * rpc_y * rpc_y * rpc_y * rpc_x;

        fints_z[i] += faa_z * b1_vals[i] * ((3.0 / 2.0) * fe_0 * rpa_y * rpa_x * rpb_z + rpa_y * rpa_y * rpa_y * rpa_x * rpb_z);
        ...
        fints_z[i] += faa_z * b2_vals[i] * (-rpa_y * rpa_y * rpa_y * rpa_x * rpc_z - rpa_y * rpa_y * rpa_y * rpb_z * rpc_x);
