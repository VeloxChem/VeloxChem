#include "PrimitiveNuclearPotentialGeom010GS_XYYZ_0.hpp"
...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += dip_x * fss * b2_vals[i] *
                      (-(1.0 / 2.0) * fe_0 * rpa_z - (1.0 / 2.0) * fe_0 * rpc_z - 2.0 * rpa_z * rpa_y * rpc_y - rpa_y * rpa_y * rpc_z);
        ...
        fints_x[i] += faa_x * b1_vals[i] * ((1.0 / 2.0) * fe_0 * rpa_z * rpa_x + rpa_z * rpa_y * rpa_y * rpa_x);
        ...
        fints_x[i] += faa_x * b2_vals[i] * (-rpa_y * rpa_y * rpa_x * rpc_z);
        ...
        fints_x[i] += faa_x * b3_vals[i] * (2.0 * rpa_y * rpa_x * rpc_z * rpc_y + rpa_y * rpa_y * rpc_z * rpc_x);
        ...
        fints_y[i] += dip_y * fss * b1_vals[i] * 2.0 * rpa_z * rpa_y * rpa_x;

        fints_y[i] += dip_y * fss * b2_vals[i] * (-2.0 * rpa_z * rpa_y * rpc_x - 2.0 * rpa_z * rpa_x * rpc_y - 2.0 * rpa_y * rpa_x * rpc_z);

        fints_y[i] += dip_y * fss * b3_vals[i] * (2.0 * rpa_z * rpc_y * rpc_x + 2.0 * rpa_y * rpc_z * rpc_x + 2.0 * rpa_x * rpc_z * rpc_y);

        fints_y[i] += dip_y * fss * b4_vals[i] * (-2.0 * rpc_z * rpc_y * rpc_x);

        fints_y[i] += faa_y * b1_vals[i] * ((1.0 / 2.0) * fe_0 * rpa_z * rpa_x + rpa_z * rpa_y * rpa_y * rpa_x);

        fints_y[i] += faa_y * b2_vals[i] *
                      (-(1.0 / 2.0) * fe_0 * rpa_z * rpa_x - (1.0 / 2.0) * fe_0 * rpa_z * rpc_x - (1.0 / 2.0) * fe_0 * rpa_x * rpc_z -
                       2.0 * rpa_z * rpa_y * rpa_x * rpc_y - rpa_z * rpa_y * rpa_y * rpc_x);

        fints_y[i] += faa_y * b2_vals[i] * (-rpa_y * rpa_y * rpa_x * rpc_z);

        fints_y[i] += faa_y * b3_vals[i] *
                      ((1.0 / 2.0) * fe_0 * rpa_z * rpc_x + (1.0 / 2.0) * fe_0 * rpa_x * rpc_z + (1.0 / 2.0) * fe_0 * rpc_z * rpc_x +
                       2.0 * rpa_z * rpa_y * rpc_y * rpc_x + rpa_z * rpa_x * rpc_y * rpc_y);

        fints_y[i] += faa_y * b3_vals[i] * (2.0 * rpa_y * rpa_x * rpc_z * rpc_y + rpa_y * rpa_y * rpc_z * rpc_x);
        ...
        fints_z[i] += faa_z * b1_vals[i] * ((1.0 / 2.0) * fe_0 * rpa_z * rpa_x + rpa_z * rpa_y * rpa_y * rpa_x);

        fints_z[i] += faa_z * b2_vals[i] *
                      (-(1.0 / 2.0) * fe_0 * rpa_z * rpa_x - (1.0 / 2.0) * fe_0 * rpa_z * rpc_x - (1.0 / 2.0) * fe_0 * rpa_x * rpc_z -
                       2.0 * rpa_z * rpa_y * rpa_x * rpc_y - rpa_z * rpa_y * rpa_y * rpc_x);

        fints_z[i] += faa_z * b2_vals[i] * (-rpa_y * rpa_y * rpa_x * rpc_z);

        fints_z[i] += faa_z * b3_vals[i] *
                      ((1.0 / 2.0) * fe_0 * rpa_z * rpc_x + (1.0 / 2.0) * fe_0 * rpa_x * rpc_z + (1.0 / 2.0) * fe_0 * rpc_z * rpc_x +
                       2.0 * rpa_z * rpa_y * rpc_y * rpc_x + rpa_z * rpa_x * rpc_y * rpc_y);

        fints_z[i] += faa_z * b3_vals[i] * (2.0 * rpa_y * rpa_x * rpc_z * rpc_y + rpa_y * rpa_y * rpc_z * rpc_x);
