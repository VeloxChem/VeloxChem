...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += faa_x * b1_vals[i] * (fe_0 * rpa_x * rpb_z + (1.0 / 2.0) * fe_0 * rpb_z * rpb_x + rpa_x * rpa_x * rpb_z * rpb_x);
        ...
        fints_x[i] += faa_x * b2_vals[i] * (-2.0 * rpa_x * rpb_z * rpb_x * rpc_x - rpa_x * rpa_x * rpb_z * rpc_x - rpa_x * rpa_x * rpb_x * rpc_z);
        ...
        fints_x[i] += faa_x * b3_vals[i] * (2.0 * rpa_x * rpb_x * rpc_z * rpc_x + rpa_x * rpa_x * rpc_z * rpc_x + rpb_z * rpb_x * rpc_x * rpc_x);
        ...
        fints_x[i] += faa_x * b5_vals[i] * rpc_z * rpc_x * rpc_x * rpc_x;

        fints_y[i] += faa_y * b1_vals[i] * (fe_0 * rpa_x * rpb_z + (1.0 / 2.0) * fe_0 * rpb_z * rpb_x + rpa_x * rpa_x * rpb_z * rpb_x);

        fints_y[i] += faa_y * b2_vals[i] *
                      (-fe_0 * rpa_x * rpb_z - fe_0 * rpa_x * rpc_z - (1.0 / 2.0) * fe_0 * rpb_z * rpb_x - (3.0 / 2.0) * fe_0 * rpb_z * rpc_x -
                       (1.0 / 2.0) * fe_0 * rpb_x * rpc_z);

        fints_y[i] += faa_y * b2_vals[i] * (-2.0 * rpa_x * rpb_z * rpb_x * rpc_x - rpa_x * rpa_x * rpb_z * rpc_x - rpa_x * rpa_x * rpb_x * rpc_z);

        fints_y[i] += faa_y * b3_vals[i] *
                      (fe_0 * rpa_x * rpc_z + (3.0 / 2.0) * fe_0 * rpb_z * rpc_x + (1.0 / 2.0) * fe_0 * rpb_x * rpc_z +
                       (3.0 / 2.0) * fe_0 * rpc_z * rpc_x + 2.0 * rpa_x * rpb_z * rpc_x * rpc_x);

        fints_y[i] += faa_y * b3_vals[i] * (2.0 * rpa_x * rpb_x * rpc_z * rpc_x + rpa_x * rpa_x * rpc_z * rpc_x + rpb_z * rpb_x * rpc_x * rpc_x);

        fints_y[i] += faa_y * b4_vals[i] *
                      (-(3.0 / 2.0) * fe_0 * rpc_z * rpc_x - 2.0 * rpa_x * rpc_z * rpc_x * rpc_x - rpb_z * rpc_x * rpc_x * rpc_x -
                       rpb_x * rpc_z * rpc_x * rpc_x);

        fints_y[i] += faa_y * b5_vals[i] * rpc_z * rpc_x * rpc_x * rpc_x;
        ...
        fints_z[i] += faa_z * b1_vals[i] * (fe_0 * rpa_x * rpb_z + (1.0 / 2.0) * fe_0 * rpb_z * rpb_x + rpa_x * rpa_x * rpb_z * rpb_x);

        fints_z[i] += faa_z * b2_vals[i] *
                      (-fe_0 * rpa_x * rpb_z - fe_0 * rpa_x * rpc_z - (1.0 / 2.0) * fe_0 * rpb_z * rpb_x - (3.0 / 2.0) * fe_0 * rpb_z * rpc_x -
                       (1.0 / 2.0) * fe_0 * rpb_x * rpc_z);

        fints_z[i] += faa_z * b2_vals[i] * (-2.0 * rpa_x * rpb_z * rpb_x * rpc_x - rpa_x * rpa_x * rpb_z * rpc_x - rpa_x * rpa_x * rpb_x * rpc_z);

        fints_z[i] += faa_z * b3_vals[i] *
                      (fe_0 * rpa_x * rpc_z + (3.0 / 2.0) * fe_0 * rpb_z * rpc_x + (1.0 / 2.0) * fe_0 * rpb_x * rpc_z +
                       (3.0 / 2.0) * fe_0 * rpc_z * rpc_x + 2.0 * rpa_x * rpb_z * rpc_x * rpc_x);

        fints_z[i] += faa_z * b3_vals[i] * (2.0 * rpa_x * rpb_x * rpc_z * rpc_x + rpa_x * rpa_x * rpc_z * rpc_x + rpb_z * rpb_x * rpc_x * rpc_x);

        fints_z[i] += faa_z * b4_vals[i] *
                      (-(3.0 / 2.0) * fe_0 * rpc_z * rpc_x - 2.0 * rpa_x * rpc_z * rpc_x * rpc_x - rpb_z * rpc_x * rpc_x * rpc_x -
                       rpb_x * rpc_z * rpc_x * rpc_x);

        fints_z[i] += faa_z * b5_vals[i] * rpc_z * rpc_x * rpc_x * rpc_x;
    }
}
