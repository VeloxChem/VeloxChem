#include "NuclearPotentialGeom010RecPS.hpp"
        ...
        for (int64_t j = bra_first; j < bra_last; j++) 
        {
            ...
            for (int64_t k = 0; k < ket_npgtos; k++)
            {
                ...
                for (int64_t l = 0; l < bra_npgtos; l++)
                {
                    ...
                    geom_npotrec::compPrimitiveNuclearPotentialGeom010PS_X_0(buffer_x,
                                                                             buffer_y,
                                                                             buffer_z,
                                                                             dipole,
                                                                             point,
            ...
            for (int64_t k = 0; k < ket_npgtos; k++)
            {
                ...
                for (int64_t l = 0; l < bra_npgtos; l++)
                {
                    ...
                    geom_npotrec::compPrimitiveNuclearPotentialGeom010PS_Y_0(buffer_x,
                                                                             buffer_y,
                                                                             buffer_z,
                                                                             dipole,
                                                                             point,
            ...
            for (int64_t k = 0; k < ket_npgtos; k++)
            {
                ...
                for (int64_t l = 0; l < bra_npgtos; l++)
                {
                    ...
                    geom_npotrec::compPrimitiveNuclearPotentialGeom010PS_Z_0(buffer_x,
                                                                             buffer_y,
                                                                             buffer_z,
                                                                             dipole,
                                                                             point,
                                           ...
                                           const int64_t       ket_dim) -> void
{
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += dip_x * faa_x * b0_vals[i] * rpa_x;

        fints_x[i] += dip_x * faa_x * b1_vals[i] * (-rpc_x);

        fints_y[i] += dip_y * faa_y * b0_vals[i] * rpa_x;

        fints_y[i] += dip_y * faa_y * b1_vals[i] * (-rpc_x);

        fints_z[i] += dip_z * faa_z * b0_vals[i] * rpa_x;
                                           ...
                                           const int64_t       ket_dim) -> void
{
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += dip_x * faa_x * b0_vals[i] * rpa_y;

        fints_x[i] += dip_x * faa_x * b1_vals[i] * (-rpc_y);

        fints_y[i] += dip_y * fss * b1_vals[i];

        fints_y[i] += dip_y * faa_y * b0_vals[i] * rpa_y;

        fints_y[i] += dip_y * faa_y * b1_vals[i] * (-rpc_y);

        fints_z[i] += dip_z * faa_z * b0_vals[i] * rpa_y;
                                           ...
                                           const int64_t       ket_dim) -> void
{
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += dip_x * faa_x * b0_vals[i] * rpa_z;

        fints_x[i] += dip_x * faa_x * b1_vals[i] * (-rpc_z);

        fints_y[i] += dip_y * faa_y * b0_vals[i] * rpa_z;

        fints_y[i] += dip_y * faa_y * b1_vals[i] * (-rpc_z);

        fints_z[i] += dip_z * fss * b1_vals[i];

        fints_z[i] += dip_z * faa_z * b0_vals[i] * rpa_z;
