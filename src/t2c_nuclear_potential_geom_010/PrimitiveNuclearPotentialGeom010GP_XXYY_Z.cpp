...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += faa_x * b1_vals[i] *
                      ((1.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpb_z + (1.0 / 2.0) * fe_0 * rpa_x * rpa_x * rpb_z + (1.0 / 4.0) * fe_0 * fe_0 * rpb_z +
                       rpa_y * rpa_y * rpa_x * rpa_x * rpb_z);

        fints_x[i] += faa_x * b2_vals[i] *
                      (-fe_0 * rpa_y * rpb_z * rpc_y - (1.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpb_z - (1.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpc_z -
                       fe_0 * rpa_x * rpb_z * rpc_x - (1.0 / 2.0) * fe_0 * rpa_x * rpa_x * rpb_z);

        fints_x[i] += faa_x * b2_vals[i] *
                      (-(1.0 / 2.0) * fe_0 * rpa_x * rpa_x * rpc_z - (1.0 / 2.0) * fe_0 * fe_0 * rpb_z - (1.0 / 4.0) * fe_0 * fe_0 * rpc_z -
                       2.0 * rpa_y * rpa_x * rpa_x * rpb_z * rpc_y - 2.0 * rpa_y * rpa_y * rpa_x * rpb_z * rpc_x);

        fints_x[i] += faa_x * b2_vals[i] * (-rpa_y * rpa_y * rpa_x * rpa_x * rpc_z);

        fints_x[i] += faa_x * b3_vals[i] *
                      (fe_0 * rpa_y * rpb_z * rpc_y + fe_0 * rpa_y * rpc_z * rpc_y + (1.0 / 2.0) * fe_0 * rpa_y * rpa_y * rpc_z +
                       fe_0 * rpa_x * rpb_z * rpc_x + fe_0 * rpa_x * rpc_z * rpc_x);

        fints_x[i] += faa_x * b3_vals[i] *
                      ((1.0 / 2.0) * fe_0 * rpa_x * rpa_x * rpc_z + (1.0 / 2.0) * fe_0 * rpb_z * rpc_y * rpc_y +
                       (1.0 / 2.0) * fe_0 * rpb_z * rpc_x * rpc_x + (1.0 / 4.0) * fe_0 * fe_0 * rpb_z + (1.0 / 2.0) * fe_0 * fe_0 * rpc_z);

        fints_x[i] += faa_x * b3_vals[i] *
                      (4.0 * rpa_y * rpa_x * rpb_z * rpc_y * rpc_x + 2.0 * rpa_y * rpa_x * rpa_x * rpc_z * rpc_y +
                       2.0 * rpa_y * rpa_y * rpa_x * rpc_z * rpc_x + rpa_y * rpa_y * rpb_z * rpc_x * rpc_x + rpa_x * rpa_x * rpb_z * rpc_y * rpc_y);

        fints_x[i] += faa_x * b4_vals[i] *
                      (-fe_0 * rpa_y * rpc_z * rpc_y - fe_0 * rpa_x * rpc_z * rpc_x - (1.0 / 2.0) * fe_0 * rpb_z * rpc_y * rpc_y -
                       (1.0 / 2.0) * fe_0 * rpb_z * rpc_x * rpc_x - (1.0 / 2.0) * fe_0 * rpc_z * rpc_y * rpc_y);

        fints_x[i] += faa_x * b4_vals[i] *
                      (-(1.0 / 2.0) * fe_0 * rpc_z * rpc_x * rpc_x - (1.0 / 4.0) * fe_0 * fe_0 * rpc_z - 4.0 * rpa_y * rpa_x * rpc_z * rpc_y * rpc_x -
                       2.0 * rpa_y * rpb_z * rpc_y * rpc_x * rpc_x - rpa_y * rpa_y * rpc_z * rpc_x * rpc_x);

        fints_x[i] += faa_x * b4_vals[i] * (-2.0 * rpa_x * rpb_z * rpc_y * rpc_y * rpc_x - rpa_x * rpa_x * rpc_z * rpc_y * rpc_y);

        fints_x[i] +=
            faa_x * b5_vals[i] *
            ((1.0 / 2.0) * fe_0 * rpc_z * rpc_y * rpc_y + (1.0 / 2.0) * fe_0 * rpc_z * rpc_x * rpc_x + 2.0 * rpa_y * rpc_z * rpc_y * rpc_x * rpc_x +
             2.0 * rpa_x * rpc_z * rpc_y * rpc_y * rpc_x + rpb_z * rpc_y * rpc_y * rpc_x * rpc_x);
        ...
        fints_y[i] += faa_y * b2_vals[i] * (-rpa_y * rpa_y * rpa_x * rpa_x * rpc_z);
        ...
        fints_y[i] += faa_y * b4_vals[i] * (-2.0 * rpa_x * rpb_z * rpc_y * rpc_y * rpc_x - rpa_x * rpa_x * rpc_z * rpc_y * rpc_y);
        ...
        fints_z[i] +=
            dip_z * fss * b3_vals[i] * (4.0 * rpa_y * rpa_x * rpc_y * rpc_x + rpa_y * rpa_y * rpc_x * rpc_x + rpa_x * rpa_x * rpc_y * rpc_y);
        ...
        fints_z[i] += faa_z * b2_vals[i] * (-rpa_y * rpa_y * rpa_x * rpa_x * rpc_z);
        ...
        fints_z[i] += faa_z * b4_vals[i] * (-2.0 * rpa_x * rpb_z * rpc_y * rpc_y * rpc_x - rpa_x * rpa_x * rpc_z * rpc_y * rpc_y);
