#ifndef NuclearPotentialGeom010SumErfRecPF_hpp
#define NuclearPotentialGeom010SumErfRecPF_hpp
...
/// @brief Computes (P|Erf(AG(1))|F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_erf_nuclear_potential_geom_010_pf(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    npotrec::comp_prim_nuclear_potential_geom_010_ss(pbuffer, 6, 1, factors, 17, omegas[l], a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_ss(pbuffer, 9, 2, factors, 17, omegas[l], a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_ss(pbuffer, 12, 3, factors, 17, omegas[l], a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_ss(pbuffer, 15, 4, factors, 17, omegas[l], a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_ss(pbuffer, 18, 5, factors, 17, omegas[l], a_exp);
                    ...
                    npotrec::comp_prim_nuclear_potential_sf(pbuffer, 132, 21, 24, 66, 72, factors, 14, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_sf(pbuffer, 142, 30, 39, 66, 78, 96, factors, 14, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_sf(pbuffer, 172, 39, 48, 72, 96, 114, factors, 14, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_pf(pbuffer, 202, 78, 96, 132, 142, 172, factors, 11, 17, a_exp);
...
#endif /* NuclearPotentialGeom010SumErfRecPF_hpp */
