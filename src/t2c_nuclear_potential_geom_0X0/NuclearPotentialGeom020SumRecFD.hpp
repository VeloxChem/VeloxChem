#ifndef NuclearPotentialGeom020SumRecFD_hpp
#define NuclearPotentialGeom020SumRecFD_hpp
...
/// @brief Computes (F|AG(2)|D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_nuclear_potential_geom_020_fd(T&                               distributor,
                                       ...
                                       const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(2240, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    npotrec::comp_prim_nuclear_potential_geom_020_ps(pbuffer, 404, 8, 23, 29, factors, 11, 17);

                    npotrec::comp_prim_nuclear_potential_geom_020_ps(pbuffer, 422, 11, 29, 35, factors, 11, 17);

                    npotrec::comp_prim_nuclear_potential_geom_020_ps(pbuffer, 440, 14, 35, 41, factors, 11, 17);

                    npotrec::comp_prim_nuclear_potential_geom_010_pp(pbuffer, 458, 8, 11, 59, 68, 77, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_pp(pbuffer, 485, 11, 14, 62, 77, 86, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_020_pp(pbuffer, 512, 23, 29, 68, 104, 122, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_020_pp(pbuffer, 566, 29, 35, 77, 122, 140, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_020_pp(pbuffer, 620, 35, 41, 86, 140, 158, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_pd(pbuffer, 674, 59, 62, 194, 200, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_pd(pbuffer, 692, 68, 77, 194, 206, 224, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_pd(pbuffer, 746, 77, 86, 200, 224, 242, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_020_pd(pbuffer, 800, 104, 122, 206, 260, 296, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_020_pd(pbuffer, 908, 122, 140, 224, 296, 332, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_020_pd(pbuffer, 1016, 140, 158, 242, 332, 368, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_020_dp(pbuffer, 1124, 104, 122, 404, 422, 458, 512, 566, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_020_dp(pbuffer, 1232, 122, 140, 422, 440, 485, 566, 620, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_dd(pbuffer, 1340, 206, 224, 458, 485, 674, 692, 746, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_020_dd(pbuffer, 1448, 260, 296, 512, 566, 692, 800, 908, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_020_dd(pbuffer, 1664, 296, 332, 566, 620, 746, 908, 1016, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_020_fd(pbuffer, 1880, 800, 908, 1124, 1232, 1340, 1448, 1664, factors, 11, 17, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 1880, quadrupoles, 6, l, ket_width, ket_npgtos);
                }
            }
...
#endif /* NuclearPotentialGeom020SumRecFD_hpp */
