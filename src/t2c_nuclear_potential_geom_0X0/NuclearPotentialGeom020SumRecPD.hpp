#ifndef NuclearPotentialGeom020SumRecPD_hpp
#define NuclearPotentialGeom020SumRecPD_hpp
...
/// @brief Computes (P|AG(2)|D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_nuclear_potential_geom_020_pd(T&                               distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    npotrec::comp_prim_nuclear_potential_geom_010_sd(pbuffer, 114, 6, 9, 39, 42, 51, factors, 14, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_020_sd(pbuffer, 132, 15, 21, 42, 60, 78, factors, 14, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_020_sd(pbuffer, 168, 21, 27, 51, 78, 96, factors, 14, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_020_pd(pbuffer, 204, 60, 78, 114, 132, 168, factors, 11, 17, a_exp);
...
#endif /* NuclearPotentialGeom020SumRecPD_hpp */
