#ifndef NuclearPotentialGeom010SumErfRecDS_hpp
#define NuclearPotentialGeom010SumErfRecDS_hpp
...
/// @brief Computes (D|Erf(AG(1))|S)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_erf_nuclear_potential_geom_010_ds(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    t2cfunc::comp_boys_args(bf_data, 4, factors, 14, a_exp, omegas[l]);

                    bf_table.compute(bf_data, 0, 4, factors, a_exp, omegas[l]);
                    ...
                    npotrec::comp_prim_nuclear_potential_geom_010_ss(pbuffer, 4, 1, factors, 14, omegas[l], a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_ss(pbuffer, 7, 2, factors, 14, omegas[l], a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_ss(pbuffer, 10, 3, factors, 14, omegas[l], a_exp);

                    npotrec::comp_prim_nuclear_potential_ps(pbuffer, 13, 1, 2, factors, 11, 14);

                    npotrec::comp_prim_nuclear_potential_geom_010_ps(pbuffer, 16, 1, 4, 7, factors, 11, 14);

                    npotrec::comp_prim_nuclear_potential_geom_010_ps(pbuffer, 25, 2, 7, 10, factors, 11, 14);

                    npotrec::comp_prim_nuclear_potential_geom_010_ds(pbuffer, 34, 4, 7, 13, 16, 25, factors, 11, 14, a_exp);
...
#endif /* NuclearPotentialGeom010SumErfRecDS_hpp */
