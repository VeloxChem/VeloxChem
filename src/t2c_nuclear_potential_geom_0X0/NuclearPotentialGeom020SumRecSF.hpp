#ifndef NuclearPotentialGeom020SumRecSF_hpp
#define NuclearPotentialGeom020SumRecSF_hpp
...
/// @brief Computes (S|AG(2)|F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_nuclear_potential_geom_020_sf(T&                               distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    npotrec::comp_prim_nuclear_potential_sp(pbuffer, 39, 2, 3, factors, 11, 14);

                    npotrec::comp_prim_nuclear_potential_geom_010_sp(pbuffer, 42, 2, 6, 9, factors, 11, 14);

                    npotrec::comp_prim_nuclear_potential_geom_010_sp(pbuffer, 51, 3, 9, 12, factors, 11, 14);

                    npotrec::comp_prim_nuclear_potential_geom_020_sp(pbuffer, 60, 6, 15, 21, factors, 11, 14);

                    npotrec::comp_prim_nuclear_potential_geom_020_sp(pbuffer, 78, 9, 21, 27, factors, 11, 14);

                    npotrec::comp_prim_nuclear_potential_geom_020_sp(pbuffer, 96, 12, 27, 33, factors, 11, 14);

                    npotrec::comp_prim_nuclear_potential_geom_010_sd(pbuffer, 114, 6, 9, 39, 42, 51, factors, 11, 14, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_020_sd(pbuffer, 132, 15, 21, 42, 60, 78, factors, 11, 14, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_020_sd(pbuffer, 168, 21, 27, 51, 78, 96, factors, 11, 14, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_020_sf(pbuffer, 204, 60, 78, 114, 132, 168, factors, 11, 14, a_exp);
...
#endif /* NuclearPotentialGeom020SumRecSF_hpp */
