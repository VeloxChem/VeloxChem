#ifndef NuclearPotentialGeom010SumRecDF_hpp
#define NuclearPotentialGeom010SumRecDF_hpp
...
/// @brief Computes (D|AG(1)|F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_nuclear_potential_geom_010_df(T&                               distributor,
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    npotrec::comp_prim_nuclear_potential_geom_010_sf(
                        pbuffer, 192, 37, 46, 82, 100, 118, factors, 14, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_sf(
                        pbuffer, 222, 46, 55, 88, 118, 136, factors, 14, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_sf(
                        pbuffer, 252, 55, 64, 94, 136, 154, factors, 14, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_pd(
                        pbuffer, 282, 37, 46, 82, 100, 118, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_pd(
                        pbuffer, 336, 46, 55, 88, 118, 136, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_pf(pbuffer, 390, 82, 88, 172, 182, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_pf(
                        pbuffer, 420, 100, 118, 172, 192, 222, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_pf(
                        pbuffer, 510, 118, 136, 182, 222, 252, factors, 11, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_geom_010_df(
                        pbuffer, 600, 192, 222, 282, 336, 390, 420, 510, factors, 11, 17, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 600, dipoles, 3, l, ket_width, ket_npgtos);
                }
            }
...
#endif /* NuclearPotentialGeom010SumRecDF_hpp */
