...
class CNuclearPotentialErfGeomX00Driver
{
   public:
    ...
    auto compute(const CMolecularBasis &basis, const CMolecule &molecule, const int iatom, const std::vector<std::array<double, 3>>& coordinates, const std::vector<double>& charges, const std::vector<double> &omegas) const -> CMatrices;
};
...
template <int N>
auto
CNuclearPotentialErfGeomX00Driver<N>::compute(const CMolecularBasis &basis, const CMolecule &molecule, const int iatom, const std::vector<std::array<double, 3>>& coords_array, const std::vector<double>& charges, const std::vector<double>& omegas) const -> CMatrices
{
    // set up operator derivatives matrices
