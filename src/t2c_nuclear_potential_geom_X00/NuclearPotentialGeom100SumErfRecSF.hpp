#ifndef NuclearPotentialGeom100SumErfRecSF_hpp
#define NuclearPotentialGeom100SumErfRecSF_hpp
...
/// @brief Computes (d^(1)/dA^(1)S|Erf(A)|F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_erf_nuclear_potential_geom_10_sf(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    npotrec::comp_prim_nuclear_potential_pf(pbuffer, 56, 18, 24, 36, 46, factors, 11, 17, a_exp);

                    t2cgeom::comp_prim_op_geom_10_sx(pbuffer, 86, 56, 1, 10, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 86, charges[l], ket_width, ket_npgtos);
                }
            }
...
#endif /* NuclearPotentialGeom100SumErfRecSF_hpp */
