#ifndef NuclearPotentialGeom200SumRecDD_hpp
#define NuclearPotentialGeom200SumRecDD_hpp
...
/// @brief Computes (d^(2)/dA^(2)D|A|D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_nuclear_potential_geom_20_dd(T&                               distributor,
                                      ...
                                      const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(842, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    t2cgeom::comp_prim_op_geom_20_dx(pbuffer, 626, 26, 248, 536, 1, 6, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 626, charges[l], ket_width, ket_npgtos);
                }
            }
...
#endif /* NuclearPotentialGeom200SumRecDD_hpp */
