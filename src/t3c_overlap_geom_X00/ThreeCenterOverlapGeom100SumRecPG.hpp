#ifndef ThreeCenterOverlapGeom100SumRecPG_hpp
#define ThreeCenterOverlapGeom100SumRecPG_hpp
...
/// @brief Computes (d^(1)/dA^(1)P|G(r)|G)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_geom_10_pg(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3ovlrec::comp_prim_overlap_pf(pbuffer, 36, 5, 11, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pg(pbuffer, 66, 11, 21, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dg(pbuffer, 111, 21, 36, 66, factors, 11, a_exp, exgtos[l]);

                    t2cgeom::comp_prim_op_geom_10_px(pbuffer, 201, 21, 111, 1, 15, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 201, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGeom100SumRecPG_hpp */
