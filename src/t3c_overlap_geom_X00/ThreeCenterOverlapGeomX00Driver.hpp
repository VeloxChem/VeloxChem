#ifndef ThreeCenterOverlapGeomX00Driver_hpp
#define ThreeCenterOverlapGeomX00Driver_hpp
...
#include "ThreeCenterOverlapGeom100Func.hpp"
...
template <int N>
class CThreeCenterOverlapGeomX00Driver
{
   public:
    /// @brief Creates an overlap derivative integrals driver.
    CThreeCenterOverlapGeomX00Driver() = default;

    /// @brief The default copy constructor.
    /// @param other The overlap derivative integrals driver to be copied.
    CThreeCenterOverlapGeomX00Driver(const CThreeCenterOverlapGeomX00Driver &other) = delete;

    /// @brief The default move constructor.
    /// @param other The overlap derivative integrals driver  to be moved.
    CThreeCenterOverlapGeomX00Driver(CThreeCenterOverlapGeomX00Driver &&other) noexcept = delete;
    ...
    auto operator=(const CThreeCenterOverlapGeomX00Driver &other) -> CThreeCenterOverlapGeomX00Driver & = delete;

    /// @brief The default move assignment operator.
    /// @param other The overlap derivative integrals driver to be move assigned.
    /// @return The assigned overlap derivative integrals driver .
    auto operator=(CThreeCenterOverlapGeomX00Driver &&other) noexcept -> CThreeCenterOverlapGeomX00Driver & = delete;
    ...
    auto operator==(const CThreeCenterOverlapGeomX00Driver &other) const -> bool = delete;
...
template <int N>
auto
CThreeCenterOverlapGeomX00Driver<N>::compute(const std::vector<double>         &exponents,
                                             const std::vector<double>         &factors,
                                             const std::vector<TPoint<double>> &coordinates,
                                             const CMolecularBasis             &basis,
                                             const CMolecule                   &molecule,
...
#pragma omp task firstprivate(bra_gtos, ket_gtos, bra_indices, ket_indices)
                {
                    ...
                    if constexpr (N == 1)
                    {
                        t3ovlfunc::compute_geom_100(distributor, bra_gtos, ket_gtos, bra_indices, ket_indices, false);
                    }
                }
            });
        }
...
#endif /* ThreeCenterOverlapGeomX00Driver_hpp */
