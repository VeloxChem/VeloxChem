#ifndef ThreeCenterOverlapGeom100SumRecSD_hpp
#define ThreeCenterOverlapGeom100SumRecSD_hpp
...
/// @brief Computes (d^(1)/dA^(1)S|G(r)|D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_geom_10_sd(T& distributor,
                            ...
                            const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(47, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3ovlrec::comp_prim_overlap_pd(pbuffer, 11, 2, 5, factors, 11, a_exp, exgtos[l]);

                    t2cgeom::comp_prim_op_geom_10_sx(pbuffer, 29, 11, 1, 6, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 29, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGeom100SumRecSD_hpp */
