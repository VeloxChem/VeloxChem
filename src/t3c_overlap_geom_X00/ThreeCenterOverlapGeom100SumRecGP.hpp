#ifndef ThreeCenterOverlapGeom100SumRecGP_hpp
#define ThreeCenterOverlapGeom100SumRecGP_hpp
...
/// @brief Computes (d^(1)/dA^(1)G|G(r)|P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_geom_10_gp(T& distributor,
                            ...
                            const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(339, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3ovlrec::comp_prim_overlap_hp(pbuffer, 141, 51, 81, 96, factors, 11, a_exp, exgtos[l]);

                    t2cgeom::comp_prim_op_geom_10_gx(pbuffer, 204, 51, 141, 1, 3, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 204, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGeom100SumRecGP_hpp */
