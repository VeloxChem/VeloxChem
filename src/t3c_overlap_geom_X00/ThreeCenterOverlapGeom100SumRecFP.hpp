#ifndef ThreeCenterOverlapGeom100SumRecFP_hpp
#define ThreeCenterOverlapGeom100SumRecFP_hpp
...
/// @brief Computes (d^(1)/dA^(1)F|G(r)|P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_geom_10_fp(T& distributor,
                            ...
                            const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(216, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3ovlrec::comp_prim_overlap_gp(pbuffer, 81, 23, 41, 51, factors, 11, a_exp, exgtos[l]);

                    t2cgeom::comp_prim_op_geom_10_fx(pbuffer, 126, 23, 81, 1, 3, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 126, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGeom100SumRecFP_hpp */
