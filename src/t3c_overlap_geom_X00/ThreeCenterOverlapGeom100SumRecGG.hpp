#ifndef ThreeCenterOverlapGeom100SumRecGG_hpp
#define ThreeCenterOverlapGeom100SumRecGG_hpp
...
/// @brief Computes (d^(1)/dA^(1)G|G(r)|G)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_geom_10_gg(T& distributor,
                            ...
                            const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(2020, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3ovlrec::comp_prim_overlap_dp(pbuffer, 141, 2, 36, 39, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dd(pbuffer, 159, 5, 39, 48, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_df(pbuffer, 195, 11, 48, 66, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dg(pbuffer, 255, 21, 66, 96, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fd(pbuffer, 345, 48, 141, 159, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ff(pbuffer, 405, 66, 159, 195, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fg(pbuffer, 505, 96, 195, 255, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_gf(pbuffer, 655, 195, 345, 405, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_gg(pbuffer, 805, 255, 405, 505, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_hg(pbuffer, 1030, 505, 655, 805, factors, 11, a_exp, exgtos[l]);

                    t2cgeom::comp_prim_op_geom_10_gx(pbuffer, 1345, 505, 1030, 1, 15, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 1345, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGeom100SumRecGG_hpp */
