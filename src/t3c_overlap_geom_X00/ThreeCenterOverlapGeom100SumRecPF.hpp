#ifndef ThreeCenterOverlapGeom100SumRecPF_hpp
#define ThreeCenterOverlapGeom100SumRecPF_hpp
...
/// @brief Computes (d^(1)/dA^(1)P|G(r)|F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_geom_10_pf(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3ovlrec::comp_prim_overlap_pd(pbuffer, 21, 2, 5, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_pf(pbuffer, 39, 5, 11, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_df(pbuffer, 69, 11, 21, 39, factors, 11, a_exp, exgtos[l]);

                    t2cgeom::comp_prim_op_geom_10_px(pbuffer, 129, 11, 69, 1, 10, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 129, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGeom100SumRecPF_hpp */
