#ifndef ThreeCenterOverlapGeom100SumRecFF_hpp
#define ThreeCenterOverlapGeom100SumRecFF_hpp
...
/// @brief Computes (d^(1)/dA^(1)F|G(r)|F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_overlap_geom_10_ff(T& distributor,
                            ...
                            const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(805, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < npoints; l++)
                {
                    ...
                    t3ovlrec::comp_prim_overlap_dp(pbuffer, 81, 2, 21, 24, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_dd(pbuffer, 99, 5, 24, 33, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_df(pbuffer, 135, 11, 33, 51, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_fd(pbuffer, 195, 33, 81, 99, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_ff(pbuffer, 255, 51, 99, 135, factors, 11, a_exp, exgtos[l]);

                    t3ovlrec::comp_prim_overlap_gf(pbuffer, 355, 135, 195, 255, factors, 11, a_exp, exgtos[l]);

                    t2cgeom::comp_prim_op_geom_10_fx(pbuffer, 505, 135, 355, 1, 10, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 505, ket_width, ket_npgtos);
                }
            }
...
#endif /* ThreeCenterOverlapGeom100SumRecFF_hpp */
