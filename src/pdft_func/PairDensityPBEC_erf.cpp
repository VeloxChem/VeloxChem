...
compute_exc_vxc(const int32_t np, const double* rho, const double* sigma, double* exc, double* vrho, double* vsigma, const double mu)
{
   ...
   for (int32_t g = 0; g < np; g++)
   {
      ...
      double e_lda = -e_lr + ec;

      double de_lda_de_lr = -1;

      double de_lda_dec = 1;

      double de_lda_drho = de_lda_de_lr*de_lr_drho + de_lda_dec*dec_drho;

      double de_lda_dpi = de_lda_de_lr*de_lr_dpi + de_lda_dec*dec_dpi;

      double betar = mbeta*pow(e_lda/epw92, c);

      double dbetar_de_lda = c*mbeta*pow(e_lda/epw92, c)/e_lda;

      double dbetar_depw92 = -c*mbeta*pow(e_lda/epw92, c)/epw92;

      double dbetar_drho = dbetar_de_lda*de_lda_drho + dbetar_depw92*depw92_drho;

      double dbetar_dpi = dbetar_de_lda*de_lda_dpi + dbetar_depw92*depw92_dpi;
      ...
      double Aexp = -1 + exp(-e_lda/(mgamma*mphi3));

      double dAexp_dmphi3 = e_lda*exp(-e_lda/(mgamma*mphi3))/(mgamma*pow(mphi3, 2));

      double dAexp_de_lda = -exp(-e_lda/(mgamma*mphi3))/(mgamma*mphi3);

      double dAexp_drho = dAexp_de_lda*de_lda_drho + dAexp_dmphi3*dmphi3_drho;

      double dAexp_dpi = dAexp_de_lda*de_lda_dpi + dAexp_dmphi3*dmphi3_dpi;

      double A = betar/(Aexp*mgamma);

      double dA_dbetar = 1/(Aexp*mgamma);

      double dA_dAexp = -betar/(pow(Aexp, 2)*mgamma);

      double dA_drho = dA_dAexp*dAexp_drho + dA_dbetar*dbetar_drho;

      double dA_dpi = dA_dAexp*dAexp_dpi + dA_dbetar*dbetar_dpi;
      ...
      double f2 = betar*f1/(mgamma*(A*f1 + 1));

      double df2_dbetar = f1/(mgamma*(A*f1 + 1));

      double df2_dA = -betar*pow(f1, 2)/(mgamma*pow(A*f1 + 1, 2));

      double df2_df1 = -A*betar*f1/(mgamma*pow(A*f1 + 1, 2)) + betar/(mgamma*(A*f1 + 1));

      double df2_dpi = dA_dpi*df2_dA + dbetar_dpi*df2_dbetar + df1_dpi*df2_df1;

      double df2_drho = dA_drho*df2_dA + dbetar_drho*df2_dbetar + df1_drho*df2_df1;
      ...
      double e_pbe_erf = H + e_lda;

      double de_pbe_erf_dH = 1;

      double de_pbe_erf_de_lda = 1;

      double de_pbe_erf_dpi = dH_dpi*de_pbe_erf_dH + de_lda_dpi*de_pbe_erf_de_lda;

      double de_pbe_erf_drho = dH_drho*de_pbe_erf_dH + de_lda_drho*de_pbe_erf_de_lda;

      double de_pbe_erf_dsig = dH_dsig*de_pbe_erf_dH;

      exc[g] =  e_pbe_erf;

      double dExc_de_pbe_erf = density;

      vsigma[3 * g + 0] =  dExc_de_pbe_erf*de_pbe_erf_dsig;

      vrho[2 * g + 0] =  dExc_de_pbe_erf*de_pbe_erf_drho + e_pbe_erf;

      vrho[2 * g + 1] =  dExc_de_pbe_erf*de_pbe_erf_dpi;
