...
compute_exc_vxc(const int np, const double* rho, const double* sigma, double* exc, double* vrho, double* vsigma)
{
    ...
    double frg = -0.75 * std::pow(3.0 / mathconst::pi_value(), f13);

    double R = 0.804;

    double mu = 0.066725 / 3.0 * std::pow(mathconst::pi_value(), 2);

    double mus2r = mu * std::pow(1.0 / 12 * std::pow(6.0 / mathconst::pi_value(), 2.0 / 3.0), 2) / R;
