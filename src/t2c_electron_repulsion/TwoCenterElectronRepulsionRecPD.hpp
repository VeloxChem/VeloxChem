#ifndef TwoCenterElectronRepulsionRecPD_hpp
#define TwoCenterElectronRepulsionRecPD_hpp
...
/// @brief Computes (P|1/|r-r'||D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_electron_repulsion_pd(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                t2ceri::comp_prim_electron_repulsion_sp(pbuffer, 3, 1, factors, 11);

                t2ceri::comp_prim_electron_repulsion_sp(pbuffer, 6, 2, factors, 11);

                t2ceri::comp_prim_electron_repulsion_sd(pbuffer, 9, 0, 1, 6, factors, 11, a_exp);

                t2ceri::comp_prim_electron_repulsion_pd(pbuffer, 15, 3, 9, factors, 8, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 15, ket_width, ket_npgtos);
            }
...
#endif /* TwoCenterElectronRepulsionRecPD_hpp */
