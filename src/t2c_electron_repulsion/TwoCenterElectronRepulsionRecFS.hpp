#ifndef TwoCenterElectronRepulsionRecFS_hpp
#define TwoCenterElectronRepulsionRecFS_hpp
...
/// @brief Computes (F|1/|r-r'||S)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_electron_repulsion_fs(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                t2ceri::comp_prim_electron_repulsion_ps(pbuffer, 3, 0, factors, 8);

                t2ceri::comp_prim_electron_repulsion_ps(pbuffer, 6, 1, factors, 8);

                t2ceri::comp_prim_electron_repulsion_ps(pbuffer, 9, 2, factors, 8);

                t2ceri::comp_prim_electron_repulsion_ds(pbuffer, 12, 0, 1, 9, factors, 8, a_exp);

                t2ceri::comp_prim_electron_repulsion_fs(pbuffer, 18, 3, 6, 12, factors, 8, a_exp);
...
#endif /* TwoCenterElectronRepulsionRecFS_hpp */
