#ifndef ThreeCenterElectronRepulsionGeom100RecPPG_hpp
#define ThreeCenterElectronRepulsionGeom100RecPPG_hpp
...
/// @brief Computes d^(1)/dA^(1)(P|1/|r-r'||PG)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom100_ppg(T& distributor,
                                    ...
                                    const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    CSimdArray<double> pbuffer(592, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t3ceri::comp_prim_electron_repulsion_psf(pbuffer, 238, 41, 85, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_psg(pbuffer, 268, 85, 145, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_psh(pbuffer, 313, 145, 217, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_dsg(pbuffer, 376, 115, 130, 238, 268, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_dsh(pbuffer, 466, 175, 196, 268, 313, pfactors, 26, a_exp);
                ...
                pbuffer.scale(2.0 * a_exp, {376, 466});

                pbuffer.scale(2.0 * a_exp, {466, 592});

                t2cfunc::reduce(cbuffer, 360, pbuffer, 376, 90, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 450, pbuffer, 466, 126, ket_width, ket_npgtos);

            }

            t3ceri::comp_bra_geom1_electron_repulsion_pxx(cbuffer, 36, 0, 360, 0, 4);

            t3ceri::comp_bra_geom1_electron_repulsion_pxx(cbuffer, 171, 15, 450, 0, 5);

            t3cfunc::bra_transform<1>(skbuffer, 0, cbuffer, 36, 0, 4);

            t3cfunc::bra_transform<1>(skbuffer, 45, cbuffer, 81, 0, 4);

            t3cfunc::bra_transform<1>(skbuffer, 90, cbuffer, 126, 0, 4);

            t3cfunc::bra_transform<1>(skbuffer, 135, cbuffer, 171, 0, 5);

            t3cfunc::bra_transform<1>(skbuffer, 198, cbuffer, 234, 0, 5);

            t3cfunc::bra_transform<1>(skbuffer, 261, cbuffer, 297, 0, 5);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 324, 0, 135, cfactors, 6, 1);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 459, 45, 198, cfactors, 6, 1);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 594, 90, 261, cfactors, 6, 1);

            t3cfunc::ket_transform<1, 4>(sbuffer, 0, skbuffer, 324, 1);

            t3cfunc::ket_transform<1, 4>(sbuffer, 81, skbuffer, 459, 1);

            t3cfunc::ket_transform<1, 4>(sbuffer, 162, skbuffer, 594, 1);

            distributor.distribute(sbuffer, 0, bra_gto_indices, c_indices, d_indices, 1, 1, 4, j, ket_range);
        }
    }
...
#endif /* ThreeCenterElectronRepulsionGeom100RecPPG_hpp */
