#ifndef ThreeCenterElectronRepulsionGeom100RecHDF_hpp
#define ThreeCenterElectronRepulsionGeom100RecHDF_hpp
...
/// @brief Computes d^(1)/dA^(1)(H|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom100_hdf(T& distributor,
                                    ...
                                    const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    CSimdArray<double> cbuffer(6164, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(5973, 1);

    // allocate aligned spherical integrals

    CSimdArray<double> sbuffer(1155, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 4561, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 150, pbuffer, 5011, 225, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 375, pbuffer, 5686, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3588, pbuffer, 7723, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3868, pbuffer, 8003, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 4288, pbuffer, 8423, 588, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {7723, 8003});

                pbuffer.scale(2.0 * a_exp, {8003, 8423});

                pbuffer.scale(2.0 * a_exp, {8423, 9011});

                t2cfunc::reduce(cbuffer, 4876, pbuffer, 7723, 280, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5156, pbuffer, 8003, 420, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 5576, pbuffer, 8423, 588, ket_width, ket_npgtos);

            }

            t3ceri::comp_bra_geom1_electron_repulsion_hxx(cbuffer, 690, 0, 4876, 0, 3);

            t3ceri::comp_bra_geom1_electron_repulsion_hxx(cbuffer, 1320, 150, 5156, 0, 4);

            t3ceri::comp_bra_geom1_electron_repulsion_hxx(cbuffer, 2265, 375, 5576, 0, 5);

            t3cfunc::bra_transform<5>(skbuffer, 0, cbuffer, 690, 0, 3);

            t3cfunc::bra_transform<5>(skbuffer, 110, cbuffer, 900, 0, 3);

            t3cfunc::bra_transform<5>(skbuffer, 220, cbuffer, 1110, 0, 3);

            t3cfunc::bra_transform<5>(skbuffer, 330, cbuffer, 1320, 0, 4);

            t3cfunc::bra_transform<5>(skbuffer, 495, cbuffer, 1635, 0, 4);

            t3cfunc::bra_transform<5>(skbuffer, 660, cbuffer, 1950, 0, 4);

            t3cfunc::bra_transform<5>(skbuffer, 825, cbuffer, 2265, 0, 5);

            t3cfunc::bra_transform<5>(skbuffer, 1056, cbuffer, 2706, 0, 5);

            t3cfunc::bra_transform<5>(skbuffer, 1287, cbuffer, 3147, 0, 5);

            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 1518, 0, 330, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 1848, 110, 495, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 2178, 220, 660, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 2508, 330, 825, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 3003, 495, 1056, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 3498, 660, 1287, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xdf(skbuffer, 3993, 1518, 2508, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xdf(skbuffer, 4653, 1848, 3003, cfactors, 6, 5);

            t3ceri::comp_hrr_electron_repulsion_xdf(skbuffer, 5313, 2178, 3498, cfactors, 6, 5);

            t3cfunc::ket_transform<2, 3>(sbuffer, 0, skbuffer, 3993, 5);

            t3cfunc::ket_transform<2, 3>(sbuffer, 385, skbuffer, 4653, 5);

            t3cfunc::ket_transform<2, 3>(sbuffer, 770, skbuffer, 5313, 5);
...
#endif /* ThreeCenterElectronRepulsionGeom100RecHDF_hpp */
