#ifndef ThreeCenterElectronRepulsionGeom100RecSPP_hpp
#define ThreeCenterElectronRepulsionGeom100RecSPP_hpp
...
/// @brief Computes d^(1)/dA^(1)(S|1/|r-r'||PP)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom100_spp(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * a_exp, {15, 24});

                pbuffer.scale(2.0 * a_exp, {24, 42});
            ...
            t3cfunc::ket_transform<1, 1>(sbuffer, 0, skbuffer, 27, 0);

            t3cfunc::ket_transform<1, 1>(sbuffer, 9, skbuffer, 36, 0);

            t3cfunc::ket_transform<1, 1>(sbuffer, 18, skbuffer, 45, 0);
...
#endif /* ThreeCenterElectronRepulsionGeom100RecSPP_hpp */
