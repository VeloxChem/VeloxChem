#ifndef ThreeCenterElectronRepulsionGeom100RecSFF_hpp
#define ThreeCenterElectronRepulsionGeom100RecSFF_hpp
...
/// @brief Computes d^(1)/dA^(1)(S|1/|r-r'||FF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom100_sff(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * a_exp, {210, 240});

                pbuffer.scale(2.0 * a_exp, {240, 285});

                pbuffer.scale(2.0 * a_exp, {285, 348});

                pbuffer.scale(2.0 * a_exp, {348, 432});
            ...
            t3cfunc::bra_transform<0>(skbuffer, 20, cbuffer, 20, 0, 3);

            t3cfunc::bra_transform<0>(skbuffer, 30, cbuffer, 30, 0, 4);

            t3cfunc::bra_transform<0>(skbuffer, 45, cbuffer, 45, 0, 4);

            t3cfunc::bra_transform<0>(skbuffer, 60, cbuffer, 60, 0, 4);

            t3cfunc::bra_transform<0>(skbuffer, 75, cbuffer, 75, 0, 5);

            t3cfunc::bra_transform<0>(skbuffer, 96, cbuffer, 96, 0, 5);

            t3cfunc::bra_transform<0>(skbuffer, 117, cbuffer, 117, 0, 5);

            t3cfunc::bra_transform<0>(skbuffer, 138, cbuffer, 138, 0, 6);

            t3cfunc::bra_transform<0>(skbuffer, 166, cbuffer, 166, 0, 6);

            t3cfunc::bra_transform<0>(skbuffer, 194, cbuffer, 194, 0, 6);
            ...
            t3cfunc::ket_transform<3, 3>(sbuffer, 0, skbuffer, 1086, 0);

            t3cfunc::ket_transform<3, 3>(sbuffer, 49, skbuffer, 1186, 0);

            t3cfunc::ket_transform<3, 3>(sbuffer, 98, skbuffer, 1286, 0);
...
#endif /* ThreeCenterElectronRepulsionGeom100RecSFF_hpp */
