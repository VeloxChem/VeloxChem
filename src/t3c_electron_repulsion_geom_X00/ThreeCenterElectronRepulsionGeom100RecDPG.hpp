#ifndef ThreeCenterElectronRepulsionGeom100RecDPG_hpp
#define ThreeCenterElectronRepulsionGeom100RecDPG_hpp
...
/// @brief Computes d^(1)/dA^(1)(D|1/|r-r'||PG)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom100_dpg(T& distributor,
                                    ...
                                    const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    CSimdArray<double> pbuffer(1246, ket_npgtos);

    // allocate aligned Cartesian integrals

    CSimdArray<double> cbuffer(1116, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(1215, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t3ceri::comp_prim_electron_repulsion_psg(pbuffer, 286, 65, 115, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_psg(pbuffer, 331, 75, 130, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_psg(pbuffer, 376, 85, 145, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_psh(pbuffer, 421, 115, 175, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_psh(pbuffer, 484, 130, 196, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_psh(pbuffer, 547, 145, 217, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_dsf(pbuffer, 610, 65, 75, 238, 256, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_dsg(pbuffer, 670, 115, 130, 256, 376, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_dsh(pbuffer, 760, 175, 196, 376, 547, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_fsg(pbuffer, 886, 286, 331, 610, 670, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_fsh(pbuffer, 1036, 421, 484, 670, 760, pfactors, 26, a_exp);

                t2cfunc::reduce(cbuffer, 0, pbuffer, 286, 45, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 45, pbuffer, 421, 63, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {886, 1036});

                pbuffer.scale(2.0 * a_exp, {1036, 1246});

                t2cfunc::reduce(cbuffer, 756, pbuffer, 886, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 906, pbuffer, 1036, 210, ket_width, ket_npgtos);

            }

            t3ceri::comp_bra_geom1_electron_repulsion_dxx(cbuffer, 108, 0, 756, 0, 4);

            t3ceri::comp_bra_geom1_electron_repulsion_dxx(cbuffer, 378, 45, 906, 0, 5);

            t3cfunc::bra_transform<2>(skbuffer, 0, cbuffer, 108, 0, 4);

            t3cfunc::bra_transform<2>(skbuffer, 75, cbuffer, 198, 0, 4);

            t3cfunc::bra_transform<2>(skbuffer, 150, cbuffer, 288, 0, 4);

            t3cfunc::bra_transform<2>(skbuffer, 225, cbuffer, 378, 0, 5);

            t3cfunc::bra_transform<2>(skbuffer, 330, cbuffer, 504, 0, 5);

            t3cfunc::bra_transform<2>(skbuffer, 435, cbuffer, 630, 0, 5);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 540, 0, 225, cfactors, 6, 2);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 765, 75, 330, cfactors, 6, 2);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 990, 150, 435, cfactors, 6, 2);

            t3cfunc::ket_transform<1, 4>(sbuffer, 0, skbuffer, 540, 2);

            t3cfunc::ket_transform<1, 4>(sbuffer, 135, skbuffer, 765, 2);

            t3cfunc::ket_transform<1, 4>(sbuffer, 270, skbuffer, 990, 2);
...
#endif /* ThreeCenterElectronRepulsionGeom100RecDPG_hpp */
