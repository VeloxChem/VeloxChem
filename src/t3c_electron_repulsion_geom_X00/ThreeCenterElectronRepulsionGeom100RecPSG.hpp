#ifndef ThreeCenterElectronRepulsionGeom100RecPSG_hpp
#define ThreeCenterElectronRepulsionGeom100RecPSG_hpp
...
/// @brief Computes d^(1)/dA^(1)(P|1/|r-r'||SG)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom100_psg(T& distributor,
                                    ...
                                    const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    CSimdArray<double> pbuffer(305, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t3ceri::comp_prim_electron_repulsion_psf(pbuffer, 140, 37, 75, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_psg(pbuffer, 170, 75, 125, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_dsg(pbuffer, 215, 95, 110, 140, 170, pfactors, 26, a_exp);

                t2cfunc::reduce(cbuffer, 0, pbuffer, 95, 15, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {215, 305});

                t2cfunc::reduce(cbuffer, 150, pbuffer, 215, 90, ket_width, ket_npgtos);

            }

            t3ceri::comp_bra_geom1_electron_repulsion_pxx(cbuffer, 15, 0, 150, 0, 4);

            t3cfunc::bra_transform<1>(skbuffer, 0, cbuffer, 15, 0, 4);

            t3cfunc::bra_transform<1>(skbuffer, 45, cbuffer, 60, 0, 4);

            t3cfunc::bra_transform<1>(skbuffer, 90, cbuffer, 105, 0, 4);

            t3cfunc::ket_transform<0, 4>(sbuffer, 0, skbuffer, 0, 1);

            t3cfunc::ket_transform<0, 4>(sbuffer, 27, skbuffer, 45, 1);

            t3cfunc::ket_transform<0, 4>(sbuffer, 54, skbuffer, 90, 1);

            distributor.distribute(sbuffer, 0, bra_gto_indices, c_indices, d_indices, 1, 0, 4, j, ket_range);
        }
    }
...
#endif /* ThreeCenterElectronRepulsionGeom100RecPSG_hpp */
