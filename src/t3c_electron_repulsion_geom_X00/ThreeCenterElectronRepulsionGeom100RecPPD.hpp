#ifndef ThreeCenterElectronRepulsionGeom100RecPPD_hpp
#define ThreeCenterElectronRepulsionGeom100RecPPD_hpp
...
/// @brief Computes d^(1)/dA^(1)(P|1/|r-r'||PD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom100_ppd(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * a_exp, {132, 168});

                pbuffer.scale(2.0 * a_exp, {168, 228});

                t2cfunc::reduce(cbuffer, 160, pbuffer, 132, 36, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 196, pbuffer, 168, 60, ket_width, ket_npgtos);

            }

            t3ceri::comp_bra_geom1_electron_repulsion_pxx(cbuffer, 16, 0, 160, 0, 2);

            t3ceri::comp_bra_geom1_electron_repulsion_pxx(cbuffer, 70, 6, 196, 0, 3);

            t3cfunc::bra_transform<1>(skbuffer, 0, cbuffer, 16, 0, 2);

            t3cfunc::bra_transform<1>(skbuffer, 18, cbuffer, 34, 0, 2);

            t3cfunc::bra_transform<1>(skbuffer, 36, cbuffer, 52, 0, 2);

            t3cfunc::bra_transform<1>(skbuffer, 54, cbuffer, 70, 0, 3);

            t3cfunc::bra_transform<1>(skbuffer, 84, cbuffer, 100, 0, 3);

            t3cfunc::bra_transform<1>(skbuffer, 114, cbuffer, 130, 0, 3);

            t3ceri::comp_hrr_electron_repulsion_xpd(skbuffer, 144, 0, 54, cfactors, 6, 1);

            t3ceri::comp_hrr_electron_repulsion_xpd(skbuffer, 198, 18, 84, cfactors, 6, 1);

            t3ceri::comp_hrr_electron_repulsion_xpd(skbuffer, 252, 36, 114, cfactors, 6, 1);

            t3cfunc::ket_transform<1, 2>(sbuffer, 0, skbuffer, 144, 1);

            t3cfunc::ket_transform<1, 2>(sbuffer, 45, skbuffer, 198, 1);

            t3cfunc::ket_transform<1, 2>(sbuffer, 90, skbuffer, 252, 1);

            distributor.distribute(sbuffer, 0, bra_gto_indices, c_indices, d_indices, 1, 1, 2, j, ket_range);
        }
    }
...
#endif /* ThreeCenterElectronRepulsionGeom100RecPPD_hpp */
