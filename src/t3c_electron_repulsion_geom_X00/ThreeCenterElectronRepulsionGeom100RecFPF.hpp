#ifndef ThreeCenterElectronRepulsionGeom100RecFPF_hpp
#define ThreeCenterElectronRepulsionGeom100RecFPF_hpp
...
/// @brief Computes d^(1)/dA^(1)(F|1/|r-r'||PF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom100_fpf(T& distributor,
                                    ...
                                    const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    CSimdArray<double> cbuffer(1275, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 555, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 60, pbuffer, 735, 90, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {1315, 1465});

                pbuffer.scale(2.0 * a_exp, {1465, 1690});

                t2cfunc::reduce(cbuffer, 900, pbuffer, 1315, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 1050, pbuffer, 1465, 225, ket_width, ket_npgtos);

            }

            t3ceri::comp_bra_geom1_electron_repulsion_fxx(cbuffer, 150, 0, 900, 0, 3);

            t3ceri::comp_bra_geom1_electron_repulsion_fxx(cbuffer, 450, 60, 1050, 0, 4);

            t3cfunc::bra_transform<3>(skbuffer, 0, cbuffer, 150, 0, 3);

            t3cfunc::bra_transform<3>(skbuffer, 70, cbuffer, 250, 0, 3);

            t3cfunc::bra_transform<3>(skbuffer, 140, cbuffer, 350, 0, 3);

            t3cfunc::bra_transform<3>(skbuffer, 210, cbuffer, 450, 0, 4);

            t3cfunc::bra_transform<3>(skbuffer, 315, cbuffer, 600, 0, 4);

            t3cfunc::bra_transform<3>(skbuffer, 420, cbuffer, 750, 0, 4);

            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 525, 0, 210, cfactors, 6, 3);

            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 735, 70, 315, cfactors, 6, 3);

            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 945, 140, 420, cfactors, 6, 3);

            t3cfunc::ket_transform<1, 3>(sbuffer, 0, skbuffer, 525, 3);

            t3cfunc::ket_transform<1, 3>(sbuffer, 147, skbuffer, 735, 3);

            t3cfunc::ket_transform<1, 3>(sbuffer, 294, skbuffer, 945, 3);

            distributor.distribute(sbuffer, 0, bra_gto_indices, c_indices, d_indices, 3, 1, 3, j, ket_range);
        }
    }
...
#endif /* ThreeCenterElectronRepulsionGeom100RecFPF_hpp */
