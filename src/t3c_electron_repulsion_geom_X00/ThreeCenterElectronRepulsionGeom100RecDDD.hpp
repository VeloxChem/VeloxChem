#ifndef ThreeCenterElectronRepulsionGeom100RecDDD_hpp
#define ThreeCenterElectronRepulsionGeom100RecDDD_hpp
...
/// @brief Computes d^(1)/dA^(1)(D|1/|r-r'||DD)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom100_ddd(T& distributor,
                                    ...
                                    const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    CSimdArray<double> cbuffer(961, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(1725, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 152, 18, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 18, pbuffer, 206, 30, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 48, pbuffer, 296, 45, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {635, 695});

                pbuffer.scale(2.0 * a_exp, {695, 795});

                pbuffer.scale(2.0 * a_exp, {795, 945});

                t2cfunc::reduce(cbuffer, 651, pbuffer, 635, 60, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 711, pbuffer, 695, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 811, pbuffer, 795, 150, ket_width, ket_npgtos);

            }

            t3ceri::comp_bra_geom1_electron_repulsion_dxx(cbuffer, 93, 0, 651, 0, 2);

            t3ceri::comp_bra_geom1_electron_repulsion_dxx(cbuffer, 201, 18, 711, 0, 3);

            t3ceri::comp_bra_geom1_electron_repulsion_dxx(cbuffer, 381, 48, 811, 0, 4);

            t3cfunc::bra_transform<2>(skbuffer, 0, cbuffer, 93, 0, 2);

            t3cfunc::bra_transform<2>(skbuffer, 30, cbuffer, 129, 0, 2);

            t3cfunc::bra_transform<2>(skbuffer, 60, cbuffer, 165, 0, 2);

            t3cfunc::bra_transform<2>(skbuffer, 90, cbuffer, 201, 0, 3);

            t3cfunc::bra_transform<2>(skbuffer, 140, cbuffer, 261, 0, 3);

            t3cfunc::bra_transform<2>(skbuffer, 190, cbuffer, 321, 0, 3);

            t3cfunc::bra_transform<2>(skbuffer, 240, cbuffer, 381, 0, 4);

            t3cfunc::bra_transform<2>(skbuffer, 315, cbuffer, 471, 0, 4);

            t3cfunc::bra_transform<2>(skbuffer, 390, cbuffer, 561, 0, 4);

            t3ceri::comp_hrr_electron_repulsion_xpd(skbuffer, 465, 0, 90, cfactors, 6, 2);

            t3ceri::comp_hrr_electron_repulsion_xpd(skbuffer, 555, 30, 140, cfactors, 6, 2);

            t3ceri::comp_hrr_electron_repulsion_xpd(skbuffer, 645, 60, 190, cfactors, 6, 2);

            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 735, 90, 240, cfactors, 6, 2);

            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 885, 140, 315, cfactors, 6, 2);

            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 1035, 190, 390, cfactors, 6, 2);

            t3ceri::comp_hrr_electron_repulsion_xdd(skbuffer, 1185, 465, 735, cfactors, 6, 2);

            t3ceri::comp_hrr_electron_repulsion_xdd(skbuffer, 1365, 555, 885, cfactors, 6, 2);

            t3ceri::comp_hrr_electron_repulsion_xdd(skbuffer, 1545, 645, 1035, cfactors, 6, 2);

            t3cfunc::ket_transform<2, 2>(sbuffer, 0, skbuffer, 1185, 2);

            t3cfunc::ket_transform<2, 2>(sbuffer, 125, skbuffer, 1365, 2);

            t3cfunc::ket_transform<2, 2>(sbuffer, 250, skbuffer, 1545, 2);
...
#endif /* ThreeCenterElectronRepulsionGeom100RecDDD_hpp */
