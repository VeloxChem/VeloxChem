#ifndef ThreeCenterElectronRepulsionGeom100RecFSF_hpp
#define ThreeCenterElectronRepulsionGeom100RecFSF_hpp
...
/// @brief Computes d^(1)/dA^(1)(F|1/|r-r'||SF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom100_fsf(T& distributor,
                                    ...
                                    const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    CSimdArray<double> cbuffer(510, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 325, 60, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {665, 815});

                t2cfunc::reduce(cbuffer, 360, pbuffer, 665, 150, ket_width, ket_npgtos);

            }

            t3ceri::comp_bra_geom1_electron_repulsion_fxx(cbuffer, 60, 0, 360, 0, 3);

            t3cfunc::bra_transform<3>(skbuffer, 0, cbuffer, 60, 0, 3);

            t3cfunc::bra_transform<3>(skbuffer, 70, cbuffer, 160, 0, 3);

            t3cfunc::bra_transform<3>(skbuffer, 140, cbuffer, 260, 0, 3);

            t3cfunc::ket_transform<0, 3>(sbuffer, 0, skbuffer, 0, 3);

            t3cfunc::ket_transform<0, 3>(sbuffer, 49, skbuffer, 70, 3);

            t3cfunc::ket_transform<0, 3>(sbuffer, 98, skbuffer, 140, 3);
...
#endif /* ThreeCenterElectronRepulsionGeom100RecFSF_hpp */
