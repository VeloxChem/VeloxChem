#ifndef ThreeCenterElectronRepulsionGeom100RecSFG_hpp
#define ThreeCenterElectronRepulsionGeom100RecSFG_hpp
...
/// @brief Computes d^(1)/dA^(1)(S|1/|r-r'||FG)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom100_sfg(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                pbuffer.scale(2.0 * a_exp, {330, 375});

                pbuffer.scale(2.0 * a_exp, {375, 438});

                pbuffer.scale(2.0 * a_exp, {438, 522});

                pbuffer.scale(2.0 * a_exp, {522, 630});
                ...
                t2cfunc::reduce(cbuffer, 108, pbuffer, 438, 84, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 192, pbuffer, 522, 108, ket_width, ket_npgtos);
            ...
            t3ceri::comp_hrr_electron_repulsion_xph(skbuffer, 435, 45, 108, cfactors, 6, 1);

            t3ceri::comp_hrr_electron_repulsion_xpi(skbuffer, 624, 108, 192, cfactors, 6, 1);

            t3ceri::comp_hrr_electron_repulsion_xdg(skbuffer, 876, 300, 435, cfactors, 6, 1);

            t3ceri::comp_hrr_electron_repulsion_xdh(skbuffer, 1146, 435, 624, cfactors, 6, 1);

            t3ceri::comp_hrr_electron_repulsion_xfg(skbuffer, 1524, 876, 1146, cfactors, 6, 1);

            t3cfunc::ket_transform<3, 4>(sbuffer, 0, skbuffer, 1524, 0);

            t3cfunc::ket_transform<3, 4>(sbuffer, 63, skbuffer, 1674, 0);

            t3cfunc::ket_transform<3, 4>(sbuffer, 126, skbuffer, 1824, 0);
...
#endif /* ThreeCenterElectronRepulsionGeom100RecSFG_hpp */
