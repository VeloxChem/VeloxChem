#ifndef ThreeCenterElectronRepulsionGeom100RecGDF_hpp
#define ThreeCenterElectronRepulsionGeom100RecGDF_hpp
...
/// @brief Computes d^(1)/dA^(1)(G|1/|r-r'||DF)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom100_gdf(T& distributor,
                                    ...
                                    const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    CSimdArray<double> cbuffer(3496, 1);

    // allocate aligned half transformed integrals

    CSimdArray<double> skbuffer(4887, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t2cfunc::reduce(cbuffer, 0, pbuffer, 2174, 100, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 100, pbuffer, 2474, 150, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 250, pbuffer, 2924, 210, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {4334, 4544});

                pbuffer.scale(2.0 * a_exp, {4544, 4859});

                pbuffer.scale(2.0 * a_exp, {4859, 5300});

                t2cfunc::reduce(cbuffer, 2530, pbuffer, 4334, 210, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 2740, pbuffer, 4544, 315, ket_width, ket_npgtos);

                t2cfunc::reduce(cbuffer, 3055, pbuffer, 4859, 441, ket_width, ket_npgtos);

            }

            t3ceri::comp_bra_geom1_electron_repulsion_gxx(cbuffer, 460, 0, 2530, 0, 3);

            t3ceri::comp_bra_geom1_electron_repulsion_gxx(cbuffer, 910, 100, 2740, 0, 4);

            t3ceri::comp_bra_geom1_electron_repulsion_gxx(cbuffer, 1585, 250, 3055, 0, 5);

            t3cfunc::bra_transform<4>(skbuffer, 0, cbuffer, 460, 0, 3);

            t3cfunc::bra_transform<4>(skbuffer, 90, cbuffer, 610, 0, 3);

            t3cfunc::bra_transform<4>(skbuffer, 180, cbuffer, 760, 0, 3);

            t3cfunc::bra_transform<4>(skbuffer, 270, cbuffer, 910, 0, 4);

            t3cfunc::bra_transform<4>(skbuffer, 405, cbuffer, 1135, 0, 4);

            t3cfunc::bra_transform<4>(skbuffer, 540, cbuffer, 1360, 0, 4);

            t3cfunc::bra_transform<4>(skbuffer, 675, cbuffer, 1585, 0, 5);

            t3cfunc::bra_transform<4>(skbuffer, 864, cbuffer, 1900, 0, 5);

            t3cfunc::bra_transform<4>(skbuffer, 1053, cbuffer, 2215, 0, 5);

            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 1242, 0, 270, cfactors, 6, 4);

            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 1512, 90, 405, cfactors, 6, 4);

            t3ceri::comp_hrr_electron_repulsion_xpf(skbuffer, 1782, 180, 540, cfactors, 6, 4);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 2052, 270, 675, cfactors, 6, 4);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 2457, 405, 864, cfactors, 6, 4);

            t3ceri::comp_hrr_electron_repulsion_xpg(skbuffer, 2862, 540, 1053, cfactors, 6, 4);

            t3ceri::comp_hrr_electron_repulsion_xdf(skbuffer, 3267, 1242, 2052, cfactors, 6, 4);

            t3ceri::comp_hrr_electron_repulsion_xdf(skbuffer, 3807, 1512, 2457, cfactors, 6, 4);

            t3ceri::comp_hrr_electron_repulsion_xdf(skbuffer, 4347, 1782, 2862, cfactors, 6, 4);

            t3cfunc::ket_transform<2, 3>(sbuffer, 0, skbuffer, 3267, 4);

            t3cfunc::ket_transform<2, 3>(sbuffer, 315, skbuffer, 3807, 4);

            t3cfunc::ket_transform<2, 3>(sbuffer, 630, skbuffer, 4347, 4);
...
#endif /* ThreeCenterElectronRepulsionGeom100RecGDF_hpp */
