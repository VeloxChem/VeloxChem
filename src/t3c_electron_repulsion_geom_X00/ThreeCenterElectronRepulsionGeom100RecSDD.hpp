                                    ...
                                    const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            t3cfunc::bra_transform<1>(skbuffer, 6, cbuffer, 6, 0, 2);

            t3cfunc::bra_transform<1>(skbuffer, 12, cbuffer, 12, 0, 2);

            t3cfunc::bra_transform<1>(skbuffer, 18, cbuffer, 18, 0, 3);

            t3cfunc::bra_transform<1>(skbuffer, 28, cbuffer, 28, 0, 3);

            t3cfunc::bra_transform<1>(skbuffer, 38, cbuffer, 38, 0, 3);

            t3cfunc::bra_transform<1>(skbuffer, 48, cbuffer, 48, 0, 4);

            t3cfunc::bra_transform<1>(skbuffer, 63, cbuffer, 63, 0, 4);

            t3cfunc::bra_transform<1>(skbuffer, 78, cbuffer, 78, 0, 4);
