#ifndef ThreeCenterElectronRepulsionGeom100RecFSG_hpp
#define ThreeCenterElectronRepulsionGeom100RecFSG_hpp
...
/// @brief Computes d^(1)/dA^(1)(F|1/|r-r'||SG)  integral derivatives.
...
template <class T>
inline auto
comp_electron_repulsion_geom100_fsg(T& distributor,
                                    ...
                                    const std::pair<size_t, size_t>& bra_range) -> void
{
    ...
    CSimdArray<double> pbuffer(1303, ket_npgtos);

    // allocate aligned Cartesian integrals

    CSimdArray<double> cbuffer(765, 1);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_range.first; j < bra_range.second; j++)
        {
            ...
            for (int k = 0; k < bra_npgtos; k++)
            {
                ...
                t3ceri::comp_prim_electron_repulsion_psp(pbuffer, 210, 4, 21, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_psd(pbuffer, 219, 21, 57, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_psf(pbuffer, 237, 45, 95, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_psf(pbuffer, 267, 51, 105, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_psf(pbuffer, 297, 57, 115, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_psg(pbuffer, 327, 95, 165, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_psg(pbuffer, 372, 105, 180, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_psg(pbuffer, 417, 115, 195, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_dsd(pbuffer, 462, 45, 51, 210, 219, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_dsf(pbuffer, 498, 95, 105, 219, 297, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_dsg(pbuffer, 558, 135, 150, 237, 327, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_dsg(pbuffer, 648, 150, 165, 267, 372, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_dsg(pbuffer, 738, 165, 180, 297, 417, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_fsf(pbuffer, 828, 237, 267, 462, 498, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_fsg(pbuffer, 928, 327, 372, 498, 738, pfactors, 26, a_exp);

                t3ceri::comp_prim_electron_repulsion_gsg(pbuffer, 1078, 558, 648, 828, 928, pfactors, 26, a_exp);

                t2cfunc::reduce(cbuffer, 0, pbuffer, 558, 90, ket_width, ket_npgtos);

                pbuffer.scale(2.0 * a_exp, {1078, 1303});

                t2cfunc::reduce(cbuffer, 540, pbuffer, 1078, 225, ket_width, ket_npgtos);

            }

            t3ceri::comp_bra_geom1_electron_repulsion_fxx(cbuffer, 90, 0, 540, 0, 4);

            t3cfunc::bra_transform<3>(skbuffer, 0, cbuffer, 90, 0, 4);

            t3cfunc::bra_transform<3>(skbuffer, 105, cbuffer, 240, 0, 4);

            t3cfunc::bra_transform<3>(skbuffer, 210, cbuffer, 390, 0, 4);

            t3cfunc::ket_transform<0, 4>(sbuffer, 0, skbuffer, 0, 3);

            t3cfunc::ket_transform<0, 4>(sbuffer, 63, skbuffer, 105, 3);

            t3cfunc::ket_transform<0, 4>(sbuffer, 126, skbuffer, 210, 3);

            distributor.distribute(sbuffer, 0, bra_gto_indices, c_indices, d_indices, 3, 0, 4, j, ket_range);
        }
    }
...
#endif /* ThreeCenterElectronRepulsionGeom100RecFSG_hpp */
