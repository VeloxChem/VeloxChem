...
class TddftOrbitalResponse(CphfSolver):
    ...
    def compute_rhs(self, molecule, basis, scf_tensors, rsp_results):
        ...
        if self.rank == mpi_master():
            ...
            print("Loading pre-requisites took : ", tm.time() - start, "s.")
            start = tm.time()

            # Transform the vectors to the AO basis
            print("vecs to AO...")
            print(xpy.shape, xmy.shape)
            ...
            for i in range(dof):
                ...
                dm_oo[i] = -0.5 * (np.matmul(xpy[i], xpy[i].T)
                                   + np.matmul(xmy[i], xmy[i].T))
                dm_vv[i] = 0.5 * (np.matmul(xpy[i].T, xpy[i])
                                + np.matmul(xmy[i].T, xmy[i]))

                print("unrel_dm_ao:")
                # Transform unrelaxed one-particle density matrix to AO basis
                unrel_dm_ao[i] = (np.linalg.multi_dot(
                                    [mo_occ, dm_oo[i], mo_occ.T])
                                  + np.linalg.multi_dot(
                                        [mo_vir, dm_vv[i], mo_vir.T]))
            #xpy_ao = np.einsum('mi,sia,na->smn', mo_occ, xpy, mo_vir, )
            #xmy_ao = np.einsum('mi,sia,na->smn', mo_occ, xmy, mo_vir)

            #print("DM_oo...")
            ## Calcuate the unrelaxed one-particle density matrix in MO basis
            #dm_oo = -0.5 * (  np.einsum('sia,sja->sij', xpy, xpy, optimize=True)
            #                + np.einsum('sia,sja->sij', xmy, xmy, optimize=True)
            #                )
            #print("Dm_vv...")
            #dm_vv = 0.5 * (  np.einsum('sia,sib->sab', xpy, xpy, optimize=True)
            #               + np.einsum('sia,sib->sab', xmy, xmy, optimize=True)
            #                )
            ...
            print("1PDMs and transforming to AO took : ", tm.time() - start, "s.")
            start = tm.time()
        ...
        # (not 1PDM part)
        # Should this be done on the master node?
        ...
        if self.rank == mpi_master():
            ...
            print("Fock matrices took : ", tm.time() - start, "s.")
            start = tm.time()
            print("fmo...")
            ...
            for i in range(dof):
                fmo_rhs_1dm = np.linalg.multi_dot(
                    [mo_occ.T, 0.5 * fock_ao_rhs_1pdm[i], mo_vir])

            # TODO: factor out overlap matrix?
                sdp_pds = 0.25 * (
              np.linalg.multi_dot([ovlp, xpy_ao[i], 0.5 * fock_ao_rhs_xpy[i].T])
            + np.linalg.multi_dot([ovlp, xmy_ao[i], 0.5 * fock_ao_rhs_xmy[i].T])
            - np.linalg.multi_dot([0.5 * fock_ao_rhs_xpy[i].T, xpy_ao[i], ovlp])
            - np.linalg.multi_dot([0.5 * fock_ao_rhs_xmy[i].T, xmy_ao[i], ovlp])
            - np.linalg.multi_dot([ovlp, xpy_ao[i], 0.5 * fock_ao_rhs_xpy[i]])
            + np.linalg.multi_dot([ovlp, xmy_ao[i], 0.5 * fock_ao_rhs_xmy[i]])
            + np.linalg.multi_dot([0.5 * fock_ao_rhs_xpy[i], xpy_ao[i], ovlp])
            - np.linalg.multi_dot([0.5 * fock_ao_rhs_xmy[i], xmy_ao[i], ovlp]))

                rhs_mo[i] = fmo_rhs_1dm + np.linalg.multi_dot(
            ...
            ##            )
            print("einsum and transforming to MO took : ", tm.time() - start, "s.")
            start = tm.time()
