...
class LoPropDriver:
    ...
    def compute(self, molecule, basis, scf_tensors):
        ...
        if self.rank == mpi_master():
            ...
            # nuclear charge
            nuclear_charge = molecule.get_identifiers()
            for a in range(natoms):
    ...
    def get_ao_indices(self, molecule, basis):
        ...
        basis_info = {}
        for atombasis in basis.basis_sets():
            element_id = atombasis.get_identifier()
            basis_info[element_id] = [
                angmom_str[basisfunc.get_angular_momentum()]
                for basisfunc in atombasis.get_basis_functions()
            ]
        ...
        ao_vir = []
        iterr = 0
        for element_id in molecule.get_identifiers():
