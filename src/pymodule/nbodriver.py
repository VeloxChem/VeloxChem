...
def _candidate_atom_index(candidate, key):
    """Return a one-based atom index for a candidate metadata field."""

    if key not in candidate or candidate.get(key) is None:
        return None
    return int(candidate.get(key)) + 1


def _build_metal_ligand_diagnostics(candidates):
    """Build NBO-facing metal-ligand diagnostic records.

    Metal-ligand candidates are generated by the shared OrbitalAnalyzer as
    diagnostic donation/back-donation channels. They are exposed here for NBO
    interpretation, but they are not part of the primary Lewis/NBO assignment.
    """
    ...
    for candidate in candidates:
        ...
        record = {
            ...
            'subtype': candidate.get('subtype'),
            'atoms': _candidate_atom_indices(candidate),
            'metal_atom': _candidate_atom_index(candidate, 'metal_atom'),
            'ligand_atom': _candidate_atom_index(candidate, 'ligand_atom'),
            'donor_atom': _candidate_atom_index(candidate, 'donor_atom'),
            'acceptor_atom': _candidate_atom_index(candidate, 'acceptor_atom'),
            'channel': candidate.get('channel'),
            'donor_acceptor_role': candidate.get('donor_acceptor_role'),
            'coordination_mode': candidate.get('coordination_mode'),
        ...
        for key in (
            ...
            if key in candidate and candidate.get(key) is not None:
                record[key] = float(candidate.get(key))
        if 'polarization' in candidate:
            record['polarization'] = candidate.get('polarization')
        diagnostics.append(record)

    return sorted(
        diagnostics,
        key=lambda item: (
            tuple(item.get('atoms', ())),
            item.get('subtype') or '',
            -float(item.get('interaction_strength', 0.0)),
...
class NboDriver:
    ...
    def compute(self,
        ...
        if self.rank == mpi_master():
            ...
            metal_ligand_diagnostics = _build_metal_ligand_diagnostics(
                nbo_candidates)
            ...
            if diagnostics:
                ...
                diagnostics['metal_ligand_summary'] = {
                    'candidate_count': len(metal_ligand_diagnostics),
                    'channels': sorted({
                        item.get('channel')
                        for item in metal_ligand_diagnostics
                        if item.get('channel')
                    }),
                }
