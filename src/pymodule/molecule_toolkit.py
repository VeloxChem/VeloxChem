    ...
    **All atom indices are 1-based**, following VeloxChem convention.
    ...
    def _get_molecule_graph(self, vlx_mol) -> nx.Graph:
        ...
        for i in range(len(coords)):
            for j in range(i + 1, len(coords)):
                r_sum = self._cov_radius(labels[i]) + self._cov_radius(
                    labels[j])
                if np.linalg.norm(coords[i] - coords[j]) < 1.3 * r_sum:
                    G.add_edge(i, j)
        return G

    def _get_neighbors_by_radii(self, coords: np.ndarray, labels: list,
        ...
        return np.array(
            [
                ...
                if j != atom_idx and np.linalg.norm(coords[j] -
                                                    coords[atom_idx]) < 1.3 *
                (r_i + self._cov_radius(lbl))
            ],
            dtype=int,
        )
    ...
    def get_rotation_matrix(self, a: np.ndarray, b: np.ndarray) -> np.ndarray:
        ...
        if np.linalg.norm(v) < 1e-9:
            ...
            perp = np.array([1., 0., 0.]) if abs(a[0]) < 0.9 else np.array(
                [0., 1., 0.])
            ax = np.cross(a, perp)
    ...
    def rotate_vector(self, v: np.ndarray, axis: np.ndarray,
    ...
    def get_geometry_vectors(self, geometry: str) -> np.ndarray:
        ...
        if key not in geoms:
            raise ValueError(
                f"Unknown geometry '{geometry}'. Choose from: {list(geoms)}")
        return geoms[key]
    ...
    def _lone_pair_and_nbonds(self, coords: np.ndarray, labels: list,
                              atom_idx: int) -> tuple[np.ndarray, int]:
        """
        ...
        if norm < 1e-9:
            v0 = (coords[last_j] - pos_i) / np.linalg.norm(coords[last_j] -
                                                           pos_i)
            perp = np.array([1., 0., 0.]) if abs(v0[0]) < 0.9 else np.array(
                [0., 1., 0.])
            lp = np.cross(v0, perp)
    ...
    def get_lone_pair_vector(self, coords: np.ndarray, labels: list,
                             atom_idx: int) -> np.ndarray:
        """
        ...
        if len(l_labels) > 1 and len(c_coords) > 1:
            ...
            for deg in np.linspace(0, 360, 360):
                ...
                K = np.array([[0, -axis[2], axis[1]], [axis[2], 0, -axis[0]],
                              [-axis[1], axis[0], 0]])
                R = np.eye(3) + np.sin(angle) * K + (1 -
                                                     np.cos(angle)) * (K @ K)
                trial = pivot + (l_coords - pivot) @ R.T
                dists = [
                    np.linalg.norm(trial[i] - c_coords[j])
                    for i in range(len(trial)) for j in range(len(c_coords))
                    if j != metal_idx
                ]
            ...
            K = np.array([[0, -axis[2], axis[1]], [axis[2], 0, -axis[0]],
                          [-axis[1], axis[0], 0]])
            R = np.eye(
                3) + np.sin(best_angle) * K + (1 - np.cos(best_angle)) * (K @ K)
    ...
    def _find_ring_carbons_2d(self, n1_2d, n2_2d, bis_2d, n_c_bond, c_c_bond):
        """
        ...
        for theta in np.linspace(0, 2 * np.pi, 72000):
            ...
            if err < best_err and np.dot(c1,
                                         bis_2d) > np.dot(n1_2d, bis_2d) * 0.5:
                best_err = err
    ...
    def add_universal_bidentate(
        ...
        if np.linalg.norm(ring_normal) < 1e-9:
            perp = np.array([1., 0., 0.]) if abs(t1[0]) < 0.9 else np.array(
                [0., 1., 0.])
            ring_normal = np.cross(t1, perp)
        ring_normal /= np.linalg.norm(ring_normal)

        e1 = t1.copy()
        e2 = t2 - np.dot(t2, e1) * e1
        ...
        def bonded_to(idx):
            return [
                j for j in range(len(l_labels))
                if j != idx and np.linalg.norm(l_coords[j] -
                                               l_coords[idx]) < 1.3 *
                (cr(l_labels[idx]) + cr(l_labels[j]))
            ]
        ...
        result2d = self._find_ring_carbons_2d(n1_2d, n2_2d, bis_2d, n_c_bond,
                                              c_c_bond)
        if result2d is None:
            mid = (n1_2d + n2_2d) / 2.0
        ...
        new_lc = np.array(
            [N1, C1, C2, N2, hN1a, hN1b, hC1a, hC1b, hC2a, hC2b, hN2a, hN2b])
        return vlx.Molecule(c_labels + new_ll, np.vstack([c_coords, new_lc]),
                            'angstrom')

    # ------------------------------------------------------------------

    def find_chelate_donors(self, ligand_mol, el1: str,
                            el2: str) -> tuple[int, int]:
        """
        ...
        * Asymmetric case (e.g. ``'N','C'`` for ppy):
        ...
        if el1 == el2:
            if len(el1_atoms) < 2:
                raise ValueError(
                    f"Need ≥2 '{el1}' atoms, found {len(el1_atoms)}")
        ...
        if not other_rings:
            raise ValueError(
                "Could not find a second ring for C^N donor detection")
        other_ring = other_rings[0]

        bridging = next(
            ((u, v)
             for u, v in G.edges() if (u in N_ring and v in other_ring) or (
                 v in N_ring and u in other_ring)),
            None,
        )
        if bridging is None:
        ...
        ortho = [
            n for n in G.neighbors(C_bridge_ph)
    ...
    def add_bidentate_ligand(
        ...
        R_align = self.get_rotation_matrix(l_c[idx1] - l_c[idx2],
                                           target1 - target2)
        l_c = l_c @ R_align.T
        ...
        for deg in np.linspace(0.0, 360.0, 3600, endpoint=False):
            ...
            R_try = np.eye(3) + np.sin(angle) * K + (1 -
                                                     np.cos(angle)) * (K @ K)
        ...
        K = np.array([[0, -nn_axis[2],
                       nn_axis[1]], [nn_axis[2], 0, -nn_axis[0]],
                      [-nn_axis[1], nn_axis[0], 0]])
        R_best = np.eye(
            3) + np.sin(best_angle) * K + (1 - np.cos(best_angle)) * (K @ K)
        l_c = (l_c - pivot) @ R_best.T + pivot

        return vlx.Molecule(c_labels + l_labels, np.vstack([c_coords, l_c]),
                            'angstrom')
            ...
            if is_eta2 and len(c_coords) > 1:
                ...
                if ml_vecs:
                    sins = [
                        np.sqrt(
                            max(0., 1. -
                                np.dot(trial_cc, v / np.linalg.norm(v))**2))
                        for v in ml_vecs
                    ]
                    geom = min(sins)
                else:
    ...
    def prepare_bidentate_conformation(
        ...
        axes_init = [
            (coords[b] - coords[a]) / np.linalg.norm(coords[b] - coords[a])
        ...
        for k in range(len(rotatable)):
            ...
            new_cumR = np.einsum('nmij,nmjk->nmik', R_k,
                                 cumR[:, None].repeat(len(angles), axis=1))
            n_combos *= len(angles)
            cumR = new_cumR.reshape(n_combos, 3, 3)

        lp2_all = np.einsum('nij,j->ni', cumR, lp2_init)
        lp2_all /= np.maximum(np.linalg.norm(lp2_all, axis=1, keepdims=True),
                              1e-10)
        scores = lp2_all @ lp1
        ...
        rem = best_i
        for _ in rotatable:
            best_indices.append(rem % len(angles))
            rem //= len(angles)
        best_indices.reverse()

        result = coords.copy()
        for k, (a, b) in enumerate(rotatable):
            axis = result[b] - result[a]
    ...
    def remove_H_from_donor(self, ligand_mol, donor_idx: int):
        ...
        h_idx = next(
            (i for i, lbl in enumerate(labels) if i != donor_idx and lbl == 'H'
             and np.linalg.norm(coords[i] - coords[donor_idx]) < 1.3 *
             (r_d + self._cov_radius('H'))),
            None,
        )
        if h_idx is None:
            raise ValueError(
                f"No H found on atom {donor_idx} ({labels[donor_idx]})")
    ...
    def remove_ligand(self, complex_mol, metal_idx: int, donor_idx: int):
        ...
        coords = np.array(complex_mol.get_coordinates_in_angstrom(),
                          dtype=float)
        G = self._get_molecule_graph(complex_mol)
        if not G.has_edge(metal_idx, donor_idx):
            raise ValueError(
                f"Atoms {metal_idx} and {donor_idx} are not bonded")
        Gc = G.copy()
        Gc.remove_edge(metal_idx, donor_idx)
    ...
    def transmetalate(self, vlx_mol, metal_idx: int, new_metal_symbol: str):
        ...
        delta = self._cov_radius(new_metal_symbol) - self._cov_radius(
            labels[metal_idx])
        ...
        for nb in G.neighbors(metal_idx):
            Gc = G.copy()
            Gc.remove_edge(metal_idx, nb)
            u = coords[nb] - metal_pos
            u /= np.linalg.norm(u)
    ...
    def _prepare_organic_fragment(self, obj, index=None):
        ...
        * a VeloxChem Molecule (``index`` is the bonding atom, 0-based).
        ...
        if isinstance(obj, str):
            ...
            join_idx = next(i for i, a in enumerate(mol_clean.GetAtoms())
            ...
            h_nb_idx = next(
                (n.GetIdx()
                 for n in join_atom.GetNeighbors() if n.GetSymbol() == 'H'),
                None)
            if h_nb_idx is None:
                # No H on join atom — point away from all neighbors
                nb_pos = np.mean(
                    [conf[n.GetIdx()] for n in join_atom.GetNeighbors()],
                    axis=0)
            ...
            f_coords = conf[keep]
            new_join = join_idx - (1 if h_nb_idx is not None
                                   and h_nb_idx < join_idx else 0)
        ...
        if h_idx != -1:
            ...
            nbs = [
                ...
                if i != index and np.linalg.norm(coords[i] -
                                                 coords[index]) < 1.8
            ]
    ...
    def _clash_torsion_scan(
        ...
        *coords_b*, minimising steric clash against *coords_a*.
        ...
        for deg in range(0, 360, step_deg):
            ...
            dists = np.linalg.norm(coords_a[:, None, :] - trial[None, :, :],
                                   axis=2)
            clash = np.sum(
                np.maximum(0, (r_a[:, None] + r_b[None, :]) - dists)**2)
            if clash < min_clash:
                min_clash = clash
                best = trial
        return best
    ...
    def link_at_vector(
        ...
        return vlx.Molecule(l_a + l_b, np.concatenate([c_a, c_b], axis=0),
                            'angstrom')
    ...
    def join_molecules(
        ...
        """
        l_a, c_a, j_a, v_a = self._prepare_organic_fragment(
            mol_a, self._i(idx_a))
        l_b, c_b, j_b, v_b = self._prepare_organic_fragment(
            mol_b, self._i(idx_b))
        ...
        pivot = np.array([0., 0., bond_length])
        c_b = self._clash_torsion_scan(c_a,
                                       l_a,
                                       c_b,
                                       l_b,
                                       pivot,
    ...
    def substitute(
        ...
        *old_smiles* should contain a ``*`` atom marking the anchor (the atom
        ...
        GM = isomorphism.GraphMatcher(
            ...
            if n2['is_star'] else n1['element'] == n2['element'],
        )
        ...
        for mapping in GM.subgraph_isomorphisms_iter():
            ...
            if key not in seen:
                unique.append((to_remove, anchor_idx))
                seen.add(key)
        ...
        for to_remove, anchor_idx in unique:
            ...
            final_mol = self.link_at_vector(pruned, new_smiles, new_anch,
                                            exit_vec, dist)
    ...
    def remove_all_fragment_matches(
        ...
        for candidate in range(len(labels)):
            ...
            GM = isomorphism.GraphMatcher(target_G,
                                          query_G,
            ...
            for mapping in GM.subgraph_isomorphisms_iter():
                ...
                cut = sum(1 for u in to_remove for v in target_G.neighbors(u)
                          if v not in to_remove)
                if cut == 1:
                    unique.append((to_remove, anchor))
                    seen.add(key)
                    print(
                        f"[remove_fragment] found {fragment_smiles} at anchor {anchor}"
                    )
    ...
    def add_smiles_radical(
        ...
        v1 = coords[ring_nbs[0]] - tpos
        v2 = coords[ring_nbs[1]] - tpos
        rnorm = np.cross(v1, v2)
        rnorm /= np.linalg.norm(rnorm)
        if sub_idx != -1:
            vsub = (coords[sub_idx] - tpos) / np.linalg.norm(coords[sub_idx] -
                                                             tpos)
        else:
        ...
        fc -= fc[rad]
        fc = fc @ self.get_rotation_matrix(fc[xe] / np.linalg.norm(fc[xe]),
                                           -v_bond_out).T
        f_labels = [
            a.GetSymbol() for i, a in enumerate(frag.GetAtoms()) if i != xe
        ]
    ...
    def run_radical_addition(
        ...
        return [
            iso for idx, gtype in ca_sites for iso in [
                self.add_smiles_radical(molecule, idx, smiles_fragment,
                                        gaff_info)
            ] if iso is not None
        ]
