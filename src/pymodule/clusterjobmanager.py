...
class ClusterJobManager:
    ...
    def connect(self, jobscript_path, input_files, geometry):
        if self.child is None or not self.child.isalive():
            if self.path_on_cluster_given is True:
                ...
                self.child = pexpect.spawn(scp_command, encoding='utf-8', timeout=0)
    ...
    def run_cluster_command(self, command, jobscript_path, input_files, geometry):
        ...
        self.path_on_cluster = self.path_on_cluster.strip()
        command = command.strip()
        full_command = f"cd {self.path_on_cluster}; {command}; echo DONE"
        print(f"Executing command: {repr(full_command)}")  # Debugging output

        # Clear buffer and sync with shell
        self.child.sendline("")
        ...
        while True:
            ...
            if i == 0:  # Found shell prompt
                output += self.child.before
                break
            elif i == 1:  # Found newline
                output += self.child.before + '\n'
                print(self.child.before, end='', flush=True)  # Print output in real-time
            elif i == 2:  # Timeout
                print("Timeout while waiting for command output.")
                break
            elif i == 3:  # EOF
                print("EOF encountered while waiting for command output.")
                break
        return output.strip()
    ...
    def is_job_running(self, job_id):
        ...
        command = f'squeue --job {job_id}'

        #output = self.run_cluster_command(command, None, None, None)
        output_2 = self.run_cluster_command(command, None, None, None)
