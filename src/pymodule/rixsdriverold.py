...
class oldRixsDriver:
    ...
    def scattering_amplitude_tensor(self, omega, core_eigenvalues, val_eigenvalue, intermediate_tdens,
        ...
        omega_product = (val_eigenvalue - core_eigenvalues) * core_eigenvalues
        ...
        if elastic:
            ...
            scatt_amp = np.einsum('n, n, xij, ijn, yab, abn -> xy', e_n, omega_product, dipole_integrals, final_tdens, dipole_integrals, intermediate_tdens, optimize='greedy')

        return scatt_amp

    def cross_section(self, F, prefactor_ratio):
        """
        ...
        """
        sigma = prefactor_ratio * 1/15 * ((2 - (1/2) * np.sin(self.theta) ** 2) * np.sum(np.abs(F)**2) 
    ...
    def compute(self, molecule, basis, scf_tensors, 
                ...
                get_num_vir_orbs=None, gen_abs=False):
        """
        ...
        if cvs_rsp_tensors is not None:
            ...
            try:
                ...
                num_core_orbitals = self.extract_orbitals(cvs_rsp_outfile)
            ...
            try:
                ...
                orbs = self.extract_orbitals(rsp_outfile, sra=self.sra)
                num_core_orbitals = orbs['core']
                num_val_orbitals  = orbs['homo']
                num_vir_orbitals  = orbs['lumo']
                eigvals           = self.extract_eigenvalues(rsp_outfile)
                if get_num_vir_orbs is not None:
                    num_vir_orbitals = get_num_vir_orbs
                core_eigvals      = eigvals[num_val_orbitals*num_vir_orbitals:]
                valence_eigvals   = eigvals[:num_val_orbitals*num_vir_orbitals]
                num_tot_states    = len(eigvals)
        ...
        try:
            ...
            try:
                ...
                self.tda = True
                core_eigvecs    = np.array([cvs_rsp_tensors['eigenvectors'].T[state] for state in core_states])
                valence_eigvecs = np.array([rsp_tensors['eigenvectors'].T[state] for state in val_states])
        except KeyError:
            core_eigvecs    = np.array([cvs_rsp_tensors['S' + str(i + 1)] for i in core_states])
            valence_eigvecs = np.array([rsp_tensors['S' + str(i + 1)] for i in val_states])
        """
        ...
        for w_ind, omega in enumerate(self.photon_energy):
            ...
            gs_to_val_tdens = np.zeros((norb_AO, norb_AO, num_final_states))
            for f in range(num_final_states):
                ...
                for n in range(num_intermediate_states):
                    ...
                    if self.tda:
                        core_z_mat = core_eigvec.reshape(
                ...
                if gen_abs:
                    gs_to_val_tdens[..., f] = np.sqrt(2) * np.linalg.multi_dot([mo_occ, valence_z_mat - valence_y_mat, mo_vir.T])
                    val_tdm = np.einsum('xij,ijf -> xf', dipole_integrals, gs_to_val_tdens)
                    core_tdm = np.einsum('xij,ijn -> xn', dipole_integrals, gs_to_core_tdens)
                    val_osc_strength = (2/3) * np.einsum('f,xf -> f', valence_eigvals, val_tdm**2)
                    core_osc_strength = (2/3) * np.einsum('n,xn -> n', core_eigvals, core_tdm**2)
                ...
                F = self.scattering_amplitude_tensor(omega, core_eigvals, valence_eigvals[f], gs_to_core_tdens,
        ...
        if gen_abs:
            self.absorption_properties = {
                'gs_to_core_tdens': gs_to_core_tdens,
                'gs_to_val_tdens': gs_to_val_tdens,
                'core_abs_ene': core_eigvals,
                'valence_abs_ene': valence_eigvals,
                'core_osc_strength': core_osc_strength,
                'valence_osc_strength': val_osc_strength,
            }
