...
def explain_grouping_result_for_non_experts(
    grouping_result: dict[str, Any],
    *,
    max_index_preview: int = 16,
    max_label_preview: int = 40,
) -> str:
    """
    Build a step-by-step explanation of grouping output for non-experts.

    This is designed for exactly the kind of dictionary returned by
    ``improved_group_by_connected_components(...)``:
    ...
    It explains:
    - how each field is constructed,
    - how to interpret labels and component membership,
    - how each count in ``group_stats`` is derived from basin sizes.
    """

    groups = grouping_result.get("groups", [])
    diagnostics = grouping_result.get("diagnostics", {})
    labels = grouping_result.get("labels", None)

    n_dp = int(diagnostics.get("n_datapoints", 0))
    n_ref = int(diagnostics.get("n_references", 0))
    n_tot = int(diagnostics.get("n_total_nodes", n_dp + n_ref))
    cutoff = float(diagnostics.get("cutoff", math.nan))
    cutoff_info = diagnostics.get("cutoff_info", {})
    used_fallback = bool(diagnostics.get("used_fallback", False))
    adjacency_density = float(diagnostics.get("adjacency_density", math.nan))

    lines: list[str] = []
    lines.append("Grouping output walkthrough (non-expert view)")
    lines.append("")
    lines.append("1) Size bookkeeping")
    lines.append(f"- n_datapoints = {n_dp}")
    lines.append(f"- n_references = {n_ref}")
    lines.append(f"- n_total_nodes = {n_tot}")
    lines.append(f"- Check: n_datapoints + n_references = {n_dp + n_ref}")
    if (n_dp + n_ref) != n_tot:
        lines.append("  Note: reported total differs from sum of parts (unexpected).")
    else:
        lines.append("  This is consistent.")

    lines.append("")
    lines.append("2) Adaptive cutoff and edge construction")
    mode = str(cutoff_info.get("mode", "unknown"))
    lines.append(f"- cutoff mode = {mode}")
    lines.append(f"- final cutoff = {cutoff:.6g}")
    if "quantile_raw" in cutoff_info:
        q_raw = float(cutoff_info["quantile_raw"])
        lines.append(f"- raw quantile cutoff before clipping = {q_raw:.6g}")
        if q_raw != cutoff:
            if q_raw < cutoff:
                lines.append("  final cutoff is larger than raw quantile (likely min-bound clipping).")
            else:
                lines.append("  final cutoff is smaller than raw quantile (likely max-bound clipping).")
    if "knn_k" in cutoff_info:
        lines.append(f"- kNN rank used = {int(cutoff_info['knn_k'])}")
    if "adaptive_quantile" in cutoff_info:
        lines.append(f"- adaptive quantile = {float(cutoff_info['adaptive_quantile']):.3f}")
    if "kth_values_size" in cutoff_info:
        lines.append(f"- number of node-wise k-th distances used = {int(cutoff_info['kth_values_size'])}")
    if "kth_values_median" in cutoff_info:
        lines.append(f"- median of node-wise k-th distances = {float(cutoff_info['kth_values_median']):.6g}")
    lines.append(f"- used fallback threshold grouping = {used_fallback}")
    if np.isfinite(adjacency_density):
        denom = max(1, n_tot * (n_tot - 1))
        approx_sum = int(round(adjacency_density * denom))
        lines.append(
            f"- adjacency_density = {adjacency_density:.6f} "
            f"(~ {approx_sum} directed edges out of {denom} possible off-diagonal entries)"
        )

    lines.append("")
    lines.append("3) Labels: what they represent")
    lines.append("- labels array stores connected-component id for each global node index.")
    lines.append("- Convention in this module:")
    lines.append("  datapoint nodes are global indices [0 .. n_datapoints-1].")
    lines.append("  reference nodes are global indices [n_datapoints .. n_total_nodes-1].")

    if labels is not None:
        labels_arr = np.asarray(labels, dtype=np.int64).reshape(-1)
        lines.append(f"- labels length = {labels_arr.size}")
        if labels_arr.size > 0:
            preview = labels_arr[: max(1, int(max_label_preview))]
            lines.append(f"- labels preview (first {len(preview)}): {preview.tolist()}")
        if labels_arr.size == n_tot:
            unique_ids = sorted(np.unique(labels_arr).tolist())
            lines.append(f"- unique component ids = {unique_ids}")

            component_summary: list[str] = []
            for cid in unique_ids:
                dp_members = [i for i in range(n_dp) if labels_arr[i] == cid]
                ref_members = [j for j in range(n_ref) if labels_arr[n_dp + j] == cid]
                component_summary.append(
                    f"  component {cid}: dp={len(dp_members)}, ref={len(ref_members)}"
                )
            lines.append("- component membership summary:")
            lines.extend(component_summary)
        else:
            lines.append("  Warning: labels length does not match n_total_nodes.")

    lines.append("")
    lines.append("4) Why `groups` contains only certain datapoints")
    lines.append(
        "- A group is emitted only for components containing BOTH datapoints and references."
    )
    lines.append(f"- number of emitted groups = {len(groups)}")

    group_stats_by_id = {
        int(item.get("basin_id", -1)): item for item in diagnostics.get("group_stats", [])
    }

    for g in groups:
        basin_id = int(g.get("basin_id", -1))
        dp_idx = [int(x) for x in g.get("datapoint_indices", [])]
        ref_idx = [int(x) for x in g.get("reference_indices", [])]
        n_dp_g = len(dp_idx)
        n_ref_g = len(ref_idx)

        lines.append("")
        lines.append(f"- Basin {basin_id}:")
        lines.append(f"  datapoint_indices = {_preview_int_list(dp_idx, max_items=max_index_preview)}")
        lines.append(f"  reference_indices = {_preview_int_list(ref_idx, max_items=max_index_preview)}")
        lines.append(f"  basin sizes: n_dp={n_dp_g}, n_ref={n_ref_g}")

        max_dp_dp_directed = n_dp_g * max(0, n_dp_g - 1)
        max_ref_ref_directed = n_ref_g * max(0, n_ref_g - 1)
        max_dp_ref = n_dp_g * n_ref_g
        lines.append(
            "  expected maximum finite counts if all pairs are available:"
            f" dp_dp={max_dp_dp_directed}, ref_ref={max_ref_ref_directed}, dp_ref={max_dp_ref}"
        )

        stat = group_stats_by_id.get(basin_id, {})
        if stat:
            dp_dp = stat.get("dp_dp", {})
            ref_ref = stat.get("ref_ref", {})
            dp_ref = stat.get("dp_ref", {})

            lines.append(
                "  observed counts:"
                f" dp_dp.count={dp_dp.get('count')},"
                f" ref_ref.count={ref_ref.get('count')},"
                f" dp_ref.count={dp_ref.get('count')}"
            )
            lines.append(
                "  distance ranges:"
                f" dp_dp[min,med,max]=[{dp_dp.get('min')}, {dp_dp.get('median')}, {dp_dp.get('max')}],"
                f" ref_ref[min,med,max]=[{ref_ref.get('min')}, {ref_ref.get('median')}, {ref_ref.get('max')}],"
                f" dp_ref[min,med,max]=[{dp_ref.get('min')}, {dp_ref.get('median')}, {dp_ref.get('max')}]"
            )

            if dp_dp.get("count", 0) == max_dp_dp_directed:
                lines.append("  dp_dp count reaches its combinatorial maximum: all dp-dp off-diagonal distances are finite.")
            if ref_ref.get("count", 0) == max_ref_ref_directed:
                lines.append("  ref_ref count reaches its combinatorial maximum: all ref-ref off-diagonal distances are finite.")
            if dp_ref.get("count", 0) == max_dp_ref:
                lines.append("  dp_ref count reaches its combinatorial maximum: every dp has a finite distance to every ref in this basin.")

    lines.append("")
    lines.append("5) Practical interpretation")
    lines.append(
        "- If only a few datapoints appear in groups while many exist globally, most datapoints are disconnected from reference-supported components under current cutoff/graph rules."
    )
    lines.append(
        "- In that case, trust-radius optimization updates only the grouped datapoints; others should keep previous radii."
    )
    lines.append(
        "- This is usually desired behavior when isolated datapoints do not have local reference support."
    )
