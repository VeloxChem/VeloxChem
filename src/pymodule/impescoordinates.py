...
def calc_elements(i, z_mat_element, internal_coordinate, coords, natoms, org_b_matrix):
    
    
    second_derivative = internal_coordinate.second_derivative(coords).reshape(-1, natoms * 3)
    b2_matrix_section = np.zeros((natoms * 3, natoms * 3))
    
    r_inv_2, r_inv_3 = 0, 0

    if len(z_mat_element) == 2:
        r = internal_coordinate.value(coords)
        r_inv_2 = 1.0 / (r * r)
        r_inv_3 = r_inv_2 / r

        b2_matrix_section =  -r_inv_2 * second_derivative
        for m in range(natoms):
            for n in range(m, natoms):
                b2_matrix_section[m*3:(m+1)*3, n*3:(n+1)*3] += 2 * r_inv_3 * np.outer(org_b_matrix[m*3:(m+1)*3], org_b_matrix[n*3:(n+1)*3])

        b2_matrix_section = 0.5 * (b2_matrix_section + b2_matrix_section.T)

    return i, second_derivative, b2_matrix_section
...
class ImpesCoordinates:
    ...
    def __init__(self, z_matrix, atom_labels=None, comm=None, ostream=None):
        ...
        # geomeTRIC internal coordinates. It is used in ImpesDriver to
        # determine the distance in internal coordinates between two
        # ImpesCoordinates objects.
    ...
    def print_keywords(self):
        """
        Prints the input keywords of the ImpesCoordinates.
        """
    ...
    def print_attributes(self):
        """
        Prints the attributes of the ImpesCoordinates.
        """
    ...
    def update_settings(self, impes_dict=None):
        """
        Updates settings for ImpesCoordinates.
    ...
    def define_internal_coordinates(self):
        ...
        assert_msg_critical(self.z_matrix is not None, 'ImpesCoordinates: No Z-matrix defined.')
        ...
        for z in self.z_matrix:
            if len(z) == 2:
                ...
                assert_msg_critical(False, 'ImpesCoordinates: Invalid entry size in Z-matrix.')
    ...
    def calculate_b_matrix(self):
        ...
        """
        assert_msg_critical(self.internal_coordinates is not None, 'ImpesCoordinates: No internal coordinates are defined.')
        assert_msg_critical(self.cartesian_coordinates is not None, 'ImpesCoordinates: No cartesian coordinates are defined.')
    ...
    def calculate_b2_matrix(self):
        ...
        b2_matrix_parallel = np.zeros((len(self.z_matrix), n_atoms * 3, n_atoms * 3))
        self.b2_matrix = np.zeros((len(self.z_matrix), n_atoms * 3, n_atoms * 3))
        # b2_matrix_half = np.zeros((len(self.z_matrix), n_atoms * 3, n_atoms * 3))
        # # print(len(self.z_matrix))
        # num_processes = mp.cpu_count()
        # pool = mp.Pool(processes=20)
        # args = [(i, element, self.internal_coordinates[i], coords, n_atoms, self.original_b_matrix[i]) for i, element in enumerate(self.z_matrix)]

        # with mp.Pool(processes=num_processes) as pool:
        # # Use starmap to map the function to the argument list
        #     results = pool.starmap(calc_elements, args)

        # # Gather the results and fill the matrix
        
        # for result in results:
            
        #     i, second_derivative, b2_matrix_section = result  # Get the result for each element

        #     if len(self.z_matrix[i]) == 2:

        #         b2_matrix_parallel[i] = b2_matrix_section
        #         #for m in range(n_atoms):
        #         #    for n in range(n_atoms):
        #         #        
        #         #        b2_matrix_parallel[i, m*3:(m+1)*3, n*3:(n+1)*3] += 2 * r_3 * np.outer(self.original_b_matrix[i, m*3:(m+1)*3], self.original_b_matrix[i, n*3:(n+1)*3])
                        
        #     else:
        #         b2_matrix_parallel[i] = second_derivative

        # self.b2_matrix = b2_matrix_parallel
        

        for i, z in enumerate(self.z_matrix):
            q = self.internal_coordinates[i]
            second_derivative = q.second_derivative(coords).reshape(-1, n_atoms * 3)
            #print('internal_coordinate', i, second_derivative)
            ...
            if len(z) == 2 and self.use_inverse_bond_length:
                ...
                # b2_matrix_half[i] = -r_inv_2 * second_derivative
                for m in range(n_atoms):
                ...
                # for m in range(n_atoms):
                #     for n in range(m, n_atoms):        
                #         b2_matrix_half[i, m*3:(m+1)*3, n*3:(n+1)*3] += 2 * r_inv_3 * np.outer(self.original_b_matrix[i, m*3:(m+1)*3], self.original_b_matrix[i, n*3:(n+1)*3])
                self.b2_matrix[i] = 0.5 * (self.b2_matrix[i] + self.b2_matrix[i].T)
                # b2_matrix_half[i] = 0.5 * (b2_matrix_half[i] + b2_matrix_half[i].T)
            else:
                self.b2_matrix[i] = second_derivative
                # b2_matrix_half[i] = second_derivative
        # print(self.b2_matrix, '############ org b matrix ##############', self.b2_matrix[0], '############ b2_matrix half ###############', b2_matrix_half[0], '############# new b2 parallel ##################', b2_matrix_parallel[0])
    def transform_gradient_to_internal_coordinates(self, tol=1e-8, alpha=0.0):
        """
        ...
        assert_msg_critical(self.gradient is not None, 'ImpesCoordinates: No gradient is defined.')
        ...
        print(
            number_of_positive_values == dimension,
            'ImpesCoordinates: The number of positive singular values is not equal to the dimension of the Hessian., restoring the last biggest elements')
    ...
    def transform_hessian_to_internal_coordinates(self, tol=1e-8, alpha=0.0):
        """
        ...
        if self.internal_gradient is None:
            self.transform_gradient_to_internal_coordinates(alpha=alpha)
    ...
    def transform_gradient_and_hessian(self, alpha=0.0):
        """
        ...
        self.transform_gradient_to_internal_coordinates(alpha=alpha)
        self.transform_hessian_to_internal_coordinates(alpha=alpha)
        
        # Check for the hessian
        eigv, _ = np.linalg.eigh(self.internal_hessian)
        if np.min(eigv) < -1.0:
            msg = 'ImpesCoordinates: Large negative eigenvalues in the internal Hessian.'
            print(msg)
            print(np.min(eigv))
            # Applies a Tikhonov regularization to the internal Hessian
            alpha = 1e-5
            self.transform_gradient_to_internal_coordinates(alpha=alpha)
            self.transform_hessian_to_internal_coordinates(alpha=alpha)
    ...
    def reset_coordinates_impes_driver(self, cartesian_coordinates):
        ...
        # vice-versa.
        self.calculate_b_matrix()
        # !!!! The b2 matrix is not necessary in the interpolation 
        #self.calculate_b2_matrix()    
    ...
    def reset_coordinates(self, cartesian_coordinates):
        ...
        profiler.set_timing_key(f'Iteration in reset coordinate (define internal coordinate)')
        ...
        self.b2_matrix = None
        self.g_minus = None
        profiler.start_timer('define_internal_coordinate')
        if self.internal_coordinates is None:
            self.define_internal_coordinates()
        profiler.stop_timer('define_internal_coordinate')
        # The B matrix is required for the transformation of the
        # gradient, while B2 is required for the transformation of
        # the Hessian from Cartesian to internal coordinates and
        # vice-versa.
        profiler.start_timer('calculate_define_b_matrix')
        self.calculate_b_matrix()
        profiler.stop_timer('calculate_define_b_matrix')
        profiler.start_timer('calculate_define_b_2_matrix')
        self.calculate_b2_matrix()
        profiler.stop_timer('calculate_define_b_2_matrix')
    ...
    def compute_internal_coordinates_values(self):
        ...
        assert_msg_critical(
            ...
            'ImpesCoordinates: No internal coordinates are defined.')

        assert_msg_critical(
            self.cartesian_coordinates is not None,
            'ImpesCoordinates: No cartesian coordinates are defined.')
    ...
    def get_z_matrix_as_np_arrays(self):
        ...
        assert_msg_critical(self.z_matrix is not None,
                            'ImpesCoordinates: No Z-matrix is defined.')
        ...
        for z in self.z_matrix:
            if len(z) == 2:
                ...
                assert_msg_critical(
                    False, 'ImpesCoordinates: Invalid entry size in Z-matrix.')
    ...
    def identify_exponents(self):

        internal_hessian = self.internal_hessian
        print(np.linalg.norm(internal_hessian))

        forbius_norm = np.linalg.norm(internal_hessian)

        eigenvalues, eigenvectors = np.linalg.eigh(internal_hessian)

        # determine the exponents based on the hessian eigenvalues and norm
    ...
    def write_hdf5(self, fname, label, write_zmat=False):
        ...
        print('in impescoordinate', fname, valid_checkpoint)
        if valid_checkpoint:
            try:
                print('Iam going in the correct File !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
            ...
            assert_msg_critical(self.energy is not None,
                                'ImpesCoordinates: No energy is defined.')

            assert_msg_critical(
                self.internal_gradient is not None,
                'ImpesCoordinates: No internal gradient is defined.')

            assert_msg_critical(
                self.internal_hessian is not None,
                'ImpesCoordinates: No internal Hessian is defined.')
    ...
    def read_hdf5(self, fname, label, NAC=False):
        """
