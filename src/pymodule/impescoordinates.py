...
class ImpesCoordinates:
    ...
    def __init__(self, z_matrix, atom_labels=None, comm=None, ostream=None):
        ...
        self.normal_modes_in_cartesian = None
        self.normal_mode_displacement_coefficients = None
        self.normal_mode_frequencies = None
    ...
    def transform_gradient_to_internal_coordinates(self, tol=1e-8):
        ...
        if number_of_positive_values > dimension:
            print('ImpesCoordinates: The number of positive singular values is not equal to the dimension of the Hessian., restoring the last biggest elements')
    ...
    def reset_coordinates_impes_driver(self, cartesian_coordinates):
        """
        Resets the Cartesian coordinates for the impesdriver.
    ...
    def write_hdf5(self, fname, label):
        ...
        if valid_checkpoint:
            ...
            assert_msg_critical(
                self.normal_mode_frequencies is not None,
                'ImpesCoordinates: No normal mode frequencies are given.')
            # Write the bonds, angles and dihedrals defined in the Z-matrix.
            # Bonds are saved in an array with two atoms indices per row.
            # Angles are saved in an array with three atom indices per row.
            # Dihedrals are saved in an array with four atom indices per row.
            full_label = label + '_normal_mode_frequencies'
            h5f.create_dataset(full_label,
                               data=self.normal_mode_frequencies,
                               compression='gzip')
            
            assert_msg_critical(
                self.normal_modes_in_cartesian is not None,
                'ImpesCoordinates: no normal modes in cartesian coordinates are given.')
            # Write the bonds, angles and dihedrals defined in the Z-matrix.
            # Bonds are saved in an array with two atoms indices per row.
            # Angles are saved in an array with three atom indices per row.
            # Dihedrals are saved in an array with four atom indices per row.
            full_label = label + '_normal_modes_cartesian'
            print(self.normal_modes_in_cartesian)
            h5f.create_dataset(full_label,
                                data=self.normal_modes_in_cartesian,
                                compression='gzip')

            assert_msg_critical(
                self.normal_mode_displacement_coefficients is not None,
                'ImpesCoordinates: No normal mode displacement coefficient are given.')
            # Write the bonds, angles and dihedrals defined in the Z-matrix.
            # Bonds are saved in an array with two atoms indices per row.
            # Angles are saved in an array with three atom indices per row.
            # Dihedrals are saved in an array with four atom indices per row.
            full_label = label + '_normal_modes_displacement_coefficient'
            h5f.create_dataset(full_label,
                               data=self.normal_mode_displacement_coefficients,
                               compression='gzip')
    ...
    def read_hdf5(self, fname, label):
        ...
        if valid_checkpoint:
            ...
            cart_normal_modes = label + '_normal_modes_cartesian'
            normal_modes_displacement_coordinates_label = label + '_normal_modes_displacement_coefficient'
            normal_mode_frequencies_label = label + '_normal_mode_frequencies'
            ...
            self.normal_mode_frequencies = np.array(h5f.get(normal_mode_frequencies_label))
            self.normal_modes_in_cartesian = list(h5f.get(cart_normal_modes))
            self.normal_mode_displacement_coefficients = list(h5f.get(normal_modes_displacement_coordinates_label))

            z_matrix = []
            print('Here is the normal mode frequencies', self.normal_mode_frequencies)
