...
class EmbeddingIntegralDriver:
    ...
    def electronic_fields(self,
        ...
        """
        return -1.0 * compute_electric_field_values(self.molecule, self.basis, coordinates, density_matrix)
    ...
    def induced_dipoles_potential_integrals(self,
        ...
        Returns:
            Product of the electronic potential integrals and the induced dipoles.
                ...
                Dtype: np.float64
        """
        return compute_electric_field_integrals(self.molecule, self.basis, coordinates, induced_dipoles)
...
class PolarizableEmbedding:
    ...
    Instance variables:
        ...
        - _integral_driver: Instance of EmbeddingIntegralDriver to calculate all relevant integrals.
        - _e_ind: Induction energy.
    ...
    def __init__(self, molecule, ao_basis, options, comm=None, log_level=20):
        ...
        if self.options['settings']['embedding_method'] != 'PE':
            raise NotImplementedError("Method set is not Polarizable Embedding.")

        self.comm = comm
        self.molecule = molecule
        self.basis = ao_basis

        self._integral_driver = EmbeddingIntegralDriver(molecule=self.molecule, ao_basis=self.basis)
        self._create_pyframe_objects()
        self._e_ind = None

        self._threshold = self.options['settings'].get('induced_dipoles', {}).get('threshold', 1e-8)
        self._max_iterations = self.options['settings'].get('induced_dipoles', {}).get('max_iterations', 100)
        self._solver = self.options['settings'].get('induced_dipoles', {}).get('solver', 'jacobi')
...
class PolarizableEmbeddingSCF(PolarizableEmbedding):
    ...
    Instance variables:
        ...
        - _f_el_es: Electrostatic Fock matrix.
        - _e_nuc_es: Electrostatic nuclear energy.
        - _e_rep: VDW repulsion energy.
        - _e_disp: VDW dispersion energy.
        - _environment_energy: Interaction energy between multipoles in the classical_subsystem.
    """
    ...
    def __init__(self, molecule, ao_basis, options, comm=None, log_level=20):
        ...
        self.e = None
        self.v = None
        self.vdw_method = self.options['settings'].get('vdw', {}).get('method', 'LJ')
        self.vdw_combination_rule = self.options['settings'].get('vdw', {}).get('combination_rule', 'Lorentz-Berthelot')
        ...
        self._environment_energy = self.classical_subsystem.environment_energy if self.options['settings'].get(
    ...
    def compute_pe_contributions(self, density_matrix):
        ...
        e_el_es = np.einsum('ij,ij', self._f_el_es, density_matrix)
        ...
        self._e_es = self._e_nuc_es + e_el_es
        self.e = self._e_ind + self._e_es + self._e_disp + self._e_rep
        self.v = self._f_el_es - f_el_ind
        self.log_manager.logger.info('Polarizable Embedding (PE) Energy Contributions'.ljust(92))
        self.log_manager.logger.info('------------------------------------'.ljust(92))
        self.log_manager.logger.info(f'Electrostatic Contributions (E_es) :{self._e_es:20.10f} a.u.')
        self.log_manager.logger.info(f'Induced Contributions (E_ind)      :{self._e_ind:20.10f} a.u.')
        if self._environment_energy != 0.0:
            self.log_manager.logger.info(f'Environment Contributions (E_mul)   :{self._environment_energy:20.10f} a.u.')
        if self._e_disp != 0.0:
            self.log_manager.logger.info(f'Dispersion Contributions (E_disp)   :{self._e_disp:20.10f} a.u.')
        if self._e_rep != 0.0:
            self.log_manager.logger.info(f'Repulsion Contributions (E_disp)   :{self._e_rep:20.10f} a.u.')
        self.log_manager.logger.info('------------------------------------'.ljust(92))
