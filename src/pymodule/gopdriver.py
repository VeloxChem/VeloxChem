...
class GlobalOptimizationDriver:
    ...
    def compute(self, filename, seed):
        ...
        for i, confgeom in enumerate(confgeoms):
            ...
            cur_str = 'Full Geometry Optimization: Conformer Candidate {:d}'.format(
                i)
            ...
            xtb_drv.compute(mol, self.ostream)
            ene = xtb_drv.get_energy()
    ...
    def select_ref_atom(self, index, molecule):
        ...
        if len(ref_indexes) > 0:
            return random.choice(ref_indexes)
        else:
            return None
