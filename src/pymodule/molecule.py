...
def _Molecule_from_dict(mol_dict):
    ...
    assert_msg_critical(mol.check_proximity(0.1),
                        'Molecule: Atoms are too close.')
...
def _Molecule_write_xyz(self, xyz_filename):
    ...
    with open(str(xyz_filename), 'w') as fh:
        ...
        for name, r in zip(self.get_labels(),
                           self.get_coordinates("angstrom")):
...
def _Molecule_get_aufbau_occupation(self, norb, flag='restricted'):
    ...
    :return:
        flag=='restricted': single vector assuming doubly occupied
        orbitals.
        flag=='unrestricted': two vectors assuming singly occupied
        spin-orbitals.
    """
