...
"""
    ...
    def extract_hessians(self):
        ...
        try:
            if self.program == 'ORCA':
                ...
                if self.roots > 0:
                    hessians_orca.update({f'excited_state_{i+1}': None for i in range(self.roots)})
                for root in range(self.roots + 1):
                    ...
                    with open(self.hessian_files[root], 'r', encoding='utf-8') as file:
                        ...
                        elif matrix is not None and root == 1:
                            hessians_orca['excited_state'] = matrix
                            hessians.append(hessians_orca['excited_state'])
                            print('Hessian matrix extracted successfully.')
                        else:
    ...
    def compute(self, current_molecule, paralell=False, molecule_geometry=None):
        ...
        if not self.do_four_point:
            for i in range(natm):
                for d in range(3):
                    ...
                    gradient, _, _ = gradient_driver.compute(new_mol, current_string)
                    grad_plus = gradient
        ...
        if not self.do_four_point:
            for i in range(natm):
                for d in range(3):
                    ...
                    # create a new XTB driver object;
                    # without this the energy is always zero...;
                    _, _, job_id_plus = gradient_driver.compute(new_mol, current_string)
                    ...
                    _, _, job_id_minus = gradient_driver.compute(new_mol, current_string)
    ...
    def create_input_file_hessian(self, input_file, root):
        ...
        try:
            if self.program == 'MOLCAS':
                ...
                if self.roots == 0:
                ...
                elif root == 0 and self.roots > 0:
                    full_path = os.path.abspath(self.xyz_filename)
                    with open(input_file, 'w') as file:
                        file.write('!wB97x-D3 freq cc-PVTZ def2/J RIJCOSX\n')
                        file.write(f'%maxcore 3000\n')
                        file.write(f'%PAL\n')
                        file.write(f'nprocs {self.qm_driver.nprocs * 2}\n')
                        file.write('END\n')
                    ...
                    full_path = os.path.abspath(self.xyz_filename)
                    with open(input_file, 'w') as file:
                        file.write('!wB97X-D3 freq cc-PVTZ TIGHTSCF\n\n')
