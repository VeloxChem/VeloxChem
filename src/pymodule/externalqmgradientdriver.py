...
class ExternalQMGradientDriver:
    ...
    def __init__(self, qm_driver, comm=None, ostream=None):
        ...
        if self.roots > 0:
            for root in range(self.roots):
                self.input_filename_excited = f'current_input_gradient_excited_{root}.inp'
                self.output_filename_excited = f'current_output_gradient_excited_{root}.out'
    ...
    def prepare_job_script(self, template_path, input_file, output_file):
        ...
        script_content = script_content.replace('NPROCS', str(self.qm_driver.nprocs * 2))
...
"""
    ...
    def extract_gradients(self):
        ...
        try:
            ...
            if self.program == 'ORCA':
                ...
                if self.roots > 0:
                    gradients_orca.update({f'excited_state_{i+1}': None for i in range(self.roots)})
                ...
                if self.roots > 0:
                    ...
                    with open('current_input_gradient_excited_0.engrad.ground.grad.txt', 'r', encoding='utf-8') as file:
                        lines = file.readlines()
                        current_gradients = []
                        
                        if len(lines) <= 1:
                            gradients_orca['ground_state'] = np.zeros((self.natoms, 3))

                        else:
                            for line in lines[1:]:  # Skip the first line which seems to be a header
                                parts = line.split()
                                
                                if len(parts) == 4 or len(parts) == 6:  # Check if the line has 4 or 6 elements
                                    # Extract the last three elements (x, y, z)
                                    x, y, z = map(float, parts[-3:])
                                    current_gradients.append([x, y, z])
                            
                            # Convert the list of gradients to a numpy array
                            gradients_orca['ground_state'] = np.array(current_gradients)
    
                    with open(self.output_files[1], 'r', encoding='utf-8') as file:
                        lines = iter(file.readlines())
                        in_ground_state_section = False
                            ...
                            if in_excited_state_section and re.search(r'CARTESIAN GRADIENT', line):
                                ...
                                for gradient_line in lines:
                                    ...
                                    if len(parts) == 6:
                                        ...
                                        current_gradients.append([float(x), float(y), float(z)])
                                gradients_orca[f'excited_state_{root}'] = np.array(current_gradients)
                                print('converting is allowed')
                                print(f'Gradients for root {root} extracted')
                                continue

                gradients.append(gradients_orca['ground_state'])
                if self.roots > 0:
                    for root in range(self.roots):
                        gradients.append(gradients_orca[f'excited_state_{root + 1}'])
            ...
            if self.program == 'QCHEM':
                ...
                for root in range(self.roots + 1):
                    ...
                    if root == 0:
                        ...
                        gradients.append(gradients_qchem[f'excited_state_{root + 1}'])
    ...
    def compute(self, current_molecule, molecule_geometry=None, paralell=False):
        ...
        if self.program in ['MOLCAS', 'ORCA'] and self.cluster_manager is None:
            if self.program == 'MOLCAS':
                ...
                if self.roots == 0:
                    self.create_input_file_gradient(self.input_files[0], self.qm_driver.tracked_roots[0])
                    ...
                    self.create_input_file_gradient(self.input_files[1], self.qm_driver.tracked_roots[1] + 1)
                    script_path = os.path.join(os.getcwd(), "run_orca.sh")
                    self.create_shell_script(script_path)
            ...
            if self.program == 'ORCA':
                ...
                if self.roots == 0:
                    ...
                    self.create_input_file_gradient(self.input_files[1], self.qm_driver.tracked_roots[1])
                    template_path = 'jobscript_orca'
        ...
        try:
            if self.program == 'MOLCAS':
                ...
                if self.roots == 0:
                    ...
                    root_str = ', '.join(map(str, self.qm_driver.tracked_roots))
                    full_path = os.path.abspath(self.xyz_filename)
                    with open(input_file, 'w') as file:
                        file.write('!wB97X-D3 ENGRAD cc-PVTZ TIGHTSCF\n\n')

                        file.write(f'%TDDFT NROOTS {self.roots}\n')
