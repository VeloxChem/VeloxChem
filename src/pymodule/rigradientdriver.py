...
def _RIFockGradDriver_mpi_comppute(self, comm, molecule, basis, aux_basis, density):
    """
    ...
    if comm.Get_rank() == 0:
        ...
        gv = ri_fock_drv.compute_bq_vector(density)
    gv = comm.bcast(gv, 0)
    
    # compute local gradient on MPI rank
    atoms = partition_atoms(natoms, rank, nodes)
    ri_grad_drv = RIFockGradDriver()
    loc_grad = ri_grad_drv.compute(basis, aux_basis, molecule, gv, density, atoms)
    for i in range(len(atoms)):
        gxyz = loc_grad[i].coordinates()
        grad_mat[atoms[i], 0] = gxyz[0]
        grad_mat[atoms[i], 1] = gxyz[1]
        grad_mat[atoms[i], 2] = gxyz[2]
    comm.Reduce([grad_mat, MPI.DOUBLE], [tot_grad, MPI.DOUBLE], op=MPI.SUM, root=0)

    return tot_grad

   
RIFockGradDriver.mpi_compute = _RIFockGradDriver_mpi_comppute
