                                           ...
                                           const CXCNewFunctional& xcFunctional) const
{
    ...
    for (int32_t box_id = 0; box_id < counts.size(); box_id++)
    {
        ...
        gridscreen::screenVxcFockForMGGA(rho, sigma, lapl, tau,
                                         vrho, vsigma, vlapl, vtau,
                                         npoints, _screeningThresholdForDensityValues);

        gridscreen::screenFxcFockForMGGA(rho, sigma, lapl, tau,
                                         v2rho2, v2rhosigma, v2rholapl, v2rhotau,
                                         v2sigma2, v2sigmalapl, v2sigmatau, v2lapl2,
                                         v2lapltau, v2tau2,
                                         npoints, _screeningThresholdForDensityValues);
                                           ...
                                           const std::string&      quadMode) const
{
    ...
    for (int32_t box_id = 0; box_id < counts.size(); box_id++)
    {
        ...
        gridscreen::screenKxcFockForMGGA(rho, sigma, lapl, tau,
                                         v3rho3, v3rho2sigma, v3rho2lapl, v3rho2tau,
                                         v3rhosigma2, v3rhosigmalapl, v3rhosigmatau, v3rholapl2,
                                         v3rholapltau, v3rhotau2, v3sigma3, v3sigma2lapl,
                                         v3sigma2tau, v3sigmalapl2, v3sigmalapltau, v3sigmatau2,
        ...
        for (int32_t idensity = 0; idensity < numdens_rw2; idensity++)
        {
            ...
            auto partial_mat_Kxc = _integratePartialKxcFockForMGGA(npoints,local_weights,mat_chi,
                                                                   mat_chi_x,mat_chi_y,mat_chi_z,
                                                                   rhograd, vsigma, v2rho2, v2rhosigma,
                                          ...
                                          const std::string&      quadMode) const
{
    ...
    for (int32_t box_id = 0; box_id < counts.size(); box_id++)
    {
        ...
        for (int32_t idensity = 0; idensity < numdens_rw3; idensity++)
        {
            ...
            auto partial_mat_Lxc = _integratePartialLxcFockForLDA(npoints, local_weights, mat_chi,
    ...
    for (int32_t box_id = 0; box_id < counts.size(); box_id++)
    {
        ...
        for (int32_t idensity = 0; idensity < numdens_rw3; idensity++)
        {
            ...
            auto partial_mat_Kxc = _integratePartialLxcFockForGGA(npoints, local_weights, mat_chi,
                                                                  ...
                                                                  rhograd, vsigma, v2rho2, v2rhosigma,
                                                                  v2sigma2, v3rho3, v3rho2sigma,
                                                                  v3rhosigma2, v3sigma3, v4rho4,
                                                                  v4rho3sigma, v4rho2sigma2, v4rhosigma3, v4sigma4,
                                          ...
                                          const std::string&      cubeMode) const
{
    ...
    for (int32_t box_id = 0; box_id < counts.size(); box_id++)
    {
        ...
        gridscreen::screenLxcFockForMGGA(rho, sigma, lapl, tau,
                                         v4rho4, v4rho3sigma, v4rho3lapl, v4rho3tau,
                                         v4rho2sigma2, v4rho2sigmalapl, v4rho2sigmatau, v4rho2lapl2,
                                         v4rho2lapltau, v4rho2tau2, v4rhosigma3, v4rhosigma2lapl,
                                         v4rhosigma2tau, v4rhosigmalapl2, v4rhosigmalapltau, v4rhosigmatau2,
        ...
        for (int32_t idensity = 0; idensity < numdens_rw2; idensity++)
        {
            ...
            auto partial_mat_Kxc= _integratePartialKxcFockForMGGA2(npoints,local_weights,mat_chi,
                                                                   mat_chi_x,mat_chi_y,mat_chi_z,
                                                                   rhograd, vsigma, v2rho2, v2rhosigma,
                                                                   v2rholapl, v2rhotau, v2sigma2,
                                                                   v2sigmalapl, v2sigmatau, v2lapl2,
...
#pragma omp simd aligned(weights, \
            ...
            for (int32_t g = grid_batch_offset; g < grid_batch_offset + grid_batch_size; g++)
            {
                ...
                xcomp +=  grada_x_g * ( xr * rhow12a[g] + xt * tauw12a[g] )
                        + x * rxw12a
                        + xx * l5contract_x;

                //ycomp += grada_y_g * ( xr * rhow12a[g] + xt * tauw12a[g] + xl * laplw12a[g])
                ycomp += grada_y_g * ( xr * rhow12a[g] + xt * tauw12a[g] )
                        + x * ryw12a
                        + xx * l5contract_y;

                //zcomp += grada_z_g * ( xr * rhow12a[g] + xt * tauw12a[g] + xl * laplw12a[g])
                zcomp += grada_z_g * ( xr * rhow12a[g] + xt * tauw12a[g] )
                        + x * rzw12a
                        + xx * l5contract_z;
...
#pragma omp simd aligned(weights, \
            ...
            for (int32_t g = grid_batch_offset; g < grid_batch_offset + grid_batch_size; g++)
            {
                ...
                xcomp +=  grada_x_g * ( xr * rhow[g] + xt * tauw[g] )
                        + x * gradw_x[g]
                        + xx * l5contract_x;

                //ycomp += grada_y_g * ( xr * rhow[g] + xt * tauw[g] + xl * laplw[g])
                ycomp += grada_y_g * ( xr * rhow[g] + xt * tauw[g] )
                        + x * gradw_y[g]
                        + xx * l5contract_y;

                //zcomp += grada_z_g * ( xr * rhow[g] + xt * tauw[g] + xl * laplw[g])
                zcomp += grada_z_g * ( xr * rhow[g] + xt * tauw[g] )
                        + x * gradw_z[g]
                        + xx * l5contract_z;
