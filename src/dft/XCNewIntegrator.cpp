                                   ...
                                   const CXCNewFunctional& xcFunctional) const
{
    ...
    for (int32_t box_id = 0; box_id < counts.size(); box_id++)
    {
        ...
        // MGD slice MO coefficients
/*
        if (aocount <= naos)
        {

            CDenseMatrix sub_ActiveMOs(aocount, n_active);
            ...
            for (int32_t i = 0; i < aocount; i++)
            {
                ...
                auto dens_row = mos + aoinds[i] * naos; //MGD check rows/column

                for (int32_t j = 0; j < n_active; j++)
                {
                    sub_mat_row[j] = dens_row[j];
                }
            }
        ...
        submat::distributeSubMatrixToKohnSham(aoFockMatrix, partial_mat_Vxc[0], aoinds, aocount, naos);
