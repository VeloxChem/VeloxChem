...
namespace gtorec {  // gtorec namespace
        ...
        #pragma omp parallel shared(pgaos, pgaox, pgaoy, pgaoz, tbsizes, tbpositions, ntasks, mgx, mgy, mgz)
        {
            #pragma omp single nowait
            {
                for (int32_t i = 0; i < ntasks; i++)
                {
                    ...
                    #pragma omp task firstprivate(tbsize, tbposition)
                    {
                        gtorec::computeGtosValuesForGGA(pgaos, pgaox, pgaoy, pgaoz, gtoContainer, mgx, mgy, mgz, gridOffset, tbposition, tbsize);
                    }
                }
            }
        }
    ...
    void
    computeGtosValuesForMGGA(double*              gtoMatrix,
                             double*              gtoMatrixX,
                             double*              gtoMatrixY,
                             double*              gtoMatrixZ,
                             const CGtoContainer* gtoContainer,
