#ifndef ElectricDipoleMomentumGeom110RecGS_hpp
#define ElectricDipoleMomentumGeom110RecGS_hpp
...
/// @brief Computes (d^(1)/dA^(1)G|r|S)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_electric_dipole_momentum_geom_10_gs(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_gs(pbuffer, 80, 16, 40, factors, 11, a_exp);

                diprec::comp_prim_electric_dipole_momentum_gs(pbuffer, 95, 22, 40, 50, factors, 11, a_exp);

                diprec::comp_prim_electric_dipole_momentum_hs(pbuffer, 140, 50, 80, 95, factors, 11, a_exp);

                t2cgeom::comp_prim_op_geom_10_gx(pbuffer, 203, 50, 140, 3, 1, a_exp);
...
#endif /* ElectricDipoleMomentumGeom110RecGS_hpp */
