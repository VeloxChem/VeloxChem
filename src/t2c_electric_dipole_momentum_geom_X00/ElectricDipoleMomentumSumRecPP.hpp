#ifndef ElectricDipoleMomentumSumRecPP_hpp
#define ElectricDipoleMomentumSumRecPP_hpp
...
/// @brief Computes (d^(1)/dA^(1)P|r|P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_electric_dipole_momentum_geom_10_pp(T&                               distributor,
                                             ...
                                             const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(196, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    diprec::comp_prim_electric_dipole_momentum_ps(pbuffer, 16, 0, 1, factors, 11, a_exp);

                    ovlrec::comp_prim_overlap_pp(pbuffer, 25, 0, 4, factors, 11, a_exp);

                    diprec::comp_prim_electric_dipole_momentum_pp(pbuffer, 34, 1, 4, 7, factors, 11, a_exp);

                    diprec::comp_prim_electric_dipole_momentum_dp(pbuffer, 61, 7, 16, 25, 34, factors, 11, a_exp);

                    t2cgeom::comp_prim_op_geom_10_px(pbuffer, 115, 7, 61, 3, 3, a_exp);
...
#endif /* ElectricDipoleMomentumSumRecPP_hpp */
