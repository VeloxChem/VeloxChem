#ifndef ElectricDipoleMomentumSumRecFF_hpp
#define ElectricDipoleMomentumSumRecFF_hpp
...
/// @brief Computes (d^(1)/dA^(1)F|r|F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_electric_dipole_momentum_geom_10_ff(T&                               distributor,
                                             ...
                                             const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(2685, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    diprec::comp_prim_electric_dipole_momentum_ps(pbuffer, 80, 0, 1, factors, 11, a_exp);

                    ovlrec::comp_prim_overlap_pp(pbuffer, 89, 0, 4, factors, 11, a_exp);

                    diprec::comp_prim_electric_dipole_momentum_pp(pbuffer, 98, 1, 4, 7, factors, 11, a_exp);

                    ovlrec::comp_prim_overlap_pd(pbuffer, 125, 4, 16, factors, 11, a_exp);

                    diprec::comp_prim_electric_dipole_momentum_pd(pbuffer, 143, 7, 16, 22, factors, 11, a_exp);

                    ovlrec::comp_prim_overlap_pf(pbuffer, 197, 16, 40, factors, 11, a_exp);

                    diprec::comp_prim_electric_dipole_momentum_pf(pbuffer, 227, 22, 40, 50, factors, 11, a_exp);

                    diprec::comp_prim_electric_dipole_momentum_dp(pbuffer, 317, 7, 80, 89, 98, factors, 11, a_exp);

                    ovlrec::comp_prim_overlap_dd(pbuffer, 371, 16, 89, 125, factors, 11, a_exp);

                    diprec::comp_prim_electric_dipole_momentum_dd(pbuffer, 407, 22, 98, 125, 143, factors, 11, a_exp);

                    ovlrec::comp_prim_overlap_df(pbuffer, 515, 40, 125, 197, factors, 11, a_exp);

                    diprec::comp_prim_electric_dipole_momentum_df(pbuffer, 575, 50, 143, 197, 227, factors, 11, a_exp);

                    diprec::comp_prim_electric_dipole_momentum_fd(pbuffer, 755, 143, 317, 371, 407, factors, 11, a_exp);

                    ovlrec::comp_prim_overlap_ff(pbuffer, 935, 197, 371, 515, factors, 11, a_exp);

                    diprec::comp_prim_electric_dipole_momentum_ff(pbuffer, 1035, 227, 407, 515, 575, factors, 11, a_exp);

                    diprec::comp_prim_electric_dipole_momentum_gf(pbuffer, 1335, 575, 755, 935, 1035, factors, 11, a_exp);

                    t2cgeom::comp_prim_op_geom_10_fx(pbuffer, 1785, 575, 1335, 3, 10, a_exp);
...
#endif /* ElectricDipoleMomentumSumRecFF_hpp */
