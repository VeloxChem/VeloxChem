#ifndef ElectricDipoleMomentumGeom110RecDD_hpp
#define ElectricDipoleMomentumGeom110RecDD_hpp
...
/// @brief Computes (d^(1)/dA^(1)D|r|D)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_electric_dipole_momentum_geom_10_dd(T&                               distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                diprec::comp_prim_electric_dipole_momentum_ps(pbuffer, 40, 0, 1, factors, 11, a_exp);

                ovlrec::comp_prim_overlap_pp(pbuffer, 49, 0, 4, factors, 11, a_exp);

                diprec::comp_prim_electric_dipole_momentum_pp(pbuffer, 58, 1, 4, 7, factors, 11, a_exp);

                ovlrec::comp_prim_overlap_pd(pbuffer, 85, 4, 16, factors, 11, a_exp);

                diprec::comp_prim_electric_dipole_momentum_pd(pbuffer, 103, 7, 16, 22, factors, 11, a_exp);

                diprec::comp_prim_electric_dipole_momentum_dp(pbuffer, 157, 7, 40, 49, 58, factors, 11, a_exp);

                ovlrec::comp_prim_overlap_dd(pbuffer, 211, 16, 49, 85, factors, 11, a_exp);

                diprec::comp_prim_electric_dipole_momentum_dd(pbuffer, 247, 22, 58, 85, 103, factors, 11, a_exp);

                diprec::comp_prim_electric_dipole_momentum_fd(pbuffer, 355, 103, 157, 211, 247, factors, 11, a_exp);

                t2cgeom::comp_prim_op_geom_10_dx(pbuffer, 535, 103, 355, 3, 6, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 535, ket_width, ket_npgtos);
            }
...
#endif /* ElectricDipoleMomentumGeom110RecDD_hpp */
