#ifndef ElectricDipoleMomentumGeom110RecSP_hpp
#define ElectricDipoleMomentumGeom110RecSP_hpp
...
/// @brief Computes (d^(1)/dA^(1)S|r|P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_electric_dipole_momentum_geom_10_sp(T& distributor,
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                diprec::comp_prim_electric_dipole_momentum_pp(pbuffer, 16, 1, 4, 7, factors, 11, a_exp);

                t2cgeom::comp_prim_op_geom_10_sx(pbuffer, 43, 16, 3, 3, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 43, ket_width, ket_npgtos);
            }
...
#endif /* ElectricDipoleMomentumGeom110RecSP_hpp */
