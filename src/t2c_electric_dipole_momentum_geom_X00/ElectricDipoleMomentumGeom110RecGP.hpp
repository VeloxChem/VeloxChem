#ifndef ElectricDipoleMomentumGeom110RecGP_hpp
#define ElectricDipoleMomentumGeom110RecGP_hpp
...
/// @brief Computes (d^(1)/dA^(1)G|r|P)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_electric_dipole_momentum_geom_10_gp(T& distributor,
                                         ...
                                         const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(1139, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                ovlrec::comp_prim_overlap_fs(pbuffer, 160, 16, 64, factors, 11, a_exp);

                diprec::comp_prim_electric_dipole_momentum_fs(pbuffer, 170, 19, 64, 70, factors, 11, a_exp);

                ovlrec::comp_prim_overlap_fp(pbuffer, 200, 28, 64, 88, factors, 11, a_exp);

                diprec::comp_prim_electric_dipole_momentum_fp(pbuffer, 230, 37, 70, 88, 106, factors, 11, a_exp);

                diprec::comp_prim_electric_dipole_momentum_gs(pbuffer, 320, 70, 160, 170, factors, 11, a_exp);

                ovlrec::comp_prim_overlap_gp(pbuffer, 365, 88, 160, 200, factors, 11, a_exp);

                diprec::comp_prim_electric_dipole_momentum_gp(pbuffer, 410, 106, 170, 200, 230, factors, 11, a_exp);

                diprec::comp_prim_electric_dipole_momentum_hp(pbuffer, 545, 230, 320, 365, 410, factors, 11, a_exp);

                t2cgeom::comp_prim_op_geom_10_gx(pbuffer, 734, 230, 545, 3, 3, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 734, ket_width, ket_npgtos);
            }
...
#endif /* ElectricDipoleMomentumGeom110RecGP_hpp */
