#ifndef ElectricDipoleMomentumGeom110RecPG_hpp
#define ElectricDipoleMomentumGeom110RecPG_hpp
...
/// @brief Computes (d^(1)/dA^(1)P|r|G)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_electric_dipole_momentum_geom_10_pg(T& distributor,
                                         ...
                                         const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(1085, ket_npgtos);
    ...
    for (size_t i = 0; i < ket_blocks; i++)
    {
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                diprec::comp_prim_electric_dipole_momentum_pf(pbuffer, 140, 22, 40, 50, factors, 11, a_exp);

                ovlrec::comp_prim_overlap_pg(pbuffer, 230, 40, 80, factors, 11, a_exp);

                diprec::comp_prim_electric_dipole_momentum_pg(pbuffer, 275, 50, 80, 95, factors, 11, a_exp);

                diprec::comp_prim_electric_dipole_momentum_dg(pbuffer, 410, 95, 140, 230, 275, factors, 11, a_exp);

                t2cgeom::comp_prim_op_geom_10_px(pbuffer, 680, 95, 410, 3, 15, a_exp);

                t2cfunc::reduce(cbuffer, pbuffer, 680, ket_width, ket_npgtos);
            }
...
#endif /* ElectricDipoleMomentumGeom110RecPG_hpp */
