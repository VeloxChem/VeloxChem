#include "PrimitiveOverlapGeom100SF_0_XYY.hpp"
...
auto
compPrimitiveOverlapGeom100SF_0_XYY(TDoubleArray&       buffer_x,
...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += fss * ((1.0 / 2.0) * fe_0 * fe_0 * tbe_0 + fe_0 * rpa_x * rpb_x * tbe_0 + fe_0 * rpb_y * rpb_y * tbe_0 +
                             2.0 * rpa_x * rpb_y * rpb_y * rpb_x * tbe_0);

        fints_y[i] += fss * (fe_0 * rpa_y * rpb_x * tbe_0 + 2.0 * fe_0 * rpb_y * rpb_x * tbe_0 + 2.0 * rpa_y * rpb_y * rpb_y * rpb_x * tbe_0);

        fints_z[i] += fss * (fe_0 * rpa_z * rpb_x * tbe_0 + 2.0 * rpa_z * rpb_y * rpb_y * rpb_x * tbe_0);
    }
}
