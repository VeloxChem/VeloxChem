...
auto
compPrimitiveOverlapGeom100FS_XYZ_0(TDoubleArray&       buffer_x,
...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += fss * (-rpa_z * rpa_y + fe_0 * rpa_z * rpa_y * tbe_0 + 2.0 * rpa_z * rpa_y * rpa_x * rpa_x * tbe_0);

        fints_y[i] += fss * (-rpa_z * rpa_x + fe_0 * rpa_z * rpa_x * tbe_0 + 2.0 * rpa_z * rpa_y * rpa_y * rpa_x * tbe_0);

        fints_z[i] += fss * (-rpa_y * rpa_x + fe_0 * rpa_y * rpa_x * tbe_0 + 2.0 * rpa_z * rpa_z * rpa_y * rpa_x * tbe_0);
    }
}
