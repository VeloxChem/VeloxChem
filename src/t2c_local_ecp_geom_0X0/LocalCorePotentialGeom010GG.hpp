#ifndef LocalCorePotentialGeom010GG_hpp
#define LocalCorePotentialGeom010GG_hpp
...
template <class T>
auto
comp_local_core_potential_geom_010_gg(T& distributor,
                                      ...
                                      const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(2754, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < ecp_nppt; l++)
                {
                    ...
                    t2lecp::comp_prim_local_core_potential_dp(pbuffer, 224, 1, 56, 59, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dd(pbuffer, 242, 4, 59, 68, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_df(pbuffer, 278, 10, 68, 86, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dg(pbuffer, 338, 20, 86, 116, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dh(pbuffer, 428, 35, 116, 161, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_fd(pbuffer, 554, 68, 224, 242, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_ff(pbuffer, 614, 86, 242, 278, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_fg(pbuffer, 714, 116, 278, 338, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_fh(pbuffer, 864, 161, 338, 428, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_gf(pbuffer, 1074, 278, 554, 614, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_gg(pbuffer, 1224, 338, 614, 714, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_gh(pbuffer, 1449, 428, 714, 864, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_hg(pbuffer, 1764, 714, 1074, 1224, pfactors, 8, 14);

                    t2cgeom::comp_prim_op_geom_010_gg(pbuffer, 2079, 714, 1074, 1449, 1764, pfactors, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 2079, ket_width, ket_npgtos);
                }
            }
...
#endif /* LocalCorePotentialGeom010GG_hpp */
