#ifndef LocalCorePotentialGeom020SG_hpp
#define LocalCorePotentialGeom020SG_hpp
...
template <class T>
auto
comp_local_core_potential_geom_020_sg(T& distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < ecp_nppt; l++)
                {
                    ...
                    t2lecp::comp_prim_local_core_potential_pf(pbuffer, 84, 4, 10, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_pg(pbuffer, 114, 10, 20, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_ph(pbuffer, 159, 20, 35, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dg(pbuffer, 222, 20, 84, 114, pfactors, 8, 14);

                    t2cgeom::comp_prim_op_geom_020_sg(pbuffer, 312, 4, 20, 56, 84, 159, 222, pfactors, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 312, ket_width, ket_npgtos);
                }
            }
...
#endif /* LocalCorePotentialGeom020SG_hpp */
