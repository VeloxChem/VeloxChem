#ifndef LocalCorePotentialGeom020PG_hpp
#define LocalCorePotentialGeom020PG_hpp
...
template <class T>
auto
comp_local_core_potential_geom_020_pg(T& distributor,
                                      ...
                                      const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(1020, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < ecp_nppt; l++)
                {
                    ...
                    t2lecp::comp_prim_local_core_potential_pd(pbuffer, 84, 1, 4, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_pf(pbuffer, 102, 4, 10, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_pg(pbuffer, 132, 10, 20, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_ph(pbuffer, 177, 20, 35, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_pi(pbuffer, 240, 35, 56, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_df(pbuffer, 324, 10, 84, 102, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dg(pbuffer, 384, 20, 102, 132, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dh(pbuffer, 474, 35, 132, 177, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_fg(pbuffer, 600, 132, 324, 384, pfactors, 8, 14);

                    t2cgeom::comp_prim_op_geom_020_pg(pbuffer, 750, 10, 35, 84, 132, 240, 324, 474, 600, pfactors, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 750, ket_width, ket_npgtos);
                }
            }
...
#endif /* LocalCorePotentialGeom020PG_hpp */
