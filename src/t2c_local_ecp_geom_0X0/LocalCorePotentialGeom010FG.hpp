#ifndef LocalCorePotentialGeom010FG_hpp
#define LocalCorePotentialGeom010FG_hpp
...
template <class T>
auto
comp_local_core_potential_geom_010_fg(T& distributor,
                                      ...
                                      const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(1668, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < ecp_nppt; l++)
                {
                    ...
                    t2lecp::comp_prim_local_core_potential_pp(pbuffer, 56, 0, 1, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_pd(pbuffer, 65, 1, 4, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_pf(pbuffer, 83, 4, 10, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_pg(pbuffer, 113, 10, 20, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_ph(pbuffer, 158, 20, 35, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dd(pbuffer, 221, 4, 56, 65, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_df(pbuffer, 257, 10, 65, 83, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dg(pbuffer, 317, 20, 83, 113, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dh(pbuffer, 407, 35, 113, 158, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_ff(pbuffer, 533, 83, 221, 257, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_fg(pbuffer, 633, 113, 257, 317, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_fh(pbuffer, 783, 158, 317, 407, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_gg(pbuffer, 993, 317, 533, 633, pfactors, 8, 14);

                    t2cgeom::comp_prim_op_geom_010_fg(pbuffer, 1218, 317, 533, 783, 993, pfactors, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 1218, ket_width, ket_npgtos);
                }
            }
...
#endif /* LocalCorePotentialGeom010FG_hpp */
