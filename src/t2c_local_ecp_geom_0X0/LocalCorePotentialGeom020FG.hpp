#ifndef LocalCorePotentialGeom020FG_hpp
#define LocalCorePotentialGeom020FG_hpp
...
template <class T>
auto
comp_local_core_potential_geom_020_fg(T& distributor,
                                      ...
                                      const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(3539, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < ecp_nppt; l++)
                {
                    ...
                    t2lecp::comp_prim_local_core_potential_dp(pbuffer, 336, 1, 84, 87, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dd(pbuffer, 354, 4, 87, 96, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_df(pbuffer, 390, 10, 96, 114, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dg(pbuffer, 450, 20, 114, 144, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dh(pbuffer, 540, 35, 144, 189, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_di(pbuffer, 666, 56, 189, 252, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_fd(pbuffer, 834, 96, 336, 354, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_ff(pbuffer, 894, 114, 354, 390, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_fg(pbuffer, 994, 144, 390, 450, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_fh(pbuffer, 1144, 189, 450, 540, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_fi(pbuffer, 1354, 252, 540, 666, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_gf(pbuffer, 1634, 390, 834, 894, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_gg(pbuffer, 1784, 450, 894, 994, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_gh(pbuffer, 2009, 540, 994, 1144, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_hg(pbuffer, 2324, 994, 1634, 1784, pfactors, 8, 14);

                    t2cgeom::comp_prim_op_geom_020_fg(pbuffer, 2639, 144, 390, 540, 834, 994, 1354, 1634, 2009, 2324, pfactors, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 2639, ket_width, ket_npgtos);
                }
            }
...
#endif /* LocalCorePotentialGeom020FG_hpp */
