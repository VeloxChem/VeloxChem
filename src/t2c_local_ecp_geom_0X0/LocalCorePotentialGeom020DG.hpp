#ifndef LocalCorePotentialGeom020DG_hpp
#define LocalCorePotentialGeom020DG_hpp
...
template <class T>
auto
comp_local_core_potential_geom_020_dg(T& distributor,
                                      ...
                                      const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(2038, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < ecp_nppt; l++)
                {
                    ...
                    t2lecp::comp_prim_local_core_potential_pp(pbuffer, 84, 0, 1, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_pd(pbuffer, 93, 1, 4, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_pf(pbuffer, 111, 4, 10, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_pg(pbuffer, 141, 10, 20, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_ph(pbuffer, 186, 20, 35, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_pi(pbuffer, 249, 35, 56, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dd(pbuffer, 333, 4, 84, 93, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_df(pbuffer, 369, 10, 93, 111, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dg(pbuffer, 429, 20, 111, 141, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_dh(pbuffer, 519, 35, 141, 186, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_di(pbuffer, 645, 56, 186, 249, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_ff(pbuffer, 813, 111, 333, 369, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_fg(pbuffer, 913, 141, 369, 429, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_fh(pbuffer, 1063, 186, 429, 519, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_gg(pbuffer, 1273, 429, 813, 913, pfactors, 8, 14);

                    t2cgeom::comp_prim_op_geom_020_dg(pbuffer, 1498, 20, 111, 186, 333, 429, 645, 813, 1063, 1273, pfactors, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 1498, ket_width, ket_npgtos);
                }
            }
...
#endif /* LocalCorePotentialGeom020DG_hpp */
