#ifndef LocalCorePotentialGeom020GD_hpp
#define LocalCorePotentialGeom020GD_hpp
...
template <class T>
auto
comp_local_core_potential_geom_020_gd(T& distributor,
                                      ...
                                      const bool bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(2332, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < ecp_nppt; l++)
                {
                    ...
                    t2lecp::comp_prim_local_core_potential_gs(pbuffer, 700, 140, 350, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_gp(pbuffer, 715, 146, 350, 360, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_gd(pbuffer, 760, 164, 360, 390, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_gf(pbuffer, 850, 200, 390, 450, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_gg(pbuffer, 1000, 260, 450, 550, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_hp(pbuffer, 1225, 360, 700, 715, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_hd(pbuffer, 1288, 390, 715, 760, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_hf(pbuffer, 1414, 450, 760, 850, pfactors, 8, 14);

                    t2lecp::comp_prim_local_core_potential_id(pbuffer, 1624, 760, 1225, 1288, pfactors, 8, 14);

                    t2cgeom::comp_prim_op_geom_020_gd(pbuffer, 1792, 164, 360, 450, 700, 760, 1000, 1225, 1414, 1624, pfactors, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 1792, ket_width, ket_npgtos);
                }
            }
...
#endif /* LocalCorePotentialGeom020GD_hpp */
