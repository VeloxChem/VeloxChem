#ifndef NuclearPotentialGridRecFD_hpp
#define NuclearPotentialGridRecFD_hpp
...
/// @brief Computes (F|A|D)  integrals for pair of basis functions on given grid.
...
auto
comp_on_grid_nuclear_potential_fd(CSubMatrix& spher_buffer,
                                  ...
                                  const int ket_igto) -> void
{
    ...
    for (size_t i = 0; i < bra_npgtos; i++)
    {
        ...
        for (size_t j = 0; j < ket_npgtos; j++)
        {
            ...
            npotrec::comp_on_grid_prim_nuclear_potential_ps(cart_buffer, 55, 10, 11, pa_x, pa_y, pa_z);

            npotrec::comp_on_grid_prim_nuclear_potential_ps(cart_buffer, 58, 11, 12, pa_x, pa_y, pa_z);

            npotrec::comp_on_grid_prim_nuclear_potential_ps(cart_buffer, 61, 12, 13, pa_x, pa_y, pa_z);

            npotrec::comp_on_grid_prim_nuclear_potential_pp(cart_buffer, 64, 10, 11, 16, 19, pa_x, pa_y, pa_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_pp(cart_buffer, 73, 11, 12, 19, 22, pa_x, pa_y, pa_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_pp(cart_buffer, 82, 12, 13, 22, 25, pa_x, pa_y, pa_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_pd(cart_buffer, 91, 16, 19, 31, 37, pa_x, pa_y, pa_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_pd(cart_buffer, 109, 19, 22, 37, 43, pa_x, pa_y, pa_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_pd(cart_buffer, 127, 22, 25, 43, 49, pa_x, pa_y, pa_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_dp(cart_buffer, 145, 16, 19, 55, 58, 64, 73, pa_x, pa_y, pa_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_dp(cart_buffer, 163, 19, 22, 58, 61, 73, 82, pa_x, pa_y, pa_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_dd(cart_buffer, 181, 31, 37, 64, 73, 91, 109, pa_x, pa_y, pa_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_dd(cart_buffer, 217, 37, 43, 73, 82, 109, 127, pa_x, pa_y, pa_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_fd(cart_buffer, 253, 91, 109, 145, 163, 181, 217, pa_x, pa_y, pa_z, a_exp + b_exp);

            // reduce integrals

            t2cfunc::reduce(cart_buffer, 313, 253, 60);
    ...
    t2cfunc::transform<3, 2>(spher_buffer, cart_buffer, 313);
}

} // npotrec namespace

#endif /* NuclearPotentialGridRecFD_hpp */
