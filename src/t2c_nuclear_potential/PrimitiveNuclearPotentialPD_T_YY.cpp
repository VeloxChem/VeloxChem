#include "PrimitiveNuclearPotentialPD_T_YY.hpp"
...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] +=
            fss * b1_vals[i] * (-(1.0 / 2.0) * fe_0 * rpa_x - (1.0 / 2.0) * fe_0 * rpc_x - 2.0 * rpa_x * rpb_y * rpc_y - rpb_y * rpb_y * rpc_x);
        ...
        fints_z[i] +=
            fss * b1_vals[i] * (-(1.0 / 2.0) * fe_0 * rpa_z - (1.0 / 2.0) * fe_0 * rpc_z - 2.0 * rpa_z * rpb_y * rpc_y - rpb_y * rpb_y * rpc_z);
