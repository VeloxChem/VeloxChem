#ifndef NuclearPotentialGridRecPF_hpp
#define NuclearPotentialGridRecPF_hpp
...
/// @brief Computes (P|A|F)  integrals for pair of basis functions on given grid.
...
auto
comp_on_grid_nuclear_potential_pf(CSubMatrix& spher_buffer,
                                  ...
                                  const int ket_igto) -> void
{
    ...
    for (size_t i = 0; i < bra_npgtos; i++)
    {
        ...
        for (size_t j = 0; j < ket_npgtos; j++)
        {
            ...
            npotrec::comp_on_grid_prim_nuclear_potential_sf(cart_buffer, 44, 14, 17, 26, 32, pb_x, pb_y, pb_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_sf(cart_buffer, 54, 17, 20, 32, 38, pb_x, pb_y, pb_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_pf(cart_buffer, 64, 26, 32, 44, 54, pa_x, pa_y, pa_z, a_exp + b_exp);

            // reduce integrals

            t2cfunc::reduce(cart_buffer, 94, 64, 30);
    ...
    t2cfunc::transform<1, 3>(spher_buffer, cart_buffer, 94);
}

} // npotrec namespace

#endif /* NuclearPotentialGridRecPF_hpp */
