#ifndef NuclearPotentialSumRecSF_hpp
#define NuclearPotentialSumRecSF_hpp
...
/// @brief Computes (S|A|F)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_nuclear_potential_sf(T&                               distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    npotrec::comp_prim_nuclear_potential_sp(pbuffer, 5, 1, 2, factors, 11, 14);

                    npotrec::comp_prim_nuclear_potential_sp(pbuffer, 8, 2, 3, factors, 11, 14);

                    npotrec::comp_prim_nuclear_potential_sp(pbuffer, 11, 3, 4, factors, 11, 14);

                    npotrec::comp_prim_nuclear_potential_sd(pbuffer, 14, 1, 2, 5, 8, factors, 11, 14, a_exp);

                    npotrec::comp_prim_nuclear_potential_sd(pbuffer, 20, 2, 3, 8, 11, factors, 11, 14, a_exp);

                    npotrec::comp_prim_nuclear_potential_sf(pbuffer, 26, 5, 8, 14, 20, factors, 11, 14, a_exp);
...
#endif /* NuclearPotentialSumRecSF_hpp */
