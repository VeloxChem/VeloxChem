#include "PrimitiveNuclearPotentialDP_ZZ_T.hpp"
...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] +=
            fss * b1_vals[i] * (-(1.0 / 2.0) * fe_0 * rpb_x - (1.0 / 2.0) * fe_0 * rpc_x - 2.0 * rpa_z * rpb_x * rpc_z - rpa_z * rpa_z * rpc_x);
        ...
        fints_y[i] +=
            fss * b1_vals[i] * (-(1.0 / 2.0) * fe_0 * rpb_y - (1.0 / 2.0) * fe_0 * rpc_y - 2.0 * rpa_z * rpb_y * rpc_z - rpa_z * rpa_z * rpc_y);
