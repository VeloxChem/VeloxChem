#ifndef NuclearPotentialGridRecSD_hpp
#define NuclearPotentialGridRecSD_hpp
...
/// @brief Computes (S|A|D)  integrals for pair of basis functions on given grid.
...
auto
comp_on_grid_nuclear_potential_sd(CSubMatrix& spher_buffer,
                                  ...
                                  const int ket_igto) -> void
{
    ...
    for (size_t i = 0; i < bra_npgtos; i++)
    {
        ...
        for (size_t j = 0; j < ket_npgtos; j++)
        {
            ...
            npotrec::comp_on_grid_prim_nuclear_potential_ss(cart_buffer, 7, 4, fovl, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_ss(cart_buffer, 8, 5, fovl, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_ss(cart_buffer, 9, 6, fovl, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_sp(cart_buffer, 10, 7, 8, pb_x, pb_y, pb_z);

            npotrec::comp_on_grid_prim_nuclear_potential_sp(cart_buffer, 13, 8, 9, pb_x, pb_y, pb_z);

            npotrec::comp_on_grid_prim_nuclear_potential_sd(cart_buffer, 16, 7, 8, 10, 13, pb_x, pb_y, pb_z, a_exp + b_exp);

            // reduce integrals

            t2cfunc::reduce(cart_buffer, 22, 16, 6);
        }
    }
    
    // transform integrals

    t2cfunc::transform<0, 2>(spher_buffer, cart_buffer, 22);
}

} // npotrec namespace

#endif /* NuclearPotentialGridRecSD_hpp */
