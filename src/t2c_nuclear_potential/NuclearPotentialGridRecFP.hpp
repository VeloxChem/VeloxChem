#ifndef NuclearPotentialGridRecFP_hpp
#define NuclearPotentialGridRecFP_hpp
...
/// @brief Computes (F|A|P)  integrals for pair of basis functions on given grid.
...
auto
comp_on_grid_nuclear_potential_fp(CSubMatrix& spher_buffer,
                                  ...
                                  const int ket_igto) -> void
{
    ...
    for (size_t i = 0; i < bra_npgtos; i++)
    {
        ...
        for (size_t j = 0; j < ket_npgtos; j++)
        {
            ...
            npotrec::comp_on_grid_prim_nuclear_potential_ps(cart_buffer, 26, 9, 10, pa_x, pa_y, pa_z);

            npotrec::comp_on_grid_prim_nuclear_potential_ps(cart_buffer, 29, 10, 11, pa_x, pa_y, pa_z);

            npotrec::comp_on_grid_prim_nuclear_potential_ps(cart_buffer, 32, 11, 12, pa_x, pa_y, pa_z);

            npotrec::comp_on_grid_prim_nuclear_potential_pp(cart_buffer, 35, 9, 10, 14, 17, pa_x, pa_y, pa_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_pp(cart_buffer, 44, 10, 11, 17, 20, pa_x, pa_y, pa_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_pp(cart_buffer, 53, 11, 12, 20, 23, pa_x, pa_y, pa_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_ds(cart_buffer, 62, 9, 10, 26, 29, pa_x, pa_y, pa_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_ds(cart_buffer, 68, 10, 11, 29, 32, pa_x, pa_y, pa_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_dp(cart_buffer, 74, 14, 17, 26, 29, 35, 44, pa_x, pa_y, pa_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_dp(cart_buffer, 92, 17, 20, 29, 32, 44, 53, pa_x, pa_y, pa_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_fp(cart_buffer, 110, 35, 44, 62, 68, 74, 92, pa_x, pa_y, pa_z, a_exp + b_exp);

            // reduce integrals

            t2cfunc::reduce(cart_buffer, 140, 110, 30);
    ...
    t2cfunc::transform<3, 1>(spher_buffer, cart_buffer, 140);
}

} // npotrec namespace

#endif /* NuclearPotentialGridRecFP_hpp */
