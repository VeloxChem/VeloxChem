#include "PrimitiveNuclearPotentialSS.hpp"
...
#pragma omp simd aligned(targs, ket_fe, ket_rx, ket_ry, ket_rz : 64)
    ...
    bf_table.compute<1>(bf_values, bf_args, ket_dim);
...
#pragma omp simd aligned(fints, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints[i] += b0_vals[i] * 2.0 * charge * std::sqrt(fxi_0 / fpi) * bra_norm * ket_fn[i] * std::pow(fe_0 * fpi, 1.50) * std::exp(-fz_0);
    }
}
