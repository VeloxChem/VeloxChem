#ifndef NuclearPotentialGridRecPD_hpp
#define NuclearPotentialGridRecPD_hpp
...
/// @brief Computes (P|A|D)  integrals for pair of basis functions on given grid.
...
auto
comp_on_grid_nuclear_potential_pd(CSubMatrix& spher_buffer,
                                  ...
                                  const int ket_igto) -> void
{
    ...
    for (size_t i = 0; i < bra_npgtos; i++)
    {
        ...
        for (size_t j = 0; j < ket_npgtos; j++)
        {
            ...
            npotrec::comp_on_grid_prim_nuclear_potential_pd(cart_buffer, 33, 12, 15, 21, 27, pa_x, pa_y, pa_z, a_exp + b_exp);

            // reduce integrals

            t2cfunc::reduce(cart_buffer, 51, 33, 18);
        }
    }
    
    // transform integrals

    t2cfunc::transform<1, 2>(spher_buffer, cart_buffer, 51);
}

} // npotrec namespace

#endif /* NuclearPotentialGridRecPD_hpp */
