#ifndef NuclearPotentialGridRecPP_hpp
#define NuclearPotentialGridRecPP_hpp
...
/// @brief Computes (P|A|P)  integrals for pair of basis functions on given grid.
...
auto
comp_on_grid_nuclear_potential_pp(CSubMatrix& spher_buffer,
                                  ...
                                  const int ket_igto) -> void
{
    ...
    for (size_t i = 0; i < bra_npgtos; i++)
    {
        ...
        for (size_t j = 0; j < ket_npgtos; j++)
        {
            ...
            npotrec::comp_on_grid_prim_nuclear_potential_pp(cart_buffer, 16, 7, 8, 10, 13, pa_x, pa_y, pa_z, a_exp + b_exp);

            // reduce integrals

            t2cfunc::reduce(cart_buffer, 25, 16, 9);
        }
    }
    
    // transform integrals

    t2cfunc::transform<1, 1>(spher_buffer, cart_buffer, 25);
}

} // npotrec namespace

#endif /* NuclearPotentialGridRecPP_hpp */
