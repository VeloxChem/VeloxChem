#include "PrimitiveNuclearPotentialPD_T_XX.hpp"
...
auto
compPrimitiveNuclearPotentialPD_T_XX(TDoubleArray&       buffer_x,
...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += fss * b0_vals[i] * ((1.0 / 2.0) * fe_0 * rpa_x + fe_0 * rpb_x + rpa_x * rpb_x * rpb_x);

        fints_x[i] += fss * b1_vals[i] *
                      (-(1.0 / 2.0) * fe_0 * rpa_x - fe_0 * rpb_x - (3.0 / 2.0) * fe_0 * rpc_x - 2.0 * rpa_x * rpb_x * rpc_x - rpb_x * rpb_x * rpc_x);

        fints_x[i] += fss * b2_vals[i] * ((3.0 / 2.0) * fe_0 * rpc_x + rpa_x * rpc_x * rpc_x + 2.0 * rpb_x * rpc_x * rpc_x);

        fints_x[i] += fss * b3_vals[i] * (-rpc_x * rpc_x * rpc_x);

        fints_y[i] += fss * b0_vals[i] * ((1.0 / 2.0) * fe_0 * rpa_y + rpa_y * rpb_x * rpb_x);

        fints_y[i] +=
            fss * b1_vals[i] * (-(1.0 / 2.0) * fe_0 * rpa_y - (1.0 / 2.0) * fe_0 * rpc_y - 2.0 * rpa_y * rpb_x * rpc_x - rpb_x * rpb_x * rpc_y);

        fints_y[i] += fss * b2_vals[i] * ((1.0 / 2.0) * fe_0 * rpc_y + rpa_y * rpc_x * rpc_x + 2.0 * rpb_x * rpc_y * rpc_x);

        fints_y[i] += fss * b3_vals[i] * (-rpc_y * rpc_x * rpc_x);

        fints_z[i] += fss * b0_vals[i] * ((1.0 / 2.0) * fe_0 * rpa_z + rpa_z * rpb_x * rpb_x);

        fints_z[i] +=
            fss * b1_vals[i] * (-(1.0 / 2.0) * fe_0 * rpa_z - (1.0 / 2.0) * fe_0 * rpc_z - 2.0 * rpa_z * rpb_x * rpc_x - rpb_x * rpb_x * rpc_z);

        fints_z[i] += fss * b2_vals[i] * ((1.0 / 2.0) * fe_0 * rpc_z + rpa_z * rpc_x * rpc_x + 2.0 * rpb_x * rpc_z * rpc_x);

        fints_z[i] += fss * b3_vals[i] * (-rpc_z * rpc_x * rpc_x);
    }
}
