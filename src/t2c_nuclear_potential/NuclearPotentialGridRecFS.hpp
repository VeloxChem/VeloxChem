#ifndef NuclearPotentialGridRecFS_hpp
#define NuclearPotentialGridRecFS_hpp
...
/// @brief Computes (F|A|S)  integrals for pair of basis functions on given grid.
...
auto
comp_on_grid_nuclear_potential_fs(CSubMatrix& spher_buffer,
                                  ...
                                  const int ket_igto) -> void
{
    ...
    for (size_t i = 0; i < bra_npgtos; i++)
    {
        ...
        for (size_t j = 0; j < ket_npgtos; j++)
        {
            ...
            npotrec::comp_on_grid_prim_nuclear_potential_ps(cart_buffer, 12, 8, 9, pa_x, pa_y, pa_z);

            npotrec::comp_on_grid_prim_nuclear_potential_ps(cart_buffer, 15, 9, 10, pa_x, pa_y, pa_z);

            npotrec::comp_on_grid_prim_nuclear_potential_ps(cart_buffer, 18, 10, 11, pa_x, pa_y, pa_z);

            npotrec::comp_on_grid_prim_nuclear_potential_ds(cart_buffer, 21, 8, 9, 12, 15, pa_x, pa_y, pa_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_ds(cart_buffer, 27, 9, 10, 15, 18, pa_x, pa_y, pa_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_fs(cart_buffer, 33, 12, 15, 21, 27, pa_x, pa_y, pa_z, a_exp + b_exp);
    ...
    t2cfunc::transform<3, 0>(spher_buffer, cart_buffer, 43);
}

} // npotrec namespace

#endif /* NuclearPotentialGridRecFS_hpp */
