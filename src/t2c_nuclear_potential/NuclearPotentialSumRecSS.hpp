#ifndef NuclearPotentialSumRecSS_hpp
#define NuclearPotentialSumRecSS_hpp
...
template <class T>
auto
comp_sum_nuclear_potential_ss(T&                               distributor,
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    t2cfunc::comp_boys_args(bf_data, 1, factors, 11, a_exp);

                    bf_table.compute(bf_data, 0, 1);

                    npotrec::comp_prim_nuclear_potential_ss(pbuffer, 1, 0, bf_data, 0, factors, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 1, charges[l], ket_width, ket_npgtos);
                }
            }
...
#endif /* NuclearPotentialSumRecSS_hpp */
