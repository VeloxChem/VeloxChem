#ifndef NuclearPotentialGridRecDS_hpp
#define NuclearPotentialGridRecDS_hpp
...
/// @brief Computes (D|A|S)  integrals for pair of basis functions on given grid.
...
auto
comp_on_grid_nuclear_potential_ds(CSubMatrix& spher_buffer,
                                  ...
                                  const int ket_igto) -> void
{
    ...
    for (size_t i = 0; i < bra_npgtos; i++)
    {
        ...
        for (size_t j = 0; j < ket_npgtos; j++)
        {
            ...
            npotrec::comp_on_grid_prim_nuclear_potential_ps(cart_buffer, 10, 7, 8, pa_x, pa_y, pa_z);

            npotrec::comp_on_grid_prim_nuclear_potential_ps(cart_buffer, 13, 8, 9, pa_x, pa_y, pa_z);

            npotrec::comp_on_grid_prim_nuclear_potential_ds(cart_buffer, 16, 7, 8, 10, 13, pa_x, pa_y, pa_z, a_exp + b_exp);
    ...
    t2cfunc::transform<2, 0>(spher_buffer, cart_buffer, 22);
}

} // npotrec namespace

#endif /* NuclearPotentialGridRecDS_hpp */
