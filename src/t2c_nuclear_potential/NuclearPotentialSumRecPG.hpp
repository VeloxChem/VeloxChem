#ifndef NuclearPotentialSumRecPG_hpp
#define NuclearPotentialSumRecPG_hpp
...
/// @brief Computes (P|A|G)  integrals for pair of basis functions blocks.
...
template <class T>
auto
comp_sum_nuclear_potential_pg(T&                               distributor,
                              ...
                              const bool                       bra_eq_ket) -> void
{
    ...
    CSimdArray<double> pbuffer(151, ket_npgtos);
        ...
        for (auto j = bra_indices.first; j < bra_indices.second; j++)
        {
            ...
            for (size_t k = 0; k < bra_npgtos; k++)
            {
                ...
                for (size_t l = 0; l < coords.size(); l++)
                {
                    ...
                    npotrec::comp_prim_nuclear_potential_sg(pbuffer, 76, 22, 28, 46, 56, factors, 14, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_sg(pbuffer, 91, 28, 34, 56, 66, factors, 14, 17, a_exp);

                    npotrec::comp_prim_nuclear_potential_pg(pbuffer, 106, 46, 56, 76, 91, factors, 11, 17, a_exp);

                    t2cfunc::reduce(cbuffer, pbuffer, 106, charges[l], ket_width, ket_npgtos);
                }
            }
...
#endif /* NuclearPotentialSumRecPG_hpp */
