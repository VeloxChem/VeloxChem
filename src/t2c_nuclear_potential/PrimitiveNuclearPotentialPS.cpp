#include "PrimitiveNuclearPotentialPS.hpp"
...
auto
compPrimitiveNuclearPotentialPS(TDoubleArray&       buffer_x,
...
#pragma omp simd aligned(fints_x, fints_y, fints_z, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_x[i] += fss * b0_vals[i] * rpa_x;

        fints_x[i] += fss * b1_vals[i] * (-rpc_x);

        fints_y[i] += fss * b0_vals[i] * rpa_y;

        fints_y[i] += fss * b1_vals[i] * (-rpc_y);

        fints_z[i] += fss * b0_vals[i] * rpa_z;

        fints_z[i] += fss * b1_vals[i] * (-rpc_z);
    }
}
