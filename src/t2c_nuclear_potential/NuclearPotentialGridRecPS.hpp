#ifndef NuclearPotentialGridRecPS_hpp
#define NuclearPotentialGridRecPS_hpp
...
/// @brief Computes (P|A|S)  integrals for pair of basis functions on given grid.
...
auto
comp_on_grid_nuclear_potential_ps(CSubMatrix& spher_buffer,
                                  ...
                                  const int ket_igto) -> void
{
    ...
    for (size_t i = 0; i < bra_npgtos; i++)
    {
        ...
        for (size_t j = 0; j < ket_npgtos; j++)
        {
            ...
            npotrec::comp_on_grid_prim_nuclear_potential_ss(cart_buffer, 6, 4, fovl, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_ss(cart_buffer, 7, 5, fovl, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_ps(cart_buffer, 8, 6, 7, pa_x, pa_y, pa_z);

            // reduce integrals

            t2cfunc::reduce(cart_buffer, 11, 8, 3);
        }
    }
    
    // transform integrals

    t2cfunc::transform<1, 0>(spher_buffer, cart_buffer, 11);
}

} // npotrec namespace

#endif /* NuclearPotentialGridRecPS_hpp */
