#ifndef NuclearPotentialGridRecSF_hpp
#define NuclearPotentialGridRecSF_hpp
...
/// @brief Computes (S|A|F)  integrals for pair of basis functions on given grid.
...
auto
comp_on_grid_nuclear_potential_sf(CSubMatrix& spher_buffer,
                                  ...
                                  const int ket_igto) -> void
{
    ...
    for (size_t i = 0; i < bra_npgtos; i++)
    {
        ...
        for (size_t j = 0; j < ket_npgtos; j++)
        {
            ...
            npotrec::comp_on_grid_prim_nuclear_potential_sd(cart_buffer, 21, 8, 9, 12, 15, pb_x, pb_y, pb_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_sd(cart_buffer, 27, 9, 10, 15, 18, pb_x, pb_y, pb_z, a_exp + b_exp);

            npotrec::comp_on_grid_prim_nuclear_potential_sf(cart_buffer, 33, 12, 15, 21, 27, pb_x, pb_y, pb_z, a_exp + b_exp);

            // reduce integrals

            t2cfunc::reduce(cart_buffer, 43, 33, 10);
    ...
    t2cfunc::transform<0, 3>(spher_buffer, cart_buffer, 43);
}

} // npotrec namespace

#endif /* NuclearPotentialGridRecSF_hpp */
