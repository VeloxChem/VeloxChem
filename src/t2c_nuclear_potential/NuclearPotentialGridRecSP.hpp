#ifndef NuclearPotentialGridRecSP_hpp
#define NuclearPotentialGridRecSP_hpp
...
/// @brief Computes (S|A|P)  integrals for pair of basis functions on given grid.
...
auto
comp_on_grid_nuclear_potential_sp(CSubMatrix& spher_buffer,
                                  ...
                                  const int ket_igto) -> void
{
    ...
    for (size_t i = 0; i < bra_npgtos; i++)
    {
        ...
        for (size_t j = 0; j < ket_npgtos; j++)
        {
            ...
            npotrec::comp_on_grid_prim_nuclear_potential_sp(cart_buffer, 8, 6, 7, pb_x, pb_y, pb_z);
    ...
    t2cfunc::transform<0, 1>(spher_buffer, cart_buffer, 11);
}

} // npotrec namespace

#endif /* NuclearPotentialGridRecSP_hpp */
