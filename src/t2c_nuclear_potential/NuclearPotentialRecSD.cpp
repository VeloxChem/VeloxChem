                                ...
                                const int64_t       ket_dim) -> void
{
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += fss * b0_vals[i] * ((1.0 / 2.0) * fe_0 + rpb_x * rpb_x);

        fints_xx[i] += fss * b1_vals[i] * (-(1.0 / 2.0) * fe_0 - 2.0 * rpb_x * rpc_x);

        fints_xx[i] += fss * b2_vals[i] * rpc_x * rpc_x;

        fints_xy[i] += fss * b0_vals[i] * rpb_y * rpb_x;

        fints_xy[i] += fss * b1_vals[i] * (-rpb_y * rpc_x - rpb_x * rpc_y);

        fints_xy[i] += fss * b2_vals[i] * rpc_y * rpc_x;

        fints_xz[i] += fss * b0_vals[i] * rpb_z * rpb_x;

        fints_xz[i] += fss * b1_vals[i] * (-rpb_z * rpc_x - rpb_x * rpc_z);

        fints_xz[i] += fss * b2_vals[i] * rpc_z * rpc_x;

        fints_yy[i] += fss * b0_vals[i] * ((1.0 / 2.0) * fe_0 + rpb_y * rpb_y);

        fints_yy[i] += fss * b1_vals[i] * (-(1.0 / 2.0) * fe_0 - 2.0 * rpb_y * rpc_y);

        fints_yy[i] += fss * b2_vals[i] * rpc_y * rpc_y;

        fints_yz[i] += fss * b0_vals[i] * rpb_z * rpb_y;

        fints_yz[i] += fss * b1_vals[i] * (-rpb_z * rpc_y - rpb_y * rpc_z);

        fints_yz[i] += fss * b2_vals[i] * rpc_z * rpc_y;

        fints_zz[i] += fss * b0_vals[i] * ((1.0 / 2.0) * fe_0 + rpb_z * rpb_z);

        fints_zz[i] += fss * b1_vals[i] * (-(1.0 / 2.0) * fe_0 - 2.0 * rpb_z * rpc_z);
