        ...
        for (int64_t j = bra_first; j < bra_last; j++) 
        {
            ...
            for (int64_t k = 0; k < ket_npgtos; k++)
            {
                ...
                for (int64_t l = 0; l < bra_npgtos; l++)
                {
                    ...
                    npotrec::compPrimitiveNuclearPotentialSD(buffer_xx,
                                                             buffer_xy,
                                                             buffer_xz,
                                                             buffer_yy,
                                                             buffer_yz,
                                ...
                                const int64_t       ket_dim) -> void
{
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += fss * bf_values[0][i] * ((1.0 / 2.0) * fe_0 + rpb_x * rpb_x);

        fints_xx[i] += fss * bf_values[1][i] * (-(1.0 / 2.0) * fe_0 - 2.0 * rpb_x * rpc_x);

        fints_xx[i] += fss * bf_values[2][i] * rpc_x * rpc_x;

        fints_xy[i] += fss * bf_values[0][i] * rpb_y * rpb_x;

        fints_xy[i] += fss * bf_values[1][i] * (-rpb_y * rpc_x - rpb_x * rpc_y);

        fints_xy[i] += fss * bf_values[2][i] * rpc_y * rpc_x;

        fints_xz[i] += fss * bf_values[0][i] * rpb_z * rpb_x;

        fints_xz[i] += fss * bf_values[1][i] * (-rpb_z * rpc_x - rpb_x * rpc_z);

        fints_xz[i] += fss * bf_values[2][i] * rpc_z * rpc_x;

        fints_yy[i] += fss * bf_values[0][i] * ((1.0 / 2.0) * fe_0 + rpb_y * rpb_y);

        fints_yy[i] += fss * bf_values[1][i] * (-(1.0 / 2.0) * fe_0 - 2.0 * rpb_y * rpc_y);

        fints_yy[i] += fss * bf_values[2][i] * rpc_y * rpc_y;

        fints_yz[i] += fss * bf_values[0][i] * rpb_z * rpb_y;

        fints_yz[i] += fss * bf_values[1][i] * (-rpb_z * rpc_y - rpb_y * rpc_z);

        fints_yz[i] += fss * bf_values[2][i] * rpc_z * rpc_y;

        fints_zz[i] += fss * bf_values[0][i] * ((1.0 / 2.0) * fe_0 + rpb_z * rpb_z);

        fints_zz[i] += fss * bf_values[1][i] * (-(1.0 / 2.0) * fe_0 - 2.0 * rpb_z * rpc_z);
