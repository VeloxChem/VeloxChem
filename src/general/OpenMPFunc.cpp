...
auto
make_work_group_bsfac(const std::vector<CBlockedGtoPairBlock>& gto_pair_blocks, const int ithreshold, const int block_size_factor) -> std::vector<std::array<size_t, 8>>
{
    auto wtasks = std::vector<std::array<size_t, 8>>();
...
auto
make_bra_ket_work_group(const std::vector<CBlockedGtoPairBlock>& bra_gto_pair_blocks,
                        ...
                        const int                                block_size_factor) -> std::vector<std::array<size_t, 8>>
{
    ...
    const auto bra_nblocks = bra_gto_pair_blocks.size();
    const auto ket_nblocks = ket_gto_pair_blocks.size();
   
    if ((bra_nblocks > 0) && (ket_nblocks > 0))
    {
        for (size_t i = 0; i < bra_nblocks; i++)
        {
            for (size_t j = 0; j < ket_nblocks; j++)
            {
                ...
                for (int bra_idx = 0; bra_idx < 16; bra_idx++)
                {
                    if (bra_gto_pair_blocks[i].is_empty_gto_pair_block(bra_idx)) continue;
                    
                    const auto bra_block = bra_gto_pair_blocks[i].gto_pair_block(bra_idx);
                    ...
                    for (int ket_idx = 0; ket_idx < 16; ket_idx++)
                    {
                        if (ket_gto_pair_blocks[j].is_empty_gto_pair_block(ket_idx)) continue;
                        
                        const auto ket_block = ket_gto_pair_blocks[j].gto_pair_block(ket_idx);
