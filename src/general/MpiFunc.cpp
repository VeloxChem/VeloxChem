...
rank(MPI_Comm comm) -> int
{
    ...
    return static_cast<int>(mrank_int32);
}

auto
nodes(MPI_Comm comm) -> int
{
    int32_t mnodes_int32 = 1;

    if constexpr (ENABLE_MPI) MPI_Comm_size(comm, &mnodes_int32);

    return static_cast<int>(mnodes_int32);
}

auto
bcastScalar(const int val, MPI_Comm comm) -> int
{
    int bcast_val = 0;

    if (mpi::rank(comm) == mpi::master()) bcast_val = val;

    auto merror = MPI_Bcast(&bcast_val, 1, MPI_INT, static_cast<int32_t>(mpi::master()), comm);
...
bcastDenseMatrix(const CDenseMatrix& matrix, MPI_Comm comm) -> CDenseMatrix
{
    if constexpr (ENABLE_MPI)
    {
        ...
        std::vector<int> num_rows_cols(2);
        ...
        auto merror = MPI_Bcast(num_rows_cols.data(), 2, MPI_INT, mpi_master_int32, comm);
...
auto
scatterStdVector(const std::vector<int>& vec, MPI_Comm comm) -> std::vector<int>
{
    if constexpr (ENABLE_MPI)
    {
        // MPI info
        ...
        if (rank == mpi_master)
        {
            nelems = static_cast<int>(vec.size());
        ...
        auto merror = MPI_Bcast(&nelems, 1, MPI_INT, mpi_master_int32, comm);
        ...
        merror = MPI_Scatterv(
            vec.data(), counts.data(), displs.data(), MPI_INT, recv_vec.data(), recv_size_int32, MPI_INT, mpi_master_int32, comm);
...
gatherDenseMatricesByColumns(const CDenseMatrix& matrix, MPI_Comm comm) -> CDenseMatrix
{
    if constexpr (ENABLE_MPI)
    {
        ...
        if (rank == mpi_master) col_counts = std::vector<int>(nodes);

        auto merror = MPI_Gather(&ncols, 1, MPI_INT, col_counts.data(), 1, MPI_INT, mpi_master_int32, comm);
