...
auto
compPrimitiveNuclearPotentialGeom020SP_0_X(      TDoubleArray& buffer_xx,
                                           ...
                                           const int64_t       ket_dim) -> void
{
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += qpol_xx * faa_x * b2_vals[i] * 2.0;

        fints_xx[i] += (faa_xx * b2_vals[i] + faa * b1_vals[i]) * rpb_x;

        fints_xx[i] += (faa_xx * b3_vals[i] + faa * b2_vals[i]) * (-rpc_x);

        fints_xy[i] += qpol_xy * faa_y * b2_vals[i];

        fints_xy[i] += faa_xy * b2_vals[i] * rpb_x;

        fints_xy[i] += faa_xy * b3_vals[i] * (-rpc_x);

        fints_xz[i] += qpol_xz * faa_z * b2_vals[i];

        fints_xz[i] += faa_xz * b2_vals[i] * rpb_x;

        fints_xz[i] += faa_xz * b3_vals[i] * (-rpc_x);

        fints_yy[i] += (faa_yy * b2_vals[i] + faa * b1_vals[i]) * rpb_x;

        fints_yy[i] += (faa_yy * b3_vals[i] + faa * b2_vals[i]) * (-rpc_x);

        fints_yz[i] += faa_yz * b2_vals[i] * rpb_x;

        fints_yz[i] += faa_yz * b3_vals[i] * (-rpc_x);

        fints_zz[i] += (faa_zz * b2_vals[i] + faa * b1_vals[i]) * rpb_x;

        fints_zz[i] += (faa_zz * b3_vals[i] + faa * b2_vals[i]) * (-rpc_x);
