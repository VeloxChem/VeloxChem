#include "PrimitiveNuclearPotentialGeom020DS_ZZ_0.hpp"
                                            ...
                                            const int64_t       ket_dim) -> void
{
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += (faa_xx * b2_vals[i] + faa * b1_vals[i]) * ((1.0 / 2.0) * fe_0 + rpa_z * rpa_z);

        fints_xx[i] += (faa_xx * b3_vals[i] + faa * b2_vals[i]) * (-(1.0 / 2.0) * fe_0 - 2.0 * rpa_z * rpc_z);

        fints_xx[i] += (faa_xx * b4_vals[i] + faa * b3_vals[i]) * rpc_z * rpc_z;

        fints_xy[i] += faa_xy * b2_vals[i] * ((1.0 / 2.0) * fe_0 + rpa_z * rpa_z);

        fints_xy[i] += faa_xy * b3_vals[i] * (-(1.0 / 2.0) * fe_0 - 2.0 * rpa_z * rpc_z);

        fints_xy[i] += faa_xy * b4_vals[i] * rpc_z * rpc_z;

        fints_xz[i] += qpol_xz * faa_x * b2_vals[i] * 2.0 * rpa_z;

        fints_xz[i] += qpol_xz * faa_x * b3_vals[i] * (-2.0 * rpc_z);

        fints_xz[i] += faa_xz * b2_vals[i] * ((1.0 / 2.0) * fe_0 + rpa_z * rpa_z);

        fints_xz[i] += faa_xz * b3_vals[i] * (-(1.0 / 2.0) * fe_0 - 2.0 * rpa_z * rpc_z);

        fints_xz[i] += faa_xz * b4_vals[i] * rpc_z * rpc_z;

        fints_yy[i] += (faa_yy * b2_vals[i] + faa * b1_vals[i]) * ((1.0 / 2.0) * fe_0 + rpa_z * rpa_z);

        fints_yy[i] += (faa_yy * b3_vals[i] + faa * b2_vals[i]) * (-(1.0 / 2.0) * fe_0 - 2.0 * rpa_z * rpc_z);

        fints_yy[i] += (faa_yy * b4_vals[i] + faa * b3_vals[i]) * rpc_z * rpc_z;

        fints_yz[i] += qpol_yz * faa_y * b2_vals[i] * 2.0 * rpa_z;

        fints_yz[i] += qpol_yz * faa_y * b3_vals[i] * (-2.0 * rpc_z);

        fints_yz[i] += faa_yz * b2_vals[i] * ((1.0 / 2.0) * fe_0 + rpa_z * rpa_z);

        fints_yz[i] += faa_yz * b3_vals[i] * (-(1.0 / 2.0) * fe_0 - 2.0 * rpa_z * rpc_z);

        fints_yz[i] += faa_yz * b4_vals[i] * rpc_z * rpc_z;

        fints_zz[i] += qpol_zz * fss * b2_vals[i] * 2.0;

        fints_zz[i] += qpol_zz * faa_z * b2_vals[i] * 4.0 * rpa_z;

        fints_zz[i] += qpol_zz * faa_z * b3_vals[i] * (-4.0 * rpc_z);

        fints_zz[i] += (faa_zz * b2_vals[i] + faa * b1_vals[i]) * ((1.0 / 2.0) * fe_0 + rpa_z * rpa_z);

        fints_zz[i] += (faa_zz * b3_vals[i] + faa * b2_vals[i]) * (-(1.0 / 2.0) * fe_0 - 2.0 * rpa_z * rpc_z);
