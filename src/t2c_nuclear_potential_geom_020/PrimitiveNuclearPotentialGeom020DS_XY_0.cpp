#include "PrimitiveNuclearPotentialGeom020DS_XY_0.hpp"
                                            ...
                                            const int64_t       ket_dim) -> void
{
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += (faa_xx * b2_vals[i] + faa * b1_vals[i]) * rpa_y * rpa_x;

        fints_xx[i] += (faa_xx * b3_vals[i] + faa * b2_vals[i]) * (-rpa_y * rpc_x - rpa_x * rpc_y);
        ...
        fints_xy[i] += faa_xy * b2_vals[i] * rpa_y * rpa_x;

        fints_xy[i] += faa_xy * b3_vals[i] * (-rpa_y * rpc_x - rpa_x * rpc_y);
        ...
        fints_xz[i] += faa_xz * b2_vals[i] * rpa_y * rpa_x;

        fints_xz[i] += faa_xz * b3_vals[i] * (-rpa_y * rpc_x - rpa_x * rpc_y);
        ...
        fints_yy[i] += (faa_yy * b2_vals[i] + faa * b1_vals[i]) * rpa_y * rpa_x;

        fints_yy[i] += (faa_yy * b3_vals[i] + faa * b2_vals[i]) * (-rpa_y * rpc_x - rpa_x * rpc_y);
        ...
        fints_yz[i] += faa_yz * b2_vals[i] * rpa_y * rpa_x;

        fints_yz[i] += faa_yz * b3_vals[i] * (-rpa_y * rpc_x - rpa_x * rpc_y);

        fints_yz[i] += faa_yz * b4_vals[i] * rpc_y * rpc_x;

        fints_zz[i] += (faa_zz * b2_vals[i] + faa * b1_vals[i]) * rpa_y * rpa_x;

        fints_zz[i] += (faa_zz * b3_vals[i] + faa * b2_vals[i]) * (-rpa_y * rpc_x - rpa_x * rpc_y);
