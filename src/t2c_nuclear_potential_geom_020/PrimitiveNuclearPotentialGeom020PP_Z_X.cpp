...
#pragma omp simd aligned(fints_xx, fints_xy, fints_xz, fints_yy, fints_yz, fints_zz, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += (faa_xx * b2_vals[i] + faa * b1_vals[i]) * rpa_z * rpb_x;

        fints_xx[i] += (faa_xx * b3_vals[i] + faa * b2_vals[i]) * (-rpa_z * rpc_x - rpb_x * rpc_z);
        ...
        fints_xy[i] += faa_xy * b2_vals[i] * rpa_z * rpb_x;

        fints_xy[i] += faa_xy * b3_vals[i] * (-rpa_z * rpc_x - rpb_x * rpc_z);
        ...
        fints_xz[i] += faa_xz * b2_vals[i] * rpa_z * rpb_x;

        fints_xz[i] += faa_xz * b3_vals[i] * (-rpa_z * rpc_x - rpb_x * rpc_z);

        fints_xz[i] += faa_xz * b4_vals[i] * rpc_z * rpc_x;

        fints_yy[i] += (faa_yy * b2_vals[i] + faa * b1_vals[i]) * rpa_z * rpb_x;

        fints_yy[i] += (faa_yy * b3_vals[i] + faa * b2_vals[i]) * (-rpa_z * rpc_x - rpb_x * rpc_z);
        ...
        fints_yz[i] += faa_yz * b2_vals[i] * rpa_z * rpb_x;

        fints_yz[i] += faa_yz * b3_vals[i] * (-rpa_z * rpc_x - rpb_x * rpc_z);
        ...
        fints_zz[i] += (faa_zz * b2_vals[i] + faa * b1_vals[i]) * rpa_z * rpb_x;

        fints_zz[i] += (faa_zz * b3_vals[i] + faa * b2_vals[i]) * (-rpa_z * rpc_x - rpb_x * rpc_z);
