#include "PrimitiveNuclearPotentialGeom020PD_X_XY.hpp"
...
auto
compPrimitiveNuclearPotentialGeom020PD_X_XY(      TDoubleArray& buffer_xx,
                                            ...
                                            const int64_t       ket_dim) -> void
{
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += qpol_xx * faa_x * b2_vals[i] * (2.0 * rpa_x * rpb_y + 2.0 * rpb_y * rpb_x);

        fints_xx[i] += qpol_xx * faa_x * b3_vals[i] * (-2.0 * rpa_x * rpc_y - 4.0 * rpb_y * rpc_x - 2.0 * rpb_x * rpc_y);

        fints_xx[i] += qpol_xx * faa_x * b4_vals[i] * 4.0 * rpc_y * rpc_x;

        fints_xx[i] += (faa_xx * b2_vals[i] + faa * b1_vals[i]) * ((1.0 / 2.0) * fe_0 * rpb_y + rpa_x * rpb_y * rpb_x);

        fints_xx[i] += (faa_xx * b3_vals[i] + faa * b2_vals[i]) * (-(1.0 / 2.0) * fe_0 * rpb_y - (1.0 / 2.0) * fe_0 * rpc_y - rpa_x * rpb_y * rpc_x - rpa_x * rpb_x * rpc_y - rpb_y * rpb_x * rpc_x);

        fints_xx[i] += (faa_xx * b4_vals[i] + faa * b3_vals[i]) * ((1.0 / 2.0) * fe_0 * rpc_y + rpa_x * rpc_y * rpc_x + rpb_y * rpc_x * rpc_x + rpb_x * rpc_y * rpc_x);

        fints_xx[i] += (faa_xx * b5_vals[i] + faa * b4_vals[i]) * (-rpc_y * rpc_x * rpc_x);

        fints_xy[i] += qpol_xy * fss * b2_vals[i] * (rpa_x + rpb_x);

        fints_xy[i] += qpol_xy * fss * b3_vals[i] * (-2.0 * rpc_x);

        fints_xy[i] += qpol_xy * faa_y * b2_vals[i] * (rpa_x * rpb_y + rpb_y * rpb_x);

        fints_xy[i] += qpol_xy * faa_y * b3_vals[i] * (-rpa_x * rpc_y - 2.0 * rpb_y * rpc_x - rpb_x * rpc_y);

        fints_xy[i] += qpol_xy * faa_y * b4_vals[i] * 2.0 * rpc_y * rpc_x;

        fints_xy[i] += qpol_xy * faa_x * b2_vals[i] * ((1.0 / 2.0) * fe_0 + rpa_x * rpb_x);

        fints_xy[i] += qpol_xy * faa_x * b3_vals[i] * (-(1.0 / 2.0) * fe_0 - rpa_x * rpc_x - rpb_x * rpc_x);

        fints_xy[i] += qpol_xy * faa_x * b4_vals[i] * rpc_x * rpc_x;

        fints_xy[i] += faa_xy * b2_vals[i] * ((1.0 / 2.0) * fe_0 * rpb_y + rpa_x * rpb_y * rpb_x);

        fints_xy[i] += faa_xy * b3_vals[i] * (-(1.0 / 2.0) * fe_0 * rpb_y - (1.0 / 2.0) * fe_0 * rpc_y - rpa_x * rpb_y * rpc_x - rpa_x * rpb_x * rpc_y - rpb_y * rpb_x * rpc_x);

        fints_xy[i] += faa_xy * b4_vals[i] * ((1.0 / 2.0) * fe_0 * rpc_y + rpa_x * rpc_y * rpc_x + rpb_y * rpc_x * rpc_x + rpb_x * rpc_y * rpc_x);

        fints_xy[i] += faa_xy * b5_vals[i] * (-rpc_y * rpc_x * rpc_x);

        fints_xz[i] += qpol_xz * faa_z * b2_vals[i] * (rpa_x * rpb_y + rpb_y * rpb_x);

        fints_xz[i] += qpol_xz * faa_z * b3_vals[i] * (-rpa_x * rpc_y - 2.0 * rpb_y * rpc_x - rpb_x * rpc_y);

        fints_xz[i] += qpol_xz * faa_z * b4_vals[i] * 2.0 * rpc_y * rpc_x;

        fints_xz[i] += faa_xz * b2_vals[i] * ((1.0 / 2.0) * fe_0 * rpb_y + rpa_x * rpb_y * rpb_x);

        fints_xz[i] += faa_xz * b3_vals[i] * (-(1.0 / 2.0) * fe_0 * rpb_y - (1.0 / 2.0) * fe_0 * rpc_y - rpa_x * rpb_y * rpc_x - rpa_x * rpb_x * rpc_y - rpb_y * rpb_x * rpc_x);

        fints_xz[i] += faa_xz * b4_vals[i] * ((1.0 / 2.0) * fe_0 * rpc_y + rpa_x * rpc_y * rpc_x + rpb_y * rpc_x * rpc_x + rpb_x * rpc_y * rpc_x);

        fints_xz[i] += faa_xz * b5_vals[i] * (-rpc_y * rpc_x * rpc_x);

        fints_yy[i] += qpol_yy * faa_y * b2_vals[i] * (fe_0 + 2.0 * rpa_x * rpb_x);

        fints_yy[i] += qpol_yy * faa_y * b3_vals[i] * (-fe_0 - 2.0 * rpa_x * rpc_x - 2.0 * rpb_x * rpc_x);

        fints_yy[i] += qpol_yy * faa_y * b4_vals[i] * 2.0 * rpc_x * rpc_x;

        fints_yy[i] += (faa_yy * b2_vals[i] + faa * b1_vals[i]) * ((1.0 / 2.0) * fe_0 * rpb_y + rpa_x * rpb_y * rpb_x);

        fints_yy[i] += (faa_yy * b3_vals[i] + faa * b2_vals[i]) * (-(1.0 / 2.0) * fe_0 * rpb_y - (1.0 / 2.0) * fe_0 * rpc_y - rpa_x * rpb_y * rpc_x - rpa_x * rpb_x * rpc_y - rpb_y * rpb_x * rpc_x);

        fints_yy[i] += (faa_yy * b4_vals[i] + faa * b3_vals[i]) * ((1.0 / 2.0) * fe_0 * rpc_y + rpa_x * rpc_y * rpc_x + rpb_y * rpc_x * rpc_x + rpb_x * rpc_y * rpc_x);

        fints_yy[i] += (faa_yy * b5_vals[i] + faa * b4_vals[i]) * (-rpc_y * rpc_x * rpc_x);

        fints_yz[i] += qpol_yz * faa_z * b2_vals[i] * ((1.0 / 2.0) * fe_0 + rpa_x * rpb_x);

        fints_yz[i] += qpol_yz * faa_z * b3_vals[i] * (-(1.0 / 2.0) * fe_0 - rpa_x * rpc_x - rpb_x * rpc_x);

        fints_yz[i] += qpol_yz * faa_z * b4_vals[i] * rpc_x * rpc_x;

        fints_yz[i] += faa_yz * b2_vals[i] * ((1.0 / 2.0) * fe_0 * rpb_y + rpa_x * rpb_y * rpb_x);

        fints_yz[i] += faa_yz * b3_vals[i] * (-(1.0 / 2.0) * fe_0 * rpb_y - (1.0 / 2.0) * fe_0 * rpc_y - rpa_x * rpb_y * rpc_x - rpa_x * rpb_x * rpc_y - rpb_y * rpb_x * rpc_x);

        fints_yz[i] += faa_yz * b4_vals[i] * ((1.0 / 2.0) * fe_0 * rpc_y + rpa_x * rpc_y * rpc_x + rpb_y * rpc_x * rpc_x + rpb_x * rpc_y * rpc_x);

        fints_yz[i] += faa_yz * b5_vals[i] * (-rpc_y * rpc_x * rpc_x);

        fints_zz[i] += (faa_zz * b2_vals[i] + faa * b1_vals[i]) * ((1.0 / 2.0) * fe_0 * rpb_y + rpa_x * rpb_y * rpb_x);

        fints_zz[i] += (faa_zz * b3_vals[i] + faa * b2_vals[i]) * (-(1.0 / 2.0) * fe_0 * rpb_y - (1.0 / 2.0) * fe_0 * rpc_y - rpa_x * rpb_y * rpc_x - rpa_x * rpb_x * rpc_y - rpb_y * rpb_x * rpc_x);

        fints_zz[i] += (faa_zz * b4_vals[i] + faa * b3_vals[i]) * ((1.0 / 2.0) * fe_0 * rpc_y + rpa_x * rpc_y * rpc_x + rpb_y * rpc_x * rpc_x + rpb_x * rpc_y * rpc_x);
