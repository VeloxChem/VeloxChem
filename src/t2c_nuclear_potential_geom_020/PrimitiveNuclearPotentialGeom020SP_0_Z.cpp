...
auto
compPrimitiveNuclearPotentialGeom020SP_0_Z(TDoubleArray&       buffer_xx,
...
#pragma omp simd aligned(fints_xx, fints_xy, fints_xz, fints_yy, fints_yz, fints_zz, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += (faa_xx * b2_vals[i] + faa * b1_vals[i]) * rpb_z;

        fints_xx[i] += (faa_xx * b3_vals[i] + faa * b2_vals[i]) * (-rpc_z);

        fints_xy[i] += faa_xy * b2_vals[i] * rpb_z;

        fints_xy[i] += faa_xy * b3_vals[i] * (-rpc_z);

        fints_xz[i] += qpol_xz * faa_x * b2_vals[i];

        fints_xz[i] += faa_xz * b2_vals[i] * rpb_z;

        fints_xz[i] += faa_xz * b3_vals[i] * (-rpc_z);

        fints_yy[i] += (faa_yy * b2_vals[i] + faa * b1_vals[i]) * rpb_z;

        fints_yy[i] += (faa_yy * b3_vals[i] + faa * b2_vals[i]) * (-rpc_z);

        fints_yz[i] += qpol_yz * faa_y * b2_vals[i];

        fints_yz[i] += faa_yz * b2_vals[i] * rpb_z;

        fints_yz[i] += faa_yz * b3_vals[i] * (-rpc_z);

        fints_zz[i] += qpol_zz * faa_z * b2_vals[i] * 2.0;

        fints_zz[i] += (faa_zz * b2_vals[i] + faa * b1_vals[i]) * rpb_z;

        fints_zz[i] += (faa_zz * b3_vals[i] + faa * b2_vals[i]) * (-rpc_z);
    }
}
