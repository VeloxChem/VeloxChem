#include "PrimitiveNuclearPotentialGeom020PP_X_Z.hpp"
...
auto
compPrimitiveNuclearPotentialGeom020PP_X_Z(      TDoubleArray& buffer_xx,
                                           ...
                                           const int64_t       ket_dim) -> void
{
    ...
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += qpol_xx * faa_x * b2_vals[i] * 2.0 * rpb_z;

        fints_xx[i] += qpol_xx * faa_x * b3_vals[i] * (-2.0 * rpc_z);

        fints_xx[i] += (faa_xx * b2_vals[i] + faa * b1_vals[i]) * rpa_x * rpb_z;

        fints_xx[i] += (faa_xx * b3_vals[i] + faa * b2_vals[i]) * (-rpa_x * rpc_z - rpb_z * rpc_x);

        fints_xx[i] += (faa_xx * b4_vals[i] + faa * b3_vals[i]) * rpc_z * rpc_x;

        fints_xy[i] += qpol_xy * faa_y * b2_vals[i] * rpb_z;

        fints_xy[i] += qpol_xy * faa_y * b3_vals[i] * (-rpc_z);

        fints_xy[i] += faa_xy * b2_vals[i] * rpa_x * rpb_z;

        fints_xy[i] += faa_xy * b3_vals[i] * (-rpa_x * rpc_z - rpb_z * rpc_x);

        fints_xy[i] += faa_xy * b4_vals[i] * rpc_z * rpc_x;

        fints_xz[i] += qpol_xz * fss * b2_vals[i];

        fints_xz[i] += qpol_xz * faa_z * b2_vals[i] * rpb_z;
        ...
        fints_xz[i] += faa_xz * b2_vals[i] * rpa_x * rpb_z;

        fints_xz[i] += faa_xz * b3_vals[i] * (-rpa_x * rpc_z - rpb_z * rpc_x);

        fints_xz[i] += faa_xz * b4_vals[i] * rpc_z * rpc_x;

        fints_yy[i] += (faa_yy * b2_vals[i] + faa * b1_vals[i]) * rpa_x * rpb_z;

        fints_yy[i] += (faa_yy * b3_vals[i] + faa * b2_vals[i]) * (-rpa_x * rpc_z - rpb_z * rpc_x);
        ...
        fints_yz[i] += faa_yz * b2_vals[i] * rpa_x * rpb_z;

        fints_yz[i] += faa_yz * b3_vals[i] * (-rpa_x * rpc_z - rpb_z * rpc_x);
        ...
        fints_zz[i] += (faa_zz * b2_vals[i] + faa * b1_vals[i]) * rpa_x * rpb_z;

        fints_zz[i] += (faa_zz * b3_vals[i] + faa * b2_vals[i]) * (-rpa_x * rpc_z - rpb_z * rpc_x);
