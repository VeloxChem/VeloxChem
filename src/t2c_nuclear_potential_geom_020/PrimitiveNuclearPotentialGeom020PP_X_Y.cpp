#include "PrimitiveNuclearPotentialGeom020PP_X_Y.hpp"
...
auto
compPrimitiveNuclearPotentialGeom020PP_X_Y(TDoubleArray&       buffer_xx,
...
#pragma omp simd aligned(fints_xx, fints_xy, fints_xz, fints_yy, fints_yz, fints_zz, ket_fe, ket_fn, ket_rx, ket_ry, ket_rz : 64)
    for (int64_t i = 0; i < ket_dim; i++)
    {
        ...
        fints_xx[i] += qpol_xx * faa_x * b2_vals[i] * 2.0 * rpb_y;

        fints_xx[i] += qpol_xx * faa_x * b3_vals[i] * (-2.0 * rpc_y);

        fints_xx[i] += (faa_xx * b2_vals[i] + faa * b1_vals[i]) * rpa_x * rpb_y;

        fints_xx[i] += (faa_xx * b3_vals[i] + faa * b2_vals[i]) * (-rpa_x * rpc_y - rpb_y * rpc_x);

        fints_xx[i] += (faa_xx * b4_vals[i] + faa * b3_vals[i]) * rpc_y * rpc_x;

        fints_xy[i] += qpol_xy * fss * b2_vals[i];

        fints_xy[i] += qpol_xy * faa_y * b2_vals[i] * rpb_y;

        fints_xy[i] += qpol_xy * faa_y * b3_vals[i] * (-rpc_y);

        fints_xy[i] += qpol_xy * faa_x * b2_vals[i] * rpa_x;

        fints_xy[i] += qpol_xy * faa_x * b3_vals[i] * (-rpc_x);

        fints_xy[i] += faa_xy * b2_vals[i] * rpa_x * rpb_y;

        fints_xy[i] += faa_xy * b3_vals[i] * (-rpa_x * rpc_y - rpb_y * rpc_x);

        fints_xy[i] += faa_xy * b4_vals[i] * rpc_y * rpc_x;

        fints_xz[i] += qpol_xz * faa_z * b2_vals[i] * rpb_y;

        fints_xz[i] += qpol_xz * faa_z * b3_vals[i] * (-rpc_y);

        fints_xz[i] += faa_xz * b2_vals[i] * rpa_x * rpb_y;

        fints_xz[i] += faa_xz * b3_vals[i] * (-rpa_x * rpc_y - rpb_y * rpc_x);

        fints_xz[i] += faa_xz * b4_vals[i] * rpc_y * rpc_x;

        fints_yy[i] += qpol_yy * faa_y * b2_vals[i] * 2.0 * rpa_x;

        fints_yy[i] += qpol_yy * faa_y * b3_vals[i] * (-2.0 * rpc_x);

        fints_yy[i] += (faa_yy * b2_vals[i] + faa * b1_vals[i]) * rpa_x * rpb_y;

        fints_yy[i] += (faa_yy * b3_vals[i] + faa * b2_vals[i]) * (-rpa_x * rpc_y - rpb_y * rpc_x);

        fints_yy[i] += (faa_yy * b4_vals[i] + faa * b3_vals[i]) * rpc_y * rpc_x;

        fints_yz[i] += qpol_yz * faa_z * b2_vals[i] * rpa_x;

        fints_yz[i] += qpol_yz * faa_z * b3_vals[i] * (-rpc_x);

        fints_yz[i] += faa_yz * b2_vals[i] * rpa_x * rpb_y;

        fints_yz[i] += faa_yz * b3_vals[i] * (-rpa_x * rpc_y - rpb_y * rpc_x);

        fints_yz[i] += faa_yz * b4_vals[i] * rpc_y * rpc_x;

        fints_zz[i] += (faa_zz * b2_vals[i] + faa * b1_vals[i]) * rpa_x * rpb_y;

        fints_zz[i] += (faa_zz * b3_vals[i] + faa * b2_vals[i]) * (-rpa_x * rpc_y - rpb_y * rpc_x);
