...
- `VbComputeOptions.orbital_relaxation_penalty` now exposes an optional quadratic penalty on organic pi VBSCF breathing amplitudes. This is a trust-region-style stabilization diagnostic for the symmetry-preserving benzene checkpoint; it does not change the unpenalized default path. The first penalized equivalent-center scan with a +/-0.35 box and penalties 0.00, 0.01, 0.05, and 0.10 still hit the amplitude bound in every row, with E_VB values -228.9607492620, -228.4456091770, -228.4910859018, and -228.4841614539 a.u.
- The wider benzene penalty/trust-box scan produced finite non-bound checkpoints. An earlier numerical gate selected bound 0.20, penalty 2.00, E_VB -228.1842986262 a.u., lowering 2.825567e-01 a.u., objective -228.0081285726 a.u., penalty energy 1.762e-01 a.u., equivalent-center amplitude 0.121165, captured-subspace weight 1.866e-03, and E_res 6.809e-01 a.u. A later notebook rerun selected a different finite non-bound row, so the acceptance cell now validates the current selected row by diagnostics instead of hard-coding the earlier bound/penalty pair. This remains a diagnostic constrained reference rather than an unpenalized production benzene VBSCF energy.
- A first determinant-space benzene BOVB prototype was routed through `mode="bovb"` for 6e/6pi singlet benzene-like active spaces. It reused the determinant-CI Hamiltonian and the equivalent-center center-local breathing optimizer with the same amplitude-bound/penalty controls as the stabilized VBSCF gate, and reported `organic_pi_bovb_model="benzene-determinant-ci-equivalent-center-breathing-prototype"`. The notebook BOVB prototype gate passed with the current accepted checkpoint controls bound 0.35 and penalty 2.00: E_initial -228.0252665302 a.u., E_BOVB -228.1406822768 a.u., lowering 1.154157e-01 a.u., penalty energy 7.159706e-02 a.u., equivalent-center amplitude -0.07724262, no amplitude-bound hit, no fixed-limit fallback, captured-subspace weight 1.207965e-04, E_res 6.085460e-01 a.u., 400 determinants, and 15 templates. This checkpoint is now superseded as the active BOVB route, but remains useful as the last common-orbital benchmark.
- The next benzene BOVB implementation step has been started in source: benzene-like 6e/6pi singlet `mode="bovb"` now routes to a template-specific determinant-CSF BOVB builder with nonorthogonal template orbitals. It builds one active-orbital set per graph-generated Kekule/Dewar template, applies a center-local breathing amplitude to the centers participating in that template's bond pairs, constructs the compact template Hamiltonian from determinant expansions with template-specific orbital overlaps, and reports `organic_pi_bovb_model="benzene-determinant-ci-template-specific-breathing"`, `organic_pi_bovb_common_orbital_prototype=False`, and `structure_specific_orbitals=True`. The notebook BOVB cell has been updated to validate this true structure/template-specific route after rebuild; numerical results are pending the user's compile/run cycle. Radical/anion determinant-space BOVB remains on the explicit fixed-orbital zero-amplitude limit.
